...
imported by ``tests/conftest.py`` to populate ``os.environ`` from
``env/.env`` so the ``AEAT_LIVE_TESTS_ENABLED`` gate matches the value
the rest of the project's pydantic-settings stack reads.
