...
``aeat app ledger add``
~~~~~~~~~~~~~~~~~~~~~~~
Add one manual ledger transaction in the active profile bucket
...
``--amount``
   *Option, required.* Signed transaction amount in the bucket currency
...
``--business-pct``
   *Option, optional.* Business-use share from 0 to 1 (only when classification is MIXED)

``--category-id``
   *Option, optional.* Deductible-expense category id (expense transactions only; income needs no category)

``--taxable-base``
   *Option, optional.* VAT-exclusive base amount
...
``--usage-ratio-id``
   *Option, optional.* Usage-ratio id for proportional business use
...
``--idempotency-key``
   *Option, optional.* Caller-supplied idempotency key recorded with the row
...
``--business-pct``
   *Option, required.* Business-use share from 0 to 1
...
``--prorrata-reference``
   *Option, optional.* IVA prorrata reference for VAT workflows
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~
Archive one ledger transaction in the active profile bucket
...
~~~~~~~~~~~~~~~~~~~~~~~~~~
Attach secure evidence objects to one ledger transaction
...
``--purchase-invoice-evidence-id``
   *Option, optional.* Purchase invoice evidence id supporting the transaction
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
List the spending-category ids accepted by --category-id, grouped by family
...
~~~~~~~~~~~~~~~~~~~~~~~~~
Probe ledger transactions in the addressed bucket (defaults to the active profile bucket) and report anomaly rows aggregated across every period a transaction touches. Local-only; never contacts AEAT.
...
``--bucket-id``
   *Option, optional.* Bucket id to probe (defaults to the active profile bucket).
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Classify one ledger transaction in the active profile bucket
...
``--from-csv``
   *Option, optional.* Path to a CSV file with columns transaction_id, classification[, category_id].

``--business-pct``
   *Option, optional.* Business-use share from 0 to 1; required with --classification MIXED and rejected for any other classification
...
``--irpf-category``
   *Option, optional.* IRPF category for modelo calculations

``--iva-category``
   *Option, optional.* Explicit IVA category override for this transaction (e.g. intra_community_supply, export_third_country_zero_rated, domestic_not_subject)

``--counterparty-eu-member-state``
   *Option, optional.* EU member state of the counterparty (ISO 3166-1 alpha-2 lowercase, e.g. de); required with --iva-category intra_community_supply, prohibited with export_third_country_zero_rated
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.LedgerClassifyResult``.

``aeat app ledger collectible-invoice add``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Register a new collectible invoice

**Command path:** ``aeat app ledger collectible-invoice add``

**Registry key:** ``ledger.collectible_invoice.add``
...
``--invoice-date``
   *Option, required.* Invoice issue date (YYYY-MM-DD)
...
``--eu-vat-id``
   *Option, optional.* Counterparty EU VAT-ID (e.g. DE345678901) for intracom operations
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.CollectibleInvoiceAddResult``.

``aeat app ledger collectible-invoice list``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
List registered collectible invoices

**Command path:** ``aeat app ledger collectible-invoice list``

**Registry key:** ``ledger.collectible_invoice.list``

**Output schema**

This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.CollectibleInvoiceListResult``.

``aeat app ledger collectible-invoice remove``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Delete a registered collectible invoice

**Command path:** ``aeat app ledger collectible-invoice remove``

**Registry key:** ``ledger.collectible_invoice.remove``

**Parameters**

``invoice_id``
   *Argument, required.* Invoice id (or unambiguous prefix)

``--yes``
   *Option, optional.* Confirm deleting the collectible invoice

**Output schema**

This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.CollectibleInvoiceRemoveResult``.

``aeat app ledger collectible-invoice update``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Update collectible invoice details

**Command path:** ``aeat app ledger collectible-invoice update``

**Registry key:** ``ledger.collectible_invoice.update``
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.CollectibleInvoiceUpdateResult``.

``aeat app ledger collectible-invoice view``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Show collectible invoice details

**Command path:** ``aeat app ledger collectible-invoice view``

**Registry key:** ``ledger.collectible_invoice.view``
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.CollectibleInvoiceViewResult``.
...
``source_path``
   *Argument, required.* Path to the invoice PDF or image
...
``--invoice-date``
   *Option, optional.* Invoice issue date (YYYY-MM-DD)
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.EvidenceAddResult``.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.EvidenceListResult``.
...
``evidence_id``
   *Argument, required.* Evidence id (or unambiguous prefix)
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.EvidenceRemoveResult``.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.EvidenceUpdateResult``.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.EvidenceViewResult``.

``aeat app ledger export``
~~~~~~~~~~~~~~~~~~~~~~~~~~
Export canonical ledger rows from the active profile bucket
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.LedgerExportResult``.
...
``transaction_id``
   *Argument, required.* Ledger transaction id (or unambiguous hex prefix) to inspect
...
``path``
   *Argument, required.* Path to the bank statement file or directory
...
``--period``
   *Option, optional.* Fiscal period for the import

**Output schema**

This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.LedgerImportResult``.
...
``actividad_id``
   *Argument, required.* Economic activity identifier
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.InventoryCreateResult``.

``aeat app ledger inventory list``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
List inventory ledgers in the active bucket
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.InventoryListResult``.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.InventoryMovementAddResult``.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.InventoryValuationPreviewResult``.
...
``--id``
   *Option, required.* Ledger transaction id (SHA-256 or unambiguous prefix).

``--invoice-id``
   *Option, optional.* Invoice id to bind bidirectionally to the transaction.

``--evidence-id``
   *Option, optional.* Purchase-invoice evidence record id to attach to the transaction.

``--by``
   *Option, optional.* Operator label recorded on bucket events.
...
~~~~~~~~~~~~~~~~~~~~~~~~
List ledger transactions in the active profile bucket
...
``aeat app ledger payable-invoice add``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Register a new payable invoice

**Command path:** ``aeat app ledger payable-invoice add``

**Registry key:** ``ledger.payable_invoice.add``
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.PayableInvoiceAddResult``.

``aeat app ledger payable-invoice list``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
List registered payable invoices

**Command path:** ``aeat app ledger payable-invoice list``

**Registry key:** ``ledger.payable_invoice.list``

**Output schema**

This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.PayableInvoiceListResult``.

``aeat app ledger payable-invoice remove``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Delete a registered payable invoice

**Command path:** ``aeat app ledger payable-invoice remove``

**Registry key:** ``ledger.payable_invoice.remove``
...
``--yes``
   *Option, optional.* Confirm deleting the payable invoice

**Output schema**

This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.PayableInvoiceRemoveResult``.

``aeat app ledger payable-invoice update``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Update payable invoice details

**Command path:** ``aeat app ledger payable-invoice update``

**Registry key:** ``ledger.payable_invoice.update``
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.PayableInvoiceUpdateResult``.

``aeat app ledger payable-invoice view``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Show payable invoice details

**Command path:** ``aeat app ledger payable-invoice view``

**Registry key:** ``ledger.payable_invoice.view``
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.PayableInvoiceViewResult``.

``aeat app ledger preflight``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Report missing ledger facts (category, taxable base, IVA amount/rate, currency, proportionality reference) for the active bucket's transactions in a given period. Local-only; never contacts AEAT.
...
``--period``
   *Option, required.* Canonical period (e.g. 2026Q1, 2026-03, 2026).
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.RatiosEligibleResult``.

``aeat app ledger ratios list``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
List category ratios in the active bucket
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.RatiosListResult``.
...
``category``
   *Argument, required.* Spending category id (e.g. USAGE_RATIO_VEHICLE)

``ratio``
   *Argument, required.* Applicable ratio between 0 and 1

**Output schema**

This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.RatiosSetResult``.
...
``category``
   *Argument, required.* Category id whose ratio you want to clear

**Output schema**

This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.RatiosUnsetResult``.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.RatiosValidateResult``.

``aeat app ledger remove``
~~~~~~~~~~~~~~~~~~~~~~~~~~
Remove one ledger transaction from the active profile bucket
...
~~~~~~~~~~~~~~~~~~~~~~~~~
Reset the active profile bucket ledger catalogue
...
``--filter``
   *Option, optional.* Filter rows by status, period, issue, or import
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.RuleAddResult``.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.RuleApplyResult``.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._ledger_payloads.RuleListResult``.
...
``--child-amount``
   *Option, optional.* One child amount (signed Decimal in the parent's currency). Repeat once per child; the count must match --child-description.
...
~~~~~~~~~~~~~~~~~~~~~~~~~
Stash one ledger transaction in the active profile bucket
...
~~~~~~~~~~~~~~~~~~~~~~~~~~
Summarize ledger state and calculation readiness for the active profile bucket
...
``--period``
   *Option, optional.* Fiscal period to preflight (YYYY-MM or modelo period)
...
``transaction_id``
   *Argument, required.* Ledger transaction id to inspect
...
``--amount``
   *Option, optional.* New signed amount (full replace)
...
``--taxable-base``
   *Option, optional.* New VAT-exclusive base amount
...
~~~~~~~~~~~~~~~~~~~~~~~~
View one ledger transaction in the active profile bucket
...
``transaction_id``
   *Argument, required.* Ledger transaction id (or unambiguous hex prefix)
...
``snapshot_id``
   *Argument, required.* Snapshot id (or unambiguous prefix)
...
``aeat app live expedientes capture``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Capture AEAT declaration register data and save a snapshot

**Command path:** ``aeat app live expedientes capture``

**Registry key:** ``app.live.expedientes.capture``
...
``aeat app live filed capture``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Capture filed declaration data from AEAT

**Command path:** ``aeat app live filed capture``

**Registry key:** ``app.live.filed.capture``
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Capture filed observations required by a target filing

**Command path:** ``aeat app live filed capture-sources``

**Registry key:** ``app.live.filed.capture_sources``
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Live-capture filed Modelo 303 history and persist secure IVA compensation state. No AEAT filing or wallet form choices are submitted.

**Command path:** ``aeat app live iva-wallet capture-history``

**Registry key:** ``app.live.iva_wallet.capture_history``
...
``aeat app live iva-wallet capture-remote-state``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Live-capture filed Modelo 303 history and attempt the AEAT IVA wallet/cartera read as one typed read-only acquisition.

**Command path:** ``aeat app live iva-wallet capture-remote-state``

**Registry key:** ``app.live.iva_wallet.capture_remote_state``
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._app_live_payloads.IvaWalletCaptureRemoteStateResult``.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Capture DEHú notifications and save a snapshot

**Command path:** ``aeat app live notifications capture``

**Registry key:** ``app.live.notifications.capture``
...
``portal_id``
   *Argument, required.* Portal identifier
...
``nif``
   *Argument, required.* NIF-IVA value (e.g. ESB12345678)
...
``nif``
   *Argument, required.* Spanish NIF/NIE to check
...
``observation_id``
   *Argument, required.* Observation id (or unambiguous prefix)
...
``--period``
   *Option, required.* Aggregation period passed to the backend service.

``--retencion-observation``
   *Option, optional.* JSON RetencionObservation object. The source_kind value must be ledger_transaction, purchase_invoice_evidence, payable_invoice, or collectible_invoice. Repeat for multiple observations.
...
``bundle_id``
   *Argument, required.* Evidence bundle id (full SHA-256 or unambiguous prefix).

**Output schema**

This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.ModeloAuditCheckResult``.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.ModeloAuditExportResult``.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.ModeloAuditReplayResult``.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.ModeloAuditShowResult``.
...
``--period``
   *Option, optional.* Period token: 0A annual, 1T-4T quarters, 01-12 months; for census modelos (036) use alta, modificacion, or baja.

``--missing``
   *Option, optional.* Filter to bindings without a resolved value (require bucket / ledger / profile data)
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.ModeloBindingsListResult``.

``aeat app modelo bindings preview``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Preview bindings with temporary overrides without mutating state

**Command path:** ``aeat app modelo bindings preview``

**Registry key:** ``modelo.bindings.preview``
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.ModeloBindingsPreviewResult``.
...
``modelo``
   *Argument, required.* Tax form code (e.g. 303, 130, 100)

``--period``
   *Option, optional.* Period token; for Modelo 036 use alta, modificacion, or baja; periodic examples include 2026Q1 and 2026-01
...
``--period``
   *Option, optional.* Period token used to resolve the modelo revision: 0A annual, 1T-4T quarters, 01-12 months; for census modelos (036) use alta, modificacion, or baja.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.ModeloDescribeResult``.

``aeat app modelo export``
~~~~~~~~~~~~~~~~~~~~~~~~~~
Export a verified-complete or filed modelo revision to a local AEAT-compatible fichero-BOE file. Local-only; never contacts AEAT.
...
``work_unit_id``
   *Argument, required.* Work unit id (SHA-256 or unambiguous prefix).

``--output``
   *Option, required.* Path to write the fichero-BOE artefact to.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.ModeloExportResult``.
...
``work_unit_id``
   *Argument, required.* Stable 64-character SHA-256 identifier of the work unit
...
``--set``
   *Option, optional.* Imported casilla value with CASILLA=DECIMAL syntax (repeatable; at least one required)
...
``--bucket-id``
   *Option, optional.* Filter by bucket
...
``filing_record_id``
   *Argument, required.* Stable filing-record identifier
...
``--period``
   *Option, optional.* Optional period filter (e.g. Q1, annual)
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Show aggregated IVA compensation carry-forward balance computed from local Modelo 303 history. Reports total_balance, lot_count, and next_expiry_year.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.IvaWalletBalanceResult``.
...
Two cases:
 ...
 --amount 0   True first-period: this is the taxpayer's first ever Modelo 303 filing under this NIF. Casilla 110 is zero because no prior compensation balance exists (LIVA art. 99.5, Ley 37/1992). The reconciliation layer treats this as first_period_zero — non-blocking and legally certain.
  ...
  --amount X   Carry-in: the taxpayer has filed prior M303s under a different tool or directly with AEAT. X is the compensacion pendiente de periodos posteriores from the last filed M303.
...
``--period``
   *Option, required.* Period of the seed amount (e.g. 2024N4).
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.IvaWalletSeedResult``.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.ModeloListResult``.
...
``--casilla``
   *Option, optional.* Additional M100 casilla override as CASILLA=DECIMAL (e.g. 0513=1150 for age supplement). Repeat for multiple overrides.
...
``--period``
   *Option, optional.* Period token (e.g. Q1, annual).
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Reconcile a modelo work unit against external evidence (justificante PDF). Local-only; never contacts AEAT.

**Command path:** ``aeat app modelo reconcile``

**Registry key:** ``modelo.reconcile``
...
``--from-justificante``
   *Option, optional.* Path to the AEAT justificante PDF to reconcile against.

``--from-declaration``
   *Option, optional.* Path to the filed declaration PDF to reconcile against (reserved; parser pending).
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.ModeloReconcileResult``.

``aeat app modelo reconcile-from-justificante``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Reconcile a modelo work unit against a justificante PDF. Sugar for operators who think "reconcile from this justificante" rather than "reconcile, source = justificante". Shares the modelo_reconcile application service entry point with the flag-based form. Local-only; never contacts AEAT.

**Command path:** ``aeat app modelo reconcile-from-justificante``

**Registry key:** ``modelo.reconcile_from_justificante``

**Parameters**

``justificante_path``
   *Argument, required.* Path to the AEAT justificante PDF to reconcile against.
...
``verification_report_id``
   *Argument, required.* Stable 64-character SHA-256 identifier of a verification report
...
``--set``
   *Option, optional.* Corrected casilla value with CASILLA=DECIMAL syntax (repeatable; at least one required)
...
``--casilla``
   *Option, optional.* Casilla value with CASILLA=DECIMAL syntax (repeatable). CASILLA accepts the casilla id, the registry number, or the BOE printed casilla number shown by `aeat app modelo casillas`.
...
``--relation``
   *Option, optional.* Prior-period relation value as KEY=VALUE. The KEY is a registry relation id; the VALUE is a decimal. Repeat to supply multiple relations.

``--row``
   *Option, optional.* Typed detail row for multi-record informational modelos. Format: TYPE FIELD=value [FIELD=value ...]. TYPE is 'miembro' (M184 attribution member), 'vinculada' (M232 related-party operation), 'operador' (M349 intracom operator), or 'contraparte' (M347 declared counterparty). Repeat to add multiple rows. M184 example: --row 'miembro nif=12345678A porcentaje=40 importe=10000'. M349 example: --row 'operador codigo_pais=DE nif_comunitario=DE123456789 clave_operacion=E importe=50000'.
...
``--sal-beneficio-neto``
   *Option, optional.* Net profit for the fiscal year of the worker-owned company (SAL/SLL, Law 44/2015 Art. 14). The assistant applies 10% to calculate the mandatory special reserve allocation, capped by the 50% share-capital threshold. Use it with --sal-reserva-dotada and --sal-capital-social.

``--sal-reserva-dotada``
   *Option, optional.* Special reserve accumulated in previous fiscal years (Law 44/2015 Art. 14). Used to check whether the reserve has already reached 50% of share capital. Required with --sal-beneficio-neto and --sal-capital-social.

``--sal-capital-social``
   *Option, optional.* Share capital of the worker-owned company (Law 44/2015 Art. 14). Denominator for the 50% test: the allocation is zero once accumulated reserve reaches 50% of share capital. Required with --sal-beneficio-neto and --sal-reserva-dotada.

``--autoconsumo-promotor-base``
   *Option, optional.* Tax base for the real-estate developer self-supply (Art. 9.1.c + Art. 79.4 LISIVA): construction or renovation cost of properties converted to the rental estate. The assistant automatically applies 21% (Art. 90 LISIVA) to compute the VAT charge. Applicable to Modelo 303 only.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.WorkCompareTaxationResult``.
...
``--period``
   *Option, required.* Work-unit period token: 0A annual, 1T-4T quarters, 01-12 months; for census modelos (036) use alta, modificacion, or baja.

``--revision``
   *Option, required.* Stable id of the targeted registry modelo revision; run `aeat app modelo describe MODELO` to see the available revision ids.

``--bucket-id``
   *Option, optional.* Bucket identifier the work unit belongs to
...
``calculation_revision_id``
   *Argument, required.* Stable 64-character SHA-256 id of a calculation revision
...
``work_unit_id``
   *Argument, required.* Work unit id whose lifecycle you want to view

**Output schema**

This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.WorkHistoryResult``.

``aeat app modelo work list``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
List registered work units, optionally filtered by bucket
...
``--name``
   *Option, required.* Optional display name for the work unit
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Validate whether you can retry an interrupted workflow run without contacting AEAT. Accepts a workflow run id or a work-unit id.
...
``target``
   *Argument, required.* 16-character workflow run id, or the 64-character work-unit id (its latest run is resolved automatically). Run `aeat app modelo work runs` to list run ids.
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.WorkRevisionResult``.
...
``work_unit_id``
   *Argument, optional.* Stable 64-character SHA-256 identifier of the work unit
...
This command emits a ``SchemaEnvelope`` whose ``result`` field is validated against ``aeat.entrypoints.cli._modelo_payloads.WorkRunsResult``.
...
``modelo``
   *Argument, required.* AEAT modelo identifier (e.g. 303, 130, 100).
...
``--period``
   *Option, optional.* Tax period to review (for example, 2026-Q1)
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
List the normatives codified in the legal corpus
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Show one normative and, optionally, one of its articles
...
``normative_id``
   *Argument, required.* Stable normative id (e.g. ley-37-1992)
...
``item_id``
   *Argument, required.* Item id from the review queue
