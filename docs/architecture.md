...
**`[setup wizard] → [config + auth provider]`** — `aeat.application.setup` (shipped via #61 / PR #66)
...
**`[submission preflight + local records]`** — `aeat.adapters.outbound.aeat.export`
...
`aeat.adapters.outbound.aeat.auth` now hangs off a provider-generic seam so the browser,
...
**`[self-healing sync] ↔ [local storage]`** — `aeat.application.sync` reconciles
...
separately by `aeat.application.filing.reconciliation`.
...
- **`aeat.llm`** is a bounded client used only by `aeat.domain.manuals` for
