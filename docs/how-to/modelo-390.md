...
strengthen the record. The rest of this guide explains each stage and the checks
around it.
...
- Mark each 303 quarter as filed locally, while its AEAT filing-obligation
  ...
  [Prepare a Modelo 303 IVA filing](modelo-303.md)), then run `aeat app modelo
  work file --modelo 303 --year 2025 --period 1T` for each. `work file` records
  the filing locally only; it does not submit to AEAT, and it refuses outside the
  obligation window.
- Capture or reconcile the official AEAT justificante for each quarter with
  `aeat app live filed pull-sources --modelo 390 --year 2025 --period 0A` and
  `aeat app modelo reconcile file --modelo 303 --year 2025 --period 1T --file
  ./303-2025-1T-justificante.pdf`. `live filed pull-sources` reads filed
  declarations from AEAT and refuses when AEAT authentication is not configured
  (it needs a Cl@ve identity matching the active profile). `reconcile file` reads
  a local justificante PDF and never contacts AEAT.
...
targets before you work on the annual target: `aeat config profile status`, then
`aeat app modelo work status --modelo 303 --year 2025 --period 1T` for each of
`1T`, `2T`, `3T`, and `4T`. List all saved work with `aeat app modelo work list`
when you need to see the active profile's broader filing surface.
...
with `aeat app modelo work revisions --modelo 303 --year 2025 --period 1T` and
`aeat app modelo work revision --modelo 303 --year 2025 --period 1T --select
filed`. If no filed revision exists locally, inspect the current or verified
revision instead with `--select latest-verified` or `--select current`.
...
--modelo 390 --year 2025 --period 0A` and `aeat app modelo reconcile file
--modelo 303 --year 2025 --period 1T --file ./303-2025-1T-justificante.pdf`.
...
reconciliation gate: `aeat app modelo iva-wallet balance --as-of-year 2025`,
`aeat app modelo iva-wallet seed --filing-year 2024 --period 4T --amount 0
--confirm`, `aeat app modelo iva-wallet correct --filing-year 2024 --period 4T
--amount 1200.50 --reason "fix opening balance" --confirm`, `aeat app live
iva-wallet history`, and `aeat app live iva-wallet pull-history --from-year 2024
--to-year 2025`. `pull-history` requires both `--from-year` and `--to-year`; it
reads filed Modelo 303 history from AEAT and refuses when AEAT authentication is
not configured.
...
Check the saved annual target and its bindings with `aeat app modelo work status
--modelo 390 --year 2025 --period 0A`, `aeat app modelo work history --modelo 390
--year 2025 --period 0A`, `aeat app modelo bindings list --modelo 390 --year 2025
--period 0A` (add `--missing` to focus on unfilled fields), `aeat app modelo
casillas 390 --period 0A`, and `aeat app modelo formulas 390 --period 0A
--explain`.
...
spreadsheet review loop when you need a wider calculation surface with `aeat
config google sync calc export --modelo 390 --year 2025 --period 0A`, then
`compute` and `verify` on the same target. The spreadsheet workflow is a review
surface; it does not submit to AEAT.
...
--modelo 390 --year 2025 --period 0A --output ./modelo-390-2025.boe`. Upload the
exported file through AEAT's official channel - the full checklist is in
[Upload your exported modelo at the AEAT portal](file-at-aeat.md). After filing,
record the local marker with `aeat app modelo work file --modelo 390 --year 2025
--period 0A`, review your filing records with `aeat app modelo filing-record
list` and `aeat app modelo filing-record view <filing-record-id>`, and reconcile
the justificante with `aeat app modelo reconcile file --modelo 390 --year 2025
