...
- **Unused amounts carry into the next period.** If a Modelo 303 quarter leaves you with more IVA paid than collected, the difference becomes an unused amount carried into a later period - a VAT credit you keep until a later return can use it.
...
A carried-forward figure is only as trustworthy as the filing it came from. So the carry-forward rests on a clear principle: a figure carried in from an earlier filing should come from a filing you actually completed and marked as filed, and where possible from one backed by the agency's receipt.
...
## The running VAT credit balance

The unused-IVA case has its own small piece of memory. When a Modelo 303 period leaves you with VAT credit, the tool keeps a running record of that unused VAT credit to carry forward and brings it into the next period's return.

That running record needs a true starting point. The first time you use the tool, it has no history of the credit you'd built up before. So you set the opening balance once - the unused VAT credit you were already carrying when you started. From there, each period updates the balance on its own.
...
What it leaves to you is the judgement:

- Confirm each carried-forward figure came from a filing you completed and marked as filed.
- Where you can, check it against the agency's receipt for that filing.
- Treat a missing prior period as a real gap to resolve - not as a zero the tool filled for you.
