...
This page covers the two outputs the tool produces once a calculation is
complete - a spreadsheet for you to review, and the exact file the AEAT
portal accepts - and why they are separate things.
...
front of you, and your reviewed edits pull back into the tool against the
exact calculation the sheet was built from. The offline `.xlsx` the tool can
also produce is a fixed keepsake of the calculation and its evidence, not a
review surface. The walkthrough is
[Review calculations with Google Sheets](../how-to/review-with-google-sheets.md).

The second output is for the machine on the other end. AEAT accepts uploads
only in one precise text format, with every value at a fixed position,
rejected if it differs by a character. The tool builds that file from your
...
These outputs come after the completeness check described in
[Editing and verifying a calculation](editing-and-verifying.md) - review and
export are worth doing only once the modelo is complete. Producing the upload
file is not the same as filing it; what happens after you have the file is
covered in
