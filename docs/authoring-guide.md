...
- **The API reference** is generated from the source modules by the `docs.tools.apidocs` tool. It writes stub pages (placeholder reference files); don't edit those pages by hand — regenerate them.
...
- **apidocs**: The `docs.tools.apidocs` command-line tool, located at `docs/tools/apidocs/`. This is the project's API reference stub builder, which crawls the codebase module tree to determine the structure of the API and generates the corresponding reference files.
...
```bash
python -m docs.tools.apidocs scaffold  # writes the API stub pages
