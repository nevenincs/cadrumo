...
Use vaultspec-rag for semantic search before reaching for grep when the concept is known but the project's chosen vocabulary is not. Run `vaultspec-rag search "<query>" --type code|vault --port 8766 --max-results N`. The RAG indexes source chunks and `.vault/` documents under one embedding model and surfaces conceptually identical locations across vocabulary mismatches that grep cannot recover. Reach for grep when the exact symbol, path, or literal string is already known.

Scope new feature surfaces with both passes. `--type code` returns implementation chunks. `--type vault` binds them to the ADR, plan, exec, and audit trail that justifies the code. Inspect the highest-score hit per directory and ignore the long 0.0x tail.
...
Reindex before consequential reasoning after substantial edits: `vaultspec-rag index --type all --port 8766`. Incremental ingest is under 15s on this codebase and the service holds the GPU models warm.
...
Phrase queries as one concise sentence or noun phrase. Verbose multi-clause paragraphs dilute the embedding signal sharply: a 50-word natural-English description caps top scores around 0.02 even when the right module is in the top hits, while the same concept phrased as a six-word noun phrase scores 0.5 to 0.9 on the same surface.
