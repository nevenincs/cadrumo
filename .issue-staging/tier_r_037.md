**Kent success moment:** Kent has the declaración PDF of his Modelo 037 filing. He runs `aeat filing import --from-declaracion <pdf>`. The tool extracts every required declarative field (named fields, not computed casillas) deterministically and presents them as a structured filing record.
...
Modelo 037 — declaración censal simplificada (subset de 036 para autónomos sin empleados sin módulos especiales). Same named-field shape as 036.

## Current state (2026-04-22 audit)

Declaración extractor MVP (5 named fields, mirror of 036). No L2 fixture. No integration test. No synthetic generator (audit flagged as the only modelo without round-trip test).

## Definition of Done — pass criteria

### Named-field schema

- [ ] Pydantic v2 frozen model `Modelo037Filing` with **every** BOE-mandated declarative field (enumerate every field in PR body, each with BOE / Orden Ministerial citation specifying the exact page/box)
...
- [ ] `src/aeat/declaracion/_extractors/modelo_037_v2025.py` named-field patterns verified against a **real L2 fixture** (not speculative regex)
...
- [ ] At least one L2 scrubbed-private PDF committed to `tests/fixtures/pdf_corpus/l2_scrubbed_private/modelo_037/` with consent-log entry in `_consent_log.jsonl`
...
- [ ] `.vault/reference/2026-037-rule-delta.md` manifest
...
- [ ] `.vault/exec/YYYY-MM-DD-modelo-037-registration-extract/…-summary.md`
