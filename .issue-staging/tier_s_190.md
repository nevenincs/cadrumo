...
Modelo 190 — resumen anual de retenciones IRPF sobre rendimientos del trabajo y actividades económicas (anual). Per-perceptor record lines + resumen totals.

## Current state (2026-04-22 audit)

Declaración extractor MVP (21 summary casillas only, no per-perceptor extraction). No ruleset. No integration test.

## Definition of Done — pass criteria

### Per-counterparty record schema

- [ ] Pydantic v2 frozen model `Modelo190RecordLine` with all BOE-required fields (enumerate every field in PR body with BOE citation)
...
- [ ] `.vault/reference/2026-190-rule-delta.md` manifest listing any schema changes per year with BOE citations (e.g., 347 umbral 3005,06 €, ClaveOperacion additions)

### PDF fixtures

- [ ] L1 public-anchor real PDF hash-pinned in `tests/fixtures/pdf_corpus/l1_public_anchors/modelo_190/` **OR** explicit `.vault/reference/` waiver
...
- [ ] `.vault/exec/YYYY-MM-DD-modelo-190-summary-verify/…-summary.md`
