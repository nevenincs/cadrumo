...
Modelo 193 — resumen anual de retenciones IRPF sobre rendimientos del capital mobiliario (anual). Per-perceptor record lines + 3 resumen casillas.

## Current state (2026-04-22 audit)

Declaración extractor MVP (3 summary casillas only). No ruleset. No integration test.

## Definition of Done — pass criteria

### Per-counterparty record schema

- [ ] Pydantic v2 frozen model `Modelo193RecordLine` with all BOE-required fields (enumerate every field in PR body with BOE citation)
...
- [ ] `.vault/reference/2026-193-rule-delta.md` manifest listing any schema changes per year with BOE citations (e.g., 347 umbral 3005,06 €, ClaveOperacion additions)

### PDF fixtures

- [ ] L1 public-anchor real PDF hash-pinned in `tests/fixtures/pdf_corpus/l1_public_anchors/modelo_193/` **OR** explicit `.vault/reference/` waiver
...
- [ ] `.vault/exec/YYYY-MM-DD-modelo-193-summary-verify/…-summary.md`
