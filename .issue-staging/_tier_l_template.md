...
- [ ] `.vault/reference/2026-{MODELO}-rule-delta.md` manifest listing every numeric/structural change per year with BOE citations
...
- [ ] Declaración extractor (`src/aeat/declaracion/_extractors/modelo_{MODELO}_v2025.py`) is casilla-complete for the liquidación block (not just MVP — enumerate full inventory)
...
- [ ] L1 public-anchor real PDF hash-pinned in `tests/fixtures/pdf_corpus/l1_public_anchors/modelo_{MODELO}/` **OR** explicit `.vault/reference/` waiver explaining why none is available
- [ ] Integration test in `tests/integration/test_kent_workflows.py::TestKentImportsModelo{MODELO}Declaracion` asserting full CLI path returns `VERIFIED` on happy path + classified discrepancies on tampered fixture (ES + EN + partial-extraction cases — follow the 130 template)
...
- [ ] `.vault/exec/YYYY-MM-DD-modelo-{MODELO}-calc-verify/…-summary.md` with mutation-harness % + coverage delta
