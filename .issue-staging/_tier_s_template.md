...
- [ ] Pydantic v2 frozen model `{Modelo}RecordLine` with all BOE-required fields (enumerate every field in PR body with BOE citation)
...
- [ ] `.vault/reference/2026-{MODELO}-rule-delta.md` manifest listing any schema changes per year with BOE citations (e.g., 347 umbral 3005,06 €, ClaveOperacion additions)

### PDF fixtures

- [ ] L1 public-anchor real PDF hash-pinned in `tests/fixtures/pdf_corpus/l1_public_anchors/modelo_{MODELO}/` **OR** explicit `.vault/reference/` waiver
...
- [ ] `.vault/exec/YYYY-MM-DD-modelo-{MODELO}-summary-verify/…-summary.md`
