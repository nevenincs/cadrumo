...
description: >-
  Path-scoped CI gate for feature commits on the shared `chore/*` factory-
  direct branch. Runs ruff + pytest + vault check against only the files the
  feature touches, isolating the feature owner from unrelated WIP carried
  by other concurrent agents on the same branch.
---
