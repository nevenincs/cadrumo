...
Write strict roundtrip tests for every persistence boundary, not just every pydantic model. Encrypted SQL via SecureObjectRepository, TOML manifests, JSON envelopes, fichero-BOE bytes, worksheet export/pull, and any CLI emit path that flows over the wire each need their own dedicated roundtrip test.
...
Avoid xfail, skip, and stub. A test that passes today but is documented as expected-to-fail is a process leak. Write tests that fail loudly today when the structural work is incomplete, and pass cleanly when it lands. Do not wrap roundtrips in try/except to hide failures.
