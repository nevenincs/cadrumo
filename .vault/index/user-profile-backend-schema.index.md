---
generated: true
tags:
  - '#index'
  - '#user-profile-backend-schema'
date: '2026-07-28'
modified: '2026-07-28'
body_schema: 'body-v1'
body_hash: 'sha256:f13feeec0f3b8b9ba808af27de641d3de47d73d1a7751fc84d5b7c6803c35867'
related:
  - '[[2026-05-07-user-profile-backend-schema-adr]]'
  - '[[2026-05-07-user-profile-backend-schema-audit]]'
  - '[[2026-05-07-user-profile-backend-schema-exec]]'
  - '[[2026-05-07-user-profile-backend-schema-plan]]'
  - '[[2026-05-08-user-profile-backend-schema-code-review-audit]]'
  - '[[2026-05-08-user-profile-backend-schema-w3-p1-registry-contract-exec]]'
  - '[[2026-05-08-user-profile-backend-schema-w3-p1-validator-integration-exec]]'
  - '[[2026-06-04-user-profile-backend-schema-research]]'
---

# `user-profile-backend-schema` feature index

Auto-generated index of all documents tagged with `#user-profile-backend-schema`.

## Documents

### adr

- `2026-05-07-user-profile-backend-schema-adr` - `user-profile-backend-schema` adr: `User Profile Backend Schema And Persistence` | (**status:** `accepted`)

### audit

- `2026-05-07-user-profile-backend-schema-audit` - `user-profile-backend-schema` audit: `Wave 1 Schema Foundation Review`
- `2026-05-08-user-profile-backend-schema-code-review-audit` - `user-profile-backend-schema` Code Review

### exec

- `2026-05-07-user-profile-backend-schema-exec` - `user-profile-backend-schema` `Wave 1 Schema Foundation Step` `Wave 1 Schema Foundation Step`
- `2026-05-08-user-profile-backend-schema-w3-p1-registry-contract-exec` - `user-profile-backend-schema` `W3.P1` `Registry Contract`
- `2026-05-08-user-profile-backend-schema-w3-p1-validator-integration-exec` - `user-profile-backend-schema` `W3.P1` `Validator Integration`

### plan

- `2026-05-07-user-profile-backend-schema-plan` - `user-profile-schema-rollout` `User Profile Schema And Config CLI Rollout` plan

### research

- `2026-06-04-user-profile-backend-schema-research` - `user-profile-backend-schema` research: `phase two research grounding`  ## Question  Which ADR-only feature finding needs an explicit live research record so VaultSpec semantic search can brief future work from an evidence node instead of an orphaned decision?  ## Findings  This note is a Phase Two vault-curation grounding record. It does not introduce new runtime behavior, change accepted architecture, or supersede a deeper feature-specific research note.  The feature health check reported an ADR without a same-feature research document. Live ADR records linked in frontmatter remain the decision sources; this research record makes the evidence path explicit and searchable.  If archived records exist for this feature, they remain historical/reference evidence only. They are not treated as current authority unless a live ADR explicitly re-enrols them.  Body wiki-links are intentionally avoided. The authoritative navigation edge is carried by frontmatter so body-link hygiene remains clean.  ## Recommendation  Keep this record as the active research bridge until a deeper feature-specific research document supersedes it and updates the related frontmatter on the ADR and index records.
