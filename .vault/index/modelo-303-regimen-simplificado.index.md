---
generated: true
tags:
  - '#index'
  - '#modelo-303-regimen-simplificado'
date: '2026-08-16'
modified: '2026-08-30'
body_schema: 'body-v2'
body_hash: 'sha256:e3a5f4e90071f20c22e1aabe0384c1d85fa0a7e095522a2966f1edcdb400206b'
related:
  - '[[2026-07-01-modelo-303-regimen-simplificado-adr]]'
  - '[[2026-07-10-modelo-303-regimen-simplificado-research]]'
  - '[[2026-08-14-modelo-303-regimen-simplificado-s84-immutable-annual-summary-handoff-review-audit]]'
---

# `modelo-303-regimen-simplificado` feature index

Auto-generated index of all documents tagged with `#modelo-303-regimen-simplificado`.

## Documents

### adr

- `2026-07-01-modelo-303-regimen-simplificado-adr` - `modelo-303-regimen-simplificado` adr: `modulos-based IVA cuota binding set` | (**status:** `accepted`)

### audit

- `2026-08-14-modelo-303-regimen-simplificado-s84-immutable-annual-summary-handoff-review-audit` - `modelo-303-regimen-simplificado` audit: `S84 immutable annual-summary handoff review`

### research

- `2026-07-10-modelo-303-regimen-simplificado-research` - modelo-303-regimen-simplificado research: warning closeout research grounding
