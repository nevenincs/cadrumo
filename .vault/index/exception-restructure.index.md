---
generated: true
tags:
  - '#index'
  - '#exception-restructure'
date: '2026-06-13'
modified: '2026-06-13'
related:
  - '[[2026-05-09-exception-restructure-adr]]'
  - '[[2026-05-09-exception-restructure-phase-1-plan]]'
  - '[[2026-05-09-exception-restructure-phase-1-step1-exec]]'
  - '[[2026-05-09-exception-restructure-research]]'
---

# `exception-restructure` feature index

Auto-generated index of all documents tagged with `#exception-restructure`.

## Documents

### adr

- `2026-05-09-exception-restructure-adr` - `exception-restructure` adr: every domain error roots at AeatError via `_errors.py` convention | (**status:** `accepted`)

### exec

- `2026-05-09-exception-restructure-phase-1-step1-exec` - `exception-restructure` `phase-1` `step-1`

### plan

- `2026-05-09-exception-restructure-phase-1-plan` - Exception Restructure Plan (Revised)

### research

- `2026-05-09-exception-restructure-research` - Exception Handling Restructure Research
