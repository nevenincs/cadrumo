---
generated: true
tags:
  - '#index'
  - '#modelo-123-calc-verify'
date: '2026-08-13'
modified: '2026-08-13'
body_schema: 'body-v1'
body_hash: 'sha256:6bb0a38d5b265240d3de49f99159d8eae0f54e2b70916b65486dfb41dc5e582e'
related:
  - '[[2026-04-27-modelo-123-calc-verify-adr]]'
  - '[[2026-04-27-modelo-123-calc-verify-plan]]'
  - '[[2026-04-27-modelo-123-calc-verify-research]]'
  - '[[2026-04-27-modelo-123-calc-verify-summary-exec]]'
  - '[[2026-04-27-modelo-123-rule-delta-reference]]'
  - '[[2026-05-27-modelo-123-calc-verify-exec]]'
---

# `modelo-123-calc-verify` feature index

Auto-generated index of all documents tagged with `#modelo-123-calc-verify`.

## Documents

### adr

- `2026-04-27-modelo-123-calc-verify-adr` - `modelo-123-calc-verify` adr: aggregation-only 2026 rollover | (**status:** `accepted`)

### exec

- `2026-04-27-modelo-123-calc-verify-summary-exec` - `modelo-123-calc-verify` summary
- `2026-05-27-modelo-123-calc-verify-exec` - `modelo-123-calc-verify` Aitor #211 — M123 casilla 06 arithmetic oracle

### plan

- `2026-04-27-modelo-123-calc-verify-plan` - `modelo-123-calc-verify` implementation plan

### reference

- `2026-04-27-modelo-123-rule-delta-reference` - modelo 123 2024-2026 rule delta

### research

- `2026-04-27-modelo-123-calc-verify-research` - `modelo-123-calc-verify` research: 2024-2026 aggregation verification
