---
generated: true
tags:
  - '#index'
  - '#live-cert-auth'
date: '2026-06-15'
modified: '2026-06-15'
related:
  - '[[2026-04-16-live-cert-auth-adr]]'
  - '[[2026-04-16-live-cert-auth-research]]'
  - '[[2026-04-21-live-cert-auth-supersession-adr]]'
  - '[[2026-04-27-live-cert-auth-research]]'
---

# `live-cert-auth` feature index

Auto-generated index of all documents tagged with `#live-cert-auth`.

## Documents

### adr

- `2026-04-16-live-cert-auth-adr` - `live-cert-auth` adr: `issue-141 live certificate auth stabilization and verification` | (**status:** `superseded` *(originally `accepted`; superseded 2026-04-21 by AuthProvider abstraction)*)
- `2026-04-21-live-cert-auth-supersession-adr` - `live-cert-auth` adr: `issue-141 pr-148 superseded by certificateauthprovider` | (**status:** `accepted`)

### research

- `2026-04-16-live-cert-auth-research` - live-cert-auth research: issue-141 live certificate auth setup and verification
- `2026-04-27-live-cert-auth-research` - `live-cert-auth` research: supersession of issue-141 pr-148 by merged auth protocol work
