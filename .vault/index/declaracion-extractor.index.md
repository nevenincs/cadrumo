---
generated: true
tags:
  - '#index'
  - '#declaracion-extractor'
date: '2026-06-13'
modified: '2026-06-13'
related:
  - '[[2026-04-21-declaracion-extractor-adr]]'
  - '[[2026-04-21-declaracion-extractor-plan]]'
  - '[[2026-04-21-declaracion-extractor-research]]'
---

# `declaracion-extractor` feature index

Auto-generated index of all documents tagged with `#declaracion-extractor`.

## Documents

### adr

- `2026-04-21-declaracion-extractor-adr` - `declaracion-extractor` adr: `label-first-bbox-fallback-acroform-opportunistic-per-modelo-registry` | (**status:** `superseded`)

### plan

- `2026-04-21-declaracion-extractor-plan` - `declaracion-extractor` plan

### research

- `2026-04-21-declaracion-extractor-research` - declaracion-extractor research
