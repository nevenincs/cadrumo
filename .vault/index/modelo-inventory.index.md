---
generated: true
tags:
  - '#index'
  - '#modelo-inventory'
date: '2026-06-15'
modified: '2026-06-15'
related:
  - '[[2026-04-13-modelo-inventory-adr]]'
  - '[[2026-04-13-modelo-inventory-audit]]'
  - '[[2026-04-13-modelo-inventory-phase-all-summary-exec]]'
  - '[[2026-04-13-modelo-inventory-phase1-scaffold-exec]]'
  - '[[2026-04-13-modelo-inventory-phase2-enums-models-exec]]'
  - '[[2026-04-13-modelo-inventory-phase3-errors-exec]]'
  - '[[2026-04-13-modelo-inventory-phase4-entries-exec]]'
  - '[[2026-04-13-modelo-inventory-phase5-registry-exec]]'
  - '[[2026-04-13-modelo-inventory-phase6-casilla-xref-exec]]'
  - '[[2026-04-13-modelo-inventory-phase7-cli-exec]]'
  - '[[2026-04-13-modelo-inventory-phase8-public-api-exec]]'
  - '[[2026-04-13-modelo-inventory-phase9-green-gates-exec]]'
  - '[[2026-04-13-modelo-inventory-plan]]'
  - '[[2026-04-13-modelo-inventory-research]]'
  - '[[2026-04-17-modelo-inventory-remediation-adr]]'
  - '[[2026-04-17-modelo-inventory-remediation-audit]]'
  - '[[2026-04-17-modelo-inventory-remediation-phase-all-summary-exec]]'
  - '[[2026-04-17-modelo-inventory-remediation-phase1-runtime-parity-exec]]'
  - '[[2026-04-17-modelo-inventory-remediation-phase2-setup-parity-exec]]'
  - '[[2026-04-17-modelo-inventory-remediation-phase3-green-gates-exec]]'
  - '[[2026-04-17-modelo-inventory-remediation-plan]]'
  - '[[2026-04-17-modelo-inventory-remediation-research]]'
---

# `modelo-inventory` feature index

Auto-generated index of all documents tagged with `#modelo-inventory`.

## Documents

### adr

- `2026-04-13-modelo-inventory-adr` - modelo-inventory adr (#108)
- `2026-04-17-modelo-inventory-remediation-adr` - `modelo-inventory` adr: `regulatory-remediation-for-037-130-347-193-and-year-plan-parity` | (**status:** `accepted`)

### audit

- `2026-04-13-modelo-inventory-audit` - modelo-inventory audit (#108)
- `2026-04-17-modelo-inventory-remediation-audit` - `modelo-inventory` Code Review

### exec

- `2026-04-13-modelo-inventory-phase-all-summary-exec` - phase-all summary — modelo inventory (#108)
- `2026-04-13-modelo-inventory-phase1-scaffold-exec` - phase 1 — scaffold aeat.domain.modelos module skeleton
- `2026-04-13-modelo-inventory-phase2-enums-models-exec` - phase 2 — enums + primitive pydantic models
- `2026-04-13-modelo-inventory-phase3-errors-exec` - phase 3 — error hierarchy
- `2026-04-13-modelo-inventory-phase4-entries-exec` - phase 4 — populate registry entries
- `2026-04-13-modelo-inventory-phase5-registry-exec` - phase 5 — registry assembly + year_plan
- `2026-04-13-modelo-inventory-phase6-casilla-xref-exec` - phase 6 — casilla catalogue cross-reference
- `2026-04-13-modelo-inventory-phase7-cli-exec` - phase 7 — CLI subcommands
- `2026-04-13-modelo-inventory-phase8-public-api-exec` - phase 8 — public API lock + docstrings
- `2026-04-13-modelo-inventory-phase9-green-gates-exec` - phase 9 — green gates
- `2026-04-17-modelo-inventory-remediation-phase-all-summary-exec` - `modelo-inventory` `phase-all` summary
- `2026-04-17-modelo-inventory-remediation-phase1-runtime-parity-exec` - `modelo-inventory` `phase1` `runtime-parity`
- `2026-04-17-modelo-inventory-remediation-phase2-setup-parity-exec` - `modelo-inventory` `phase2` `setup-parity`
- `2026-04-17-modelo-inventory-remediation-phase3-green-gates-exec` - `modelo-inventory` `phase3` `green-gates`

### plan

- `2026-04-13-modelo-inventory-plan` - modelo-inventory plan (#108)
- `2026-04-17-modelo-inventory-remediation-plan` - `modelo-inventory` `remediation` plan

### research

- `2026-04-13-modelo-inventory-research` - modelo-inventory research: authoritative AEAT modelo catalogue
- `2026-04-17-modelo-inventory-remediation-research` - `modelo-inventory` research: `regulatory-remediation-for-037-130-347-193-and-year-plan-parity`
