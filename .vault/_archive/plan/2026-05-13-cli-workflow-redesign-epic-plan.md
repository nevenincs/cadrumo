...
date: '2026-05-13'
modified: '2026-07-15'
body_hash: 'sha256:babde4b34ef495d8507e2040eb4f089456dcef4842a66e4f00639daba8c42b22'
tier: L4
related:
...
> Archived on 2026-07-17 as non-authoritative historical evidence. The checked
> rows are not accepted as execution proof where no execution record exists;
> current CLI work is owned by the CLI authority verb conformance plan.
...
- [x] `W09.P042.S0248` - Delete duplicate backend branches that compete with profile backend schema; `src/aeat/application/profile`.
...
This Wave implements the 2026-05-14 audit amendment to the apex ADR. Each
Phase resolves one verified finding from the test-user run, wiring the
backend, the CLI surface, and the tests that prove the fix. The Wave does
not renumber any prior Wave; it appends.

### Phase `W70.P330` - finding P0 #1: uv Windows dev-environment launcher
...
This Phase implements the config-doctor-shape ADR's retirement status (superseded by config-repair-shape) and the config-repair-shape ADR's "absorbs from retired doctor" section. It covers BOTH legacy retirement AND repair-surface exhaustiveness verification in the same wave; no shim, no alias, no half-and-half (see project rule `no_backwards_compat_no_deprecation`).
...
## Wave `W86` - Late-bucket gap quantification

Quantify the as-yet-unquantified gaps surfaced during the lint-zero-out session: incomplete S2058 verb implementations (validate, preflight), incomplete S2019 history test coverage, S233 canonical period rendering ambiguity, S1853/S1913 surface-design ambiguities still pending ADR decisions, modelo top-level verb roster pinned, and ledger verb count gate pinned.

### Phase `W86.P415` - S2058 sub-decomposition: validate + preflight verbs
...
- [x] `W86.P416.S2357` - Author ADR resolving S1853 declaration-verification canonical surface ambiguity: app modelo verify + reconcile vs current work_app subgroup mount, including verify is currently work_app.command('verify') at _modelo.py:3647 not top-level; `.vault/adr/`.
...
- [x] `W86.P416.S2359` - Author ADR pinning canonical period rendering form (registry-native 1T vs display 2026Q1) so S233 (overview backlog M111 inconsistency) can land without breaking the 5+ test_engine.py expectations on the 2026Q1 form; `.vault/adr/`.
