...
The `set_classification` service function in `src/cadrumo/domain/transactions/_service.py` will be modified to accept these two optional arguments and apply them when creating the updated `Transaction` instance.
...
**Neutral:**
- Will require updating the unit tests in `src/cadrumo/domain/transactions/test_cli.py` to ensure the new flags are properly parsed and persisted.
