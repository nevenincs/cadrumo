...
- The implementation should stay inside `aeat.financial` and `aeat.financial.providers`, plus the minimal CLI/provider-selection changes needed for ingest. It must not introduce a new parallel raw-document storage subsystem in this issue.
...
- **Provider shape.** Add `SourceFormat.PDF` to `RawProvenance`, implement `PdfN26Provider` under `aeat.financial.providers`, and register it in the provider exports, auto-detection, and CLI provider selection.
