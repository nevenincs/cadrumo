...
date: "2026-04-21"
modified: '2026-07-17'
body_hash: 'sha256:7e7dd425cd5722ea148e6df097f7c0b70fece0956af6e5b55a6d936fad23cc94'
related:
...
- The typed registry schema at
  `src/cadrumo/domain/calculations/registry/_schema.py` models casilla shape,
  formula links, constraints, and provenance.
- Rulesets (`src/cadrumo/domain/calculations/registry/modelo_*.py`) are today the de-facto source of truth for "which casillas exist" on the modelos that have a formula implementation. They cover 130 and 303 only; no 390 ruleset exists (see #221).
- The formula runtime at
  `src/cadrumo/domain/calculations/registry/_formula_runtime.py` evaluates only
  casillas declared by the selected revision; registry completeness is
  therefore part of the verification contract.
...
- **No breakage of `#271` shipping contract.** `cadrumo.domain.justificante`, `FilingDraft.draft_id` hash shape, `SubmittedFiling`, amendment baseline flow — all untouched.
...
- **Relative imports** inside `src/cadrumo/`.
...
New test at `src/cadrumo/application/filing/test_schema_completeness.py`:
...
`src/cadrumo/domain/schema/_forms_xml_loader.py` (new):
...
Fallback for modelos where the interactive form XML is unavailable: a BOE PDF fallback wired to `src/cadrumo/domain/schema/_boe_extractor.py`. Implemented only if needed — scope depends on which modelos XML covers.
