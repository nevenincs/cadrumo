...
The existing codebase already uses explicit ownership boundaries for cleanup. `src/aeat/cli/browser/health.py::_RealProbe.probe()`, `src/aeat/justificante/_verify.py::verify_csv`, and `src/aeat/status/_reader.py::StatusReader.close()` all close the context they created, and stop Playwright only when they own that lifecycle. The session abstraction should fit that model without shifting cleanup responsibility onto context-owning consumers.
