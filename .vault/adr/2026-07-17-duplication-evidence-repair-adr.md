...
---
# `duplication-evidence-repair` adr: `duplication-evidence-repair rescope grounding` | (**status:** `accepted`)
...
This successor's scope: Repair the duplication-measurement instrument (the clone-scanner runner and the health-report verdict) so a scanner that inspected no files can no longer be reduced to a false green, reducing three independent scanner invocations to one typed platform-neutral runner and keeping the clone count advisory - an amber verdict carrying a measured count is an acceptable close.
...
Scaffold this ADR under the successor's own feature tag with `vault add adr`, relate it to the shared governing ADR and to this successor's plan, and mark it accepted. Execution records for this plan then scaffold normally with `vault add exec --feature duplication-evidence-repair --step S##`. No source code changes accompany this ADR; it is a vault-lifecycle grounding record only.
