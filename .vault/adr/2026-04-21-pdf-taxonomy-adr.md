...
date: "2026-04-21"
modified: '2026-07-17'
body_hash: 'sha256:247d7b398321be0d696450e5faa559f754f9c20c9986eb7ddacfcc60b3ca46cf'
related:
...
- The project mandate pins Spanish as the authoritative AEAT terminology. Keeping the original Spanish names at module level (`cadrumo.domain.justificante`, `cadrumo.adapters.inbound.declaracion`, `cadrumo.adapters.inbound.borrador`, …) is idiomatic and stable across AEAT template revisions.
- `cadrumo.domain.justificante` is already a public surface (re-exported from `cadrumo.application.filing._import` via `parse_justificante`, referenced by the `SubmittedFiling` amendment baseline, used by `_complementaria._resolve_original_metadata`). Renaming it mid-flight breaks the amendment flow. Preserving the name is the lower-risk path.
...
- **Zero breakage of shipped `#271` contracts.** `cadrumo.domain.justificante` keeps its name, shape, and public surface. `aeat filing import --from-justificante` keeps its behaviour.
- **Zero cert-auth coupling.** All documents in scope are user-local (Kent downloaded them from Sede).
- **Spanish module names** for PDF-class packages (`cadrumo.adapters.inbound.declaracion`, etc.) — consistent with project mandate.
- **Strict+frozen pydantic v2** for every new boundary record.
- **Errors rooted at `CadrumoError`.** New `DeclaracionParseError` / `BorradorParseError` / `PredeclaracionParseError` inherit from a common `PdfFilingImportError(CadrumoError)` defined once.
...
| --- | --- | --- | --- | --- |
| justificante de presentación | `cadrumo.domain.justificante` ✅ (exists) | `Justificante` ✅ (exists) | No | Yes |
| declaración / copia de la declaración | `cadrumo.adapters.inbound.declaracion` (cluster D) | `DeclaracionFiling` (cluster D) | Yes | Yes |
| borrador | `cadrumo.adapters.inbound.borrador` (cluster F) | `BorradorFiling` (cluster F) | Yes | No |
...
Each new PDF-class module mirrors `cadrumo.domain.justificante`'s proven shape:
...
src/cadrumo/<class>/
    ...
    _errors.py           # <Class>ParseError(PdfFilingImportError(CadrumoError))
...
```python
# src/cadrumo/adapters/inbound/pdf/_shared.py  (new)
...
```
CadrumoError (existing)
└── PdfFilingImportError (new, under src/cadrumo/adapters/inbound/pdf/_errors.py)
...
Re-homing `JustificanteError` is a compatibility-preserving refactor: callers that catch `JustificanteError` keep catching it; callers that catch `CadrumoError` keep catching it; callers that want "any PDF import error" get the new `PdfFilingImportError` as the unifying type.

### 5. Coverage-matrix axis

Add a new column to `docs/coverage/modelos.md`:

| Modelo | … existing columns … | `cadrumo.domain.justificante` | `cadrumo.adapters.inbound.declaracion` | `cadrumo.adapters.inbound.borrador` | `aeat.predeclaracion` |
...
- Any actual parser / extractor code for `cadrumo.adapters.inbound.declaracion`, `cadrumo.adapters.inbound.borrador`, `aeat.predeclaracion`, `aeat.datos_fiscales` — those are clusters D, F, and later.
- Renaming or moving `cadrumo.domain.justificante`. Name, shape, and public surface are locked.
...
- `cadrumo.domain.justificante` keeps its name and public surface — `#271` ships unchanged, the amendment-baseline flow keeps working.
