...
This successor's scope: Consolidate authentication, certificate, and recovery custody onto single typed authorities and cut their CLI grammar over to match, keeping profile-logout, auth-logout, auth-reset, passphrase-change, and recovery deliberately distinct, and remove the module-global secret-store test-double seam via constructor dependency-injection.
...
Scaffold this ADR under the successor's own feature tag with `vault add adr`, relate it to the shared governing ADR and to this successor's plan, and mark it accepted. Execution records for this plan then scaffold normally with `vault add exec --feature auth-cert-recovery-custody --step S##`. No source code changes accompany this ADR; it is a vault-lifecycle grounding record only.
