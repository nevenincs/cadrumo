...
modules (`cadrumo.application.filing._builders._modelo_130_schema.py`) or curated
...
- `cadrumo.domain.modelos` (#6) owns `ModeloCode` — 20 enum members; every
  extracted `Modelo` MUST cite one of these.
- `cadrumo.domain.portals` (#7) owns `Portal` — every `ModeloCode` with a
...
We will implement a new public subpackage **`cadrumo.domain.schema`** that
...
- **Location**: `src/cadrumo/domain/schema/` (the directory exists on `main`
  with only a smoke test; this issue owns its first real contents).
- **Public API**: imports allowed *only* from `cadrumo.domain.schema` root.
...
- **Non-goal**: `cadrumo.domain.schema` is NOT a curated catalogue. It is an
  extraction pipeline + typed IR. The hand-reviewed catalogue is
  `cadrumo.domain.casillas` (#23).
...
- **`CasillaDataType`** — `StrEnum`. **Owned by `cadrumo.domain.schema`**;
  `cadrumo.domain.casillas.models.CasillaDataType` is the current home but
  will be re-exported from `cadrumo.domain.schema` in a follow-up (see §7).
  For v1 the `cadrumo.domain.schema.CasillaDataType` is a new StrEnum with
...
- **`Casilla`**:
  ...
  - `label: Translatable` — authoritative Spanish REQUIRED
    via `cadrumo.core.i18n.require_authoritative`, English and Hungarian
...
- **`Modelo`**:
  - `modelo_code: ModeloCode` (imported from `cadrumo.domain.modelos`)
  - `portal: Portal | None` (imported from `cadrumo.domain.portals`;
  ...
  - `period: str` — validated against `ModeloCode` cadence
    ...
    `cadrumo.domain.schema._models` and consumes cadence metadata via the
    public `cadrumo.domain.modelos` API (`get_modelo(code).cadence`).
  ...
  - Portal cross-ref: if `portal` is set, `Portal` metadata must
    ...
    `cadrumo.domain.portals.get_portal`.
...
- The real `BoeOrdenExtractor` is exercised directly against a small,
  redistributable PDF source checked into
...
- Manifest sidecar `<file>.manifest.json` records
  `SchemaProvenance` only — mirrors the `cadrumo.domain.manuals`
  pattern exactly.
- The cache root is configurable via **new settings** (§5).

### 5. Settings additions (`src/cadrumo/core/config.py`)
...
- `aeat_schema_source_urls_override: str`
  ...
  - description: "Optional JSON-encoded mapping of
    ...
    single-shape contract the rest of `cadrumo.core.config.Settings`
...
- New Typer subcommand group `aeat schema` authored in
  `src/cadrumo/entrypoints/cli/schema.py`, registered in
  `src/cadrumo/entrypoints/cli/__init__.py`. (The ADR originally proposed
  `cadrumo.domain.schema._cli` as the owning module; the plan-audit round
  moved the CLI into `cadrumo.entrypoints.cli.schema` to match the existing
  `cadrumo.entrypoints.cli.casillas` precedent and preserve public-API discipline
  — see plan §7.)
- Commands in v1:
...
- **Fetch helper** — `cadrumo.domain.schema._fetch.fetch_boe_pdf(boe_ref,
  origin_url, cache_dir) -> FetchedSchemaSource` mirrors the
  `cadrumo.domain.manuals._fetch` pattern exactly: streams the bytes via
...
- Offline verification composes the real extractor with the committed BOE
  source bytes at the application boundary. Product CLI and settings surfaces
  carry no test-only option.
...
### 7. Interaction with `cadrumo.domain.casillas` (#23)

- `cadrumo.domain.casillas.models.ModeloCode` (the local duplicate) is
  flagged for removal. The follow-up PR imports
  `cadrumo.domain.modelos.ModeloCode` and deletes the local StrEnum. **Not
  done in this PR** to keep the blast radius contained.
- The curated casilla layer references the canonical
  `cadrumo.domain.schema.FormulaNode` and `ValidationRule` contracts through an
  explicit adapter where reviewer-level fields differ from the raw extracted
  model. It does not declare parallel placeholder types.
- `cadrumo.domain.schema` MUST NOT import from `cadrumo.domain.casillas` (reverse
  dependency direction only). Enforced by smoke test.

### 8. Error hierarchy

`src/cadrumo/domain/schema/_errors.py` defines:

- `SchemaError(CadrumoError)` — base.
...
- All new tests `@pytest.mark.unit`. They exercise the production extractor,
  cache, evaluator, and CLI composition against real, licence-clean AEAT source
  bytes and real filesystem behavior.
- Coverage additions:
  ...
  - `test_cli.py` — CLI `refresh` and `show` composition against the
    committed source through the real application boundary; product CLI and
    settings carry no testing-only control.
  - `test_smoke.py` — `__all__` completeness, package docstring
    present, no imports of `cadrumo.domain.casillas`.
...
- `cadrumo.core.logging.get_logger(__name__)` for any module that emits
  logs — no `print`, no raw `logging.getLogger`.
- Domain errors inherit from `cadrumo.core.errors.CadrumoError`.
- Trilingual labels via `cadrumo.core.i18n.Translatable` with Spanish
  authoritative (`require_authoritative(..., domain="aeat")`).
- Tests are classified by the behavior they exercise. Offline extraction and
  cache tests are unit-scoped; external network verification is an explicit
  live surface and failures remain failures when enabled.
- Public API discipline: consumers outside `cadrumo.domain.schema` import
...
- `cadrumo.domain.schema` cleanly owns the IR; `cadrumo.domain.casillas` cleanly
  owns the reviewer catalogue; `cadrumo.domain.modelos` + `cadrumo.domain.portals`
...
- Two modelo enums remain (`cadrumo.domain.modelos.ModeloCode` and
  `cadrumo.domain.casillas.models.ModeloCode`) until the #23-side
  follow-up merges.
...
- `cadrumo.domain.casillas` integration (adapter PR).
...
- [x] `ModeloCode` from `cadrumo.domain.modelos` is the authoritative
  identity; `cadrumo.domain.schema.Modelo.modelo_code` types against it. ✓
- [x] `Portal` from `cadrumo.domain.portals` is referenced from
  `cadrumo.domain.schema.Modelo.portal`; enum invariants enforced via
  `get_portal`. ✓
- [x] Scope matches issue #9 acceptance criteria: research doc
  (present), ADR (this doc), `src/cadrumo/domain/schema/` types, one
...
- [x] Deferrals explicit: `schema diff`, live probe, LLM drafts,
  XSD, `cadrumo.domain.casillas` adapter. ✓
...
| 2 | MINOR | §2 Modelo.period bullet — helper owned by `cadrumo.domain.schema._models`; cadence logic non-duplication rule stated. |
| 3 | MINOR | §4 persistence — path nailed to `var/schema-cache/modelo_<value>/<boe_ref>.json`. |
| 4 | MINOR | §2 FormulaNode evaluator — all four kinds are implemented. Division-by-zero wraps `SchemaExtractionError`. |
| 5 | MINOR | §6 — fetch helper named (`cadrumo.domain.schema._fetch.fetch_boe_pdf`); composition order stated. |
| 6 | MINOR | §6 — offline verification uses the real application composition; no testing-only product option is allowed. |
...
| 9 | NIT | §11 added — Google docstrings, logging, `CadrumoError`, Translatable, `@pytest.mark.unit`, public API discipline restated. |
...
- `retired architecture decision` — the downstream curated
  catalogue this extractor feeds.
- `src/cadrumo/domain/manuals/_fetch.py` — the sha256-verified fetch
