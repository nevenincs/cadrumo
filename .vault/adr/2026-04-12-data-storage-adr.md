...
4. **Public API discipline**: callers import only from `aeat.storage`. Internal
   modules (`aeat.storage.engine`, `aeat.storage.session`, `aeat.storage.base`,
   `aeat.storage.models`) are not importable from outside the subpackage per project
   convention.
5. **Type discipline** (honours the project pydantic mandate — every record that
   crosses a boundary is a pydantic v2 model):
   - SQLAlchemy ORM mapper classes live in `aeat.storage._orm` and are **internal**.
     They exist only to back the declarative schema used by Alembic autogenerate;
     they are never exported.
   - The public surface of `aeat.storage` exposes **pydantic v2** record models
...
6. **Errors**: a single `StorageError(AeatError)` base in
   `src/aeat/storage/errors.py`, with narrow subclasses (`MigrationError`,
   `RepositoryError`) added only when the public API actually raises them.
7. **Logging**: `aeat.logging.get_logger(__name__)` in every module. Alembic's
...
1. Edit or add a declarative model under `src/aeat/storage/models.py`.
...
4. Run the migration round-trip test (`uv run pytest
   src/aeat/storage/_test_migrations.py`). It must pass against a fresh ephemeral
   database.
