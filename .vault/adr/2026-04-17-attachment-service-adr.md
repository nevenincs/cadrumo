...
date: "2026-04-17"
modified: '2026-07-17'
body_hash: 'sha256:bc321eed525c1073d151125740cca77aca24a5926585e7f8eec4fa757bb992bb'
related:
...
- `cadrumo.domain.transactions` is the already-merged downstream consumer of attachment links. Attachments must hold `transaction_id` references as opaque strings and avoid coupling to the transaction runtime type beyond a typing-level Protocol.
- `cadrumo.domain.invoices/` is still in flight on `#75`, so the attachment package cannot hard-import invoice types. Invoice foreign keys must be plain `str` at runtime with a typing-only `Protocol` placeholder for documentation and IDE help.
...
- The public API must be importable only from `cadrumo.domain.attachments`; callers must not reach into underscored submodules.
- Every persisted and boundary-crossing structure must be strict pydantic v2; closed sets must use `enum.StrEnum`.
- All domain errors must inherit from `cadrumo.core.errors.CadrumoError`; logging must use `cadrumo.core.logging.get_logger(__name__)`.
...
- Create `src/cadrumo/domain/attachments/` with a public `__init__.py` and private underscored modules for enums (`_enums.py`), errors (`_errors.py`), models (`_models.py`), the byte/manifest store (`_store.py`), service helpers (`_service.py`), and typing stubs (`_stubs.py`).
...
- Add an `AEAT_ATTACHMENTS_DIR` setting to `cadrumo.core.config.Settings` defaulting to `PROJECT_ROOT / "var" / "financial" / "attachments"`, and mirror the entry in `env/.env.example`.
- Mount the CLI at the root as `aeat attachments`: a Typer sub-app in `src/cadrumo/entrypoints/cli/attachments.py` exposing `add`, `list`, and `show` that resolve the store directory from the configured setting.
...
- Colocated unit tests under `src/cadrumo/domain/attachments/`:
...
- Tuple-typed linked IDs match the issue's stated types and align with the immutability discipline already used in `cadrumo.domain.transactions`.
- Typing-only `Protocol` stubs for invoice/transaction references keep the attachment package from creating hard imports into sibling branches that are still in flight, mirroring the pattern established by `cadrumo.domain.transactions._stubs`.
