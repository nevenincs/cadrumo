...
- `aeat.justificante` is a correctly-scoped module. Renaming breaks the amendment-baseline path (`#271`, `_complementaria._resolve_original_metadata`). The code stays.
...
- **No code rename.** `aeat.justificante` stays.
...
- `aeat.justificante` module rename, export rename.
- CLI flag rename.
