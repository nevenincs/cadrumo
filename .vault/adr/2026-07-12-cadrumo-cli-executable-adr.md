...
decision overrides one element of the parent tuple: the human executable
...
- product display name: `Cadrumo` (title case in prose; all-caps only as the wordmark lockup);
...
> **Operator re-confirmation, third (2026-07-13, supersession repair).** The parent
> rename ADR had no reciprocal `superseded_by` marker in committed history, so two
> ADRs read as simultaneously accepted and concurrent fleets enforced contradictory
> casings from each. The chain is now repaired: THIS document is the sole active
> naming authority; the parent is `superseded`. The operator's rulings stand:
> executable `aeat`; display prose `Cadrumo`; all-caps `CADRUMO` only as the
> wordmark/logotype lockup. Plan step W01.P02.S93's "CADRUMO authority regression"
> framing is retired by this repair.
...
locale catalogue prose, or any persisted value. The i18n render boundary
(`_normalise_product_identity_references`) enforces this projection at runtime
in both directions.
