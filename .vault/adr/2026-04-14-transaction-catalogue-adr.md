...
- Public API must be imported from `aeat.financial.transactions` only.
- Every persisted and boundary-crossing structure must be strict pydantic v2; closed sets must use `enum.StrEnum`.
- Errors must inherit from `aeat.errors.AeatError`, and logging must use `aeat.logging.get_logger(__name__)`.
- The wrapped `raw` record must remain verbatim and never be mutated in place.

## Implementation

- Create `src/aeat/financial/transactions/` with a public `__init__.py` and private underscored modules for enums, models, persistence, service functions, CLI helpers, and typing stubs.
...
- A dedicated `aeat.financial.transactions` public surface matches the package discipline already established by `aeat.financial.vat` and avoids callers reaching into internal helpers.
