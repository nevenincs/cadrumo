...
S47's 23 annual-summary values resolve from `CalculationRevision.casilla_values` joined to canonical observations and evidence. Arbitrary `producer_reference` and `marker_reference` authority is deleted. A28 and the S50 activity/module rows become immutable revision evidence under S58, with the latter referencing the registry-owned annual Orden authority established by S59. A29 is the derived annual-volume observation and never a `FilingProducerKey`. Export and review replay those facts; they do not author them.

S59's annual Orden authority is compiled independently for each filing year from the hash-pinned normative BOE HTML through one production-neutral DOM parser shared with canonical corpus preprocessing. The checked registry artefact requires exactly 49 Annex II IVA activity tables and 141 module rows per 2023-2026 source, table-scoped legal references with semantic Annex II activity anchors, exact source and content digests, and deterministic `.extracted.json`/`.extracted.md` pairs. Hash or ordinal anchors, incomplete sidecars, hand-authored table maps, caller-supplied Orden tuples, raw IAE lookup, cross-year source contamination, extra authority files, or missing and stale manifests refuse. A non-M303 registry does not probe S59; any registry with at least one M303 revision requires the strict authority and cannot substitute an empty catalogue.
