...
modified: '2026-08-11'
body_hash: 'sha256:5a4d14372dc73c24298a960af1797636888bbea78ef321ae6d3072686708b7fa'
---
...
One application author-or-replace operation may write this evidence only while the revision is `BORRADOR`. Verification requires and freezes the complete applicable branch. A verified or filed revision is immutable; changing filing-instance evidence requires a new revision. Existing pre-release M303 revisions without this evidence are invalid and must be recalculated. There is no backfill, tolerant read, or inferred false value.

Export and review-package assembly reload the evidence from the selected revision. Quickfile collects it once before calculation, persists it through the same authoring operation, and then reloads it; no command may pass an authoritative duplicate directly to export. The resolved `M303FilingFacts` snapshot projection composes A19 and A24-A26 from revision evidence, A21-A23 from their canonical arrivals, and A28 from the S51 applicability envelope. The applicability value must equal the resolved revision/export context or assembly refuses.

M303 export and verification refuse missing evidence, a wrong-model branch, a period mismatch, or an S51 disagreement before target creation. Commands may expose typed collection inputs only for revision authoring, never as an export override.
...
- Joint-return and insolvency facts are encrypted calculation-revision evidence with one BORRADOR-only writer and immutable verified/filed replay. Export arguments, defaults, and historical M303 revisions without evidence are not accepted as authority.
