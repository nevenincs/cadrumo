...
- Commit 3 (CLI mount): the `m036_app` Typer subgroup under `aeat app modelo`, the four CLI tests enumerated in the research, and the locale-catalogue keys added via `python -m cadrumo.locales set` for the new verb help and refusal strings across `en` / `es` / `ca` / `hu` per `aeat-locales-cli`.
