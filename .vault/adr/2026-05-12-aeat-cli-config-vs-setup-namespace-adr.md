...
Alias mode. `aeat config` is a thin wrapper that routes keyed operations through the same `cadrumo.application.profile._actions` backend that powers `aeat setup profile set`. One source of truth (the WorkflowState.profiles mapping); two co-equal CLI presentations.
