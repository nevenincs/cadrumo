...
date: "2026-04-21"
modified: '2026-07-17'
body_hash: 'sha256:bd16ffadf85276779755c3fd2bc2178f6831b6bf30f723cedb1a92c435c40cbf'
related:
...
- The `cadrumo.adapters.inbound.borrador` module name (cluster-A taxonomy) covers all three Renta artefact types — borrador, predeclaración, declaración — because they share a summary-block layout for cluster-F purposes.
...
- **`cadrumo.adapters.inbound.borrador` module name** per cluster-A taxonomy.
- **Partial ruleset**: ~5 derivations for the summary block; register under `src/cadrumo/domain/calculations/registry/modelo_100_summary_2025.py`.
...
### 1. `cadrumo.adapters.inbound.borrador` module

```
src/cadrumo/adapters/inbound/borrador/
...
`src/cadrumo/domain/calculations/registry/modelo_100_summary_2025.py`:
...
- `cadrumo.adapters.inbound.borrador` module lands with artefact auto-detection; all three Renta artefact types share one extractor family.
...
- `cadrumo.domain.calculations` gains its first Renta ruleset entry; future anexo rulesets plug into the same registry.
