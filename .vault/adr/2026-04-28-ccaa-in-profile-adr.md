...
`#216` is changing database-backed storage in a sibling branch. This issue must not depend on that moving surface. `aeat.config.Settings` is stable, but pure env-var storage would make a user-facing profile command less natural and would treat identity state as process configuration.
...
Create a new top-level public `aeat.profile` subpackage. It exports `KentTaxResidence`, `ResidenceChange`, `load_tax_residence`, `save_tax_residence`, `clear_tax_residence`, `default_path`, and the profile errors. `KentTaxResidence` carries `schema_version: str = "1"`, `ccaa: CCAA`, `tax_residence_since: date | None`, and `tax_residence_change_history: tuple[ResidenceChange, ...]`.
