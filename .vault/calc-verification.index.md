---
generated: true
tags:
  - '#calc-verification'
date: '2026-04-27'
related:
  - '[[2026-04-21-calc-verification-adr]]'
  - '[[2026-04-21-calc-verification-plan]]'
  - '[[2026-04-21-calc-verification-research]]'
---

# `calc-verification` feature index

Auto-generated index of all documents tagged with `#calc-verification`.

## Documents

### adr

- `2026-04-21-calc-verification-adr` - `calc-verification` adr: `classify-discrepancies-then-produce-kent-readable-verdict` | (**status:** `accepted`)

### plan

- `2026-04-21-calc-verification-plan` - `calc-verification` plan

### research

- `2026-04-21-calc-verification-research` - calc-verification research
