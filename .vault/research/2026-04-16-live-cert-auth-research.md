...
- `aeat.cli.doctor` already has a certificate health row, which is useful as a preflight check for path, password, and expiry, but it is not a live AEAT acceptance test. The live test modules reinforce the same split: `src/aeat/auth/test_certificate_live.py` is a gated handshake smoke test, while `src/aeat/status/test_live.py` is still an intentional skip placeholder until the cert-backed reader path lands.
