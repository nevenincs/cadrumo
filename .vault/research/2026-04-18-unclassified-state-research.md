...
- `src/aeat/financial/transactions/_enums.py:16-22` — `StrEnum` with `BUSINESS`, `PERSONAL`, `MIXED`, `UNCLASSIFIED`.
- `src/aeat/financial/transactions/_models.py:71` — `Transaction.business_classification: BusinessClassification = BusinessClassification.UNCLASSIFIED`.
- `src/aeat/financial/transactions/_models.py:152-163` — `_validate_business_pct` couples `MIXED` to `business_pct ∈ [0,1]`; every non-`MIXED` state enforces `business_pct is None`.

### Classification write-paths

- `src/aeat/financial/transactions/_service.py:117-152` — `set_classification` mutates one transaction into a fresh catalogue entry, stamping `classified_at = datetime.now(UTC)` and `classified_by ∈ {"auto", "manual", "rule:<id>"}`.
- `src/aeat/cli/financial/txs.py:80-120` — `aeat financial txs classify <tx> --as <STATE> --pct 0.5` is the only manual surface; it calls `set_classification` with `classified_by="manual"`.
...
- `src/aeat/cli/financial/txs.py:30-64` — `aeat financial txs list --unclassified` filters to `business_classification is UNCLASSIFIED`.
- `src/aeat/cli/financial/txs.py:67-77` — `aeat financial txs show <tx>` dumps the full transaction JSON.

### Tests that depend on `UNCLASSIFIED`

- `src/aeat/financial/transactions/test_cli.py:36` — default factory classification.
- `src/aeat/financial/transactions/test_cli.py:115` — asserts `payload["business_classification"] == "UNCLASSIFIED"`.
- `src/aeat/financial/transactions/test_catalogue.py:56, 129` — factory default and `set_classification` before-state assertion.
...
1. **Where does `aeat review` live?** There is no `review` top-level app yet; `#204` C4l sketches a future unified review queue. For `#237` the minimal ship is a new `src/aeat/cli/review/` subpackage that hosts `history <tx-id>` today and receives `queue` / `approve` in a follow-up PR.
