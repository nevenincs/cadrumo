...
4. A **round-trip verification pass** running the formula engine's `Engine.audit_against(ruleset, provided, tolerance)` (already present at `src/aeat/formulas/_engine.py:51`) and reporting discrepancies classified by cause (extraction bug / formula bug / un-modelled AEAT rule / rounding).
...
| --- | --- | --- |
| Modelos with `FilingBuilder` | 130, 303, 390 (3 of ~21) | `src/aeat/filing/_builders/` |
| Modelos with formula ruleset | 130 (2024, 2025), 303 (2024, 2025) | `src/aeat/formulas/_rulesets/` |
...
| PDF parsing backends | `pdfplumber` only (text extraction) | `src/aeat/justificante/_parsers/_pdfplumber_backend.py:15` |
| Round-trip verification primitive | `Engine.audit_against` emits `Discrepancy` records | `src/aeat/formulas/_engine.py` |
| `aeat.justificante` scope | Receipt metadata only (CSV + totales + modelo + período) | `src/aeat/justificante/_schema.py` |
...
| --- | --- | --- | --- |
| A | **PDF taxonomy** | Document the AEAT PDF zoo: justificante vs. declaración vs. borrador vs. predeclaración vs. datos fiscales; decide which one(s) each import flow targets; correct the naming drift in `aeat.justificante`. | — |
...
| G | **`aeat.justificante` reframing** | Decide: rename → `aeat.receipts`? narrow docs? keep as-is with explicit scope note? Backwards-compat for #271 amendment baseline must survive. | A |
