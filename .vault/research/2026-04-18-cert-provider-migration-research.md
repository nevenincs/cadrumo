...
## Goal
Locate and analyze the existing `AeatAuthenticator` and `certificate.py` code in `src/aeat/auth/` to prepare for its refactoring into the `AuthProvider` protocol shape.

## Findings

1. **Current Structure**:
   - `src/aeat/auth/_authenticator.py` contains `AeatAuthenticator`, which acts as the single entry point for live AEAT access using certificates. It handles the browser session creation, certificate loading, TLS handshake verification, and login assertion.
   - `src/aeat/auth/certificate.py` and the `src/aeat/auth/_certificate_backends/` directory handle the low-level PKCS#12 certificate parsing, loading, and Playwright integration.
...
3. **Required Actions**:
   - Move `certificate.py` and `_certificate_backends/` into `src/aeat/auth/_providers/_certificate/`.
   - Define `AuthProvider`, `AuthProviderKind`, and related protocols in a new module (e.g., `src/aeat/auth/provider.py` or `_protocols.py`).
