...
`aeat.workflow._models.compute_run_id` exists; nothing in
`aeat.submission`, `aeat.sync`, `aeat.inbox`, `aeat.status` propagates a run
...
- `src/aeat/workflow/_models.py::compute_run_id` hashes
...
- `aeat.cli` already has a `workflow run` subcommand
  (`src/aeat/cli/workflow/run.py`) — the new CLI surface must be a separate
...
- `aeat.errors.AeatError` is the hierarchy root; subclasses carry structured
...
- `feature/117-live-submit-hardening` owns the internals of
  `src/aeat/submission/` and may rename or restructure internal modules. This
...
- `feature/74-transaction-catalogue` owns
  `src/aeat/financial/transactions/` — Track B, no overlap.
- `feature/106-n26-research` is research-only.

`src/aeat/models/`, `src/aeat/financial/providers/`, and `src/aeat/workflow/`
...
- **Q:** How do we compute `cert_fingerprint`?
  **A:** Delegate to `aeat.auth` if available; otherwise record the SHA-256
...
- **Q:** Where do colocated tests live?
  **A:** `src/aeat/observability/test_*.py` — matches the Rust-style
  colocated test convention used elsewhere in the repo
  (e.g. `src/aeat/cli/test_*_cli.py`).
...
- `src/aeat/workflow/_models.py:110-187`
- `src/aeat/cli/__init__.py:45-96`
- `src/aeat/cli/workflow/run.py:13-59`
