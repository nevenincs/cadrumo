...
modified: '2026-08-24'
body_schema: 'body-v1'
body_hash: 'sha256:4c7083427f82351573856d1af78f063577a064ec81ac1f3e567361b24ee6d4a4'
related:
  - "[[2026-08-11-tui-architecture-adr]]"
  - "[[2026-08-24-tui-registry-api-gate-architecture-reconciliation-audit]]"
---

# `tui-operation-observation` research: `Public operation observation amendment`

The accepted TUI architecture already owns operation observation, but its live
application surface returns a persistence-oriented snapshot while progress and
replay state remain in a separate event stream. The unfinished visual cohort
therefore has no frontend-safe, atomic contract to consume. The evidence favors
an application-owned, versioned observation assembled from one snapshot/event
read boundary, with the accepted operation lifecycle and supervisor remaining
authoritative. The ADR must refine that existing authority, delimit supported
interaction and replay behavior, and make the public version and dependency
receipt explicit before `W05.P11` begins.
...
amendment of that accepted decision. `.vault/adr/2026-08-11-tui-architecture-adr.md:116`,
...
### The visual cohort needs a contract receipt before implementation
...
The ADR must settle which public version and conformance evidence make
`W05.P11` executable. The plan—not a parallel plan—must then record a receipt
whose live-tree gate verifies the accepted amendment, source ancestry, exported
public types and methods, atomic observation proof, cursor/resynchronization
proof, independent terminal-axis proof, interaction scope, forbidden DTO
imports, and secret non-retention.
...
The fourth option best fits the existing ownership evidence. The ADR must still
decide its exact public models, interaction boundary, version refusal, and
receipt gate.

### Not investigated

No authenticated browser operation, process cancellation, event compaction, or
slow-consumer load test was executed. No retention policy beyond the current
full-history filesystem adapter was evaluated. The review was read-only at
commit `5c369d46c181160511560c7825b3cc799d125035`; implementation and full-suite
health remain for the canonical plan.
...
- `src/cadrumo/adapters/persistence/operations/_journal_validation.py:25`
- `src/cadrumo/adapters/persistence/operations/_journal_validation.py:31`
- commit `5c369d46c181160511560c7825b3cc799d125035`
