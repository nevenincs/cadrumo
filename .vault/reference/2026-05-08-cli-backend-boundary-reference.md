...
| API-003 | partial-invoice-cli-003-landed | API-GAP | High | `src/aeat/application/invoices`, `src/aeat/domain/invoices/_service.py` | Invoice import and review/match projections are now application services. Remaining gap: atomic reconciliation persistence for `CLI-006`. |
