...
| TEST-006 | partial-invoice-cli-006-link-landed | TEST-FP-META | Medium | `tests/import_contract/domain/invoices/test_cli.py`, `src/aeat/application/invoices/test_linking.py`, `src/aeat/application/invoices/test_reconciliation.py` | Link and reconciliation round-trip behavior are now covered at the application layer. Remaining CLI import-contract tests must be pruned so list/verify/unmatched business assertions live behind backend services. |
...
| API-003 | backend-owned-wrapper-verification | API-GAP | High | `src/aeat/application/invoices`, `src/aeat/domain/invoices/_service.py` | Invoice import, review/match projections, explicit bidirectional link commands, and reconciliation apply persistence are now application services. |
