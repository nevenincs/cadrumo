...
| CLI-003 | partial-import-service-landed | CLI-BL-ORCH | High | `src/aeat/entrypoints/cli/_invoice.py::invoice_import`, `src/aeat/entrypoints/cli/_invoice.py::invoice_review` | Invoice import parsing, defaults, line synthesis, duplicate handling, and import summaries now live in `application.invoices`; the remaining CLI-owned behavior recomputes IVA display totals, derives status, and performs payment matching. |
...
| CLI-006 | deferred-for-backend-gap | CLI-BL-STATE | High | `src/aeat/entrypoints/cli/financial/invoices.py::reconcile_cmd`, `src/aeat/entrypoints/cli/financial/invoices.py::link_cmd` | CLI applies reconciliation suggestions, mutates invoice and transaction catalogues, saves both, and passes dummy `Path()` values into backend. |
...
| TEST-006 | pending | TEST-FP-META | Medium | `tests/import_contract/domain/invoices/test_cli.py` | Tests treat CLI as business owner for invoice list/link/reconcile/verify/unmatched despite existing domain services. |
...
| API-003 | partial-import-service-landed | API-GAP | High | `src/aeat/application/invoices`, `src/aeat/domain/invoices/_service.py` | Invoice flat JSON/CSV import is now an application service. Remaining gaps: review/match projections, IVA display totals, payment status, period-aware matching, and atomic reconciliation persistence. |
