...
| CLI-006 | partial-invoice-cli-006-reconcile-landed | CLI-BL-STATE | High | `src/aeat/entrypoints/cli/financial/invoices.py::reconcile_cmd`, `src/aeat/entrypoints/cli/financial/invoices.py::link_cmd` | `reconcile --apply` now delegates suggestion application and persistence to `application.invoices`. Remaining violation: `link_cmd` passes dummy `Path()` values into backend. |
...
| TEST-006 | partial-invoice-cli-006-reconcile-landed | TEST-FP-META | Medium | `tests/import_contract/domain/invoices/test_cli.py`, `src/aeat/application/invoices/test_reconciliation.py` | Reconciliation round-trip behavior is now covered at the application layer. Remaining CLI import-contract tests must be pruned so list/link/verify/unmatched business assertions live behind backend services. |
...
| API-003 | partial-invoice-cli-006-reconcile-landed | API-GAP | High | `src/aeat/application/invoices`, `src/aeat/domain/invoices/_service.py` | Invoice import, review/match projections, and reconciliation apply persistence are now application services. Remaining gap: explicit application-layer link command input replacing dummy path arguments. |
