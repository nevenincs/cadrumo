...
| CLI-003 | deferred-for-backend-gap | CLI-BL-ORCH | High | `src/aeat/entrypoints/cli/_invoice.py::invoice_import`, `src/aeat/entrypoints/cli/_invoice.py::_parse_invoice_payload`, `src/aeat/entrypoints/cli/_invoice.py::invoice_review` | CLI parses JSON/CSV invoice payloads, supplies defaults, synthesizes lines, merges duplicates, recomputes IVA display totals, derives status, and performs payment matching. |
...
| API-003 | deferred-for-backend-gap | API-GAP | High | `src/aeat/domain/invoices/_service.py` | Invoice workflows need application services for flat JSON/CSV import, review/match projections, IVA display totals, payment status, period-aware matching, and atomic reconciliation persistence. |
