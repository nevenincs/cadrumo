...
This document is the authoritative human-readable publication surface for the `LedgerCapabilityMatrixV1` campaign contract in `dev/quality/clitui_ledger_capability_matrix.py`. S08 now serializes the exact S04-S07 union as `LedgerUnionDenominatorV1`: 760 raw observations produce 769 observation-to-row selections and 693 semantic rows. The publication remains a **provisional baseline** rather than an accepted campaign matrix: S09 has not recorded the cross-plan hold control, S11-S13 have not completed row-level hold and review enforcement, and S14 has not accepted the frozen denominator. The contract therefore publishes the S08 union digest while continuing to refuse a matrix digest, acceptance attestation, or G0 closure.
...
| Denominator revision / digest | `s08-semantic-union-4` / `sha256:8119bedd03babf7b1400bf310cd9e847b1f7f472a671b6ccb482cc583ad9a3bb` |
...
`build_ledger_union_denominator` is the reproducible S08 projection. It consumes the live command census, the authored 63-operation S05 register plus its 21-file source set, the ten missing-product observations, the validated 546-row registry census, six artifact-product observations, and the live seven-route TUI census. The schema-v2 envelope embeds the validated registry and TUI projections. An exact committed mapping binds `(source, observation identity)` to ordered semantic-row tuples for all 214 non-registry observations; any added, removed, duplicated, relabelled, reordered, or changed observation fails closed even when it reuses an existing capability identity. The 546 registry keys and selections are rederived from the embedded canonical route census rather than duplicated as business authority. Validation recomputes exact source counts and every source digest from the canonical observations and underlying projections, then recomputes row memberships, row sources, accounting, and the aggregate digest. Serialization uses the domain `cadrumo:ledger-union-denominator:v2`, a NUL terminator, unsigned-eight-byte-big-endian payload length, canonical ASCII JSON, and SHA-256.
...
| **Union** | **760 observations / 769 selected edges / 693 semantic rows** | `sha256:8119bedd03babf7b1400bf310cd9e847b1f7f472a671b6ccb482cc583ad9a3bb` |
...
| `evidence.s08.semantic_union` | `build_ledger_union_denominator` in `dev/quality/clitui_ledger_capability_matrix.py` | `sha256:8119bedd03babf7b1400bf310cd9e847b1f7f472a671b6ccb482cc583ad9a3bb` | Schema-v2 domain-separated union: embedded registry/TUI projections; canonical authority for all 760 source/identity/selection tuples; recomputed source counts, source digests, row memberships, row sources, accounting, and aggregate digest |
