...
modified: '2026-09-05'
body_schema: 'body-v2'
body_hash: 'sha256:689bc044fc01b892a203e4ecc22cdce3e60487da8406ad835f7b6091834674fc'
related:
...
| `backend_only` | 64 public frontend-neutral operations are catalogued below; 14 are lower-level backend-helper-only rows, while the installed workspace read/projection remains the one backend-only product capability absent from the CLI census | Complete current backend-operation stream; all observations have S08 semantic rows | S05 and S08 complete |
...
| **Union** | **761 observations / 770 selected edges / 694 reviewed semantic rows; 680 TUI-applicable rows held until G3; 14 TUI-not-applicable rows unheld** | refreshed from the stable candidate tree |
...
The following prose coordinates are the human-readable publication of the canonical matrix and TUI census. Their values are derived from the live projections and are guarded against omission, duplication, or arithmetic drift by the governance harness.
...
Production composition injects exactly two read `ActionReference`s, `operator.ledger.review` and `operator.ledger.evidence.review.list`, and reads the evidence review queue at screen-factory invocation. It also passes the classification `ActionReference` `operator.ledger.classify`, but production supplies neither a classification target nor a submitter, so that reference is inert. It injects zero executable mutation doors: no classification submitter, import submitter, or link submitter. The component harness separately supplies synthetic doors for classification, one-file import, and invoice/transaction linking; those tests prove frontend behavior behind an injected contract, not production enrollment. None of the 78 CLI invocables changes this verdict: all 78 command specs independently declare `TuiCapability.NOT_IMPLEMENTED`, which is CLI global-`--tui` metadata rather than evidence that a similarly named TUI component is installed.
