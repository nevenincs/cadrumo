...
date: '2026-07-14'
modified: '2026-07-17'
body_hash: 'sha256:344b5a259e1b51cc9ad95ccdba4d23f1b939dd116160b7043207823e4a7c3cc3'
step_id: 'S21'
related:
  - "[[2026-07-14-calculation-export-import-adjudication-plan]]"
---

# Gate Modelo 353 declaration extraction on a sanitized filed specimen and prohibit speculative profile authoring
...
- Reconcile the Modelo 353 declaration-PDF candidate against the accepted live
  filed-declaration fallback mandate rather than the legacy fixture checkbox.
- Inspect the canonical Modelo 353 revision, exact registered source windows,
  extraction-profile data, shared declaration parser, committed specimens, and
  real Modelo 353 tests.
...
### Modelo 353 declaration PDF, exercises 2008 through 2025

- **Candidate:** Modelo 353 declaration-PDF extraction for the monthly revision
...
- **Mandate:** `proven`; the accepted live filed-declaration data-capture ADR
  ...
  legacy unchecked “Modelo 353 live/read fixture evidence” row is corroborating
  backlog history, not the mandate source.
- **Exact official authority:** `missing`; `boe-modelo-353-2007-form`
  establishes the legal form from `2007-11-30`, but the only registered
  machine-readable record design, `aeat-dr-353-2026`, applies from
...
- **Canonical implementation state:** `gap`; `parse_declaracion_bytes` and exact
  ...
  Modelo 353 revision contains no `extraction_profiles` data and therefore
...
- **Real evidence or specimen:** `missing`; repository discovery found the
  official 2026 XLSX and a 2024 group-aggregation manual-oracle JSON, but no
  sanitized filed Modelo 353 declaration PDF for exercises 2008 through 2025.
  A record design, BOE form specification, or calculation oracle is not filed
  declaration bytes.
- **Retirement:** `false`; no accepted retirement or successor decision was
  found for Modelo 353.
...
### Modelo 353 declaration PDF, 2026-01-01 and following
...
- **Exact official authority:** `available` for the candidate's applicability
  ...
  `aeat-dr-353-2026` applies from `2026-01-01` and is referenced by the active
...
- **Canonical implementation state:** `gap`; the canonical generic declaration
  ...
  while Modelo 353 has no extraction profile. The required addition, if the
  evidence gate is later cleared, is reviewed registry profile data and real
  corpus coverage—not another parser path.
- **Real evidence or specimen:** `missing`; no sanitized filed Modelo 353
  ...
  workbook parity, and IVA aggregation bindings. The application continuity
  and 2024 manual-worked-example suites exercise group aggregation and
  calculation behavior, not extraction from real Modelo 353 PDF bytes; no
  Modelo 353 declaration-parser corpus test exists.
- **Retirement:** `false`; no accepted retirement or successor decision was
  found for Modelo 353.
- **Evidence block:** `true`; a sanitized filed exercise-2026-or-later Modelo
  353 declaration PDF is unavailable.
- **Four-condition gate:** `mandate_met = true`,
...
- Direct source inspection confirmed the `2008-y-siguientes` revision begins on
  `2008-01-01`, source `aeat-dr-353-2026` begins on `2026-01-01`, and the
  Modelo 353 registry tree has no `extraction_profiles` directory.
- Repository specimen and parser-test searches found no sanitized filed Modelo
  353 PDF and no Modelo 353 declaration-parser test. The committed
  `test_modelo_353_registry.py`,
  `test_modelo_353_grupo_aggregation_continuity.py`, and
  `test_m353_2024_grupo_entidades_manual_worked_example.py` suites are real
  Modelo 353 behavioral coverage, but they do not evidence PDF extraction.
