...
date: '2026-07-14'
modified: '2026-07-17'
body_hash: 'sha256:2abb4f5c5a155642dc3c92662077d2f66007d153b45d4afaaa7461de3946462c'
step_id: 'S12'
related:
...
`src/cadrumo/_data/registry/aeat/modelos/353/revisions/2008-y-siguientes/revision.toml`
uses `valid_from = 2008-01-01` with the monthly period selector. Confirm that
the reviewed `aeat-dr-353-2026` XLSX record design in
...
353 machine-file generation; the legacy goal asks for export/file layout
support only "where required," a conditional pointer, not a mandate.

Inspect the Modelo 353 registry and the generic export engine. The revision
...
exposed. No real Modelo 353 golden outbound payload or mutation-sensitive
export/parse round trip exists; the mutation-sensitive fichero-BOE suite
covers Modelos 130, 303, and 390 only.

## Outcome

### `modelo-353-outbound-2026-open` | `mandate-gated`

- **Candidate:** Modelo 353 outbound fichero generation for monthly filings
  from `2026-01-01` with an open end under revision `2008-y-siguientes`.
- **Mandate:** `conditional`. The legacy goal names Modelo 353 export/file
  layout support only "where required" and no accepted decision or explicit
  current product goal confirms that requirement yet.
- **Exact authority window:** `aeat-dr-353-2026` is a reviewed AEAT XLSX
  record design registered from `2026-01-01` with an open end and covers
  this candidate window exactly.
- **Canonical implementation state:** `gap` only in optional Modelo 353
...
- **Real evidence or specimen:** `missing`. The reviewed 2026 record-design
  XLSX is available as authority, but no real Modelo 353 golden outbound
  payload or mutation-sensitive round trip exists.
- **Retirement:** `false`.
- **Evidence block:** `true`; taxonomy precedence selects `mandate-gated`
  before any evidence-gated state because the mandate is unresolved.
...
- **Next action:** confirm an explicit current product decision for Modelo
  353 outbound machine-file generation before authoring any registry layout,
  renderer, parser, or golden test.

### `modelo-353-outbound-2008-2025` | `mandate-gated`

- **Candidate:** Modelo 353 outbound fichero generation for filing years
...
- **Exact authority window:** `missing`. `aeat-dr-353-2026` starts on
...
- **Real evidence or specimen:** `missing`; no exact-window official record
  design and no real Modelo 353 golden outbound payload exist for 2008-2025.
- **Retirement:** `false`.
- **Evidence block:** `true`.
...
- Grounding used the accepted adjudication contract, exact Modelo 353
  registry/legal files, generic exporter boundaries, and the existing
  reference register entry for Modelo 353.
