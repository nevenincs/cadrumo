...
# `P03.S02` — define `SyncStateRow` pydantic record + `SyncStateStatus` enum

## Scope

- `src/aeat/application/storage/sync/_records.py` placeholders below are machine-filled
     by `vaultspec-core vault add exec` from the originating Step row;
     do not fill them by hand. -->

# define `SyncStateRow` pydantic record + `SyncStateStatus` enum

## Scope

- `src/aeat/application/storage/sync/_records.py`

## Description

- Reconcile `P03.S02` against the accepted July architecture and current code.
