...
# `P03.S15` — implement filename surface form `<hmac_prefix_8>--<label>.<ext>` on push

## Scope

- `rename detection on label drift`
- `src/aeat/application/storage/sync/_coordinator.py` placeholders below are machine-filled
     by `vaultspec-core vault add exec` from the originating Step row;
     do not fill them by hand. -->

# implement filename surface form `<hmac_prefix_8>--<label>.<ext>` on push
...
- Reconcile `P03.S15` against the accepted July architecture and current code.
