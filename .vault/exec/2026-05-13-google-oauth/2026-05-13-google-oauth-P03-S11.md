...
# `P03.S11` — implement `aeat config google sync orphans [--format json|text]` Typer command

## Scope

- `src/aeat/entrypoints/cli/_config/_google.py` placeholders below are machine-filled
     by `vaultspec-core vault add exec` from the originating Step row;
     do not fill them by hand. -->

# implement `aeat config google sync orphans [--format json|text]` Typer command
...
- Reconcile `P03.S11` against the accepted July architecture and current code.
