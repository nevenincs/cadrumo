...
- **Feasibility: FEASIBLE** — printed labels confirmed from casilla label text and
  instructions; label patterns derived from registry casilla labels
...
- **Feasibility: FEASIBLE** — `decl.ejercicio` and `decl.periodo` can be targeted
  with `named_label`; IVA casillas have numeric slugs that are not
  printed literally on the declaration summary page so numeric matching
  would also not work — `named_label` on the header fields is the right scope
