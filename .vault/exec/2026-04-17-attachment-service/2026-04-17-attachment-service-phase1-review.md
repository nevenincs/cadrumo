...
5. **Subpackage discipline.** No external deep imports into `aeat.financial.attachments._*`. `_stubs.py` declares typing-only Protocols with no runtime imports. External callers go through `aeat.financial.attachments.__init__`.
