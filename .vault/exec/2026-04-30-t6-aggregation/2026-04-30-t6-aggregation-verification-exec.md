...
- `uv run --no-sync pytest src/aeat/financial/aggregation/test_aggregation.py src/aeat/cli/test_json_schema_conformance.py src/aeat/errors/test_registry.py src/aeat/errors/test_registry_enforcement.py -q` — 45 passed after adding the workflow inputs-provider handoff regression.
...
- `just test-cov` — 5003 passed, 19 skipped, 24 deselected; total coverage 82.57%, above the 60% floor.
