...
Collect-only is clean and the campaign surface is fully green: after the marker/import conformance commits, every failure that touched a substrate-owned file passes (marker integrity rows, relative-imports gate, copy-assembly and localized-failure suites re-run green). The remaining suite reds are owner-triaged below; the step stays open for a settled-tree re-run once the peer campaigns land.
...
- Not individually reproduced: a tail of inventory/ratchet gates (size budgets, hashing adoption, registry reviewability, record-design corpus, invoice validators, and similar repo-wide sweeps) red in the parallel run; they scan the whole working tree and are expected to settle with the peer landings. They are queued for the settled-tree re-run that closes this step.
