...
<!-- FRONTMATTER RULES:
     ...
     The S04 and 2026-07-08-mcp-protocol-hardening-plan placeholders are machine-filled by
...
<!-- STEP RECORD:
     ...
     The Add real-behaviour runtime tests: a deliberately slow subprocess hits its tier timeout, cancellation terminates the full process tree on Windows, and the refusal names the tier and retry guidance and ## Scope

- `src/aeat/entrypoints/mcp/tests/test_call_runtime.py` placeholders below are machine-filled
     by `vaultspec-core vault add exec` from the originating Step row;
     do not fill them by hand. -->

# Add real-behaviour runtime tests: a deliberately slow subprocess hits its tier timeout, cancellation terminates the full process tree on Windows, and the refusal names the tier and retry guidance

## Scope

- `src/aeat/entrypoints/mcp/tests/test_call_runtime.py`
