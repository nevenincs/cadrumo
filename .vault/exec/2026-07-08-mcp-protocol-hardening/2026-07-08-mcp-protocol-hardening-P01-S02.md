...
<!-- FRONTMATTER RULES:
     ...
     The S02 and 2026-07-08-mcp-protocol-hardening-plan placeholders are machine-filled by
...
<!-- STEP RECORD:
     ...
     The Route the direct and meta call paths through the supervised runner and emit notifications/progress heartbeats (elapsed plus coarse stage) when the client supplied a progress token and ## Scope

- `src/aeat/entrypoints/mcp/_server.py` placeholders below are machine-filled
     by `vaultspec-core vault add exec` from the originating Step row;
     do not fill them by hand. -->

# Route the direct and meta call paths through the supervised runner and emit notifications/progress heartbeats (elapsed plus coarse stage) when the client supplied a progress token
