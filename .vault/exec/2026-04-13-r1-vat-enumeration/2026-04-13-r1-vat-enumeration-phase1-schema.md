...
- Created `src/aeat/financial/__init__.py` as a minimal subpackage
  root (docstring + empty `__all__`). No cross-subpackage re-exports.
- Created `src/aeat/financial/vat/_schema.py` with strict pydantic v2
  ...
  validator.
- Created `src/aeat/financial/vat/errors.py` with the `VatError`
  hierarchy rooted at `aeat.errors.AeatError`.
...
- `src/aeat/financial/__init__.py` (new)
- `src/aeat/financial/vat/__init__.py` (new — see phase 2)
- `src/aeat/financial/vat/_schema.py` (new)
- `src/aeat/financial/vat/errors.py` (new)
- `src/aeat/config.py`
- `env/.env.example`
