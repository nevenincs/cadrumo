...
The output-rendering tests now exercise the public `render_command_output` API with real sensitive canaries in text lines and nested JSON payloads. Coverage asserts raw profile ids, tax ids, bearer tokens, URLs, and secure-object keys do not reach rendered output, while normal project types such as `Path`, `Decimal`, and `date` remain serialized in the existing JSON shape.
