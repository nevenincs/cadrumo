...
<!-- FRONTMATTER RULES:
     ...
     The S62 and 2026-05-19-live-iva-compensation-wallet-plan placeholders are machine-filled by
...
<!-- STEP RECORD:
     ...
     The Add secure-storage roundtrip tests for persisted remote IVA evidence using `aeat.tests.secure_sql` and `Settings.aeat_dev_test_database_password`. Completed 2026-05-27: persisted Cl@ve session metadata has isolated runtime-profile storage coverage, and remote IVA filed-history state, wallet observation, and reconciliation decision now roundtrip through profile secure SQL using `aeat.tests.secure_sql` without private taxpayer fixtures and ## Scope

- `src/aeat/application/live src/aeat/tests` placeholders below are machine-filled
