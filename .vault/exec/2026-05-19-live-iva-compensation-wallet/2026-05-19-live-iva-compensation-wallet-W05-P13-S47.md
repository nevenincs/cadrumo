...
<!-- FRONTMATTER RULES:
     ...
     The S47 and 2026-05-19-live-iva-compensation-wallet-plan placeholders are machine-filled by
...
<!-- STEP RECORD:
     ...
     The Enforce the operator-facing auth wait window from centralized settings, with a production default not exceeding 120 seconds for Cl@ve approval waits and ## Scope

- `src/aeat/core/config.py src/aeat/adapters/outbound/aeat/auth` placeholders below are machine-filled
     by `vaultspec-core vault add exec` from the originating Step row;
     do not fill them by hand. -->

# Enforce the operator-facing auth wait window from centralized settings, with a production default not exceeding 120 seconds for Cl@ve approval waits

## Scope

- `src/aeat/core/config.py src/aeat/adapters/outbound/aeat/auth`
