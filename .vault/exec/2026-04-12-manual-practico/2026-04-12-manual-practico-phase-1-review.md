...
related:
  ...
  - "[[2026-04-12-manual-practico-phase-1-summary]]"
---
...
- src/aeat/manuals/__init__.py
- src/aeat/manuals/_schema.py
- src/aeat/manuals/_loader.py
- src/aeat/manuals/_verify.py
- src/aeat/manuals/_fetch.py
- src/aeat/manuals/_ids.py
- src/aeat/manuals/_stubs.py
- src/aeat/manuals/errors.py
...
- src/aeat/cli/manual.py
- src/aeat/cli/test_manual_cli.py
...
- Public API discipline — __init__.py re-exports the full documented surface and the __all__ list matches. No cross-package imports reach into aeat.manuals._schema / _loader / _verify / _fetch / _ids / _stubs. src/aeat/cli/manual.py imports only from aeat.manuals and aeat.manuals.errors (allowed by brief). _stubs is not re-exported.
...
- Protocol stubs inert — _stubs.py is imported only by _schema.py (for MODELO_CASILLA_PATTERN). Nothing in aeat.manuals instantiates or calls any of the four Protocols at runtime. All four classes carry pragma no cover stub.
- Sibling territory respect — No diff under src/aeat/corpus/, src/aeat/models/, src/aeat/storage/, pyproject.toml, root conftest.py, or tests/README.md. src/aeat/llm/ does not exist on this branch. tests/test_config.py change is exactly one line (adding encoding=utf-8 on the env read).
...
- Dev loop green — Re-verified locally on this branch: uv run ruff check on touched paths is clean; uv run ty check src/aeat is clean; uv run pytest src/aeat/manuals src/aeat/cli/test_manual_cli.py tests/test_config.py returned 45 passed in 1.01s. I did not re-run just hooks end-to-end.
...
- When #21 lands, replace _stubs.LLMClientProtocol / TranslatorProtocol / BulkTranslatorProtocol with the real aeat.llm surfaces and swap the three pending commands (structure, extract-rules, translate) onto real implementations. Land Layer C one chapter at a time with a real reviewed_by.
...
- When #6 (modelo identifiers) lands, replace _stubs.MODELO_CASILLA_PATTERN with a direct aeat.models.ModeloId cross-reference in _schema._CasillaRef.
