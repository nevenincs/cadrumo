...
<!-- FRONTMATTER RULES:
     ...
     The S09 and 2026-05-27-m210-irnr-phase-2-engine-plan placeholders are machine-filled by
...
<!-- STEP RECORD:
     ...
     The FETCH-GATED (fetch: per-treaty BOE consolidated convenio texts for FR/PT/US/NL/BE) - author tranche-1 Convenio corpus, `legal/irnr.toml` entries, and `treaties/es-XX.toml` rows keyed by `TipoRentaIrnr` with typed `ConvenioOverrideKind`, pinned by continuity parity tests and ## Scope

- `src/aeat/_data/registry/aeat/treaties` placeholders below are machine-filled
