...
- `uv run python -m aeat.mcp.launch_google_workspace --single-user --transport streamable-http` successfully exposed an MCP endpoint at `http://127.0.0.1:8000/mcp`, and an MCP client session completed `initialize` and `tools/list` against the launcher-backed server.
