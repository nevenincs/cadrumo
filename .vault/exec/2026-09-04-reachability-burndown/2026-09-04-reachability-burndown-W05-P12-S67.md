...
Neither entry was wired, and in both cases wiring would have been wrong. The
localizable-field union has no correct annotation target: every site is narrower
by design, so applying it would widen a contract to accept kinds the surface
must refuse. The art-108 threshold has no value to compare against: the register
delegates eligibility to an operator flag and stores the input IVA borne rather
than the acquisition value, so the remedy is a schema change and a decision
about who judges the article, not a call site.
