...
- `src/aeat/justificante/_schema.py`, `src/aeat/justificante/_extract.py` — expose `ejercicio` on `Justificante`.
- `src/aeat/justificante/test_parser.py` — assertions for the new `ejercicio` field across the three fixture PDFs.
- `src/aeat/filing/_errors.py` — add `FilingImportError`.
- `src/aeat/filing/_import.py` — new module: `import_filing_from_justificante`, `JustificanteImportResult`, `_normalise_period`, `_build_submission_record`, `_EMPTY_CASILLA_WARNING`.
- `src/aeat/filing/__init__.py` — re-export the import API.
- `src/aeat/filing/test_import.py` — unit coverage for the import path, period canonicaliser, unsupported-modelo error, missing-PDF error.
- `src/aeat/cli/filing/__init__.py` — new `import` Typer subcommand mapped onto the import helper.
- `src/aeat/cli/filing/test_filing_cli.py` — CLI smoke tests for the import command (happy path, missing PDF, unsupported modelo).
- `src/aeat/auth/__init__.py` — **pre-existing bug unblocker**: re-export `AuthProviderDescription` from `._protocols` so `aeat.submission` can import it (no functional change; restores the `aeat.filing → aeat.submission` import chain main was already broken on).
...
- `uv run pytest src/aeat/filing/ src/aeat/justificante/ src/aeat/cli/filing/ -m unit` — 88 passed.
- `uv run pytest src/aeat/submission/ -m unit` — 44 passed (no regressions).
...
- [x] No cert auth is required at any point — the code path never touches `aeat.auth` or the submission engine's transport.
