...
- DEFERRED (locale, pending serialization): dropping the orphaned `cli.ledger.link.evidence_id_help` / `cli.ledger.link.errors.missing_target` keys and updating the `invoice_not_found` / `help` values still referencing `--evidence-id`. The four `en/es/ca/hu.yml` files currently carry exec-authcert-p04's live uncommitted passphrase/recovery locale WIP (disjoint keys), and the locales CLI does a whole-file read-modify-write, so a naive edit would sweep the peer WIP. Pinged team-lead to serialize (peer commits their locale WIP first, then this link-locale cleanup lands own-keys-only). Until it lands, the codebase-to-locale parity gate carries two extra owner-attributed link keys on top of its pre-existing 9-missing-key peer drift.
