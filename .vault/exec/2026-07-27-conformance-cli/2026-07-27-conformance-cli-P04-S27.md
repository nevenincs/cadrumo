...
<!-- FRONTMATTER RULES:
     ...
     The S27 and 2026-07-27-conformance-cli-plan placeholders are machine-filled by
...
<!-- STEP RECORD:
     ...
     The widen the dev-path literal detection to the realistic PROJECT_ROOT join, os.path.join, f-string and backslash forms, invert the test that pins the hole open, and mirror the missing shipped conftest case, remediating the review finding dev-path-literal-hole and ## Scope

- `src/cadrumo/tests/test_dev_path_isolation.py` placeholders below are machine-filled
