...
- Add real authenticator tests that drive the invalidation helper and login-probe failure path directly, asserting that rendered errors, structured output, and captured logs do not expose sensitive basenames, absolute paths, or exception payload text.
...
- `uv run pytest -q src/aeat/adapters/outbound/aeat/auth/test_authenticator.py` passed: 43 passed.
