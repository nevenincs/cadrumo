...
Implemented Path A by changing `src/aeat/auth/test_clave_movil.py` from `pytest.mark.unit` to `pytest.mark.live_read` while preserving `pytest.mark.domain_aeat_remote`.
...
Documentation cleanup: searched `justfile`, `.github/workflows`, `docs`, `tests/README.md`, `src/aeat`, and `.vaultspec/rules/rules`; no `--ignore=src/aeat/auth/test_clave_movil.py` reference exists in this checkout.
...
- `uv run ruff check src/aeat/auth/test_clave_movil.py`: passed.
- `AEAT_LIVE_TESTS_ENABLED=0 uv run --no-sync pytest -m live_read src/aeat/auth/test_clave_movil.py -q`: 14 skipped.
- `AEAT_LIVE_TESTS_ENABLED=1 uv run --no-sync pytest -m live_read src/aeat/auth/test_clave_movil.py -q`: 14 passed against the current protocol stand-in.
- `just lint`: passed.
