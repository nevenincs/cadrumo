...
- `src/aeat/submission/_preflight.py` — four-gate `Preflight` validator
  (draft-ready, no-ERROR-findings, window-open, cert-loads) with INFO
  logging on every gate outcome.
- `src/aeat/submission/_engine.py` — `SubmissionEngine.submit_draft`
...
- `uv run pytest src/aeat/submission/test_preflight.py src/aeat/submission/test_engine.py -q` — passed.
