...
date: '2026-07-28'
modified: '2026-07-28'
body_hash: 'sha256:b22482574c089807cdb0243e5a48729c1c92b7e89cda451bd9ac55787125297c'
step_id: 'S297'
related:
...
COMPLETE — the full in-surface set is read and classified; every genuine corpus-scanning emptiness gate is floored and mutation-verified.

Arithmetic across the three passes (a finding in its own right): roughly 17 genuine gates were floored, and on the order of 90 candidates were excluded as already-controlled, each with a stated mechanism. That is a false-positive rate near 90 percent for the screening heuristic. The original "87 in-surface candidates" was never 87 defects; anyone re-running the ad-hoc scanner or the canonical vacuity screen later must read each hit before editing, because the overwhelming majority are already floored by a sibling, self-flooring, a detector control, or a behavioural assertion over an ephemeral corpus. The exclusion reasoning below is the durable deliverable of this Step, not the floors.

The one previously peer-blocked gate, `test_typer_help_sources_are_direct_translations`, is now floored: after peer commit `97e87b339f` wrapped the four bare `help=` literals in `src/cadrumo/locales/cli.py` through `tr()` (scaffolded and translated across all four catalogues — sixteen leaves with real Spanish, Catalan, and Hungarian rather than an intentional-identical allowlist entry, which the translation-honesty ratchet correctly refuses for untranslated strings), the gate is green. Its scan path is re-anchored from the relative `Path("src/cadrumo")` to `PROJECT_ROOT / "src" / "cadrumo"` and given a `scanned > 500` floor; mutation-checked by collapsing the module's `PROJECT_ROOT` binding, which fires at 0.
...
- Third pass surfaced a live peer regression as a side effect, since resolved: `test_typer_help_sources_are_direct_translations` was RED at HEAD because peer commit `a8dbc4f91a` ("refactor(locales): split the five locale-surface complexity hotspots", 2026-07-28) introduced four plain `help=` string literals in `src/cadrumo/locales/cli.py` (lines 136/140/144/148) instead of `tr(...)` calls. That gate also carried a genuine latent vacuity — its corpus was the relative path `Path("src/cadrumo")`, which scans nothing whenever the working directory is not the repo root (the root-literal fragility this campaign hunts). Peer commit `97e87b339f` wrapped the four strings through `tr()` and translated the sixteen resulting leaves across all four catalogues, greening the gate; this Step then re-anchored the scan on `PROJECT_ROOT` and added the floor.
- RESIDUE: none in-surface. The full in-surface candidate set has been read and classified across three passes — the CLI/entrypoints walk-gates, the `src/cadrumo/tests/` tranche, the `dev/audit` gates, the CLI conformance gates, and the `application/user_profile` and `bucket_maintenance` candidates. Every genuine corpus-scanning emptiness gate is floored and mutation-verified. The `dev/docs`, `dev/packaging`, and `dev/deploy` candidates are peer surfaces and were explicitly out of this campaign's scope by direction; they are the only unaddressed part of the tree-wide screen output and belong to their owning campaigns.
