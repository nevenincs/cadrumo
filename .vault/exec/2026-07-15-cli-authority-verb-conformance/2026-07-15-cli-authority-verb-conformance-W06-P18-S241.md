...
date: '2026-07-25'
modified: '2026-07-25'
body_hash: 'sha256:00a191791e29425bad323e50e58a3c187e798438110722b6f27a9e11fc43663d'
step_id: 'S241'
related:
...
Verdict: SATISFIED. Count expanded to 2853 after corrected scope paths and new committed test files.

The original command used several paths that no longer exist at current HEAD due to package relocations. Corrected paths applied: `src/cadrumo/adapters/inbound/declaracion/tests/` (was `domain/declaracion/`), `src/cadrumo/adapters/outbound/aeat/auth/tests/`, `src/cadrumo/adapters/outbound/aeat/browser/tests/`, `src/cadrumo/adapters/outbound/aeat/export/tests/`, `src/cadrumo/adapters/outbound/aeat/sede/tests/`, `src/cadrumo/adapters/outbound/aeat/verify/tests/` (were `adapters/outbound/aeat/tests/`), `src/cadrumo/domain/manuals/tests/` (was `application/manuals/tests/`), `src/cadrumo/domain/submission/tests/` (was `application/submission/tests/`), `src/cadrumo/application/storage/calc_sheets/tests/` (was `application/calc_sheets/tests/`), `src/cadrumo/application/aggregation/tests/` (was `domain/aggregation/tests/`), `src/cadrumo/agent/eval/tests/` (was `application/agent_evaluation/tests/`). Both new in-flight files are now committed and included.

Command: `uv run --no-sync pytest -q -p no:cacheprovider -n auto --dist=loadfile --tb=no --no-header -m "(unit or integration) and not serial and not os_keychain and not external_tool and not perf"` over the corrected 23-directory scope plus the two new test files.

Collected 2853, passed 2853, failed 0, skipped 0. Exit line: `2853 passed, 6 warnings in 777.84s`, exit code 0. One serial test was held by xdist (`test_iva_quarterly_aggregation_partitioned_p95_latency` in `application/aggregation/tests/`); that is a performance benchmark that correctly requires `-n0`, not a failure. HEAD at run time: `9c4b780e1aed5c41938e16eaed2eccdcbddd3cfd`. The hashing subject remains fully satisfied; the expanded count reflects corrected paths and new committed tests.
