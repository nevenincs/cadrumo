...
<!-- FRONTMATTER RULES:
     ...
     The S02 and 2026-07-01-determinism-replay-residual-plan placeholders are machine-filled by
...
<!-- STEP RECORD:
     ...
     The Content-address evidence_id and invoice_id via a clock-free content digest over identifying fields with genuine-duplicate disambiguation and ## Scope
...
- `src/aeat/core/observability/_golden.py` placeholders below are machine-filled
...
- The ledger-evidence `--format json` golden scenario paired with this decision landed as a completing commit: `entrypoints/cli/tests/test_evidence_add_golden.py` captures `ledger.evidence.add`'s envelope across two fresh-bucket runs (same synthetic PDF, same fields, same injected profile identity, same frozen instant) and asserts a byte-identical envelope with ZERO residual differing fields — so the content-addressed evidence_id needs no mask, and the parent anti-tautology proof stays exactly {snapshot_id, run_id} (re-run green). Placed at the entrypoints test surface (not application/ledger/tests) so the registered `EvidenceAddResult` schema is a same-package import, not a cross-package private one — the same placement rationale as the D4 axis. This is distinct from the D4 axis's ledger-add retried-no-op case.
