...
- New subpackage `src/aeat/filing/` with the strict pydantic v2
...
- Test-grade pydantic doubles in `aeat.filing.testing` (synthetic
...
| Errors inherit from `aeat.errors.AeatError` | ✅ `FilingDraftError`, `FilingBuilderError`, `FilingValidationError`, `FilingComputationError`. |
| Logging via `aeat.logging.get_logger(__name__)` | ✅ `_builders/modelo_130.py`, `_validator.py`, `__init__.py`, CLI. |
| Public API discipline — callers import from `aeat.filing` only | ✅ `_builders/`, `_builder.py`, `_validator.py`, `_schema.py`, `_protocols.py`, `_errors.py` are private. CLI imports the public surface only. |
...
| No mocks/patches/fakes/stubs in tests | ✅ Tests build real pydantic doubles via `aeat.filing.testing`. CLI tests use `monkeypatch.setenv` only to point at a temporary `AEAT_DRAFTS_DIR`. |
| Sibling-branch isolation (Protocol stubs for #6/#9/#23/#38) | ✅ No imports from `aeat.models`, `aeat.schema`, `aeat.casillas`, `aeat.deadlines`. Each is mediated by a Protocol. |

### Files changed

- `src/aeat/filing/__init__.py` (new)
- `src/aeat/filing/_schema.py` (new)
- `src/aeat/filing/_errors.py` (new)
- `src/aeat/filing/_protocols.py` (new)
- `src/aeat/filing/_builder.py` (new)
- `src/aeat/filing/_validator.py` (new)
- `src/aeat/filing/_builders/__init__.py` (new)
- `src/aeat/filing/_builders/_modelo_130_schema.py` (new)
- `src/aeat/filing/_builders/modelo_130.py` (new)
- `src/aeat/filing/testing.py` (new)
- `src/aeat/filing/test_filing.py` (new)
- `src/aeat/cli/filing/__init__.py` (new)
- `src/aeat/cli/filing/test_filing_cli.py` (new)
- `src/aeat/cli/__init__.py` (wire `filing` sub-app)
...
- Replace `aeat.filing._protocols` with hard imports once the
...
- Wire `aeat.filing` into `aeat.storage` (#10) once the storage
  layer can persist arbitrary frozen pydantic models.
- Replace `aeat.filing.testing.default_schema_provider()` with the
  real `aeat.casillas` provider once #23 is merged; the CLI
