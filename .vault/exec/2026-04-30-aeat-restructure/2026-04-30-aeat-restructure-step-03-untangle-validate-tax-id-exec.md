...
- `_validators.py` local definition removed; all callers updated to import directly from `aeat.adapters.inbound.identity` in this same change-set.
...
- Existing `aeat/domain/financial/invoices/test_validators.py` was updated to import from `aeat.adapters.inbound.identity` directly; all tests pass.
