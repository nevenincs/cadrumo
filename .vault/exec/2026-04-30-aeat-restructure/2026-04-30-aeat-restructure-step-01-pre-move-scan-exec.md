...
| 1.4 | `pyproject.toml:75` — `[project.scripts] aeat = "aeat.entrypoints.cli:app"` | console-script entry point pinning `aeat.entrypoints.cli` import path | **FIX** — hard-cutover: `aeat/entrypoints/cli/__init__.py` retains `app` at its original dotted path. No pyproject.toml change required; all callers updated in the same change-set as the Step 7 keystone PR. |
