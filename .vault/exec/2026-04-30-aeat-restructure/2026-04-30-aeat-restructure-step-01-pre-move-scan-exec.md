...
| 1.2 | `src/aeat/domain/formulas/test_smoke.py:15` | `importlib.import_module("aeat.domain.formulas")` | **FIX** — rewrite map: `aeat.domain.formulas` → `aeat.domain.formulas` post-move (with shim preserving `aeat.domain.formulas`); the smoke test passes via shim or via direct rewrite (rebase script's call) |
