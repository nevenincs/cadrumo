...
| 1.4 | `pyproject.toml:75` — `[project.scripts] aeat = "aeat.entrypoints.cli:app"` | console-script entry point pinning `aeat.entrypoints.cli` import path | **FIX** — retained as the canonical entrypoint path in the hard-cut layout. No compatibility shim is required because callers use the canonical package directly. |
...
All 8 splitting-module `__init__.py` boundary risks: **FIX** in Step 7's keystone PR. The rebase-script (Step 5) ingests the OLD `__init__.py` export inventories and emits the NEW `__init__.py` files split by layer. The delivered hard-cutover uses pytest import-contract tests for the canonical surfaces and deleted root modules; CI fails the keystone PR if any new `__init__.py` cross-imports.

The old compatibility-shim plan was superseded by the hard-cutover ADR outcome. OLD locations are not retained as backward-compatible public surfaces; callers move to canonical paths.
...
|---|---|---|---|
| 1.1 | sub-pass 1 — registry-walk dynamic imports | FIX | Step 7 keystone (rebase-script + canonical path rewrite) |
...
| 1.4 | sub-pass 1 — pyproject.toml `aeat.entrypoints.cli:app` entry point | FIX (canonical entrypoint retained) | Step 7 keystone |
