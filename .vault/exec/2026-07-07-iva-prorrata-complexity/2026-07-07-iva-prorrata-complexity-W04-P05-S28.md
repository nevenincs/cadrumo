...
<!-- FRONTMATTER RULES:
     ...
     The S28 and 2026-07-07-iva-prorrata-complexity-plan placeholders are machine-filled by
...
<!-- STEP RECORD:
     ...
     The Surface a non-blocking WARNING Notice on ledger add when --sector names a sector absent from the bucket's declared SectorDefinition partition, so a typo'd sector tag that would silently deduct at the common-use percentage is instead disclosed (LIVA arts. 9.1.c / 101) and ## Scope

- `mirror the S24 inert-classification notice pattern and stay silent when the sector is declared`
- `src/aeat/entrypoints/cli/_ledger.py`
- `src/aeat/locales/`
- `src/aeat/entrypoints/cli/tests/test_prorrata_register_cli.py` placeholders below are machine-filled
