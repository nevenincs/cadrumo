...
- **New `src/aeat/observability/` subpackage** with strict pydantic v2
...
- **Log-record factory** in `aeat.logging` (replaces the planned
...
- **CLI wraps** at every outermost public entry point under
  ...
  `src/aeat/cli/_observability.py::cli_run_context`. The workflow `run`
...
1. **Log record enrichment:** `setLogRecordFactory` replaces the planned
   ...
   in `aeat.logging` so `configure_logging()` does not need to import
