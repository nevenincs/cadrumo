...
<!-- STEP RECORD:
     ...
     The add previous-filing resolver coverage for singular `source_output` offsets and ## Scope

- `src/aeat/domain/calculations/registry/test_selector_shape.py` placeholders below are machine-filled
     by `vaultspec-core vault add exec` from the originating Step row;
     do not fill them by hand. -->

# add previous-filing resolver coverage for singular `source_output` offsets

## Scope

- `src/aeat/domain/calculations/registry/test_selector_shape.py`

## Description

- Reconciled the historical checked `P02.S01` row to a per-step exec record.
