...
date: '2026-07-25'
modified: '2026-07-26'
body_hash: 'sha256:f42ffbfb13603cbd1bb4250ef9087ba0368d36698e85d5aa3a354a2fb3a22175'
step_id: 'S11'
related:
...
This submission is technically sound, unlike the same product's Scoop manifest: its digest is real and verified against the published asset, and its URL resolves. It is a naming defect only, which is why the correction is forward-only and why it is worth sequencing before the next release rather than after.

The question was delegated to a subagent in parallel. Its report arrived after the direct queries had already settled the referent, but it carried one piece of evidence stronger than anything gathered here: the submitted digest matches the `.sha256` companion published alongside the release asset, byte for byte. That upgrades the identification from convergent agreement across naming fields to a content match no coincidence can produce, and it independently excluded the sibling product on packaging shape rather than on name. The record was strengthened accordingly.

One further defect surfaced in that confirmation and is carried into the correction: the listing describes a dashboard UI while shipping a portable command-line executable.
