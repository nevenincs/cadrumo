...
One consequence needs an owner. The concurrent on-disk copy of the manager-actions module is anchored to the content that preceded this removal, so committing it wholesale would silently restore the deleted action and re-break the import. Whoever lands that file next must rebuild their edits against the current committed content. The same pass should correct that module's docstring, which still describes filling the profile in from what the taxpayer already filed; it was left alone here because the concurrent change is adding an authentication action and will almost certainly rewrite the same paragraph.

Four orphaned locale keys for the removed action remain. All four catalogues carry uncommitted concurrent work and the catalogue files have a known write race, so removing the keys through the locales CLI was held rather than risk a collision; unreferenced keys are inert, whereas the dead verb was not. This is the one part of the Step's scope not yet landed, so the Step is left open.
