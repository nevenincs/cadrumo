...
- AEAT-facing live-path observations:
  - `src/aeat/submission/test_live_submission.py` uses `SubmissionEngine.submit_draft(...)` on the dry-run path only.
  - `src/aeat/filing/test_live_complementaria.py` explicitly uses `dry_run=True`.
  - `src/aeat/workflow/test_live.py` does not submit anything.
  - `src/aeat/sync/test_live_sync.py` is read-only and currently dependency-gated.
...
- No test-side reference patches `aeat.submission._confirm.request_human_submit_confirmation`.
...
- Unit-test result: `src/aeat/submission/test_engine.py` uses local doubles to simulate submitter/session behavior while asserting the live gate.
...
- Drift found in unit suites:
  - `src/aeat/submission/test_engine.py` uses `_RecordingSubmitter`, `_Session`, `_OpenDeadlines`, and related doubles.
  - `src/aeat/workflow/test_engine.py` uses `_FakeSubmissionEngine` and multiple sibling `_Fake*` collaborators around the submission boundary.
...
- `OPEN | submission-boundary-doubles | src/aeat/submission/test_engine.py`
  ...
  - Follow-up issue: `#150`.
- `OPEN | workflow-boundary-doubles | src/aeat/workflow/test_engine.py`
