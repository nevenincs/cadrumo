...
date: '2026-05-27'
modified: '2026-07-17'
body_hash: 'sha256:1731679fff4ed50cc7c292ba9327e413874caa98e9775d437dec5ab6fa56eb87'
related:
  - "[[2026-05-26-modelo-130-relation-regression-plan]]"
  - "[[2026-05-26-modelo-130-relation-regression-adr]]"
---

# `modelo-130-relation-regression` audit: `rd-439-art-110-current-corpus-correction`

## Scope

Plan step `P04.S15` originally asked to extend
`[legal."rd-439-2007:art-110"].required_text` with a BOE-verbatim
art. 110.5 carry-forward fragment. The 2026-06-29 current-state
verification supersedes that premise: current RD 439/2007 art. 110
has no vigente apartado 5 to fetch.

## Current-state correction 2026-06-29

The current bundled source is
`src/aeat/_data/corpus/normatives/html/rd-439-2007-art-110.html#a110`.
It matches the official BOE consolidated text for BOE-A-2007-6820:
art. 110 contains apartados 1-4 only. The official BOE amendment
history records that apartado 5 was suppressed by Real Decreto
1461/2018, disposicion final 2.2 (BOE-A-2018-17992). Therefore the
old recommendation to re-fetch a current art. 110.5 carry-forward
sentence is invalid.

The current legal catalogue intentionally keeps one
`rd-439-2007:art-110` entry and does not declare
`rd-439-2007:art-110-5`. Its `required_text` covers the vigente
payment-rate and same-year payment-fraccionado anchors, including
`20 por ciento del rendimiento neto` and
`trimestres anteriores del mismo año`. Its notes explicitly state
that the Modelo 130 resultados-negativos carry is verified against
the AEAT Modelo 130 instructions, not a vigente apartado 110.5.

The casilla 15 mechanism remains grounded and implemented, but its
current source split is:

- legal framework: `rd-439-2007:art-110`, `orden-eha-672-2007:art-1`,
  `ley-35-2006:art-99`, and `rd-439-2007:art-95`;
- source instruction mechanics: `aeat-modelo-130-instructions`,
  whose corpus text contains the required casilla 15 fragments
  `importe (sin signo) de los resultados negativos` and
  `en ningún caso podrá figurar en la casilla 15 un importe superior`.

Focused verification on 2026-06-29:

- `uv run pytest src\aeat\domain\calculations\registry\tests\test_modelo_130_registry.py -q`
  passed: 14 tests.
- `uv run pytest src\aeat\application\calculations\tests\test_modelo_130_carry_forward_continuity.py -q`
  passed: 5 tests after updating that test profile fixture to the
  current filing-readiness schema (`activities.description` and
  `censo.activity_start_date`).
- `uv run pytest src\aeat\domain\calculations\registry\tests\test_catalogue_verification.py::test_committed_registry_tree_has_coherent_shared_catalogues -q`
  passed: 1 test.

Disposition: no further art. 110.5 corpus fetch should be pursued.
Future work must keep the current split: RD 439/2007 art. 110 for
the reglamentary payment framework and AEAT Modelo 130 instructions
for the casilla 15 negative-result carry mechanics.
