...
The core external-constants tests now derive the repository root and registry test paths from the current test topology, so the meta-tests assert the real source files again. Verification covers Ruff, compileall, 93 focused core/meta tests, 41 focused record-design tests, and the 2-test hard codebase size-budget guard. Remaining open work is outside this slice: CLI transport decomposition, non-CLI callable decomposition, and residual adapter/storage test splits.
