...
- **A2 (LOW) — advisory prose is an English literal, not a locale key. [ACTIONED 2026-06-25.]** The advisory `message` / `next_action` were hardcoded English. Fixed: both now route through `tr("application.modelo.findings.cross_period_non_official_local_chain.message" / ".next_action")`; the `message` interpolates `%{source_modelo}`/`%{filing_year}`/`%{period}`/`%{origin}`. The two keys were registered through the sanctioned locale CLI (`scaffold` then `set`), with real, non-identical translations authored for en/es/ca/hu. Verified: `tr` renders with interpolation in all four languages (no surviving `%{}` tokens); `python -m aeat.locales audit` reports all four catalogues OK; the parity gate, the translation-honesty ratchet, and `test_local_cross_period_carry.py` all pass (27 tests green together). A first attempt on 2026-06-21 was correctly deferred because the catalogue was under uncommitted peer WIP (casilla-id-canonicalization); once that landed, the global `scaffold` resolved cleanly and the keys were added without entangling peer state. The change rides in the working tree alongside that peer locale WIP and the `_verification_actions.py` CasillaId migration; it lands on the next sweep (not committed solo, to avoid sweeping peer WIP). Sibling cross-period finding literals remain English-literal; a family-wide migration is a sensible follow-up but is no longer a blocker for this finding.
...
- A2 (LOW) — DONE (2026-06-25): both advisory strings routed through `tr(...)` with real en/es/ca/hu translations registered via the locale CLI; parity, honesty, and focused tests green (27 together); locale audit OK. Rides the working tree with peer WIP, lands on the next sweep.
