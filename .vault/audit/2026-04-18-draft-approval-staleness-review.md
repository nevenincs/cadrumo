...
The approved implementation plan explicitly requires `src/aeat/submission/_preflight.py`
...
`src/aeat/submission/_protocols.py` still exposes a `DraftStatus` enum that stops
at `READY_TO_SUBMIT`, and `src/aeat/cli/submission/_helpers.py` parses draft JSON
...
in `aeat.filing`, but the downstream protocol seam was left behind.
...
configured drafts directory, but `src/aeat/cli/review/__init__.py` still
...
Gemini’s high-priority review note is still valid in `src/aeat/filing/_review.py`.
...
The current working tree now enforces `APPROVED` at `src/aeat/submission/_preflight.py`,
extends the submission protocol shim to understand `APPROVED` / `APPROVAL_STALE`,
accepts draft-id based review commands in `src/aeat/cli/review/__init__.py`, and
...
`src/aeat/filing/_review.py` now caches default transaction-catalogue loads with
...
`src/aeat/submission/_protocols.py` no longer invents the non-existent
`INCOMPLETE` status and now carries the full `aeat.filing.FilingDraftStatus`
...
loader in `src/aeat/cli/submission/_helpers.py` still assumes a legacy
...
`aeat.cli.submission._helpers.load_draft()` raises `ValueError: dictionary
update sequence element #0 has length 5; 2 is required`. The current
`src/aeat/cli/submission/test_cli.py` fixtures only pass because they fabricate
...
`src/aeat/cli/filing/__init__.py` now calls `validate_draft(...)` and then
...
and `approval_basis` through `src/aeat/filing/_review.py`. In direct
...
`src/aeat/submission/_preflight.py`, the submission CLI entrypoints
`src/aeat/cli/submission/preflight.py` and `src/aeat/cli/submission/dry_run.py`
...
`src/aeat/cli/submission/_helpers.py` now loads the real persisted
`aeat.filing.FilingDraft` JSON shape, refreshes review staleness through
`refresh_review_status(...)`, and rewrites the draft file when that refresh
changes the stored status. `src/aeat/filing/__init__.py` now revalidates drafts
...
Kent-facing regression outside pytest: `src/aeat/cli/filing/__init__.py`
...
`src/aeat/cli/browser/test_health.py`, `src/aeat/schema/test_fetch.py`, and
`src/aeat/financial/invoices/test_reconciliation.py` were rewritten to use
...
either a draft id or a draft path. `src/aeat/cli/submission/_helpers.py` now
...
`src/aeat/casillas/test_live_cli.py`,
`src/aeat/inbox/test_live_inbox.py`, `src/aeat/justificante/test_verify_live.py`,
`src/aeat/auth/test_certificate_live.py`, `src/aeat/auth/test_authenticator_live.py`,
`src/aeat/browser/test_live_evasion.py`, `src/aeat/history/test_live.py`,
`src/aeat/llm/test_live_anthropic.py`, and `src/aeat/status/test_live.py`.
One ordinary unit-test skip in `src/aeat/schema/test_fetch.py` has now been
...
`src/aeat/browser/test_session.py` and `src/aeat/browser/test_evasion.py` no
