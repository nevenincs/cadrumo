...
- `src/aeat/submission/_engine.py:94-130` `SubmissionEngine.submit_draft` — reachable live write; still defaults `dry_run=True`, but can dispatch a real submit when explicitly flipped.
...
- `src/aeat/cli/submission/_helpers.py:148-199` `_NullSession` — stub session used by the CLI submission path.
- `src/aeat/cli/submission/_helpers.py:148-199` `build_engine` — wires the CLI submission engine around the null session stub.
- `src/aeat/cli/submission/submit.py:80-115` `submit_cmd` — CLI live mode still routes through the old confirmation flag path.
- `src/aeat/cli/filing/__init__.py:83-85,399-427` `_submission_engine` — helper wiring for submission engine creation.
- `src/aeat/cli/filing/__init__.py:399-427` `submit_complementaria_cmd` — CLI live path still relies on `typer.confirm` plus `override_confirmation`.
- `src/aeat/cli/workflow/_helpers.py:69-159` `_build_engine` — workflow helper wiring, not a direct write leaf.
- `src/aeat/cli/workflow/_helpers.py:69-159` `_build_profile` — workflow helper wiring.
- `src/aeat/cli/workflow/_helpers.py:69-159` `run_engine_next` — workflow CLI wrapper into the staged submit path.
- `src/aeat/cli/workflow/_helpers.py:69-159` `run_engine_for_period` — workflow CLI wrapper into the staged submit path.
- `src/aeat/browser/session.py:68-120` `BrowserSession.create_context` — auth-backend branch is a no-op stub.

### read-only

- `src/aeat/justificante/_verify.py:36-105` `verify_csv` — justificante verification only.
...
- `src/aeat/browser/session.py:122-157` `BrowserSession.navigate` — navigation helper only.
- `src/aeat/browser/health.py:33-47` `run_health_check` — health probe only.
- `src/aeat/filing/__init__.py:141-270` `build_draft`, `validate_draft`, `iter_findings`, `utc_now` — draft and validation helpers only.
- `src/aeat/filing/_complementaria.py:67-160` `build_complementaria`, `load_amendment`, `list_amendments`, `_persist_amendment` — complementaria persistence/read helpers only.
- `src/aeat/submission/_engine.py:249-313` `_persist`, `_persist_amendment_result`, `load_submission`, `list_submissions` — local persistence and read helpers only.

### generic helper

- `src/aeat/config.py:43-501` `Settings`, `load_settings` — configuration surface only.
- `src/aeat/submission/_submitters/__init__.py:24-93` `Submitter` — protocol/helper surface only.
- `src/aeat/submission/_protocols.py:44-268` protocol and value-model stubs — type surface only.
- `src/aeat/workflow/_protocols.py:92-107` `SubmissionEngineProtocol` — protocol surface only.
- `src/aeat/cli/submission/_helpers.py:175-204` `build_engine`, `load_draft` — CLI helper surface only.
- `src/aeat/workflow/_adapters.py:231-289` `default_engine` — wiring helper; not itself a direct write leaf.

## gap list

- `src/aeat/submission/_engine.py:94-197` `SubmissionEngine.submit_draft` / `submit_amendment` / `_submit_with_transport` — violates R2, R3, R4, R5, R6: `dry_run` is still optional, the live gate still relies on `override_confirmation` plus the old settings flag, no `PYTEST_CURRENT_TEST` refusal exists, no blocking human confirmation hook exists, and no append-only audit log exists.
...
- `src/aeat/submission/_confirm.py` is absent, so the charter-required confirmation hook is unreachable by construction.
- `src/aeat/browser/session.py:112-115` contains a certificate-auth branch that logs and `pass`es; it is a no-op placeholder.
...
- `src/aeat/cli/workflow/_helpers.py:69-159` is inert for production live-write control because no default workflow engine/profile factory is wired.

## import graph check: `aeat.submission._confirm`

- `src/aeat/submission/_confirm.py` does not exist.
...
- Public `aeat.submission` re-exports engine, models, errors, preflight, protocols, and submitter symbols only; it does not expose a confirmation API.
