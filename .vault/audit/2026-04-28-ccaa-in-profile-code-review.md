...
related:
  ...
  - '[[2026-04-28-ccaa-in-profile-summary]]'
---
...
The new `aeat.profile` package correctly re-exports `CCAA`, but changed public-facing surfaces still referenced private formula internals. The profile CLI, setup code/tests, integration tests, and concept documentation now import or document the CCAA type through `aeat.profile`. The integration test still imports the private autonomic calculator for a focused expected-value assertion because that calculator is the M100 ruleset behavior under test, not the new public profile surface.

## Invariant Audit

- Strict frozen Pydantic v2 model using existing CCAA enum: PASS. `KentTaxResidence.model_config` and `ResidenceChange.model_config` include `strict=True`, `frozen=True`, and `extra='forbid'`; `src/aeat/profile/__init__.py` consumes the existing `CCAA` enum.
- Local-state Path A JSON persistence with no #216 storage dependency: PASS. `src/aeat/profile/_storage.py` uses config-directory JSON, same-directory temp files, and `os.replace`; no `aeat.storage`, SQLAlchemy, or Alembic dependency was introduced.
- No namespace pollution with existing profile surfaces: PASS. New code is isolated under `src/aeat/profile` and `src/aeat/cli/profile`; existing financial, browser, and category profile surfaces were not extended.
...
- ErrorCode registration for new errors: PASS. `TaxResidenceProfileError`, `ProfileNotConfiguredError`, and `ForalRegimeError` are registered in `src/aeat/errors/_registry.py`.
- Public API imports from `aeat.profile` and `aeat.cli` only: PASS for the profile CCAA surface; see CCAA-003 remediation.
...
Audited modified tracked files: `docs/coverage/kent-capabilities.md`, `env/.env.example`, `src/aeat/browser/test_session.py`, `src/aeat/cli/__init__.py`, `src/aeat/cli/filing/__init__.py`, `src/aeat/cli/test_json_schema_conformance.py`, `src/aeat/config.py`, `src/aeat/errors/_registry.py`, `src/aeat/schema/test_cache.py`, `src/aeat/schema/test_models.py`, `src/aeat/sede/_declarations.py`, `src/aeat/setup/_env_writer.py`, `src/aeat/setup/_models.py`, `src/aeat/setup/_wizard.py`, `src/aeat/setup/test_cli.py`, `src/aeat/setup/test_env_writer.py`, `src/aeat/setup/test_models.py`, `src/aeat/setup/test_verifier.py`, `src/aeat/setup/test_wizard.py`, `tests/integration/test_kent_workflows.py`, and `uv.lock`.

Audited new untracked files: `.vault/adr/2026-04-28-ccaa-in-profile-adr.md`, `.vault/exec/2026-04-28-ccaa-in-profile/2026-04-28-ccaa-in-profile-summary.md`, `.vault/plan/2026-04-28-ccaa-in-profile-plan.md`, `.vault/research/2026-04-28-ccaa-in-profile-research.md`, `docs/concepts/tax-residence.md`, `src/aeat/cli/profile/__init__.py`, `src/aeat/cli/profile/test_cli.py`, `src/aeat/profile/__init__.py`, `src/aeat/profile/_errors.py`, `src/aeat/profile/_storage.py`, `src/aeat/profile/test_errors.py`, `src/aeat/profile/test_model.py`, and `src/aeat/profile/test_storage.py`.
