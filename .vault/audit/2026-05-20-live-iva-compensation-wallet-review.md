...
WALLET-057 | HIGH | Legacy active-profile manifest blocked live auth diagnostics and history capture
On 2026-05-21, a live `iva-wallet capture-history` attempt failed before history capture because the active profile bucket manifest predated the required plaintext lifecycle `status` mirror. The strict manifest reader correctly failed closed, but `aeat config repair` and auth readiness both tried to read the manifest through the normal live scanner and crashed instead of surfacing a repair path. That made live Cl@ve troubleshooting look like an auth-only failure while the local profile projection was also degraded.

Resolution: fixed. Active-profile health now catches `StorageValidationError` for a missing manifest lifecycle status and reports `manifest_unreadable` with the exact recovery command. `aeat config repair profile --repair-manifest-status --yes` parses the legacy TOML only inside the repair path, reads the encrypted profile record as lifecycle authority, verifies the bucket/profile IDs agree, and rewrites the manifest with the authoritative status. The strict `read_manifest` behavior remains unchanged for normal reads, so missing-status manifests still fail closed outside repair.

Live/local verification:

- `uv run aeat config repair` no longer crashes; before repair it reported `profile.storage ... status=manifest_unreadable` and `next_action=aeat config repair profile --repair-manifest-status --yes`.
- `uv run aeat config repair profile --repair-manifest-status --yes` repaired the active manifest with `manifest_status=active`.
- `uv run aeat config auth status --provider clave_movil` now reports the active profile as registered and ready.
- `uv run aeat app live iva-wallet history --as-of-year 2026` reloads the persisted wallet authority decision without a live AEAT session.

Focused verification:

- `uv run pytest src/aeat/application/workflow/test_profile_health.py src/aeat/entrypoints/cli/test_profile_lifecycle_verbs.py::test_repair_profile_manifest_status_backfills_legacy_active_manifest -q --disable-warnings` completed with 5 passed.
- `uv run ruff check src/aeat/application/workflow/_profile_health.py src/aeat/application/workflow/test_profile_health.py src/aeat/entrypoints/cli/_config/__init__.py src/aeat/entrypoints/cli/test_profile_lifecycle_verbs.py` passed.
- `uv run python -m aeat.locales audit` reported `ca.yml`, `en.yml`, `es.yml`, and `hu.yml` ok.

WALLET-058 | HIGH | Cl@ve non-QR auth still timed out after visible verification-code prompts
After WALLET-057, two live capture-history attempts reached AEAT's non-QR Cl@ve Móvil page and surfaced verification codes to the operator within the 120-second production approval window. Both attempts timed out on the AEAT auth completion wait; the driver remained on the `ObtenerClaveMovil` page and could not infer the phone/app outcome. Diagnostics recorded the active profile as registered, the Cl@ve identity aligned with the profile identity fingerprint, NIE support configured, and certificate authentication absent.

Status: open. This is not a wallet calculation failure and not a profile-identity mismatch. It is an unresolved live-auth completion failure requiring operator phone-state classification for the captured diagnostics (`app_prompted_and_accepted`, `app_prompted_not_accepted`, `app_did_not_prompt`, or `operator_did_not_check`) before the next driver change is selected. No private identity, support number, wallet amount, filing content, or live AEAT write was persisted in this audit note.

WALLET-058-MITIGATION | MEDIUM | Cl@ve timeout diagnostics now carry exact phone-state report commands
Mitigation: fixed offline. The Cl@ve timeout context now includes `operator_report_commands`, one exact `aeat config auth diagnostics report DIAGNOSTIC_ID --phone-state ...` command for each allowed phone-state value. The timeout suggestion renders those exact commands instead of providing one example plus prose. Persisted diagnostic detail also exposes the same commands through `aeat config auth diagnostics show DIAGNOSTIC_ID`, so the operator can recover the reporting command after the original timeout output scrolls away. The allowed phone-state tuple is now centralized in the Cl@ve auth module and reused by application diagnostics, removing the prior duplicated list.

This does not claim to fix the live AEAT timeout itself. It makes the next operator retry auditable: the diagnostic artefact tells the operator exactly how to record whether the app prompted, was accepted, was not accepted, did not prompt, or was not checked. No live AEAT contact was made for this mitigation.
...
WALLET-059 | HIGH | Secure-object unreadable rows are in scope for live IVA reconciliation
After the active manifest repair, `aeat config repair` remained at `warn` because it found unreadable secure-object rows across wallet-relevant and calculation-relevant namespaces, including bucket event history, filing drafts, invoices, Modelo calculation revisions, Modelo work units, transaction bucket data, auth sessions, filed-declaration artefacts, and filed-declaration observations. This is not safe to dismiss as unrelated housekeeping: multiyear IVA wallet reconciliation depends on durable filed-history, local calculation history, wallet observations, and authority decisions. If these rows are stale test debris, they need classification; if they are live profile evidence, they can block or distort the local-vs-AEAT reconciliation ladder.

Status: partially mitigated and still in scope. Do not quarantine or delete these rows blindly. The next implementation pass must extend the non-destructive inventory path with row-level bucket/profile context where recoverable from non-secret metadata or safe key structure before any destructive repair command is considered.

Progress: added `aeat config repair list <namespace> [--unreadable]` as a read-only inventory surface. The report exposes opaque object-key digest, row id, sensitivity class, schema version, written timestamp, decryptability, and failure reason without decrypting or printing payload content. A live probe against the filed-declaration observations namespace confirmed unreadable financial rows with AES-256-GCM tag verification failures. This does not close the finding: the remaining work is classifying whether those rows are disposable prior-run artefacts or required IVA filing-history evidence before any quarantine action.

W04.F12 mitigation: the repair integrity backend no longer suggests direct `aeat config repair quarantine --yes` as the next action when unreadable rows are found; it routes operators to `aeat config repair list <namespace> --unreadable` first. The list report now classifies the namespace role, IVA reconciliation relevance, compensation-history participation, destructive-repair risk, and a conservative operator note. Wallet observations, filed-declaration artefacts/observations, IVA compensation history, wallet reconciliation decisions, calculation observations, Modelo state, filing/submission/justificante records, ledger transactions, invoices, bucket events, user profile/live snapshots, and auth namespaces are treated as preserve-first evidence categories. Unknown namespaces are explicitly reported as `unknown_do_not_quarantine_blindly`. This mitigates the direct-quarantine hazard but does not yet provide row-level bucket/profile attribution.

W04.F14 resolution: the repair list rows now include row-level context derived without decrypting payloads or reversing object-key HMACs: active-profile context, object-key kind, safe object-key hint, context confidence, and context note. Singleton catalogue namespaces report the repository-contract key (`catalogue`), workflow state reports `state`, active-bucket transaction and usage-ratio rows report their active-bucket key shape, and active-profile/apoderado rows report the active profile key shape. Rows whose natural keys include taxpayer, expediente, snapshot, diagnostic, submission, or period details remain redacted and are labelled `unrecoverable_hmac_digest`. When the current master key can prove a safe hint by digest comparison, the row reports `active_key_digest_match`; otherwise it reports a repository-contract confidence rather than guessing. W04.F25 later tightened this further so the active profile bucket id is used only internally for digest matching and rendered as `active_profile` plus placeholder object-key hints. This completes the row-context mitigation while keeping destructive quarantine out of scope unless an operator has separately verified replacement evidence.

WALLET-060 | MEDIUM | Runtime dependency-sync drift is in scope for live-driver reliability
`aeat config repair` also reported `runtime.dependency_sync` as stale during live wallet verification. Because the current blocker is a Playwright/Cl@ve live-driver completion issue, dependency drift is part of the reliability surface: an out-of-sync environment can mask browser, Playwright, timeout, or storage behavior regressions.

Status: fixed. `uv sync --frozen --dry-run` and `uv sync --frozen` both reported no package changes, but the Windows timestamp heuristic still considered `.venv/pyvenv.cfg` older than `pyproject.toml`. The diagnostic now treats uv's authoritative frozen dry-run as the source of truth when timestamps look stale: if uv would make no changes, `runtime.dependency_sync` reports `ok` with detail `uv sync --frozen --dry-run would make no changes`. During verification, `aeat config repair` reported `ok runtime.dependency_sync Venv in sync` while still preserving the unrelated auth-readiness and secure-object integrity warnings. This finding remains separate from tax-calculation correctness and from the open Cl@ve live-auth completion issue.

WALLET-061 | HIGH | Representation gate exposed own-name selector but label-click path failed
The live attempt with verification code `LQQ` reached AEAT's `DialogoRepresentacion` page after Cl@ve auth and then failed closed with `representation_gate_own_name_unavailable`. The encrypted diagnostic HTML showed the own-name radio `#propio`, `label[for="propio"]`, `form#repForm`, and the submit button were present. That means the failure was not an unknown representative-only page; it was a brittle Playwright interaction path around the own-name label click. The fail-closed behavior was correct: no represented-party fields were filled and no representation choice was sent.

Resolution: fixed offline and awaiting live confirmation. Both the Cl@ve post-auth continuation and wallet Sede continuation now prefer directly checking the configured own-name radio selector before clicking submit, with the label-click path retained only as a fallback when direct radio checking is unavailable. The latest live retry after this change did not reach the representation gate; it timed out at Cl@ve completion on a new verification code, so WALLET-058 remains open and this fix still needs live confirmation once Cl@ve advances again.

Verification:

- `uv run pytest src/aeat/adapters/outbound/aeat/auth/test_clave_movil.py::TestPostAuthLanding::test_representation_dispatcher_continues_only_own_name src/aeat/adapters/outbound/aeat/auth/test_clave_movil.py::TestPostAuthLanding::test_representation_dispatcher_prefers_direct_radio_check src/aeat/adapters/outbound/aeat/sede/test_iva_compensation_wallet.py src/aeat/application/test_repair_integrity.py src/aeat/application/test_diagnostics.py::test_secure_objects_integrity_check_reports_unreadable_rows_from_rotated_master_key -q --disable-warnings` completed with 28 passed.
- `uv run ruff check src/aeat/adapters/outbound/aeat/auth/_clave_movil.py src/aeat/adapters/outbound/aeat/auth/test_clave_movil.py src/aeat/adapters/outbound/aeat/sede/_iva_compensation_wallet.py src/aeat/adapters/outbound/aeat/sede/test_iva_compensation_wallet.py src/aeat/application/diagnostics.py src/aeat/application/repair_integrity.py src/aeat/application/test_diagnostics.py src/aeat/application/test_repair_integrity.py` passed.

WALLET-062 | HIGH | Live capture succeeded but default history output exposed private row detail
On 2026-05-21, a Cl@ve-approved live `iva-wallet capture-history --from-year 2023 --to-year 2026` run completed after surfacing the AEAT verification code to the operator. It captured and persisted eight Modelo 303 filing-history observations, transformed eight observations into the IVA compensation calculation history, and reloaded eight profile-local secure history rows. This confirms the own-name representation continuation and multiyear secure reload path for the current live profile. The run did not submit a filing, payment, represented-taxpayer choice, or represented-party data.

The follow-up local `iva-wallet history --as-of-year 2026` command also confirmed the persisted rows can drive the carry-forward and wallet-authority decision report without a new AEAT session. However, the default text output printed private period amounts, filing identifiers, and authority details. That is acceptable only as an explicit operator diagnostic surface, not as the default command behavior in a shared worktree where terminal output can be copied into audit notes or issue comments.

Resolution: fixed. `aeat app live iva-wallet history` now defaults to summary mode with only row count, as-of year, carry-forward lot count, and authority-decision count. Operators must pass `--details` to print private rows and amounts. The same pass corrected `capture-history` text output to report encrypted profile-local `secure_objects` storage instead of implying that plaintext files were written under `output_root`.

Verification:

- Live `uv run aeat app live iva-wallet capture-history --from-year 2023 --to-year 2026 --output-root var/aeat/live/iva-compensation-history` completed after Cl@ve approval and reported eight captured observations plus eight reloaded secure history rows. No private identity, filing identifier, or amount is recorded in this audit note.
- `uv run aeat app live iva-wallet history --as-of-year 2026` now emits `privacy_mode=summary`, aggregate counts, and a `--details` opt-in hint without private rows.
- `uv run pytest src/aeat/entrypoints/cli/test_registry_cli.py -k "iva_wallet or capture_iva_history"` completed with 5 passed.
- `uv run ruff check src/aeat/entrypoints/cli/_app_live.py src/aeat/entrypoints/cli/test_registry_cli.py` passed.
- `uv run python -m aeat.locales audit` reported `ca.yml`, `en.yml`, `es.yml`, and `hu.yml` ok after adding the new wallet history details help string and resolving the existing overview locale drift.

WALLET-063 | CRITICAL | Secure-object and database drift affects complete application state
The live IVA wallet work exposed a broader persistence integrity issue. A non-destructive active-profile probe reported many unreadable secure-object rows across ledger, invoices, Modelo work units, filing drafts, justificantes, filed-declaration observations/artefacts, bucket event history, and auth sessions. The current IVA wallet and IVA compensation calculation namespaces are readable, so the latest live wallet/history calculation chain is reloadable, but the wider application state is not healthy enough to treat as production-stable.

Root-cause class confirmed: several tests opened `EphemeralMasterKeyProvider` without isolating `AEAT_DATABASE_URL`. Any default `SecureObjectRepository()` constructed in those tests could write rows into the active profile DB under a throwaway key. Those rows then exist as durable rows but cannot decrypt under the real active profile key. This exactly matches the drift shape observed in the active DB. No destructive repair was run.

Resolution in this pass: partial. `SecureBoundRepository` enumeration now fails closed with `SecureObjectUnreadableError` instead of silently skipping unreadable rows. `CalculationObservationRepository.iter_modelo()` no longer raw-scans HMAC keys and encrypted payloads; it enumerates decrypted records and filters by the stored observation modelo. The discovered invoice, invoice reconciliation, complementaria, and Sede declaration observation tests now isolate `AEAT_DATABASE_URL` to a temporary SQLite DB and dispose cached engines around the ephemeral-key context.

W04.F14 mitigation: active-profile unreadable rows now carry non-destructive row-level context where it can be derived from repository contracts or current-key digest confirmation. The repair inventory reports active-bucket/profile context, singleton-vs-multirow key expectations, safe object-key hints, confidence, and redaction notes without decrypting payloads or reversing HMAC natural keys.

W04.F20 mitigation: `aeat config repair` now inspects relational SQL drift before the secure storage probes can invoke ORM table auto-creation. The new read-only `relational_database.integrity` row checks expected ORM table presence, SQLite `PRAGMA integrity_check`, and SQLite `PRAGMA foreign_key_check`. Missing database files remain acceptable for first-run bootstrap, but a missing database for an active profile fails closed. Details are limited to table names, SQLite diagnostic messages, and foreign-key row ids/table names; row payloads and taxpayer values are not printed.

W04.F21 mitigation: a config-domain persona pass found that `aeat config auth status` failed on an unreadable `aeat.domain.filing.drafts` row even though the operator was asking only for auth readiness. `auth status` and `auth test` now use the canonical projection with workspace counters and period-obligation calculation disabled, so unrelated ledger/invoice/draft/work-unit drift remains surfaced by `config repair` without blocking auth readiness inspection.

W04.F22 mitigation: the same pass found that corrupt persisted auth-session errors suggested `aeat config auth test --provider certificate` even when the operator was testing Cl@ve Móvil. The error registry now suggests `aeat config auth clear --sessions`, matching the localized recovery text and the actual config clear surface.

W04.F23 mitigation: the profile lifecycle persona found that `aeat config profile list` aborted on a legacy bucket manifest missing lifecycle `status`, while `profile status` and `profile show` for the active profile succeeded. The manifest scanner now skips malformed manifests on live profile enumeration, preserving fail-closed switch/name resolution, and `profile list` reports only a skipped-manifest count so operators know degraded buckets exist without exposing manifest contents or profile identifiers.

W04.F24 mitigation: the auth-readiness persona found that `aeat config auth diagnostics list` exposed active-profile labels/ids, DNI/NIE identity kind, and profile/alignment context in an inventory view. List mode now disables private auth-attempt context at the application boundary and the text renderer emits only diagnostic id, timestamp, reason, auth mode, headless flag, phone-state report, and capture flags. `auth diagnostics show <id>` remains the deliberate redacted detail command for a chosen diagnostic.

W04.F25 mitigation: a follow-up config-domain pass found that `aeat config repair list <namespace> --unreadable`, now the recommended safe inventory path, printed the active profile bucket UUID and embedded it in active-bucket object-key hints. The row-context path now uses the active bucket id only internally for digest matching, while text and JSON inventory render `active_profile` plus placeholder hints such as `transaction-catalogue:<active-profile>`. This preserves row-level provenance and confidence signals without copying active profile identifiers into audit-prone terminal output.

W04.F26 mitigation: the same pass found that top-level `aeat config repair` still echoed the active profile bucket UUID in the profile summary and `profile.storage` diagnostic row. The repair report now redacts the setup `active_profile` field for JSON output, renders the profile line with a generic `active_profile` marker, and uses the same marker in `profile.storage` summaries while keeping readiness counts, auth provider, and next actions.

W04.F27 verification hardening: the repair privacy guarantees are now covered through the public CLI rather than only through application helpers and shell smoke checks. The new CLI contract creates a real operator profile through `config profile create`, persists a real encrypted secure-object row under the active storage backend, and then exercises `config repair` and `config repair list` in text and JSON modes. The repair attribution invariant now checks that actual payload strings are absent while allowing the explicit metadata field `payload_disclosure`.

W04.F28 verification hardening: the attribution command is now covered through the public CLI with a real encrypted unreadable wallet-observation row written under a different key. The test exercises `config repair integrity attribution` in text and JSON modes and proves the output keeps `metadata_only` disclosure, namespace role, owner semantics, and redacted active-profile context without printing payload text, taxpayer ids, period tokens, active profile UUIDs, or wallet amount text.

W05 scope expansion: the wallet plan now explicitly carries a new secure-object reconciliation and calculation-confidence Wave. The expanded scope separates unreadable-row attribution, preserve-first remediation decisions, calculation confidence across ledger/Modelo 303/Modelo 390/multiyear carry-forward/AEAT remote-state reconciliation, and testimonial-driven acceptance. This records that the remaining W04.F17 critical issue is not a cleanup task; it is a production-readiness architecture track.

W05.P01.S05 ADR reconciliation: the binding and secure-storage ADR trail was reread and reconciled into a new terminology ADR for profile, profile label, profile UUID, bucket, bucket manifest, active profile pointer, `BucketSession`, domain repository, `SecureObjectRepository`, calculation binding, source observation, and wallet reconciliation decision. The ADR makes profile UUIDs internal/private on repair and diagnostic inventory surfaces, distinguishes calculation binding from storage attachment, and requires source resolver storage health to feed calculation confidence before Modelo 303, Modelo 390, multiyear carry-forward, or AEAT remote-state output can be treated as filing-grade.

W05.P01.S02 mitigation: unreadable-row attribution now assigns a conservative likely origin from metadata only. It distinguishes test namespace residue, unregistered repository routing, active-profile key digest matches, missing active-profile context, legacy HMAC-only residue, tax evidence keychain/restore mismatch, and generic repository keychain/restore mismatch. The attribution remains metadata-only and does not decrypt payloads or print natural keys.

W05.P01.S03 mitigation: repair namespace classifications now carry replacement-evidence requirements. Wallet evidence requires a fresh read-only AEAT wallet observation or exported existing observation plus replayed reconciliation; critical receipt/submission namespaces require verified AEAT Sede or justificante evidence and stay destructive-quarantine disabled without a later override ADR; unknown namespaces require owner-repository identification, encrypted backup, and engineer preserve-first review.

W05.P01.S04 mitigation: `aeat config repair integrity attribution` now defaults to summary mode so hundreds of unreadable rows are not dumped by default. Summary text and JSON retain totals, namespace role, owner semantics, timestamp range, classification counts, and replacement-evidence requirements while omitting per-row `unreadable_rows`. Operators must pass `--details` for row-level metadata. The public CLI privacy contract covers both modes and verifies that active profile UUIDs, taxpayer ids, period tokens, wallet payload text, and private natural keys remain absent.

W05.P02.S01 mitigation: durable repair decisions now exist as encrypted profile-local AUDIT records. `RepairRemediationDecision` models preserve, quarantine, rebuild, and export-required planning outcomes, but its `mutation_authorized` field is hard-typed false. The repository stores decisions under opaque SHA-256 decision ids and supports save, load, and decision-time ordered listing. This creates the durable preserve-first planning substrate for the next dry-run repair planner without authorizing destructive action.

W05.P02.S01 review mitigation: the review found the initial decision id validation only required a SHA-256-shaped value. The model now requires the decision id to be content-bound to the target namespace, target digest, outcome, decision time, actor, reason, likely origin, replacement-evidence requirements, and verified evidence references, preventing arbitrary opaque keys from weakening the repair decision audit trail.

W05.P02.S02 mitigation: `aeat config repair plan` now provides a dry-run preserve-first remediation plan. It derives metadata-only namespace recommendations from unreadable-row attribution, reports `dry_run=true`, `planned_mutations=0`, and `mutation_allowed=false`, and presents replacement-evidence requirements before any quarantine or rebuild can be considered. The command does not save repair decisions, quarantine rows, rebuild state, delete data, or perform live AEAT operations.

W05.P02.S03 mitigation: `RepairRemediationDecision` now rejects `quarantine` and `rebuild` outcomes unless the decision carries explicit `verified_replacement_evidence_refs`. Replacement-evidence requirements remain the domain checklist; verified references are the operator-named evidence needed before a destructive outcome can even be recorded. This still does not authorize mutation because `mutation_authorized` remains hard-typed false.

W05.P02.S04 mitigation: critical submission records, justificante receipt metadata, remote filed-declaration evidence, local filing history, and unknown namespaces now carry a destructive-quarantine policy that disables quarantine without a separate engineer override ADR. `RepairRemediationDecision` rejects quarantine for those namespaces even when replacement-evidence requirements and verified evidence references are present. The dry-run repair planner surfaces the policy while preserving `planned_mutations=0`.

W06-W08 scope expansion: the wallet plan now carries adverse-condition backend hardening, complete IVA backend end-to-end verification, and release-readiness/evidence-closure waves. These waves cover profile/bucket routing faults, root fallback database guards, envelope-contract validation, degraded-evidence calculation behavior, live-read adverse browser state, source mesh completeness, replayable IVA regression matrices, export/readiness gates, persona regression, and final safety/privacy gates.

Still open: active-profile unreadable secure-object rows remain present and require a non-destructive reconciliation/remediation policy before broad application state can be treated as production-stable. No destructive quarantine or deletion has been run.

Verification:

- `uv run aeat config repair profile` reported the active profile pointer and profile record ready.
- `uv run aeat config repair integrity objects` reported 74 readable and 374 unreadable secure-object rows; private payloads were not printed or recorded.
- `uv run aeat config repair` reported `ok relational_database.integrity 9 relational table(s) present`, plus the known auth-readiness and secure-object unreadable warnings; private payloads were not printed or recorded.
...
- `uv run aeat config auth status` completed successfully after W04.F21 and reported the local Cl@ve Móvil provider readiness without opening unreadable filing drafts; active profile identifiers were not recorded.
- `uv run aeat config auth test --provider clave_movil` still failed correctly on the corrupt local session, but now suggested `aeat config auth clear --sessions`.
...
- `uv run aeat config profile list` completed successfully after W04.F23 and reported valid live profiles plus a skipped invalid-manifest count; skipped bucket identifiers were not recorded.
...
- `uv run aeat config auth diagnostics list` completed after W04.F24 without printing profile labels/ids or identity kind.
- `uv run aeat --format json config auth diagnostics list` emitted empty/null profile/identity context fields in list rows.
...
- `uv run aeat config repair list aeat.domain.transactions.bucket --unreadable` and the same command in JSON mode completed after W04.F25 without UUID-shaped active profile identifiers in row context.
...
- `uv run aeat config repair` and `uv run aeat --format json config repair` completed after W04.F26 without UUID-shaped active profile identifiers in the profile summary or `profile.storage` row.
...
- `uv run pytest src/aeat/entrypoints/cli/test_repair_privacy_contract.py -q --disable-warnings` completed after W04.F28 with 3 passed.
- `uv run ruff check src/aeat/entrypoints/cli/test_repair_privacy_contract.py` passed.
- `uv run pytest src/aeat/application/test_repair_integrity.py src/aeat/entrypoints/cli/test_repair_privacy_contract.py -q --disable-warnings` completed after W05.P02.S02 with 41 passed.
- `uv run ruff check src/aeat/application/repair_integrity.py src/aeat/entrypoints/cli/_config/__init__.py src/aeat/entrypoints/cli/test_repair_privacy_contract.py` passed after W05.P02.S02.
- `uv run pytest src/aeat/application/test_repair_integrity.py src/aeat/entrypoints/cli/test_repair_privacy_contract.py -q --disable-warnings` completed after W05.P02.S03 with 42 passed.
- `uv run ruff check src/aeat/application/repair_integrity.py src/aeat/application/test_repair_integrity.py src/aeat/entrypoints/cli/_config/__init__.py src/aeat/entrypoints/cli/test_repair_privacy_contract.py` passed after W05.P02.S03.
- `uv run pytest src/aeat/application/test_repair_integrity.py src/aeat/entrypoints/cli/test_repair_privacy_contract.py -q --disable-warnings` completed after W05.P02.S04 with 43 passed.
- `uv run ruff check src/aeat/application/repair_integrity.py src/aeat/application/test_repair_integrity.py src/aeat/entrypoints/cli/_config/__init__.py src/aeat/entrypoints/cli/test_repair_privacy_contract.py` passed after W05.P02.S04.
- `uv run pytest src/aeat/application/calculations/test_observations_repository_roundtrip.py src/aeat/domain/invoices/test_repository.py src/aeat/domain/invoices/test_reconciliation.py src/aeat/application/filing/test_complementaria.py src/aeat/adapters/outbound/aeat/sede/test_declarations.py::TestSubmittedFileObservation::test_modelo_100_redacted_xml_observation_roundtrips_through_encrypted_store src/aeat/adapters/outbound/aeat/sede/test_declarations.py::TestFiledObservationBindings::test_encrypted_filed_observation_roundtrip_supplies_previous_filing_binding -q` completed with 31 passed.
- `uv run ruff check src/aeat/adapters/persistence/storage/envelope/_secure_repository.py src/aeat/application/calculations/_observations_repository.py src/aeat/application/calculations/test_observations_repository_roundtrip.py src/aeat/domain/invoices/test_repository.py src/aeat/domain/invoices/test_reconciliation.py src/aeat/application/filing/test_complementaria.py src/aeat/adapters/outbound/aeat/sede/test_declarations.py` passed.
- Research persisted in `.vault/research/2026-05-21-secure-object-database-drift-research.md`.
