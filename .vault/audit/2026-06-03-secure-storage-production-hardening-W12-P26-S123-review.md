...
- Declaration PDFs that require bbox word-position parsing are routed through `_temporary_sensitive_pdf_path()` because the parser needs a real path; the helper uses `mkstemp`, writes through the raw descriptor, closes the descriptor, and unlinks the path in `finally`.
...
Disposition: close `AFR-021` as `remote-mirror`. The temporary PDF scratch path is accepted as parser scratch, not durable sensitive storage, because it is scoped to a context manager, uses OS-created temporary permissions, and has focused unlink coverage.
