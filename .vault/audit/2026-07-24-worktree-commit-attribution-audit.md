...
For an entangled file, use the apply-cached gated drive documented in `uncommitted-wip-is-not-orphaned`: write the committed version aside with `git show HEAD:<path>`, apply only your own edits to that copy, diff it into a HEAD-anchored own-edits-only patch, stage it with `git apply --cached` so the peer's live working-tree state is untouched, verify the staged diff carries zero foreign markers, then commit the verified index. This is the only mechanism in the available toolset that isolates by change rather than by file, and it was proven across three independent landings in an earlier campaign.
