...
## S160-003 | PASS | Settings, errors, and tests follow project conventions
...
- `uv run --no-sync pytest -q src/aeat/adapters/persistence/storage/bucket/test_lockfile.py` passed with 9 tests.
- `uv run --no-sync pytest -q src/aeat/adapters/persistence/storage/bucket` passed with 99 tests.
- `uv run --no-sync ruff check src/aeat/adapters/persistence/storage/bucket/_lockfile.py src/aeat/adapters/persistence/storage/bucket/test_lockfile.py` passed.
- Touched-file hygiene scan found no broad exception catches, suppressions, fake/stub/monkeypatch markers, skipped/xfail tests, direct output, local secure-object marker construction, direct settings construction, or direct environment access. The only `Settings()` match is explanatory docstring text describing the removed anti-pattern.
