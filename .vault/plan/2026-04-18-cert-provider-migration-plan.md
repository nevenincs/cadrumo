...
1. **Protocol and Model Definition**
   - Create `src/aeat/auth/_protocols.py` containing `AuthProvider`, `AuthProviderKind` (enum), `AuthProviderDescription` (for describe method).
   - Update `AeatSession` in `src/aeat/auth/_authenticator.py` (or move it to a central models file) to include the `provider_detail` discriminator, creating `CertificateSessionDetail`. `certificate_nif` becomes `identity_nif`.

2. **Move Files**
   - Create directory `src/aeat/auth/_providers/_certificate/`.
   - Move `src/aeat/auth/certificate.py` -> `src/aeat/auth/_providers/_certificate/certificate.py`.
   - Move `src/aeat/auth/_certificate_backends/` -> `src/aeat/auth/_providers/_certificate/_certificate_backends/`.
   - Update imports in the moved files (e.g. adjust relative imports).

3. **Implement CertificateAuthProvider**
   - Create `src/aeat/auth/_providers/_certificate/provider.py`.
...
4. **Update Facade and References**
   ...
   - Fix all imports of `load_certificate`, `certificate.py`, etc., across `src/aeat/auth/__init__.py`.
