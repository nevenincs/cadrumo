...
- **Task 1.6**: Ensure all public symbols have Google-style docstrings, type hints, use `aeat.errors.AeatError` derived exceptions, and use the project logger factory. Add minimal usage example in `src/aeat/browser/__init__.py`.
...
## 3. Testing Phase
- **Task 3.1**: Add `@pytest.mark.unit` tests colocated in `src/aeat/browser/` (e.g., `test_profile.py`, `test_session.py`, `test_health.py`) for the factory composition, profile loader, proxy wiring, and health check. These tests will use Playwright mocks or local-only checks.
- **Task 3.2**: Add one `@pytest.mark.live` test in `src/aeat/browser/test_live_evasion.py` that is opt-in (`AEAT_LIVE_TESTS=1`), hits a real public bot-detection probe (e.g., `https://bot.sannysoft.com/`), and asserts a successful non-automated score without using mocks.
...
- **Vaultspec Rules**: Files are correctly routed to `src/aeat/browser/` with strict adherence to `tags` format and wiki-links.
