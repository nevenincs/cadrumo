...
date: '2026-08-07'
modified: '2026-08-07'
body_hash: 'sha256:b3ab04714304b41c14f6f75263ff8d3d6050c98e103bb8995da28f79bbe69837'
tier: L1
related:
...
- [ ] `S06` - Add a real optional register-injection seam to list_filed_data_bulk and capture_filed_data_bulk so the sweep can accept an already-opened DeclaracionesRegisterSession, defaulting to today's resolve-a-live-session behaviour so every existing caller and signature is unchanged, then cover BEST_EFFORT continuation with it. Gate: a test passes a REAL DeclaracionesRegisterSession, never a stub or a patched production path, driven through a real headless browser whose navigation is route-intercepted with synthetic fixture content so no AEAT contact occurs, arranges one query pair to render a truncated grid and another to render a complete one, and asserts the truncated pair becomes a FiledDataCaptureFailureRow while the complete pair still yields its rows. This closes a gap wider than truncation: no failure kind of any sort currently has coverage of the sweep continuing past a failed pair; `src/cadrumo/application/live/_filed_data_capture.py`.
