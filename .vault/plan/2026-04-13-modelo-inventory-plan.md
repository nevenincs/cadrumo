...
date: '2026-04-13'
modified: '2026-07-17'
body_hash: 'sha256:52c266264e9c7ef0263f20bb3f15259de24586fe9a8fe0f11489d770f484cbf2'
related:
...
> **Reconciled 2026-07-12 â€” historical, not active.** Every unchecked legacy criterion below was delivered in the original Phase 1â€“9 execution records and then corrected or superseded by the accepted Modelo-inventory remediation. The current authority is the revision-backed registry contract; this plan must not be treated as a current backlog. Evidence is recorded in `2026-07-12-modelo-inventory-audit`.
...
- Research (data): `.vault/research/2026-04-13-modelo-inventory-research.md`
  sections 3.1â€“3.20 (D1) and 4 (D2 matrix).
- Existing public surfaces consumed by this feature:
  - `src/aeat/domain/deadlines/__init__.py` â€” `DeadlineEngine`, `CALENDAR`,
    `CanonicalWindow`, `PeriodKind`, `AutonomoProfile`, `IVARegime`,
    `applies_to`, `explain`.
  - `src/aeat/core/i18n/__init__.py` â€” `Translatable`, `Language`,
    `require_authoritative`.
  - `src/aeat/errors.py` â€” `AeatError` base.
  - `src/aeat/core/logging/...` â€” `get_logger`.
  - `src/aeat/domain/casillas/...` â€” catalogue loader (used only from
    `test_casilla_cross_reference.py`).
  - `src/aeat/entrypoints/cli/__init__.py` â€” Typer root `app`; sub-apps wired via
    `app.add_typer(...)`.
...
- **Conventional commits** on every commit â€” literal messages below.
- **No `.github/workflows/`** files. `tests/test_release_config.py`
  already guards the directory.
- **No new settings** in `src/aeat/config.py` â€” the catalogue is static.
- **Windows path safety** â€” tests must not hardcode POSIX separators;
  file lookups use `pathlib.Path`.

### Critical ADR clarification â€” `DeadlineRule`

The ADR (Â§7) describes `ModeloMetadata.deadline_rule: DeadlineRule`
...
- `year_plan` is the only public callable that touches
  ...
  dependency.
- The ADR's intent â€” "the CLI `year-plan` command consumes the deadline
  engine to produce a calendar listing" â€” is preserved exactly. The
...
- `_categories.py` â€”
  ...
  - `TaxpayerProfile(StrEnum)`: the eight members from the ADR Â§4.
  - `LegalCitationSource(StrEnum)`: `LEY="ley"`,
    ...
    `BOE="boe"`.
- `_citations.py` â€” `LegalCitation(BaseModel)` with
  ...
  values after `.strip()`.
- `_applicability.py` â€” `ModeloApplicability(BaseModel)` with the same
...
- `_metadata.py` â€” `ModeloMetadata(BaseModel)` with
  `model_config = ConfigDict(frozen=True, extra="forbid", strict=True, arbitrary_types_allowed=False)`
  (arbitrary types NOT needed â€” see the `DeadlineRule` clarification
  above). Fields:
  - `code: ModeloCode`
...
### Phase 3 â€” Error hierarchy

- `_errors.py` â€”
  - `ModeloRegistryError(AeatError)` â€” base.
  - `UnknownModeloError(ModeloRegistryError)` â€” raised from
    `get_modelo(code)` on unknown codes. Carries the offending string.
  - `RegistryIntegrityError(ModeloRegistryError)` â€” raised at import
...
### Phase 4 â€” Registry entries (one file per modelo)

Populate every `_entries/modelo_<code>.py` with a module-level
`ENTRY: ModeloMetadata = ModeloMetadata(...)`. Each entry is
constructed from the research doc Â§3 data; every `LegalCitation`
carries `is_curated_summary=True` in v1 (the on-disk corpus ships
curated Spanish summaries, not BOE verbatim bodies â€” ADR Â§9).

The data for each entry (from research Â§3 / Â§4) is:

| Code | Category | Cadence | Primary citations (article â†’ source) | `caps_into` | Mandatory profiles (from D2) | Optional profiles |
|:---:|:---:|:---:|:---|:---:|:---|:---|
...
| 200 | `SOCIEDADES` | `ANNUAL` | `ley-58-2003#29` (LEY) â€” Ley 27/2014 gap flagged in `known_gotchas` | `None` | `SL` | (none) |
| 202 | `SOCIEDADES` | `QUARTERLY` (3 periods â€” categorise as `QUARTERLY`; research maps to April/October/December) | `ley-58-2003#29` (LEY) | `MODELO_200` | (none) | `SL` |
...
| 369 | `IVA` | `QUARTERLY` | `ley-37-1992#164` (LEY), `rd-1624-1992#71` (REAL_DECRETO) â€” OSS TÃ­tulo IX Cap. XI gap flagged | `None` | (none) | all eight profiles (OSS opt-in) |
| 390 | `IVA` | `ANNUAL` | `ley-37-1992#164` (LEY), `rd-1624-1992#71` (REAL_DECRETO) | `None` | all seven autonomo profiles + `SL` | (none) |
| 720 | `INFORMATIVA` | `ANNUAL` | `rd-1065-2007#30` (REAL_DECRETO) â€” RGAT 42 bis/ter/54 bis gap flagged | `None` | `AUTONOMO_ED_BIENES_EXTRANJERO` | `SL` |
| 840 | `OTROS` | `AD_HOC` | `ley-58-2003#29` (LEY) â€” TRLHL arts 78â€“91 gap flagged | `None` | (none) | `SL` |

Notes applied to every entry:

- `display_label` carries `es` / `en` / `hu` from research Â§3
...
- `trigger_notes_es` copies the D2 footnote prose (research Â§4 f1â€“f20)
  matching the modelo.
...
- `submission_portal_hint` is the `Channel hint` URL from research Â§3.
- `known_gotchas` copies the research `Gotchas` bullets as a tuple of
  Spanish strings plus â€” where applicable â€” a note for corpus gaps
  ...
  720, "TRLHL arts 78â€“91 no disponibles en corpus on-disk" on 840,
  "OSS TÃ­tulo IX Cap. XI no disponible en corpus on-disk" on 369,
...
### Phase 5 â€” Registry assembly + invariants + `year_plan`
...
- Public helpers (re-exported from `__init__.py` in Phase 8):
  - `get_modelo(code: ModeloCode | str) -> ModeloMetadata` â€” accepts a
  ...
  - `modelos_for_profile(profile: TaxpayerProfile) -> tuple[ModeloMetadata, ...]`
    â€” returns every entry where `profile` is in
    `applicability.mandatory_profiles | applicability.optional_profiles`,
    sorted by `ModeloCode` value.
  - `year_plan(year: int, profile: AutonomoProfile) -> Schedule` â€”
    thin wrapper around
...
- `test_registry.py` covers:
  ...
  - A synthetic `RegistryIntegrityError` branch â€” exercised by
...
### Phase 6 â€” Casilla cross-reference test
...
- `_cli.py` builds a Typer app:
  ...
  - `year_plan(year: int, profile_tax_id: str, iva_regime: IVARegime,
    ...
    json_out: bool)` â€” builds an `AutonomoProfile` from the flags,
...
### Phase 8 â€” Public API lock + docstrings

- `src/aeat/domain/modelos/__init__.py` sets `__all__` to the exact tuple from
  the ADR Â§12 **minus any symbol that the `DeadlineRule` clarification
...
- Run, in order, `just lint`, `just typecheck`, `just test`,
  ...
  limitation.
- Verify `.env.example` and `tests/test_config.py` are untouched â€”
...
- **Windows path separators in tests.** **Mitigation:** every path
  ...
  paths.
- **Modelo 123 `caps_into = 193` dangling reference.** Research Â§3.10
...
- **Categorising 202's three-period cadence.** Research Â§3.19 says
...
(Â§1â€“Â§17); CLAUDE.md rules (`src/aeat/` layout, pydantic v2 mandate,
...
- **ADR deviation required.** ADR Â§7 names `DeadlineRule` as a field
  ...
  functional intent â€” CLI `year-plan` consumes the deadline engine â€”
...
- **Modelo 123 `caps_into` gap.** The research says `caps_into=193`;
  ...
  gotcha. Consistent with ADR Â§8 (`caps_into must resolve inside the
  registry`).
- **Cadence taxonomy matches ADR Â§3 exactly.** Monthly-only modelos
  (349 rolling-threshold escalation) are stored as `QUARTERLY` with
  a gotcha, matching research Â§3.6's default.
...
- **Curated-summary policy honoured.** Every v1 citation carries
  `is_curated_summary=True`. ADR Â§9 accepts this explicitly.
- **Sibling-branch stability.** The locked `__all__` tuple matches
  ADR Â§12 exactly except for `DeadlineRule` removal (which is not in
  the ADR's `__all__` either). `ModeloCode` member names are
  `MODELO_<code>` as ADR Â§2 locks. #77 and #93 can import
...
- **Commit messages** follow conventional-commits with
  `feat(models): â€¦` / `test(models): â€¦` / `docs(models): â€¦` /
  `chore(models): â€¦` prefixes.
...
**Verdict.** **Approved â€” execution may proceed**, with the single
...
- [x] `ModeloCode` StrEnum with 20 members, stable import surface â€” Phase 2.
- [x] `ModeloMetadata` strict/frozen pydantic v2 record â€” Phase 2.
- [x] `LegalCitation` with non-empty `quoted_text_es` invariant â€” Phase 2.
- [x] `ModeloApplicability` partition invariant â€” Phase 2.
- [x] All 20 modelos populated with citations, applicability, gotchas â€” Phase 4.
- [x] `MODELO_REGISTRY` frozen, complete, `caps_into`-verified â€” Phase 5.
- [x] `get_modelo` / `modelos_for_profile` / `year_plan` public helpers â€” Phase 5.
- [x] Error hierarchy rooted at `AeatError` â€” Phase 3.
- [x] Casilla catalogue cross-reference coverage â€” Phase 6.
- [x] CLI: `aeat modelos list/show/applicable-to/year-plan` + `--json` â€” Phase 7.
- [x] Trilingual display labels (`es`/`en`/`hu`) on every entry â€” Phase 4.
- [x] Public `__all__` locked per ADR Â§12 (minus the `DeadlineRule` deviation) â€” Phase 8.
- [x] Every test `@pytest.mark.unit`, zero mocks/patches/fakes/stubs â€” Phases 2/5/6/7.
- [x] Google-style docstrings + type hints on every public signature â€” Phase 8.
- [x] Conventional commits on every commit â€” every phase.
- [x] `just lint` / `just typecheck` / `just test` / `just hooks` green â€” Phase 9.
- [x] No `.github/workflows/` files â€” Phase 9.
- [x] No new settings in `src/aeat/config.py` / `.env.example` â€” Phase 9.
