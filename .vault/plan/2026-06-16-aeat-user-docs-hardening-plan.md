---
tags:
  - '#plan'
  - '#aeat-user-docs-hardening'
date: '2026-06-16'
modified: '2026-07-17'
tier: L1
related:
  - '[[2026-07-21-aeat-user-docs-hardening-adr]]'
  - '[[2026-07-13-docs-cli-sequences-adr]]'
  - '[[2026-06-04-aeat-cli-userdocs-hardening-research]]'
---

# `aeat-user-docs-hardening` plan

## Description

## Steps

- [x] `S01` - Harden authenticate-with-aeat.md; `docs/how-to/authenticate-with-aeat.md`.
- [x] `S02` - Harden censo-update.md; `docs/how-to/censo-update.md`.
- [x] `S03` - Harden check-aeat-notifications.md; `docs/how-to/check-aeat-notifications.md`.
- [x] `S04` - Harden choose-modelo.md; `docs/how-to/choose-modelo.md`.
- [x] `S05` - Harden classify-transactions.md; `docs/how-to/classify-transactions.md`.
- [x] `S06` - Harden classify-with-llm-evidence.md; `docs/how-to/classify-with-llm-evidence.md`.
- [x] `S07` - Harden classify-with-llm.md; `docs/how-to/classify-with-llm.md`.
- [x] `S08` - Harden correct-ledger-entries.md; `docs/how-to/correct-ledger-entries.md`.
- [x] `S09` - Harden file-at-aeat.md; `docs/how-to/file-at-aeat.md`.
- [x] `S10` - Harden filing-calendar.md; `docs/how-to/filing-calendar.md`.
- [x] `S11` - Harden filing-periods.md; `docs/how-to/filing-periods.md`.
- [x] `S12` - Harden filing-readiness.md; `docs/how-to/filing-readiness.md`.
- [x] `S13` - Harden filing-spine.md; `docs/how-to/filing-spine.md`.
- [x] `S14` - Harden import-bank-statements.md; `docs/how-to/import-bank-statements.md`.
- [x] `S15` - Harden index.md; `docs/how-to/index.md`.
- [x] `S16` - Harden justificante-receipts.md; `docs/how-to/justificante-receipts.md`.
- [x] `S17` - Harden ledger-evidence.md; `docs/how-to/ledger-evidence.md`.
- [x] `S18` - Harden modelo-036.md; `docs/how-to/modelo-036.md`.
- [x] `S19` - Harden modelo-303.md; `docs/how-to/modelo-303.md`.
- [x] `S20` - Harden modelo-390.md; `docs/how-to/modelo-390.md`.
- [x] `S21` - Harden onboarding.md; `docs/how-to/onboarding.md`.
- [x] `S22` - Harden profile-setup.md; `docs/how-to/profile-setup.md`.
- [x] `S23` - Harden protect-data-access.md; `docs/how-to/protect-data-access.md`.
- [x] `S24` - Harden quickstart.md; `docs/how-to/quickstart.md`.
- [x] `S25` - Harden read-live-aeat-data.md; `docs/how-to/read-live-aeat-data.md`.
- [x] `S26` - Harden reconcile.md; `docs/how-to/reconcile.md`.
- [x] `S27` - Harden review-calculation-values.md; `docs/how-to/review-calculation-values.md`.
- [x] `S28` - Harden review-queue.md; `docs/how-to/review-queue.md`.
- [x] `S29` - Harden review-with-google-sheets.md; `docs/how-to/review-with-google-sheets.md`.
- [x] `S30` - Harden setup-llm-classification.md; `docs/how-to/setup-llm-classification.md`.
- [x] `S31` - Harden troubleshooting.md; `docs/how-to/troubleshooting.md`.
- [x] `S32` - Harden verification-reports.md; `docs/how-to/verification-reports.md`.
## Parallelization

## Verification
