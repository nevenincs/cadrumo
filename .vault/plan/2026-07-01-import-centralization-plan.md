...
- [ ] `W01.P04.S03` - Promote `Modelo`, `OUT_OF_SCOPE_OBLIGATIONS`, `Period`, `PeriodError`, `PostFilingEventKind`, `ResultDisposition`, `STRICT_FROZEN_CONFIG`, `TaxDomain`, `UNMODELED_OBLIGATIONS`, `classify_post_filing_event_kind`, `post_filing_event_is_actionable`, `resolve_active_bucket_id`, `result_disposition_is_refund` to `aeat.core.__all__` with eager re-exports so the 35 existing cross-package consumer site(s) can import from the facade; `src/aeat/core/__init__.py`.
...
- [ ] `W01.P08.S08` - Promote `CarriedSecureObject`, `CoverageManifest`, `ProfileExportError`, `UserProfileError`, `UserProfileValidationError`, `utc_now` to `aeat.domain.user_profile.__all__` with eager re-exports so the 10 existing cross-package consumer site(s) can import from the facade; `src/aeat/domain/user_profile/__init__.py`.
...
- [ ] `W01.P10.S10` - Promote `FiscalResidency`, `compute_deduccion_maternidad_0611`, `modelo100_ecivil_export_code`, `register_profile_keys` to `aeat.domain.contribuyente.__all__` with eager re-exports so the 5 existing cross-package consumer site(s) can import from the facade; `src/aeat/domain/contribuyente/__init__.py`.
- [ ] `W01.P10.S11` - Decide and apply the public-surface disposition for `_profile_keys` (rename-to-public and promote, expose a narrower public API, or remove the reach) currently defined in `aeat.domain.contribuyente._keys` and consumed cross-package from `src/aeat/application/user_profile/_keys_validation.py`; `src/aeat/domain/contribuyente/__init__.py`.
...
- [ ] `W01.P14.S21` - Promote `decrypt_encrypted_bytes_column`, `decrypt_secure_object_payload`, `encrypt_secure_object_payload`, `secure_object_payload_aad` to `aeat.adapters.persistence.storage.crypto.__all__` with eager re-exports so the 9 existing cross-package consumer site(s) can import from the facade; `src/aeat/adapters/persistence/storage/crypto/__init__.py`.
...
- [ ] `W01.P15.S22` - Promote `WizardStatusError`, `WizardStatusReport`, `build_wizard_status`, `load_active_taxpayer_profile` to `aeat.application.wizard.__all__` with eager re-exports so the 4 existing cross-package consumer site(s) can import from the facade; `src/aeat/application/wizard/__init__.py`.
...
- [ ] `W01.P20.S32` - Promote `Base`, `SecureObjectRow` to `aeat.adapters.persistence.storage.sql.__all__` with eager re-exports so the 2 existing cross-package consumer site(s) can import from the facade; `src/aeat/adapters/persistence/storage/sql/__init__.py`.
...
- [ ] `W01.P21.S33` - Promote `find_invoice`, `find_unmatched` to `aeat.domain.invoices.__all__` with eager re-exports so the 4 existing cross-package consumer site(s) can import from the facade; `src/aeat/domain/invoices/__init__.py`.
...
- [ ] `W01.P23.S36` - Promote `OperatorInputScenario`, `verify_modelo_parity` to `aeat.application.storage.calc_sheets.__all__` with eager re-exports so the 2 existing cross-package consumer site(s) can import from the facade; `src/aeat/application/storage/calc_sheets/__init__.py`.
...
- [ ] `W01.P24.S37` - Promote `canonical_decimal_string` to `aeat.domain.__all__` with eager re-exports so the 6 existing cross-package consumer site(s) can import from the facade; `src/aeat/domain/__init__.py`.
...
- [ ] `W01.P25.S38` - Promote `is_link_only_mime_type` to `aeat.domain.attachments.__all__` with eager re-exports so the 1 existing cross-package consumer site(s) can import from the facade; `src/aeat/domain/attachments/__init__.py`.
...
- [ ] `W01.P27.S40` - Promote `BucketEventHistoryRepositoryProtocol` to `aeat.domain.buckets.__all__` with eager re-exports so the 24 existing cross-package consumer site(s) can import from the facade; `src/aeat/domain/buckets/__init__.py`.
...
- [ ] `W01.P28.S41` - Promote `RentaValidationError` to `aeat.domain.renta.__all__` with eager re-exports so the 2 existing cross-package consumer site(s) can import from the facade; `src/aeat/domain/renta/__init__.py`.
...
- [ ] `W01.P29.S42` - Promote `AuthState` to `aeat.application.workflow.__all__` with eager re-exports so the 1 existing cross-package consumer site(s) can import from the facade; `src/aeat/application/workflow/__init__.py`.
...
- [ ] `W01.P30.S43` - Promote `CasillaFieldKind` to `aeat.domain.calculations.__all__` with eager re-exports so the 2 existing cross-package consumer site(s) can import from the facade; `src/aeat/domain/calculations/__init__.py`.
...
- [ ] `W01.P31.S44` - Promote `PurchaseInvoiceEvidenceRepository` to `aeat.application.ledger.__all__` with eager re-exports so the 1 existing cross-package consumer site(s) can import from the facade; `src/aeat/application/ledger/__init__.py`.
...
- [ ] `W01.P33.S46` - Promote `clear_output_language_cache` to `aeat.core.i18n.__all__` with eager re-exports so the 5 existing cross-package consumer site(s) can import from the facade; `src/aeat/core/i18n/__init__.py`.
...
- [ ] `W01.P34.S47` - Promote `portal_host_name` to `aeat.domain.portals.__all__` with eager re-exports so the 1 existing cross-package consumer site(s) can import from the facade; `src/aeat/domain/portals/__init__.py`.
...
- [ ] `W02.P36.S49` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/__init__.py`.
- [ ] `W02.P36.S50` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_action_errors.py`.
- [ ] `W02.P36.S51` - Rewire 17 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`, `aeat.domain.modelos`; `src/aeat/application/modelo/_amendment_actions.py`.
- [ ] `W02.P36.S52` - Rewire 12 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core`, `aeat.domain.modelos`, `aeat.domain.transactions`; `src/aeat/application/modelo/_art109_activity_income.py`.
- [ ] `W02.P36.S53` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_art20_advisory.py`.
- [ ] `W02.P36.S54` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.aggregation`, `aeat.domain.modelos`; `src/aeat/application/modelo/_binding_resolution.py`.
- [ ] `W02.P36.S55` - Rewire 6 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.aggregation`, `aeat.domain.modelos`; `src/aeat/application/modelo/_borrador_binding.py`.
- [ ] `W02.P36.S56` - Rewire 32 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.aggregation`, `aeat.application.user_profile`, `aeat.application.workflow`, `aeat.core`, `aeat.domain.calculations.registry`, `aeat.domain.contribuyente`, `aeat.domain.modelos`; `src/aeat/application/modelo/_calculate_input.py`.
- [ ] `W02.P36.S57` - Rewire 33 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.aggregation`, `aeat.application.calculations`, `aeat.application.live`, `aeat.core`, `aeat.domain.buckets`, `aeat.domain.calculations.registry`, `aeat.domain.invoices`, `aeat.domain.modelos`, `aeat.domain.transactions`; `src/aeat/application/modelo/_calculation_actions.py`.
- [ ] `W02.P36.S58` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_calculation_aggregation_context.py`.
- [ ] `W02.P36.S59` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_calculation_helpers.py`.
- [ ] `W02.P36.S60` - Rewire 6 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.calculations`, `aeat.application.user_profile`, `aeat.domain.modelos`; `src/aeat/application/modelo/_calculation_preparation.py`.
- [ ] `W02.P36.S61` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain`, `aeat.domain.modelos`; `src/aeat/application/modelo/_calculation_resolution.py`.
- [ ] `W02.P36.S62` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.aggregation`; `src/aeat/application/modelo/_calculation_source_policy.py`.
- [ ] `W02.P36.S63` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_dt12_advisory.py`.
- [ ] `W02.P36.S64` - Rewire 14 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`, `aeat.domain.buckets`, `aeat.domain.iva_compensation`, `aeat.domain.modelos`; `src/aeat/application/modelo/_export.py`.
- [ ] `W02.P36.S65` - Rewire 20 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`, `aeat.domain.modelos`; `src/aeat/application/modelo/_external_import_actions.py`.
- [ ] `W02.P36.S66` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_filed_revision_observation.py`.
- [ ] `W02.P36.S67` - Rewire 17 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.calculations`, `aeat.domain.buckets`, `aeat.domain.modelos`; `src/aeat/application/modelo/_filing_actions.py`.
- [ ] `W02.P36.S68` - Rewire 9 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`, `aeat.domain.modelos`; `src/aeat/application/modelo/_history.py`.
- [ ] `W02.P36.S69` - Rewire 13 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.calculations`, `aeat.application.user_profile`, `aeat.domain.iva_compensation`, `aeat.domain.modelos`; `src/aeat/application/modelo/_iva_wallet_gate.py`.
- [ ] `W02.P36.S70` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva_compensation`, `aeat.domain.modelos`; `src/aeat/application/modelo/_iva_wallet_seed.py`.
- [ ] `W02.P36.S71` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_ledger_evidence_gate.py`.
- [ ] `W02.P36.S72` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.calculations.registry`; `src/aeat/application/modelo/_m210_rate.py`.
- [ ] `W02.P36.S73` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.calculations`; `src/aeat/application/modelo/_maritime_preview.py`.
- [ ] `W02.P36.S74` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_objective_estimation_advisory.py`.
- [ ] `W02.P36.S75` - Rewire 14 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_participation_index_rebuild.py`.
- [ ] `W02.P36.S76` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.aggregation`, `aeat.domain.modelos`; `src/aeat/application/modelo/_profile_binding.py`.
- [ ] `W02.P36.S77` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`, `aeat.domain.modelos`; `src/aeat/application/modelo/_profile_readiness_gate.py`.
- [ ] `W02.P36.S78` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_projection.py`.
- [ ] `W02.P36.S79` - Rewire 7 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`, `aeat.application.workflow`, `aeat.domain.modelos`; `src/aeat/application/modelo/_reconcile.py`.
- [ ] `W02.P36.S80` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core`, `aeat.domain.calculations.registry`; `src/aeat/application/modelo/_registry_discovery.py`.
- [ ] `W02.P36.S81` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_registry_helpers.py`.
- [ ] `W02.P36.S82` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_registry_resources.py`.
- [ ] `W02.P36.S83` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_required_binding_gate.py`.
- [ ] `W02.P36.S84` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_result_disposition_resolution.py`.
- [ ] `W02.P36.S85` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_result_summary.py`.
- [ ] `W02.P36.S86` - Rewire 19 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`, `aeat.domain.modelos`; `src/aeat/application/modelo/_revision_persistence.py`.
- [ ] `W02.P36.S87` - Rewire 6 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain`, `aeat.domain.modelos`; `src/aeat/application/modelo/_revision_replay_inputs.py`.
- [ ] `W02.P36.S88` - Rewire 13 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_selectors.py`.
- [ ] `W02.P36.S89` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_semantic_role_resolution.py`.
- [ ] `W02.P36.S90` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_taxation_comparison.py`.
- [ ] `W02.P36.S91` - Rewire 58 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.aggregation`, `aeat.application.calculations`, `aeat.application.workflow`, `aeat.core`, `aeat.domain.buckets`, `aeat.domain.calculations.registry`, `aeat.domain.iva_compensation`, `aeat.domain.modelos`, `aeat.domain.transactions`; `src/aeat/application/modelo/_verification_actions.py`.
- [ ] `W02.P36.S92` - Rewire 11 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.calculations`, `aeat.domain.modelos`; `src/aeat/application/modelo/_verification_cross_period.py`.
- [ ] `W02.P36.S93` - Rewire 9 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.calculations.registry`, `aeat.domain.modelos`; `src/aeat/application/modelo/_work_addressing.py`.
- [ ] `W02.P36.S94` - Rewire 10 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`, `aeat.domain.contribuyente`, `aeat.domain.modelos`; `src/aeat/application/modelo/_work_lifecycle.py`.
- [ ] `W02.P36.S95` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.deadlines`, `aeat.domain.modelos`; `src/aeat/application/modelo/_work_plazo.py`.
- [ ] `W02.P36.S96` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/modelo/_workflow_gate.py`.
...
- [ ] `W02.P37.S97` - Rewire 7 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.fincas`; `src/aeat/adapters/persistence/profile/fincas.py`.
- [ ] `W02.P37.S98` - Rewire 61 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.blob_store`, `aeat.adapters.persistence.storage.crypto`, `aeat.adapters.persistence.storage.envelope`, `aeat.adapters.persistence.storage.master_key`, `aeat.adapters.persistence.storage.secret_store`; `src/aeat/adapters/persistence/storage/__init__.py`.
- [ ] `W02.P37.S99` - Rewire 8 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.blob_store`, `aeat.adapters.persistence.storage.crypto`, `aeat.adapters.persistence.storage.envelope`, `aeat.adapters.persistence.storage.master_key`; `src/aeat/adapters/persistence/storage/_rotation.py`.
- [ ] `W02.P37.S100` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.attachments`; `src/aeat/adapters/persistence/storage/attachment.py`.
- [ ] `W02.P37.S101` - Rewire 10 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.crypto`, `aeat.adapters.persistence.storage.envelope`, `aeat.adapters.persistence.storage.master_key`; `src/aeat/adapters/persistence/storage/blob_store/_blob_store.py`.
- [ ] `W02.P37.S102` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.secret_store`; `src/aeat/adapters/persistence/storage/blob_store/_materialisation.py`.
- [ ] `W02.P37.S103` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`; `src/aeat/adapters/persistence/storage/bucket/_export_header.py`.
- [ ] `W02.P37.S104` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`; `src/aeat/adapters/persistence/storage/bucket/_manifest.py`.
- [ ] `W02.P37.S105` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.master_key`; `src/aeat/adapters/persistence/storage/crypto/_encrypted_columns.py`.
- [ ] `W02.P37.S106` - Rewire 6 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.crypto`, `aeat.adapters.persistence.storage.master_key`, `aeat.core.time`; `src/aeat/adapters/persistence/storage/envelope/_envelope.py`.
- [ ] `W02.P37.S107` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.crypto`, `aeat.adapters.persistence.storage.sql`; `src/aeat/adapters/persistence/storage/envelope/_repository_test_suite.py`.
- [ ] `W02.P37.S108` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.crypto`; `src/aeat/adapters/persistence/storage/master_key/_master_key.py`.
- [ ] `W02.P37.S109` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.crypto`; `src/aeat/adapters/persistence/storage/master_key/_master_key_bucket_dek.py`.
- [ ] `W02.P37.S110` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.crypto`; `src/aeat/adapters/persistence/storage/master_key/_master_key_derivation.py`.
- [ ] `W02.P37.S111` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.crypto`; `src/aeat/adapters/persistence/storage/master_key/_master_key_ephemeral.py`.
- [ ] `W02.P37.S112` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.crypto`; `src/aeat/adapters/persistence/storage/master_key/_recovery.py`.
- [ ] `W02.P37.S113` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.crypto`; `src/aeat/adapters/persistence/storage/master_key/_recovery_facade.py`.
- [ ] `W02.P37.S114` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`; `src/aeat/adapters/persistence/storage/master_key/_recovery_record.py`.
- [ ] `W02.P37.S115` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.master_key`; `src/aeat/adapters/persistence/storage/runtime.py`.
- [ ] `W02.P37.S116` - Rewire 8 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.blob_store`, `aeat.adapters.persistence.storage.crypto`, `aeat.adapters.persistence.storage.envelope`, `aeat.adapters.persistence.storage.master_key`, `aeat.core.time`; `src/aeat/adapters/persistence/storage/secret_store/_secret_store.py`.
- [ ] `W02.P37.S117` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.crypto`; `src/aeat/adapters/persistence/storage/sql/_orm.py`.
- [ ] `W02.P37.S118` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.crypto`; `src/aeat/adapters/persistence/storage/sql/_secure_object_integrity.py`.
- [ ] `W02.P37.S119` - Rewire 6 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.crypto`, `aeat.adapters.persistence.storage.master_key`; `src/aeat/adapters/persistence/storage/sql/secure_objects.py`.
...
- [ ] `W02.P38.S120` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.i18n`; `src/aeat/entrypoints/cli/__init__.py`.
- [ ] `W02.P38.S121` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.portals`; `src/aeat/entrypoints/cli/_app_live_portals_cli.py`.
- [ ] `W02.P38.S122` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.aeat.sede`; `src/aeat/entrypoints/cli/_app_live_verify_cli.py`.
- [ ] `W02.P38.S123` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.i18n`; `src/aeat/entrypoints/cli/_common.py`.
- [ ] `W02.P38.S124` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.workflow`; `src/aeat/entrypoints/cli/_config/_capabilities_cli.py`.
- [ ] `W02.P38.S125` - Rewire 9 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.google`; `src/aeat/entrypoints/cli/_config/_google.py`.
- [ ] `W02.P38.S126` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.google`; `src/aeat/entrypoints/cli/_config/_google_folder.py`.
- [ ] `W02.P38.S127` - Rewire 11 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.google`, `aeat.adapters.outbound.storage`, `aeat.application.storage.calc_sheets`; `src/aeat/entrypoints/cli/_config/_google_sync_calc.py`.
- [ ] `W02.P38.S128` - Rewire 22 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage.master_key`, `aeat.application.user_profile`, `aeat.application.workflow`, `aeat.core`, `aeat.domain.buckets`, `aeat.domain.user_profile`; `src/aeat/entrypoints/cli/_config/_profile_bundle.py`.
- [ ] `W02.P38.S129` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.invoices`, `aeat.domain.iva`; `src/aeat/entrypoints/cli/_ledger.py`.
- [ ] `W02.P38.S130` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.google`, `aeat.adapters.outbound.storage`; `src/aeat/entrypoints/cli/_ledger_lifecycle_cli.py`.
- [ ] `W02.P38.S131` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva`, `aeat.domain.transactions`; `src/aeat/entrypoints/cli/_ledger_llm_cli.py`.
- [ ] `W02.P38.S132` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/entrypoints/cli/_ledger_read_cli.py`.
- [ ] `W02.P38.S133` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.contribuyente`, `aeat.domain.deadlines`; `src/aeat/entrypoints/cli/_ledger_support.py`.
- [ ] `W02.P38.S134` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva_compensation`; `src/aeat/entrypoints/cli/_modelo_iva_wallet_cli.py`.
- [ ] `W02.P38.S135` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.renta`; `src/aeat/entrypoints/cli/_modelo_maritime_cli.py`.
- [ ] `W02.P38.S136` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/entrypoints/cli/_modelo_payloads.py`.
- [ ] `W02.P38.S137` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/entrypoints/cli/_modelo_records_cli.py`.
- [ ] `W02.P38.S138` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/entrypoints/cli/_modelo_work_revision_payloads.py`.
- [ ] `W02.P38.S139` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.calculations`; `src/aeat/entrypoints/cli/_modelo_work_verification_cli.py`.
...
- [ ] `W02.P38.S141` - Rewire 6 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.overview`, `aeat.core`; `src/aeat/entrypoints/cli/_overview_rendering.py`.
...
- [ ] `W02.P39.S142` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`; `src/aeat/adapters/outbound/aeat/auth/_authenticator_types.py`.
- [ ] `W02.P39.S143` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.user_profile`; `src/aeat/adapters/outbound/aeat/auth/_clave_movil.py`.
- [ ] `W02.P39.S144` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`; `src/aeat/adapters/outbound/aeat/auth/certificate.py`.
- [ ] `W02.P39.S145` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`; `src/aeat/adapters/outbound/aeat/browser/_site_health_parsers.py`.
- [ ] `W02.P39.S146` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.aeat.auth`; `src/aeat/adapters/outbound/aeat/sede/_auth_state.py`.
- [ ] `W02.P39.S147` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.aeat.auth`; `src/aeat/adapters/outbound/aeat/sede/_censo_live.py`.
- [ ] `W02.P39.S148` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.aeat.auth`; `src/aeat/adapters/outbound/aeat/sede/_declarations.py`.
- [ ] `W02.P39.S149` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva_compensation`; `src/aeat/adapters/outbound/aeat/sede/_declarations_observations.py`.
- [ ] `W02.P39.S150` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.aeat.auth`; `src/aeat/adapters/outbound/aeat/sede/_iva_compensation_wallet.py`.
- [ ] `W02.P39.S151` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.aeat.auth`, `aeat.core.parsing`; `src/aeat/adapters/outbound/aeat/sede/_notifications.py`.
- [ ] `W02.P39.S152` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.aeat.auth`; `src/aeat/adapters/outbound/aeat/sede/_walker.py`.
- [ ] `W02.P39.S153` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.aeat.sede`, `aeat.domain.justificante`; `src/aeat/adapters/outbound/aeat/verify/__init__.py`.
- [ ] `W02.P39.S154` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.storage`; `src/aeat/adapters/outbound/google/_api.py`.
- [ ] `W02.P39.S155` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.storage`; `src/aeat/adapters/outbound/google/_calc_sheets_apply.py`.
- [ ] `W02.P39.S156` - Rewire 9 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.storage`, `aeat.application.storage.calc_sheets`, `aeat.core.time`; `src/aeat/adapters/outbound/google/_calc_sheets_pull.py`.
- [ ] `W02.P39.S157` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.storage`; `src/aeat/adapters/outbound/google/_document_link_resolver.py`.
- [ ] `W02.P39.S158` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.google`; `src/aeat/adapters/outbound/storage/_factory.py`.
...
- [ ] `W02.P40.S159` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`, `aeat.domain.modelos`, `aeat.domain.transactions`; `src/aeat/application/ledger/_actions_classification.py`.
- [ ] `W02.P40.S160` - Rewire 10 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`, `aeat.domain.attachments`, `aeat.domain.buckets`, `aeat.domain.invoices`, `aeat.domain.modelos`, `aeat.domain.transactions`; `src/aeat/application/ledger/_actions_common.py`.
- [ ] `W02.P40.S161` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`, `aeat.domain.transactions`; `src/aeat/application/ledger/_actions_export.py`.
- [ ] `W02.P40.S162` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`, `aeat.domain.transactions`; `src/aeat/application/ledger/_actions_import.py`.
- [ ] `W02.P40.S163` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`, `aeat.domain.invoices`, `aeat.domain.modelos`, `aeat.domain.transactions`; `src/aeat/application/ledger/_actions_lifecycle.py`.
- [ ] `W02.P40.S164` - Rewire 6 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.attachments`, `aeat.domain.buckets`, `aeat.domain.invoices`, `aeat.domain.modelos`, `aeat.domain.transactions`; `src/aeat/application/ledger/_actions_manual.py`.
- [ ] `W02.P40.S165` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`, `aeat.domain.modelos`, `aeat.domain.transactions`; `src/aeat/application/ledger/_actions_split_merge.py`.
- [ ] `W02.P40.S166` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain`, `aeat.domain.buckets`; `src/aeat/application/ledger/_business_operation_invoice.py`.
- [ ] `W02.P40.S167` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain`, `aeat.domain.buckets`; `src/aeat/application/ledger/_evidence.py`.
- [ ] `W02.P40.S168` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.inbound.pdf`; `src/aeat/application/ledger/_evidence_textlayer.py`.
- [ ] `W02.P40.S169` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`, `aeat.domain.buckets`, `aeat.domain.transactions`; `src/aeat/application/ledger/_llm_classification.py`.
- [ ] `W02.P40.S170` - Rewire 7 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva`, `aeat.domain.modelos`, `aeat.domain.transactions`; `src/aeat/application/ledger/_models.py`.
- [ ] `W02.P40.S171` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/ledger/_participation_read.py`.
- [ ] `W02.P40.S172` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`; `src/aeat/application/ledger/_review_projection.py`.
- [ ] `W02.P40.S173` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.transactions`; `src/aeat/application/ledger/_rule_repository.py`.
...
- [ ] `W02.P41.S174` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`, `aeat.domain.user_profile`; `src/aeat/application/user_profile/_aggregate.py`.
- [ ] `W02.P41.S175` - Rewire 14 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.modelo`, `aeat.domain.modelos`, `aeat.domain.transactions`, `aeat.domain.user_profile`; `src/aeat/application/user_profile/_bundle.py`.
- [ ] `W02.P41.S176` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.workflow`; `src/aeat/application/user_profile/_capabilities.py`.
- [ ] `W02.P41.S177` - Rewire 8 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.aeat.sede`, `aeat.application.live`, `aeat.domain.buckets`, `aeat.domain.user_profile`; `src/aeat/application/user_profile/_censo_sync.py`.
- [ ] `W02.P41.S178` - Rewire 39 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.aeat.sede`, `aeat.adapters.persistence.storage.envelope`, `aeat.adapters.persistence.storage.sql`, `aeat.application.aggregation`, `aeat.application.calculations`, `aeat.application.evidence`, `aeat.application.filing`, `aeat.application.ledger`, `aeat.application.live`, `aeat.application.modelo`, `aeat.domain.filing`, `aeat.domain.justificante`, `aeat.domain.submission`, `aeat.domain.user_profile`; `src/aeat/application/user_profile/_custody_carry.py`.
- [ ] `W02.P41.S179` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.contribuyente`; `src/aeat/application/user_profile/_keys_validation.py`.
- [ ] `W02.P41.S180` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.workflow`; `src/aeat/application/user_profile/_language_resolver.py`.
- [ ] `W02.P41.S181` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`, `aeat.domain.user_profile`; `src/aeat/application/user_profile/_lifecycle.py`.
- [ ] `W02.P41.S182` - Rewire 11 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.persistence.storage`, `aeat.adapters.persistence.storage.bucket`, `aeat.adapters.persistence.storage.master_key`, `aeat.application.workflow`, `aeat.domain.user_profile`; `src/aeat/application/user_profile/_orchestration.py`.
- [ ] `W02.P41.S183` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.workflow`, `aeat.domain.user_profile`; `src/aeat/application/user_profile/_profile_repository.py`.
- [ ] `W02.P41.S184` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.deadlines`; `src/aeat/application/user_profile/_projections.py`.
- [ ] `W02.P41.S185` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.i18n`; `src/aeat/application/user_profile/_repository.py`.
- [ ] `W02.P41.S186` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.workflow`, `aeat.domain.deadlines`; `src/aeat/application/user_profile/_testing.py`.
- [ ] `W02.P41.S187` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.parsing`; `src/aeat/application/user_profile/_validation.py`.
...
- [ ] `W02.P42.S188` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.inbound.pdf`; `src/aeat/adapters/inbound/borrador/_extractors/modelo_100_summary_v2025.py`.
- [ ] `W02.P42.S189` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.inbound.pdf`; `src/aeat/adapters/inbound/borrador/_parsers/_pdfplumber_backend.py`.
- [ ] `W02.P42.S190` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.inbound.pdf`; `src/aeat/adapters/inbound/borrador/_schema.py`.
- [ ] `W02.P42.S191` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.inbound.pdf`; `src/aeat/adapters/inbound/declaracion/_parser.py`.
- [ ] `W02.P42.S192` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.inbound.pdf`; `src/aeat/adapters/inbound/declaracion/_parsers/_pdfplumber_backend.py`.
- [ ] `W02.P42.S193` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.inbound.pdf`; `src/aeat/adapters/inbound/declaracion/_schema.py`.
- [ ] `W02.P42.S194` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain`; `src/aeat/adapters/inbound/financial/__init__.py`.
- [ ] `W02.P42.S195` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.inbound.pdf`, `aeat.domain.justificante`; `src/aeat/adapters/inbound/justificante/_extract.py`.
- [ ] `W02.P42.S196` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.justificante`; `src/aeat/adapters/inbound/justificante/_parser.py`.
- [ ] `W02.P42.S197` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.inbound.pdf`, `aeat.domain.justificante`; `src/aeat/adapters/inbound/justificante/_parsers/__init__.py`.
- [ ] `W02.P42.S198` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.inbound.pdf`, `aeat.domain.justificante`; `src/aeat/adapters/inbound/justificante/_parsers/_pdfplumber_backend.py`.
...
- [ ] `W02.P43.S199` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva_compensation`; `src/aeat/application/calculations/_binding_prefill.py`.
- [ ] `W02.P43.S200` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.aggregation`; `src/aeat/application/calculations/_iva_compensation_annual_partition.py`.
- [ ] `W02.P43.S201` - Rewire 8 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva_compensation`; `src/aeat/application/calculations/_iva_compensation_history.py`.
- [ ] `W02.P43.S202` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva_compensation`; `src/aeat/application/calculations/_iva_wallet_balance.py`.
- [ ] `W02.P43.S203` - Rewire 11 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.aggregation`, `aeat.domain.iva_compensation`; `src/aeat/application/calculations/_iva_wallet_reconciliation.py`.
- [ ] `W02.P43.S204` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`; `src/aeat/application/calculations/_m111_no_retenciones.py`.
- [ ] `W02.P43.S205` - Rewire 8 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.renta`; `src/aeat/application/calculations/_maritime_exemption_service.py`.
- [ ] `W02.P43.S206` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.aggregation`; `src/aeat/application/calculations/_multi_year.py`.
- [ ] `W02.P43.S207` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva_compensation`; `src/aeat/application/calculations/_observations_repository.py`.
- [ ] `W02.P43.S208` - Rewire 9 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.aggregation`, `aeat.application.storage.calc_sheets`, `aeat.application.user_profile`; `src/aeat/application/calculations/_relation_prefill.py`.
- [ ] `W02.P43.S209` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.parsing`; `src/aeat/application/calculations/_row_set_assembly.py`.
...
- [ ] `W02.P44.S210` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.calculations.registry`; `src/aeat/domain/calculations/_export_field_kind.py`.
- [ ] `W02.P44.S211` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.parsing`; `src/aeat/domain/calculations/registry/_export_parse.py`.
- [ ] `W02.P44.S212` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/domain/calculations/registry/_invoice_bindings.py`.
- [ ] `W02.P44.S213` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva`; `src/aeat/domain/calculations/registry/_ledger_bindings.py`.
- [ ] `W02.P44.S214` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core`; `src/aeat/domain/calculations/registry/_queries.py`.
- [ ] `W02.P44.S215` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core`; `src/aeat/domain/calculations/registry/_schema.py`.
- [ ] `W02.P44.S216` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.user_profile`; `src/aeat/domain/calculations/registry/_validate.py`.
...
- [ ] `W02.P45.S217` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`; `src/aeat/application/auth/_acquisition_lock.py`.
- [ ] `W02.P45.S218` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.workflow`; `src/aeat/application/auth/_actions.py`.
- [ ] `W02.P45.S219` - Rewire 6 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`, `aeat.application.workflow`; `src/aeat/application/auth/_operator.py`.
- [ ] `W02.P45.S220` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.aeat.auth`, `aeat.application.user_profile`, `aeat.application.workflow`; `src/aeat/application/auth/_operator_probes.py`.
- [ ] `W02.P45.S221` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`; `src/aeat/application/auth/_operator_scope.py`.
- [ ] `W02.P45.S222` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`, `aeat.core.time`; `src/aeat/application/auth/_sessions.py`.
...
- [ ] `W02.P46.S223` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.submission`; `src/aeat/application/filing/_calculate.py`.
- [ ] `W02.P46.S224` - Rewire 17 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core`, `aeat.domain.calculations`, `aeat.domain.calculations.registry`, `aeat.domain.contribuyente`, `aeat.domain.submission`; `src/aeat/application/filing/_export.py`.
- [ ] `W02.P46.S225` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain`; `src/aeat/application/filing/_history_models.py`.
- [ ] `W02.P46.S226` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain`, `aeat.domain.submission`; `src/aeat/application/filing/_review.py`.
- [ ] `W02.P46.S227` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.filing`; `src/aeat/application/filing/_testing_registry.py`.
- [ ] `W02.P46.S228` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.wizard`, `aeat.application.workflow`; `src/aeat/application/filing/runtime.py`.
...
- [ ] `W02.P47.S229` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core`, `aeat.domain.deadlines`; `src/aeat/application/overview/_agenda.py`.
- [ ] `W02.P47.S230` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core`, `aeat.domain.deadlines`, `aeat.domain.modelos`; `src/aeat/application/overview/_backlog.py`.
- [ ] `W02.P47.S231` - Rewire 22 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.aeat.sede`, `aeat.application.live`, `aeat.core`, `aeat.domain.calculations.registry`, `aeat.domain.deadlines`, `aeat.domain.justificante`, `aeat.domain.modelos`; `src/aeat/application/overview/_calendar.py`.
- [ ] `W02.P47.S232` - Rewire 6 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core`, `aeat.domain.deadlines`; `src/aeat/application/overview/_calendar_models.py`.
- [ ] `W02.P47.S233` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.modelo`, `aeat.core`, `aeat.domain.deadlines`; `src/aeat/application/overview/_coverage.py`.
- [ ] `W02.P47.S234` - Rewire 8 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core`, `aeat.core.resources`, `aeat.domain.calculations.registry`, `aeat.domain.deadlines`; `src/aeat/application/overview/_explain.py`.
...
- [ ] `W02.P48.S235` - Rewire 8 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.contribuyente`, `aeat.domain.deadlines`; `src/aeat/application/wizard/_catalogue.py`.
- [ ] `W02.P48.S236` - Rewire 12 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`, `aeat.application.workflow`, `aeat.domain.contribuyente`, `aeat.domain.deadlines`; `src/aeat/application/wizard/_commands.py`.
- [ ] `W02.P48.S237` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.contribuyente`; `src/aeat/application/wizard/_compiler.py`.
- [ ] `W02.P48.S238` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`, `aeat.application.workflow`; `src/aeat/application/wizard/_persistence.py`.
- [ ] `W02.P48.S239` - Rewire 6 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`, `aeat.application.workflow`, `aeat.domain.deadlines`; `src/aeat/application/wizard/_status.py`.
...
- [ ] `W02.P49.S240` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva`; `src/aeat/application/aggregation/_iva_ledger.py`.
- [ ] `W02.P49.S241` - Rewire 11 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/aggregation/_ledger_filing_snapshot.py`.
- [ ] `W02.P49.S242` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/aggregation/_source_mesh.py`.
- [ ] `W02.P49.S243` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.modelo`; `src/aeat/application/aggregation/_source_profile.py`.
...
- [ ] `W02.P50.S244` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.invoices`; `src/aeat/application/invoices/__init__.py`.
- [ ] `W02.P50.S245` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.invoices`; `src/aeat/application/invoices/_queries.py`.
- [ ] `W02.P50.S246` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.invoices`, `aeat.domain.transactions`; `src/aeat/application/invoices/_reconciliation.py`.
- [ ] `W02.P50.S247` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.aggregation`, `aeat.domain.invoices`; `src/aeat/application/invoices/_source_resolver.py`.
...
- [ ] `W02.P51.S249` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.i18n`; `src/aeat/application/workflow/_persistence.py`.
- [ ] `W02.P51.S250` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`; `src/aeat/application/workflow/_profile_health.py`.
- [ ] `W02.P51.S251` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.submission`; `src/aeat/application/workflow/_protocols.py`.
...
- [ ] `W02.P52.S253` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`; `src/aeat/application/review/_adapters.py`.
...
- [ ] `W02.P53.S255` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`; `src/aeat/application/storage/calc_sheets/_evidence.py`.
- [ ] `W02.P53.S256` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.adapters.outbound.google`; `src/aeat/application/storage/calc_sheets/_parity_harness.py`.
- [ ] `W02.P53.S257` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`; `src/aeat/application/storage/calc_sheets/_records.py`.
...
- [ ] `W02.P54.S258` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.errors`; `src/aeat/core/resources/_errors.py`.
- [ ] `W02.P54.S259` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva`; `src/aeat/core/resources/_repos/iva_catalogues.py`.
- [ ] `W02.P54.S260` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva`; `src/aeat/core/resources/_repos/iva_rate_tables.py`.
...
- [ ] `W02.P55.S261` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.parsing`; `src/aeat/domain/contribuyente/__init__.py`.
- [ ] `W02.P55.S262` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.parsing`; `src/aeat/domain/contribuyente/_descendant_facts.py`.
- [ ] `W02.P55.S263` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.parsing`; `src/aeat/domain/contribuyente/family.py`.
...
- [ ] `W02.P56.S264` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.modelos`, `aeat.domain.transactions`; `src/aeat/domain/user_profile/_portable_export.py`.
- [ ] `W02.P56.S265` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.calculations`; `src/aeat/domain/user_profile/_registry_contract.py`.
- [ ] `W02.P56.S266` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.parsing`; `src/aeat/domain/user_profile/_values.py`.
...
- [ ] `W02.P57.S267` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`, `aeat.domain.modelos`; `src/aeat/application/evidence/_models.py`.
- [ ] `W02.P57.S268` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`; `src/aeat/application/evidence/_service.py`.
...
- [ ] `W02.P58.S269` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva_compensation`, `aeat.domain.modelos`; `src/aeat/application/live/_filed_observation_persistence.py`.
- [ ] `W02.P58.S270` - Rewire 6 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`, `aeat.domain.iva_compensation`; `src/aeat/application/live/_iva_remote_state.py`.
...
- [ ] `W02.P59.S271` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`; `src/aeat/core/observability/_context.py`.
- [ ] `W02.P59.S272` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`; `src/aeat/core/observability/_models.py`.
...
- [ ] `W02.P60.S273` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.iva`; `src/aeat/domain/invoices/_enums.py`.
- [ ] `W02.P60.S274` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.parsing`; `src/aeat/domain/invoices/_models.py`.
...
- [ ] `W02.P61.S275` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.calculations.registry`; `src/aeat/domain/modelos/_calculation_revision.py`.
- [ ] `W02.P61.S276` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.contribuyente`; `src/aeat/domain/modelos/_work_unit.py`.
...
- [ ] `W02.P62.S277` - Rewire 4 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`, `aeat.domain.iva`; `src/aeat/domain/transactions/_models.py`.
- [ ] `W02.P62.S278` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`; `src/aeat/domain/transactions/_raw_transaction.py`.
...
- [ ] `W02.P64.S280` - Rewire 5 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`, `aeat.application.workflow`; `src/aeat/application/config_reset.py`.
...
- [ ] `W02.P65.S281` - Rewire 7 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`, `aeat.application.wizard`, `aeat.application.workflow`; `src/aeat/application/diagnostics.py`.
...
- [ ] `W02.P66.S282` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.buckets`; `src/aeat/application/inventory/_service.py`.
...
- [ ] `W02.P67.S283` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.manuals`; `src/aeat/application/registry/_corpus.py`.
...
- [ ] `W02.P68.S284` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`, `aeat.application.workflow`; `src/aeat/application/setup/_service.py`.
...
- [ ] `W02.P69.S285` - Rewire 9 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.user_profile`, `aeat.application.workflow`, `aeat.domain.modelos`; `src/aeat/application/state_projection.py`.
...
- [ ] `W02.P70.S286` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.application.modelo`; `src/aeat/application/storage_write_policy.py`.
...
- [ ] `W02.P71.S287` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.transactions`; `src/aeat/application/transactions/_import.py`.
...
- [ ] `W02.P72.S288` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.i18n`; `src/aeat/core/config.py`.
...
- [ ] `W02.P73.S289` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`; `src/aeat/core/corpus_manifest/__init__.py`.
...
- [ ] `W02.P74.S290` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.observability`; `src/aeat/core/json_contract.py`.
...
- [ ] `W02.P75.S291` - Rewire 3 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.observability`; `src/aeat/core/logging.py`.
...
- [ ] `W02.P76.S292` - Rewire 1 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.core.time`; `src/aeat/domain/attachments/_models.py`.
...
- [ ] `W02.P77.S293` - Rewire 2 cross-package private import name(s) in this file onto the promoted facade(s) of `aeat.domain.contribuyente`; `src/aeat/domain/deadlines/_models.py`.
...
- [ ] `W06.P90.S387` - Codify the durable lessons: refine service-imports-via-top-level-reexports with the mechanical-vs-disposition promotion split, and author a new rule dynamic-import-targets-the-public-facade capturing the setup_answers lazy-import retargeting lesson; `.vaultspec/rules/rules/`.
