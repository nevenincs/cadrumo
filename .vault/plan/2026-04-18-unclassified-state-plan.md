...
Edit `src/aeat/financial/transactions/_enums.py`:
...
Edit `src/aeat/financial/transactions/_models.py`:
...
- Export `ClassificationHistoryEntry` from `src/aeat/financial/transactions/__init__.py`.

### Step 3 — extend `set_classification` to append history

Edit `src/aeat/financial/transactions/_service.py`:
...
Edit `src/aeat/cli/financial/txs.py`:
...
Create `src/aeat/cli/review/__init__.py`:
...
- `history.py` exposes `history_cmd(transaction_id: str)` that loads the configured catalogue (via the shared helper used by `aeat financial txs`, extracted to `src/aeat/cli/financial/_catalogue.py` so both CLIs call the same loader) and prints a JSON list of history entries + the current head, oldest first. Missing transactions exit 2 with a clear message.
- Wire `app.add_typer(review_module.app, name="review", help="Pipeline decision review surfaces (#237/#232/#204).")` in `src/aeat/cli/__init__.py`.
- Relative-imports mandate (#162): every import inside `src/aeat/cli/review/` uses `.history`, `..financial._catalogue`, `...financial.transactions`, etc. No absolute `aeat.*` imports; the `scripts/check_relative_imports.py` guard enforces this in CI.

**Scope note** — the `_catalogue_path` / `_load_catalogue_required` helpers already live in `src/aeat/cli/financial/txs.py` and need to be shared by the new `review` command. The extraction to `src/aeat/cli/financial/_catalogue.py` is a pure code move, no behaviour change, and is called out explicitly so it does not surprise the code reviewer as an un-ADR'd refactor.
...
- `src/aeat/financial/transactions/test_cli.py` — replace `UNCLASSIFIED` string / enum; update the `test_financial_txs_show_emits_json_payload` assertion from `"UNCLASSIFIED"` to `"NOT_YET_PROCESSED"`; add tests for `--state`, for `--state + --unclassified` rejection, and for `classify --reason`.
- `src/aeat/financial/transactions/test_models.py` — keep passing; add a negative test `BusinessClassification("UNCLASSIFIED")` raises `ValueError`; add `ClassificationHistoryEntry` JSON round-trip.
- `src/aeat/cli/review/test_review.py` — new. Exercise `aeat review history` on a seeded catalogue across three classifications (e.g. initial `NOT_YET_PROCESSED` → manual `BUSINESS` → override `MIXED`), asserting oldest-first order and the current head appended last.
...
- **Catalogue loader sharing** — extracting `_catalogue_path` / `_load_catalogue_required` into `src/aeat/cli/financial/_catalogue.py` is a pure move. Imports in `txs.py` and the new review command both read from it.

## Definition of done

- Every file in the Call-site inventory is updated; no occurrence of `UNCLASSIFIED` remains in `src/aeat/financial/transactions/` or `src/aeat/cli/financial/` outside the legacy-alias test.
