...
- [x] `W04.P05.S23` - Resolve register_schema gate via clean-file-only key realignment: rename 13 stale capture/refresh/reconcile_from_justificante/reconciliation_history register_schema keys in the 4 CLEAN payload modules to the committed pull/file/history leaf paths. Gate test_every_cli_leaf_has_a_registered_schema green; `conformance 92/92, invocation 43/43. No dirty/peer-WIP file touched; `src/aeat/entrypoints/cli/_app_live_payloads.py`.
- [x] `W04.P05.S24` - Resolve exception-base hygiene red: allowlist build-time TerminologyError(ValueError) in the gate's sanctioned _ALLOWLIST (mirrors EnrollmentEvidenceError precedent) since the loader deliberately keeps Handbook errors outside the AeatError runtime hierarchy per its documented rationale; `gate green; `src/aeat/core/errors/tests/test_exception_base_hygiene.py`.
- [x] `W04.P05.S25` - Full CLI suite triage (847s run, 1538 passed / 70 failed / 25 errors): ZERO failures touch this campaign's surfaces; `all touched surfaces green (105 passed: conformance+hygiene+suggestions+error-registry). Remaining reds are active peer-WIP regressions in dirty/peer-owned files - 30 EOFError from a shared auth-preflight getpass in non-tty harness, wizard profile-key registration break (wizard package dirty), storage bucket-DEK key-schedule (MasterKeyMaterialMissingError), plus ledger/modelo peer campaigns - all forbidden to edit under worktree-safety. Also fixed the pre-existing vendored-click error-registry suggestion contract (same family as the synonym fix); `src/aeat/entrypoints/cli/tests/test_error_registry_contract.py`.
- [x] `W04.P05.S26` - Exhaustive clean-file convergence sweep: fixed 3 additional isolated clean-file bugs beyond the 4 gates - test_errors cast-rationale path (parent.parent after tests/ move), test_error_registry_contract vendored-click TyperGroup walk, test_common_output empty-Typer get_command. 110 passed across all fixed surfaces. Classified every remaining suite failure to a specific peer campaign (master-key BUCKET_DEK_V1 + custody getpass cascade in peer-dirty _master_key_io.py/_catalogue.py, participation-index AeatError WIP, period-grammar ledger filter, payload-split import allowlist, pull/file verb spine). Full suite is a MOVING TARGET: count grew 70->77 as peer agents committed WIP mid-run. Zero failures on any campaign-owned surface; `src/aeat/entrypoints/cli/tests`.
