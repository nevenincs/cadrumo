...
date: '2026-08-07'
modified: '2026-08-08'
body_hash: 'sha256:29f3825a32c72714a07cd2fb77d82240264d28ec14fc8a10c150f914e91d9a1b'
tier: L1
related:
...
- [ ] `S12` - Add casillas 72 and 73 to BOTH modelo 303 revisions, the compensacion and devolucion election amounts. CORRECTED RATIONALE, read this before building. These are NOT needed to recover the disposition generally. derive_result_disposition maps the sign of casilla 71, which both revisions already carry and the declaracion extraction profile already targets, onto INGRESO for positive, COMPENSACION for negative and NEGATIVA for zero, so I, C and N are already recoverable with no registry change at all. The one pair the sign cannot separate is COMPENSACION versus DEVOLUCION, since both are a negative resultado and DEVOLUCION is never derived for this modelo but is the operator RefundElection opt-in under LIVA art. 116. That single split is what boxes 72 and 73 decide and it is the whole reason this row exists. Both revisions lack the pair, 2009-y-siguientes at 118 casillas and 2023-y-siguientes at 129, so the change is two revisions and the construct sweep doubles. Gate one is already satisfied: the establishing provisions ley-37-1992 arts 99, 115 and 116 are existing reviewed catalogue entries cited by the neighbouring casillas in the same section, so no new legal entry and no operator gate. Validate against a temporary registry root, never the shared bundled path; `src/cadrumo/_data/registry/aeat/modelos/303/revisions/2023-y-siguientes/casillas/, src/cadrumo/_data/registry/aeat/modelos/303/revisions/2009-y-siguientes/casillas/, src/cadrumo/locales`.
- [ ] `S13` - Add the two election label patterns to both modelo 303 declaracion extraction profiles, targeting casillas 72 and 73 by named_label against the printed compensacion and devolucion sections, and blocked on S12. RECOVERY KEYS ON WHICH SLOT CARRIES A VALUE, NEVER ON A PRINTED LETTER. The C, I and D letters beside those sections are pre-printed form furniture present on all four bundled AEAT facsimiles including the two that elected ingreso, so a pattern matching the letter reports the same disposition for every filing while appearing to read the form. The discriminating signal is a populated amount casilla, measured as box 72 populated on exactly the two negative-resultado quarters and empty on the two positive ones. The pair being separated is COMPENSACION versus DEVOLUCION, not COMPENSACION versus NEGATIVA, because the sign of casilla 71 already separates N from both. ResultDisposition carries TEN members, not five and not seven, so state per member which are unreachable from this channel and why rather than assuming a closed set: the two cuenta-corriente codes, domiciliacion, the foreign-transfer devolucion and the renuncia code have no printed amount slot of their own on this form. Carry the NEGATIVA limit forward unchanged: sin actividad occupies a distinct numbered position and is a header field in the record design rather than a casilla, so the shape is proven but no bundled specimen filed one and the value stays unproven until a sin-actividad specimen exists; `src/cadrumo/_data/registry/aeat/modelos/303/revisions/2023-y-siguientes/extraction_profiles/, src/cadrumo/_data/registry/aeat/modelos/303/revisions/2009-y-siguientes/extraction_profiles/`.
