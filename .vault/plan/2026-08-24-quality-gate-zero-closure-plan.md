...
- [x] `W08.P25.S116` - Repair the locale-dependent assertions the axis surfaces, replacing each with the stable transport token it stood for or with an explicit locale pin, and reporting a removed redundant branch as a strengthening rather than a defect fixed (Terra xhigh fixes and refactors); `src/cadrumo/`.
...
- [x] `W08.P28.S120` - Land the verdict-grammar detector: a containment, prefix or suffix test applied to a gate output token whose real grammar admits a trailing parenthetical is fixed on the weaker reading and cannot fail for the reason it was written, decidable from the AST joined against the gate's declared output shape (Terra xhigh fixes and refactors); `dev/quality/`.
