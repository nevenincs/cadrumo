...
# `schema-hardening` `m309-standardization` plan

### Phase `P01` - M309 single-revision directory standardization

Normalize Modelo 309 from the largest remaining root-level single-file modelo into the generic directory/fragments substrate without changing schema semantics, loader behavior, or per-modelo definitions.
...
- [ ] `P01.S02` - Mechanically split M309 into `manifest.toml` and `revisions/2004-y-siguientes` fragments using the existing generic loader layout; `src/aeat/_data/registry/aeat/modelos/309`.
