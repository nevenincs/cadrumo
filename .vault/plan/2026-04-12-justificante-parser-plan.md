...
New subpackage ``src/aeat/justificante/`` with:
...
- ``_errors.py`` — four error classes under ``aeat.errors.AeatError``.
...
New CLI subgroup ``src/aeat/cli/justificante/__init__.py`` with ``parse``
and ``verify`` subcommands, wired into ``aeat.cli.app``.
...
No modifications to sibling branch territory: ``src/aeat/submission/``
(`#42`), ``src/aeat/status/`` (`#43`), ``src/aeat/models/`` (`#6`),
...
- Unit tests are colocated (``src/aeat/justificante/test_parser.py``)
  per the Rust-style convention.
- Every error class inherits from ``aeat.errors.AeatError``.
- ``verify_csv`` takes a **string** CSV (not a typed ``Expediente``) so
  `#43` does not need to import from ``aeat.justificante``.
- The live test ``pytest.skip``s on ``JustificanteVerificationError``
  so the known `#41` ``playwright_stealth`` bug cannot red-line the
  suite.
- Public API imports only from ``aeat.justificante`` — all
  private ``_*`` modules are hidden.
- No hard imports from ``aeat.submission``, ``aeat.status``,
  ``aeat.models``, ``aeat.auth.certificate``.
...
3. Create ``src/aeat/justificante/`` subpackage with the schema, errors,
   extractor, public parse entry point, pdfplumber backend, and the
   async ``verify_csv`` helper.
4. Create the ``src/aeat/cli/justificante/`` sub-app and wire it into
   ``aeat.cli.app``.
5. Add the two settings fields to ``aeat.config.Settings`` and to
