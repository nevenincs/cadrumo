...
- [ ] `P02.S11` - add CensusSnapshot pydantic v2 model with content-addressed snapshot_id and state machine; `src/aeat/application/live/_census.py`.
- [ ] `P02.S12` - add CensusSnapshotState closed StrEnum with ACTIVE SUPERSEDED DISCARDED members; `src/aeat/application/live/_census.py`.
- [ ] `P02.S13` - add derive_census_snapshot_id helper hashing profile_id captured_at source_url and canonical-json census_facts; `src/aeat/application/live/_census.py`.
- [ ] `P02.S14` - add CensusSnapshotRepository over SecureObjectRepository under namespace aeat.application.live.census_snapshot; `src/aeat/application/live/_census.py`.
- [ ] `P02.S15` - add CensusSnapshotService.capture with auto-supersession of any prior ACTIVE for the same profile; `src/aeat/application/live/_census.py`.
- [ ] `P02.S16` - add CensusSnapshotService.latest_active and discard; `src/aeat/application/live/_census.py`.
- [ ] `P02.S17` - real-behavior tests for id derivation supersession and encrypted round-trip; `src/aeat/application/live/test_census_snapshot.py`.

### Phase `P03` - sede G313 adapter and provenance wiring

Land the live-gated census read against the Mis Datos Censales endpoint.

- [ ] `P03.S18` - add CensusFactSet strict pydantic envelope carrying every census field from the schema delta; `src/aeat/adapters/outbound/aeat/sede/_census.py`.
- [ ] `P03.S19` - add CensusSedeDriver mirroring the _renta_web_open driver structure; `src/aeat/adapters/outbound/aeat/sede/_census.py`.
- [ ] `P03.S20` - parse the G313 result page into a CensusFactSet refusing on unknown fields; `src/aeat/adapters/outbound/aeat/sede/_census.py`.
- [ ] `P03.S21` - persist the raw G313 HTML to the observation store for the audit trail; `src/aeat/adapters/outbound/aeat/sede/_census.py`.
- [ ] `P03.S22` - wire UserProfileFact.source aeat_census_read as the provenance tag for captured facts; `src/aeat/application/profile/_census_sync.py`.
- [ ] `P03.S23` - real-behavior adapter test against a saved-HTML G313 fixture; `src/aeat/adapters/outbound/aeat/sede/test_census_driver.py`.
...
- [ ] `P04.S24` - add CENSUS_REFRESHED profile.census.refreshed to BucketEventType; `src/aeat/domain/buckets/_event.py`.
- [ ] `P04.S25` - add CENSUS_APPLIED profile.census.applied to BucketEventType; `src/aeat/domain/buckets/_event.py`.
- [ ] `P04.S26` - add CENSUS_DEPENDENT_STAMPED_STALE modelo.census.dependent_stamped_stale to BucketEventType; `src/aeat/domain/buckets/_event.py`.
- [ ] `P04.S27` - add CensusSyncError base plus CensusNotAvailableError CensusFieldValidationError CensusApplyConflictError; `src/aeat/application/profile/_census_errors.py`.
- [ ] `P04.S28` - register every new error in the application error registry; `src/aeat/core/errors/registry/_application.py`.
- [ ] `P04.S29` - add CensusProfileComparison pydantic envelope carrying the field-by-field comparison in operator-readable form; `src/aeat/application/profile/_census_sync.py`.
- [ ] `P04.S30` - add CensusSyncService.refresh_census; `src/aeat/application/profile/_census_sync.py`.
- [ ] `P04.S31` - add CensusSyncService.show_census; `src/aeat/application/profile/_census_sync.py`.
- [ ] `P04.S32` - add CensusSyncService.compare_census_with_profile; `src/aeat/application/profile/_census_sync.py`.
- [ ] `P04.S33` - add CensusSyncService.apply_census_to_profile that overwrites profile walks dependents stamps stale emits events; `src/aeat/application/profile/_census_sync.py`.
- [ ] `P04.S34` - real-behavior service tests for each verb; `src/aeat/application/profile/test_census_sync.py`.
...
- [ ] `P05.S35` - add census_stamped_stale_at and census_stale_reason fields to WorkUnit; `src/aeat/domain/modelos/_work_unit.py`.
- [ ] `P05.S36` - add the same fields to CalculationRevision; `src/aeat/domain/modelos/_calculation_revision.py`.
- [ ] `P05.S37` - add the same fields to FilingDraft; `src/aeat/domain/filing/_draft.py`.
- [ ] `P05.S38` - add the same fields to FilingRecord; `src/aeat/domain/modelos/_filing_record.py`.
- [ ] `P05.S39` - add CensusStaleRefusedError and register it in the domain error registry; `src/aeat/domain/modelos/_errors.py`.
- [ ] `P05.S40` - refuse on stale in calculate_modelo_revision; `src/aeat/application/modelo/_actions.py`.
- [ ] `P05.S41` - refuse on stale in verify_modelo_revision; `src/aeat/application/modelo/_actions.py`.
- [ ] `P05.S42` - refuse on stale in file_modelo_revision; `src/aeat/application/modelo/_actions.py`.
- [ ] `P05.S43` - refuse on stale in build_draft; `src/aeat/application/filing/__init__.py`.
- [ ] `P05.S44` - refuse on stale in approve_draft; `src/aeat/application/filing/_review.py`.
- [ ] `P05.S45` - refuse on stale in export_draft; `src/aeat/application/filing/_export.py`.
- [ ] `P05.S46` - add the cross-validation walker that apply_census_to_profile uses to enumerate every affected dependent; `src/aeat/application/profile/_census_sync.py`.
- [ ] `P05.S47` - real-behavior tests covering every refusal path; `src/aeat/application/modelo/test_census_stale_refusal.py`.

### Phase `P06` - mount CLI verbs, locales, surface tests

Operator-facing CLI surface in plain English with no programmer jargon.

- [ ] `P06.S48` - mount the aeat config profile census refresh verb; `src/aeat/entrypoints/cli/_config/_profile_census.py`.
- [ ] `P06.S49` - mount the aeat config profile census show verb; `src/aeat/entrypoints/cli/_config/_profile_census.py`.
- [ ] `P06.S50` - mount the aeat config profile census compare verb; `src/aeat/entrypoints/cli/_config/_profile_census.py`.
- [ ] `P06.S51` - mount the aeat config profile census apply verb; `src/aeat/entrypoints/cli/_config/_profile_census.py`.
- [ ] `P06.S52` - scaffold locale entries via python -m aeat.locales scaffold; `src/aeat/locales/`.
- [ ] `P06.S53` - translate English help strings into operator-readable prose; `src/aeat/locales/en.yml`.
- [ ] `P06.S54` - translate the four sets into Spanish; `src/aeat/locales/es.yml`.
- [ ] `P06.S55` - translate the four sets into Catalan; `src/aeat/locales/ca.yml`.
- [ ] `P06.S56` - translate the four sets into Hungarian; `src/aeat/locales/hu.yml`.
- [ ] `P06.S57` - CLI surface tests for the refresh verb; `src/aeat/entrypoints/cli/test_profile_census_refresh_verb.py`.
- [ ] `P06.S58` - CLI surface tests for the show verb; `src/aeat/entrypoints/cli/test_profile_census_show_verb.py`.
- [ ] `P06.S59` - CLI surface tests for the compare verb; `src/aeat/entrypoints/cli/test_profile_census_compare_verb.py`.
- [ ] `P06.S60` - CLI surface tests for the apply verb covering walker stale events and refusal cascade; `src/aeat/entrypoints/cli/test_profile_census_apply_verb.py`.

### Phase `P07` - boundary regression and legal-binding validator

Lock the invariants the ADR amendment declares.

- [ ] `P07.S61` - registry validator unit test asserting every census field carries legal_refs; `src/aeat/_data/registry/aeat/user_profile/test_legal_grounding.py`.
- [ ] `P07.S62` - backend-boundary regression asserting CensusModeloFoundationCommand stays unimported by the CLI; `src/aeat/entrypoints/cli/test_backend_boundary.py`.
- [ ] `P07.S63` - boundary regression asserting CensusSyncService is the only CLI-facing census surface; `src/aeat/entrypoints/cli/test_backend_boundary.py`.
- [ ] `P07.S64` - CENSUS_STALE bypass regression attempting every refusal path; `src/aeat/application/modelo/test_census_stale_bypass_regression.py`.
- [ ] `P07.S65` - walker-coverage regression seeding every dependent catalogue and asserting every entry is stamped; `src/aeat/application/profile/test_census_apply_walker_coverage.py`.
