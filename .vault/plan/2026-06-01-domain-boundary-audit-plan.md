...
- [x] `W10.P33.S109` - Investigate the 13 pre-existing test_cli_surface ledger-lifecycle 'No active bucket session is open' failures (test_app_ledger_lifecycle_reset_*, test_app_ledger_import_reimport_*). Proven unrelated to W10 (fail identically on the old import) but only 1 of 13 individually confirmed; `confirm the shared root cause (master-key session not seen by ledger storage in the lifecycle round-trip helper) and either fix or file as a tracked storage-runtime/session-setup flake; `src/aeat/entrypoints/cli/test_cli_surface.py`.
