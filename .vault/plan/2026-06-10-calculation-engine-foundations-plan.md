...
- [x] `W04.P11.S34` - Triage the 25 calibrated orphaned M100 cross-period relations (coordinator registry sweep: relation neither formula-referenced nor casilla-bound) - M100/2020-2023 retenciones from 111/115/123 (x4 each), M100/2024 rel-115-retenciones-trimestrales + rel-193-retenciones-anuales, M100/2025 retenciones from 111/115/123/180/190/193 + pagos 130/131. The wiring is inconsistent across revision years (only M100/2024 0604 pagos is wired, by S17). For EACH orphan determine via live calc: (a) should-fold-but-doesnt -> wire the credit/pagos casilla + legal-ground + prove live; `(b) feeds an intentionally-manual casilla (operator enters retenciones from certificados) -> document as by-design; (c) folds via a path the static sweep missed (like M200 DP200014B:00611 formula relation-operand) -> confirm + prove. Apply the M200 false-alarm lesson: do NOT wire blindly. S30 informs the retenciones-credit subset; `registry modelos/100 + legal catalogue; application/modelo/tests`.
- [x] `W04.P11.S35` - Port the proven M100/2024 retenciones-credit wiring to M100/2025 (S34 class-A, coordinator-approved): 2025 has the live 0609 total-pagos-a-cuenta chain but left 0596/0597 manual with their relation_prefill bindings unconsumed. Wire 0596 <- renta-2025-modelo-111-retenciones-periodicas and 0597 <- renta-2025-modelo-123-retenciones-periodicas (mirroring the 2024 bound-casilla shape proven live by S30), carry the same legal grounding, + live E2E proof seeding M111/M123 2025 quarters. EXCLUDE rel-115->0598 (double-count with manual 0153 - see S36). 0596/0597 are clean (no competing manual-copy path); `registry modelos/100/revisions/2025; application/modelo/tests`.
- [x] `W04.P11.S36` - Document the S34 adjudicated by-design M100 supplementary relations (coordinator ruling): (a) rel-115->0598 arrendamientos stays MANUAL - 0598=copy(0153 manual per-inmueble retención) is the form-native entry, M115 c03 is the same economic quantity so auto-populating double-counts; `M115 relation is a supplementary cross-check not auto-populate. (b) annual-resumen relations M180/M190/M193 are supplementary cross-checks of withholdings already folded via the periodic M111/M115/M123 quarters (folding both double-counts). Document as by-design supplementary in the revisions (or retire the unconsumed relation_prefill bindings per no-dead-code), and open a follow-up for an annual-resumen-vs-periodic-fold consistency ADVISORY (compare e.g. M190 annual vs the M111 quarterly fold to surface operator data-entry errors); `registry modelos/100; .vault follow-up`.
- [x] `W04.P11.S37` - Larger gap surfaced by S34 (lower priority / scope check): M100/2020-2023 revisions lack the 0609 total-pagos-a-cuenta credit chain ENTIRELY - no 0609 formula, so 0596/0597 and the orphaned relations never reach 0610 cuota-diferencial/0670 resultado; `the retenciones-credit determination for those years is ABSENT, not merely unwired (wiring relations there is inert). Model the 2020-2023 credit chain (0596..0609..0610) with grounding + live proof IF 2020-2023 live calculation is in scope; otherwise document as a known historical-revision limitation and retire the inert orphaned relations/bindings; `registry modelos/100/revisions/{2020,2021,2022,2023}`.
...
- [x] `W04.P12.S20` - Re-baseline the affected enrollment and continuity suites against the live calculate path rather than the direct-call path; `application/calculations/tests + application/modelo/tests`.
...
- [x] `W04.P12.S29` - Close the Sheets-pull vs live-calculate drift (F5): unify the six assemble_* row-set functions, resolve_relations_from_local_store, and resolve_modelo_ledger_binding_values_from_repositories onto the one enrolled resolver set so both surfaces share one aggregation logic, and add a regression proving pull-path == calculate-path casilla values for a shared revision; `application/calculations/_row_set_assembly.py + _modelo_bindings.py + application/calculations/tests`.
- [x] `W04.P12.S31` - Engine robustness (coordinator ruling c, finding #26 surfaced by S17/S18): a cross-modelo fold-in relation with NO or PARTIAL prior filing MUST resolve its target casilla to blank/unresolved plus a non-blocking source_diagnostics advisory naming the missing modelo+periods, NOT raise RegistryValidationError 'relation has no supplied value' (current _formula_runtime.py behaviour) and NOT silently zero-contribute (would under-declare); `preserve found-0 vs genuine-wiring-bug distinction; land AFTER the happy-path matrix (S17/S18/S19/S28) so those proofs guard the raise-to-blank semantics change; `domain/calculations/registry/_formula_runtime.py + application/calculations/_relation_prefill.py + application/aggregation/_source_mesh.py; tests`.
- [x] `W04.P12.S32` - PROVE the M200<-M202 pagos-fraccionados fold live (S19 false-alarm reconciled): the fold is NOT dead wiring - it reaches the computed casilla DP200014B:00611 (cuota diferencial) via 00611 = subtract(DP200014B:00599, relation[modelo-200-2024-rel-202-pagos-fraccionados]), the relation summing the three M202 c34 instalments through the enrolled RelationPrefillSourceResolver. S19 mistakenly tested casilla 00601 (a separate manual is_pagos_fraccionados box). Proven live E2E with distinct seeded M202 pagos; `no registry change needed (00601 stays manual by design); `application/modelo/tests/test_modelo_202_sociedades_fold_in_live.py`.
- [x] `W04.P12.S33` - Build the M369 OSS/IOSS ledger projection (DEAD WIRING found by S28): the live path hard-wires OssIossLedgerSourceResolver(candidates=()) and there is NO ledger->OssIossLedgerCandidate projection in the codebase (the resolver is sound at the mesh boundary but nothing feeds it on the live path), so real OSS data resolves to a claimed-zero. Build aggregate_oss_ioss_*_from_repositories(bucket_id, period, ...) projecting bucket ledger lines (cross-border B2C distance/digital sales, per esquema union/import/exterior) into OssIossLedgerCandidate, wire it into _calculation_actions.py (replacing candidates=()), and complete the carved-out S28 OSS live E2E proof; `ground the OSS classification in the registry/regulatory source; `application/aggregation/_oss_ioss.py + application/modelo/_calculation_actions.py; application/modelo/tests/test_dormant_ledger_resolvers_fire_live.py`.
- [x] `W04.P12.S38` - DURABLE M369 OSS build follow-up (deferred by S33's stopgap): OssIossLedgerCandidate needs regime (union/import/exterior) + transaction_kind (goods/services) which NO live source carries - IvaCategory has no OSS member, Invoice/Transaction have no OSS regime or goods-vs-services axis. To make the live OSS fold buildable: add OSS IvaCategory members (or an OSS classification field) + a goods/services discriminator to the Invoice model, grounded in the OSS/IOSS regime (LIVA + M369 instruction); `then build aggregate_oss_ioss_from_repositories(bucket_id, period, invoice_repository) projecting cross-border B2C OSS invoices into candidates, wire into _calculation_actions.py (replacing the candidates=() the S33 stopgap left), and upgrade the S33 oss_no_live_source advisory to a live-fold proof. Same no-source shape as the S27 withholding deferral; `domain/iva + domain/invoices; application/aggregation/_oss_ioss.py; application/modelo/_calculation_actions.py`.
...
- [x] `W05.P14.S22` - Run a fresh-context epic-close honesty review against the epic summary and a full-suite verification proving the dormant census is empty or every residual carries a tracked advisory + follow-up; `.vault/audit; full suite`.
- [x] `W05.P14.S39` - Honesty-review F-B (HIGH): the M200 BIN-pendiente carry and M200 dotaciones-deterioro carry are enrolled (so they fire) but proven LIVE nowhere - the only proof is on the now-non-canonical direct resolve_relations_from_local_store path. Add test_modelo_200_fold_in_live.py seeding a prior-year M200 BIN + dotaciones observation and asserting the carry through calculate_modelo_revision_from_bucket_aggregation_with_diagnostics (distinct seeds, real-adapter, anti-tautological). Closest item to a silently-incomplete fold; `registry modelos/200; application/modelo/tests/test_modelo_200_fold_in_live.py`.
- [x] `W05.P14.S40` - Honesty-review F-C (HIGH): S06 open-ended orden_aplicabilidad backfill is incomplete - 22 of 26 *-y-siguientes revisions (incl. M180/2023, M190/2024, M193/2024, M200/2024, M202/2025, M349/2020, M390/2010 - precisely the modelos this epic proved live) carry NO orden_aplicabilidad and degrade to non-blocking follow_up, leaving the 'law-determined' claim honor-system. Backfill orden_aplicabilidad on the open-ended revisions in the live-fold matrix with grounded ordenes, OR honestly downgrade S06's claim + open a tracked backfill follow-up; `registry modelos/**/revisions/*-y-siguientes/revision.toml + legal catalogue`.
- [x] `W05.P14.S41` - Honesty-review F-F (MEDIUM): the no-dormant-source-resolvers rule's first clause (every ModeloSourceResolver enrolled or deleted) is NOT gate-backed - the boundary tests pin currently-named resolvers but no reflective test enumerates ALL exported ModeloSourceResolver subclasses and asserts each is in the live mesh tuple (or carries a documented deferral). MultiYearResolver is the living proof (exported, zero callers, flagged by no gate). Add a reflective test over the resolver class hierarchy - this closes the mechanism by which the original F2 dormancy accumulated; `application/modelo/tests + application/aggregation`.
- [x] `W05.P14.S42` - Honesty-review F-E (MEDIUM): dead parallel aggregation capacity - _aggregate_filing_inputs (cli/_common.py:476) -> resolve_modelo_ledger_binding_values_from_repositories duplicates the live mesh RentaExpense/RentaIncome ledger logic with ZERO production callers (only a symbol-boundary test). Latent F5 drift not covered by the pull==calculate parity test. Delete it (per no-legacy-compatibility/no-dead-code) or route it through the enrolled resolver set and extend the parity test; `entrypoints/cli/_common.py + application/aggregation/_modelo_bindings.py`.
- [x] `W05.P14.S43` - Honesty-review F-D (MEDIUM): no monotonic-shrink ceiling on the orden_aplicabilidad follow-up advisory population - test_orden_aplicabilidad.py only unit-tests the gate on hand-built revisions; `no test runs it over the LIVE registry and asserts the unstamped follow_up count is at-or-below a pinned ceiling (ADR Ruling 4 pitfall: the advisory population must shrink monotonically). Add a registry-wide count-ratchet test pinned at the current unstamped count; `domain/calculations/registry/tests/test_orden_aplicabilidad.py`.
