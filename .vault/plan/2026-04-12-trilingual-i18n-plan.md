...
- Phase 1
  1. Scaffold i18n package and configuration
     ...
     - Details: Create `src/aeat/i18n/__init__.py`. Update `src/aeat/config.py` to add `aeat_output_language` (mapping to `AEAT_OUTPUT_LANGUAGE`). Update `env/.env.example` and `tests/test_config.py` to ensure alignment.
  ...
  3. Add unit tests for i18n primitives
     ...
     - Executing agent: vaultspec-standard-executor
     - References: `2026-04-12-trilingual-i18n-adr`
     - Details: Create `src/aeat/i18n/test_i18n.py` (or `test_smoke.py`). Add comprehensive unit tests for all primitives. No mocks, patches, fakes, or stubs allowed.
