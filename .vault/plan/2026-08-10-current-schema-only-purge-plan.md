...
date: '2026-08-10'
modified: '2026-08-10'
body_hash: 'sha256:144079fa4dea26216bc73e0fbaf86efee115c82ba97c086576a724182717c9a7'
tier: L3
related:
...
- [ ] `W03.P07.S36` - Decide whether a first-period zero on the compensacion may rest on an operator-declared date that AEAT has never contrasted. Surfaced by the onboarding-path row rather than assumed. The activity-start fact is declared optional in the user-profile schema, is asked as an optional wizard question, and is written by the wizard alone. The censo reconciliation does NOT populate it despite the fact key sitting in the censo namespace, and the shipped operator-facing text says so plainly, describing it as operator-declared and not yet contrasted with the authority. So the only input that can turn a missing prior compensation into a PROVEN first-period zero is a date the taxpayer typed and nothing has checked. Absence is already fail-closed and needs no change, because the predicate returns false and the gate blocks, which is why this is about the declared case rather than the undeclared one. THE OPEN QUESTION IS WHETHER A SELF-DECLARED UNCONTRASTED FACT SHOULD CARRY THAT WEIGHT, given that the same fact is grounded enough to suppress obligation windows elsewhere and that the standing position treats operator-manual sources as non-official. Candidate answers to weigh rather than assume: leave it and surface a non-blocking advisory naming the fact as uncontrasted, require a censo contrast before the proof is admitted, or keep it unconditional because a first filer has no AEAT censo record to contrast against yet. The disconfirming observation that decides this row: if the censo reconciliation CAN in fact supply this date for a genuine first filer, the uncontrasted case is a transient onboarding state rather than a standing one and the row must say so instead of hardening a gate around it; `src/cadrumo/_data/registry/cadrumo/user_profile/schema.toml, the wizard question, the censo reconciliation fact set, and the first-period predicate in modelo/_iva_wallet_gate.py`.
