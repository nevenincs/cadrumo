...
date: '2026-08-10'
modified: '2026-08-10'
body_hash: 'sha256:d64427c21edad0492edc2149404e1820cee60d394310f46300f94a4b014e800a'
tier: L3
related:
...
- [ ] `W03.P07.S35` - Resolve two producers of the local Modelo 303 compensation recurrence, one of which still contributes to the result after a caller has declared it must not. Found by semantic sweep and confirmed by targeted census, not by name. The generic extractor in the binding prefill module and the strict envelope resolver in the wallet gate both build the same recurrence carrier, and the reconciliation function selects between them with a boolean that defaults to using the generic one. The wallet gate passes it false on purpose, documented as stopping a legacy generic observation from regaining authority by fallback. THE SELECTION IS DOWNSTREAM OF THE CALL. The generic extractor executes UNCONDITIONALLY before the flag is consulted, so it runs a repository read and a history reconstruction even on the strict path that has declared it must have no authority. Its recurrence is discarded there. ITS PREFILL REPORT IS NOT, and rides out on the returned reconciliation report regardless. So a resolver a caller switched off still contributes to the artefact the caller receives. NOT YET MEASURED and it must be measured before any change: whether that report is load-bearing for any consumer, and whether the discarded read is merely wasted work or an observable difference. The aggregation rule names exactly one canonical mechanism for this channel, the IVA wallet decision, which is why two producers behind a default-on boolean is a canonical-home question rather than a tidiness one. The disconfirming observation that decides this row: if the two producers have genuinely different constraint shapes, the strict one refusing what the generic one accepts, then they are not substitutable and the remedy is to make the selection happen BEFORE the work rather than to collapse them, and this row must say so instead of merging two resolvers with different contracts; `src/cadrumo/application/calculations/_iva_wallet_reconciliation.py at the unconditional extract and the flag, and the two producers in _binding_prefill.py and modelo/_iva_wallet_gate.py`.
