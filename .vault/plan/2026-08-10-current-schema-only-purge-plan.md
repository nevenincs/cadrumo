...
date: '2026-08-10'
modified: '2026-08-10'
body_hash: 'sha256:3c8f1ffd17c4c4e5170c4f49a3734a555eda43a9c12ea00ca63738bd80a1be3c'
tier: L3
related:
...
- [ ] `W03.P07.S30` - Decide where the Modelo 303 filing-disposition requirement belongs, then land it there as a STATIC author-facing gate. THE PREMISE IS WRITTEN OUT HERE ON PURPOSE so no successor has to re-derive it. Both runtime candidates are measured out by the 2026-08-10 ADR amendment and neither may be re-proposed. The wide persistence door is the observation repository save and prepare pair, a shared primitive with fourteen direct call sites. The carry normalisation is opt-in behind a flag that defaults off, and only two writers pass it, so an envelope written through the door without it carries neither a result disposition nor the normalized pair and is non-canonical by construction. Three further modules, the compensation history reader, the annual partition loader and the wallet gate, call the normalise or validate functions DIRECTLY without the door opt-in, so the gated ingress is not a filing boundary either and the remedy of moving the screen inside it is unavailable. The validator is a fixed-point assertion rather than a transform, running normalisation purely as an oracle and returning the ORIGINAL envelope, so it cannot be repurposed to repair what it refuses. The mechanism must be static because the discriminating fact, is this a filing path, is available at the CALL SITE and was never available in the payload, so the gate constrains authors rather than payloads and costs the test corpus nothing. SCOPE IT BY WHAT A CALLER WRITES INTO THE SHARED DOOR, NOT BY SOURCE-KIND OFFICIALNESS. That constraint comes from measurement rather than from argument: the live laundering defect reached the door through an unrestricted operator CLI verb that takes the modelo as a plain string and stamps OPERATOR_MANUAL, so the reverted official-source screen would not have caught it even had it stood. Write the PROPERTY, that a production caller of the shared door either does not write disposition-bearing observations or passes the ingress, and let Modelo 303 fall out of it rather than naming the modelo. The disconfirming observation that decides this row: if the property cannot be expressed without naming Modelo 303, that is evidence the requirement is not about the door at all and the row must stop and report rather than encode the name. State a control proving every legitimate caller still passes, and do not close on the refusal firing; `src/cadrumo/application/calculations/ at the shared observation door and the carry ingress, the operator local-observation write, and a static gate under src/cadrumo/tests/`.
