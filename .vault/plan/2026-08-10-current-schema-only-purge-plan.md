...
date: '2026-08-10'
modified: '2026-08-10'
body_hash: 'sha256:f17ef0ad7f14cc778434b8819ca2bbc96c8e90d2448478b2473743beec8d20d6'
tier: L3
related:
...
- [ ] `W03.P07.S31` - Own the carry-ingress refusal swallow at the IVA wallet gate, surfaced by the normalisation-path characterisation and deliberately not folded into it. Three of the four assert-sites propagate M303CarryIngressError and this one catches it and returns None, so a carry envelope this build cannot interpret reaches the gate as ABSENT EVIDENCE rather than as a refusal. The visible consequence is on the compensacion path, where a taxpayer's carried credit goes silently unseen rather than reported. Decide whether the gate should propagate, surface a typed advisory, or continue returning None for a stated reason, and record which. The disconfirming observation: if some legitimate envelope shape reaches this catch during ordinary operation rather than only on a genuinely uninterpretable one, propagating would convert a quiet miss into a broken path and this row must measure that population before changing the behaviour; `src/cadrumo/application/modelo/_iva_wallet_gate.py`.
