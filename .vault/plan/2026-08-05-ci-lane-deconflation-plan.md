...
date: '2026-08-05'
modified: '2026-08-10'
body_hash: 'sha256:729160a27c1db7ce793d24c49d70c5469879c4d2905b9fe6b53ec3702bab7ea2'
tier: L2
related:
...
- [ ] `P02.S38` - Delegate the export provenance manifest write to the shared hardened atomic-write tier instead of hand-rolling mkstemp and fsync and replace, because that one site reimplements the staging and fsync and rename sequence that forty-two production sites reach through the core helper, and it is the only bypass measured on that surface, and the core hardened tier already offers the refusal-on-pre-existing behaviour this site open-codes with its own exists check, and the ruling was to fix the one site rather than widen the write-site census because the census scope is production and tests by construction so dev write sites are outside its declared reach by design rather than by oversight and widening it would commit that instrument to adjudicating a class it was never built for, noting this row sits here because the lane enrolment brought the registry dev tree into this campaign's watch and it moves to the drift campaign if that campaign would rather own it; `dev/registry/_provenance_manifest.py`.
