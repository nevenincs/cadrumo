...
Ship a strict pydantic v2 category substrate under `aeat.financial.categories`
...
Audit surface: `#77`, `#104`, `aeat.casillas`, `aeat.manuals`,
`aeat.normatives`, `aeat.i18n`, the root CLI, and the new
`aeat.financial.categories` package.
...
- Create `src/aeat/financial/categories/` with the strict public model surface.
- Re-export the package at `aeat.financial.categories`.
...
- `phase-1-foundation`
  1. Create `src/aeat/financial/__init__.py` and
  ...
  1. Add the strict model modules:
     ...
     `_casilla_mapping.py`.
  1. Reuse `aeat.casillas.ModeloCode` and add the minimum additive casillas enum
...
- `phase-1-loader`
  ...
  1. Attempt to read the manual corpus through `aeat.manuals`.
  1. Fall back deterministically to `CATEGORY_PROFILES_2025`.
- `phase-1-cli`
  1. Add `src/aeat/cli/categories.py`.
  1. Mount the subgroup from `src/aeat/cli/__init__.py`.
...
- `aeat.financial.categories` exports the full public API.
- Every `SpendingCategory` value has a `CategoryProfile`.
- Every `ProportionalityRule` has at least one citation.
- Every referenced casilla code exists in the current `aeat.casillas` corpus.
...
- Scope stays inside TDP step `T4` and does not leak into `#78`, `#79`, `#85`,
  or `#87`.
- `aeat.financial.categories` is the only public feature surface.
