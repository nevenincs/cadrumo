...
date: '2026-08-13'
modified: '2026-08-13'
body_hash: 'sha256:55ad2a6540a65f412c72002a94b9a8a4e172dfe7cd6e4abed978d6d26d852720'
tier: L2
related:
...
- [ ] `P03.S12` - Author the missing envelope header and footer export-layout fragments for Modelo 232 revision 2016-2017, or retire the empty directory if the revision is genuinely layout-less; `src/cadrumo/_data/registry/aeat/modelos/232/revisions/2016-2017/export_layouts/`.
- [ ] `P03.S13` - Diagnose the absent Modelo 390 page-02 field at official record position 1628 before re-anchoring the disclosure-split gate, per that gate's own instruction; `src/cadrumo/_data/registry/aeat/modelos/390/`.
- [ ] `P03.S14` - Attribute the bundled Modelo 303 published designs for 2015 and model the 2018 mid-course AEAT split as its own revision pair; `src/cadrumo/_data/registry/aeat/modelos/303/revisions/`.
- [ ] `P03.S15` - Narrow the Modelo 720 revision 2013-y-siguientes claimed filing years to those its declared layout design covers, or declare the design that covers 2012; `src/cadrumo/_data/registry/aeat/modelos/720/revisions/`.
- [ ] `P03.S16` - Set the missing English help string for the standard-rate repercutido casilla through the locales CLI in all four catalogues; `src/cadrumo/locales/`.
...
- [x] `P03.S19` - Not implemented, adjudication only. _observed_value_token in src/cadrumo/adapters/outbound/aeat/sede/_declarations_observations.py, lines 223-246, is the second site the LEFT_ZERO class exposes and it rules against itself. Its own closing docstring sentence states the governing rule verbatim - the typed value is the faithful reading there, and the raw token is the faithful reading only where the parser's own conversion discards the artefact's spelling - but the dispatch only reads raw for the boolean case, str of value for everything else. For a LEFT_ZERO text field the parser's own conversion does discard the artefact's spelling, exactly what lstrip zero does to a value like 01, so by the function's own stated rule that case should read raw and the implementation does not detect it. The author reasoned carefully about two categories - booleans, where str of value would emit a Python repr no AEAT surface uses, and fixed-width money, where raw is zero-padded and scaled by 100 so recording it would enrol 10000 where the taxpayer filed 100 - and both conclusions are right, but a third category, a text identifier whose leading zeros the unpad destroys, was not in view. Fixing the nineteen padding declarations in P03.S18 closes the instance - with padding none, unpad never strips and str of value is faithful again - but not the class: the next text field authored with left_zero padding re-opens it silently and nothing in the capture path objects. The remedy shape is that the dispatch should honour its own stated rule and read raw wherever the parse discards spelling, not only for booleans, making the capture path robust regardless of how a future field is declared. The docstring's own measurement bounds the blast radius of widening the raw branch - 75 of 77 rows already agree between raw and str of value on a real Modelo 100 artefact, with the two exceptions being exactly the boolean rows - so extending the raw branch to LEFT_ZERO text fields is measured-safe for the population already flowing through, not a guess. This is a different file, a different owner, and needs its own bite proof - break the dispatch, confirm a LEFT_ZERO text case reds, restore - before it lands.; `src/cadrumo/adapters/outbound/aeat/sede/_declarations_observations.py`.
- [x] `P03.S20` - Record the concurrent-write loader-cache race as its own durable finding, not implemented, adjudication only. Signature is the literal exception message RegistryLoadError registry directory changed during cache fingerprinting, retry after concurrent registry writes settle, raised from _collect_registry_tree_fingerprints_for_cache in src/cadrumo/domain/calculations/registry/_loader.py. Cause is peers writing registry TOML while another process's loader is mid-fingerprint of the same tree, a tree-wide hash-and-stat walk that a concurrent write can invalidate partway through. It scales with concurrent agent count on this shared worktree, not with anything in the code under test, and is unrelated to correctness of any registry declaration. Measured tonight at real scale, not as a hunch - a full sequential registry suite run produced 37 phantom failures out of roughly 203 observed, about a quarter of the total, every one of which passed clean on isolated single-test re-run, with the sole literal cache-fingerprint message appearing directly on one of them and the other 36 failing with assorted derived symptoms from reading a tree mid-write. It independently misled three people the same night - inflated a 166-baseline comparison to a reported 203, produced two failures inside a separate patched-module verification, and surfaced inside a peer's own dev-tooling measurement as part of a 48-error report that was itself stale by the time it was read. The only sound method is to never classify a failure from a long concurrent run directly - re-run every candidate in isolation, sequentially, before deciding it is real, and record the run's start time beside any count reported, since a run's number is a timestamp against a moving tree, not a fixed state of it. A full run under contention should be treated as a lower bound on the false-positive rate, not a report of the true failure count.; `src/cadrumo/domain/calculations/registry/_loader.py`.
