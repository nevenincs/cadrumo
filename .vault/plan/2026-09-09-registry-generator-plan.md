...
modified: '2026-09-10'
body_schema: body-v2
body_hash: 'sha256:26a756fa7606bc5eb1db42d6f69d1ddd697e6005296fdee0515950a8f4d15ae9'
---
...
- [ ] `W05.P15.S57` - Two blockers, both grounded. (1) Requirement: 10 fields state a qualified wording a boolean cannot carry, and carrying it needs a new ExportFieldDefinition key that the normalisation schema version and the canonical-JSON manifest round-trip both refuse at authority load. (2) Modelo 202's republish refusal is a staging artefact, not a declaration defect: its semantic roles appear once per revision and three times per modelo, and the full registry validates clean. Traced read-only: BOTH staging paths delete sibling revisions - stage_generated_export_candidate (dev/registry/pipeline/candidate_staging.py:210), which builds the authority the role gate validates, and _stage_published_modelo (dev/registry/pipeline/cli.py:187), which builds the published witness. The deletion carries no stated reason and no test pins it: test_m390_isolation_excludes_both_export_authorities_and_keeps_required_support pins only that the TARGET revision's export directories are excluded. Proposed direction, matching the edition-authoring lane's independent need to keep an edition's predecessor: stage sibling revisions whole so the candidate validates against the real registry minus only the target's export, rather than making cardinality gates inert on a partial stage. Declaring intentional_singleton to quiet it would write a falsehood into shipped declarations. Implement after the dev/registry/tests harness conversion lands; `dev/registry/pipeline/candidate_staging.py`.
...
- [ ] `W05.P15.S76` - Repair and gate modelo 200's casilla bindings, which bind a printed box number without the sheet that qualifies it. VERIFIED in 200/2025-y-siguientes: the design prints [00399] on sheet DP200010 as an ECPN II slot, the map entry m200-2025.dp200010.f0018 binds DP200014B:00399, and 00399 is printed on three sheets; 812 of 3,417 printed numbers appear on more than one sheet. Of 137 casilla-bound fields, 41 bind their own sheet and 96 bind the right number on the wrong sheet. The 96 split: 33 have the correct-sheet casilla already declared (DP200010:00399 exists, section estado_de_cambios_patrimonio_neto_ii matching the design text) and are repairable in the map; 63 point at no declared correct-sheet casilla and need casilla declarations first, which is the edition-authoring lane's surface. NOT a mechanical repair: the wrong entries also copied the wrong box's legal_refs, and correctly-bound entries never mirror their casilla's legal_refs - 30 cite a curated subset, 11 cite refs the casilla lacks - so each rebinding needs its authority chosen per entry, not derived. Republishing also writes export_refs into casillas/, so coordinate with that lane first. The 2024 map carries the same defect but ships no tree. Never key a binding on an unqualified printed number; `dev/registry/mappings/modelo_200/2025/`.
