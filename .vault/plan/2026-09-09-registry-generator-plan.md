...
modified: '2026-09-10'
body_schema: body-v2
body_hash: 'sha256:1f85b26e7b41301586952d70f284ef0b75a464c0ddc62cb8515b89bcbe087f16'
---
...
- [ ] `W05.P15.S78` - Land the blocked uses of the opened schema, each gated. DONE: the sign-position convention (920e6e68d8) - sign_position blank_or_n / n_unless_zero, read from every design that subdivides a SIGNO position; 11 declarations across modelos 165, 180, 189, 190 and 280 (180 and 190 were missing from the original finding), plus a 180 fixture that had encoded the defect. RULED, NO SCHEMA CHANGE: modelo 714's 60 'signed integer' fields are all the '% participacion individual/familiar' cells of H1/H2, typed N where the same design types every other percentage Num; a shareholding percentage cannot be negative, the unsigned manual-input declaration writes identical bytes for every legal value and refuses a negative, so no signed integer is added and the gate declares them with that reason. OPEN: requirement_statement carrying a qualified requirement wording (10 fields: 'Obligatorio PI' x6 in 303, 'OBLIGATORIO (persona fisica)' x4 in 390); a mandated value domain on a signed field (390/2025's 80 'rellenas a 0' amounts); a part identity for a design cell whose own text declares sub-offsets (modelo 347's provincia/pais cell, letting both 347 trees republish and correcting the 2011 blank-region NIF binding); and a gate that ties each design SIGNO position to a declared sign_position, replacing the one-off survey. The 303, 390 and 347 uses change generated trees and wait for the S14 export_refs change to land before republishing; `src/cadrumo/domain/calculations/registry/schema_exports.py`.; `src/cadrumo/domain/calculations/registry/schema_exports.py`.
