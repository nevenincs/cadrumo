...
- [ ] `W03.P07.S27` - Compute the verdict at generation time from the parser row beside the emitted field, refusing an unreviewed divergence rather than recording it as agreement. Lands with S26 in the same republication, after the S14 freeze lifts; `dev/registry/pipeline/export_fragment_provenance.py`.; `dev/registry/pipeline/export_fragment_provenance.py`.
- [ ] `W03.P07.S28` - Regenerate all 32 manifests with verdicts through the disposition-admitted publication path, proving the reproduction, type-column and ledger gates before and after each tree. Waits for the S14 freeze; `dev/registry/pipeline/_tree_check.py`.; `dev/registry/pipeline/_tree_check.py`.
...
- [ ] `W03.P07.S29` - Prove the verdict has teeth: a planted divergence in an isolated tree yields refused, never agrees, beside the normal path in one suite. Lands with S26-S28; `dev/registry/tests/`.; `dev/registry/tests/`.
...
- [ ] `W05.P15.S58` - Read modelo 303's 'Obligatorio PI' as the unconditional requirement it is. The earlier finding misread it as qualified: in every 303 design since 2008 the cell is 'Tipo Declaracion | Obligatorio PI | Nota 1', and Nota 1 defines 'PI: El tipo de declaracion puede ser: C ... D ... G ... I ... N ... V', so PI names the value list, not a condition, and no schema key is needed. The fix adds 'obligatorio pi' to the adjudicated unconditional wordings in _is_required (patch prepared); it lands after S14 because test_export_tree.py, which pins the old reading, is in S14's change, and it republishes with the six 303 trees; `dev/registry/pipeline/_export_tree.py`.; `src/cadrumo/domain/calculations/registry/`.
- [ ] `W05.P15.S59` - Carry modelo 390's 'OBLIGATORIO (persona fisica)' (4 cells, the sujeto pasivo nombre, header producer taxpayer.given_name) as a requirement that holds only for a natural-person taxpayer. MECHANISM LANDED (beabae2335): ExportFieldDefinition.required_for = 'natural_person', admitted only on header fields and never beside required; the header renderer refuses a blank for a natural person or an unreadable legal form and admits it for an entity; TaxpayerIdentityFacts now refuses an entity legal name beside personal names, so the form is read exactly. REMAINING, after S14: the generator maps that wording to required_for (test_export_tree.py pins the old reading) and the four 390 trees republish with S60; `dev/registry/pipeline/_export_tree.py`.; `src/cadrumo/domain/calculations/registry/`.
- [ ] `W05.P15.S60` - After S14 lands, in one change: relocate the disposition ledger into its own module so the renderer can read type-column rulings, attach verdicts in render_complete_export_tree, switch the design-agreement test to read verdicts through design_stated_divergence, and republish all 32 generated trees through the disposition-admitted path, proving reproduction, type-column and ledger gates before and after each; the six 303 trees also land their reviewed sign correction and S58, and their drift rows retire; `dev/registry/pipeline/_export_tree.py`.; `src/cadrumo/domain/calculations/registry/`.
...
- [ ] `W05.P15.S78` - Land the blocked uses of the opened schema, each gated. DONE: the sign-position convention (920e6e68d8) - sign_position blank_or_n / n_unless_zero, 11 declarations across modelos 165, 180, 189, 190 and 280 plus a 180 fixture that had encoded the defect; and its permanent gate, replacing the one-off survey: every money field starting on a SIGNO position printed by its revision's pinned design must declare sign_position, every declared one must stand on such a position, and every pinned design must be readable (PDFs and workbooks without extracted text are read directly) - its first run found the 165/2016-2022 design had never been read. RULED, NO SCHEMA CHANGE: modelo 714's 60 'signed integer' fields are shareholding percentages typed N where the same design types every other percentage Num; they cannot be negative, and the unsigned declaration writes identical bytes and refuses a negative. OPEN: requirement_statement (10 fields: 'Obligatorio PI' x6 in 303, 'OBLIGATORIO (persona fisica)' x4 in 390); a mandated value domain on a signed field (390/2025's 80 'rellenas a 0' amounts); a part identity for modelo 347's provincia/pais cell. All three change generated trees and wait for the S14 export_refs change to land before republishing; `src/cadrumo/domain/calculations/registry/schema_exports.py`.; `src/cadrumo/domain/calculations/registry/schema_exports.py`.
