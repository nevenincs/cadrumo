...
- [x] `W01.P03.S10` - Author modelo 347's declarado repeat and row bindings, then give the generator a split-cell identity. Landed in d833cf8e2b: SemanticMapPart lets an entry declare the part of its cell it fills, held at validation to the cell's verbatim text, its printed range and an exact tiling; design_view feeds the renderer and render-profile eligibility one view; both 347 trees republish with f009a provincia (two-digit rule) and f009b pais, and the 2011 tree stops writing a counterparty NIF into f027, which the design declares BLANCOS; `dev/registry/mappings/modelo_347/`.; `dev/registry/mappings/modelo_347/`.
...
- [x] `W01.P03.S12` - Retire the 347 drift rows once the map reproduces the counterparty records. Done in d833cf8e2b: both rows became reviewed republish rows, the trees were republished, and the rows retired; `dev/registry/pipeline/generated_tree_dispositions.toml`.; `dev/registry/pipeline/generated_tree_dispositions.toml`.
...
- [x] `W03.P07.S26` - Add the per-field divergence verdict (agrees / adjudicated with recorded evidence / refused) to every field derivation in the provenance manifest. NO LONGER blocked on the manifest format: S82 showed a new key costs no migration when absent. But D3 puts a verdict on EVERY field, so every manifest changes and all 32 trees republish through the disposition-admitted publication path; that waits for the S14 export_refs change (shared casilla files) to land and lift the publication freeze; `dev/registry/pipeline/export_fragment_provenance.py`.; `dev/registry/pipeline/export_fragment_provenance.py`.
- [x] `W03.P07.S27` - Compute the verdict at generation time from the parser row beside the emitted field, refusing an unreviewed divergence rather than recording it as agreement. Lands with S26 in the same republication, after the S14 freeze lifts; `dev/registry/pipeline/export_fragment_provenance.py`.; `dev/registry/pipeline/export_fragment_provenance.py`.
- [x] `W03.P07.S28` - Regenerate the manifests with verdicts through the disposition-admitted publication path. Landed in ef97b8dae6: 28 of 32 trees republished through the publish CLI; 185 and 222 are refused on their honest applicability grade and both 347 trees are repair_inputs, and test_shipped_fields_carry_verdicts.py declares exactly those four as unattested, checked in both directions; `dev/registry/pipeline/_tree_check.py`.; `dev/registry/pipeline/_tree_check.py`.
...
- [x] `W03.P07.S29` - Prove the verdict has teeth: a planted divergence in an isolated tree yields refused, never agrees, beside the normal path in one suite. Lands with S26-S28; `dev/registry/tests/`.; `dev/registry/tests/`.
...
- [x] `W05.P15.S58` - Read modelo 303's 'Obligatorio PI' as the unconditional requirement it is. The earlier finding misread it as qualified: in every 303 design since 2008 the cell is 'Tipo Declaracion | Obligatorio PI | Nota 1', and Nota 1 defines 'PI: El tipo de declaracion puede ser: C ... D ... G ... I ... N ... V', so PI names the value list, not a condition, and no schema key is needed. The fix adds 'obligatorio pi' to the adjudicated unconditional wordings in _is_required (patch prepared); it lands after S14 because test_export_tree.py, which pins the old reading, is in S14's change, and it republishes with the six 303 trees; `dev/registry/pipeline/_export_tree.py`.; `src/cadrumo/domain/calculations/registry/`.
- [x] `W05.P15.S59` - Carry modelo 390's 'OBLIGATORIO (persona fisica)' (4 cells, the sujeto pasivo nombre, header producer taxpayer.given_name) as a requirement that holds only for a natural-person taxpayer. MECHANISM LANDED (beabae2335): ExportFieldDefinition.required_for = 'natural_person', admitted only on header fields and never beside required; the header renderer refuses a blank for a natural person or an unreadable legal form and admits it for an entity; TaxpayerIdentityFacts now refuses an entity legal name beside personal names, so the form is read exactly. REMAINING, after S14: the generator maps that wording to required_for (test_export_tree.py pins the old reading) and the four 390 trees republish with S60; `dev/registry/pipeline/_export_tree.py`.; `src/cadrumo/domain/calculations/registry/`.
- [x] `W05.P15.S60` - Relocate the disposition ledger, attach verdicts in the renderer and republish. Landed in ef97b8dae6: generated_tree_dispositions.py owns the ledger, render_complete_export_tree attaches verdicts through type_column_rulings_for, the six 303 and four 390 drift rows and the six 303 reproduction pins retire with their republished trees, and the 390/2025 type-column row is gone because its fields now agree; `dev/registry/pipeline/_export_tree.py`.; `src/cadrumo/domain/calculations/registry/`.
- [x] `W05.P15.S80` - Extend the type-column comparison to the 62 hand-authored revisions it could not see, and correct what it finds. Landed in df4ffab003: dev/registry/analysis/hand_authored_type_column.py joins each shipped record to the one sheet of its revision's own pinned design that carries all of its (offset, length) slots, within one revision only; 250 of 290 records align, reproducing the hand-verified modelo 490 count of 72. 1,484 money fields typed N now carry the sign (one line each; non-negative amounts keep identical bytes). The gate dev/registry/tests/test_hand_authored_layouts_agree_with_type_column.py declares the remainder per revision and checks each declaration in both directions: 60 modelo 714 integer fields the schema cannot sign, and 44 records plus two design-less revisions that cannot be aligned, reported as UNCHECKED rather than passed. Planted tests prove both detections. Open follow-ups: the 60 integer fields need a schema that signs integer amounts; the 44 unaligned records need a sheet assignment; five fields are over-declared signed where the design is not N; `dev/registry/tests/`.
- [x] `W05.P15.S81` - Bind every modelo 200 field to the casilla printed on its own sheet, and gate the class. Landed in e2e7b7232e: the 33 wrong-sheet bindings whose own-sheet casilla exists are rebound (target section matches the field's design text in all 33; each entry's legal_refs already a subset of the target's, so only casilla_id changes), own-sheet bindings go from 41 to 74, and the back-reference writer now replaces a declaration naming only the layout's own fields while still refusing one naming any other. The gate dev/registry/tests/test_casilla_bindings_name_their_own_sheet.py fails any field bound to another sheet while its own-sheet casilla exists, and declares the remaining 63 per revision as unrebindable because their own-sheet casilla was never declared; authoring those casillas is unowned corpus work, and the gate fails once any of them exists so the rebinding is not forgotten; `dev/registry/tests/`.
- [x] `W05.P15.S82` - Open the export field schema without a migration. The planned one-shot manifest migration proved unnecessary: a key added after trees were attested is serialised in the provenance manifest and projected in the loader digest ONLY when a field declares it (the module's existing pattern for a later layout member), so all 32 attested trees keep exact bytes and digests, nothing is rewritten, no legacy reader exists, and a stored explicit null refuses as noncanonical. Landed with its first use, sign_position (920e6e68d8); later keys join _FIELD_KEYS_PRESENT_ONLY_WHEN_DECLARED; `dev/registry/pipeline/export_fragment_provenance.py`.; `dev/registry/pipeline/export_fragment_provenance.py`.
- [x] `W05.P15.S83` - Land the blocked uses of the opened schema. All landed: sign_position (920e6e68d8) and its two-way gate (bd68791bb3); 714's N-typed percentages ruled unsignable by value (f52f65c200); a mandated value domain on signed money (2028acbda7) adopted by 390/2025's 80 zero-mandated slots (ef97b8dae6); required_for (beabae2335) adopted by 390's sujeto pasivo nombre and 303's 'Obligatorio PI' read as unconditional (ef97b8dae6); and the part identity for 347's provincia/pais cell (d833cf8e2b); `src/cadrumo/domain/calculations/registry/schema_exports.py`.; `src/cadrumo/domain/calculations/registry/schema_exports.py`.
