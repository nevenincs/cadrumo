...
- [ ] `W02.P04.S28` - Route profile-health repair mutation through the shared reentrant active-profile pointer transaction with locked reassessment, bounded fail-closed contention, and the health result's repairable flag as the sole eligibility authority, then prove pointer-sourced unreadable-manifest repair and cold no-op behavior; `src/cadrumo/application/workflow/_profile_health.py, src/cadrumo/application/workflow/tests/test_profile_health.py`.
