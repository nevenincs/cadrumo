...
- [ ] `W02.P04.S28` - Route profile-health repair mutation through the shared reentrant active-profile pointer transaction with locked reassessment, bounded fail-closed contention, and the health result's repairable flag as the sole eligibility authority; correct the three lifecycle CLI integration pointer setup calls to use the isolated backend root; then prove pointer-sourced unreadable-manifest repair, cold no-op behavior, and real CLI pointer repair, absence, and dangling-pointer outcomes; `src/cadrumo/application/workflow/_profile_health.py, src/cadrumo/application/workflow/tests/test_profile_health.py, src/cadrumo/entrypoints/cli/tests/test_profile_lifecycle_verbs.py`.
