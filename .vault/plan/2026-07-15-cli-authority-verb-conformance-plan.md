...
- [ ] `W02.P04.S29` - Prove byte-exact failed-create rollback through the nested orchestration and repository transaction layers, then prove dangling-pointer repair fails closed under real process contention and succeeds after lock release against real files; `src/cadrumo/application/user_profile/tests/test_orchestration_pointer.py`.
