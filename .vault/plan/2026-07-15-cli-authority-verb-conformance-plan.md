...
- [ ] `W02.P04.S30` - Prove that a real lifecycle-repository-backed active override resolves its encrypted record and makes a lower-priority dangling pointer ineligible for repair, then prove clearing the override makes that pointer authoritative and repairable; `src/cadrumo/application/workflow/tests/test_active_profile_resolution.py`.
