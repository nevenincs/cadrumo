...
- [ ] `W02.P04.S24` - Add byte-exact pointer capture, atomic restore, idempotent clear with restrictive temporary permissions and fsync, complete short writes in the hardened byte writer, prove complete writes against a real operating-system descriptor, and expose the core pointer API; `src/cadrumo/core/_bucket_pointer_io.py, src/cadrumo/core/atomic_write.py, src/cadrumo/core/__init__.py, src/cadrumo/core/tests/test_atomic_write.py`.
...
- [ ] `W02.P04.S26` - Route every orchestration pointer write, clear, capture, restore, registration, and selection through the core pointer authority under an explicit rollback concurrency policy; `src/cadrumo/application/user_profile/_orchestration.py`.
