...
date: '2026-08-07'
modified: '2026-08-09'
body_hash: 'sha256:e9017b79c5c2fefd8d0be629dcb4b1a0c4d3785b2a573cc640b22cb322cc9bd5'
tier: L1
related:
...
(now `accepted`). Each said its implementation would be tracked as a separate
open row; none had a `.vault/plan` Step until this document. This plan is the
durable home for that tracking, surfaced by the 2026-08-07 ADR corpus
...
rather than pending so the plan does not misstate finished work as open. S04's
implementation landed in `83b78ac464` after the ADR became accepted. It is no
longer blocked: this record now needs current-HEAD verification, an execution
record, and the mandatory review audit before closure.
...
- [x] `S04` - Land the recargo mismatch advisory comparing an operator-supplied recargo figure against the rate resolved for its applied rate and date; `src/cadrumo/application/aggregation/`.
...
S04 depended on S05 (closed) and `recargo-equivalencia-source-of-truth-adr`
reaching `accepted`; both prerequisites have landed. Its remaining work is
current-HEAD verification, an execution record, and the mandatory review audit.
...
M390 regimen blocks, applying the two-layer rate-box shape only where the
