...
- [ ] `W02.P03a.S64` - Physically delete every external-client acquisition, capture, marketplace-publication, client-smoke, and distribution-identity module and its dedicated tests, with no compatibility import or fallback; `dev/packaging/`.
- [ ] `W02.P03a.S65` - Delete the 130 MB runtime wheelhouse builder, archive, manifest contract, cohort member, dependency-closure logic, consumers, fixtures, and tests from the base product release; `dev/packaging/runtime_wheelhouse.py, dev/packaging/python_cohort.py, and dev/packaging/release_cohort.py`.
- [ ] `W02.P03a.S66` - Remove the external-client capability-exposure branch and its client-policy tests from the operator surface while retaining only protocol-neutral command, schema, and execution-policy contracts owned and consumed by the base product; `src/cadrumo/application/operator_surface/`.
- [ ] `W02.P03a.S67` - Delete external-client consumer URIs, mapper names, and prose from corpus search while preserving the base-owned citation and search contracts; `src/cadrumo/application/corpus_search/`.
- [ ] `W02.P03a.S68` - Remove external-client identity, acquisition, destination, session, and client-row schemas from distribution evidence and delete their model tests without tolerating the retired payload shape; `dev/packaging/evidence.py and dev/packaging/tests/test_evidence.py`.
- [ ] `W02.P03a.S69` - Delete the installed MCP oracle and remove all client binders, real-client rows, server identities, and MCP branches from the distribution evidence emitter while preserving only base Python, Scoop, and Homebrew evidence; `dev/packaging/distribution_evidence_emit.py, dev/packaging/installed_mcp_oracle.py, and dev/packaging/tests/`.
- [ ] `W02.P03a.S70` - Remove external-client wheel building, provisioning, environment, artifact, and fallback branches from shared packaging smoke helpers and base channel generators, then delete their client-specific tests; `dev/packaging/_smoke_common.py and packaging/homebrew/ and packaging/scoop/`.
- [ ] `W02.P03a.S71` - Remove external-client EvidenceLane values, artifact routes, cohort records, jobs, credentials, promotion and publication paths, and recipes from release code and workflows, and physically delete the dedicated harness and Claude workflows; `dev/release/, .github/workflows/, and justfile`.
- [ ] `W02.P03a.S72` - Remove external-client channel kinds, marketplace fields, runtime-wheelhouse rows, destinations, and parser branches from the download descriptor schema and delete their schema fixtures and tests; `dev/docs/download_matrix.py, docs/_data/download_channels.toml, and dev/docs/tests/`.
- [ ] `W02.P03a.S73` - Remove every deleted-client path, import, allowlist, locale source, test target, and lane enrollment from development quality, locale, CI, and project configuration surfaces; `dev/quality/, dev/locales/, dev/ci/, pyproject.toml, and justfile`.
- [ ] `W02.P03a.S74` - Delete external-client cases and synthetic compatibility fixtures from mixed packaging, release, source-artifact, and channel tests while retaining and re-grounding only base-owned assertions; `dev/packaging/tests/, dev/release/tests/, src/cadrumo/tests/, packaging/homebrew/tests/, and packaging/scoop/tests/`.
- [ ] `W02.P03a.S75` - Delete external-client installation, download, environment, release, executable-sequence, localization, and operator prose, including every base claim about client artifacts, credentials, destinations, or lifecycle; `docs/, RELEASING.md, and .github/ci-control-plane.md`.
- [ ] `W02.P03a.S76` - Bind the sealed base source archive to an explicit forbidden-topology gate that rejects any harness, MCPB, marketplace, Claude, agent-evaluation, client-workflow, or runtime-wheelhouse member and prove each detector with planted archive entries; `dev/packaging/python_cohort.py and dev/packaging/tests/`.
- [ ] `W02.P03a.S77` - Add repository-wide inverse gates that reject any reintroduced app agent grammar, embedded client package, client identity or state, materializer, artifact or cohort record, binder or evidence path, workflow, credential, documentation, alias, shim, fallback, or publication step, and prove every detector with planted negatives and zero-remnant scans; `src/cadrumo/entrypoints/cli/tests/ and dev/release/tests/test_external_client_release_boundary.py`.
- [ ] `W02.P03a.S58` - Re-prove one immutable Git-archive base-only Python cohort against exhaustive installed-runtime CommandSpec identities, locale metadata, policy, schema, selected-path import budgets, and forbidden-artifact absence, then require only base-owned smoke, Scoop, Homebrew, PyPI, and GitHub release lanes to consume the sealed Cadrumo, data, and source artifacts without rebuilding or regenerating command authority, with every external-client and runtime-wheelhouse artifact or destination absent; `dev/packaging/ and dev/release/`.
- [ ] `W02.P03a.S59` - Run two independent post-correction architecture reviews against base-only shipping and reconcile every command-authority, production-development boundary, base build-lane, base shipping-lane, installed-runtime, external-client inverse-boundary, and zero-legacy-remnant finding before resuming the remaining performance campaign; `.vault/audit/`.
