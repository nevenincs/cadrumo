...
date: '2026-08-09'
modified: '2026-08-11'
body_hash: 'sha256:e718a6200c5f5f4ae66f2845b4eb4fab41db491e57e2e3f5db44cb71feb9939d'
tier: L3
related:
...
- [ ] `W05.P09.S33` - Replace provisioning optional-extra recovery prose with typed failed-condition facts and explicit no-recovery outcomes including the local-model stored-without-extra row and hand the changed dependency projection to S89 without retained remediation compatibility; `src/cadrumo/application/provisioning.py`.
...
- [ ] `W05.P10.S38` - Migrate ledger evidence-draft optional-extra catches to retain only typed reader-availability verdicts and machine facts never MissingOptionalExtraError prose and consume S65 normalized identity without a compatibility bridge; `src/cadrumo/application/ledger`.
...
- [ ] `W05.P10.S41` - Replace locale-facing optional-extra install guidance with locale keys accepting only machine extra identities add S65 error keys through dev.locales in all four catalogues and remove canonical command and English feature identity from render templates; `src/cadrumo/locales`.
...
- [ ] `W05.P10.S89` - Migrate config-check raw Google installation prose and provisioning dependency projection plus profile-inspect command drift to resolved typed actions or explicit no-recovery outcomes with no raw package command or remediation-forwarding compatibility; `src/cadrumo/entrypoints/cli/_config`.
...
- [ ] `W05.P10.S93` - Migrate residual MCP action producers and transport notices to the shared resolved action projection; `src/cadrumo/entrypoints/mcp/_tools.py; src/cadrumo/entrypoints/mcp/_transport.py; src/cadrumo/entrypoints/mcp/_resources.py`.
- [ ] `W05.P10.S94` - Migrate LLM optional-extra wrappers and continuations to preserve machine identities or application-owned typed outcomes including rewrapped MissingOptionalExtraError paths with no raw installation prose or compatibility; `src/cadrumo/llm`.
...
- [ ] `W05.P10.S107` - Migrate bucket-maintenance recovery producers to typed conditions and canonical actions; `src/cadrumo/application/bucket_maintenance/_sandbox.py; src/cadrumo/application/bucket_maintenance/_service.py`.
...
- [ ] `W05.P10.S109` - Migrate application setup recovery producers to typed conditions and canonical actions; `src/cadrumo/application/setup/_service.py`.
...
- [ ] `W05.P10.S112` - Migrate core topics recovery producers to typed conditions and canonical actions; `src/cadrumo/core/topics/__init__.py`.
...
- [ ] `W05.P10.S117` - Replace financial OFX optional-extra forwarding and notice consumers with typed machine facts and explicit no-recovery outcomes preserving capability classification without raw installation prose or wrapper compatibility; `src/cadrumo/adapters/inbound/financial/providers/_ofx.py`.
