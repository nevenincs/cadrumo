...
- [ ] `P06.S16` - Finish reference-validator balancing by splitting remaining snapshot reference sections only if the next pass keeps each module under the current reviewability baseline; `src/aeat/domain/calculations/registry/_validate_references.py`.
