...
- [ ] `W02.P11.S409` - FU-S325-A review remaining attribution-entity exclusions on payer-fact and informative modelos; `S325 intentionally left M115/M180/M349/M347 unchanged; determine with source grounding whether attribution entities can owe each obligation when the corresponding payer/trade-threshold fact is present, then either widen the canonical applicability rules with non-tautological controls or document why the exclusion is legally correct; `src/aeat/domain/calculations/registry/_applicability.py`.
