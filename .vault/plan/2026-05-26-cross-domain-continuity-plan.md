...
- [x] `W04.P19.S398` - FU-S378 author M131 structural implies_nonzero predicate - if rendimientos base C01 is positive then suma-pagos-fraccionados-previos C07 must be non-zero - this is the registry-graph correctness check NOT the regulatory cuota-minima floor which remains corpus-blocked per task 226; `lands across all four M131 revisions 2019-2023 2024 2025 2026; gives implies_nonzero a second non-M210 consumer; 2 anti-tautology tests mutating C07 to zero with C01 positive and asserting the predicate fires; `src/aeat/_data/registry/aeat/modelos/131/revisions/`.
