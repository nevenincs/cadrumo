...
- [ ] `W04.P19.S340` - R9-ANDREA-SHOW-STOPPER M100 verify abort surfaces DRAFT_HAS_ERRORS=4 without exposing the 4 findings to operator; `review queue empty; log records the count not identity; expose findings via aeat app modelo work errors <revision_id> subcommand OR include them in verify --json output; `src/aeat/application/modelo/_actions.py`.
...
- [ ] `W05.P22.S342` - R9-ANDREA-SHOW-STOPPER ledger_renta_income_aggregation does NOT consume manual ledger add entries with irpf_category=actividad_economica; `M130 rendimiento neto stays 0.00 even after explicit ledger add with taxable_base + iva_rate; for estimación directa the ledger is the only data source so the autónomo cannot file M130; supplements W05.P22 income aggregation work which closed but does not handle the manual-ledger-add path; `src/aeat/application/aggregation/_renta_income_ledger.py`.
...
- [ ] `W09.P41.S212` - fix Real Decreto-ley 4/2004 legal citation typo in M200 parameters.toml; `Wave-3 audit FU-F; `src/aeat/_data/registry/aeat/modelos/200/`.
- [ ] `W09.P41.S213` - add clarifying comment in M100 binding-schema pin test explaining the 30-binding sentinel includes 19 scalar bindings plus 11 family-repeating-collection bindings; `prevents future drift in the sentinel meaning; Wave-3 audit FU-H; `src/aeat/application/modelo/test_profile_binding_real_path.py`.
...
- [ ] `W09.P41.S339` - FU-S96-A two stale .vault-scratch checkpoint files were swept into the bab2adac8 audit-verdict commit; `pre-existing untracked leftovers staged by git add; not peer WIP but worth a stricter explicit-pathspec discipline in future commits to avoid sweeping unrelated tracked leftovers; `.vaultspec/`.
- [ ] `W09.P41.S341` - R9-ANDREA-SHOW-STOPPER M303 calculate completely blocked by registry validation; `modelo-303-declaracion-pdf extraction profile sets corpus_round_trip_verified=true without verification_source set; affects ALL M303 calculations (2009 and 2023 revisions); Andrea cannot file three years of M303 backlog (~12 quarterly returns); `src/aeat/_data/registry/aeat/modelos/303/`.
