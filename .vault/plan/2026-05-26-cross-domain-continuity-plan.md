...
- [ ] `W09.P41.S357` - R9-TOMAS-HIGH AEAT_LOCAL_STORAGE_ROOT collision silent acceptance; `triage found profile create already uses duplicate-label refusal and provisional pointer rollback, but the exact second-profile-while-another-is-active edge still needs a real integration repro/guard: create alpha, create beta under the same storage root, assert beta becomes active and alpha remains readable without silent reuse; if it fails, fix the create path rather than falling back to the active profile; `src/aeat/application/wizard/ src/aeat/application/user_profile/ src/aeat/entrypoints/cli/tests/test_profile_lifecycle_verbs.py`.
