...
- [ ] `W09.P41.S408` - FU-S359-A compute activity-start Art. 109 coverage per payment period instead of relying only on the declared profile fact; `S359 honestly represents activity-start coverage as the same declared fact and fails closed when absent/false, but it does not yet derive the current-period 70 percent test from period income/withholding evidence; model a period-scoped calculation source only when the ledger/observation data can prove the numerator and denominator without casilla inference; `src/aeat/application/modelo/ src/aeat/domain/deadlines/ src/aeat/_data/registry/aeat/modelos/130/`.
