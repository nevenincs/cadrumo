...
- [x] `W09.P41.S411` - add the socio-profile repeatable attribution_received.N.* fact group (entity_nif, entity_name, share_pct, base_imponible_attributed, filing_year) to the central profile schema, mirroring the S323 attribution_entity_socios shape (schema-generic capture via profile import/edit + inline field descriptions; `NO dedicated wizard prompt and NO tr() locale keys, matching the S323 precedent). First-class wizard capture + en/es/ca/hu locale keys are tracked as the cross-cutting attribution profile-UX follow-up, not this step; `src/aeat/_data/registry/aeat/user_profile/schema.toml`.
...
- [x] `W09.P45.S329` - R9-ZSOFIA-B identify and localise commands where --language flag is accepted but has no effect on output; `config profile show config auth status modelo work calculate (closing prose only) confirmed broken; the parity test S144 must catch these as ineffective-flag cases not just absent-flag cases; `src/aeat/entrypoints/cli/`.
...
- [x] `W09.P45.S332` - R9-ZSOFIA-E broad --help text globally not hu-localised; `even where parent command supports --language hu the help text remains spanish/english; the --help text emission must consult active locale for option descriptions and section headers; `src/aeat/entrypoints/cli/`.
