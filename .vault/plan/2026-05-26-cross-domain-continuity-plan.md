...
- [ ] `W02.P11.S306` - R8-NURIA-SHOW-STOPPER add cross-profile calendar/agenda surface; `today aeat app overview calendar and aeat app overview agenda only process the active profile; a gestor with five clients must switch+view five times and manually consolidate; add --all-profiles flag OR a dedicated aeat app overview multiprofile-calendar verb that emits the union of every readable profile's deadlines; `src/aeat/entrypoints/cli/_overview.py`.
...
- [ ] `W04.P19.S305` - R8-NURIA-SHOW-STOPPER cannot create multiple profiles in same workspace; `NIF uniqueness checker _refuse_duplicate_tax_id opens UserProfileLifecycleRepository with the NEW bucket session but tries to read existing profiles which live in different buckets; StorageValidationError: the primary database route does not match the active bucket session; every gestor with multiple clients is blocked; single-directory-per-client workaround unacceptable for professional use; fix the uniqueness scan to use cross-bucket-readable session OR a session-independent profile index; `src/aeat/application/user_profile/_repository.py`.
...
- [ ] `W09.P41.S304` - ESCALATED FROM PRE-EXISTING TO CRITICAL — fix circular import _applicability deadlines _engine paralyzing the CLI itself per Nuria persona round-8; `aeat --version cannot run from a certain point in the session; chore/eliminate-shims branch added from ._applicability import ... to calculations/registry/__init__.py creating cycle calculations.registry → _applicability → deadlines.taxpayer_model → deadlines.__init__ → deadlines._engine → calculations.registry; CLI-blocking issue not just test failure; `src/aeat/domain/calculations/registry/__init__.py`.
