...
- [ ] `W04.P19.S296` - R8-M200-1 CRITICAL BLOCKER M200 cannot reach VERIFICADO_COMPLETO; casilla DP200014:00562 cuota integra Liquidacion III is declared manual+required=true in registry but the formula graph classifies it as computed (motor calculates it from DP200014:00552 times tipo de gravamen); operator cannot supply it as input and the W04.P19 verification contract cannot find it in inputs_snapshot; resolve the registry-vs-graph mismatch — either change input_kind to computed (and adapt the required gate accordingly) OR adapt the verification contract to accept required casillas that the engine itself produced; `src/aeat/_data/registry/aeat/modelos/200/`.
