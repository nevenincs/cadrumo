...
- [ ] `W07.P31.S353` - R9-TOMAS-CRITICAL M100 casilla 0505 base liquidable general sometida a gravamen is manual input with no formula derivation from 0500; `if operator does not explicitly supply 0505 the entire tarifa chain (0528→0532→0545→0570) computes zero silently producing cuota=0 on €14k+ base; either derive 0505 from 0500 via a formula OR mark it required=true so the gate fires at verify time; `src/aeat/_data/registry/aeat/modelos/100/revisions/2024/casillas/`.
...
- [ ] `W09.P41.S352` - R9-TOMAS-CRITICAL M303 wallet gate locks first-time users; `modelo-303-compensacion-pendiente-anteriores requires persisted IVA wallet reconciliation decision that only exists after a prior M303 filing; no init/seed CLI path (--binding rejected with 'caller binding values cannot override bucket-derived source bindings'); SHOW-STOPPER for autonomos starting their first M303; add aeat app modelo iva-wallet seed verb OR allow explicit zero-initialisation OR detect first-period and auto-seed zero; `src/aeat/application/calculations/_iva_wallet.py`.
