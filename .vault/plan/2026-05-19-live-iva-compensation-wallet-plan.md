...
- [ ] `W06.P15.S53` - Build a backend read-only acquisition orchestration that authenticates once, fetches filed Modelo history across requested years, attempts wallet/cartera read, and returns typed evidence plus typed failures. Follow-up 2026-06-03: the orchestration is now live-proven for the observed profile. It reused the persisted Cl@ve session, separately reported filed-history and wallet/cartera success, captured 12 Modelo 303 calculation observations across 2022-2026, persisted the acquisition manifest, and reloaded the wallet authority decision without private-value output. This row remains open only for production hardening and broader non-private regression coverage.; `src/aeat/application/live`.
- [ ] `W06.P15.S54` - Keep wallet/cartera direct-read outcomes distinct from filed-history success so either surface can succeed or fail independently without discarding the other evidence source. Follow-up 2026-06-03: live AEAT cartera read now succeeds through the guarded read-only query path while filed-history remains separately reported. Prior 403 and DOM-drift wording is historical and no longer the current blocker. Continue preserving fail-closed behavior for future wallet DOM drift, auth loss, or parser mismatch.; `src/aeat/application/live src/aeat/adapters/outbound/aeat/sede`.
...
- [ ] `W06.P15.S56` - Add and keep open an opt-in live read-only test path that requires operator-observed authentication and records only redacted diagnostics and aggregate evidence shape, not private taxpayer values. Follow-up 2026-06-03: wallet/cartera now yields parseable live AEAT cartera evidence through the read-only protected query path. A 2026-only read succeeded with filed-history zero rows and wallet success. A full 2022-2026 read succeeded with 12 Modelo 303 calculation observations and wallet success. Profile-local reload shows 12 history rows, 8 carry-forward lots, 7 wallet observations, latest wallet row count 1, selected authority aeat_wallet, divergence wallet_only, and blocked false. This row remains open as a standing live-verification path and privacy guard, not because wallet evidence is still missing.; `src/aeat/application/live src/aeat/adapters/outbound/aeat/sede src/aeat/core`.
...
- [ ] `W07.P18.S64` - Persist divergence decisions that compare AEAT evidence, filed-history-derived recurrence, local ledger recurrence, and explicit taxpayer override without merging the source values. Follow-up 2026-06-03: live wallet capture now persists a wallet authority decision on reload. Aggregate reload shows selected authority aeat_wallet, divergence wallet_only, and blocked false. Remaining work is broader exact-match, higher, lower, stale, and local-incomplete ladder coverage across sanitized non-private scenarios.; `src/aeat/application/calculations src/aeat/application/live`.
...
- [ ] `W08.P20.S70` - Compare persisted AEAT remote evidence against local recurrence and classify exact match, AEAT higher, AEAT lower, stale remote, local incomplete, and override-required outcomes. Follow-up 2026-06-03: the live AEAT wallet-only path is no longer provisional for the observed profile-local capture. Reload reports aeat_wallet authority with wallet_only divergence and blocked false. Remaining work is comprehensive non-private regression coverage for the other divergence states and calculation-readiness routing.; `src/aeat/application/calculations src/aeat/application/modelo`.
...
- [ ] `W09.P22.S77` - Run focused live-auth, Sede read-only, secure-storage, Modelo 303 and 390, Modelo 130 relation, and constants-guard test gates. Follow-up 2026-06-03: focused ruff and pytest gates passed for live acquisition, wallet parser, external constants, config surfaces, wallet persistence, Modelo 303 decision binding, Modelo 303 engine integration, and export/readiness behavior. Read-only live 2026 capture succeeded with wallet success. Read-only live 2022-2026 capture succeeded with 12 Modelo 303 calculation observations and wallet success. Profile-local reload succeeded with wallet authority decision present. The wallet backend fixture was updated to the current cartera table contract using synthetic amounts. Remaining gates stay open for broader Modelo 390, Modelo 130, and final readiness coverage.; `tests src/aeat/application/live src/aeat/application/modelo src/aeat/adapters/outbound/aeat/sede src/aeat/core`.
...
- [ ] `W09.P22.S79` - Run final code review before treating live IVA grounding as production-ready. Follow-up 2026-06-03: the previous persistent-failure status is superseded for the observed live wallet surface because the read-only cartera pull now succeeds and reloads an aeat_wallet authority decision. This row remains open for a fresh final code review of the live wallet parser, persisted authority decision routing, downstream Modelo 303 readiness/export behavior, privacy guarantees, and non-private divergence coverage before production-ready closure.; `.vault/audit src/aeat/application/live src/aeat/application/calculations src/aeat/application/modelo src/aeat/adapters/outbound/aeat/sede`.
