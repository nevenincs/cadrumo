...
- [ ] `W07.P02.S07` - Add tests for every divergence state using synthetic local ledgers and sanitized official/filed-history shapes, not private taxpayer history. Partial 2026-05-27: focused calculation tests now cover `filed_history_only` classification and production IVA-history repository fallback source preservation without private taxpayer fixtures; follow-up 2026-05-27: Modelo 303 engine integration now proves missing wallet/cartera plus persisted AEAT filed-history recurrence drives an explicit non-blocking `filed_history_only` decision; full readiness/modelo coverage remains open; `src/aeat/application/calculations src/aeat/application/modelo`.
