...
- [ ] `W06.P15.S53` - Build a backend read-only acquisition orchestration that authenticates once, fetches filed Modelo history across requested years, attempts wallet/cartera read, and returns typed evidence plus typed failures. Partial 2026-05-27 follow-up: `capture_iva_remote_state` code now acquires one session and reports filed-history and wallet/cartera as separate typed surfaces in `IvaRemoteStateAcquisitionReport`. 2026-05-28 testimonial correction: prior wording claiming a persisted Cl@ve session fetched Modelo 303 filed-history evidence is not accepted; no completed operator-confirmed AEAT auth/read is established. This row remains open until the orchestration is proven by read-only tests and a fresh operator-observed live run; `src/aeat/application/live`.
- [ ] `W06.P15.S54` - Keep wallet/cartera direct-read failures distinct from filed-history success so a 403 wallet outcome does not discard usable filed-history evidence. Blocker: direct wallet/cartera live acquisition is not yet accepted as reliable production evidence and must remain fail-closed until the live route yields parseable wallet evidence or a legally grounded no-wallet surface. Partial 2026-05-27 follow-up: combined acquisition report composition preserves filed-history success when wallet/cartera returns typed `AEAT_403` in code/tests. 2026-05-28 testimonial correction: prior wording claiming live filed-history success is not accepted; `src/aeat/application/live src/aeat/adapters/outbound/aeat/sede`.
- [ ] `W06.P15.S55` - Add multi-year filed-history parsing coverage using sanitized official or committed corpus evidence only, never the operator's private tax history as a fixture. 2026-05-28 testimonial correction: any prior "live filed-history shape" wording is not accepted as completed operator-authenticated evidence. Remaining fixture work must use sanitized official or synthetic non-private evidence only; `src/aeat/adapters/outbound/aeat/sede`.
- [ ] `W06.P15.S56` - Add and keep open an opt-in live read-only test path that requires operator-observed authentication and records only redacted diagnostics and aggregate evidence shape, not private taxpayer values. This test path must prove the production driver reaches the intended AEAT read surfaces and must not be replaced by mocks, fixtures, local secure-storage reloads, or unauthenticated navigation. 2026-05-28 testimonial correction: the prior claim that read-only live capture was exercised for 2022 through 2026 is invalidated/operator-disputed and must not be used as evidence. 2026-05-28 live retry: fresh Cl@ve Móvil auth succeeded, the combined read-only command reused the authenticated session, and the run returned typed failed surfaces: filed-history hit the live IVA surface timeout and wallet/cartera reported AEAT DOM drift. No private values were recorded and no AEAT write action was submitted. Follow-up 2026-05-28: the backend capture path now opens the selected active profile's secure-storage session before auth/persistence, eliminating the local route/session crash; a short live smoke run confirmed redacted auth preflight output and fail-closed surface timeouts. Follow-up 2026-06-02: the declarations driver and combined acquisition report now emit redacted live-surface phase diagnostics; a one-year read-only live run reached the authenticated Modelo 303 declaration-query route for 2026 and returned a successful zero-row filed-history result, while wallet/cartera still failed closed. A shorter live smoke proved timeout diagnostics now identify filed-history progress as `walk_declarations_register` for Modelo 303/2026 and wallet progress as `fetch_iva_compensation_wallet`, without private values. Follow-up 2026-06-02 discovered-entrypoint repair: official AEAT Pre303/cartera guidance is recorded in `[[2026-06-02-live-iva-compensation-consultation-research]]`, and the wallet driver now discovers a Pre303-provided cartera entrypoint before falling back to the configured direct route; the new action is centrally enrolled as `wallet-discovered-entrypoint-open` and remains read-only. Follow-up 2026-06-02 storage/auth retry: a live-profile secure-storage regression was fixed sufficiently for the active Cl@ve profile to become readable and identity-aligned, but the fresh Cl@ve login attempt failed with typed `auth_completion_timeout` after the configured 120 second operator window. Follow-up 2026-06-02 auth retry 2: the combined read-only IVA capture is explicitly recorded as failed because it produced no authenticated evidence, and a narrower Cl@ve retry failed again with typed `auth_completion_timeout`. Follow-up 2026-06-02 session-reuse repair: profile-scoped auth session persistence was fixed and a fresh operator-approved Cl@ve run produced a reusable persisted session. A single-year 2026 read-only capture reused that session and succeeded for filed-history with zero rows, but wallet/cartera still failed with `dom_drift` after the guarded read-query action. A 2022-2026 read reused auth but failed filed-history at 2024 Q1, and a 2024-only retry exceeded the outer command window without structured output. Follow-up 2026-06-02 bounded filed-history failure: per-declaration capture now returns a structured failed filed-history surface instead of hanging the CLI; a live 2024 retry then failed earlier because the Cl@ve session had expired, and the auth refresh failed with typed `auth_completion_timeout`. Follow-up 2026-06-02 wallet query-state repair: the Pre303-discovered wallet entrypoint no longer strips validated query parameters before browser navigation, while fragments are dropped and diagnostics still redact query values. Review follow-up 2026-06-02: a nested-timeout race was fixed by adding central `aeat_live_iva_declaration_capture_timeout_ms` settings validation below the outer surface timeout, so individual declaration capture can fail structurally before whole-surface cancellation. Post-commit live-auth retry 2026-06-02 failed with typed `auth_completion_timeout` before any IVA read surface ran; diagnostic `20260602T174331Z` was later classified by the operator as prompted but missed. Follow-up 2026-06-02 representation-gate repair: auth reached AEAT's representation dispatcher but failed closed when only the own-name input selector was available; the auth and wallet handlers now accept either centrally configured own-name label or own-name input selector, still refusing representative/unknown choices. Post-fix auth retry failed with typed `auth_completion_timeout` and diagnostic `20260602T181937Z`; no persisted session was saved and no IVA read surface ran. Follow-up 2026-06-02 live success/failure split: a fresh Cl@ve auth succeeded and was persisted; 2026-only filed-history succeeded with zero rows, 2023-only filed-history succeeded with four Modelo 303 observations, and full-span filed-history exposed that the fixed 180 second outer timeout is too small for multi-year capture. The filed-history timeout now scales by requested year count. Follow-up 2026-06-02 full-span success: after the CLI profile-key registration repair and year-scaled filed-history timeout, the full 2022-2026 read-only capture reused auth and succeeded for filed-history with 12 Modelo 303 observations captured/calculation-promoted. Local reload returned the same 12 filed-history rows and 8 carry-forward lots, but no authority decisions and a nonzero unallocated applied amount; exact live amounts are not recorded in this plan. Wallet/cartera still fails closed after the guarded read-query shell and has not yielded parseable wallet evidence. This row remains open until wallet/cartera yields parseable evidence or a legally grounded alternate remote-state source is implemented; `src/aeat/adapters/outbound/aeat/sede/test_*_live.py`.
...
- [ ] `W08.P20.S70` - Compare persisted AEAT remote evidence against local recurrence and classify exact match, AEAT higher, AEAT lower, stale remote, local incomplete, and override-required outcomes. 2026-05-28 testimonial correction: prior wording claiming live filed-history evidence can be promoted into persisted calculation observations is not accepted as completed live evidence. The code path may model filed-history-only recurrence as a distinct blocking fallback state when wallet/cartera is unavailable, but full persisted remote/local ladder coverage and live verification remain open; `src/aeat/application/calculations`.
...
- [ ] `W09.P22.S77` - Run focused live-auth, Sede read-only, secure-storage, Modelo 303/390, Modelo 130 relation, and constants-guard test gates. The live-auth and Sede read-only gates remain open until an operator-observed, opt-in AEAT run completes authentication and records redacted aggregate evidence shape from the intended read surfaces. Local gates may demonstrate taxonomy, persistence, privacy, registry, and fail-closed behavior only; they must not be used to claim live functionality. Partial 2026-05-27: focused ruff and pytest gates passed for persisted session metadata and Modelo 303 submitted-file fallback behavior, but any prior live filed-capture wording is not accepted as live AEAT evidence. Partial 2026-05-27 follow-up: live application acquisition taxonomy, wallet backend, and combined remote-state report tests pass locally; follow-up 2026-05-27: the affected live/modelo gate exposed and fixed missing Modelo 303 extraction-profile `verification_source` registry provenance for the two IVA wallet-backed revisions; follow-up 2026-05-27 reload: focused Modelo 303 wallet export/readiness tests, Renta Art. 84 scenario tests, user-profile schema/profile-family tests, touched-file ruff checks, and privacy scans pass after privacy and registry-drift repairs; reload follow-up 2026-05-27: registry-backed filing helper, calculation summary, and reconciliation tests pass with Modelo 130 previous-filing bindings routed through `binding_values`, reconciliation providers built from each draft snapshot ref, and direct provider checks for Modelos 180, 200, 111, 123, and 130 pass; 2026-05-28 live retry: fresh auth passed, the read-only command returned typed filed-history timeout plus wallet/cartera DOM-drift failures, and the new focused timeout/taxonomy/aggregation gates passed; 2026-05-28 storage/privacy follow-up: focused ruff plus live acquisition/CLI renderer tests passed, and a short read-only live smoke run confirmed auth-session reuse, active-profile redaction, and fail-closed surface timeouts. Follow-up 2026-05-28: focused cancellation-filtering tests passed, and a live smoke run with an expired persisted session produced a typed Cl@ve approval timeout without post-command Playwright `TargetClosedError` noise. Follow-up 2026-06-02: after codebase centralisation drift, focused ruff, live acquisition/taxonomy/config tests, locale audit, and a read-only short live smoke passed; the smoke reused a persisted Cl@ve session and failed closed with typed surface timeouts, so it proves shutdown hygiene and redacted auth diagnostics only, not accepted IVA data extraction. Follow-up 2026-06-02 diagnostics slice: focused ruff passed on touched live/declarations/CLI/locale files, focused tests passed, `aeat.locales audit` passed for `es/en/ca/hu`, and two read-only live smokes established redacted phase diagnostics plus one successful zero-row Modelo 303/2026 filed-history query; wallet/cartera remains unresolved and keeps this row open. Follow-up 2026-06-02 discovered-entrypoint repair: focused ruff passed on touched wallet/constants files and focused pytest passed with 81 tests for wallet parser/discovery and external constants; no live success is claimed. Follow-up 2026-06-02 storage/auth retry: focused ruff passed on secure-object, access-gate, IVA classification, invoice public-surface, wallet, constants, and profile files; focused secure-object, profile create/health, auth diagnostics/session, IVA classification, and invoice-model suites passed. The live login retry failed with typed `auth_completion_timeout`, so live IVA extraction remains unproven. Follow-up 2026-06-02 auth retry 2: another read-only auth attempt failed with typed `auth_completion_timeout`, and the IVA capture was not run because the session was not authenticated. Follow-up 2026-06-02 session-reuse repair: focused auth ruff passed and focused auth operator/session tests passed; live Cl@ve auth succeeded and persisted-session probing now reports a reusable non-expired session. The single-year live read reused auth and succeeded for 2026 filed-history only; wallet/cartera and 2024 filed-history remain failed live surfaces. Follow-up 2026-06-02 bounded filed-history failure: focused live-acquisition tests pass for partial filed-history failure reporting, but live 2024 verification remains blocked by an expired session and a subsequent Cl@ve auth timeout. Follow-up 2026-06-02 wallet query-state and timeout-hierarchy repair: focused ruff passed on touched config/wallet/live/auth files, and focused pytest passed with 126 tests covering wallet entrypoint query preservation, central timeout hierarchy validation, live acquisition failure taxonomy, and auth-session reuse. Follow-up 2026-06-02 full-span filed-history: focused ruff/tests passed for multi-year timeout scaling and wallet execute-shell polling, cold-process CLI profile-key registration passed, and live 2022-2026 filed-history succeeded with 12 Modelo 303 observations while wallet/cartera still failed closed. No live wallet/cartera success is claimed; `tests src/aeat/_data/registry`.
- [ ] `W09.P22.S78` - Persist review findings for any discovered non-IVA problems and keep them in scope until assigned to an owning plan row. Partial 2026-05-27: Modelo 130 casilla 15 binding-resolution failure is explicitly tracked as separate IRPF/shared-infrastructure work and must remain in scope outside the IVA compensation authority path. Follow-up 2026-05-27: Modelo 303 engine integration exposed a ledger application export-contract regression; the missing `_actions` rule/bulk classification exports and `_result` helper were restored so calculation preflight can import, and the ledger CLI `--from-csv` / `ledger rule` surface now passes its focused regression gate with rule/manual provenance projected in list output. Follow-up 2026-05-27 reload: wallet readiness tests no longer carry DNI/NIE-shaped synthetic taxpayer data, and shared Renta registry/profile-schema drift discovered by the IVA verification gate is recorded in `RELOAD-008`; reload follow-up 2026-05-27: exact operator-identity leaks are recorded in `RELOAD-009`, and the reconciliation gate's Modelo 130 previous-filing projection plus Modelo 180 legal-ref blockers are recorded in `RELOAD-010`; reload follow-up 2026-05-27: RELOAD-010 is resolved for the active reconciliation surface, with Modelo 180/200 provider drift rechecked as clear in the current tree; follow-up 2026-06-02: live capture emitted Modelo 347 quarter semantic-role warnings during registry loading, slated for owner review and not fixed in this IVA cancellation slice; follow-up 2026-06-02 live-gate drift repair: the focused live IVA test gate exposed a non-IVA PDF-boundary import regression after adapters/inbound/pdf/_errors.py was removed; the owning domain-boundary plan row `W04.P10.S41` now records the remaining pdf/__init__ and pdf/_scrub caller repairs. Follow-up 2026-06-02 storage/import repair: live IVA auth was blocked by non-IVA backend regressions in secure-object deterministic-key migration, access-gate public exports, IVA/invoice import cycles, master-key strict-config import ordering, and ledger manual-classification constant re-export; these were fixed for the current slice and remain slated for broader secure-storage/domain-boundary/constants review; `.vault/audit .vault/plan src/aeat/application/filing src/aeat/application/ledger src/aeat/entrypoints/cli src/aeat/_data/registry src/aeat/adapters/inbound/pdf src/aeat/adapters/persistence/storage src/aeat/core/access_gate src/aeat/domain/iva src/aeat/domain/invoices`.
- [ ] `W09.P22.S79` - Run final code review before treating live IVA grounding as production-ready. This row cannot close until S56 and S77 include a fresh operator-observed live read-only verification, or the review explicitly declares the feature failed/provisional/not production-ready. Partial 2026-05-27: focused review of stored remote IVA reload and ledger regression fixes found and closed two HIGH issues: rule-apply failures now surface instead of being counted as no-match, and wallet secure-storage tests no longer use valid-looking taxpayer identifiers. Follow-up 2026-06-02: the persistent-failure team brief records that the implementation has failed the required product outcome: it has not yet programmatically consulted AEAT live state for the declarante and extracted the binding IVA compensation/running-balance evidence needed to ground local Modelo 303 calculations. Declaration-query consultation is explicitly required, not optional, and wallet/Pre303 remains unresolved; see `[[2026-06-02-live-iva-persistent-failure-team-brief]]`; `.vault/audit`.
