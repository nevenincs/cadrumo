...
- [ ] `W07.P02.S06` - Surface blocked, stale, missing-wallet, filed-history-only, and override-required states through workflow/modelo readiness before export or verification. Partial 2026-05-27: Modelo 303 calculation now treats `filed_history_only`, `wallet_missing`, and stale-wallet fallback decisions as blocking evidence states; direct wallet/cartera match, wallet-only evidence, or explicit taxpayer override remain the non-blocking paths. Workflow-level readiness wording remains open; `src/aeat/application/modelo src/aeat/application/workflow`.
- [ ] `W07.P02.S07` - Add tests for every divergence state using synthetic local ledgers and sanitized official/filed-history shapes, not private taxpayer history. Partial 2026-05-27: focused calculation tests now cover `filed_history_only` classification and production IVA-history repository fallback source preservation without private taxpayer fixtures; follow-up 2026-05-27: Modelo 303 engine integration now proves missing wallet/cartera plus persisted AEAT filed-history recurrence blocks automatic prefill, and a separate explicit taxpayer override is required before the real engine can use that fallback amount; full workflow readiness coverage remains open; `src/aeat/application/calculations src/aeat/application/modelo`.
...
- [ ] `W08.P02.S05` - Block Modelo 303 prior-compensation prefill when remote/local divergence is unresolved, and require an explicit persisted decision for any override. Partial 2026-05-27: unresolved fallback states now block prefill in the Modelo 303 engine; an explicit taxpayer override decision can unblock the same filed-history fallback amount while preserving separate authority sources; export/verification inherited blocked-decision refusal remains covered, and broader workflow readiness coverage remains open; `src/aeat/application/modelo src/aeat/application/calculations`.
