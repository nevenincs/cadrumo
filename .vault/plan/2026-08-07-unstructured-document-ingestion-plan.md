...
date: '2026-08-07'
modified: '2026-08-07'
body_hash: 'sha256:59cd97f7dba228e8fb8bae7119095b4e7c410e2de810ea4bb7bfb2f9036bf80e'
tier: L3
related:
...
- [ ] `W02.P05.S105` - Wire the classification criteria the existing rule table consumes, since the criteria model is constructed nowhere in production, only in its own docstring example and in tests, so the classifier is unreachable from every real path rather than merely missing one input. Assemble it from the producers, territorial scope from the establishment resolver, taxable-person status from the printed identifier and the profile authority, transaction kind from the direction, and call the table where a category is needed, refusing rather than defaulting when any input is absent so an unreachable classifier is not replaced by one that answers on incomplete evidence; `src/cadrumo/application/ledger`.
