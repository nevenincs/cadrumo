...
- [ ] `P05.S28` - Assess whether the same post-auth failure spends a Clave Permanente credential, a question recorded only inline in one step record and tracked nowhere, escalating to a coding campaign with a discovery gate if it proves a defect; `src/cadrumo/adapters/outbound/aeat/auth/_clave_permanente_support.py`.
- [ ] `P05.S29` - Repair the semantic code index until its indexed section count matches the tracked file count, the instrument measured 466 then 20 then zero sections while reporting success and a rebuild did not converge it; `operator action, semantic index service`.
- [ ] `P05.S30` - Rerun the twelve semantic duplication probes once the index is trustworthy, closing the residue of a row that was superseded rather than satisfied; `src/cadrumo/, dev/audit/`.
