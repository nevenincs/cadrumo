...
date: '2026-08-03'
modified: '2026-08-03'
body_hash: 'sha256:80decdfb80d02421ad6b4f528b96ba4ba88ef1e379a61025af10bbc0447e9826'
tier: L3
related:
...
- [ ] `W01.P03.S26` - Correct WINDOWS_WORST_CASE_OBJECT_PATH_SUFFIX_LENGTH to include the missing outbound namespace segment the local_provider_object_sidecar grammar already declares, deriving the literal from that grammar rather than re-pointing it onto the bucket-layout members as originally written, since re-pointing alone swaps literals for constants and leaves the omission in place, measured 136 versus the true 155, and rewrite the anti-tautology guard in test_paths.py so it fails against the current value before passing against the corrected one, also correcting the adjacent docstring's citation of the superseded _namespace_registry as the declaring module; `src/cadrumo/core/paths.py, src/cadrumo/core/tests/test_paths.py`.
...
- [ ] `W02.P07.S95` - Re-point the profile bucket scan module's root resolver onto effective_storage_root once S10 lands, deleting the local override-or-settings-default duplicate; `src/cadrumo/application/workflow/_profile_bucket_scan.py`.
- [ ] `W02.P07.S96` - Re-point the profile repository's root resolution onto effective_storage_root once S10 lands, deleting the inline override-or-settings-default duplicate; `src/cadrumo/application/user_profile/_profile_repository.py`.
- [ ] `W02.P07.S97` - Re-point the config reset repository's root resolution onto effective_storage_root once S10 lands, deleting the inline override-or-settings-default duplicate; `src/cadrumo/application/_config_reset_repository.py`.
- [ ] `W02.P07.S98` - Re-point the bundle export operation's root resolution onto effective_storage_root once S10 lands, deleting the inline override-or-settings-default duplicate, closing the cross-platform identity-comparison defect a relative or differently-cased override currently risks since only the reference body in the pointer transaction module normalises today; `src/cadrumo/application/user_profile/_bundle_export_operation.py`.
...
- [ ] `W02.P08.S99` - Add one parameterised helper collapsing the five copy-pasted optional-root Typer resolvers, covering both the bundled-default family in registry.py and the settings-default family in registry.py and _app_live.py, so the two families stop drifting apart independently; `src/cadrumo/entrypoints/cli/registry.py, src/cadrumo/entrypoints/cli/_app_live.py`.
- [ ] `W02.P08.S100` - Converge the five resolvers onto the new parameterised helper and delete the five standalone functions, lower priority than the other Wave 2 phases and not a closure blocker; `src/cadrumo/entrypoints/cli/registry.py, src/cadrumo/entrypoints/cli/_app_live.py`.
