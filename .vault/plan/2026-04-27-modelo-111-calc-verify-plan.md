...
- **Phase 2 — 2026 ruleset + harness extension**
  1. Author `src/aeat/formulas/_rulesets/modelo_111_2026.py` mirroring
  ...
  1. Register `MODELO_111_2026` in
     `src/aeat/formulas/_rulesets/__init__.py` (import + `ALL_RULESETS`
     + `__all__`).
  ...
  1. Run `uv run pytest src/aeat/formulas/_rulesets/ -k 111` —
     baseline green.
...
- **Phase 3 — sibling extractor classes**
  1. Edit `src/aeat/declaracion/_extractors/modelo_111_v2025.py`:
  ...
  1. Update the module docstring to document the multi-year layout
     stability.
  1. Edit `src/aeat/declaracion/_extractors/__init__.py`: import the
...
- **Phase 5 — colocated 2024 + 2026 test files**
  1. Author `src/aeat/formulas/_rulesets/test_modelo_111_2024.py` —
     mirrors `test_modelo_111_2025.py` with a 2024-distinct fixture +
     `test_2024_no_drift_from_2025` regression + external-anchored
     worked example.
  1. Author `src/aeat/formulas/_rulesets/test_modelo_111_2026.py` —
  ...
  1. Run `uv run pytest src/aeat/formulas/_rulesets/test_modelo_111_2024.py
     src/aeat/formulas/_rulesets/test_modelo_111_2026.py -v` — all
     green.
...
1. **2026 ruleset exists + registered**: `MODELO_111_2026` resolves
   via `aeat.formulas._rulesets.ALL_RULESETS`, has
...
2. **Sibling extractors registered**: `Modelo111V2024Extractor` and
   ...
   `aeat.declaracion._extractors.get_extractor` for the respective
...
- ✅ Errors inherit from `aeat.errors.AeatError` (no new errors
