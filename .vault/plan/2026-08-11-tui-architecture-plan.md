...
modified: '2026-08-25'
body_hash: 'sha256:a719dc6dc0f04e0799749c60d88907b234ad8912d24db974337bb1863e269d38'
---
...
- [ ] `W03.P20.S163` - Expose the canonical registry-closure native atomic capture and owner generation through its public application registry facade without recreating the closure join, development-only authority, or blocker classification; `src/cadrumo/application/registry/_closure.py, src/cadrumo/application/registry/__init__.py, and focused closure parity tests`.
- [ ] `W03.P20.S164` - Expose the canonical calculation materialization and source-graph safe native atomic capture and owner generation by reusing the sole calculation-revision and provenance authorities without a parallel calculation, graph, persistence, or redaction path; `src/cadrumo/application/modelo/_calculation_actions.py and focused materialization/provenance parity tests`.
- [ ] `W03.P20.S165` - Expose the canonical locale-catalogue native atomic capture and owner generation through the locale facade without reimplementing key resolution, Spanish fallback, suppression, catalogue loading, or digest semantics; `src/cadrumo/locales/ and focused locale parity tests`.
- [ ] `W03.P20.S166` - Expose the application-owned field-manifest native atomic capture and owner generation, correct its contributor owner to application.modelo.workspace_manifest, and preserve the generated manifest as the sole field-classification authority without a second walker or permanent allowlist; `src/cadrumo/application/modelo/_workspace_manifest.py and focused manifest identity/generation tests`.
