...
date: '2026-08-11'
modified: '2026-08-25'
body_hash: 'sha256:acb9967ccee0946ae12ff01b35894b85d29d801f0513ecc380c8ec3cce0e1778'
tier: L3
related:
...
Implement the read-only Workspace V1 contract, stamped contributing ports, generated schema-field denominator, canonical owner projections, and live conformance without exposing registry grammar or duplicating ModeloWorkReview. Every open step in this phase begins with Vaultspec RAG semantic codebase discovery and exact source census, and closes only after the same census proves one canonical home and deletion of any parallel implementation, legacy path, shim, alias, fallback, or re-export bridge found in its scope. Before native WORK capture, serialize the active-pointer transition authority, one-record singleton observation, pure captured-catalogue selector convergence, S125 independent revision axes and contributor-epoch digest, S126 epoch-schema-v2 comparison domain, S159 native comparison-domain correction, and pure S159-backed two-axis assertion/raw-loader teardown. S160 follows every prerequisite; S161-S166 return their owner-native opaque comparison domains; S167 follows all eight corrected native surfaces and S126 v2; S128 follows S160-S167 and the S125 correction.
...
- [ ] `W03.P20.S170` - Relocate visible, exact, and active-only ModeloWorkResolution policy into one pure selector over a supplied captured WorkUnitCatalogue, atomically converge work review, external import, overview, calculation, and every substitutable strict-ID consumer, and delete parallel scans/read paths with semantic-plus-exact fixed-point proof and no shim or re-export; `src/cadrumo/application/modelo/_selectors.py, src/cadrumo/application/modelo/work_review_projection.py, src/cadrumo/application/modelo/_external_import_actions.py, src/cadrumo/application/modelo/_calculate_input.py, src/cadrumo/application/overview/_data_prep.py, and focused selector fixed-point tests`.
- [ ] `W03.P20.S171` - Replace S125's single revision assertion atomically with fixed requested-target and stored-work typed axes, each carrying its source, optional asserted revision, and not_present, matched, or mismatched outcome, thread both through resolved targets and typed mismatch refusals, bind the full epoch-v2 coordinates through contributor_epoch_digest into baselines, cursors, and facets, and delete the old assertion and reader without alias, default synthesis, or compatibility path; `src/cadrumo/application/modelo/_workspace_models.py and src/cadrumo/application/modelo/tests/test_workspace_models.py`.
- [ ] `W03.P20.S172` - Replace S126 epoch schema v1 atomically with schema v2 carrying the native opaque comparison domain beside the unchanged generation, require exact domain equality before integer comparison, regenerate producer-contract and inventory digests, and delete the v1 path while proving same-root success and distinct-root, root-switch, ABA, and cross-process refusal; `src/cadrumo/application/modelo/_workspace_producers.py and src/cadrumo/application/modelo/tests/test_workspace_producers.py`.
- [ ] `W03.P20.S173` - Extend the checked S159 registry-native capture/currentness coordinate to derive and return a neutral opaque comparison domain bound to the physical registry and source roots plus the application process incarnation beside the unchanged generation, promote the corrected capture/current-coordinate surface through the canonical registry facade, and prove same-domain currentness plus distinct-root, switched-root, reset, and cross-process refusal without importing or returning any ModeloWorkspace type; `src/cadrumo/domain/calculations/registry/_authority.py, src/cadrumo/domain/calculations/registry/__init__.py, and focused authority native-capture/currentness tests`.
- [ ] `W03.P20.S174` - Relocate revision identity to one pure application assertion that evaluates the independent S125 requested and stored axes against the law-selected revision from exactly one S159 RegistryAuthorityCapture, promote that assertion through the canonical cadrumo.application.modelo facade in the same commit, atomically migrate every addressing, work-review, calculation, external-import, quickfile, lifecycle, and CLI consumer, and delete resolve_registry_revision_for_work_target, its facade export, load_registry_tree, asserted-ID selection, stale docstring references, and every parallel registry read from the work path with fixed-point proof and no shim, alias, fallback, or re-export bridge; `src/cadrumo/application/modelo/_calculation_helpers.py, src/cadrumo/application/modelo/_work_addressing.py, src/cadrumo/application/modelo/work_review_projection.py, src/cadrumo/application/modelo/_calculate_input.py, src/cadrumo/application/modelo/_external_import_actions.py, src/cadrumo/application/modelo/_quickfile.py, src/cadrumo/application/modelo/_work_lifecycle.py, src/cadrumo/application/modelo/__init__.py, src/cadrumo/entrypoints/cli/_modelo_work_lifecycle_cli.py, src/cadrumo/entrypoints/cli/_config/_profile_inspect.py, and focused revision-identity/facade-consumer tests`.
- [ ] `W03.P20.S160` - Expose through the sole cadrumo.application.modelo facade a work-only native atomic capture/current-coordinate pair over the canonical visible or exact operand that returns the strict frozen ModeloWorkResolution, native generation, and neutral opaque physical-scope/process comparison domain, composes implicit pointer and one-record catalogue coordinates with bounded retry/currentness and an injective order-preserving generation, preserves the explicit catalogue generation while excluding the pointer limb, keeps physical root, bucket, namespace, and key identity private, and proves pointer/catalogue ABA, same-observation singleflight, distinct-root independence, facade ownership, and zero registry access, second read, Workspace dependency, shim, alias, fallback, or bridge; `src/cadrumo/application/modelo/_work_addressing.py, src/cadrumo/application/modelo/__init__.py, and focused work-capture/root/pointer concurrency tests`.
- [ ] `W03.P20.S161` - Expose and promote through the sole canonical cadrumo.application.modelo facade in the same commit the bounded ModeloWorkReview native atomic capture/current-coordinate pair, owner generation, and neutral opaque comparison domain by delegating the sole build_modelo_work_review semantic join, and prove public-surface parity without reconstructing any review field, retaining a parallel assembler, or introducing a shim, alias, fallback, or non-facade re-export bridge; `src/cadrumo/application/modelo/work_review_projection.py, src/cadrumo/application/modelo/__init__.py, and focused complete-parity/currentness/facade tests`.
- [ ] `W03.P20.S162` - Expose the canonical ProjectionModeloReadiness native atomic capture, owner generation, and neutral opaque comparison domain without inferring capability, collapsing readiness axes, or duplicating operator-state computation; `src/cadrumo/application/state_projection.py and focused readiness parity/currentness tests`.
- [ ] `W03.P20.S163` - Expose the canonical registry-closure native atomic capture, owner generation, and neutral opaque comparison domain through its public application registry facade without recreating the closure join, development-only authority, or blocker classification; `src/cadrumo/application/registry/_closure.py, src/cadrumo/application/registry/__init__.py, and focused closure parity/currentness tests`.
- [ ] `W03.P20.S164` - Expose and promote through the sole canonical cadrumo.application.modelo facade in the same commit the calculation-materialization and source-graph-safe native atomic capture/current-coordinate pair, owner generation, and neutral opaque comparison domain by reusing the sole calculation-revision and provenance authorities, and prove public-surface parity without a parallel calculation, graph, persistence, or redaction path or any shim, alias, fallback, or non-facade re-export bridge; `src/cadrumo/application/modelo/_calculation_actions.py, src/cadrumo/application/modelo/__init__.py, and focused materialization/provenance parity/currentness/facade tests`.
- [ ] `W03.P20.S165` - Expose the canonical locale-catalogue native atomic capture, owner generation, and neutral opaque comparison domain through the locale facade without reimplementing key resolution, Spanish fallback, suppression, catalogue loading, or digest semantics; `src/cadrumo/locales/ and focused locale parity/currentness tests`.
- [ ] `W03.P20.S166` - Expose and promote through the sole canonical cadrumo.application.modelo facade in the same commit the application-owned field-manifest native atomic capture/current-coordinate pair, owner generation, and neutral opaque comparison domain, correct its contributor owner to application.modelo.workspace_manifest, preserve the generated manifest as the sole field-classification authority, and prove public-surface parity without a second walker, permanent allowlist, shim, alias, fallback, or non-facade re-export bridge; `src/cadrumo/application/modelo/_workspace_manifest.py, src/cadrumo/application/modelo/__init__.py, and focused manifest identity/generation/currentness/facade tests`.
- [ ] `W03.P20.S167` - Register exactly one application-owned S126 epoch-v2 port realization over each of the eight canonical native owner capture/current-coordinate surfaces, atomically relocate the existing field-manifest contract into that sole registration inventory, and prove exact contributor identities, one native capture, contract-derived stamps, unchanged owner generations and comparison domains, admission-set coverage, same-domain validation, and refusal of missing, duplicate, stale, torn, ABA, cross-domain, or cross-incarnation coordinates without an alternate authority, shim, alias, fallback, re-export bridge, or adapter-package implementation; `src/cadrumo/application/modelo/_workspace_producers.py and focused native-seam conformance tests`.
- [ ] `W03.P20.S128` - Assemble static Workspace projections by capturing WORK exactly once before REGISTRY exactly once, deriving the REGISTRY coordinate only from the captured ModeloWorkResolution, evaluating the independent requested and stored axes through the sole pure S159-backed assertion, and then capturing locale-catalogue and field-manifest for static admission or all remaining S126 registrations for graded admission, with epoch-v2 same-domain two-pass validation, process-incarnation-scoped baselines and cursors, bounded materialization, and no pre-capture work read, owner reread, or registry grammar; `src/cadrumo/application/modelo/_workspace_projection.py`.
...
The discharged `casilla-schema` prerequisite no longer blocks execution. Independent backend contract phases may proceed in parallel when they do not share an authority or write surface, but receipt minting, composition, cutover, and deletion remain serialized. Within W03.P20, the seven pre-S160 remediation Steps are serialized in plan order; S160 follows all seven and S159, S161-S166 require the epoch-v2 native-coordinate contract, S167 waits for every corrected native surface plus S126 v2, and S128 waits for S160-S167 plus the corrected S125 assertion axes. No TUI operation projection begins before the exact C0 receipt. Interface C2 waits for its exact Workspace C2 dependency receipt, interface C3 waits for the exact C2 exit plus financial-operand and Edit Contract dependency receipts, interface C4 waits for the C3 exit plus the enrolled action denominator, and C5 waits for the C4 exit and final structural fixed point. The interface plan consumes these receipts and never redeclares their application contracts.
