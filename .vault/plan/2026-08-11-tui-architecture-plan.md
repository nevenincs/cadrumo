...
modified: '2026-08-25'
body_hash: 'sha256:0de040c4f05f27b69cb47b9df8305d8bfc766da791b800c23526d0334400ef6e'
---
...
- [ ] `W03.P20.S168` - Promote the active-profile pointer owner to one public atomic observation/current-coordinate transaction that persists an absent tombstone and monotonic transition revision under the canonical custody-root lock, atomically migrate every production pointer reader and writer, and prove idempotence, cross-process A -> B -> A, restore/clear lineage, and zero dual mutation paths without a shim or re-export; `src/cadrumo/core/_bucket_pointer_io.py, src/cadrumo/application/user_profile/_profile_pointer_transaction.py, src/cadrumo/application/user_profile/__init__.py, production pointer consumers, and focused custody/pointer concurrency tests`.
...
- [ ] `W03.P20.S174` - Relocate revision identity to one pure application assertion that evaluates the independent S125 requested and stored axes against the law-selected revision from exactly one S159 RegistryAuthorityCapture, atomically migrate addressing, work-review, and calculation consumers, and delete load_registry_tree, asserted-ID selection, and parallel registry reads from the work path with fixed-point proof; `src/cadrumo/application/modelo/_calculation_helpers.py, src/cadrumo/application/modelo/_work_addressing.py, src/cadrumo/application/modelo/work_review_projection.py, src/cadrumo/application/modelo/_calculate_input.py, and focused revision-identity tests`.
...
- [ ] `W03.P20.S161` - Expose the canonical bounded ModeloWorkReview native atomic capture, owner generation, and neutral opaque comparison domain by delegating the sole build_modelo_work_review semantic join without reconstructing any review field or retaining a parallel assembler; `src/cadrumo/application/modelo/work_review_projection.py and focused complete-parity/currentness tests`.
...
- [ ] `W03.P20.S164` - Expose the canonical calculation materialization and source-graph safe native atomic capture, owner generation, and neutral opaque comparison domain by reusing the sole calculation-revision and provenance authorities without a parallel calculation, graph, persistence, or redaction path; `src/cadrumo/application/modelo/_calculation_actions.py and focused materialization/provenance parity/currentness tests`.
...
- [ ] `W03.P20.S166` - Expose the application-owned field-manifest native atomic capture, owner generation, and neutral opaque comparison domain, correct its contributor owner to application.modelo.workspace_manifest, and preserve the generated manifest as the sole field-classification authority without a second walker or permanent allowlist; `src/cadrumo/application/modelo/_workspace_manifest.py and focused manifest identity/generation/currentness tests`.
