...
date: '2026-08-04'
modified: '2026-08-05'
body_hash: 'sha256:9f1d6d0cbf0a068f7d76f35f31a841e163f595c62fee6c779f7bb881261ab697'
tier: L2
related:
...
- [ ] `P04.S35` - Decide how casilla 0611 becomes genuinely registry-computed now that it is no longer a function of any scalar the registry can bind, because the per-hijo cap varies with which hijo carries the alta-posterior increment so one month count maps to several correct totals, which leaves per-hijo detail rows through the row channel as the only honest route and BARS the derived-fact-plus-copy shape that would close the asymmetry cosmetically while leaving the whole legal rule in Python and harder to see than the honest gap; `src/cadrumo/_data/registry/aeat/modelos/100/revisions/2024/formulas/, src/cadrumo/application/modelo/_profile_binding.py, src/cadrumo/domain/calculations/registry/`.
...
- [ ] `P04.S39` - Model both limbs of Art. 61 norma 4a in one slice, the death date on the descendant record with its roundtrip and anti-tautology proof, the flat amount replacing the birth-order tranche for the deceased, the exclusion of the deceased from the ordering that ranks the survivors, and an entry surface shipping with the field, because omitting the second limb moves every younger sibling to a cheaper rank so the survivors are over-granted too and a flat-amount-only fix would look complete while leaving half the over-grant standing; `src/cadrumo/domain/contribuyente/family.py, src/cadrumo/domain/contribuyente/_descendant_facts.py, src/cadrumo/_data/registry/cadrumo/user_profile/schema.toml, src/cadrumo/entrypoints/cli/_config/_descendiente.py`.
