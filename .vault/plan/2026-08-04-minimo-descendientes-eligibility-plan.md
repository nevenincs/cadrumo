...
date: '2026-08-04'
modified: '2026-08-05'
body_hash: 'sha256:4f66e7e250f5244bfc106896f50654139b8ac5f9b4267b351efacd1a39d08e1b'
tier: L2
related:
...
- [ ] `P04.S40` - Prorate the Art. 81.2 guarderia increment per child as 1000 divided by 12 times the months in which the Art. 81.1 AND 81.2 requirements hold SIMULTANEOUSLY counting only complete paid months, then bound that prorated figure by that child own total non-subsidised spend before summing across children, because the formula applies a flat 1000 per child with no month term and the manual works the case as 1000 over 12 times 2 equals 166.67 with the note that the increment must not exceed the gasto efectivo no subvencionado, so a two-month child is granted 1000 against a correct 166.67, an over-grant of 833.33 euros reachable for every mid-year birth and every partial-year enrolment; `src/cadrumo/_data/registry/aeat/modelos/100/revisions/2024/formulas/, src/cadrumo/domain/contribuyente/family.py, src/cadrumo/application/modelo/_profile_binding.py`.
- [ ] `P04.S41` - Bound the interpolated descendant list in both maternidad advisories the way the accepted-set hint in the same module already is, because their length is dynamic rather than static so each grows one entry per descendant and crosses the 512 character diagnostic cap at 13 and at 25 descendants respectively, surfacing a raw pydantic ValidationError to the operator and stopping the filing outright, and add tests constructing each above those counts rather than at a single index; `src/cadrumo/application/modelo/_calculate_input.py`.
