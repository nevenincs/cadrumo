...
date: '2026-08-04'
modified: '2026-08-10'
body_hash: 'sha256:1bcec2066bf4db170226ac21dec16dfb22d080640763630d5ca8895b6b3fae57'
tier: L2
related:
...
- [x] `P04.S35` - Reopen the 0611 registry-computation question because its closure rested on a false premise, the claim that 0613 cap never varies per child being a description of the un-prorated formula rather than of the law, since the manual prorates that cap by each child qualifying months so both casillas do share a rule shape, and decide the question again once the proration defect is fixed rather than citing the earlier reasoning; `src/cadrumo/_data/registry/aeat/modelos/100/revisions/2024/formulas/`.
...
- [x] `P04.S47` - Drop the message length cap from the TRANSIENT calculation diagnostic aligning it with the uncapped operator-facing notice it actually reaches, keep the cap on the PERSISTED source issue where it guards an encrypted record but make the projection truncate rather than raise so a persistence bound can never fail a calculation, and replace the two per-site length spot-checks with a static gate that walks the syntax tree resolving message expressions transitively through local assignments and asserts the literal floor plus a declared per-slot budget fits the cap, because the transient model inherited the persisted model cap while only unrouted-observation diagnostics carrying a binding source are ever persisted so both crashing advisories were refused by a constraint guarding a store they could never reach; `src/cadrumo/application/aggregation/_source_mesh.py, src/cadrumo/application/modelo/_calculation_actions.py, src/cadrumo/application/modelo/tests/`.
- [x] `P04.S48` - Stop the calculate boundary projecting an internal ValidationError to the generic CLI-validation refusal, because the operator is told to check arguments that are correct while the real cause reaches only the error log, which is the same defect S20 fixed for the descendiente add verb still live on calculate; `src/cadrumo/entrypoints/cli/, src/cadrumo/application/modelo/_calculate_input.py`.
- [x] `P04.S49` - Move the remedy detail out of the diagnostic message prose and onto the notice context, because a message carrying nearly five hundred characters of remedy text is what pushes these advisories against the cap in the first place, and the context field is structured, already the sanctioned home for provenance, and read by an agent operator rather than skimmed, so the durable reduction is contract-shaped rather than another round of prose trimming; `src/cadrumo/application/modelo/_calculate_input.py, src/cadrumo/application/modelo/_minimo_descendientes_advisory.py, src/cadrumo/core/json_contract.py`.
- [x] `P04.S50` - Promote M100/2024 casilla 0611 to the established derived-profile-scalar registry producer, removing the manual casilla-input path and fallback while preserving revision-specific legal oracles, diagnostics, provenance, and calculate/pull convergence; `src/cadrumo/application/modelo/_calculate_input.py, src/cadrumo/_data/registry/aeat/modelos/100/revisions/2024/, src/cadrumo/application/modelo/tests/, src/cadrumo/entrypoints/cli/tests/`.
