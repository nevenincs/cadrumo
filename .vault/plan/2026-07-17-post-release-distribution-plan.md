...
- [ ] `P01.S01` - Install from the intended bucket in Windows Sandbox and execute CLI, MCP, update, and persistence behavior (lifted from distribution W02.P04.S19). RE-SCOPED 2026-07-25 by the distribution-repo-topology ADR onto the in-repository bucket, Scoop resolves manifests from a bucket/ subdirectory so nevenincs/cadrumo is its own bucket and the private nevenincs/scoop-cadrumo is retired. The bucket is now PUBLIC because the product repo is public, so the former private-bucket precondition is GONE. BLOCKED only on Windows Sandbox enabled on the Windows host and on the S13 publish that writes bucket/cadrumo.json; `dev/packaging/smoke_scoop.ps1`.
...
- [ ] `P03.S16` - Acquire Cadrumo through the public Scoop bucket and repeat installed behavior (lifted from distribution W04.P10.S47) - TOOLING READY: dev/packaging/acquire_scoop.ps1 windows-container flow (ad6318eeb3, reviewed PASS), executes post-publication. RE-SCOPED 2026-07-25, the bucket source is now the public product repo nevenincs/cadrumo serving its own bucket/ directory, so no bucket repo has to be created and the private-repo blocker is GONE. BLOCKED on the S13 publish that promotes the manifest and on the docker Windows-container mode that blocks S02; `dev/packaging/acquire_scoop.ps1`.
- [ ] `P03.S17` - Acquire Cadrumo through the public Homebrew tap and repeat installed behavior (lifted from distribution W04.P10.S48) - TOOLING READY: dev/packaging/acquire_homebrew.py (ad6318eeb3, reviewed PASS), executes post-publication. RE-SCOPED 2026-07-25 onto the shared product-neutral account tap nevenincs/tap (repo nevenincs/homebrew-tap, var HOMEBREW_TAP_REPO), the private per-product nevenincs/homebrew-cadrumo is retired. BLOCKED on operator creation of that PUBLIC tap repo, on re-adding the renamed HOMEBREW_TAP_TOKEN secret, and on the S13 publish; `dev/packaging/acquire_homebrew.py`.
- [ ] `P03.S18` - Acquire the public marketplace plugin through Claude and repeat the MCP tax-work call (lifted from distribution W04.P10.S49) - TOOLING READY: dev/packaging/acquire_claude_plugin.py (ad6318eeb3, reviewed PASS), executes post-publication. PARTIALLY UNBLOCKED 2026-07-25, the dangling repo variable defect is FIXED, the target is the existing PUBLIC nevenincs/neve-marketplace under the renamed var CLAUDE_MARKETPLACE_REPO. BLOCKED on re-adding the renamed CLAUDE_MARKETPLACE_TOKEN secret, on the S13 publish, and on a one-time operator delete of the stale pre-rename plugins/aeat entry; `dev/packaging/acquire_claude_plugin.py`.
...
- [ ] `P03.S20` - Reacquire every public Claude artifact and verify its cadrumo- harness namespace and English and Spanish MCP product descriptions against the cohort manifest (lifted from distribution W04.P10.S70) - TOOLING READY: namespace and description capture built into acquire_claude_plugin.py and acquire_mcpb.py (ad6318eeb3, reviewed PASS), executes post-publication. PARTIALLY UNBLOCKED 2026-07-25, the missing public marketplace that blocked this is resolved with S18, the target is now the existing PUBLIC nevenincs/neve-marketplace. BLOCKED on the unpromoted cohort of S15; `dev/packaging/acquire_claude_plugin.py, dev/packaging/acquire_mcpb.py`.
...
- [ ] `P04.S23` - Document Claude Code, Desktop, and Cowork plugin and MCPB acquisition with real verification commands (lifted from distribution W05.P11.S54). PARTIALLY UNBLOCKED 2026-07-25, the missing public marketplace repo clause is resolved, CADRUMO_MARKETPLACE_REPO now names the existing PUBLIC nevenincs/neve-marketplace. BLOCKED on S18, S19, and S20 reacquisition evidence; `docs/how-to/connect-an-agent.md`.
