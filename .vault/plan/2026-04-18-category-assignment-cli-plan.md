...
- **File:** `src/aeat/financial/transactions/_service.py`
...
- **File:** `src/aeat/cli/financial/txs.py`
- **Action:** Add `category` option typed as `SpendingCategory | None = typer.Option(None, "--category", ...)` mapping to the `SpendingCategory` enum from `src/aeat/financial/categories/_spending_category.py`.
...
### 3. Update tests
- **File:** `src/aeat/financial/transactions/test_cli.py`
