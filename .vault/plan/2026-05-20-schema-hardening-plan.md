...
- [ ] `W09.P20.S143` - close Bucket B (repair_integrity backend gap): defer-to-other-campaign — investigation grounded against in-flight vault docs showed the four symbols (`RepairRemediationDecision`, `RepairRemediationDecisionRepository`, `repair_remediation_decision_id`, `build_repair_policy_command_surface_catalog`) are owned by the parallel `live-iva-compensation-wallet` campaign (`.vault/exec/2026-05-19-live-iva-compensation-wallet/2026-05-22-*w05-p02-s01.md` and siblings). Cross-campaign scaffolding from this branch would contaminate two parallel campaigns. The W09.P20 cross-module-import gate's baseline carries the 4 entries as known foreign-WIP-in-flight; the silent-fix detector will demand the baseline trim automatically once the owning campaign lands; `src/aeat/application/repair_integrity.py` (other-campaign-owned).
...
- [ ] `W09.P21.S142` - extend the vault-doctor gate (or add a new plan-format check) so future missing-`;` rows fail at `vault check all` rather than only at the next mutating CLI call; OUT OF SCOPE for the AEAT test suite — this is a vaultspec-core hygiene concern that belongs in vaultspec-core itself, not in aeat (tax-filing) code; the prior attempt to land it as `src/aeat/tests/test_vault_plan_format.py` was reverted as scope overreach. Re-scope: file the regression-gate request upstream against vaultspec-core's own test surface; `vaultspec-core upstream`.
