...
- [ ] `W09.P20.S144` - close Bucket C (CLI `_ledger` missing): restore the `_ledger` private module under `aeat.entrypoints.cli` (or rename the import to its successor) so `test_backend_boundary.py` collects; trim baseline; `src/aeat/entrypoints/cli/`.
- [ ] `W09.P20.S145` - close Bucket A-private (`_fetch_g313_census_with_storage_state` missing in `_censo_live`): land the storage-state-aware variant or refactor `test_censo_live.py` to use the public `fetch_g313_census`; trim baseline; `src/aeat/adapters/outbound/aeat/sede/_censo_live.py`.
