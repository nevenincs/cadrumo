...
- [ ] `W09.P20.S143` - close Bucket B (repair_integrity backend gap): land or relocate `RepairRemediationDecision`, `RepairRemediationDecisionRepository`, `repair_remediation_decision_id`, `build_repair_policy_command_surface_catalog` so the four tests in `test_runtime_migrated_repositories.py` and `test_repair_policy_coverage.py` collect cleanly; trim baseline; `src/aeat/application/repair_integrity.py`.
...
- [x] `W09.P21.S142` - extend the vault-doctor gate (or add a new plan-format check) so future missing-`;` rows fail at `vault check all` rather than only at the next mutating CLI call; landed as `src/aeat/tests/test_vault_plan_format.py` running `vaultspec_core.plan.parser.parse_plan` against every `.vault/plan/*.md` and asserting the live failing-plan set equals a committed `_PLANS_KNOWN_TO_FAIL_PARSING` allow-list. Catches both `PlanParseError` (missing `;`) and `PlanFrontmatterError` (e.g. unquoted ISO-date frontmatter parsed as `date` not `str`). 47 historical plans land in the allow-list as the W09.P21 close state; burndown tracked separately. Gate fails on new failing plans (regression) and on allow-list entries that now parse (silent fix demands trim); `.vault/plan/`.
