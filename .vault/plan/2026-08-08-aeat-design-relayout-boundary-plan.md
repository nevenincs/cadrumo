...
Neither record names the localization consequence and it is substantial. Modelo and casilla labels resolve through derived dotted locale keys carrying the revision id, so every new revision id mints a new key set that must exist with a real value in all four catalogues, with no sanctioned untranslated state. `W05.P10` carries one row per modelo for that work.
