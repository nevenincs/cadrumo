...
- [x] `P05.S20` - run the M130 registry suite, the formula-runtime suite, the cross-dependency contract and calculation suites, and the full registry test directory to confirm no regression across the campaign; `src/aeat/domain/calculations/registry/`.
...
- [x] `P06.S27` - fix cross-campaign drift: test_registry_schema.py::test_validator_allows_modelo_145_communication_link_for_non_filing_casillas fails with a registry validation error. Caused by parallel modelo-145 surface changes; `identify the validator delta and align test expectations or the registry data; `src/aeat/_data/registry/aeat/modelos/145/ and src/aeat/domain/calculations/registry/test_registry_schema.py`.
