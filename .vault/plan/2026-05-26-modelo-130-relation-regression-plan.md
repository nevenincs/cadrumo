...
- [ ] `P05.S20` - run the M130 registry suite, the formula-runtime suite, the cross-dependency contract and calculation suites, and the full registry test directory to confirm no regression across the campaign. Partial 2026-05-27: the M130 registry, formula-runtime, cross-dependency contract/calculation suites passed, and the declaration parser suite passed; `the full registry directory completed under `pytest -n 4` with 1,911 passed and 5 non-M130 failures that must be resolved before this gate closes; `src/aeat/domain/calculations/registry/`.
...
- [x] `P06.S27` - fix cross-campaign drift: test_registry_schema.py::test_validator_allows_modelo_145_communication_link_for_non_filing_casillas fails with a registry validation error. Reopened 2026-05-27: the full registry gate still fails because modelo 145 declaration-PDF extraction profile lacks either a committed specimen fixture or an explicit provisional-pending-specimen acknowledgement; `src/aeat/_data/registry/aeat/modelos/145/ and src/aeat/domain/calculations/registry/test_registry_schema.py`.
...
- [x] `P06.S29` - fix cross-campaign drift: test_tautology_gate.py::test_no_hand_summed_aggregation_tests_across_codebase flags hand-summed assertions in test_renta_income_aggregation.py and test_filed_capture_calculation_history.py. Rewrite the offending assertions to use the canonical aggregation operator or restructure them as graph-wiring checks; `src/aeat/application/aggregation/test_renta_income_aggregation.py and src/aeat/application/live/test_filed_capture_calculation_history.py`.
