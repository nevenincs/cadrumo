...
- [ ] Step W1.A.1: from a clean shell, run `aeat setup status` against an existing local profile and capture stdout, stderr, exit code.
- [ ] Step W1.A.2: from a clean shell, run `aeat app overview status` and capture stdout, stderr, exit code. Confirm the literal `INTEGRITY: AES-256-GCM tag verification failed` line appears.
- [ ] Step W1.A.3: from a clean shell, run `aeat app overview status --calendar` and capture stdout, stderr, exit code. Confirm the same INTEGRITY line.
- [ ] Step W1.A.4: from a clean shell, run `aeat app declaration calculate --modelo 303 --period 2026Q1` and capture the printed `draft_id`. Confirm the command succeeds.
- [ ] Step W1.A.5: from a clean shell, run `aeat app declaration status --modelo 303 --period 2026Q1` and confirm it resolves the same draft id.
- [ ] Step W1.A.6: from a clean shell, run `aeat app declaration review --modelo 303 --period 2026Q1` and capture the INTEGRITY failure.
- [ ] Step W1.A.7: from a clean shell, run `aeat config doctor` and capture its `secure_state.load` row verdict. Confirm it reports `ok` while step W1.A.2 reports failure.
- [ ] Step W1.A.8: enumerate every secure-object kind written by `src/aeat/adapters/persistence/storage/secure_objects` and `src/aeat/adapters/persistence/storage/sql`. Record the kinds.
- [ ] Step W1.A.9: by hand, attempt to read each kind's records using the same key derivation the writer uses. Record which kinds fail tag verification.
- [ ] Step W1.A.10: classify the failure as one of {key-derivation drift, partial-write race, schema migration with stale records, AAD context mismatch}.

### W1.B Backend localisation

- [ ] Step W1.B.1: open `src/aeat/adapters/persistence/storage/crypto/...` and identify the AES-256-GCM `encrypt` and `decrypt` functions.
- [ ] Step W1.B.2: confirm the `decrypt` function raises `IntegrityError` (or equivalent) with the literal `AES-256-GCM tag verification failed`. Locate the raise site.
- [ ] Step W1.B.3: trace every caller of `encrypt`. Confirm AAD (additional authenticated data), key derivation context, nonce policy are uniform across writers.
- [ ] Step W1.B.4: trace every caller of `decrypt`. Confirm the same AAD, key derivation context, nonce policy are applied on read.
- [ ] Step W1.B.5: run a probe that writes a fresh secure-object kind X, then reads it back in the same process. Record success.
- [ ] Step W1.B.6: run the same probe across two processes (writer terminates, reader starts fresh). Record success or failure.
- [ ] Step W1.B.7: identify which secure-object kind is written by `aeat setup status` / `aeat app overview status` read paths. Confirm whether the kind is computed-on-read or persisted-on-write.

### W1.C Backend repair

- [ ] Step W1.C.1: if step W1.B.7 found a key-derivation drift, fix the writer to derive the key using the same context the reader applies.
- [ ] Step W1.C.2: if step W1.B.7 found AAD mismatch, fix the AAD on whichever side disagrees with the canonical contract; document the canonical contract in the module docstring of the secure-objects layer.
- [ ] Step W1.C.3: if step W1.B.7 found stale records from a prior schema, add an idempotent forward migration in `src/aeat/adapters/persistence/storage/sql` that re-encrypts records under the current key derivation.
- [ ] Step W1.C.4: if step W1.B.7 found a partial-write race, make the writer transactional: write to a staging row, fsync, atomically rename / commit, only then expose to readers.
- [ ] Step W1.C.5: extend the `secure_state.load` diagnostic in `src/aeat/application/diagnostics.py` to actually exercise the same code path that `aeat app overview status` triggers. Today it returns `ok` while the user-facing command crashes; the divergence is a defect.
- [ ] Step W1.C.6: introduce one shared function `verify_secure_object_kinds(profile)` in `src/aeat/application/diagnostics.py` that enumerates every kind, attempts a representative read, and returns a typed result; both `config doctor` and `overview status` consume it.
- [ ] Step W1.C.7: confirm no transient metadata is written into the source code by these edits (no dates, no "phase" markers, no vault paths).
