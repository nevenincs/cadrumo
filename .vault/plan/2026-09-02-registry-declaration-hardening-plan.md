...
modified: '2026-09-07'
body_schema: body-v2
body_hash: 'sha256:7c048e8d7b5e61b0ad880a86c406f4abe2c1ee7c896bde93d25f32a1ed1e448d'
---
...
- [x] `W01.P01.S851` - Skip an unreadable construction site in the workspace field-population scan instead of crashing on it, following the module's own direction-of-error argument that a file another lane is rewriting is not evidence about fields and skipping it can only over-report, never under-report; HEAD raised PermissionError on a path its own walk had listed; `dev/quality/workspace_field_population_scan.py dev/tests/test_workspace_field_population_gate.py`.
- [x] `W01.P01.S852` - Report rather than drop a test module the default-lane visibility screen could not read or parse: the screen's findings list is its only output and its emptiness reads as full visibility, so a dropped module is indistinguishable from one the default lane selects; HEAD crashed on the unreadable shape and silently lost the half-written one, and both now surface as an `unread` row in the census and the summary tally; `dev/quality/default_lane_visibility.py dev/quality/tests/test_default_lane_visibility.py`.
- [x] `W01.P01.S853` - Return the unread set from the import-centralization codemod's planning walk and print one row per file, so the codemod's `Collected N statements across M files` line stops being a coverage figure over a tree it never finished reading; the planning phase precedes every write, so the omission was silent rather than destructive; `dev/quality/import_centralization_codemod.py`.
- [x] `W01.P01.S854` - Read the workflow tool-invocation contract off the executable `run:` lines yaml.safe_load yields rather than off the whole file's bytes: commenting out all six `uv run --no-sync python -m dev.*` invocations leaves the substring assertion green while no tool runs, and a sibling gate already parses that exact file because it deliberately inspects executable fields rather than comments; `dev/ci/tests/test_workflow_tool_invocation.py`.
- [x] `W01.P01.S855` - Prove the workbook-parity module carries its external-tool marker by reading the module-level `pytestmark` assignment from the syntax tree rather than by substring: the claim is that the marker holds the module out of the offline unit lane, and a comment or docstring naming the marker satisfied the text reading while the marker itself was gone; `dev/ci/tests/test_ci_workflow.py`.
- [x] `W01.P01.S856` - Refuse a module the duplication corpus could not READ through the same channel it already refuses one it could not PARSE, and keep the two refusals distinguishable: the screen's own docstring says a short corpus and a clean one look identical downstream, so a raw PermissionError traceback named neither the loss nor its size; the messages are split rather than merged because one shared message makes any `pytest.raises(match=...)` satisfiable by the wrong cause, proven by a third gate asserting each message carries its own fragment and not the other's; `dev/audit/semantic_duplication.py dev/audit/tests/test_duplication.py`.
- [x] `W01.P01.S857` - Adjudicate the six surviving read-failure sites as honest and close the taxonomy: no handler in any of the seven owning files catches OSError, Exception, BaseException or bare, so an unreadable file raises out of the test and lands as a red error rather than a false green, and four of them additionally collect the parse axis into a named channel with a written direction-of-error rationale; the census closes exactly at sixteen candidates, ten fixed and six honest; `dev/quality/tests/test_no_dunder_init_module_imports.py dev/tests/test_canonical_definitions_stay_singular.py dev/tests/test_modelo_workspace_fixed_point.py dev/tests/test_no_casilla_is_routed_to_a_valueless_slot.py dev/tests/test_repository_root_constants_resolve.py dev/tests/test_text_writer_newline_pinning.py`.
- [x] `W01.P01.S858` - Tighten both tier refusal patterns so each names the branch it drives: `require_model_tier`'s two refusals both say `model_tier` and both interpolate the whole accepted tier set, so `(?i)model_tier|tier` and `on_host_small` each matched BOTH messages at runtime and neither test could say which branch fired -- deleting the absent-value guard leaves ModelTier(None) raising the unknown-tier refusal with the missing-tier test still green; a third gate now pins each fragment present in its own message and absent from the other, and records the two shared fragments that made the loose patterns pass; `dev/ingest_harness/tests/test_harness_contracts.py`.
- [x] `W01.P01.S859` - Invalidate the conformance snapshot cache from every write path that lands on the bundled registry tree: `_cached_profile` and `_cached_locale_index` are lru_cached on `validate: bool` alone -- zero registry-source input -- while serving a read of the whole bundled tree, and only one of the seven write paths called `reset_conformance_cache()`; non-causation for the two failures in the confirmation batch is proven rather than inferred, since neither traceback touches conformance/manager.py or any reset call site; `dev/registry/newmodelo/manager.py dev/registry/pipeline/_tree_publication.py dev/registry/analysis/m200_2024_adjudication_publication.py dev/registry/analysis/m200_2024_full_reconciliation.py dev/registry/analysis/m303_orden_anual.py dev/registry/result_disposition_fragment_generator.py`.
- [x] `W01.P01.S860` - Move the export staging sibling out of the revision directory and verify recovery after the move rather than before: a `.export.generated-stage-<hash>` directory parked beside `export/` inside `revisions/<rev>/` is litter the loader has no owned-fragment name for, so its provenance JSON tripped the revision-fragment suffix validator and made the whole revision unloadable; with the litter gone a second defect surfaced, two recovery branches reading `revision.export_layouts` off disk BEFORE moving the candidate into place, which can never succeed by construction because the read only reflects the canonical export path; `dev/registry/pipeline/_tree_publication.py dev/registry/tests/test_generated_tree_publication.py`.
- [x] `W01.P01.S861` - Floor committed sequence enrolment in the test layer where the repo already states that idiom, since a total collapse to zero directives reads as clean on every surface: `discover_sequences` floors an addressed miss in both directions but the unscoped sweep is silent, so `problems == ()` stays green over zero enrolment and the refresh entry point prints `nothing to refresh` and returns 0; the engine stays neutral because it legitimately runs over scaffolded trees carrying no directives, and renaming the fence token `cli-sequence` is the retired-vocabulary degradation the old reading cannot see; `dev/docs/sequences/tests/test_committed_enrolment.py`.
- [x] `W01.P01.S862` - Give each of `build_result_row`'s three refusals a pattern that names the branch it drives: all three shipped patterns were ambiguous under a runtime drive, and `(?i)authored|scorable` matched ALL THREE, so the test carrying it proved only that the function refused something; the fragments were drawn from the shared subject -- every message talks about authored truth -- so the discriminating words are the verbs, and a 3x3 cross-check gate now asserts each fragment present in its own message and absent from the other two, recording that `authored` is in all three while `denominator` is in exactly the two the narrower-looking pattern conflated; `dev/ingest_harness/tests/test_harness_contracts.py`.
- [x] `W01.P01.S863` - Drive the refusal the whole object-name rehearsal exists to evaluate: coverage showed 33 of the module's 50 raise sites are never reached by its own 35-test suite, and among them was `rehearsal introduces an enforced object-name finding` -- the single comparison every preceding step (isolated copy, bounded transform, re-scan) is machinery for reaching; the gate appends one real enforced finding to the result of the REAL `_inventory_after_allowed_changes` so the real delta and the real comparison decide, and its pattern spans the retained-root suffix the outer handler appends rather than pinning the original message; `dev/quality/tests/test_object_name_rehearsal.py`.
- [x] `W01.P01.S864` - Drive the copy-integrity refusal the rehearsal's every later step depends on: if the temporary copy does not carry the selected component's own bytes then every gate, digest and receipt below it describes a different file than the one under review, and no test reached that comparison; the gate corrupts the GUARDED component inside the copy where the sibling race test corrupts an unguarded path and lands on a different refusal much later, and removing the real refusal reddens the new gate while leaving the sibling green, so the two are not redundant; `dev/quality/tests/test_object_name_rehearsal.py`.
- [x] `W01.P01.S865` - Adjudicate the registry ordering sweep a clean negative on a structural exemption: only a file that MUTATES can host a check ordered before the mutation it verifies, which cuts 201 files to the 13 that write, all read in full; the look-alike is `conformance/_stamp.py` asserting a revision is compiled AFTER its write_bytes, which is honest because `load_modelo_directory` keys its cache on a freshly recomputed per-call fingerprint and so invalidates itself, and the round-sixteen swap-then-verify fix is now confirmed by a real run rather than by inspection; `dev/registry/pipeline/_tree_publication.py dev/registry/conformance/_stamp.py`.
- [x] `W01.P01.S866` - Read the executable content of workflow and justfile scripts through one canonical `executed_lines` helper rather than raw bytes, closing five gates that a shell comment satisfied: a commented-out eval command left the harness-eval boundary green while nothing ran, a hand-copied loop under a note naming the watchdog module satisfied a raw substring, two workflows resolve to exactly ONE pytest line each so commenting it kept `assert lines` green with zero pytest running, `dist/*.whl` tokens were found inside a commented `uv publish`, and three change-class readings were satisfied by commented text in a trigger file carrying 24 comment lines above 6 of command; the comment rule is the shell's own and no wider, so a `#` inside `--body 'see #412'` survives, and absence assertions were deliberately left raw because stripping comments weakens rather than strengthens them; `dev/ci/workflow_run_text.py dev/ci/tests/test_workflow_run_text.py dev/ci/tests/test_runner_queue_watchdog.py dev/ci/tests/test_machine_aware_load.py dev/ci/tests/test_change_class_tiers.py dev/ci/tests/test_action_pinning.py dev/release/tests/test_external_client_release_boundary.py`.
- [x] `W01.P01.S867` - Drive the residual-identity refusal for a PDF-shaped artefact that will not decompress: the module's own comment argues the case, since identities inside Flate streams are invisible on the raw surface so absorbing would return an empty tuple a caller cannot tell apart from a clean specimen; coverage showed the except and the if executed while the raise never did, so the absorb path for headerless images was driven and the refusal was not, and the specimen is one checksum-valid NIF compressed into a Flate stream that scans clean without the PDF marker and refuses with it; `dev/sanitizer/tests/test_undecompressable_pdf_refusal.py`.
- [x] `W01.P01.S868` - Refuse an incoherent persisted golden frame through its real model validator: the stream-coherence body ran on every golden and never on a violating one, so a frame describing two different runs would be committed and replayed forever with the incoherent half never compared; the gate drives both directions of the envelope/source pairing that a one-sided guard misses plus each stream credited with the envelope while storing raw content, and anchors its match on the pydantic-wrapped message rather than the inner ValueError; `dev/docs/sequences/tests/test_golden_frame_stream_coherence.py`.
- [x] `W01.P01.S869` - Drive the object-name rehearsal's three unexercised safety refusals and record the scope's refusal population: 50 raise sites with only 17 executed by the module's own 35-test suite, and across the orchestrator scope 405 raise sites in 64 production modules of which the six object-name modules carry 232, so the density is a property of that cluster rather than of one module; the introduced-finding refusal, the copy-integrity refusal and the occupied-output-temp refusal now each have two-sided teeth, and the occupied path is derived from the real planner's own outputs rather than a guessed filename; `dev/quality/tests/test_object_name_rehearsal.py`.
...
- [x] `W02.P02.S718` - Widen the unbounded-child-wait screen to every Popen binding form so its absence claim covers the spellings it could not parse; `dev/tests/test_no_unbounded_subprocess_wait.py`.
- [x] `W02.P02.S719` - Read bare callees in the M200 identity CLI no-write absence claim so its forbidden builtin spellings become detectable, with a floor per callee shape; `dev/registry/tests/test_m200_semantic_casilla_candidates.py`.
...
- [x] `W06.P13.S880` - Route an unreadable module into the repository-root constant scan's existing unread channel instead of ending the run in a traceback: the scan already announces an unparsed module because a wrong parents[N] inside it is never resolved and never reported, and its own comment sizes the stake at forty-seven constants found against a floor of thirty, so seventeen could vanish before anything fired; an unreadable module loses the same constant by a different route and now reports through the same notice; `dev/tests/test_repository_root_constants_resolve.py`.
- [x] `W06.P13.S881` - Route an unreadable mapping fragment into the valueless-slot gate's existing unread channel: the gate already announces a fragment that does not parse because every export ref into it then reads as kind None and is skipped as a hand-authored layout rather than judged - a silent skip converting judged routings into unjudged ones, indistinguishable from a clean result in its own words - and an unreadable fragment shortens the corpus identically, so it now announces instead of ending the run before any routing is judged; `dev/tests/test_no_casilla_is_routed_to_a_valueless_slot.py`.
...
- [x] `W06.P13.S886` - Correct a reachability claim that describes a state which cannot exist: the replay allowlist refusal was documented as reachable when the rehearsal agrees with the receipt while the manifest's reviewed set does not, but the rehearsal derives the receipt's changed paths as the sorted union over the component's operations and replay recomputes its allowlist as that same union over a manifest and component already pinned by digest and identity, so the two sides are one derivation and cannot disagree; the comment now states the unreachability and names the post-apply sibling that IS reachable because it compares paths actually written against the receipt; `dev/quality/tests/test_object_name_replay.py`.
- [x] `W06.P13.S887` - Rename two wall-advisory gates whose names claimed what their bodies disprove: one asserted equality against a two-name frozenset while four thresholds are pinned, so the gate was green precisely because the cold-start pair has no consumer and would redden the moment its own name became true, and its sibling claimed a site still catches a wedged spawn while driving only the constants because that emitting site is gone; both names corrected to what the bodies prove and neither body widened, since widening would silently change what the gates do - the absent advisory site is reported as a behaviour change, not folded into a naming fix; `dev/ci/tests/test_wall_advisory.py`.
- [x] `W06.P13.S888` - Give the display-class weight gate a second root: it compared a mapping rebuilt by iterating ResultDisplayClass against set(ResultDisplayClass), so both sides carried the enum unconditionally and the declared weight table the test is named for appeared on neither, leaving the assertion true for every table that could exist; the subject now reads off the declared table, so narrowing the enum strands a table key instead of moving both sides together and a table key that is no display class reds on sight; `dev/docs/terminology/tests/test_display_class_ranking_ladder.py`.
- [x] `W06.P13.S889` - Replace a vacuous justification in the distribution-claims gate: its docstring anchored the teeth on keeping coverage for the channels a release does NOT claim, but the descriptor model has no claimed or unclaimed notion at all - the two row constants are the same object and DownloadChannel forbids extra fields - so the stated protection could never apply; the reasoning now rests on the claim patterns and the descriptor TOML being independent roots, where a row renamed in the descriptor moves only one side; `dev/docs/tests/test_distribution_claims.py`.
- [x] `W06.P13.S890` - Remove a documented safety mechanism that does not exist from the download-matrix module docstring: it described an availability and public-launch gating under which the generator withholds install commands so the page never advertises a channel ahead of its passing distribution evidence, but no such field exists on the channel model, none appears in the descriptor, the rendered strings are gettext-obsolete, and public_launch has no occurrence anywhere in the tree; replaced with the mechanism that actually holds - unprovable channels are removed from the descriptor and the claims gate enforces the property - and a stale render-zone column description corrected; `dev/docs/download_matrix.py`.
- [x] `W06.P13.S891` - Correct a code-health docstring that denies a state its function returns: audit_layering claimed there is no AMBER state because a layering contract is a hard boundary, but it returns AMBER on three reachable paths - lint-imports missing from PATH, the runner timing out, and the runner raising OSError - all of which are the module's own stated convention that a scan which could not run is AMBER-unavailable and never GREEN; the docstring now separates the verdict axis, which genuinely carries no AMBER, from the availability axis, which does; `dev/audit/report.py`.
- [x] `W06.P13.S892` - Give the layering availability contract its first test: audit_layering was reachable only from the aggregator and no test drove it, which is how its docstring came to deny the state it returns, and audit_shadowing and audit_complexity are equally undriven; the new case pins AMBER-unavailable through a REAL failure rather than a substituted one, a working directory that does not exist making the production subprocess raise OSError, so the claim carried is that the shipped path degrades rather than that a stub does; `dev/audit/tests/test_advisory_dimensions_scan.py`.
- [x] `W06.P13.S893` - Carry a retirement across to the dimension it also silenced: audit_complexity's docstring and the module's Complexity bullet both advertise AMBER as baselined grandfathered debt, but load_baseline returns an empty baseline unconditionally now that the committed baseline and reviewed allowlist were retired, so allowed is always zero and that branch is unreachable - a fact already documented for the Shadowing dimension and never carried to Complexity, leaving a reader expecting a debt tier the audit can no longer report; `dev/audit/report.py`.
- [x] `W06.P13.S894` - Join the corpus sidecar floors and the prorrata language probes to the populations they stand for: each roster drove its own loop, so the committed gate passed all sixty-four combinations of roster subset and population shape - the empty roster included, which checks nothing and still reports a corpus reaching every family in it - and the language probe table is the one place a human must hand-author a term while build, catalogue and switcher all pick a new language up from the enum; `dev/docs/preprocess/tests/test_corpus_sidecar_freshness.py,dev/docs/tests/test_prorrata_smoke_gate.py`.
- [x] `W06.P13.S895` - Retract a promised enforcement the roundtrip-coverage gate cannot make: its docstring said new boundaries must be registered alongside a roundtrip test or the gate will fail, but the loop walks the inventory, so it catches a declared boundary whose test file has gone and never a real boundary that was never declared - twenty-five of the twenty-six entries can be dropped and the gate stays green, the non-empty guard stopping only the fully-empty case; the wording now states which half is covered and records why the other half is named rather than approximated, since the entries span repositories, sinks, a fixed-width codec and session stores with no predicate separating them from their neighbours; `dev/tests/test_roundtrip_coverage.py`.
- [x] `W06.P13.S896` - Remove three live counts that had all gone stale while still reading as current, from the comment that justifies the workspace-assembly census floors: the stated 6931 .py, 20495 .toml and 1769 .rst re-derive today as 6994, 20496 and 1830, so a reader calibrating a floor against them was calibrating against fiction; the durable claim - each floor near three fifths of its own population, stated per suffix because a single total is dominated by .toml - is kept and the counts that drift with every commit are not restated, since correcting them would only restart the clock; `dev/tests/test_workspace_assembly_forbidden_paths.py`.
- [x] `W06.P13.S897` - Retire two more stale live counts whose arguments survive without them: the canonical-definition gate justified refusing an unparsed module by saying all 2,121 production modules parse, which re-derives as 2,136 today, and the dunder-init scan sized its post-swallow floor at 6,906 modules against 1,000, which re-derives as 6,993; both drifted unnoticed while reading as current, so each now carries the claim that survives re-derivation - every production module parses, and the floor is weak by roughly seven to one - with the exact figures recorded as evidence that counts do not keep in prose rather than as facts to maintain; `dev/tests/test_canonical_definitions_stay_singular.py,dev/quality/tests/test_no_dunder_init_module_imports.py`.
- [x] `W06.P13.S898` - Repair three consumers left pointing at a symbol's old home after the export-tree split: test_generated_tree_check and test_generated_tree_cli died on ImportError before collecting a single case and test_generated_tree_publication carried the same broken import, so a size-driven relocation that reconciled its origin and destination silently removed three modules from the suite; each now imports from the defining module rather than through a re-export, which the architecture rules forbid, and collection recovers from three errors to fifty-one cases; `dev/registry/tests/test_generated_tree_check.py,dev/registry/tests/test_generated_tree_cli.py,dev/registry/tests/test_generated_tree_publication.py`.
- [x] `W06.P13.S899` - Drop a drifted count from the facade-retirement argument for landing the stronger initialiser gate: it read that all 63 initialisers under dev satisfy the property today, and 67 live there now, so the sentence justifying a gate as landable rested on a figure that had gone quietly false; the universal claim is kept and re-derived, and no count is restated because the claim is universal and a number adds nothing it does not also put at risk of drifting; `dev/quality/tests/test_facade_retirement.py`.
- [x] `W06.P13.S900` - Establish that a CLI refusal gate now proves a shallower claim than its name: a bootstrap target-selection guard added thirty-three minutes after the test refuses an unknown source before the record-design check is reached, so the gate still refuses before staging but no longer exercises the record-design-source mismatch branch it was written for; both messages exist at distinct production sites, the ordering is proven by git log -S dates and a merge-base ancestry check plus a code-path trace through the unconditional prepare step, and the expected fragment is deliberately left untouched because matching it to the new message would turn the test green while dropping that branch's only CLI-level coverage; `dev/registry/tests/test_generated_tree_cli.py,dev/registry/pipeline/cli.py,dev/registry/pipeline/render_check.py`.
- [x] `W06.P13.S901` - Floor the text-writer newline census per source tree, because a single total could not see one tree leave: the walk reaches 2998 modules against a total floor of 2500, and entrypoints at 462 fits inside that headroom along with dev, adapters, core, tests and llm - entrypoints carrying two of the four writers this gate reports today, so the subtree holding half its offenders could stop being walked while the total stayed green; floors are keyed independently of the walk so a tree that drops out keeps its floor and reds at zero, and the stale live total in the neighbouring comment is removed rather than reset; `dev/tests/test_text_writer_newline_pinning.py`.
- [x] `W06.P13.S902` - Floor the valueless-slot gate per modelo family, because its anti-vacuity totals cannot see a family leave: the walk indexes 21,199 map entries against a floor of 1,000 and resolves 10,165 refs against 500, so all seventeen families fit inside the slack and dropping m200 alone - 13,509 entries, most of the corpus - still leaves the entry total at 7,690, comfortably clear; a modelo whose maps stop being read contributes no entries, every export ref into it then reads as a hand-authored layout and its casillas are never judged, which is the same silent conversion of judged routings into unjudged ones the unread announcement guards against arriving by another route; the existing totals are kept because their comment claims only anti-vacuity and is honest, and the residual limit is stated rather than implied - a family shrunk to a handful of entries still counts as present; `dev/tests/test_no_casilla_is_routed_to_a_valueless_slot.py`.
- [x] `W06.P13.S903` - Make each search-record kind earn its place on the excluded or included side of the display-class agreement check, replacing a floor that was a constant equal to the authoring-time survivor count and so was satisfied entirely by the exclusions' complement being frozen: an excluded kind must now derive more than one class across contexts and an included kind exactly one, which also proves the single-context agreement call is representative rather than arbitrary, and the kind map is pinned against the enum by equality so a sixth kind added to both the enum and the exclusions can no longer sit unmapped and unchecked; `dev/docs/terminology/tests/test_unified_record.py`.
- [x] `W06.P13.S904` - Floor the governance-corpus isolation scan per product package, because its module total cannot see one package leave: the walk reaches 5,907 modules against a floor of 4,000 and every package fits inside the 1,907 slack, so dropping application alone leaves 4,033 and the total still clears while the largest body of code in the product goes unread and the one-way boundary reads absolute; a package that stops being scanned yields no offender because none was looked for, and the floors are keyed independently of the walk so such a package keeps its floor and reds at zero; `dev/tests/test_governance_corpus_isolation.py`.
- [x] `W06.P13.S905` - Floor test-module discovery per root, closing the partial case of the failure the gate was written for: its docstring says a mistake in discover_test_roots would hand the collector an empty target list and present as a clean gate over nothing, but the plausibility floor of 500 only catches a TOTAL collapse - src carries 3,420 of 3,964 tracked test modules, so losing that root entirely leaves 544 and the floor still passes while the great bulk of the corpus goes uncollected without one error being reported; floors are keyed independently of discovery so a root that stops being walked reds at zero, and packaging carries too few modules to floor meaningfully and deliberately has none; `dev/tests/test_every_test_module_is_collectable.py`.
- [x] `W06.P13.S906` - Restore CLI-level coverage of the record-design-mismatch refusal and split it from the bootstrap guard that had been intercepting it: the original case asserted text that never appears on the path it ran, staying green because a source-existence check fired first and its message was never compared, so the branch it named was covered by nothing; a catalogued source declared for the revision but carrying manual_pdf rather than record_design now drives the intended site, against a modelo whose export tree already exists so the bootstrap branch is skipped, and the traceback shows the raise originating at the render check rather than the earlier guard; `dev/registry/tests/test_generated_tree_cli.py`.
- [x] `W06.P13.S907` - Break a closed loop in the apidocs alias tables, where the drift check compared stubs against output the same table produced and the canonical-target test asserted back the line the table had just written: one entry claimed a package exposes a name its inert initialiser does not resolve, and because the directive is manual the anchor materialised anyway while the module actually defining the alias carried none, inverting the table's one-canonical-target purpose; both tables now prove their warrant against the imported module object, which cannot be edited into agreement, and the stubs were regenerated through the generator rather than hand-edited; `dev/docs/apidocs/manager.py,dev/docs/apidocs/tests/test_manager.py`.
- [x] `W06.P13.S908` - Reach the BOE acquirer's in-force refusals, which no test had driven because the module's bundled payloads are the wrong shape for them: coverage shows ten of seventeen raise sites unexercised, and four of those sit in one function whose matchers key on envelope, bloque and version markers that an act.php page does not carry, while sixty bundled redacciones payloads do; the new cases stay inside the module's stated discipline of real payloads over synthetic markup by asking a genuine document for a block it does not describe, changing exactly one token of it, and pinning the unmutated happy path so the refusals prove acceptance of the shape BOE serves rather than only rejection; `dev/tests/test_fetch_boe_normative.py`.
- [x] `W06.P13.S909` - Correct two justifications that were false when written rather than drifted, each checked by counting at its introducing commit: a workflow gate described five sibling checks as prohibitions when two were always positive universals, and a pair of sites claimed the justfile is the sole declaration site for every dev lane when the very commit asserting that left two inline selections in the workflow, one of which survives and has since diverged from the recipe it duplicates; both corrected to the narrower claims that hold, with the surviving duplication named as the counterexample; `dev/ci/tests/test_change_class_tiers.py,dev/ci/tests/test_ci_workflow.py`.
- [x] `W06.P13.S910` - Pin the three refusals the semantic-duplication loader raises, which coverage showed were added, proved once by hand, and never driven: an empty corpus would let every detector report clean, and an unreadable or unparsable module would leave the corpus short by exactly the module nobody can analyse, yet all three sites were unexercised in a module that also has no justfile recipe and no workflow lane; the read and parse refusals are asserted on their distinct wordings, which the module raises separately so a caller matching one cause cannot be satisfied by the other, and the unreadable case uses a directory named as a module so it needs no symlink privilege; `dev/audit/tests/test_semantic_duplication.py`.
- [x] `W06.P13.S911` - Retarget two adjudication refusal tests that were reaching a different guard than the one they name, both masked by a regex alternation admitting the fixture might land on either: a bare casilla number always raised at the record-page resolver and never reached the section-contradiction guard the test describes, and a membership test always raised at the closed-membership check called first and never reached the withheld-candidate guard; each fixture now genuinely reaches its named site with a tight match, and each branch the old fixture had been accidentally covering keeps an honestly-named sibling test so the repair loses no coverage; `dev/registry/tests/test_m200_2024_page_resolved_adjudications.py,dev/registry/tests/test_m200_2024_unique_adjudications.py`.
- [x] `W06.P13.S912` - Give the casilla reference gates a root outside the renderer's own report, replacing a check whose two sides were the same value: the page's grounding inventory was the input echoed back, so the gate asserted a set is a subset of itself and held for every input forever, and the anchor gate compared a projection-derived slug against the slug the renderer had just reported deriving; stamping anchors that resolve to nothing and emitting no grounding markup at all both left the suite green, so the inventory is now read back out of the emitted markup beside the existing self-consistency checks, which stay because a projection and renderer disagreeing about the slug is a real failure worth catching; `dev/docs/casilla_reference.py,dev/docs/tests/test_casilla_anchor_parity.py`.
- [x] `W06.P13.S913` - Give the ingest harness's decimal sanity line teeth and pin a reference-point invariant that held only by nobody having broken it: the sanity check is printed in every report header as evidence that a cent tolerance means a cent, and its own docstring says a failure would mean every amount figure above it is untrustworthy, yet coverage showed both its refusals unexercised so the reassurance was unfalsifiable - degrading the probe to binary floats now drives the real failure shape; separately a validator refusing a matched count above its denominator was dark while the tier and caveat refusals beside it were driven, and reference points reach full statement coverage; `dev/ingest_harness/tests/test_harness_contracts.py`.
- [x] `W06.P13.S914` - Drive the tabular-truth refusal at the two call sites where it was assumed rather than proven: the same guard is raised by three functions, only the emission lookup was driven, so the truth-document lookup and the defensible-alternates lookup relied on nobody having asked for an unauthored document; the document lookup also refuses before its corpus key is touched, and that ordering is itself the claim, since an unauthored id must be refused rather than sent to the corpus to fail there, while returning empty alternates would read as no defensible alternate rather than as no authored truth; `dev/ingest_harness/tests/test_tabular_truth.py`.
- [x] `W06.P13.S915` - Collapse a release distribution roster that was written twice with nothing forcing agreement, and quantify a workflow duplication rather than estimating it: the cohort names appeared in four places of which one was pinned against the published pyproject set, proven by typing the unforced copy wrong and watching every test stay green while the same typo in the pinned copy reds; separately the dev-CI recipe's serial pass has no workflow counterpart at all, measured at two tests by collecting the serial marker over the four directories, correcting an earlier claim that two documentation paths were lost when other recipes cover them; `dev/release/tests/test_justfile_release_guidance.py`.
- [x] `W06.P13.S916` - Pin the stream a scaffold CLI refusal is written to, closing a hole that an accommodating assertion had been hiding: the test accepted the error text on either stdout or stderr, but the sole emission site writes unconditionally to stderr, so the stdout branch was dead and the width meant the CLI could start leaking diagnostics onto stdout - breaking any pipeline that separates the streams - while the test stayed green, which was demonstrated by flipping the emission and watching the old assertion pass; the exit code, an empty stdout and the stderr content are now all asserted, and no sibling was needed because a single raise and echo site leaves no second path the width could have been covering; `dev/registry/newmodelo/tests/test_cli.py`.
- [x] `W06.P13.S917` - Count the glossary entries the emitted page actually carries, rather than trusting the renderer's own tally: the rendered counter is incremented in the same loop that appends the entry, so report and page are one act, and the gate compared that counter against the approved concept set while nothing anywhere read the page - dropping the single approved concept that carries neither a legal link nor a rendered term reference left every gate green, because the two assertions that do read the page could not see that member; the emitted entry runs are now counted and compared against the reported figure with a vacuity floor, added beside the report-versus-source check rather than replacing it; `dev/docs/tests/test_glossary_reference.py`.
...
- [x] `W06.P13.S922` - Surface the command refusal that blocks the modelo 200 under-declaration golden gate, which handed a refusal envelope straight to the schema decoder: an error document carries no result key, so the decoder reported a shape mismatch and the CLI's own code and message never reached the operator, leaving a legal-soundness gate failing for a reason nobody could read from the failure; the gate now reports that the 2024 revision declares calculation authority grade and so cannot satisfy the filing-grade snapshot the verify path requests, which is why the advisory it exists to prove is never reached; `dev/agent_eval/tests/test_under_declaration_golden.py`.
- [x] `W06.P13.S923` - Gate the two temp reaper branches its owning test module did not reach, after a duplicate module written on a false premise was withdrawn: a session root that exists but cannot be scanned must raise, because returning no verdicts there prints the same spared-none summary a machine with nothing to reclaim prints and a failed scan would reach the operator as good news, and a partial removal must report none of the bytes it did not free, since the absorbed removal failure is deliberate but the total is shown as removed and may only under-report; both proven by injection against the real module, held open by a live file handle rather than simulated; `dev/tests/test_temp_reaper.py`.
- [x] `W06.P13.S924` - Retire the phantom sibling distribution premise from the acquisition and smoke surfaces, where four sites explained a command-line-only install by a companion harness distribution that does not exist: the packaging declaration packs both packages into the single product wheel and declares both console scripts on it, so the scoop conclusion was right for the wrong reason and now carries the real one, the private virtual environment landing both scripts while the manifest shims only the command-line entry point; `dev/packaging/scoop/acquire_scoop.ps1`.
- [x] `W06.P13.S925` - Correct the homebrew acquisition driver's claim that the formula installs a command-line surface only, a half-applied edit whose broken grammar fused the old assertion to the replacement: the formula links every console script the distribution declares, so it is the lane and not the formula that is limited, matching the sibling smoke driver corrected in the previous round; `dev/packaging/acquire_homebrew.py`.
- [x] `W06.P13.S926` - Correct two further declaration contradictions found by the same sweep: an asynchronous runtime described as belonging to an optional extra that the packaging declaration deliberately leaves absent, making the carve-out entry that named it permanently inert, and a base image module claiming every container derives from one Linux image when the container definition gives the runner stage its own build argument, a carve-out a sibling gate and a sibling module both already stated correctly; `dev/packaging/_smoke_common.py`.
- [x] `W06.P13.S927` - Pin the output channel as well as the content in the generated-tree refusal gates, which asserted only that the refusal text appeared: routing refusals to standard output instead of standard error left both tests passing at exit zero, so the tests now require an empty standard output and find the message on standard error; `dev/registry/tests/test_generated_tree_cli.py`.
- [x] `W06.P13.S928` - Pin the machine-readable payload to standard output in the parity maintenance gates, where every payload passes through a single emit with no channel argument and the contract was held by convention alone: sending the whole payload to standard error left all eight tests passing, meaning a consumer piping the output to a parser could have broken with nothing in the suite noticing; `dev/registry/parity/tests/test_maintenance_cli.py`.
- [x] `W06.P13.S929` - Pin the single-stream contract of the registry closure report, whose plain-text branch had no channel assertion despite never using the error channel in production, so diverting it left the offline gate green; all four command tests now require an empty error channel and locate their content on standard output; `dev/registry/conformance/tests/test_closure.py`.
- [x] `W06.P13.S930` - Repair the installed-oracle cohort module, whose module-scoped fixture read a wheel attribute that the cohort dataclass has never declared, so the fixture raised on every run and four of its five tests were unrunnable while reporting only bare downstream errors naming nothing; the phantom sibling distribution was carried there by nine further sites across prose, two dead constants, embedded probe source, five dictionary keys, a dataclass field, an install argument and a digest key, and one assertion actively required the opposite of what the packaging declaration states about the server console script, so the removed teeth are replaced by a positive proof that the single wheel carries both source packages and a negative that no separate requirement is needed to obtain either command; `dev/packaging/tests/test_installed_oracles.py`.
- [x] `W06.P13.S931` - Correct the product wheel accessor's return annotation, which declared a four-element tuple while returning three: the fourth slot encoded the same phantom distribution in a type rather than in prose, so the arity is fixed while the accessor keeps its name, which carried no false claim; `dev/packaging/python_cohort.py`.
- [x] `W06.P13.S932` - Resolve the terminal surface identities against the live devtools registry reached through the harness, closing a join that was previously self-referential: the coverage check was passed the table's own keys as its surfaces argument, so it compared the table with itself and a coordinated rename on both sides passed against a harness offering no such surface, leaving in-suite joined membership at none of the forty-nine rather than the five previously believed bound; the new gate keys on the painted interface qualname rather than the identity under test, so it cannot be satisfied by editing that identity, and carries vacuity floors proven against a starved registry; `dev/tui/tests/test_tui_surface_identity_resolution.py`.
- [x] `W06.P13.S933` - Require a scored outcome before the driver gate quotes its denominator: the outcome is a union of a scored result and an emitted-only result, only the former carries a field count, and the emitted-only variant is returned for a document that authored no truth, which this gate can reach because it takes the first readable document rather than one chosen for having truth; reading the count straight off the union raised a missing-attribute error that named the attribute instead of the absence, which is the confusion the emitted-only type exists to prevent, so the gate now asserts the outcome is scored and reports the type's own explanation when it is not; `dev/ingest_harness/tests/test_driver.py`.
- [ ] `W06.P13.S934` - Record that the development tree is outside every configured type checker, the strict checkers being scoped to four production packages with tests excluded and the remaining authority covering the source tree only, which is why an attribute read against a field its dataclass had never declared survived in a packaging gate long enough to make four of that module's five tests unrunnable; a type check across the seven directories of this scope reports one hundred and thirty-nine diagnostics, of which forty are unresolved attributes, and admitting the tree to a checker is a packaging-configuration decision outside the working constraint; `pyproject.toml`.
- [x] `W06.P13.S935` - Remove two measurement debug writes that an automatic commit swept into history alongside unrelated legitimate changes: a standard-error write at the head of the census scan and a representation of the internal report object echoed to standard output from the closure command, neither of them production code; both files verified byte-identical to their pristine hashes with line-ending counts unchanged, one an all-carriage-return file and the other pure line-feed; `dev/registry/analysis/load_census.py`.
- [x] `W06.P13.S936` - Derive the glossary anchor expectations from the documentation builder itself rather than from a table of literals whose docstring claimed they came from the rendered page while nothing re-derived them: the builder's identifier function is callable here against a live document, so the anchor a built page will carry is computed and swept against every rendered term line, and the environment stand-in refuses the serial-number fallback so a failure cannot land outside the claim; building the join exposed a latent divergence, the builder retaining full stops and underscores inside an identifier where the local helper folds every non-alphanumeric run to a hyphen, which no approved headword exercises today and which would otherwise have shipped a dead deep link; `dev/docs/tests/test_glossary_anchor_parity.py`.
- [x] `W06.P13.S937` - Make the serving-path benchmark's setup failure name its cause, where all seven of its gates errored at fixture setup on any non-interactive host and reported only a truncated subprocess status followed by a two-thousand character argument dump repeated identically seven times: the failure message now surfaces the product's own typed refusal first, which states that no passphrase channel is available and how to supply one, and the parser reads both output streams because a first implementation read only standard output, where the refusal envelope never appears, and would have named nothing on precisely the failures it exists to explain; `dev/packaging/tests/test_serving_path_benchmark.py`.
- [ ] `W06.P13.S938` - Record for the operator that the committed continuous-integration control plane document names operator machine hostnames, which the privacy gate over tracked files correctly refuses: the document was added today and the tokens are genuine host identifiers rather than functional survivors, so the decision between scrubbing them and recording them in the gate's allowlist with a stated reason belongs to the operator, the containing directory being outside every working scope in this campaign; `.github/ci-control-plane.md`.
- [x] `W06.P13.S939` - Guard a recovery action that declares no command path in the operator evaluation runner, where the field is optional on the action contract and defaults to absent while every guard below the insertion point assumes it present, the safety check taking a plain tuple and the invocation unpacking it positionally: such an action would have raised a type error out of the helper rather than recording a failure, discarding the scenario that produced it, so the ladder that already reports each non-executable case by name gains the missing rung in the same shape; the branch is not exercised by any fixture in the tree, no declared action currently omits the path, and it is reported as unexercised rather than tested; `dev/agent_eval/_runner.py`.
- [ ] `W06.P13.S940` - Record the lane-selection measurement over the packaging, continuous-integration, deployment, release and container directories, where every one of the one thousand and seventy-one tests is selected by at least one lane, so the reachability ratchet already owns that question, while one hundred and twenty-nine are selected only by workflows that run on manual dispatch and never on a push: seventy-eight sit outside the paths of the only per-push lane over the development tree, fifty-one are inside those paths but excluded by every per-push marker expression, and the highest-consequence entries are the publish-authority and lane-isolation gates over the documentation publishing boundary, whose regressions would ship between dispatches; the two remedies are path and marker additions to the task runner, outside every working scope; `dev/ci/lane_reachability.py`.
- [x] `W06.P13.S941` - Let the modelo 347 binding tripwire reach its own assertion, a gate that requires readiness to report the casilla-empty draft not ready and fires if a registry edit ever unbinds the modelo: its dispatch helper first demanded a zero exit status, but the readiness command exits non-zero precisely when it reports not ready, so the gate was red in both directions, stopped before its assertion while the modelo is bound and failing that assertion if it were unbound, leaving a permanent red that could not distinguish the two states it exists to separate; the verdict is now read from the envelope and only a response carrying no readiness verdict at all is refused, naming the command's own error when there is none; `dev/agent_eval/tests/test_lifecycle_contradiction_golden.py`.
- [ ] `W06.P13.S942` - Record for the operator a shipped user-visible defect in the legal normative extraction, where the permalink fragment is taken from the source marker or else derived from the article heading through a short-circuit that compares nothing: over the captured corpus the two derivations disagree six hundred and fifty-three times, the fallback fires four hundred and twenty times across two hundred and fifty-seven files, and ten committed extraction sidecars ship a fragment pointing at a different provision of the value-added tax law, which resolves rather than erroring and so misleads silently; no heading-based rule can repair four of the ten because the official fragment follows the document's block sequence rather than the heading, so the remedy is to refuse where the derivation cannot know or to resolve from the consolidated capture, and it rewrites committed artefacts outside every working scope; `dev/docs/preprocess/normatives_html.py`.
- [x] `W06.P13.S943` - Turn the silent-fallback class on this scope and record a clean negative: across two hundred and eighty-one modules, nine assignments take a declared value or else a computed one, and a detector validated against the live extraction defect in the documentation tree fires on that known case before being trusted here; seven of the nine are boolean predicates rather than competing derivations, one is a hash cache whose supplied and computed halves are provably equal because the single production caller hashes the same bytes it decodes, and one is load-bearing because the builtin import function is never import-bound and the raw spelling is the only way it is detected, with the result compared against two exact names so a false resolution cannot widen; `dev/audit/object_names.py`.
- [ ] `W06.P13.S944` - Record for the operator that an untracked scratch file of roughly forty kilobytes sits at the repository root, where two probe dumps were permanently committed earlier in this campaign by a broad staging commit that swept whatever the tree held; the file belongs to neither this session nor any dispatched agent and has never been committed, so it is left untouched, but its position carries the same risk that already materialised twice; `scratch_steps.json`.
- [x] `W06.P13.S945` - Repair two defects in the default-lane visibility screen. Its declared test roots were filtered on directory existence, so when the top-level test tree moved under the source tree the roster silently became a single root and the census kept reporting a clean scan over half of what it named, a loss no vacuity floor could detect because the surviving root alone clears any floor the pair would; roots are now declared and resolved through a function that refuses one that has stopped existing, with a gate proving both directions. Separately the screen asked whether individual tests carried decorators only when a module declared no module-level marker assignment at all, so a module declaring one that names no execution marker fell through to the sharpest condition, reported as running nowhere, which was false for two modules including one whose ten tests ran under ten execution decorators; that channel now reports none, and only a decorated test defers the question, since a decorated fixture confers no marker on anything it serves; `dev/quality/default_lane_visibility.py`.
- [ ] `W06.P13.S946` - Record for the operator a live size-budget refusal naming three subjects over their ceilings, a locale test module at one thousand two hundred and fifty-six lines against a ceiling of one thousand two hundred and fifty, and two modelo application modules, one by file length and one by callable length; non-causation for this campaign is established by inspection, since every named subject lies in the locale tree or the source tree and no file edited by this session or its dispatched agents appears among them; `dev/audit/tests/test_size_budget_dev_corpus.py`.
- [ ] `W06.P13.S947` - Record for the operator that sixty-eight per cent of the gates in this working scope never run on a push. Of two thousand and four collected tests across the quality, audit, general, ingest-harness, agent-evaluation, environment and identity test trees, six hundred and forty are selected by a push-triggered workflow: six hundred and twenty-five from the quality tree through the per-push development recipe, twelve from one suggestion-conformance module through the per-push integration recipe, and three from an import-hygiene module filtered by keyword in the runtime-compatibility workflow. The remaining one thousand three hundred and sixty-four are reachable only through the development tooling recipe, whose sole invoker declares manual dispatch as its only trigger, verified by reading that workflow's own trigger block rather than by inference. Separately, the architecture recipe naming four facade, import-edge and vocabulary gates is invoked by no workflow at all, and those gates are reached only through the same dispatch-only lane. The remedy is a path addition to the per-push recipe or a workflow step, both outside every working scope in this campaign; `justfile`.
- [x] `W06.P13.S948` - Remove two stale live counts from floor rationales in the general gate tree, both stated in the present tense and both drifted: the facade export gate claimed the head commit carries five thousand eight hundred and fifty-seven modules and two hundred and ninety-five facades, where re-deriving through the module's own scan gives five thousand nine hundred and sixteen and two hundred and ninety-six, and the migration manifest claimed the package holds the same module figure while its own recursive glob counts five thousand nine hundred and eighteen over a different population, so one number had been copied onto a measurement it never described; correcting them would only restart the clock, the rationale each comment exists to give does not depend on the exact figure, and both assertions already report the live count on failure; `dev/tests/test_facade_export_gate.py`.
- [x] `W06.P13.S949` - Gate the localized documentation surface against an exclusion that removes a page from the translation work and from the audit of that work in one move, so no downstream instrument can observe the loss: the generated-page predicate is a bare substring test, an authored page discussing generated files in its opening bytes is dropped, and fifty-seven live pages sit against a floor of thirty-eight, meaning nineteen could vanish with every gate green; the new gate recomputes the candidate set from the production constants it imports rather than restates, subtracts the published surface, and requires the residue to be declared in both directions, with a teeth case proving an undeclared drop is named and a witness pinning the false positive as reachable; `dev/docs/tests/test_localized_surface_exclusions.py`.
- [x] `W06.P13.S950` - Gate the lane trigger model against resolving nothing at all, now that each lane carries the workflow events that reach it and so distinguishes a lane running on every push from one running only when a person asks: the concrete failure mode is that the trigger key in a workflow file is a boolean under the older markup specification, so a reader spelling it as a string gets nothing for every workflow in the repository and the block is stored under the boolean key instead, which empties every trigger set and makes every lane read as automatic, reporting full coverage exactly when the instrument stopped measuring; three gates cover the lane model, the workflow reader read against the files themselves, and a floor over the automatic set, while deliberately not refusing on how many lanes are manual-only, since that number is large and whether it is wrong belongs to the operator, and shadowing the reader with the naive spelling fails all three while the other thirty-nine pass; `dev/tests/test_lane_reachability.py`.
- [x] `W06.P13.S951` - Reconcile the apparent contradiction between a reachability gate reporting no test outside every lane and a parallel measurement reporting ninety-seven selected by none: both are correct and answer different questions, since against the thirty-three declared lanes the residue is empty while against the nineteen that a workflow actually invokes it is ninety-seven, and the difference is precisely a lane that is declared but reached by no workflow; the residue partitions exactly and disjointly into four environment classes, thirty-seven needing the operating system credential store, thirty-one a live tax-authority endpoint, twenty-four an external binary and five a resident service, with none uncovered and none in two classes, and a gate asserting that composition against a documented holdout set already exists in the owning module, so no new instrument was warranted; `dev/tests/test_lane_reachability.py`.
- [x] `W06.P13.S952` - Reconcile the unconsumed-export ratchet baseline with the tree after finding it silently drifted: four recorded modules published no unconsumed names at all and three more published fewer than recorded, so the four spent entries were removed and the three counts lowered to their live values, every change in the shrink-only direction the ratchet permits and guarded by asserting the verdict carried no added and no grown entries before the file was touched; the tool now exits zero where it had been failing unobserved; `dev/quality/unconsumed_export_ratchet.toml`.
- [ ] `W06.P13.S953` - Record for the operator that three ratchets and the quality suite that runs them have no automatic invoker, which is why the export baseline drifted unnoticed: each ratchet has its own task-runner recipe, an aggregate suite runs all three as subprocesses, the recipe invoking that suite is called by no workflow and no other recipe, and every test module for these ratchets drives synthetic temporary trees so none asserts a committed baseline against the live tree; the two measurable ratchets cost ninety and sixty-six seconds respectively, so placement is a real decision rather than a free addition, and the directory holding their tests is selected per push by a lane accepting integration-marked tests, which makes wiring the suite the cheaper remedy than duplicating it as a gate; `dev/quality/suite.py`.
- [ ] `W06.P13.S954` - Record that the dead-code screen cannot see dead code in the development tree, its targets naming the production package and a single whitelist file, so roughly a thousand tooling modules including every instrument used in this campaign and the screen's own module lie outside its population; two live instances prove the gap, a measurement helper with no callers found by hand in a parallel scope, and four evidence-recording calls placed after a return in a packaging smoke module, which therefore never execute and leave four proofs the function claims to record permanently absent. Admitting the development tree would be cheap, six findings at default confidence against eight over a production tree six times larger, a density comparison that validates the invocation rather than trusting a suspiciously small number; all three findings in this scope are false positives, two being protocol call parameters passed by keyword at the call site and one a name used inside a string annotation, so the widening is deferred because the genuine findings belong to other scopes and enabling it now would create a refusal this scope cannot resolve; `dev/audit/dead_code.py`.
- [ ] `W06.P13.S955` - Record that a second ratchet has drifted and this one carries genuine new debt rather than stale bookkeeping: the unused-symbol ratchet refuses, reporting one recorded module that now carries no unused symbol and, more importantly, two modules newly carrying unused symbols that the shrink-only baseline does not record, both last changed by the operator in the production tree and so outside every working scope here. The baseline is deliberately left untouched, because recording the two additions would launder new debt into a ratchet whose whole contract is that the number may only fall, and the refusal must stay visible until the symbols are addressed. This is the fourth ratchet, not the third as earlier recorded, the count having been missed by truncating a file listing, and all four run only through an aggregate suite whose recipe no workflow invokes, which is why both drifts accumulated unobserved; `dev/quality/unused_symbol_ratchet.toml`.
- [x] `W06.P13.S956` - Bound the two documentation pruning remedies that could delete their own evidence without limit, the interface stub tree and the translation catalogues, where the gate detects a narrowed filter exactly once and the prescribed fix then removes the pages the narrowing orphaned, leaving the gate permanently green over a smaller published surface: measured against the live tree, widening the package exclusion by one segment removes between thirty-one and six hundred and fifty-one of one thousand eight hundred and thirty pages and a collapse removes all but one, while a collapsed page authority removes every one of the hundred and seventy-one catalogues; both remedies now compute the whole removal set before the first deletion so a refusal is atomic, re-check each doomed path at deletion time, and refuse past a declared allowance that the accompanying gate re-derives from the live tree rather than pinning, which caught the author's first bound as too loose against the smallest measured collapse; `dev/docs/apidocs/manager.py`.
- [ ] `W06.P13.S957` - Correct an earlier verification recorded in this campaign: the promotion of the currency predicates module from private to public was reported clean on the evidence that the old private name appeared nowhere, that nine consumers referenced the public module and that seven importing modules loaded successfully, but that enumeration covered code consumers only and the generated interface reference is a consumer too, so the stub correspondence gate is refused at head with three missing, one orphaned and three stale stubs arising from that same promotion; enumerating a moved symbol's consumers includes non-code artefacts, and the remedy belongs to the operator because regenerating the tree lies outside every working scope here; `dev/docs/tests/test_api_stubs.py`.
- [x] `W06.P13.S958` - Turn the regeneration-destroys-evidence class on this scope and record a clean negative with numbers: none of the four ratchets exposes a write mode, so the hand-authored rationales carried in two of their baselines cannot be destroyed by a regenerator; the size budget baseline does have one but carries no hand-authored content and gates growth behind an explicit flag; the two large disposition files, one of them a hundred and forty kilobytes, have no writer at all and are maintained by hand. A syntax-tree census of write calls across the scope's production modules initially returned one hundred and thirty-one sites, too broad to act on because most write to the output stream, and narrowing to writes whose receiver is a path rather than a stream gives ten, with the known baseline writer recovered as a positive control; of those ten, two write a transient run directory, two write operator-specified report paths, one writes a temporary file, one is the gated baseline, and the four in the rename sweep are each guarded by its apply flag, the last of them verified by reading past the window that had hidden the guard; `dev/quality/namespace_retirement_sweep.py`.
- [x] `W06.P13.S959` - Record which quality screens are gated only outside the per-push lane, sharpening the earlier lane measurement from a directory count to a named list: of forty-five production modules in the quality tree, none lacks an importing test, thirty-five are imported by a test in the directory the per-push development recipe names, and ten are imported only from paths reachable through the dispatch-only tooling lane, among them the closed-vocabulary, contract-drift, tautological-assertion, write-path-coverage, workspace-field-population, type-checking-runtime-use and governance-isolation screens. One of the ten is partially covered after all, three of its tests running per push under a keyword filter in the runtime-compatibility workflow, a nuance the classifier missed because it considered only the quality test directory, so the honest split is nine fully dispatch-only and one partial. Two weaker instruments were rejected before this one: a naming convention alone reported twelve modules as uncovered and every one was a false positive, and an import census alone cannot distinguish the lane, so the two were intersected; `dev/quality/suite.py`.
- [x] `W06.P13.S960` - Teach the proof-contract gate to distinguish an assertion that exists from one that can run, its own subject being a declared claim with no assertion behind it: the reader asked only whether a recording call appeared in the syntax tree, so four calls placed after a return counted as backing four claims, and a corpus pattern separately excused an entire nine-claim lane whose helper-built declaration the reader could not parse; corrected against the prior commit the reader names three previously accepted claims as unbacked, and a negative teeth case keeps the reachability rule no wider than reachability so an early guard return does not erase every recording after it; `dev/packaging/tests/test_proof_contract.py`.
- [x] `W06.P13.S961` - Correct the smoke lane dispatch gate, a reachability screen that omitted one lane from its own population and judged wiring by raw text, so a commented-out invocation counted as dispatch, which is exactly the defect the sibling module beside it was written to remove; the omitted lane was wired, so the gate was green and would have stayed green had it been unwired, and the comment filter is proven not to unwire the single genuinely text-dispatched lane; `dev/packaging/tests/test_smoke_lane_dispatch_reachability.py`.
- [x] `W06.P13.S962` - Widen the container base image singularity gate to the one other file whose purpose is to name an image, the development container definition, which its surface filter excluded by extension while walking seven thousand others; admitting the format costs no false positives across the fourteen hundred surfaces of that type, and required teaching the non-Python reader that the format's comment marker differs, since the live file documents the derived tag in prose the stricter reader would have flagged; `dev/packaging/tests/test_container_base_image_singularity.py`.
- [x] `W06.P13.S963` - Correct the placement of four evidence recordings that sat after a return in the development smoke lane and so never executed: the consequence was not a wrong manifest, because the manifest writer refuses before writing when a declared claim is unrecorded, but a lane that would fail at the end of a full environment build, and the reason it had never been observed is that the dispatch gate reported the lane reachable on the strength of registration rather than execution; each claim is recorded where its own checked subprocess returns, following the ledger's rule that a claim is asserted on success, rather than being hoisted before the work it attests; `dev/packaging/smoke_dev.py`.
- [x] `W06.P13.S964` - Announce the modules the unconsumed-export walk cannot parse, adopting the shared unread-inputs sink that nine other development modules already use and that exists for precisely this: a dropped module contributes neither its exports nor its imports, so it can move a count in either direction while the baseline records the result as though the tree had been read entire, and this ratchet's baseline had already drifted once with nothing reporting it. The skip itself stays, since a walk over a tree a sibling process is editing must survive a half-written file, and two gates in the per-push lane prove both directions, the notice naming the dropped file while the readable half still counts, and a complete walk staying silent because a notice on every run carries no information. Found by censusing read-failure handlers across the development tree, three hundred and twelve in total of which forty-one have a body consisting only of continue or pass; `dev/quality/unconsumed_export_ratchet.py`.
- [x] `W06.P13.S965` - Stop the synonym mining command writing off a human reviewer's verdicts, a queue file that declares itself generated while carrying three fields the miner structurally cannot emit, every mined row being born proposed and the validator forbidding a proposed row from carrying review fields at all: the whole blast radius was one default, the output option pointing at the committed queue, so a single invocation with no arguments reduced three entries to one and exited zero, with the validator then reporting the survivor clean because it only iterates the rows the queue still has, making the loss invisible from the producing end and the checking end alike; verdicts are now carried forward on the identity tuple the queue's own uniqueness validator uses, the entire loss set is computed before the first byte so the refusal is atomic, discarding is an explicit opt-in, and the gate re-derives its premise from the live artefact rather than freezing a count. The decisive evidence was a sibling verb in the same package whose output option defaults to writing nowhere, the same idiom with the opposite blast radius; `dev/docs/terminology/_synonym_cli.py`.
- [x] `W06.P13.S966` - Refuse when the unused-logger-binding gate could not read a shipped module: the gate walked every shipped module, skipped any it failed to read and then asserted no offenders, so an empty list read as a clean tree while the unread module is precisely where an offender would hide; the skip stays because a walk over a tree a sibling process is editing must survive a half-written file, but the gate now names what it could not read and refuses, proven by injecting a path under the repository root that does not exist and observing both the refusal naming it and the untouched tree still passing. The sibling floor at the head of the same module is deliberately left alone, a skip there lowering the observed count against its floor and so failing toward a false alarm rather than a false pass. This is one of seven gate-side instances located by censusing read-failure handlers in this scope, thirty of which are silent, twenty-three in tooling and seven in gates, six of those seven sitting in the directory the per-push lane selects; `dev/quality/tests/test_no_unused_logger_binding.py`.
- [x] `W06.P13.S967` - Refuse when the cited-constant gate could not read one of its subject modules, the reader that collects constants whose comments cite a binding provision supplying the gate's subjects rather than its evidence, so a module it failed to read contributes no constants and a provision cited inside one, an article of the income tax law or a rate fixed by royal decree, is never checked while the gate reports clean over ground it never walked; the population floor cannot detect this because it asserts only that more than twenty constants survive, which whatever survived satisfies. The walk now returns the unread modules alongside what it found and the gate refuses, proven by injecting an unreadable subject path and observing both the refusal naming it and the untouched tree still passing. The sibling reader in the same module is deliberately unchanged, supplying the usage evidence for the unprotected test, so a module skipped there lowers the count and flags more constants rather than fewer, failing toward a false alarm; `dev/quality/tests/test_cited_constants_are_protected.py`.
- [x] `W06.P13.S968` - Stop the terminal snapshot verb silently destroying a named review run, its whole blast radius being one absent flag: a snapshot taken under a name already in use removed the entire existing run, every image, manifest and log, with no prompt and a zero exit, over a tree the project documents as never durable and which version control therefore cannot restore, on the one verb whose purpose is preservation and behind roughly twenty-five minutes of rendering. Replacement is now an explicit opt-in, a taken name refuses while naming how many files it would have discarded, and the directory is re-checked immediately before removal so only the path the refusal measured can go; no caller relied on the previous behaviour, established by search rather than assumed. The decisive observation was that the verb already refused for the reserved current name and had a test for it, so it knew how to refuse and was guarding the name rather than the contents; `dev/tui/cli.py`.
- [x] `W06.P13.S969` - Refuse when the unconsumed-consumer-claim gate could not read one of its subject modules, this corpus being both the gate's subject and its evidence with the two directions differing: as evidence a skip is safe, since a module absent from the corpus consumes nothing and a claim elsewhere therefore looks less consumed so the gate flags more, but as subject it is not, the claims made inside an unread module never being examined at all, and the population floor of five hundred sources cannot see the loss because whatever survived satisfies it; the walk now returns what it could not read and the gate refuses, proven by injecting an unreadable subject path and observing both the refusal naming it and the untouched tree still passing. Third of the seven gate-side instances in this scope, the two deliberately unchanged being those whose skip lowers a count against a floor and so fails toward a false alarm; `dev/quality/tests/test_no_unconsumed_consumer_claim.py`.
- [x] `W06.P13.S970` - Refuse when the test-only public alias gate could not read a module, a single skip failing three ways here of which only one is safe: a production module lost as evidence lowers the production use count so an alias looks less used and the gate flags more, a false alarm, but the same module lost as subject removes its own aliases from the scan entirely, and a test module lost drops the test use count below the threshold the offender condition requires so a genuine test-only alias stops resembling one, leaving two of the three directions silent and the production population floor blind to all of them; the walk now returns what it could not read and the gate refuses, proven by injecting an unreadable path and observing both the refusal naming it and the untouched tree still passing. Fourth of the seven gate-side instances in this scope; `dev/quality/tests/test_no_test_only_public_alias.py`.
- [x] `W06.P13.S971` - Widen the terminal render's stale-frame sweep, whose guard certified that no surviving file was named by the current manifest while the harm lies in a file having no replacement from the current run: rendering a single surface after a full render removed forty-eight of seventy-two frames with the adjacent guard passing cleanly throughout, over a tree version control cannot restore and behind roughly twenty-five minutes of rendering, and the existing test was the guard's mirror, certifying name-membership and asking nothing about replacement. The name check is kept and a bound derived from one surface's full matrix now refuses a larger sweep, following the catalogue-prune precedent in the documentation tree; two timing windows at the same site are also closed, the removal now re-checking each path immediately before unlinking, and the sweep no longer running ahead of the manifest and index writes, which had placed the only irreversible step before the one that made the render durable; `dev/tui/_artifacts.py`.
- [x] `W06.P13.S972` - Widen the sanitiser's signature refusal to descend the whole interactive form field tree rather than reading only its top row, the field collection being the root of a tree whose children hang under a nested key: a signature field nested one level down passed the guard entirely, so the document would have been rewritten and the signature the refusal exists to protect voided, and the producer-written flag does not cover the gap because it sits on the very dictionary the nesting hides the field in; the descent carries visited object identities, since a cyclic field tree in a guard that runs before every sanitisation would hang rather than refuse, and all three probed shapes now refuse where previously the nested one passed; `dev/sanitizer/_pipeline.py`.
- [x] `W06.P13.S973` - Give the executable digest one implementation across the boundary whose only purpose is to compare it, two oracle modules having minted the digest with a direct hash of the whole file while the evidence emitter verified it through the shared path-hashing helper, so one rule stood as two implementations on opposite sides of the comparison and their agreement was a coincidence maintained by hand; both producers now call the helper. Recorded alongside the wider adoption measurement across five helpers in this scope, the shell-comment reader rising from seven importers to eighteen with its remaining hand-rolled sites each adjudicated correct, the distribution-name normaliser reaching full adoption, the published file-size limit already fully adopted with no literal copies left, and the subprocess wrapper reported as thirty-one raw call sites against seventeen adopters, measured rather than converted; `dev/packaging/installed_tax_oracle.py`.
- [x] `W06.P13.S974` - Give the task-runner recipe reader one home and correct the defect both of its copies carried, the shell tokeniser retaining a leading comment marker as an ordinary token so a switched-off recipe answered yes to a membership test for the test runner and read as a live invocation in both copies; the obstacle previously accepted as blocking, that these readers depend on leading whitespace to separate a recipe body from its header, was already solved elsewhere in the tree by reading indentation from the raw line before handing it to the executed-lines helper, structure first and execution second, and the reader now lives in a single module with an injectable source and a detector-teeth case over a temporary fixture; `dev/packaging/tests/_justfile_recipes.py`.
- [x] `W06.P13.S975` - Announce the modules the secure-store write-path scan could not parse, a scan that decides where a secure store is written and whose skipped module contributes neither its reads nor its writes, so a store written only from the unread module reads as written nowhere; the skip stays because a walk over a tree a sibling process is editing must survive a half-written file, but it now routes through the shared unread-inputs sink, proven both directions with the notice naming the unparseable module and a complete walk staying silent. Nothing would have reported it: the scan's own task-runner recipe has no invoker and its gate exercises only synthetic temporary trees, the same shape as the four ratchets in the same directory; `dev/quality/secure_store_write_path.py`.
- [x] `W06.P13.S976` - Record that the operator is working the unused-symbol ratchet's debt down in the permitted direction, the pending change to that baseline being two removals and no additions, one of them the spent entry identified earlier and the other a module the operator is concurrently editing in the production tree; the decision to leave that baseline refusing rather than absorbing its two new entries is therefore vindicated, since recording them would have laundered new debt into a ratchet whose contract is that the number may only fall; `dev/quality/unused_symbol_ratchet.toml`.
- [x] `W06.P13.S977` - Scan the tracked file the transient-path gate had been silently skipping and refuse the rest: the walk dropped any tracked text file that would not read, so a transient tree path recorded inside one was never looked for, and the population floor could not detect this because it counts what the walk listed rather than what it read, leaving an unreadable file contributing to the floor while contributing nothing to the scan. Adding the refusal turned the gate red immediately on a real file, a tracked official-gazette corpus text that is not encoded in the expected form and had therefore been skipped since the gate was written; refusing that file would be wrong since it is a legitimate corpus document, so its content is now decoded lossily, which answers this scan's question exactly because the segments searched for are plain ASCII and a replacement character can neither forge nor hide one, while the refusal is reserved for a genuine read error and proven still to fire by injecting a path that does not exist; `dev/tests/test_tracked_content_excludes_transient_trees.py`.
- [ ] `W06.P13.S978` - Record a stale exemption that the transient-path gate's own staleness check is correctly refusing, a declared naming site in the production test tree that no longer contains any transient tree segment, so the exemption now only suppresses; non-causation with respect to this campaign's edits is established because the refusing test reads the declaration mapping directly and never calls the reader that was changed, and the file it names lies in the production tree outside every working scope here; `dev/tests/test_tracked_content_excludes_transient_trees.py`.
- [x] `W06.P13.S979` - Announce the modules the development-path and development-prose scans could not parse, both enforcing the rule that production code stands alone by forbidding a development-tree path built in source and a development-tree mention in a comment or docstring, and both having silently skipped any module they failed to parse so a violation sitting inside one was never found; each now routes the skip through the shared unread-inputs sink with its consequence stated per caller as that helper requires, proven in both directions for each scan with the notice naming the unparseable module and a complete walk staying silent. Two of eight silent handlers in this module, the remaining six sitting in different functions with no shared reader and each needing its own direction judgement, since a skip that loses a subject is silent while one that loses evidence commonly fails toward a false alarm; `dev/quality/import_hygiene_scan.py`.
- [x] `W06.P13.S980` - Floor the population of the step-identifier gate before it asserts that population is empty, this gate expecting the empty set while its own opening paragraph identifies that as the dangerous shape, a pattern that has stopped being checked being indistinguishable from one that matches nothing, so a walk reaching no test names or a name reader that stopped returning them would satisfy it exactly as a clean tree does; the floor is evaluated before the emptiness assertion so it runs whether or not a leak is present, and sits far below the live population of four thousand eight hundred and seventy-three names across five hundred and fifty-one modules so ordinary churn never touches it. The gate is refusing at present on a genuine leak in another working scope, that failure is unchanged and still reported, and starving the walk to three modules makes the floor fire instead; `dev/tests/test_campaign_marker_patterns.py`.
- [x] `W06.P13.S981` - Record that the census behind the preceding fix was unusable until its own detector was corrected three times, the raw count of live-tree emptiness gates lacking a vacuity floor falling from sixty-three to sixteen to eight and finally to one as three distinct false-positive modes were removed, each of them a floor written in a form the previous pass could not recognise, first a bare non-empty guard on the population rather than a numeric comparison, then a comparison over variables the keyword list did not name, then existence checks mistaken for population guards; the single surviving case was the only one worth acting on and the intermediate numbers were reported as unreliable rather than quoted; `dev/tests/test_campaign_marker_patterns.py`.
- [x] `W06.P13.S982` - Remove three pattern projections that nothing consumes, each a one-line derivation of the case tuple beside it, imported by neither consumer and unread within their own module as verified across both the development and production trees; they are the interface of the walk this module's own opening paragraph records as deleted and deliberately not restored, so the walk went and its projections stayed, and the module's own reasoning that a declaration nobody checks is indistinguishable from one matching nothing is the argument for removing rather than leaving them, each being re-derivable in a single line should a future walk want it. Found by censusing public table-shaped constants in this scope referenced neither elsewhere nor in their own module, forty-two candidates before the intra-module reference check and three after, all three verified individually; `dev/tests/_marker_metadata_patterns.py`.
- [ ] `W06.P13.S983` - Record a governance-isolation refusal naming a quality test module that reads a harness tree without an exemption, established as unrelated to this session's work because the named module was last changed by the operator on the preceding day and the lines removed here mention no governance tree at all; the refusal is correct behaviour by the gate and the remedy, either an exemption carrying its reason or moving the logic into the harness, belongs to whoever introduced the reference; `dev/quality/tests/test_rule_citation_resolves.py`.
- [x] `W06.P13.S984` - Record the consolidated measurement behind every scattered no-invoker finding in this campaign: of one hundred and eleven task-runner recipes, forty are reached by a workflow or by another recipe and seventy-one are invoked by nothing at all, of which thirty-four are gates named as checks, tests or audits. The census carries controls in both directions, the two recipes known to run per push resolving as invoked and the two established as dead in earlier rounds resolving as dead. Many dead recipes are legitimately operator-facing, an environment doctor, a documentation server, a provisioning step, and are not defects; the actionable subset is the gates needing no special environment, which includes the whole audit suite and its aggregate, the architecture invariants, the four ratchets and their aggregate, the secure-store write-path scan, the semantic duplication scan, the dependency check and the corpus text check. Each of these was previously recorded separately as having no invoker; this supersedes those individual notes with one measured list, and the remedy is a workflow step or recipe dependency, outside every working scope in this campaign; `justfile`.
- [x] `W06.P13.S985` - Run the uninvoked gates of this scope to establish which are refusing unseen, three being cheap enough to execute: the dead-code screen and the write-path backlog both pass, while the complexity audit refuses with six hundred and seventy-two hotspots over threshold. That refusal is by design rather than drift, the baseline loader returning an empty baseline unconditionally with its docstring stating the committed baseline and reviewed allowlist were both retired so every hit is reported against no ceiling, which also explains why every entry carries a new-finding label, that label meaning only absent from an empty baseline; the closing summary already discloses the absence of a baseline, allowlist and accept flag, so the report is honest where it counts and no change is warranted. The figure is a debt total nobody sees rather than a regression, which is a different condition from the two ratchets found genuinely drifted earlier; `dev/audit/complexity.py`.
- [x] `W06.P13.S986` - Turn the class of a test named for an artefact its body never touches on this scope and record a clean negative: fifty-one test modules bear the name of an existing production module, and every one of them imports it. The first pass reported a single offender and that offender was a defect in the detector rather than in the tree, because a relative import written as a from-package import carries the imported name in the alias list rather than in the module attribute, and the walk collected only the latter, so that entire import form was invisible; correcting it resolved the one candidate and left none. The intermediate count of one was never reported as a finding, and both controls hold, the module known to import its subject staying unflagged before and after and the falsely flagged module resolving once the missed form was admitted; `dev/quality/tests/test_object_name_declustering.py`.
- [x] `W06.P13.S987` - Move version canonicalisation into the burned-version ledger itself, closing a release-safety comparison that failed open: a burned version is one somebody decided must never ship again, yet the membership check compared raw strings, so a leading zero, a version prefix or surrounding whitespace all answered not burned for a number the ledger burns, and the ledger's own reader accepted two spellings of one version as two separate burns, which is precisely the conflicting-evidence state its duplicate refusal exists to prevent. The canonicalisation already existed and was correct but lived at a single caller, so correctness depended on every future caller remembering; the record now carries its canonical form and the membership check, the reason lookup and the duplicate refusal all compare numbers, with the caller-side copy removed and both defects reproduced as teeth; `dev/release/burned_versions.py`.
- [x] `W06.P13.S988` - Give the artifact-map digest one implementation shared by the runner that produces it and the evidence module that verifies it, the two having been separate transcriptions of one canonical serialisation rule; unifying them is proven behaviour-preserving because both head implementations agree on ascii, non-ascii and empty inputs, while the teeth are that a third spelling of the same rule elsewhere in the packaging tree already disagrees on non-ascii input, so the agreement between the first two was a coincidence that a nearby module already breaks; `dev/packaging/evidence.py`.
- [x] `W06.P13.S989` - Record two coordination facts for the operator. A second instance of the packaging agent was left running when its successor was launched, briefly putting two writers on one surface; no collision occurred, the predecessor having made no commits with all twenty of its files reaching the head commit by absorption across three commits and the successor's in-flight files being disjoint, and it stood down on request. Separately, an exit-status idiom in use by that agent captured the status of the pager rather than the test runner because the echo followed a pipe, invalidating the exit codes in several of its reports though not the pass and fail counts; the same idiom as used by this session places the echo inside the command group before any pipe and was verified to report a genuine failure status; `dev/packaging/tests/test_installed_cli_resolution.py`.
- [x] `W06.P13.S990` - Turn the call-site-rule class on path normalisation in this scope and record a clean negative with numbers: forty-seven modules spell a repository-relative path through the path object's own posix accessor across a hundred and five sites, twenty modules do it by replacing separators by hand across forty-two sites, and eight modules use both idioms. Mixing spellings is not itself a defect since both yield the same string, so the question is whether any comparison crosses a base rather than a separator, and the export ratchet is the case that matters, carrying three spellings at once, repository-relative for its audit key, package-relative for its count keys and unnormalised for the finding set. The single comparison that joins them is proven consistent empirically rather than by its docstring, the finding paths being repository-relative in fact and the audit key matching their shape, and a base mismatch would zero every count and report all one hundred and ninety-four baseline entries as spent, where the tool reports none; `dev/quality/unconsumed_export_ratchet.py`.
- [x] `W06.P13.S991` - Gate the three refusal sites of the conformance stamp verb, a module that previously carried no tests at all while formatting arbitrary underlying exceptions, a registry load failure, a markup parse error and a schema validation failure, straight into the message a caller sees, where the parser and validator both routinely echo the offending literal value; the new gate proves that none of the three sites reaches the validator's raw input dump, that a refusal never carries the reviewer identity whatever that identity contains, that the message is independent of it, and that the refusal still names the revision and the missing field so the diagnostic remains useful, which is the balance the sensitive-data rule requires of user-visible diagnostics, stable identifiers and remediation rather than raw payloads; `dev/registry/conformance/tests/test_stamp_refusal_diagnostics.py`.
- [x] `W06.P13.S992` - Detect record-design corpus files that carry no attestation, walking disk to manifest where the check previously walked only manifest to disk: one tracked and committed tax-authority workbook is named by no manifest at all, carrying no source address, licence, digest or retrieval date, and its history explains why rather than leaving it as drift, a bulk pass having removed twenty-five workbook copies together with their entries and a later commit having restored this one file because the older reader cannot read the formula text on which the modelo two hundred totals assertion is grounded, so the bytes came back and the entry did not. Under the calculation grounding rule a value without traceable authority cannot be promoted to filing grade, so this is evidence rather than bookkeeping; the new check compares the undeclared set for equality against a named census of known debt rather than adding an ignore rule, so a second orphan refuses and so does attesting this one without removing it from the census, while the attestation itself is left to the owner of the production tree; `dev/corpus/sync_aeat_record_design_corpus.py`.
- [x] `W06.P13.S993` - Move the terminal cell height onto the per-frame record, resolving the previously undecidable case by evidence: the rasterise path keeps any frame whose vector source is missing as already recorded, an ordinary non-fatal branch rather than only an interruption, and then stamps the requested height onto the run manifest, asserting a resolution those pixels were never repainted to; the per-item field now carries the height each frame is actually at, the run-level field is redocumented as the height the run was invoked with, a disagreement accessor names the mixture and the index banner reports it instead of a single number, with the schema version raised and the teeth building real images at two heights and running the retired reading in the same test to show the run-level scalar claiming one height for pixels measured at another; `dev/tui/_artifacts.py`.
- [ ] `W06.P13.S994` - Record for the operator that multi-part extraction sidecars carry no part total, so an interruption between parts is silent: the writer holds the total only as a local, the persisted record has no part index or part total, and the freshness gate treats a single present part as covering the whole workbook, so most of a record design can be absent from the indexed corpus while every content-change transition remains loud. The project already records the part total one layer above the layer that persists to disk, which shows the total is known to be the fact that matters and is lost on the way down; the remedy needs a schema version bump whose loader refusal would force regenerating seven hundred and seven committed sidecars in the production tree, making it a design decision rather than a hardening change; `dev/docs/preprocess/sidecar.py`.
