...
- `src/aeat/errors.py` gains a `FormulasError` subclass hierarchy
  root (kept in `aeat.errors` per the cross-subpackage base-class
  pattern).
...
- `corpus/casillas/` is NOT modified — wave 1 does not rewrite
  ...
  `aeat.casillas` onto `aeat.formulas`.
- `src/aeat/filing/_builders/modelo_130.py` is NOT modified —
  the synthetic builder remains; swapping it to the new engine
  is a follow-up issue.
- All internal imports inside `src/aeat/formulas/` use relative
...
- `Phase 2 — Core models`
  1. Create `src/aeat/formulas/_casilla.py` with
     ...
     existing enum from `aeat.casillas`),
     `legal_basis: tuple[LegalCitation, ...]`,
     `notes_es: str | None`.
  2. Create `src/aeat/formulas/_formula.py` with:
     ...
     - `FormulaDefinition(casilla_id: str, formula: Formula,
       formula_id: str)`.
  3. Create `src/aeat/formulas/_ruleset.py` with:
  ...
  4. Create `src/aeat/formulas/_registry.py` with:
...
- `Phase 3 — Ledger + engine`
  1. Create `src/aeat/formulas/_ledger.py` with `LedgerEntry`,
  ...
  2. Create `src/aeat/formulas/_engine.py` with the
     `Engine` facade:
...
- `Phase 4 — Modelo 130 rulesets`
  ...
  2. Create `src/aeat/formulas/_rulesets/_common.py` with
  ...
  3. Create `src/aeat/formulas/_rulesets/modelo_130_2024.py`:
  ...
  4. Create `src/aeat/formulas/_rulesets/modelo_130_2025.py`:
...
- `Phase 5 — Public API`
  1. Fill `src/aeat/formulas/__init__.py` with the full public
...
- `Phase 6 — CLI`
  1. Create `src/aeat/formulas/_cli.py` with Typer `app`:
     ...
     - `aeat formulas audit --modelo MODELO_130 --period
       ...
       `AuditReport` as JSON.
  2. Create `src/aeat/cli/formulas.py` shim re-exporting the
     Typer app (mirrors `src/aeat/cli/modelos.py`).
  3. Wire into `src/aeat/cli/__init__.py`:
...
- `Phase 7 — Unit tests (@pytest.mark.unit, colocated)`
  ...
  3. `src/aeat/formulas/test_formula.py` — every pydantic
     ...
     `model_validate_json`.
  4. `src/aeat/formulas/test_ruleset.py` —
  ...
  5. `src/aeat/formulas/test_registry.py` —
     ...
     - registry binds exclusively to
       `aeat.models.ModeloCode` (not the casillas-local
       restricted enum).
  ...
  7. `src/aeat/formulas/test_cli.py` — JSON shape + exit
     ...
     exist.
  8. `src/aeat/formulas/test_smoke.py` — assert every public
     name in `aeat.formulas.__all__` is importable.
  9. `src/aeat/formulas/test_integration_modelos.py` —
     cross-checks that each ruleset's modelo is in
     `aeat.models.MODELO_REGISTRY`. This closes the loop
...
- `aeat.formulas` subpackage exists with the exact public API
  declared in the ADR.
...
- Coverage on `src/aeat/formulas/` is ≥ 90 % (the engine is
  pure-logic; full coverage is achievable).
- No new dependencies added to `pyproject.toml`
  (`graphlib`, `decimal`, `pydantic` v2 are all already
  present).
- `src/aeat/filing/_builders/modelo_130.py` continues to work
  unchanged (its swap is a follow-up issue).
- `corpus/casillas/` is NOT modified.
- The existing `aeat.casillas.models.FormulaReference` stub
...
- **Risk**: `ModeloCode` collision between
  `aeat.models._codes` (authoritative) and
  `aeat.casillas.models` (restricted). **Mitigation**: the
