...
**Agent** is the dispatch target. `sonnet-high` remains appropriate for mechanical, bounded work
whose correctness is decided by an existing transformation and check. `opus-medium` remains the
contract-design target. `luna-max` owns the registry-authoring campaign: small official-evidence
cohorts where identity, retirement, introduction, or evolution must be decided and written into
TOML. Agent names on completed historical steps are not rewritten; open authoring steps state the
current dispatch target.

Each cohort is sized for one agent, but Wave 3 is a coordinated fleet. The coordinator owns the
corpus-wide signal, partitions modelos into disjoint write lanes, and pauses all writers at a
measurement boundary. Writers never share a modelo concurrently.

### How this work should be done

This campaign has two distinct authoring modes. A **mechanical transformation** whose identity
ruling is already settled uses a scratch script run from a scratch directory and never committed.
An **evidence adjudication** reads the official predecessor and successor sources and authors the
smallest coherent TOML cohort directly; a script must not manufacture a legal or semantic ruling
from identifiers, numbers, spans, or similarity. Each adjudication batch owns one modelo lane,
re-reads its target files before writing, and must either reduce the live edge worklist or record
an evidenced root, retirement, evolution, or refusal.

The edition-delta signal is the campaign ledger. It is measured at fleet boundaries, not after
every row: agents use focused per-modelo diagnostics while authoring, then pause together for one
corpus-wide measurement. A modelo enters delta migration only when every adjacent edge has zero
unresolved lineage, zero ambiguity, and no unretired withdrawal. Similarity alone never makes an
edge ready.
...
- [ ] `W03.P06.S19` - [L | luna-max] Complete evidence-grounded lineage for modelo 490 before migration. Work in non-overlapping official-design cohorts; every batch must either reduce an unchained edge count or classify an exact root, retirement, or refusal with evidence. Direct TOML authoring is the required method for adjudication; scripts may apply only transformations whose identity ruling is already settled. Migrate only after all three edges report zero unresolved lineage and zero ambiguity. Proof: the edition-delta signal reports zero unchained rows, every new root and retirement is evidenced, chain diagnostics are clean, then the standard round-trip, export-byte, and minimality checks pass.; `src/cadrumo/_data/registry/aeat/modelos/490`.
...
- [ ] `W03.P06.S21` - [L | luna-max] Complete evidence-grounded lineage for modelo 322 before migration, using DR32201/DR32202 to prove row identity across each actual endpoint and to preserve introductions, retirements, table boundaries, and formula evolutions. Work in bounded adjacent-box cohorts and never force four-revision continuity where an edition lacks the concept. Migrate only when every edge reports zero unresolved lineage and zero ambiguity. Proof: raw TOML and canonical loading are clean, the edition-delta and chain signals are clean at zero unresolved rows, and the standard migration checks pass on both authoring lanes.; `src/cadrumo/_data/registry/aeat/modelos/322`.
- [ ] `W03.P06.S22` - [XL | luna-max] Complete modelo 200's page-qualified lineage after the export lane's binding repair closes, then migrate it. Re-measure the post-repair tree; author missing predecessor declarations and successor continuity only where the official page, row, and equity-column context proves identity. Treat each bounded matrix cohort as adjudication, never as a number-keyed rewrite. Migration is forbidden while any edge gap, ambiguity, or wrong-sheet binding remains. Proof: the post-repair edition-delta signal is zero for unresolved lineage, all export bindings resolve on the correct page, and round-trip, export-byte, and minimality checks pass.; `src/cadrumo/_data/registry/aeat/modelos/200`.
...
- [ ] `W03.P07.S25` - [L | luna-max] Migrate modelo 100 only after the dedicated AEIP lineage campaign in S60 reaches zero unresolved rows on all five edges. Preserve genuine legal change rather than compressing it as restatement. Proof: S60's zero-gap and zero-ambiguity signal is recorded, legal-period diagnostics are clean, and the standard round-trip, export applicability, and minimality checks explain the resulting delta.; `src/cadrumo/_data/registry/aeat/modelos/100`.
...
- [ ] `W03.P07.S39` - [L | luna-max] Finish modelo 151 lineage adjudication before migration. Treat official record position, width, type, and C/B/IIC block identity as authoritative over label similarity; correct stale cross-block claims rather than preserving false compatibility. Continue bounded cohorts until the sole edge reports zero unchained rows and zero ambiguity, with every disappearance explicitly retired or refused. Proof: raw TOML loads, ambiguity and unretired-withdrawal findings are zero, chain diagnostics are clean, and the edge worklist is empty.; `src/cadrumo/_data/registry/aeat/modelos/151`.
...
- [ ] `W03.P07.S60` - [XL | luna-max] Run modelo 100 as a dedicated AEIP-grounded lineage campaign, separate from its eventual migration. Partition work by adjacent edition edge and coherent official manual/table block; no identity may be inferred from identifier, number, or span alone. Each bounded cohort must reduce the live unchained worklist or record a proven expiry, re-declaration, new root, or refusal. Migrate only after all five edges reach zero unresolved lineage and zero ambiguity. Proof: every row is grounded against official AEIP/manual context or explicitly classified, chain diagnostics are clean, and the standard migration checks explain rather than compress genuine legal change.; `src/cadrumo/_data/registry/aeat/modelos/100`.

### Phase `W03.P08` - Remainder

Every modelo not named above, in file-count order, plus the one whose classification is unknown.

- [ ] `W03.P08.S27` - [L | luna-max] Close the remaining lineage worklists not named in P06 or P07, ordered by unresolved predecessor rows and handled one modelo per write lane. For each edge, use its strongest official source to author exact chains, roots, retirements, evolutions, or a grounded disposition; never carry a modelo-wide exemption. Current signal priorities include the low-volume 136, 165, 182, 188, 189, 222, and 345 edges. Proof: each completed edge reports zero ambiguity and zero unclassified lineage, or an explicit data disposition names why it remains full-copy.; `src/cadrumo/_data/registry/aeat/modelos`.
- [ ] `W03.P08.S28` - [L | luna-max] Migrate only the remainder modelos that S27 or an earlier named step has made signal-ready, ordered by successor-row count. Keep lineage adjudication and migration as separate checkpoints: no modelo enters the migration tool merely because similarity is high. Proof: a per-modelo ledger records the zero-gap input signal followed by round-trip equality, export bytes or explicit not-applicable status, and delta minimality.; `src/cadrumo/_data/registry/aeat/modelos`.
...
strictly first. After that, run three disjoint Luna-max authoring lanes under one coordinator:
P06 high-similarity official-design cohorts, P07 divergent/AEIP adjudication, and P08 small
residues plus explicit dispositions. A lane owns one modelo at a time. It reports an evidence
target before writing, authors a bounded cohort, raw-parses the touched TOML, and runs only the
focused modelo delta and chain diagnostics. The coordinator reassigns a lane immediately after a
cohort or a precise evidence refusal; it does not cycle the aggregate tests between cohorts.

At a synchronized boundary all writers pause and the coordinator runs the corpus-wide edition
delta signal once. That snapshot determines the next lane allocation. A decreasing count is not
enough by itself: ambiguity, unretired withdrawal, false roots, and unsupported evolution are
blocking dimensions even when the aggregate gap falls.
