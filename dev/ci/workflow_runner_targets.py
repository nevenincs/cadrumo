...
_MATRIX_DIMENSION: Final = re.compile('matrix\\.([A-Za-z_][\\w-]*)')
_MATRIX_FROM_JSON: Final = re.compile('^\\$\\{\\{\\s*fromJSON\\(\\s*needs\\.(?P<job>[A-Za-z_][\\w-]*)\\.outputs\\.(?P<output>[A-Za-z_][\\w-]*)\\s*\\)\\s*\\}\\}$')
_STEP_OUTPUT: Final = re.compile('^\\$\\{\\{\\s*steps\\.(?P<step>[A-Za-z_][\\w-]*)\\.outputs\\.(?P<key>[A-Za-z_][\\w-]*)\\s*\\}\\}$')
_RUNNER_LABEL_LITERAL: Final = re.compile('["\'](?P<label>(?:ubuntu|windows|macos)-[0-9a-z][0-9a-z.\\-]*|self-hosted)["\']')
_RUNNER_LABEL_GROUP: Final = re.compile('\\[(?P<body>(?:\\s*["\\\'][A-Za-z0-9_.\\-]+["\\\']\\s*,?)+)\\]')
_GROUP_MEMBER: Final = re.compile('["\\\'](?P<label>[A-Za-z0-9_.\\-]+)["\\\']')
_HOSTED_IMAGE: Final = re.compile('^(?:ubuntu|windows|macos)-[0-9a-z][0-9a-z.\\-]*$')
...
def runner_targets(job: Mapping[str, Any], workflow: Mapping[str, Any]) -> list[object]:
    ...
    return _dp_or('dev/ci/workflow_runner_targets.py:152:or', lambda: targets, lambda: [UNRESOLVED_ZERO_TARGETS])
...
def _runtime_matrix_targets(expression: str, workflow: Mapping[str, Any]) -> list[object]:
    ...
    if step_match is None:
        return [unresolvable(f'{producer_name}.outputs.{output_name} is {output!r}, not a ${{{{ steps.<id>.outputs.<key> }}}} reference')]
    step_id = step_match.group('step')
    step = _dp_next('dev/ci/workflow_runner_targets.py:182:next', (candidate for candidate in _sequence(producer.get('steps')) if _mapping(candidate).get('id') == step_id), None)
    if step is None:
        return [unresolvable(f'job {producer_name!r} has no step {step_id!r} to emit the matrix')]
    labels = _runner_label_literals(str(_dp_get('dev/ci/workflow_runner_targets.py:188:get', _mapping(step), 'run', '')))
    return _dp_or('dev/ci/workflow_runner_targets.py:189:or', lambda: labels, lambda: [unresolvable(f'step {step_id!r} of {producer_name!r} names no runner label literal')])
