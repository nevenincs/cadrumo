...
from ..packaging._smoke_common import build_companion_wheels, build_sdist, clean_product_env, commit_defined_build_root, require_executable, resolve_work_dir, venv_bin_dir, venv_cadrumo_path, venv_python_path
...
_SHA256_RE: Final[re.Pattern[str]] = re.compile('^[0-9a-f]{64}$')
_RUNTIME_VERSION_RE: Final[re.Pattern[str]] = re.compile('^3\\.(?P<minor>[0-9]+)')
_OBSERVED_RUNTIME_VERSION_RE: Final[re.Pattern[str]] = re.compile('^3\\.(?P<minor>[0-9]+)(?:\\.[0-9]+)?(?:[._-]?(?:a|b|rc|dev)[0-9]+)?$')
_DEFAULT_BUILDER_PIN: Final[Path] = REPO_ROOT / '.python-version'
_MISSING_WHEEL_PATTERNS: Final[tuple[re.Pattern[str], ...]] = (re.compile('\\bno solution found\\b'), re.compile('\\bno matching distribution\\b'), re.compile('\\bno compatible wheels?\\b'), re.compile('\\bcould not find a version\\b'), re.compile('\\bno wheels? (?:are|were) available\\b'))
_REQUIRED_FOCUSED_TESTS: Final[frozenset[str]] = frozenset({'installed-package-behavior', 'installed-cadrumo-mcp-help'})
...
class CommandEvidence:
    ...
    def from_result(cls, result: CommandResult) -> CommandEvidence:
        ...
        return cls(argv=result.argv, cwd=result.cwd, started_at=result.started_at.isoformat(), completed_at=result.completed_at.isoformat(), exit_status=result.returncode, stdout_sha256=sha256_text(result.stdout), stderr_sha256=sha256_text(result.stderr))
...
class ProbeEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if self.mode == ProbeMode.BINARY.value and self.status == ProbeStatus.PASSED.value:
            ...
            if not re.fullmatch('3\\.[0-9]+', _dp_get('dev/ci/python_runtime_compatibility.py:230:get', self.dependency, 'wheelhouse_runtime', '')):
        ...
        if self.status == ProbeStatus.PASSED.value:
            ...
            if set(names) != _REQUIRED_FOCUSED_TESTS:
                raise CompatibilityProbeError('passing compatibility evidence must include the complete focused runtime test set')
...
def _cohort_lock_digest(cohort: Any) -> str:
    ...
    with zipfile.ZipFile(cohort.source_archive) as archive:
        try:
            ...
            raise CompatibilityProbeError('Python cohort source archive omits uv.lock', category='cohort-invalid') from exc
    if source_lock_digest != value:
        raise CompatibilityProbeError('Python cohort wheelhouse lock digest disagrees with its source archive', category='cohort-invalid')
    return value

def _builder_pin(repo_root: Path) -> str:
    """Return the exact release-builder interpreter identity, if declared."""
    try:
        value = (_DEFAULT_BUILDER_PIN if repo_root == REPO_ROOT else repo_root / '.python-version').read_text(encoding=_UTF_8).strip()
    except OSError as exc:
...
def _runtime_identity(python: Path, *, runtime_id: str, selector: str, stability: str, cwd: Path) -> dict[str, str]:
    ...
    code = "import json,platform,sys; print(json.dumps({'python': platform.python_version(), 'implementation': platform.python_implementation(), 'platform': sys.platform, 'machine': platform.machine()}, sort_keys=True))"
    ...
    if result.returncode != 0:
        raise CompatibilityProbeError(f'selected interpreter identity probe failed: {result.stderr.strip() or result.stdout.strip()}', category='runtime-probe-failed')
    try:
        identity = json.loads(result.stdout)
    except json.JSONDecodeError as exc:
        raise CompatibilityProbeError('selected interpreter identity was not JSON', category='runtime-probe-failed') from exc
    if not isinstance(identity, dict):
        raise CompatibilityProbeError('selected interpreter identity was not an object', category='runtime-probe-failed')
    ...
    if not isinstance(observed_python, str) or not isinstance(implementation, str):
        raise CompatibilityProbeError('selected interpreter identity omitted Python fields', category='runtime-probe-failed')
    if implementation != 'CPython':
        raise CompatibilityProbeError(f'compatibility matrix requires CPython, got {implementation!r}', category='implementation-unsupported')
    selector_match = _RUNTIME_VERSION_RE.match(selector)
    observed_match = _RUNTIME_VERSION_RE.match(observed_python)
    if selector_match is None or observed_match is None or selector_match.group('minor') != observed_match.group('minor'):
        raise CompatibilityProbeError(f'selected interpreter {observed_python!r} does not satisfy selector {selector!r}', category='runtime-identity-mismatch')
    return {'id': runtime_id, 'selector': selector, 'python': observed_python, 'implementation': implementation, 'stability': stability, 'platform': str(_dp_get('dev/ci/python_runtime_compatibility.py:358:get', identity, 'platform', '')), 'machine': str(_dp_get('dev/ci/python_runtime_compatibility.py:359:get', identity, 'machine', ''))}
...
def _isolated_environment(work_dir: Path, executable_dir: Path) -> dict[str, str]:
    ...
    for name in ('PYTHONPATH', 'PYTHONHOME', 'PYTHONUSERBASE', 'VIRTUAL_ENV', 'CONDA_PREFIX', 'CONDA_DEFAULT_ENV', 'UV_PROJECT_ENVIRONMENT'):
        _dp_pop('dev/ci/python_runtime_compatibility.py:375:pop', environment, name, None)
    environment['PYTHONNOUSERSITE'] = '1'
    environment['PYTHONDONTWRITEBYTECODE'] = '1'
    environment['PATH'] = str(executable_dir)
    environment['CADRUMO_COMPATIBILITY_WORK_DIR'] = str(work_dir.resolve())
    return environment
_RESOLVER_ENVIRONMENT: Final[tuple[str, ...]] = ('UV_DEFAULT_INDEX', 'UV_EXTRA_INDEX_URL', 'UV_FIND_LINKS', 'UV_INDEX', 'UV_INDEX_URL', 'UV_NATIVE_TLS', 'UV_NO_INDEX', 'UV_OFFLINE', 'UV_REQUEST_TIMEOUT', 'PIP_EXTRA_INDEX_URL', 'PIP_FIND_LINKS', 'PIP_INDEX_URL', 'PIP_NO_INDEX', 'PIP_TRUSTED_HOST')
...
def _binary_environment() -> dict[str, str]:
    ...
    for name in _RESOLVER_ENVIRONMENT:
        _dp_pop('dev/ci/python_runtime_compatibility.py:412:pop', environment, name, None)
    for name in ('PYTHONPATH', 'PYTHONHOME', 'VIRTUAL_ENV', 'UV_PROJECT_ENVIRONMENT'):
        _dp_pop('dev/ci/python_runtime_compatibility.py:414:pop', environment, name, None)
    return environment
...
def _wheelhouse_platform(runtime: Mapping[str, str]) -> str:
    ...
    machine = _dp_get('dev/ci/python_runtime_compatibility.py:421:get', runtime, 'machine', '').lower().replace('-', '_')
    ...
    raise CompatibilityProbeError(f'sealed runtime wheelhouse has no target for {operating_system!r}/{machine!r}', category='platform-unsupported')

def _runtime_minor(runtime: Mapping[str, str]) -> str:
    """Return the observed interpreter's canonical ``3.N`` wheelhouse key."""
    observed = _dp_get('dev/ci/python_runtime_compatibility.py:439:get', runtime, 'python', '')
    match = _OBSERVED_RUNTIME_VERSION_RE.fullmatch(observed)
    if match is None:
        raise CompatibilityProbeError(f'selected interpreter reported an invalid Python version: {observed!r}', category='runtime-identity-mismatch')
    return f"3.{int(match.group('minor'))}"

def _select_runtime_wheelhouse(manifest: Mapping[str, Any], runtime: Mapping[str, str]) -> tuple[str, Mapping[str, Any]]:
    ...
    if not isinstance(entry, Mapping):
        raise CompatibilityProbeError(f'sealed runtime wheelhouse has no entry for Python {python_minor}', category='missing-wheel')
    status = entry.get('status')
    if status == 'missing-wheel':
        missing = entry.get('missing')
        details = '; '.join((f"{item.get('distribution')} ({item.get('platform')}, {item.get('requirement')})" for item in missing if isinstance(item, Mapping))) if isinstance(missing, list) else 'unattributed dependency closure'
        raise CompatibilityProbeError(f'sealed runtime wheelhouse for Python {python_minor} is missing wheels: {details}', category='missing-wheel')
    if status != 'ready':
        raise CompatibilityProbeError(f'sealed runtime wheelhouse entry for Python {python_minor} is not ready', category='cohort-invalid')
    return (python_minor, entry)

def _binary_wheel_targets(wheelhouse_dir: Path, manifest: Mapping[str, Any], *, platform_target: str) -> tuple[str, ...]:
    ...
    if not isinstance(rows, Mapping) or not rows:
        raise CompatibilityProbeError(f'sealed runtime wheelhouse has no dependency rows for {platform_target!r}', category='cohort-invalid')
    if not isinstance(wheels, Mapping) or not wheels:
        raise CompatibilityProbeError('sealed runtime wheelhouse declares no wheel records', category='cohort-invalid')
    targets: list[str] = []
    for distribution, filename in sorted(rows.items(), key=lambda item: str(item[0])):
        if not isinstance(distribution, str) or not distribution or (not isinstance(filename, str)):
            raise CompatibilityProbeError(f'sealed runtime wheelhouse has an invalid {platform_target!r} row', category='cohort-invalid')
        record = wheels.get(filename)
        if not isinstance(record, Mapping) or record.get('distribution') != distribution:
            raise CompatibilityProbeError(f'sealed runtime wheelhouse target swaps distribution bytes: {platform_target!r}/{distribution!r}', category='cohort-invalid')
        expected_digest = record.get('sha256')
        expected_size = record.get('size')
        if not isinstance(expected_digest, str) or _SHA256_RE.fullmatch(expected_digest) is None or (not isinstance(expected_size, int)) or (expected_size < 0):
            raise CompatibilityProbeError(f'sealed runtime wheelhouse wheel record is invalid: {filename!r}', category='cohort-invalid')
        try:
            path = (wheelhouse_dir / filename).resolve(strict=True)
        except OSError as exc:
            raise CompatibilityProbeError(f'sealed runtime wheelhouse omitted {filename!r}', category='cohort-invalid') from exc
        if path.parent != wheelhouse_dir.resolve():
            raise CompatibilityProbeError(f'sealed runtime wheelhouse wheel escapes its extraction directory: {filename!r}', category='cohort-invalid')
        if path.stat().st_size != expected_size or sha256_path(path) != expected_digest:
            raise CompatibilityProbeError(f'sealed runtime wheelhouse wheel bytes drifted: {filename!r}', category='cohort-invalid')
...
def _venv(uv: str, *, repo_root: Path, work_dir: Path, selector: str) -> tuple[Path, list[CommandEvidence]]:
    ...
    environment = clean_product_env()
    for name in ('PYTHONPATH', 'PYTHONHOME', 'VIRTUAL_ENV', 'UV_PROJECT_ENVIRONMENT'):
        _dp_pop('dev/ci/python_runtime_compatibility.py:564:pop', environment, name, None)
    venv = work_dir / 'venv'
    ...
    if result.returncode != 0:
        raise CompatibilityProbeError(f'target virtualenv creation failed: {result.stderr.strip() or result.stdout.strip()}', category='runtime-provisioning-failed')
    return (venv, [command])

def _install(uv: str, *, repo_root: Path, work_dir: Path, venv: Path, artifacts: tuple[tuple[str, Path], ...], mode: ProbeMode, wheelhouse_dir: Path | None=None, wheelhouse_manifest: Mapping[str, Any] | None=None, wheelhouse_platform: str | None=None) -> tuple[list[CommandEvidence], PythonRuntimeDependencyStatus, str | None]:
    ...
    if mode is ProbeMode.BINARY:
        if wheelhouse_dir is None or wheelhouse_manifest is None or wheelhouse_platform is None:
            raise CompatibilityProbeError('binary mode requires an extracted sealed runtime wheelhouse', category='cohort-invalid')
        try:
            wheelhouse = wheelhouse_dir.resolve(strict=True)
        except OSError as exc:
            raise CompatibilityProbeError(f'sealed runtime wheelhouse extraction is unavailable: {wheelhouse_dir}', category='cohort-invalid') from exc
        if not wheelhouse.is_dir():
            raise CompatibilityProbeError(f'sealed runtime wheelhouse extraction is not a directory: {wheelhouse}', category='cohort-invalid')
        targets += _binary_wheel_targets(wheelhouse, wheelhouse_manifest, platform_target=wheelhouse_platform)
        argv.extend(('--offline', '--no-index', '--find-links', str(wheelhouse), '--only-binary', ':all:', '--require-hashes'))
    argv.extend(targets)
    ...
    check = run_command((uv, 'pip', 'check', '--python', str(python)), cwd=repo_root, environment=environment)
...
def _installed_probe(venv: Path, *, work_dir: Path) -> tuple[list[CommandEvidence], dict[str, bool]]:
    ...
    for root in (venv / 'Lib' / 'site-packages', venv / 'lib'):
        if root.is_dir():
            install_root = root if root.name == 'site-packages' else _dp_next('dev/ci/python_runtime_compatibility.py:665:next', (path for path in root.rglob('site-packages') if path.is_dir()), None)
            if install_root is not None:
                break
    if install_root is None:
        install_root = venv / ('Lib' / 'site-packages' if os.name == 'nt' else 'lib')
    code = "import json,sys; from pathlib import Path; import cadrumo; origins=[str(Path(m.__file__).resolve()) for n,m in sys.modules.items() if (n == 'cadrumo' or n.startswith('cadrumo.')) and getattr(m,'__file__',None)]; root=Path(sys.argv[1]).resolve(); assert origins and all(Path(p).is_relative_to(root) for p in origins), origins; assert not any(n == 'dev' or n.startswith('dev.') for n in sys.modules); print(json.dumps({'origins_inside':True,'checkout_imports_removed':True}, sort_keys=True))"
    env = _isolated_environment(work_dir, venv_bin_dir(venv))
    result = run_command((str(python), '-I', '-W', 'error::DeprecationWarning', '-c', code, str(install_root)), cwd=work_dir, environment=env)
    commands = [CommandEvidence.from_result(result)]
    if result.returncode != 0:
        raise CompatibilityProbeError(f'installed import probe failed: {result.stderr.strip() or result.stdout.strip()}', category='import-probe-failed')
    cli = venv_cadrumo_path(venv)
    cli_result = run_command((str(cli), '--version'), cwd=work_dir, environment=env)
    commands.append(CommandEvidence.from_result(cli_result))
    if cli_result.returncode != 0:
        raise CompatibilityProbeError(f'installed CLI probe failed: {cli_result.stderr.strip() or cli_result.stdout.strip()}', category='cli-probe-failed')
...
def _run_focused_test(name: str, argv: Sequence[str], *, cwd: Path, environment: Mapping[str, str], stdout_marker: str | None=None) -> FocusedTestEvidence:
    ...
    try:
        ...
        raise CompatibilityProbeError(f'focused runtime test timed out after 120 seconds: {name}', category='focused-test-timeout') from exc
    detail: str | None = None
    ...
    return FocusedTestEvidence(name=name, status=status, command=CommandEvidence.from_result(result), detail=detail)

def _focused_runtime_tests(venv: Path, *, work_dir: Path) -> tuple[tuple[FocusedTestEvidence, ...], list[CommandEvidence], str | None]:
    ...
    behavior_code = "import json\nimport cadrumo\nimport cadrumo_harness.mcp as mcp\nfrom cadrumo.core.toml import parse_toml_text\nparsed = parse_toml_text('value = 42\\n', error_factory=ValueError)\nassert parsed == {'value': 42}, parsed\nassert cadrumo.__version__\nassert callable(mcp.main) and callable(mcp.build_server)\nprint(json.dumps({'runtime_behavior_ok': True}, sort_keys=True))\n"
    tests = (_run_focused_test('installed-package-behavior', (str(python), '-I', '-W', 'error::DeprecationWarning', '-c', behavior_code), cwd=work_dir, environment=environment, stdout_marker='runtime_behavior_ok'), _run_focused_test('installed-cadrumo-mcp-help', (str(_mcp_executable_path(venv)), '--help'), cwd=work_dir, environment=environment, stdout_marker='usage:'))
...
def _load_binary_artifacts(cohort_dir: Path, *, repo_root: Path) -> tuple[Any, tuple[tuple[str, Path], ...], str, str, str | None]:
    ...
    if release_manifest.is_file():
        ...
        if builder_python != expected:
            raise CompatibilityProbeError(f'sealed release cohort builder drifted: expected {expected!r}, got {builder_python!r}', category='builder-identity-mismatch')
    artifacts = (('cadrumo', cohort.root_wheel), ('cadrumo-data-manuals', cohort.manuals_wheel), ('cadrumo-data-official', cohort.official_wheel))
    digests = {name: cohort.sha256[key] for name, key in (('cadrumo', 'cadrumo'), ('cadrumo-data-manuals', 'cadrumo-data-manuals'), ('cadrumo-data-official', 'cadrumo-data-official'))}
    return (cohort, artifacts, lock_sha256, artifact_map_digest(digests), builder_python)

def _source_artifacts(repo_root: Path, work_dir: Path) -> tuple[tuple[tuple[str, Path], ...], str, dict[str, str], str | None]:
    ...
    artifacts = (('cadrumo', sdist), ('cadrumo-data-manuals', manuals), ('cadrumo-data-official', official))
    digests = {name: sha256_path(path) for name, path in artifacts}
    commit_result = run_command((require_executable('git'), 'rev-parse', 'HEAD'), cwd=build_root, environment=clean_product_env())
...
def run_probe(*, mode: ProbeMode | str, python: str, runtime_id: str, stability: str='stable', repo_root: Path=REPO_ROOT, work_dir: Path, cohort_dir: Path | None=None) -> ProbeEvidence:
    ...
    dependency = {'status': PythonRuntimeDependencyStatus.FAILED.value, 'detail': 'probe did not reach installation'}
    if selected_mode is ProbeMode.BINARY:
        dependency.update({'source': 'sealed-runtime-wheelhouse', 'wheelhouse_platform': 'unresolved', 'wheelhouse_runtime': 'unresolved'})
    isolation = {'checkout_imports_removed': False, 'ambient_product_executables_removed': False}
    focused_tests: tuple[FocusedTestEvidence, ...] = ()
    failure: dict[str, str] | None = None
    runtime: dict[str, str] = {'id': runtime_id, 'selector': python, 'python': 'unknown', 'implementation': 'unknown', 'stability': stability, 'platform': platform.system(), 'machine': platform.machine()}
    try:
        if selected_mode is ProbeMode.SOURCE:
            ...
            cohort, artifacts, lock_sha256, artifact_sha256, builder_python = _load_binary_artifacts(cohort_dir, repo_root=repo_root)
            artifact_digests = {name: cohort.sha256[key] for name, key in (('cadrumo', 'cadrumo'), ('cadrumo-data-manuals', 'cadrumo-data-manuals'), ('cadrumo-data-official', 'cadrumo-data-official'))}
            wheelhouse_bundle = load_runtime_wheelhouse(cohort.runtime_wheelhouse, expected_lock_sha256=lock_sha256)
        ...
        runtime = _runtime_identity(venv_python_path(venv), runtime_id=runtime_id, selector=python, stability=stability, cwd=work_dir)
        if selected_mode is ProbeMode.BINARY:
            ...
            if wheelhouse_bundle is None:
                raise CompatibilityProbeError('binary mode did not load its sealed runtime wheelhouse', category='cohort-invalid')
            wheelhouse_runtime, selected_wheelhouse = _select_runtime_wheelhouse(wheelhouse_bundle.manifest, runtime)
            dependency['wheelhouse_runtime'] = wheelhouse_runtime
            wheelhouse_dir = work_dir / f'runtime-wheelhouse-{wheelhouse_runtime}'
            extract_runtime_wheelhouse(wheelhouse_bundle.archive, wheelhouse_dir, python_version=wheelhouse_runtime)
            wheelhouse_manifest = selected_wheelhouse
            dependency['wheelhouse_platform'] = wheelhouse_platform
        install_commands, dependency_status, dependency_detail = _install(uv, repo_root=repo_root, work_dir=work_dir, venv=venv, artifacts=artifacts, mode=selected_mode, wheelhouse_dir=wheelhouse_dir, wheelhouse_manifest=wheelhouse_manifest, wheelhouse_platform=wheelhouse_platform)
        commands.extend(install_commands)
        dependency = {**dependency, 'status': dependency_status.value, 'detail': _dp_or('dev/ci/python_runtime_compatibility.py:1009:or', lambda: dependency_detail, lambda: 'resolved')}
        if dependency_status is not PythonRuntimeDependencyStatus.RESOLVED:
            raise CompatibilityProbeError(_dp_or('dev/ci/python_runtime_compatibility.py:1013:or', lambda: dependency_detail, lambda: 'dependency installation failed'), category=dependency_status.value)
    ...
    return ProbeEvidence(schema=_SCHEMA, runtime=runtime, mode=selected_mode.value, status=status, stability=stability, lock_sha256=lock_sha256, artifact_sha256=artifact_sha256, artifact_digests=artifact_digests, source_commit=source_commit, cohort_manifest_sha256=cohort_manifest_sha256, builder_python=builder_python, dependency=dependency, isolation=isolation, commands=tuple(commands), focused_tests=focused_tests, failure=failure, observed_at=datetime.now(UTC).isoformat())
...
def write_probe_evidence(path: Path, evidence: ProbeEvidence) -> Path:
    ...
    temporary.write_text(json.dumps(evidence.to_dict(), ensure_ascii=False, indent=2, sort_keys=True) + '\n', encoding=_UTF_8, newline='\n')
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--stability', '--phase', dest='stability', choices=('stable', 'prerelease'), default='stable')
...
def main(argv: Sequence[str] | None=None) -> int:
    ...
    mode = _dp_or('dev/ci/python_runtime_compatibility.py:1089:or', lambda: args.mode_option, lambda: args.mode_positional)
    if mode is None:
        print('compatibility probe requires --mode source|binary (or a mode positional)', file=sys.stderr)
        return 2
    repo_root = args.repo_root.resolve()
    selector = _dp_or('dev/ci/python_runtime_compatibility.py:1094:or', lambda: args.python, lambda: _builder_pin(repo_root))
    runtime_id = _dp_or('dev/ci/python_runtime_compatibility.py:1095:or', lambda: args.runtime_id, lambda: 'cp' + selector.replace('.', '').replace('-', '').replace('+', ''))
    work_dir = _dp_or('dev/ci/python_runtime_compatibility.py:1096:or', lambda: args.work_dir, lambda: repo_root / 'var' / 'python-runtime-compatibility' / f'{runtime_id}-{mode}')
    try:
        work_dir = resolve_work_dir(repo_root, str(work_dir))
        evidence = run_probe(mode=mode, python=selector, runtime_id=runtime_id, stability=args.stability, repo_root=repo_root, work_dir=work_dir, cohort_dir=args.cohort_dir)
        destination = _dp_or('dev/ci/python_runtime_compatibility.py:1108:or', lambda: args.evidence, lambda: work_dir / 'compatibility-evidence.json')
...
__all__ = ['CommandEvidence', 'CompatibilityProbeError', 'FocusedTestEvidence', 'FocusedTestStatus', 'ProbeEvidence', 'ProbeMode', 'ProbeStatus', 'PythonRuntimeDependencyStatus', 'main', 'run_probe', 'write_probe_evidence']
