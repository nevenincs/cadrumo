...
from cadrumo.domain.iva_compensation.carry_forward import IvaCompensationCarryForwardLot, IvaCompensationExpiryReviewState
from cadrumo.entrypoints.cli.app_live_iva_remote_state_composition import carry_forward_lot_row, taxpayer_ref
...
def test_a_lot_carrying_a_subject_projects_a_pseudonymised_digest_ref() -> None:
    ...
    """
    row = carry_forward_lot_row(_lot(_SYNTHETIC_NIF))
...
def test_a_subjectless_lot_projects_an_explicit_marker_not_a_digest() -> None:
    ...
    """
    row = carry_forward_lot_row(_lot(None))
...
def test_the_absent_marker_is_shared_by_subjectless_lots_but_never_by_a_real_subject() -> None:
    ...
    absent_ref = carry_forward_lot_row(_lot(None)).taxpayer_ref
    other_absent_ref = carry_forward_lot_row(_lot(None)).taxpayer_ref
    populated_ref = carry_forward_lot_row(_lot(_SYNTHETIC_NIF)).taxpayer_ref
...
def test_a_subject_that_normalises_to_nothing_takes_the_absent_exit(blank: str) -> None:
    ...
    """
    ref = taxpayer_ref(blank)

    assert not ref.startswith("sha256:")
    assert ref == taxpayer_ref(None)
...
def test_the_blank_subject_is_reachable_through_the_wallet_observation() -> None:
    ...
    assert taxpayer_ref(observation.taxpayer_nif) == taxpayer_ref(None)
