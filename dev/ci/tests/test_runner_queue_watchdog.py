...
from ..runner_queue_watchdog import JobView, Verdict, _epoch_of, classify, confirm_unschedulable, occupied_label_keys, parse_jobs
...
_NOW_EPOCH_LITERAL: Final = 1785934355.0
...
def _payload() -> dict[str, Any]:
    ...
    """
    return {'jobs': [{'name': 'Cadrumo probe / unschedulable Windows lane', 'status': 'queued', 'labels': ['self-hosted', 'Windows', 'X64'], 'created_at': '2026-08-05T12:50:25Z', 'started_at': '2026-08-05T12:50:25Z', 'runner_name': None}, {'name': 'Cadrumo probe / schedulable control B', 'status': 'queued', 'labels': ['self-hosted', 'Linux', 'X64'], 'created_at': '2026-08-05T12:50:25Z', 'started_at': '2026-08-05T12:50:25Z', 'runner_name': None}, {'name': 'Cadrumo probe / schedulable control A', 'status': 'in_progress', 'labels': ['self-hosted', 'Linux', 'X64'], 'created_at': '2026-08-05T12:50:25Z', 'started_at': '2026-08-05T12:50:27Z', 'runner_name': 'cadrumo-linux-x64'}, {'name': 'Cadrumo probe / queue watchdog', 'status': 'queued', 'labels': ['self-hosted', 'Linux', 'X64'], 'created_at': '2026-08-05T12:50:25Z', 'started_at': '2026-08-05T12:50:25Z', 'runner_name': None}]}

def _classify(payload: dict[str, Any], **overrides: Any) -> tuple[Any, ...]:
    kwargs: dict[str, Any] = {'now_epoch': _NOW, 'threshold_seconds': 120.0, 'watchdog_job_name': 'Cadrumo probe / queue watchdog'}
...
def test_queued_since_is_parsed_as_utc_not_local_time() -> None:
    ...
    for stamp in ('2026-01-15T03:00:00Z', '2026-08-05T12:52:35Z', '2026-12-31T23:59:59Z'):
        ...
        assert _epoch_of(stamp) == expected, f'{stamp} parsed to {_epoch_of(stamp)}, not the UTC truth {expected} (delta {_epoch_of(stamp) - expected:+.0f}s) — the parse is reading local time'
...
def test_a_freshly_queued_job_does_not_fire_on_a_timezone_offset() -> None:
    ...
    payload = {'jobs': [{'name': 'Cadrumo probe / just queued', 'status': 'queued', 'labels': ['self-hosted', 'Windows', 'X64'], 'created_at': just_now, 'started_at': just_now, 'runner_name': None}]}
    verdicts = classify(parse_jobs(payload), now_epoch=time.time(), threshold_seconds=300.0, watchdog_job_name='')
...
def _unschedulable(name: str) -> Verdict:
    ...
    return Verdict(job_name=name, label_key=('Windows', 'X64', 'self-hosted'), waited_seconds=3992.0, unschedulable=True, reason='no job anywhere in this repository is running on this label set')
...
def test_nothing_fires_before_the_threshold() -> None:
    ...
    assert _classify(_payload(), threshold_seconds=100000.0) == ()
...
def _workflow_documents() -> list[tuple[Path, dict[str, Any]]]:
    ...
    assert len(paths) >= _MINIMUM_WORKFLOW_DOCUMENTS, f'only {len(paths)} workflow(s) found under {_WORKFLOWS_DIR}; the gates that iterate this census assert nothing at all over a walk that reached less than the lanes this repository runs'
...
def test_the_gate_refuses_a_new_unwatched_lane(tmp_path: Path) -> None:
    ...
    """
    (tmp_path / 'unwatched.yml').write_text('name: Cadrumo Unwatched\non: workflow_dispatch\njobs:\n  lonely-windows:\n    runs-on: [self-hosted, Windows, X64]\n    steps: []\n', encoding='utf-8')
...
def test_a_runtime_matrix_on_hosted_images_needs_no_watchdog() -> None:
    ...
    assert runner_targets(document['jobs']['fanned'], document) == ['ubuntu-latest', 'macos-latest', 'windows-latest']
...
def test_every_watchdog_job_invokes_the_shared_module() -> None:
    ...
    for path, document in _workflow_documents():
        ...
        assert job.get('runs-on') is not None and tuple(sorted(job['runs-on'])) == _WATCHDOG_LABELS, f'{path.name}: the watchdog must run on a lane the fleet can always serve'
...
def test_a_watchdog_whose_invocation_is_commented_out_is_not_invoking_it() -> None:
    ...
    """
    job = next(((document.get('jobs') or {})[_WATCHDOG_JOB_ID] for _, document in _workflow_documents() if _WATCHDOG_JOB_ID in (document.get('jobs') or {})))
    ...
    assert _WATCHDOG_MODULE in ''.join((str(_dp_get('dev/ci/tests/test_runner_queue_watchdog.py:501:get', step, 'run', '')) for step in commented['steps']))
...
def test_the_watchdog_is_created_no_later_than_the_lanes_it_watches() -> None:
    ...
    """
    for path, document in _workflow_documents():
        jobs = _dp_or('dev/ci/tests/test_runner_queue_watchdog.py:529:or', lambda: document.get('jobs'), lambda: {})
        ...
        for job_name in _off_watchdog_lane_jobs(document):
            ...
            assert watched_needs <= watchdog_needs, f"{path.name}: '{job_name}' is gated on {sorted(watched_needs - watchdog_needs)}, which the watchdog does not wait for — the watchdog would begin (and could finish) its watch window before that job is created"
...
def test_the_needs_parity_gate_refuses_an_ungated_watchdog(tmp_path: Path) -> None:
    ...
    """
    (tmp_path / 'late.yml').write_text('name: Cadrumo Late\non: workflow_dispatch\njobs:\n  runner-queue-watchdog:\n    runs-on: [self-hosted, Linux, X64]\n    steps: []\n  build:\n    runs-on: [self-hosted, Linux, X64]\n    steps: []\n  late-macos:\n    needs: build\n    runs-on: [self-hosted, macOS, ARM64]\n    steps: []\n', encoding='utf-8')
    document = yaml.safe_load((tmp_path / 'late.yml').read_text(encoding='utf-8'))
    jobs = document['jobs']
    assert _off_watchdog_lane_jobs(document) == ['late-macos']
    assert not _needs_of(jobs['late-macos']) <= _needs_of(jobs[_WATCHDOG_JOB_ID]), 'the gate must reject a watchdog that does not wait for a dependency its watched job waits for'
...
def test_every_watchdog_job_can_read_jobs_and_cancel_the_run() -> None:
    ...
    for path, document in _workflow_documents():
        if _WATCHDOG_JOB_ID not in (document.get('jobs') or {}):
            continue
        assert granted_level(document, _WATCHDOG_JOB_ID, 'actions') == 'write', f'{path.name}: watchdog needs actions: write to cancel'