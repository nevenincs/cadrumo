...
def _document() -> dict[str, Any]:
    ...
    return {
        ...
        "jobs": {
            "watchdog": {"permissions": {"actions": "write", "contents": "read"}, "steps": []},
            "acquisition": {"permissions": {"actions": "read", "contents": "read"}, "steps": []},
            "inheritor": {"steps": []},
        },
    }
...
def test_a_job_block_replaces_the_workflow_block_rather_than_merging() -> None:
    ...
    # contents:read is declared at workflow level and the job still does not hold it.
...
def test_scalar_shorthands_resolve_to_the_level_they_grant_everywhere() -> None:
    ...
    document = {
        ...
        "jobs": {"wide": {"permissions": "write-all", "steps": []}, "narrow": {"steps": []}},
    }
...
def test_jobs_granting_names_the_write_holder_the_workflow_text_hides() -> None:
    ...
    document = _document()
    assert document["permissions"]["actions"] == "read"  # the reading a declared-text gate takes
    assert jobs_granting(document, "actions", "write") == ("watchdog",)
    assert jobs_granting(document, "contents", "write") == ()

    # A job added later that takes the capability is named, where the declared
    # workflow map is unchanged and a gate reading it stays green.
