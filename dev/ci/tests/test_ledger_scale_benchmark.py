...
from cadrumo.application.aggregation import aggregate_iva_ledger_observations_from_repositories, aggregate_renta_ledger_expenses_from_repositories
...
_TOTAL_TRANSACTIONS = 30000
_FILING_YEARS = 10
_TRANSACTIONS_PER_YEAR = _TOTAL_TRANSACTIONS // _FILING_YEARS
_FIRST_YEAR = 2021
_LAST_YEAR = _FIRST_YEAR + _FILING_YEARS - 1
...
_M130_MANUAL_INPUTS: dict[CasillaId, Decimal] = {validated_casilla_id('06', surface='bench M130 retenciones'): Decimal('0'), validated_casilla_id('08', surface='bench M130 agrarian volume'): Decimal('0'), validated_casilla_id('10', surface='bench M130 agrarian withheld'): Decimal('0'), validated_casilla_id('16', surface='bench M130 home deduction'): Decimal('0'), validated_casilla_id('18', surface='bench M130 prior return result'): Decimal('0')}
_M100_ANNUAL_PERIOD = '0A'
_M100_ACTIVIDAD_ECONOMICA_NET_INCOME_CASILLA = validated_casilla_id('0224', surface='bench M100 net income')
...
_PARTITION_LOG_MARKERS = ('partitioned transaction catalogue via date index', 'partitioned transaction catalogue via full-scan fallback')

def _raw(idx: int, *, booked: date) -> RawTransaction:
    return RawTransaction(provider_transaction_id=f'bench-row-{idx:06d}', booked_date=booked, value_date=booked, amount=Decimal('121.00'), currency='EUR', counterparty='Proveedor Escala SL', description=f'scale benchmark row {idx}', provenance=RawProvenance(source_path=Path(__file__), source_sha256='e' * 64, source_row_index=idx + 1, source_format=SourceFormat.CSV, ingested_at=datetime(booked.year, booked.month, booked.day, 12, 0, tzinfo=UTC), provider_name='scale benchmark provider'), raw_fields={'Concepto': f'row {idx}'})
...
def _synthetic_transaction(idx: int, *, booked: date) -> Transaction:
    ...
    return Transaction.model_validate({'raw': _raw(idx, booked=booked), 'direction': direction, 'group_label': None, 'source_jurisdiction': 'ES', 'business_classification': BusinessClassification.BUSINESS, 'business_pct': None, 'purchase_invoice_evidence_id': None, 'category_id': 'material_oficina', 'taxable_base': Decimal('100.00'), 'iva_rate': Decimal('0.21'), 'iva_amount': Decimal('21.00'), 'lifecycle_state': TransactionLifecycleState.ACTIVE, 'classified_at': datetime(booked.year, booked.month, booked.day, 13, 0, tzinfo=UTC), 'classified_by': 'manual'})
...
def _seed_prior_year_m130_minoracion(objects: SecureObjectRepository) -> None:
    ...
    for target_year in _M130_DIAGNOSTIC_YEARS:
        ...
        observation_repo.save(observation_repo.prepare_observation_envelope(RegistryModeloObservation(modelo='100', filing_year=prior_year, period=_M100_ANNUAL_PERIOD, observations=registry_grounded_observations(modelo='100', filing_year=prior_year, period=_M100_ANNUAL_PERIOD, casilla_values={_M100_ACTIVIDAD_ECONOMICA_NET_INCOME_CASILLA: _PRIOR_YEAR_NET_INCOME, _M100_RENDIMIENTO_SOURCE_1479_CASILLA: Decimal('0'), _M100_RENDIMIENTO_SOURCE_1553_CASILLA: Decimal('0'), _M100_RENDIMIENTO_SOURCE_1577_CASILLA: Decimal('0'), _M100_BASE_LIQUIDABLE_NEGATIVA_GENERAL_CASILLA: Decimal('0')})), source_kind='app_filing', captured_at=datetime(target_year, 4, 6, 12, 0, tzinfo=UTC)))
...
def _seed_taxpayer_profile() -> None:
    ...
    """
    record = UserProfileRecord(profile_id=_BUCKET_ID, setup_state=ProfileSetupState.COMPLETE, facts=(UserProfileFact(path='identity.tax_id', value=_TAX_ID), UserProfileFact(path='identity.name', value='Scale'), UserProfileFact(path='identity.surnames', value='Bench Tester'), UserProfileFact(path='activities.description', value='design services'), UserProfileFact(path='tax_residence.ccaa', value='madrid'), UserProfileFact(path='tax_residence.jurisdiction_scope', value='common_regime'), UserProfileFact(path='iva.regime', value='GENERAL'), UserProfileFact(path='iva.m303_regime_composition', value='general'), UserProfileFact(path='iva.redeme_enrolled', value=False), UserProfileFact(path='iva.cash_accounting_regime_enrolled', value=False), UserProfileFact(path='iva.voluntary_sii_enrolled', value=False), UserProfileFact(path='iva.hydrocarbon_deposit_advance_payment_deduction_entitled', value=False), UserProfileFact(path='taxpayer_type.entity_type', value='natural_person'), UserProfileFact(path='taxpayer_type.irpf_income_categories', value='actividad_economica'), UserProfileFact(path='irpf.estimation_regime', value='directa_normal'), UserProfileFact(path='censo.activity_start_date', value=date(2020, 1, 1))), created_at=datetime(_FIRST_YEAR, 1, 1, tzinfo=UTC), updated_at=datetime(_FIRST_YEAR, 1, 1, tzinfo=UTC))
...
def _partition_log_messages(records: Iterable[logging.LogRecord]) -> tuple[str, ...]:
    ...
    return tuple((message for record in records for message in (record.getMessage(),) if any((marker in message for marker in _PARTITION_LOG_MARKERS))))
...
def scale_bucket() -> Iterator[SecureObjectRepository]:
    ...
    """
    import tempfile
    with tempfile.TemporaryDirectory(prefix='cadrumo-scale-bench-') as tmp_dir, isolated_runtime_profile(tmp_path=Path(tmp_dir), bucket_id=_BUCKET_ID) as profile:
...
def test_scale_fixture_seeds_the_documented_volume(scale_bucket: SecureObjectRepository) -> None:
    ...
    assert len(catalogue) == _TOTAL_TRANSACTIONS, f'expected {_TOTAL_TRANSACTIONS} seeded transactions, found {len(catalogue)}'
...
def test_ledger_read_reports_full_catalogue_latency(scale_bucket: SecureObjectRepository) -> None:
    ...
    p95 = _p95(samples)
    print(f'\n[bench] ledger_read_diagnostic: n={_DIAGNOSTIC_SAMPLE_COUNT} p95={p95:.3f}s mean={statistics.mean(samples):.3f}s min={min(samples):.3f}s max={max(samples):.3f}s budget_scope=out_of_scope_full_catalogue_read')
...
def test_annual_renta_aggregation_reports_full_scan_latency(scale_bucket: SecureObjectRepository) -> None:
    ...
    for _ in range(_DIAGNOSTIC_SAMPLE_COUNT):
        started = time.perf_counter()
        result = aggregate_renta_ledger_expenses_from_repositories(bucket_id=_BUCKET_ID, period=period, transaction_repository=tx_repo, invoice_repository=invoice_repo, profile_year=_CATEGORY_PROFILE_YEAR, prorrata_register_repository=ProrrataRegisterRepository(bucket_id=_BUCKET_ID))
        samples.append(time.perf_counter() - started)
        assert isinstance(result.casilla_values, Mapping)
    p95 = _p95(samples)
    print(f'\n[bench] annual_renta_aggregation_diagnostic: n={_DIAGNOSTIC_SAMPLE_COUNT} p95={p95:.3f}s mean={statistics.mean(samples):.3f}s min={min(samples):.3f}s max={max(samples):.3f}s budget_scope=out_of_scope_pending_invoice_date_key')
...
def test_single_transaction_save_reports_write_path_latency(scale_bucket: SecureObjectRepository) -> None:
    ...
    target_filing_date = _dp_or('dev/ci/tests/test_ledger_scale_benchmark.py:504:or', lambda: target.raw.value_date, lambda: target.raw.booked_date)
    ...
    for _ in range(_DIAGNOSTIC_SAMPLE_COUNT):
        started = time.perf_counter()
        payload_hashes = tuple((sha256_hex(repo._serialise_transaction(transaction)) for transaction in original_catalogue.transactions.values()))
    ...
    print(f'\n[bench] transaction_save_namespace_hash_scan: n={_DIAGNOSTIC_SAMPLE_COUNT} p95={namespace_p95:.3f}s mean={statistics.mean(namespace_hash_samples):.3f}s min={min(namespace_hash_samples):.3f}s max={max(namespace_hash_samples):.3f}s namespace={TX_BUCKET_NAMESPACE}')
    print(f'[bench] transaction_save_serialize_hash_all_rows: n={_DIAGNOSTIC_SAMPLE_COUNT} rows={_TOTAL_TRANSACTIONS} p95={serialise_hash_p95:.3f}s mean={statistics.mean(serialise_hash_samples):.3f}s min={min(serialise_hash_samples):.3f}s max={max(serialise_hash_samples):.3f}s')
    print(f'[bench] single_transaction_save: n={_DIAGNOSTIC_SAMPLE_COUNT} rows={_TOTAL_TRANSACTIONS} changed_rows=1 p95={save_p95:.3f}s mean={statistics.mean(save_samples):.3f}s min={min(save_samples):.3f}s max={max(save_samples):.3f}s serialize_hash_p95={serialise_hash_p95:.3f}s namespace_hash_scan_p95={namespace_p95:.3f}s')
...
def quarterly_iva_samples(scale_bucket: SecureObjectRepository) -> _QuarterlyIvaSamples:
    ...
    try:
        for sample_index in range(_BUDGET_SAMPLE_COUNT):
            ...
            wall_started = time.perf_counter()
            cpu_started = time.process_time()
            partitioned_result = aggregate_iva_ledger_observations_from_repositories(bucket_id=_BUCKET_ID, period=period, transaction_repository=tx_repo, prorrata_register_repository=ProrrataRegisterRepository(bucket_id=_BUCKET_ID), investment_asset_register=BienesInversionIvaRegister(), investment_asset_profile_id=_BUCKET_ID)
    ...
    return _QuarterlyIvaSamples(partitioned_wall=tuple(partitioned_wall), partitioned_cpu=tuple(partitioned_cpu), paired_partitioned_wall=tuple(paired_partitioned_wall), paired_partitioned_cpu=tuple(paired_partitioned_cpu), full_scan_wall=tuple(full_scan_wall), full_scan_cpu=tuple(full_scan_cpu), partition_messages=_partition_log_messages(collector.records))

@pytest.mark.serial
def test_iva_quarterly_aggregation_partitioned_p95_cpu_within_budget(quarterly_iva_samples: _QuarterlyIvaSamples) -> None:
    ...
    print(f'\n[bench] iva_quarterly_full_scan_diagnostic: n={_DIAGNOSTIC_SAMPLE_COUNT} cpu_p95={full_scan_cpu_p95:.3f}s cpu_mean={statistics.mean(samples.full_scan_cpu):.3f}s wall_p95={_p95(list(samples.full_scan_wall)):.3f}s (wall advisory) cpu_min={min(samples.full_scan_cpu):.3f}s cpu_max={max(samples.full_scan_cpu):.3f}s')
    print(f'[bench] iva_quarterly_partitioned: n={_BUDGET_SAMPLE_COUNT} cpu_p95={partitioned_cpu_p95:.3f}s cpu_mean={statistics.mean(samples.partitioned_cpu):.3f}s cpu_min={min(samples.partitioned_cpu):.3f}s cpu_max={max(samples.partitioned_cpu):.3f}s gate=cpu<{_P95_BUDGET_CPU_SECONDS:.1f}s wall_p95={partitioned_wall_p95:.3f}s wall_mean={statistics.mean(samples.partitioned_wall):.3f}s wall_max={max(samples.partitioned_wall):.3f}s (wall advisory<{_P95_WALL_ADVISORY_SECONDS:.1f}s, warned never asserted) paired_cpu_p95_delta_vs_full_scan={full_scan_cpu_p95 - _p95(list(samples.paired_partitioned_cpu)):.3f}s partition_reads={partition_read_count} partition_in_window_rows={partition_in_window_rows}')
    wall_advisory_message('iva_quarterly_partitioned p95', wall_seconds=partitioned_wall_p95, cpu_seconds=partitioned_cpu_p95, wall_advisory_seconds=_P95_WALL_ADVISORY_SECONDS, hang_wall_to_cpu_ratio=_P95_WEDGE_WALL_TO_CPU_RATIO)
    assert partitioned_cpu_p95 < _P95_BUDGET_CPU_SECONDS, f'IVA quarterly aggregation (partitioned) P95 {partitioned_cpu_p95:.3f} CPU-s at {_TOTAL_TRANSACTIONS}-row ledger scale exceeds the {_P95_BUDGET_CPU_SECONDS:.1f} CPU-s budget (cpu samples={samples.partitioned_cpu!r}; wall p95 {partitioned_wall_p95:.3f}s is advisory only)'

@pytest.mark.serial
def test_iva_quarterly_budget_still_fails_without_the_partition(quarterly_iva_samples: _QuarterlyIvaSamples) -> None:
    ...
    samples = quarterly_iva_samples
    full_scan_cpu_p95 = _p95(list(samples.full_scan_cpu))
    assert full_scan_cpu_p95 > _P95_BUDGET_CPU_SECONDS, f'the unpartitioned full scan measured {full_scan_cpu_p95:.3f} CPU-s P95, which does NOT exceed the {_P95_BUDGET_CPU_SECONDS:.1f} CPU-s budget — the budget can no longer detect the loss of the date-index partition and must be tightened (full-scan cpu samples={samples.full_scan_cpu!r})'

def test_modelo_130_calculate_p95_cpu_within_budget_and_full_scan_control(scale_bucket: SecureObjectRepository, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.DEBUG, logger=_TRANSACTION_REPOSITORY_LOGGER):
        for year in _M130_DIAGNOSTIC_YEARS:
            for quarter in quarters:
                ...
                work_unit = create_work_unit(bucket_id=_BUCKET_ID, modelo='130', filing_year=year, period=Period.from_year_and_code(year, quarter), revision_id=_M130_REVISION, repository=wu_repo, clock=filed_at)
                wall_started = time.perf_counter()
                cpu_started = time.process_time()
                revision = calculate_modelo_revision_from_bucket_aggregation(work_unit.work_unit_id, casilla_inputs=_M130_MANUAL_INPUTS, work_unit_repository=wu_repo, calculation_repository=cr_repo, transaction_repository=tx_repo, invoice_repository=invoice_repo, clock=filed_at)
                ...
                persist_filed_revision_observation(revision=revision, work_unit=work_unit, repository=observation_repo, captured_at=filed_at)
    calculation_log_messages = tuple((record.getMessage() for record in caplog.records))
    full_catalogue_reads = tuple((message for message in calculation_log_messages if message.startswith('loaded transaction catalogue bucket_id=')))
    assert not full_catalogue_reads, 'M130 calculation performed a full-catalogue load instead of targeted contributor reads'
    assert len(per_quarter) == len(cpu_samples) == len(_M130_DIAGNOSTIC_YEARS) * len(quarters), f'expected one measurement per filed quarter, got {len(per_quarter)}'
    ...
    print(f'\n[bench] modelo_130_calculate: n={len(cpu_samples)} cpu_worst={cpu_worst:.3f}s cpu_mean={statistics.mean(cpu_samples):.3f}s per_quarter={[(label, round(cpu, 3)) for label, cpu, _ in per_quarter]} cpu_min={min(cpu_samples):.3f}s cpu_max={max(cpu_samples):.3f}s gate=cpu<{_P95_BUDGET_CPU_SECONDS:.1f}s wall_worst={wall_worst:.3f}s wall_mean={statistics.mean(wall_samples):.3f}s full_scan_control_cpu={full_scan_cpu:.3f}s partition_reads={partition_read_count} partition_in_window_rows={partition_in_window_rows}')
    over_budget = [(label, cpu) for label, cpu, _ in per_quarter if cpu >= _P95_BUDGET_CPU_SECONDS]
    assert not over_budget, f'M130 calculate exceeds the {_P95_BUDGET_CPU_SECONDS:.1f} CPU-s budget for {[label for label, _ in over_budget]} at {_TOTAL_TRANSACTIONS}-row ledger scale; per-quarter CPU-s: {[(label, round(cpu, 3)) for label, cpu, _ in per_quarter]}'
    assert full_scan_cpu > _P95_BUDGET_CPU_SECONDS, f'the removed full-catalogue draft-anchor read measured {full_scan_cpu:.3f} CPU-s and no longer breaks the {_P95_BUDGET_CPU_SECONDS:.1f}s budget; tighten the gate before it becomes vacuous'