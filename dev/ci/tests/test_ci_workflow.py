...
_PROHIBITED_AEAT_PRODUCT_FORMS = (('python-import', re.compile('(?i)\\b(?:from\\s+aeat(?:\\.|\\s+import\\b)|import\\s+(?:[a-z_]\\w*(?:\\.[a-z_]\\w*)*\\s*,\\s*)*aeat(?:\\.|(?=\\s|$|[;"\'])))')), ('python-module', re.compile('(?i)\\bpython(?:\\d+(?:\\.\\d+)*)?\\s+-m\\s+aeat(?:\\.[a-z_]\\w*)*(?=\\s|$)')), ('distribution-install', re.compile('(?i)\\b(?:(?:uv\\s+)?pip\\s+install|uv\\s+add)\\b[^&|;\\r\\n]*?(?<![\\w-])aeat(?=\\[|\\s|$|[<>=!~@;"\'])')), ('uv-package', re.compile('(?i)\\b(?:uv\\s+run\\s+--(?:package|with)|uvx\\s+--from)(?:=|\\s+)["\']?aeat(?=\\[|\\s|$|[<>=!~@;"\'])')), ('former-distribution', re.compile('(?i)(?<![\\w-])aeat(?:-cli|-data(?:-[\\w-]+)?|_data(?:_[\\w-]+)?)(?![\\w-])')), ('former-source-path', re.compile('(?i)(?<![\\w])(?:src|packaging)[/\\\\]aeat(?:[/\\\\_.-]|$)')))
...
def test_ci_workflow_runs_canonical_cadrumo_commands_and_paths() -> None:
    ...
    assert set(document['jobs']) == {'cadrumo-workflow-lint', 'cadrumo-static', 'cadrumo-unit', 'cadrumo-test-harness'}
    static = document['jobs']['cadrumo-static']
    static_commands = '\n'.join((str(_dp_get('dev/ci/tests/test_ci_workflow.py:99:get', step, 'run', '')) for step in static['steps']))
    ...
    unit_commands = '\n'.join((str(_dp_get('dev/ci/tests/test_ci_workflow.py:117:get', step, 'run', '')) for step in unit['steps']))
...
def test_harness_recipe_runs_every_real_proof_outer_serially_and_non_vacuously() -> None:
    ...
    assert commands == (*(f'{pytest_prefix} --collect-only -n0 {member}' for member in members), f"{pytest_prefix} -rsf -n0 --timeout={_HARNESS_WALL_CEILING_SECONDS} {' '.join(members)}")
...
def test_the_harness_real_proof_outruns_the_default_per_test_wall_ceiling() -> None:
    ...
    """
    ini_ceiling = int(re.search('(?m)^timeout\\s*=\\s*(\\d+)', _PYPROJECT.read_text(encoding='utf-8')).group(1))
    commands = resolved_recipe_commands(_REPOSITORY_ROOT, 'test-harness')
    real_proof = commands[-1]
    assert ini_ceiling < _HARNESS_WALL_CEILING_SECONDS, f'the harness ceiling ({_HARNESS_WALL_CEILING_SECONDS}s) must exceed the ini default ({ini_ceiling}s), or raising it accomplishes nothing'
    assert f'--timeout={_HARNESS_WALL_CEILING_SECONDS}' in real_proof
    assert all(('--timeout=' not in command for command in commands[:-1])), 'only the combined real-proof pass needs the raised ceiling; a preflight that needs it is doing real work'
...
def test_harness_member_preflight_rejects_empty_collection_even_when_another_member_exists(tmp_path: Path) -> None:
    ...
    populated_member.write_text('import pytest\n\npytestmark = pytest.mark.integration\n\ndef test_real_item_is_collectable() -> None:\n    pass\n', encoding='utf-8')
    members = _declared_harness_members()
    commands = resolved_recipe_commands(_REPOSITORY_ROOT, 'test-harness')
    preflight = _dp_next('dev/ci/tests/test_ci_workflow.py:209:next', (command for command in commands if '--collect-only' in command and members[0] in command), None)
    ...
    assert command == ['uv', 'run', '--no-sync', 'pytest', '-q', '-m', 'integration', '--collect-only', '-n0', members[0]]
    invocation = [*command[:-1], '--rootdir', str(tmp_path)]
    aggregate = run_command([*invocation, str(populated_member), str(empty_member)], cwd=_REPOSITORY_ROOT, timeout_seconds=30)
    assert aggregate.returncode == 0, f'the populated control must make aggregate collection non-empty\ncommand: {shlex.join([*invocation, str(populated_member), str(empty_member)])}\nstdout:\n{aggregate.stdout}\nstderr:\n{aggregate.stderr}'
    empty_preflight = run_command([*invocation, str(empty_member)], cwd=_REPOSITORY_ROOT, timeout_seconds=30)
    assert empty_preflight.returncode == 5, f'the per-member collect preflight must preserve pytest exit 5 for an empty member\ncommand: {shlex.join([*invocation, str(empty_member)])}\nstdout:\n{empty_preflight.stdout}\nstderr:\n{empty_preflight.stderr}'
...
def test_ci_harness_verdict_is_a_standalone_blocking_job() -> None:
    ...
    commands = tuple((str(_dp_get('dev/ci/tests/test_ci_workflow.py:268:get', step, 'run', '')) for step in harness['steps'] if step.get('run')))
    assert commands[-1] == 'just test-harness'
    assert sum((command == 'just test-harness' for command in commands)) == 1
    routine_commands = '\n'.join((str(_dp_get('dev/ci/tests/test_ci_workflow.py:273:get', step, 'run', '')) for job_name in ('cadrumo-static', 'cadrumo-unit') for step in document['jobs'][job_name]['steps']))
...
def test_the_dev_ci_recipe_carries_the_substance_the_workflow_delegates() -> None:
    ...
    """
    recipe = _dp_next('dev/ci/tests/test_ci_workflow.py:298:next', (line for line in _JUSTFILE.read_text(encoding='utf-8').splitlines() if 'dev/ci/tests' in line), None)
...
def test_the_test_unit_recipe_carries_the_substance_the_workflow_delegates() -> None:
    ...
    """
    recipe = _dp_next('dev/ci/tests/test_ci_workflow.py:315:next', (line for line in _JUSTFILE.read_text(encoding='utf-8').splitlines() if line.startswith('test-unit ')), None)
    assert recipe is not None, 'no justfile recipe line named test-unit; the delegated lane has no home'
    body = _dp_next('dev/ci/tests/test_ci_workflow.py:321:next', (line for line in _JUSTFILE.read_text(encoding='utf-8').splitlines() if '--dist=loadfile' in line), None)
    ...
    assert 'external_tool' in parity_markers, f'test_workbook_parity.py no longer carries external_tool, so nothing holds it out of the unit lane; either restore the marker or give the lane an explicit exclusion (found {sorted(parity_markers)})'
...
def test_the_per_push_integration_gates_recipe_carries_the_substance_the_workflow_delegates() -> None:
    ...
    """
    recipe = _dp_next('dev/ci/tests/test_ci_workflow.py:378:next', (line for line in _JUSTFILE.read_text(encoding='utf-8').splitlines() if line.startswith('test-per-push-integration-gates:')), None)
    assert recipe is not None, 'no justfile recipe line named test-per-push-integration-gates'
    body = _dp_next('dev/ci/tests/test_ci_workflow.py:388:next', (line for line in _JUSTFILE.read_text(encoding='utf-8').splitlines() if 'test_suggestion_command_conformance.py' in line), None)
    ...
    for target in ('src/cadrumo/application/user_profile/tests/test_status_projection.py', 'src/cadrumo/entrypoints/cli/tests/test_self_referential_string_conformance.py', 'dev/tests/test_suggestion_command_conformance.py'):
...
def test_ci_per_push_integration_conformance_step_is_exact_and_blocking() -> None:
    ...
    document = yaml.safe_load(_WORKFLOW.read_text(encoding='utf-8'))
    static = document['jobs']['cadrumo-static']
    step_name = 'Per-push integration conformance gates (rule-surface, status-frontend, self-referential-string, suggestion-command)'
    step = _dp_next('dev/ci/tests/test_ci_workflow.py:414:next', (candidate for candidate in static['steps'] if candidate.get('name') == step_name), None)
...
def test_ci_per_push_jobs_carry_the_speed_budget_ceilings() -> None:
    ...
    commands = '\n'.join((str(_dp_get('dev/ci/tests/test_ci_workflow.py:437:get', step, 'run', '')) for job in document['jobs'].values() for step in job['steps']))
...
def test_full_lane_carries_every_slow_conformance_surface() -> None:
    ...
    commands = '\n'.join((str(_dp_get('dev/ci/tests/test_ci_workflow.py:456:get', step, 'run', '')) for step in document['jobs']['cadrumo-full-conformance']['steps']))
...
def _both_lane_workflows() -> tuple[tuple[Path, dict[str, Any]], ...]:
    ...
    for path in (_WORKFLOW, _FULL_WORKFLOW):
        ...
        jobs = document['jobs']
        assert len(jobs) >= _MINIMUM_LANE_JOBS, f'{path.name} declares {len(jobs)} job(s); a lane with none satisfies every prohibition below without running anything'
        steps = sum((len(_dp_or('dev/ci/tests/test_ci_workflow.py:492:or', lambda: job.get('steps'), lambda: ())) for job in jobs.values()))
        assert steps >= _MINIMUM_LANE_STEPS, f'{path.name} declares {steps} step(s) across its jobs; below this the prohibitions below hold because there is nothing to prohibit'
...
def test_ci_lanes_use_no_actions_artifact_storage() -> None:
    ...
    for path, document in _both_lane_workflows():
        offending = [str(step.get('uses')) for job in document['jobs'].values() for step in job['steps'] if 'upload-artifact' in str(_dp_get('dev/ci/tests/test_ci_workflow.py:515:get', step, 'uses', '')) or 'download-artifact' in str(_dp_get('dev/ci/tests/test_ci_workflow.py:515:get', step, 'uses', ''))]
...
def test_ci_workflow_does_not_materialise_operator_dotenv() -> None:
    ...
    for _path, document in _both_lane_workflows():
        commands = '\n'.join((str(_dp_get('dev/ci/tests/test_ci_workflow.py:523:get', step, 'run', '')) for job in document['jobs'].values() for step in job['steps']))
...
def test_ci_workflow_provisions_browser_before_unit_tests() -> None:
    ...
    step_names = [str(_dp_get('dev/ci/tests/test_ci_workflow.py:533:get', step, 'name', '')) for step in steps]
...
def test_ci_workflow_product_surface_has_no_former_identity() -> None:
    ...
    product_surface = '\n'.join((document['name'], *(str(job['name']) for job in jobs), *(str(_dp_get('dev/ci/tests/test_ci_workflow.py:549:get', step, 'name', '')) for job in jobs for step in job['steps']), *(str(_dp_get('dev/ci/tests/test_ci_workflow.py:550:get', step, 'run', '')) for job in jobs for step in job['steps'])))
    commands = tuple((line.strip() for job in document['jobs'].values() for step in job['steps'] for line in str(_dp_get('dev/ci/tests/test_ci_workflow.py:557:get', step, 'run', '')).splitlines() if line.strip()))
    registry_commands = {command for command in commands if ' app registry ' in command}
    assert registry_commands == {'uv run --no-sync aeat app registry verify'}
    assert 'uv run --no-sync python -m dev.registry.parity.maintenance_cli audit-oracles' in commands
    assert not any((re.match('^(?:uv run(?: --no-sync)? )?cadrumo(?:\\s|$)', command) for command in commands))
    assert _prohibited_aeat_product_forms(product_surface) == ()

@pytest.mark.parametrize('surface', ('uv run --no-sync aeat app registry verify', 'aeat --version', "echo 'AEAT is the Spanish tax authority'", 'uv add cadrumo && aeat --version', 'pip install cadrumo && echo AEAT is the Spanish tax authority'))
...
@pytest.mark.parametrize(('surface', 'expected_family'), (('from aeat import core', 'python-import'), ('from aeat.core import Settings', 'python-import'), ('import aeat', 'python-import'), ('import aeat.core', 'python-import'), ('python -c "import os, aeat as retired"', 'python-import'), ('python -m aeat config check', 'python-module'), ('python -m aeat.cli check', 'python-module'), ('uv pip install aeat', 'distribution-install'), ('uv pip install "aeat"', 'distribution-install'), ('pip install "aeat[agent]>=1"', 'distribution-install'), ('uv add cadrumo aeat', 'distribution-install'), ('pip install cadrumo aeat>=1', 'distribution-install'), ('uv run --package aeat python verify.py', 'uv-package'), ('uv run --package=aeat python verify.py', 'uv-package'), ("uv run --with 'aeat==1.2.3' python verify.py", 'uv-package'), ('uvx --from aeat==1.2.3 aeat --version', 'uv-package'), ('uv build packaging/aeat_data_manuals', 'former-distribution'), ('ruff check src/aeat/', 'former-source-path')))
...
def test_full_lane_runs_the_channel_generator_tests_explicitly_and_serially() -> None:
    ...
    step = _dp_next('dev/ci/tests/test_ci_workflow.py:632:next', (str(step['run']) for step in steps if 'test-channel-artifacts' in str(_dp_get('dev/ci/tests/test_ci_workflow.py:633:get', step, 'run', ''))), None)
    assert step is not None, 'no full-lane step invokes the test-channel-artifacts recipe'
    generator = _dp_next('dev/ci/tests/test_ci_workflow.py:638:next', (line for line in _JUSTFILE.read_text(encoding='utf-8').splitlines() if 'packaging/homebrew/tests' in line), None)
