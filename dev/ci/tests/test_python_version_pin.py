...
_EXACT_PATCH: Final = re.compile('\\d+\\.\\d+\\.\\d+')
_MATRIX_EXPRESSION: Final = re.compile('\\$\\{\\{\\s*matrix\\.([A-Za-z][\\w-]*)\\s*\\}\\}')
...
def _is_compatibility_matrix_override(*, selection: object, matrix: object, pin: str) -> bool:
    ...
    return isinstance(values, list) and pin in {str(value) for value in values} and any((str(value) != pin for value in values))

def _assert_setup_uv_consumers_follow_pin(documents: list[tuple[Path, dict[str, Any]]], *, pin: str) -> None:
    ...
    for path, document in documents:
        for job_name, job in (document.get('jobs') or {}).items():
            if not isinstance(job, dict):
                continue
            steps = _dp_or('dev/ci/tests/test_python_version_pin.py:69:or', lambda: job.get('steps'), lambda: [])
            for step_index, step in enumerate(steps):
                if not isinstance(step, dict) or not str(_dp_get('dev/ci/tests/test_python_version_pin.py:71:get', step, 'uses', '')).startswith('astral-sh/setup-uv@'):
                    continue
                consumer_found = True
                selection = declared_python_selection(document, job_name, step_index)
                if selection is None:
                    checked_out = any((isinstance(previous, dict) and str(_dp_get('dev/ci/tests/test_python_version_pin.py:80:get', previous, 'uses', '')).startswith('actions/checkout@') for previous in steps[:step_index]))
                    if not checked_out:
                        violations.append(f'{path.name}:{job_name}: setup-uv precedes checkout')
                    continue
                matrix = _dp_or('dev/ci/tests/test_python_version_pin.py:87:or', lambda: (job.get('strategy') or {}).get('matrix'), lambda: {})
                if path != _COMPATIBILITY_WORKFLOW or not _is_compatibility_matrix_override(selection=selection, matrix=matrix, pin=pin):
    ...
    assert violations == [], f'setup-uv Python overrides bypass .python-version unless they are a compatibility matrix containing the exact pin and an alternative: {violations}'
...
@pytest.mark.parametrize(('selection', 'matrix', 'expected'), [('${{ matrix.python-version }}', {'python-version': ['{pin}', '3.13t']}, True), ('${{ matrix.python-version }}', {'python-version': ['3.13t', '3.12']}, False), ('${{ matrix.python-version }}', {'python-version': ['{pin}']}, False), ('3.13', {}, False), ('${{ inputs.python-version }}', {'python-version': ['{pin}', '3.13t']}, False), ('${{ matrix.python-version }}', {'other-version': ['{pin}', '3.13t']}, False)], ids=('canonical-pin-and-alternative', 'missing-canonical-pin', 'canonical-pin-without-alternative', 'loose-literal', 'non-matrix-expression', 'mismatched-matrix-key'))
def test_compatibility_matrix_override_classifier(selection: str, matrix: dict[str, list[str]], expected: bool) -> None:
...
def test_matrix_override_is_rejected_outside_the_compatibility_workflow() -> None:
    ...
    pin = _python_pin()
    foreign = Path('ci.yml')
    documents = [(foreign, {'jobs': {'foreign-matrix': {'steps': [{'uses': 'actions/checkout@v4'}, {'uses': 'astral-sh/setup-uv@v5', 'with': {'python-version': '${{ matrix.python-version }}'}}], 'strategy': {'matrix': {'python-version': [pin, '3.15']}}}}})]
...
@pytest.mark.parametrize('document', [{'env': {'UV_PYTHON': '3.12'}, 'jobs': {'build': {'steps': [_checkout(), _uv_step()]}}}, {'jobs': {'build': {'env': {'UV_PYTHON': '3.12'}, 'steps': [_checkout(), _uv_step()]}}}, {'jobs': {'build': {'steps': [_checkout(), _uv_step(env={'UV_PYTHON': '3.12'})]}}}], ids=('workflow-env', 'job-env', 'step-env'))
def test_a_uv_python_override_is_refused_at_every_env_scope(document: dict[str, Any]) -> None:
    ...
    """
    with pytest.raises(AssertionError, match='bypass \\.python-version'):
...
def test_the_innermost_uv_python_declaration_wins() -> None:
    ...
    """
    document = {'env': {'UV_PYTHON': '3.11'}, 'jobs': {'build': {'env': {'UV_PYTHON': '3.12'}, 'steps': [_checkout(), _uv_step(env={'UV_PYTHON': '3.13'})]}}}
...
def test_the_with_channel_still_outranks_an_ambient_declaration() -> None:
    ...
    document = {'env': {'UV_PYTHON': '3.11'}, 'jobs': {'build': {'steps': [_checkout(), _uv_step(**{'with': {'python-version': '3.14'}})]}}}
