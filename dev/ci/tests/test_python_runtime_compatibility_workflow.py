...
_GUARD: Final = "github.event_name != 'pull_request' || github.event.pull_request.head.repo.full_name == github.repository"
_MATRIX_JOBS: Final = ('compatibility-source', 'compatibility-binary')
_REQUIRED_PATHS: Final = frozenset({'.github/workflows/python-runtime-compatibility.yml', '.python-version', 'pyproject.toml', 'uv.lock', 'dev/**', 'src/**', 'packaging/**'})
...
def _probe_step(job: dict[str, Any], mode: str) -> dict[str, Any]:
    ...
    matches = [step for step in _steps_with_run(job) if 'dev.ci.python_runtime_compatibility' in executed_text(step['run']) and f'--mode {mode}' in executed_text(step['run'])]
...
def _assert_evidence_upload_contract(job: dict[str, Any], *, mode: str) -> None:
    ...
    uploads = [step for step in job['steps'] if isinstance(step, dict) and 'actions/upload-artifact@' in str(_dp_get('dev/ci/tests/test_python_runtime_compatibility_workflow.py:118:get', step, 'uses', ''))]
    ...
    assert f'Hash {mode} compatibility evidence' in '\n'.join((str(_dp_get('dev/ci/tests/test_python_runtime_compatibility_workflow.py:129:get', step, 'name', '')) for step in job['steps'] if isinstance(step, dict)))
...
def test_every_job_is_self_hosted_and_fork_guarded() -> None:
    ...
    document = _document()
    jobs = document['jobs']
    assert set(jobs) == {'runtime-inventory', 'build-python-cohort', 'compatibility-source', 'compatibility-binary'}
...
def test_inventory_job_is_the_only_matrix_authority() -> None:
    ...
    future_steps = [step for step in inventory['steps'] if isinstance(step, dict) and step.get('name') == 'Enforce future-directive AST policy']
...
def test_probe_mode_gate_has_detector_teeth() -> None:
    ...
    source = document['jobs']['compatibility-source']
    probe = next((step for step in source['steps'] if 'dev.ci.python_runtime_compatibility' in str(_dp_get('dev/ci/tests/test_python_runtime_compatibility_workflow.py:214:get', step, 'run', ''))))
...
def test_advisory_and_blocking_outcomes_are_explicit_without_workflow_skips() -> None:
    ...
    for job_name in document['jobs']:
        ...
        for step in _dp_get('dev/ci/tests/test_python_runtime_compatibility_workflow.py:232:get', job, 'steps', []):
            examined += 1
            assert isinstance(step, dict)
            assert 'continue-on-error' not in step, f"{job_name}:{_dp_get('dev/ci/tests/test_python_runtime_compatibility_workflow.py:235:get', step, 'name', '')}"
            if 'if' in step:
                assert str(step['if']) == 'always()', f'unexpected conditional step in {job_name}'
    assert examined >= _MINIMUM_WORKFLOW_STEPS, f"only {examined} step(s) were examined across {len(document['jobs'])} job(s); below this the skip prohibition holds because there are no steps to prohibit"
...
def test_binary_rows_download_and_verify_one_cohort() -> None:
    ...
    assert 'cadrumo-python-runtime-cohort' in '\n'.join((str(_dp_get('dev/ci/tests/test_python_runtime_compatibility_workflow.py:276:get', _dp_get('dev/ci/tests/test_python_runtime_compatibility_workflow.py:276:get', step, 'with', {}), 'name', '')) for step in build['steps'] if isinstance(step, dict)))
    binary = document['jobs']['compatibility-binary']
    download = [step for step in binary['steps'] if isinstance(step, dict) and 'actions/download-artifact@' in str(_dp_get('dev/ci/tests/test_python_runtime_compatibility_workflow.py:283:get', step, 'uses', ''))]
...
def test_cohort_builder_preserves_the_exact_pin_lane() -> None:
    ...
    document = _document()
    build = document['jobs']['build-python-cohort']
    setup = [step for step in build['steps'] if isinstance(step, dict) and str(_dp_get('dev/ci/tests/test_python_runtime_compatibility_workflow.py:300:get', step, 'uses', '')).startswith('astral-sh/setup-uv@')]
    assert len(setup) == 1
...
def test_evidence_upload_gate_has_detector_teeth() -> None:
    ...
    document = deepcopy(_document())
    source = document['jobs']['compatibility-source']
    upload = next((step for step in source['steps'] if 'actions/upload-artifact@' in str(_dp_get('dev/ci/tests/test_python_runtime_compatibility_workflow.py:310:get', step, 'uses', ''))))
