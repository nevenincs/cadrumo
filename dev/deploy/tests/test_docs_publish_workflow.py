...
def _run_surface() -> str:
    return '\n'.join((str(_dp_get('dev/deploy/tests/test_docs_publish_workflow.py:41:get', step, 'run', '')) for step in _job()['steps'] if 'run' in step))
...
def test_publication_never_waits_on_documentation() -> None:
    ...
    for name, job in publication['jobs'].items():
        surface = '\n'.join((str(_dp_get('dev/deploy/tests/test_docs_publish_workflow.py:60:get', step, 'run', '')) for step in job['steps'] if 'run' in step))
        assert 'docs_static_site' not in surface, f'publication job {name!r} must not build or publish the docs site'
        assert 'docs-publish' not in str(_dp_get('dev/deploy/tests/test_docs_publish_workflow.py:62:get', job, 'needs', '')), f'publication job {name!r} must not wait on docs'
...
def test_it_builds_the_released_commit_not_the_current_head() -> None:
    ...
    checkout = next((step for step in _job()['steps'] if str(_dp_get('dev/deploy/tests/test_docs_publish_workflow.py:67:get', step, 'uses', '')).startswith('actions/checkout@')))
...
def test_the_publish_step_exports_the_role_under_the_publisher_own_name() -> None:
    ...
    publish = next((step for step in _job()['steps'] if 'docs_static_site' in str(_dp_get('dev/deploy/tests/test_docs_publish_workflow.py:108:get', step, 'run', ''))))
    assert variable in _dp_get('dev/deploy/tests/test_docs_publish_workflow.py:109:get', publish, 'env', {}), f'the publish step must export {variable!r}'
...
def test_a_documentation_failure_cannot_unwind_the_release() -> None:
    ...
    steps = _job()['steps']
    alert = next((step for step in steps if 'failure()' in str(_dp_get('dev/deploy/tests/test_docs_publish_workflow.py:121:get', step, 'if', ''))))
    body = str(alert['run'])
