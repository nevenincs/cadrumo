...
def _write_pyprojects(root: Path, version: str) -> None:
    ...
    root_project.write_text(root_project.read_text(encoding='utf-8') + f'dependencies = ["cadrumo-data-manuals=={version}", "cadrumo-data-official=={version}"]\n', encoding='utf-8')
    _write_project(root / 'packaging' / 'cadrumo_data_manuals' / 'pyproject.toml', name='cadrumo-data-manuals', version=version)
    _write_project(root / 'packaging' / 'cadrumo_data_official' / 'pyproject.toml', name='cadrumo-data-official', version=version)
...
@pytest.mark.parametrize(('relative', 'former_name'), (('pyproject.toml', 'aeat-cli'), ('packaging/cadrumo_data_manuals/pyproject.toml', 'aeat-data-manuals'), ('packaging/cadrumo_data_official/pyproject.toml', 'aeat-data-official')))
def test_project_names_reject_each_former_product_distribution(tmp_path: Path, relative: str, former_name: str) -> None:
...
def test_version_surfaces_agree_fails_on_companion_project_drift(tmp_path: Path) -> None:
    ...
    root = _make_repo_root(tmp_path, version='2.0.0')
    _write_project(root / 'packaging' / 'cadrumo_data_manuals' / 'pyproject.toml', name='cadrumo-data-manuals', version='1.9.9')
    check = readiness.check_version_surfaces_agree(root)
    assert check.passed is False
...
def test_version_surfaces_agree_fails_on_nonmatching_mandatory_exact_pin(tmp_path: Path) -> None:
    ...
    project.write_text(project.read_text(encoding='utf-8').replace('cadrumo-data-official==2.0.0', 'cadrumo-data-official==1.9.9'), encoding='utf-8')
    check = readiness.check_version_surfaces_agree(root)
    assert check.passed is False
...
def test_version_surfaces_agree_fails_when_a_mandatory_companion_is_missing(tmp_path: Path) -> None:
    ...
    root = _make_repo_root(tmp_path, version='2.0.0')
    project = root / 'pyproject.toml'
    project.write_text(project.read_text(encoding='utf-8').replace(', "cadrumo-data-official==2.0.0"', ''), encoding='utf-8')
    check = readiness.check_version_surfaces_agree(root)
    assert check.passed is False
...
def _write_real_manifest(directory: Path, *, ok: bool, lane: str) -> Path:
    ...
    manifest = PackagingSmokeManifest(ok=ok, lane=lane, completed_at=datetime.now(UTC), work_dir=str(directory), artifacts={}, checks=())
...
def test_packaging_smoke_evidence_reads_a_pruned_checkpoint(tmp_path: Path) -> None:
    ...
    manifest = PackagingSmokeManifest(ok=True, lane='docker-core', completed_at=datetime.now(UTC), work_dir=str(evidence_dir), artifacts={}, checks=())
...
def test_packaging_smoke_evidence_refuses_a_truthy_non_boolean_ok(tmp_path: Path, truthy_ok: object) -> None:
    ...
    (smoke_dir / 'packaging-smoke-manifest.json').write_text(json.dumps({'ok': truthy_ok, 'lane': 'core-wheel'}), encoding='utf-8')
    check = readiness.check_latest_packaging_smoke_evidence(root)
    assert check.severity == 'advisory'
    assert check.passed is False
...
def test_packaging_smoke_evidence_refuses_a_manifest_missing_required_fields(tmp_path: Path) -> None:
    ...
    smoke_dir = root / 'var' / 'packaging-smoke' / 'core-20260101T000000Z'
    smoke_dir.mkdir(parents=True)
    (smoke_dir / 'packaging-smoke-manifest.json').write_text(json.dumps({'ok': True, 'lane': 'core-wheel'}), encoding='utf-8')
    check = readiness.check_latest_packaging_smoke_evidence(root)
    assert check.severity == 'advisory'
    assert check.passed is False
    assert 'not a valid packaging-smoke manifest' in check.detail
...
def test_packaging_smoke_evidence_refuses_an_empty_lane(tmp_path: Path) -> None:
    ...
    smoke_dir = root / 'var' / 'packaging-smoke' / 'core-20260101T000000Z'
    smoke_dir.mkdir(parents=True)
    (smoke_dir / 'packaging-smoke-manifest.json').write_text(json.dumps({'ok': True, 'lane': '', 'completed_at': datetime.now(UTC).isoformat(), 'work_dir': str(smoke_dir)}), encoding='utf-8')
    check = readiness.check_latest_packaging_smoke_evidence(root)
    assert check.severity == 'advisory'
    assert check.passed is False
    assert 'not a valid packaging-smoke manifest' in check.detail
...
def test_packaging_smoke_evidence_end_to_end_through_the_real_smoke_writer(tmp_path: Path) -> None:
    ...
    write_smoke_manifest(work_dir, lane='core-wheel', artifacts={'wheel': 'wheel/cadrumo.whl'}, declared=('installed CLI config/profile smoke',))
    check = readiness.check_latest_packaging_smoke_evidence(root)
    assert check.passed is True
...
def test_build_report_makes_the_blocker_check_strict_on_the_cohort_dir_path(tmp_path: Path) -> None:
    ...
    blocker = _dp_next('dev/release/tests/test_readiness.py:476:next', (c for c in report.checks if c.name == 'no-open-release-blockers'), None)
...
def test_build_report_leaves_the_blocker_check_advisory_without_a_cohort_dir(tmp_path: Path) -> None:
    ...
    blocker = _dp_next('dev/release/tests/test_readiness.py:489:next', (c for c in report.checks if c.name == 'no-open-release-blockers'), None)
    assert blocker is not None
...
def test_build_report_blocks_without_complete_distribution_evidence(tmp_path: Path) -> None:
    ...
    assert {check.name for check in report.blocking_failures} == {'distribution-evidence-complete', 'generated-surface-versions'}
...
def test_build_report_wires_in_the_packaging_smoke_evidence_check(tmp_path: Path) -> None:
    ...
    root = _make_repo_root(tmp_path)
    report = readiness.build_report(root, skip_network=True)
    smoke = _dp_next('dev/release/tests/test_readiness.py:515:next', (c for c in report.checks if c.name == 'packaging-smoke-evidence'), None)
...
def test_main_cli_json_contract_blocks_without_distribution_evidence(tmp_path: Path) -> None:
    ...
    driver.write_text(f'import json, sys\nsys.path.insert(0, {str(Path(readiness.__file__).parents[2])!r})\nfrom dev.release import readiness as r\nreport = r.build_report({str(root)!r}, skip_network=True)\nprint(json.dumps(report.to_dict()))\nsys.exit(0 if report.ok else 1)\n', encoding='utf-8')
