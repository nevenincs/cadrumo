...
def _cohort(root: Path, *, version: str='0.2.1', scoop_version: str | None=None, scoop_hashes: list[str] | None=None, homebrew_version: str | None=None, homebrew_sha: str | None=None) -> Path:
    ...
    (root / 'scoop' / 'cadrumo.json').write_text(json.dumps({'version': _dp_or('dev/release/tests/test_generated_surface_versions.py:51:or', lambda: scoop_version, lambda: version), 'architecture': {'64bit': {'hash': _dp_or('dev/release/tests/test_generated_surface_versions.py:53:or', lambda: scoop_hashes, lambda: [wheel_cadrumo, wheel_manuals, wheel_official])}}}), encoding='utf-8')
    hv = _dp_or('dev/release/tests/test_generated_surface_versions.py:59:or', lambda: homebrew_version, lambda: version)
    hs = _dp_or('dev/release/tests/test_generated_surface_versions.py:60:or', lambda: homebrew_sha, lambda: sdist_cadrumo)
    (root / 'homebrew' / 'Formula' / 'cadrumo.rb').write_text(f'class Cadrumo < Formula\n  url "https://github.com/nevenincs/cadrumo/releases/download/v{hv}/cadrumo-{hv}.tar.gz"\n  sha256 "{hs}"\nend\n', encoding='utf-8')
    return root
...
def test_generated_json_non_object_surface_blocks_readiness(tmp_path: Path, payload: object) -> None:
