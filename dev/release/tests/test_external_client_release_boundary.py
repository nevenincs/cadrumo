...
def _assert_lane_is_an_assurance_surface(path: Path, *, invocation: str) -> None:
    ...
    assert isinstance(document, dict), f'{path.name} does not parse as a workflow mapping; a file GitHub cannot read is a lane that never runs, and it occupies the path a presence check is satisfied by'
    declared = document[True] if True in document else document.get('on')
    triggers = {declared} if isinstance(declared, str) else set(_dp_or('dev/release/tests/test_external_client_release_boundary.py:59:or', lambda: declared, lambda: ()))
    assert _AUTOMATIC_TRIGGERS & triggers, f'{path.name} declares only {sorted(triggers)}; a lane nothing fires automatically is run by nobody, and its green is the green of a lane that never ran'
    jobs = _dp_or('dev/release/tests/test_external_client_release_boundary.py:65:or', lambda: document.get('jobs'), lambda: {})
    assert jobs, f'{path.name} declares no job; nothing about the lane can run'
    invoking = [(job_name, job, step) for job_name, job in jobs.items() for step in job.get('steps') or () if invocation in executed_text(step.get('run'))]
    assert invoking, f'no step across the {len(jobs)} job(s) in {path.name} invokes {invocation!r}; the lane exists but exercises nothing'
    for job_name, job, step in invoking:
        where = f"{path.name}:{job_name}:{_dp_get('dev/release/tests/test_external_client_release_boundary.py:79:get', step, 'name', '<unnamed>')}"
...
def test_base_cli_never_imports_the_harness() -> None:
    ...
    clean boundary.
    """
    assert _BASE_PACKAGE.is_dir(), f'no base package at {_BASE_PACKAGE}; a relocated root walks nothing and this gate would report the release boundary intact'
    walked = tuple(_BASE_PACKAGE.rglob('*.py'))
    assert len(walked) >= _MINIMUM_BASE_MODULES, f'only {len(walked)} base module(s) were walked; below this an empty crossing set says nothing about whether the shipped package reaches its harness'
    crossings = {path.relative_to(REPO_ROOT): target for path in walked for target in _import_targets(path) if target == 'cadrumo_harness' or target.startswith('cadrumo_harness.')}
...
def test_the_harness_reaches_the_base_cli_through_its_command_api() -> None:
    ...
    """
    production_imports = {target for path in _HARNESS_PACKAGE.rglob('*.py') if 'tests' not in path.parts for target in _import_targets(path) if target == 'cadrumo' or target.startswith('cadrumo.')}
...
def test_the_assurance_gate_reads_what_the_lane_executes(tmp_path: Path) -> None:
    ...
    invoking = next((step for step in steps if 'src/cadrumo_harness' in str(_dp_get('dev/release/tests/test_external_client_release_boundary.py:176:get', step, 'run', ''))))
    newline = chr(10)
