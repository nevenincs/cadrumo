...
from ..packaging.evidence import DistributionEvidence, EvidenceStatus, PackagingSmokeManifest, load_distribution_evidence
from ..packaging.python_cohort import load_python_cohort
_UTF_8: Final = UTF_8
_VERSION_RE: Final = re.compile('^__version__\\s*=\\s*[\\"\']([^\\"\']+)[\\"\']', re.MULTILINE)
_BLOCKER_LABEL: Final = 'priority:P0-blocker'
_GH_TIMEOUT_SECONDS: Final = 15
_PROJECT_NAME_PATHS: Final = ((Path('pyproject.toml'), PRODUCT_IDENTITY.distribution), (Path('packaging/cadrumo_data_manuals/pyproject.toml'), PRODUCT_IDENTITY.companion_distributions[0]), (Path('packaging/cadrumo_data_official/pyproject.toml'), PRODUCT_IDENTITY.companion_distributions[1]))
...
class ReadinessReport:
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        return {'ok': self.ok, 'checks': [{'name': c.name, 'severity': c.severity, 'passed': c.passed, 'detail': c.detail} for c in self.checks]}
...
def check_project_names_are_canonical(repo_root: Path) -> ReadinessCheck:
    ...
    observed = tuple(((relative, _read_project_name(repo_root / relative), expected) for relative, expected in _PROJECT_NAME_PATHS))
    mismatches = tuple(((relative, actual, expected) for relative, actual, expected in observed if actual != expected))
    if mismatches:
        detail = '; '.join((f'{relative}: found {actual!r}, expected {expected!r}' for relative, actual, expected in mismatches))
...
def _read_manifest_version(repo_root: Path) -> str:
    ...
    return str(_dp_get('dev/release/readiness.py:167:get', payload, '.', ''))
...
def check_version_surfaces_agree(repo_root: Path) -> ReadinessCheck:
    ...
    project_versions = tuple(((relative, _read_project_version(repo_root / relative)) for relative, _expected_name in _PROJECT_NAME_PATHS))
    ...
    observed_pins = tuple((str(requirement) for requirement in root_project['project']['dependencies'] if any((str(requirement).startswith(distribution) for distribution in PRODUCT_IDENTITY.companion_distributions))))
    expected_pins = tuple((f'{distribution}=={pyproject_version}' for distribution in PRODUCT_IDENTITY.companion_distributions))
...
def check_changelog_is_ready(repo_root: Path) -> ReadinessCheck:
    ...
    if found:
        return ReadinessCheck('changelog-ready', 'blocking', False, f'CHANGELOG.md carries unresolved merge markers: {found}')
    return ReadinessCheck('changelog-ready', 'blocking', True, 'CHANGELOG.md present, non-empty, no merge markers')

def check_no_open_release_blockers(*, repo_slug: str='nevenincs/cadrumo', gh_executable: str | None=None, strict: bool=False) -> ReadinessCheck:
    ...
    try:
        result = subprocess.run([resolved_gh, 'issue', 'list', '--repo', repo_slug, '--label', _BLOCKER_LABEL, '--state', 'open', '--json', 'number,title'], capture_output=True, text=True, check=False, timeout=_GH_TIMEOUT_SECONDS)
    except (OSError, subprocess.TimeoutExpired) as exc:
        _dp_mark('dev/release/readiness.py:293:except')
    ...
    try:
        issues = json.loads(_dp_or('dev/release/readiness.py:298:or', lambda: result.stdout, lambda: '[]'))
    except json.JSONDecodeError:
        _dp_mark('dev/release/readiness.py:299:except')
        return _undetermined('gh returned non-JSON output')
    if issues:
        titles = ', '.join((f"#{i['number']} {i['title']}" for i in issues[:5]))
        return ReadinessCheck('no-open-release-blockers', 'blocking', False, f'{len(issues)} open {_BLOCKER_LABEL} issue(s): {titles}')
...
def check_latest_packaging_smoke_evidence(repo_root: Path) -> ReadinessCheck:
    ...
    manifests = sorted((*smoke_dir.glob('*/packaging-smoke-manifest.json'), *evidence_dir.glob('*.json')), key=lambda path: path.stat().st_mtime)
    if not manifests:
        return ReadinessCheck('packaging-smoke-evidence', 'advisory', False, 'no packaging-smoke manifest found under var/packaging-smoke or its evidence checkpoint — run `just packaging-smoke` first')
    latest = manifests[-1]
    try:
        manifest = PackagingSmokeManifest.model_validate_json(latest.read_text(encoding=_UTF_8))
    except (json.JSONDecodeError, ValidationError, UnicodeDecodeError) as exc:
        _dp_mark('dev/release/readiness.py:335:except')
        return ReadinessCheck('packaging-smoke-evidence', 'advisory', False, f'{latest} is not a valid packaging-smoke manifest: {exc}')
    run_label = latest.stem if latest.parent == evidence_dir else latest.parent.name
    return ReadinessCheck('packaging-smoke-evidence', 'advisory', manifest.ok, f'most recent manifest ({run_label}, lane={manifest.lane}) ok={manifest.ok}')
...
def _checked_out_commit(repo_root: Path) -> str:
    ...
    completed = subprocess.run([git, 'rev-parse', 'HEAD'], cwd=repo_root, capture_output=True, text=True, check=False, timeout=_GH_TIMEOUT_SECONDS)
    commit = completed.stdout.strip().lower()
    if completed.returncode != 0 or re.fullmatch('[0-9a-f]{40}', commit) is None:
        detail = _dp_or('dev/release/readiness.py:366:or', lambda: completed.stderr.strip()[:200], lambda: completed.stdout.strip()[:200])
...
def _passing_identity(record: DistributionEvidence) -> tuple[object, ...]:
    ...
    return (record.runtime.operating_system, record.runtime.architecture, client.name if client is not None else None, client.version if client is not None else None, client.executable if client is not None else None, record.destination.kind)

def _select_canonical_passing_evidence(records: Sequence[DistributionEvidence]) -> tuple[dict[str, DistributionEvidence], tuple[str, ...]]:
    ...
    for row_id, group in sorted(by_row.items()):
        ...
        if len(identities) > 1:
            ...
            conflicts.append(f'{row_id} ({len(group)} passing records, {len(identities)} distinct identities): {conflicting_ids!r}')
            continue
        canonical[row_id] = max(group, key=lambda record: record.observed_at)
    return (canonical, tuple(conflicts))

def check_distribution_evidence_set(repo_root: Path, *, cohort_directory: Path | None=None, evidence_directory: Path | None=None, required_rows: tuple[str, ...]=REQUIRED_DISTRIBUTION_ROWS) -> ReadinessCheck:
    """Require passing, exact-cohort evidence for every declared release row."""
    cohort_root = _dp_or('dev/release/readiness.py:433:or', lambda: cohort_directory, lambda: repo_root / 'var' / 'release-cohort')
    evidence_root = _dp_or('dev/release/readiness.py:434:or', lambda: evidence_directory, lambda: repo_root / 'var' / 'distribution-install-readiness')
    try:
        cohort = load_release_cohort(cohort_root)
        checked_out_commit = _checked_out_commit(repo_root)
    except (OSError, SystemExit, ValueError) as exc:
        _dp_mark('dev/release/readiness.py:438:except')
    ...
    if manifest.source.commit != checked_out_commit:
        return ReadinessCheck('distribution-evidence-complete', 'blocking', False, f'cohort commit {manifest.source.commit} does not match checked-out commit {checked_out_commit}')
    if manifest.source.tag != expected_tag:
        return ReadinessCheck('distribution-evidence-complete', 'blocking', False, f'cohort tag {manifest.source.tag!r} does not match version tag {expected_tag!r}')
    if not evidence_root.is_dir():
        return ReadinessCheck('distribution-evidence-complete', 'blocking', False, f'distribution evidence directory is missing: {evidence_root}')
    paths = scan_directory(evidence_root, pattern='*.json')
    if not paths:
        return ReadinessCheck('distribution-evidence-complete', 'blocking', False, f'distribution evidence directory is empty: {evidence_root}')
    records = []
    for path in paths:
        try:
            records.append(load_distribution_evidence(path, cohort_directory=cohort.directory))
        except (OSError, SystemExit, ValueError) as exc:
            _dp_mark('dev/release/readiness.py:478:except')
            return ReadinessCheck('distribution-evidence-complete', 'blocking', False, f'invalid or mismatched evidence {path.name}: {exc}')
    failed = sorted({record.row_id for record in records if record.result.status is EvidenceStatus.FAILED})
    if failed:
        return ReadinessCheck('distribution-evidence-complete', 'blocking', False, f'cohort has failed distribution rows: {failed!r}')
    canonical_passing, conflicts = _select_canonical_passing_evidence(records)
    if conflicts:
        return ReadinessCheck('distribution-evidence-complete', 'blocking', False, f"conflicting passing evidence for the same row (runtime/client/destination disagree): {'; '.join(conflicts)}")
    passed_rows = set(canonical_passing)
    missing = sorted(set(required_rows) - passed_rows)
    if missing:
        return ReadinessCheck('distribution-evidence-complete', 'blocking', False, f'missing passing distribution evidence rows: {missing!r}')
    return ReadinessCheck('distribution-evidence-complete', 'blocking', True, f'{len(required_rows)} required rows pass for cohort {manifest.cohort_id}')

def check_generated_surface_versions(repo_root: Path, *, cohort_directory: Path | None=None) -> ReadinessCheck:
    ...
    try:
        cohort_manifest = _require_json_object(json.loads(manifest_path.read_text(encoding=_UTF_8)), surface='release cohort manifest')
        ...
        _dp_mark('dev/release/readiness.py:547:except')
        return ReadinessCheck(name, 'blocking', False, f'cohort version/digest surfaces unreadable under {cohort_root}: {exc}')
    failures: list[str] = []
    try:
        scoop = _require_json_object(json.loads((cohort_root / 'scoop' / 'cadrumo.json').read_text(encoding=_UTF_8)), surface='scoop manifest')
        if str(scoop.get('version')) != version:
            failures.append(f"scoop version {scoop.get('version')!r} != cohort {version!r}")
        expected_hashes = [python_sha['cadrumo'], python_sha['cadrumo-data-manuals'], python_sha['cadrumo-data-official']]
    ...
    try:
        ...
        url_match = re.search('url "[^"]*/cadrumo-([0-9][^"/]*)\\.tar\\.gz"', formula)
        sha_match = re.search('sha256 "([0-9a-f]{64})"', formula)
    ...
    return ReadinessCheck(name, 'blocking', True, f'scoop and homebrew bind cohort version {version} and digests')

def build_report(repo_root: Path | str | None=None, *, skip_network: bool=False, gh_executable: str | None=None, cohort_directory: Path | None=None, evidence_directory: Path | None=None) -> ReadinessReport:
    ...
    checks: list[ReadinessCheck] = [check_project_names_are_canonical(root), check_version_surfaces_agree(root), check_changelog_is_ready(root), check_distribution_evidence_set(root, cohort_directory=cohort_directory, evidence_directory=evidence_directory), check_generated_surface_versions(root, cohort_directory=cohort_directory), check_latest_packaging_smoke_evidence(root)]
    if not skip_network:
        checks.append(check_no_open_release_blockers(gh_executable=gh_executable, strict=cohort_directory is not None))
...
def main(argv: list[str] | None=None) -> int:
    ...
    parser.add_argument('--skip-network', action='store_true', help='Skip the gh-backed open-blocker check (offline/no-gh environments).')
    parser.add_argument('--cohort-dir', type=Path, default=None, help='Release-cohort directory to gate (default: var/release-cohort).')
    parser.add_argument('--evidence-dir', type=Path, default=None, help='Distribution-evidence directory to gate (default: var/distribution-install-readiness).')
    args = parser.parse_args(argv)
    report = build_report(skip_network=args.skip_network, cohort_directory=args.cohort_dir, evidence_directory=args.evidence_dir)
    if args.json:
