...
_OBJECT_NAME: Final[re.Pattern[str]] = re.compile('[0-9a-f]{40}')
PYPI_PROJECTS: Final[tuple[str, ...]] = ('cadrumo', 'cadrumo-data-manuals', 'cadrumo-data-official')
...
def manifest_floor(manifest_path: Path | None=None) -> str:
    ...
    """
    path = _dp_or('dev/release/version_identity.py:204:or', lambda: manifest_path, lambda: MANIFEST_PATH)
    try:
...
def _object_name(value: str, *, label: str) -> str:
    ...
    if _OBJECT_NAME.fullmatch(normalised) is None:
        raise VersionIdentityError(f"{label} {value!r} is not a 40-character object name; a branch name there would exempt every ref that targets that branch instead of this run's own")
    return normalised

def version_conflicts(version: str, *, owning_projects: Iterable[str]=(), target_projects: Iterable[str]=PYPI_PROJECTS, existing_tags: Iterable[str]=(), existing_releases: Iterable[str]=(), floor: str | None=None) -> tuple[str, ...]:
    ...
    if carrying and targets and (targets <= set(carrying)):
        refusals.append(f"package index already carries {version} for every project in the cohort ({', '.join(carrying)}); nothing is left to upload and an index upload cannot be undone, so this version can never be republished: cut a new version")
    tags = sorted(set(existing_tags))
    if tags:
        refusals.append(f"the tag namespace already carries {', '.join(tags)}, which this run did not cut; the version belongs to another release, so cut a new version")
    releases = sorted(set(existing_releases))
    if releases:
        refusals.append(f"the release namespace already carries {', '.join(releases)}, which this run was not dispatched for (drafts included, because a draft holds its tag); cut a new version")
    ...
    if floor is not None:
        ...
        if candidate < recorded:
            refusals.append(f'version {version} is below the recorded floor {floor}; the release line only moves forward, and a lower version would be ordered behind one already shipped, so every resolver would keep serving the older bytes as the newer release: cut a new version')
    return tuple(refusals)

def index_convergence_notice(version: str, *, owning_projects: Iterable[str]=(), target_projects: Iterable[str]=PYPI_PROJECTS) -> str | None:
    ...
    if not carrying or not remaining:
        return None
    return f"the package index already carries {version} for {', '.join(carrying)} and not yet for {', '.join(remaining)}; this run completes that partial upload, and the files already uploaded are checked against the index rather than replaced"

def gate_conflicts(gate: Gate, version: str, *, owning_projects: Iterable[str]=(), target_projects: Iterable[str]=PYPI_PROJECTS, existing_tags: Iterable[str]=(), existing_releases: Iterable[str]=(), floor: str | None=None) -> tuple[str, ...]:
    ...
    call site.
    """
    return version_conflicts(version, owning_projects=owning_projects if gate.checks_index else (), target_projects=target_projects, existing_tags=existing_tags if gate.checks_forge else (), existing_releases=existing_releases if gate.checks_forge else (), floor=floor if gate.checks_floor else None)

def pypi_projects_owning(version: str, *, projects: Iterable[str]=PYPI_PROJECTS, index_url: str=_PYPI_JSON_INDEX) -> tuple[str, ...]:
    ...
    for project in projects:
        request = urllib.request.Request(f'{index_url}/{project}/{number}/json', headers={'Accept': 'application/json'})
        try:
...
def _forge_refs(endpoint: str, jq: str) -> tuple[str, ...]:
    ...
    try:
        completed = subprocess.run(['gh', 'api', endpoint, '--paginate', '--jq', jq], capture_output=True, text=True, check=True, timeout=_PROBE_TIMEOUT_S * 3)
    except FileNotFoundError as exc:
...
DRAFT_BLIND_NOTICE: Final[str] = 'the release namespace was read with a credential that does not report push access, and the forge returns draft releases only to callers that have it; this answer covers published releases only'
...
def refs_owning(entries: Iterable[str], version: str, *, own_source_commit: str | None=None) -> tuple[str, ...]:
...
def assert_gate_permits(gate: Gate, version: str, *, owning_projects: Iterable[str]=(), target_projects: Iterable[str]=PYPI_PROJECTS, existing_tags: Iterable[str]=(), existing_releases: Iterable[str]=(), floor: str | None=None) -> None:
    ...
    """
    refusals = gate_conflicts(gate, version, owning_projects=owning_projects, target_projects=target_projects, existing_tags=existing_tags, existing_releases=existing_releases, floor=floor)
    if refusals:
        joined = '\n  - '.join(refusals)
...
def forge_arguments(repository: str | None, own_source_commit: str | None) -> tuple[str, str]:
    ...
    if not own_source_commit:
        raise VersionIdentityError('--own-source-commit is required: the tag and release being published already exist, and without the commit they sit on they cannot be told apart from a foreign ref')
...
def main(argv: list[str] | None=None) -> int:
    ...
    parser.add_argument('--scope', required=True, choices=tuple(GATES), help='seal: a cohort BUILD, which uploads nothing, so only a burned version is refused. publish: the irreversible upload, which additionally refuses a version every index project already carries, a tag or release namespace that owns it, and a version below the recorded floor.')
    parser.add_argument('--repository', help='owner/name of the forge repository; required by --scope publish')
    parser.add_argument('--own-source-commit', help="the commit this run's own tag and release sit on; required by --scope publish")
    ...
    try:
        ...
        if forge is not None:
            ...
            releases = forge_releases_owning(args.version, repository=repository, own_source_commit=own_source_commit)
            if not forge_push_access(repository):
                notices.append(DRAFT_BLIND_NOTICE)
        assert_gate_permits(gate, args.version, owning_projects=owning, existing_tags=tags, existing_releases=releases, floor=manifest_floor() if gate.checks_floor else None)
