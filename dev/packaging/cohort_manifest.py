...
_COMMIT_PATTERN: Final[str] = '^[0-9a-f]{40}$'
...
class ArtifactRecord(BaseModel):
    ...
    def _relative_normal_path(cls, value: str) -> str:
        ...
        if path.is_absolute() or value == '.' or '\\' in value or any((part in {'', '.', '..'} or ':' in part for part in path.parts)) or (path.as_posix() != value):
...
class CohortManifest(BaseModel):
    ...
    def _complete_and_canonical(self) -> Self:
        ...
        expected_id = cohort_identifier(version=self.version, source=self.source, artifacts=self.artifacts)
        if self.cohort_id != expected_id:
            raise ValueError(f'cohort identifier mismatch: expected {expected_id}, got {self.cohort_id}')
        if self.created_at.tzinfo is None or self.created_at.utcoffset() is None:
            raise ValueError('created_at must carry a timezone')
        return self
REQUIRED_ARTIFACT_KINDS: Final[dict[str, ArtifactKind]] = {'cadrumo-data-manuals-sdist': ArtifactKind.PYTHON_SDIST, 'cadrumo-data-manuals-wheel': ArtifactKind.PYTHON_WHEEL, 'cadrumo-data-official-sdist': ArtifactKind.PYTHON_SDIST, 'cadrumo-data-official-wheel': ArtifactKind.PYTHON_WHEEL, 'cadrumo-sdist': ArtifactKind.PYTHON_SDIST, 'cadrumo-wheel': ArtifactKind.PYTHON_WHEEL, 'cadrumo-source-archive': ArtifactKind.PYTHON_SOURCE_ARCHIVE, 'cadrumo-runtime-wheelhouse': ArtifactKind.PYTHON_WHEELHOUSE, 'homebrew-formula': ArtifactKind.HOMEBREW_FORMULA, 'python-cohort-manifest': ArtifactKind.PYTHON_MANIFEST, 'scoop-manifest': ArtifactKind.SCOOP_MANIFEST}
...
class LoadedReleaseCohort:
    ...
    def artifact(self, name: str) -> Path:
        ...
        record = _dp_next('dev/packaging/cohort_manifest.py:170:next', (item for item in self.manifest.artifacts if item.name == name), None)
        if record is None:
            raise KeyError(name)
        return (self.directory / PurePosixPath(record.path)).resolve(strict=True)

def cohort_identifier(*, version: str, source: SourceIdentity, artifacts: tuple[ArtifactRecord, ...]) -> str:
    """Hash stable source and artifact identities into one cohort identifier."""
    payload = CohortIdentityPayload(schema_name=_SCHEMA, version=version, source=source, artifacts=artifacts)
    canonical = json.dumps(payload.model_dump(mode='json'), ensure_ascii=False, separators=(',', ':'), sort_keys=True)
    return sha256_text(canonical)

def artifact_record(*, root: Path, name: str, kind: ArtifactKind, path: Path) -> ArtifactRecord:
    ...
    return ArtifactRecord(name=name, kind=kind, path=relative, sha256=sha256_path(artifact), size=artifact.stat().st_size)

def create_manifest(*, root: Path, version: str, source: SourceIdentity, created_at: datetime, builder: BuildIdentity, artifacts: Iterable[tuple[str, ArtifactKind, Path]]) -> CohortManifest:
    """Validate a complete set of artifact bytes and construct its authority."""
    records = tuple(sorted((artifact_record(root=root, name=name, kind=kind, path=path) for name, kind, path in artifacts), key=lambda item: item.name))
    return CohortManifest(schema_name=_SCHEMA, cohort_id=cohort_identifier(version=version, source=source, artifacts=records), version=version, source=source, created_at=created_at, builder=builder, artifacts=records)
...
def load_release_cohort(directory: Path) -> LoadedReleaseCohort:
    ...
    manifest = CohortManifest.model_validate_json(manifest_path.read_text(encoding=_UTF_8))
    ...
    if observed != declared:
        raise SystemExit(f'release cohort file inventory drifted: declared={sorted(declared)!r}, observed={sorted(observed)!r}')
    for record in manifest.artifacts:
        ...
        if artifact.stat().st_size != record.size:
            raise SystemExit(f'cohort artifact size mismatch for {record.name!r}: expected {record.size}, got {artifact.stat().st_size}')
        actual = sha256_path(artifact)
        if actual != record.sha256:
            raise SystemExit(f'cohort artifact digest mismatch for {record.name!r}: expected {record.sha256}, got {actual}')
    return LoadedReleaseCohort(directory=root, manifest_path=manifest_path, manifest=manifest)
__all__ = ['REQUIRED_ARTIFACT_KINDS', 'ArtifactKind', 'ArtifactRecord', 'BuildIdentity', 'CohortManifest', 'LoadedReleaseCohort', 'SourceIdentity', 'artifact_record', 'cohort_identifier', 'create_manifest', 'load_release_cohort', 'write_manifest']