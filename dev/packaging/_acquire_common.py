...
# reacquisition proves these three, and only these three, byte-for-byte; the
# third-party dependencies pulled by the ``[agent]`` extra are not cohort
# members and are out of scope for digest verification.
...
def match_downloaded_cohort_wheels(
    ...
    *,
    ...
    for distribution in PYTHON_COHORT_WHEEL_NAMES:
        ...
        matches = sorted(
            path for path in download_dir.glob("*.whl") if path.name.startswith(prefix)
        )
        if not matches:
            refuse_unavailable(
                mechanism=mechanism,
                ...
                next_step=(
                    f"publish {distribution}=={cohort.version} to {endpoint} and rerun this check"
                ),
            )
        if len(matches) != 1:
            raise AcquisitionError(
    ...
    for record in cohort.manifest.artifacts:
        ...
        if not candidate.is_file():
            refuse_unavailable(
                ...
                next_step=(
                    f"attach every cohort artifact to {endpoint} before verifying reacquisition"
                ),
            )
