...
if not __package__:
    _dp_mark('dev/packaging/installed_tax_oracle.py:26:ifnot')
...
CASILLAS = ('00501=100000.00', 'DP200013:00417=0.00', 'DP200013:00418=0.00', '01032=0.00', 'DP200014:00547=0.00', 'DP200014:01033=0.00', 'DP200014:01034=0.00')
BINDINGS = ('modelo-200-2024-profile-legal-entity-form=sl', 'modelo-200-2024-profile-new-entity-flag=0', 'modelo-200-2024-profile-incn-prior-12-months=500000', 'modelo-200-2024-profile-tributacion-estado-porcentaje=100', 'modelo-200-2024-bin-pendiente-ejercicios-anteriores=0', 'modelo-200-2024-dotaciones-deterioro-creditos-saldo-no-cumplido-anteriores=0', 'modelo-200-2024-dotaciones-deterioro-creditos-saldo-cumplido-anteriores=0')
RELATIONS = ('modelo-200-2024-rel-202-pagos-fraccionados=0', 'modelo-200-2024-rel-202-pagos-fraccionados-40-2=0')
...
class InstalledTaxEvidence:
    ...
    def to_jsonable(self) -> dict[str, Any]:
        ...
        document['commands'] = [{'argv': list(command.argv), 'cwd': command.cwd, 'started_at': command.started_at.isoformat(), 'completed_at': command.completed_at.isoformat(), 'duration_seconds': command.duration_seconds, 'returncode': command.returncode, 'stdout': command.stdout, 'stderr': command.stderr} for command in self.commands]
        return document
...
def ambient_product_executables_removed(environment: Mapping[str, str]) -> bool:
    ...
    return shutil.which('aeat', path=_dp_get('dev/packaging/installed_tax_oracle.py:147:get', environment, 'PATH', '')) is None
...
def isolated_product_environment(storage_root: Path) -> dict[str, str]:
    ...
    _dp_pop('dev/packaging/installed_tax_oracle.py:172:pop', environment, 'PYTHONHOME', None)
    _dp_pop('dev/packaging/installed_tax_oracle.py:173:pop', environment, 'PYTHONPATH', None)
    environment['PATH'] = path_without_product_executables(_dp_get('dev/packaging/installed_tax_oracle.py:174:get', environment, 'PATH', ''))
    environment.update({'CADRUMO_CLI_REVEAL_IDENTIFIERS': '1', 'CADRUMO_LOCAL_STORAGE_ROOT': str(resolved_root), 'CADRUMO_OUTPUT_LANGUAGE': 'en', 'CADRUMO_SECRET_STORE_BACKEND': 'unsecured', 'PYTHONIOENCODING': _UTF_8})
    return environment

def profile_create_arguments() -> tuple[str, ...]:
    """Return the public profile-creation argument sequence."""
    return ('config', 'profile', 'create', PROFILE_LABEL, '--quiet', '--accept-defaults', '--entity-type', 'legal_entity', '--legal-entity-form', 'sl', '--tax-id', PROFILE_TAX_ID, '--legal-name', 'Installed Oracle SL', '--activity', 'software services', '--incn-prior-12-months', '500000', '--no-new-entity-first-two-profit-periods', '--iva-regime', 'GENERAL', '--iva-m303-regime-composition', 'general', '--no-iva-redeme-enrolled', '--no-iva-cash-accounting-regime-enrolled', '--no-iva-voluntary-sii-enrolled', '--no-iva-hydrocarbon-deposit-advance-payment-deduction-entitled', '--tax-residence-jurisdiction-scope', 'common_regime', '--tax-residence-ccaa', 'madrid')

def work_create_arguments() -> tuple[str, ...]:
    """Return the public work-unit creation argument sequence."""
    return ('app', 'modelo', 'work', 'create', '--modelo', MODEL, '--year', YEAR, '--period', PERIOD, '--revision', REGISTRY_REVISION, '--name', 'Installed Modelo 200 oracle', '--by', 'installed-tax-oracle')

def work_calculate_arguments(work_unit_id: str) -> tuple[str, ...]:
    """Return the public calculation arguments for the created work unit."""
    arguments = ['app', 'modelo', 'work', 'calculate', work_unit_id]
...
def _run(argv: tuple[str, ...], *, cwd: Path, env: dict[str, str], timeout_seconds: float, input_text: str | None=None, inherited_descriptors: tuple[int, ...]=()) -> CommandResult:
    result = run_command(argv, cwd=cwd, environment=env, timeout_seconds=timeout_seconds, input_text=input_text, inherited_descriptors=inherited_descriptors)
    if result.returncode != 0:
        raise InstalledTaxOracleError(f'installed command failed ({result.returncode}): {argv!r}\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}')
    return result

def assert_envelope_contract(envelope: Mapping[str, Any], *, command: str, error: Callable[[str], Exception]) -> dict[str, Any]:
...
def assert_no_diagnostic_notices(envelope: Mapping[str, Any], *, command: str, error: Callable[[str], Exception], excused_codes: Collection[str]=()) -> None:
    ...
    """
    diagnostics = [notice for notice in envelope['notices'] if isinstance(notice, dict) and notice.get('severity') in {'warning', 'error'}]
...
def _json_envelope(evidence: CommandResult, *, expected_command: str) -> dict[str, Any]:
    try:
        ...
        raise InstalledTaxOracleError(f'{expected_command} did not emit one JSON document: {evidence.stdout!r}') from exc
...
def _assert_no_diagnostic_notices(document: dict[str, Any], *, command: str, authenticated: bool=False) -> None:
    assert_no_diagnostic_notices(document, command=command, error=InstalledTaxOracleError, excused_codes=ISOLATION_NOTICE_CODES if authenticated else ())

def create_installed_profile(cli: Path, *, cwd: Path, environment: dict[str, str], passphrase: str, timeout_seconds: float) -> CommandResult:
    ...
    """
    with enrolled_profile_creation(cli=cli, arguments=(*_JSON_FORMAT, *profile_create_arguments(), '--secrets-stdin')) as invocation:
        return _run(invocation.argv, cwd=cwd, env=environment, timeout_seconds=timeout_seconds, input_text=json.dumps({'passphrase': passphrase, 'passphrase_confirmation': passphrase}, separators=(',', ':')), inherited_descriptors=invocation.inherited_descriptors)

def assert_grounded_observations(observations_result: dict[str, Any], *, calculation_revision_id: str, work_unit_id: str) -> dict[str, Any]:
    ...
    ungrounded = [observation for observation in observations if not observation.get('legal_refs') or not observation.get('source_refs')]
    if ungrounded:
        raise InstalledTaxOracleError(f'{len(ungrounded)} persisted observations lack legal or source grounding')
    targets = [observation for observation in observations if observation.get('casilla_id') == TARGET_CASILLA]
    if len(targets) != 1:
        raise InstalledTaxOracleError(f'expected one {TARGET_CASILLA} observation, got {len(targets)}')
    target = targets[0]
    if Decimal(str(target.get('value'))) != EXPECTED_VALUE:
        raise InstalledTaxOracleError(f"{TARGET_CASILLA} expected {EXPECTED_VALUE}, got {target.get('value')!r}")
    if target.get('formula_id') != EXPECTED_FORMULA:
        raise InstalledTaxOracleError(f"{TARGET_CASILLA} expected formula {EXPECTED_FORMULA!r}, got {target.get('formula_id')!r}")
    if EXPECTED_LEGAL_REF not in target['legal_refs']:
        raise InstalledTaxOracleError(f'{TARGET_CASILLA} does not cite {EXPECTED_LEGAL_REF!r}')
    if EXPECTED_SOURCE_REF not in target['source_refs']:
        raise InstalledTaxOracleError(f'{TARGET_CASILLA} does not cite {EXPECTED_SOURCE_REF!r}')
    return target

def run_installed_tax_oracle(cli: Path, *, storage_root: Path, work_dir: Path, cohort_source_commit: str, cohort_manifest_sha256: str, cohort_root_wheel_sha256: str, timeout_seconds: float=180.0) -> InstalledTaxEvidence:
    ...
    version = _run((str(resolved_cli), '--version'), cwd=resolved_work_dir, env=environment, timeout_seconds=timeout_seconds)
    commands.append(version)
    profile = create_installed_profile(resolved_cli, cwd=resolved_work_dir, environment=environment, passphrase=passphrase, timeout_seconds=timeout_seconds)
    ...
    complete_setup = _run((*authenticated_base, 'config', 'profile', 'complete-setup'), cwd=resolved_work_dir, env=environment, timeout_seconds=timeout_seconds, input_text=profile_authentication)
    ...
    _assert_no_diagnostic_notices(complete_setup_document, command='config.profile.complete_setup', authenticated=True)
    create = _run((*authenticated_base, *work_create_arguments()), cwd=resolved_work_dir, env=environment, timeout_seconds=timeout_seconds, input_text=profile_authentication)
    ...
    work_unit_id = str(_dp_get('dev/packaging/installed_tax_oracle.py:531:get', create_document['result'], 'work_unit_id', ''))
    ...
    calculate = _run((*authenticated_base, *work_calculate_arguments(work_unit_id)), cwd=resolved_work_dir, env=environment, timeout_seconds=timeout_seconds, input_text=profile_authentication)
    ...
    calculation_revision_id = str(_dp_get('dev/packaging/installed_tax_oracle.py:547:get', result, 'calculation_revision_id', ''))
    if not _REVISION_ID.fullmatch(calculation_revision_id):
        raise InstalledTaxOracleError(f'calculation returned an invalid persisted revision id: {calculation_revision_id!r}')
    ...
    if not isinstance(casilla_values, dict) or Decimal(str(casilla_values.get(TARGET_CASILLA))) != EXPECTED_VALUE:
        raise InstalledTaxOracleError(f'calculation expected {TARGET_CASILLA}={EXPECTED_VALUE}, got {casilla_values!r}')
    ...
    if notice_codes - ISOLATION_NOTICE_CODES != EXPECTED_NOTICE_CODES:
        raise InstalledTaxOracleError(f'calculation notices expected {sorted(EXPECTED_NOTICE_CODES)!r}, got {sorted(notice_codes)!r}')
    ...
    observations = _run((*authenticated_base, 'app', 'modelo', 'work', 'observations', calculation_revision_id), cwd=resolved_work_dir, env=environment, timeout_seconds=timeout_seconds, input_text=profile_authentication)
    commands.append(observations)
    observations_document = _json_envelope(observations, expected_command='modelo.work.observations')
    _assert_no_diagnostic_notices(observations_document, command='modelo.work.observations', authenticated=True)
    target = assert_grounded_observations(observations_document['result'], calculation_revision_id=calculation_revision_id, work_unit_id=work_unit_id)
    return InstalledTaxEvidence(requested_executable=str(requested_cli), resolved_executable=str(resolved_cli), version_output=version.stdout.strip(), cohort_source_commit=cohort_source_commit, cohort_manifest_sha256=cohort_manifest_sha256, cohort_root_wheel_sha256=cohort_root_wheel_sha256, executable_sha256=executable_sha256, installed_wheel_payload_sha256=installed_payload_sha256, storage_root=str(storage_root.resolve()), work_unit_id=work_unit_id, calculation_revision_id=calculation_revision_id, target_casilla=TARGET_CASILLA, target_value=str(EXPECTED_VALUE), formula_id=str(target['formula_id']), legal_refs=tuple((str(value) for value in target['legal_refs'])), source_refs=tuple((str(value) for value in target['source_refs'])), notice_codes=tuple(sorted(notice_codes)), checkout_imports_removed=checkout_imports_removed(environment), ambient_product_executables_removed=ambient_product_executables_removed(environment), commands=tuple(commands))
...
def main() -> int:
    ...
    evidence = run_installed_tax_oracle(args.cli, storage_root=args.storage_root, work_dir=args.work_dir, cohort_source_commit=args.cohort_source_commit, cohort_manifest_sha256=args.cohort_manifest_sha256, cohort_root_wheel_sha256=args.cohort_root_wheel_sha256, timeout_seconds=args.timeout_seconds)
