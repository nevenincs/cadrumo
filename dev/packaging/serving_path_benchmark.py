...
from .installed_tax_oracle import isolated_product_environment, profile_create_arguments, work_calculate_arguments, work_create_arguments
...
_RESEARCH_PROJECTIONS: Final[dict[str, str]] = {'server_warm_calculate_projection_s': '1.5 (installed-cohort, research lighter-baseline)', 'subprocess_warm_calculate_target_s': '3.0 (installed-cohort, proven by the acquisition lanes)', 'subprocess_first_touch_target_s': '5.0 (installed-cohort, proven by the acquisition lanes)', 'subprocess_first_touch_former_cliff_s': '49.6 (installed v0.2.1 pre-fix)'}
...
class ServingPathEvidence:
    ...
    def to_jsonable(self) -> dict[str, Any]:
        ...
        return {'schema_version': self.schema_version, 'environment': self.environment, 'projections': self.projections, 'measurements': [asdict(measurement) for measurement in self.measurements], 'gate_failures': list(self.gate_failures)}
...
def subprocess_failure_message(returncode: int, argv: tuple[str, ...], stdout: str, stderr: str) -> str:
    ...
    for stream in (stderr, stdout):
        try:
            ...
            _dp_mark('dev/packaging/serving_path_benchmark.py:169:except')
            continue
        if isinstance(error, dict) and (error.get('code') or error.get('message')):
            named = _dp_or('dev/packaging/serving_path_benchmark.py:172:or', lambda: error.get('code'), lambda: 'unnamed refusal')
            detail = _dp_or('dev/packaging/serving_path_benchmark.py:173:or', lambda: error.get('message'), lambda: 'no message')
            summary = f'{summary}: {named} - {detail}'
            break
    return f'{summary}\nargv: {argv!r}\n{stdout}\n{stderr}'

def _timed_subprocess(argv: tuple[str, ...], *, env: dict[str, str], cwd: Path, timeout_s: float) -> tuple[float, float, str]:
    ...
    if timing.returncode != 0:
        raise ServingPathBenchmarkError(subprocess_failure_message(timing.returncode, argv, timing.stdout, timing.stderr))
...
def measure_subprocess(cli: Path, *, work_dir: Path, storage_root: Path, timeout_s: float) -> list[CallMeasurement]:
    ...
    version_wall, version_cpu, _ = _timed_subprocess((str(cli), '--version'), env=env, cwd=work_dir, timeout_s=timeout_s)
    out.append(ungated('version', version_wall, version_cpu))
    profile_wall, profile_cpu, _ = _timed_subprocess((*base, *profile_create_arguments()), env=env, cwd=work_dir, timeout_s=timeout_s)
    out.append(ungated('profile create', profile_wall, profile_cpu))
    first_touch_wall, first_touch_cpu, create_stdout = _timed_subprocess((*base, *work_create_arguments()), env=env, cwd=work_dir, timeout_s=timeout_s)
    out.append(CallMeasurement('work create (first-touch, fresh state)', 'subprocess', _ENVIRONMENT_IDENTITY, first_touch_wall, first_touch_cpu, True, _SUBPROCESS_FIRST_TOUCH_CLIFF_MAX_CPU_S, first_touch_cpu <= _SUBPROCESS_FIRST_TOUCH_CLIFF_MAX_CPU_S, 'cliff-gone gate in child CPU seconds (the 49.6 s cliff was compute, not waiting); the installed-cohort <= 5 s target is proven by the acquisition lanes'))
    work_unit_id = str(json.loads(create_stdout)['result']['work_unit_id'])
    warm_create_wall, warm_create_cpu, _ = _timed_subprocess((*base, *work_create_arguments()), env=env, cwd=work_dir, timeout_s=timeout_s)
    out.append(ungated('work create (warm)', warm_create_wall, warm_create_cpu, 'editable-tree per-process import floor; installed-cohort target proven by the acquisition lanes'))
    calculate_wall, calculate_cpu, _ = _timed_subprocess((*base, *work_calculate_arguments(work_unit_id)), env=env, cwd=work_dir, timeout_s=timeout_s)
    out.append(ungated('work calculate (warm)', calculate_wall, calculate_cpu, 'editable-tree per-process import floor; installed-cohort <= 3 s target proven by the lanes'))
    list_wall, list_cpu, _ = _timed_subprocess((*base, 'app', 'modelo', 'list'), env=env, cwd=work_dir, timeout_s=timeout_s)
...
def _server_environment(storage_root: Path) -> Generator[None]:
    ...
    overrides = {'CADRUMO_LOCAL_STORAGE_ROOT': str((storage_root / 'storage').resolve()), 'CADRUMO_SECRET_STORE_BACKEND': 'unsecured', 'CADRUMO_SECRET_STORE_DIR': str((storage_root / 'secrets').resolve()), 'CADRUMO_SECRET_PASSPHRASE': DEV_TEST_DATABASE_PASSWORD, 'CADRUMO_OUTPUT_LANGUAGE': 'en', 'CADRUMO_CLI_REVEAL_IDENTIFIERS': '1'}
    ...
    try:
        ...
        for key, value in previous.items():
            if value is None:
                _dp_pop('dev/packaging/serving_path_benchmark.py:296:pop', os.environ, key, None)
            else:
...
def measure_server_mode(*, storage_root: Path, acquire_timeout_s: float) -> list[CallMeasurement]:
    ...
    with _server_environment(storage_root):
        ...
        out.append(CallMeasurement('modelo list read', 'server', _ENVIRONMENT_IDENTITY, read_wall, read_cpu, True, _SERVER_READ_MAX_CPU_S, read_cpu <= _SERVER_READ_MAX_CPU_S, 'sub-second read bar in CPU seconds'))
        write_wall, write_cpu, _ = _run_in_process(work_create_arguments(), acquire_timeout_s=acquire_timeout_s)
        out.append(CallMeasurement('work create (simple write, idempotent re-touch)', 'server', _ENVIRONMENT_IDENTITY, write_wall, write_cpu, True, _SERVER_SIMPLE_WRITE_MAX_CPU_S, write_cpu <= _SERVER_SIMPLE_WRITE_MAX_CPU_S, 'sub-second simple-write bar in CPU seconds'))
        first_calc_wall, first_calc_cpu, _ = _run_in_process(work_calculate_arguments(work_unit_id), acquire_timeout_s=acquire_timeout_s)
        out.append(CallMeasurement('work calculate (first in-process)', 'server', _ENVIRONMENT_IDENTITY, first_calc_wall, first_calc_cpu, False, None, None, 'one-time lazy calc-engine import; recorded, not gated'))
        steady_calc_wall, steady_calc_cpu, _ = _run_in_process(work_calculate_arguments(work_unit_id), acquire_timeout_s=acquire_timeout_s)
        out.append(CallMeasurement('work calculate (warm steady-state)', 'server', _ENVIRONMENT_IDENTITY, steady_calc_wall, steady_calc_cpu, True, _SERVER_WARM_CALCULATE_MAX_CPU_S, steady_calc_cpu <= _SERVER_WARM_CALCULATE_MAX_CPU_S, 'research ~1.5 s was a lighter baseline; the 16-input M200 oracle measures ~1.7-1.9 CPU-seconds steady-state, meeting the low-single-digit-seconds bar; 2.5 CPU-s = honest steady-state + margin, load-immune by construction'))
        mcp_read_wall, mcp_read_cpu = _timed_build_server_read('review.queue', timeout_s=acquire_timeout_s)
        out.append(CallMeasurement('review.queue read (MCP memory transport)', 'server', _ENVIRONMENT_IDENTITY, mcp_read_wall, mcp_read_cpu, True, _SERVER_READ_MAX_CPU_S, mcp_read_cpu <= _SERVER_READ_MAX_CPU_S, 'full build_server SDK memory transport round-trip; sub-second read bar in CPU seconds'))
    return out

def _environment_block(cli: Path) -> dict[str, Any]:
    return {'identity': _ENVIRONMENT_IDENTITY, 'editable_install': True, 'python_version': platform.python_version(), 'platform': platform.platform(), 'aeat_executable': str(cli)}

def run_serving_path_benchmark(*, cli: Path | None=None, work_dir: Path | None=None, subprocess_timeout_s: float=120.0, acquire_timeout_s: float=120.0) -> ServingPathEvidence:
    ...
    measurements.extend(measure_subprocess(resolved_cli, work_dir=resolved_work_dir, storage_root=resolved_work_dir / 'subprocess-state', timeout_s=subprocess_timeout_s))
    measurements.extend(measure_server_mode(storage_root=resolved_work_dir / 'server-state', acquire_timeout_s=acquire_timeout_s))
    gate_failures = tuple((f'{measurement.label} [{measurement.mode}/{measurement.environment}] {measurement.cpu_seconds:.3f} CPU-s exceeds {measurement.threshold_cpu_seconds} CPU-s (wall {measurement.seconds:.3f}s, advisory)' for measurement in measurements if measurement.gated and measurement.within_threshold is False))
    return ServingPathEvidence(schema_version=_SCHEMA_VERSION, environment=_environment_block(resolved_cli), projections=_RESEARCH_PROJECTIONS, measurements=tuple(measurements), gate_failures=gate_failures)
...
def assert_acceptance(evidence: ServingPathEvidence) -> None:
    ...
    if evidence.gate_failures:
        raise ServingPathBenchmarkError('serving-path acceptance gate(s) failed:\n' + '\n'.join((f' - {failure}' for failure in evidence.gate_failures)))
...
def main() -> int:
    ...
    evidence = run_serving_path_benchmark(cli=args.cli, work_dir=args.work_dir, subprocess_timeout_s=args.subprocess_timeout_seconds, acquire_timeout_s=args.acquire_timeout_seconds)
