...
PROOF_SCOPE_PATHS: Final[tuple[str, ...]] = ('src', 'packaging', 'dev/packaging', 'pyproject.toml', 'uv.lock')
...
def environment_fingerprint() -> str:
    ...
    parts = (platform.system(), platform.machine(), platform.release(), platform.python_version(), uv_version)
...
def lookup(cache_dir: Path, proof_kind: str, source_fp: str, env_fp: str) -> ProofRecord | None:
    ...
    try:
        ...
        _dp_mark('dev/packaging/proof_cache.py:161:except')
        return None
    matches = record.proof_kind == proof_kind and record.source_fingerprint == source_fp and (record.environment_fingerprint == env_fp)
    return record if matches else None

def record(cache_dir: Path, proof_kind: str, source_fp: str, env_fp: str, repo_root: Path, max_records: int | None=None) -> Path:
    """Persist a fresh proof record for the key and return its path."""
    commit = _git_output(repo_root, 'rev-parse', 'HEAD').strip()
    proof = ProofRecord(proof_kind=proof_kind, source_fingerprint=source_fp, environment_fingerprint=env_fp, created_at=datetime.now(UTC).isoformat(timespec='seconds'), origin=ProofOrigin(commit=commit, run_id=os.environ.get('GITHUB_RUN_ID'), run_attempt=os.environ.get('GITHUB_RUN_ATTEMPT')))
...
def _evict_beyond_cap(cache_dir: Path, keep: Path, cap: int) -> None:
    ...
    for stale in records:
        ...
        try:
            ...
            _dp_mark('dev/packaging/proof_cache.py:217:except')
            continue
        excess -= 1