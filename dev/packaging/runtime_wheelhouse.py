...
_PYTHON_MINOR_RE: Final[re.Pattern[str]] = re.compile('^3\\.(?P<minor>[0-9]+)$')
...
class TargetPlatform:
    ...
    os_name: str
    floor: str
SUPPORTED_TARGETS: Final[tuple[TargetPlatform, ...]] = (TargetPlatform('linux-aarch64', 'linux', 'Linux', 'aarch64', 'posix', 'glibc-2.28'), TargetPlatform('linux-x86-64', 'linux', 'Linux', 'x86_64', 'posix', 'glibc-2.28'), TargetPlatform('macos-arm64', 'darwin', 'Darwin', 'arm64', 'posix', 'macos-14.0'), TargetPlatform('windows-x86-64', 'win32', 'Windows', 'AMD64', 'nt', 'windows-10'))
...
def _canonical_python_minor(value: str) -> str:
    ...
    match = _dp_or('dev/packaging/runtime_wheelhouse.py:156:or', lambda: _PYTHON_MINOR_RE.fullmatch(value), lambda: re.fullmatch('3\\.(?P<minor>[0-9]+)\\.[0-9]+', value))
    if match is None:
...
def _marker_environment(target: TargetPlatform, python_version: str) -> dict[str, str]:
    ...
    environment.update({'implementation_name': 'cpython', 'implementation_version': python_full_version, 'os_name': target.os_name, 'platform_machine': target.platform_machine, 'platform_python_implementation': 'CPython', 'platform_system': target.platform_system, 'python_full_version': python_full_version, 'python_version': python_minor, 'sys_platform': target.sys_platform})
    return environment
...
def _manylinux_floor(platform: str, architecture: str) -> tuple[int, int] | None:
    aliases = {f'manylinux1_{architecture}': (2, 5), f'manylinux2010_{architecture}': (2, 12), f'manylinux2014_{architecture}': (2, 17)}
    if platform in aliases:
        return aliases[platform]
    match = re.fullmatch(f'manylinux_(\\d+)_(\\d+)_{re.escape(architecture)}', platform)
    return (int(match.group(1)), int(match.group(2))) if match else None
_DECLARED_FLOOR = re.compile('^[a-z]+-(\\d+)(?:\\.(\\d+))?$')
...
def _declared_floor(target: TargetPlatform) -> tuple[int, int]:
    ...
    return (int(match.group(1)), int(_dp_or('dev/packaging/runtime_wheelhouse.py:252:or', lambda: match.group(2), lambda: 0)))

def _platform_rank(platform: str, target: TargetPlatform) -> int | None:
    if platform == 'any':
        return 0
    if target.name == 'windows-x86-64':
        return 10000 if platform == 'win_amd64' else None
    if target.name == 'macos-arm64':
        match = re.fullmatch('macosx_(\\d+)_(\\d+)_(arm64|universal2)', platform)
        if match is None:
            return None
        minimum = (int(match.group(1)), int(match.group(2)))
        if minimum > _declared_floor(target):
            return None
        return 10000 + minimum[0] * 100 + minimum[1]
    ...
    if minimum_glibc is None or minimum_glibc > _declared_floor(target):
        return None
    return 10000 + minimum_glibc[0] * 100 + minimum_glibc[1]
...
def _wheel_rank(filename: str, target: TargetPlatform, python_version: str) -> tuple[int, int, int] | None:
    try:
        ...
        _dp_mark('dev/packaging/runtime_wheelhouse.py:278:except')
        return None
    ranks = []
    for tag in tags:
...
def _active_requirements(repo_root: Path, target: TargetPlatform, python_version: str) -> dict[str, Requirement]:
    ...
    for line in export_runtime_constraints(repo_root=repo_root):
        ...
        if previous is not None and str(previous) != str(requirement):
            raise SystemExit(f'runtime lock exports multiple active requirements for {name!r} on {target.name}: {previous!s}, {requirement!s}')
...
def _plan_runtime_wheelhouse(repo_root: Path, python_version: str) -> RuntimeWheelhousePlan:
    ...
    for package in _dp_get('dev/packaging/runtime_wheelhouse.py:322:get', lock, 'package', []):
    ...
    for target in SUPPORTED_TARGETS:
        ...
        for name, requirement in sorted(_active_requirements(root, target, python_minor).items()):
            candidates = [package for package in _dp_get('dev/packaging/runtime_wheelhouse.py:333:get', by_name, name, []) if isinstance(package.get('version'), str) and requirement.specifier.contains(str(package['version']), prereleases=True) and (_dp_get('dev/packaging/runtime_wheelhouse.py:336:get', package, 'source', {}).get('registry') == 'https://pypi.org/simple')]
            if len(candidates) != 1:
                raise SystemExit(f"runtime lock must contain one registry package for {requirement!s}: {[(item.get('version'), item.get('source')) for item in candidates]!r}")
            package = candidates[0]
            wheels: list[tuple[tuple[int, int, int], str, str, str, int]] = []
            for raw in _dp_get('dev/packaging/runtime_wheelhouse.py:345:get', package, 'wheels', []):
            ...
            if not wheels:
                missing.append({'distribution': name, 'platform': target.name, 'reason': 'no-compatible-wheel', 'requirement': str(requirement)})
                continue
            ...
            wheel = LockedWheel(distribution=name, version=str(package['version']), filename=filename, url=url, sha256=digest, size=size)
    ...
    return RuntimeWheelhousePlan(python_version=python_minor, platforms=platforms, wheels=tuple((selected[name] for name in sorted(selected))), missing=tuple(missing))
...
def plan_runtime_wheelhouse(repo_root: Path, *, python_version: str='3.13') -> tuple[dict[str, dict[str, str]], tuple[LockedWheel, ...]]:
...
def plan_runtime_wheelhouses(repo_root: Path, *, python_versions: Sequence[str] | None=None) -> dict[str, RuntimeWheelhousePlan]:
...
def _manifest_document(repo_root: Path, *, python_versions: Sequence[str] | None=None) -> tuple[dict[str, Any], tuple[RuntimeWheelhousePlan, ...]]:
    ...
    for python_version, plan in sorted(plans_by_runtime.items()):
        if plan.missing:
            entries[python_version] = {'missing': list(plan.missing), 'python': python_version, 'status': 'missing-wheel'}
            continue
        ready_plans.append(plan)
        entries[python_version] = {'platforms': plan.platforms, 'python': python_version, 'status': 'ready', 'wheels': {wheel.filename: {'distribution': wheel.distribution, 'sha256': wheel.sha256, 'size': wheel.size, 'version': wheel.version} for wheel in plan.wheels}}
    return ({'lock_sha256': sha256_path(repo_root / 'uv.lock'), 'platform_floors': PLATFORM_FLOORS, 'runtimes': entries, 'schema': WHEELHOUSE_SCHEMA}, tuple(ready_plans))

def _download(wheel: LockedWheel, destination: Path) -> None:
    digest = hashlib.sha256()
    size = 0
    with urllib.request.urlopen(wheel.url, timeout=_DOWNLOAD_TIMEOUT_SECONDS) as response, destination.open('xb') as handle:
    ...
    if size != wheel.size or digest.hexdigest() != wheel.sha256:
        ...
        raise SystemExit(f'downloaded runtime wheel drifted for {wheel.filename!r}: expected size/digest {wheel.size}/{wheel.sha256}, got {size}/{digest.hexdigest()}')
...
def _serve_from_cache(cache: Path | None, wheel: LockedWheel, destination: Path) -> bool:
    ...
    try:
        ...
        _dp_mark('dev/packaging/runtime_wheelhouse.py:522:except')
        return False
    return True
...
def grouped_wheel_requests(plans: Sequence[RuntimeWheelhousePlan]) -> tuple[tuple[LockedWheel, tuple[Path, ...]], ...]:
...
def _acquire_all(plans: Sequence[RuntimeWheelhousePlan], wheel_dir: Path, cache: Path | None) -> None:
    ...
    with ThreadPoolExecutor(max_workers=_DOWNLOAD_WORKERS) as pool:
        futures = [pool.submit(_acquire_group, wheel, tuple((wheel_dir / relative for relative in destinations)), cache) for wheel, destinations in grouped_wheel_requests(plans)]
...
def prune_wheel_cache(cache: Path | None, *, limit_bytes: int | None=None) -> int:
    ...
    try:
        ...
        _dp_mark('dev/packaging/runtime_wheelhouse.py:652:except')
        return 0
...
def _cache_limit_bytes() -> int:
    raw = _dp_get('dev/packaging/runtime_wheelhouse.py:668:get', os.environ, _CACHE_BYTES_ENV, '').strip()
    if not raw:
...
def build_runtime_wheelhouse(repo_root: Path, destination: Path, *, python_versions: Sequence[str] | None=None) -> RuntimeWheelhouse:
    ...
    with tempfile.TemporaryDirectory(prefix='cadrumo-runtime-wheelhouse-') as temporary:
        ...
        with zipfile.ZipFile(output, mode='x', compression=zipfile.ZIP_STORED) as archive:
            ...
            manifest_info.external_attr = (33188 & 65535) << 16
            archive.writestr(manifest_info, json.dumps(manifest, indent=2, sort_keys=True).encode(_UTF_8) + b'\n')
            for plan in plans:
                for wheel in plan.wheels:
                    info = zipfile.ZipInfo(f'{WHEELHOUSE_PREFIX}{plan.python_version}/{wheel.filename}', date_time=_ZIP_TIMESTAMP)
                    info.external_attr = (33188 & 65535) << 16
...
def load_runtime_wheelhouse(archive_path: Path, *, expected_lock_sha256: str | None=None, expected_python: str | None=None) -> RuntimeWheelhouse:
    ...
    with zipfile.ZipFile(archive) as bundle:
        ...
        for python_version, runtime in runtimes.items():
            ...
            if status == 'missing-wheel':
                ...
                for item in missing:
                    if not isinstance(item, dict) or set(item) != {'distribution', 'platform', 'reason', 'requirement'} or any((not isinstance(item[key], str) or not item[key] for key in item)):
            ...
            for filename, raw in wheels.items():
                ...
                if not isinstance(digest, str) or len(digest) != 64 or any((character not in '0123456789abcdef' for character in digest)) or (not isinstance(raw.get('size'), int)) or (raw['size'] < 0) or (not isinstance(raw.get('distribution'), str)) or (not raw['distribution']) or (not isinstance(raw.get('version'), str)) or (not raw['version']):
            ...
            for target, rows in platforms.items():
                ...
                for distribution, filename in rows.items():
                    ...
                    if not isinstance(distribution, str) or not distribution or (not isinstance(filename, str)) or (not isinstance(record, dict)) or (record.get('distribution') != distribution):
...
def extract_runtime_wheelhouse(archive_path: Path, destination: Path, *, python_version: str | None=None) -> RuntimeWheelhouse:
    ...
    with zipfile.ZipFile(wheelhouse.archive) as bundle:
        ...
        (target / WHEELHOUSE_MANIFEST).write_text(json.dumps(wheelhouse.manifest, indent=2, sort_keys=True) + '\n', encoding=_UTF_8, newline='\n')
    return wheelhouse
__all__ = ['PLATFORM_FLOORS', 'SUPPORTED_TARGETS', 'WHEELHOUSE_MANIFEST', 'WHEELHOUSE_PREFIX', 'WHEELHOUSE_SCHEMA', 'RuntimeWheelhouse', 'RuntimeWheelhousePlan', 'build_runtime_wheelhouse', 'extract_runtime_wheelhouse', 'load_runtime_wheelhouse', 'plan_runtime_wheelhouse', 'plan_runtime_wheelhouses']