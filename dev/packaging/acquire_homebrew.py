...
from ._acquire_common import AcquisitionError, expected_oracle_target_value, require_command_succeeded
...
_FORMULA_SOURCE_TO_COHORT: Final[dict[str, str]] = {'cadrumo': 'cadrumo-sdist', 'cadrumo-data-manuals': 'cadrumo-data-manuals-sdist', 'cadrumo-data-official': 'cadrumo-data-official-sdist'}
...
def verify_homebrew_formula_digests(formula: dict[str, Any], cohort: PythonCohort) -> dict[str, str]:
    ...
    """
    stable = _dp_get('dev/packaging/acquire_homebrew.py:77:get', formula, 'urls', {}).get('stable')
    ...
    for resource in _dp_get('dev/packaging/acquire_homebrew.py:81:get', formula, 'resources', []):
    ...
    for source_name, cohort_key in _FORMULA_SOURCE_TO_COHORT.items():
        ...
        if actual != expected:
            raise AcquisitionError(f'public reacquisition digest mismatch: Homebrew formula {source_name!r} declares sha256 {actual} but the promoted cohort sdist declares {expected}; refusing')
...
def _resolve_brew(override: Path | None) -> Path:
    ...
    if found is None:
        raise AcquisitionError('brew (Homebrew) not found on PATH; run this reacquisition on macOS or Linux with Homebrew installed')
...
def _run(brew: Path, arguments: list[str], *, cwd: Path, log: Path, timeout: float) -> CommandResult:
    ...
    log.write_text(f'argv={json.dumps([str(brew), *arguments])}\nexit_code={completed.returncode}\nstdout:\n{completed.stdout}\nstderr:\n{completed.stderr}\n', encoding=_UTF_8, newline='\n')
    return completed

def run_homebrew_acquisition(*, cohort_dir: Path, evidence_dir: Path, tap: str, brew_executable: Path | None, timeout_seconds: float, release_cohort_dir: Path | None=None, row_id: str | None=None, distribution_evidence_dir: Path | None=None) -> Path:
    ...
    require_command_succeeded(returncode=tap_result.returncode, stderr=tap_result.stderr, mechanism='brew tap', endpoint=tap, version=cohort.version, next_step=f'publish the public Homebrew tap {tap} and rerun')
    install = _run(brew, ['install', qualified], cwd=run_root, log=logs / 'brew-install.log', timeout=timeout_seconds)
    require_command_succeeded(returncode=install.returncode, stderr=install.stderr, mechanism='brew install', endpoint=qualified, version=cohort.version, next_step=f'publish cadrumo {cohort.version} to the public tap {tap} and rerun')
    info = _run(brew, ['info', '--json=v2', qualified], cwd=run_root, log=logs / 'brew-info.log', timeout=timeout_seconds)
    ...
    prefix = _run(brew, ['--prefix', qualified], cwd=run_root, log=logs / 'brew-prefix.log', timeout=timeout_seconds)
    ...
    tax_evidence = run_installed_tax_oracle(cli=aeat, storage_root=run_root / 'oracle-state', work_dir=run_root / 'oracle-work', cohort_source_commit=cohort.source_commit, cohort_manifest_sha256=sha256_path(cohort.manifest), cohort_root_wheel_sha256=cohort.sha256['cadrumo'], timeout_seconds=timeout_seconds)
    ...
    evidence = {'schema': 'cadrumo.packaging.acquire-homebrew.v1', 'status': 'passed', 'completed_at': datetime.now(UTC).isoformat().replace('+00:00', 'Z'), 'tap': tap, 'formula': qualified, 'platform': platform.platform(), 'cohort': {'source_commit': cohort.source_commit, 'version': cohort.version}, 'verified_formula_digests': verified_digests, 'installed_prefix': str(installed_prefix), 'installed_tax_oracle': tax_evidence.to_jsonable(), 'installed_mcp_oracle': None}
    ...
    if release_cohort_dir is not None and row_id is not None:
        ...
        emit_installed_oracle_evidence(directory=_dp_or('dev/packaging/acquire_homebrew.py:264:or', lambda: distribution_evidence_dir, lambda: _DEFAULT_DISTRIBUTION_EVIDENCE_DIR), row_id=row_id, cohort=release_cohort, tax_evidence=tax_evidence, acquisition=AcquisitionIdentity(mechanism='brew', source=qualified), destination=DestinationIdentity(kind='homebrew-cellar', locator=str(installed_prefix), version=release_cohort.manifest.version))
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--release-cohort-dir', type=Path, default=None, help='Full release-cohort directory to bind a sanctioned flat evidence record to.')
    parser.add_argument('--row-id', default=None, help='Distribution row this run proves (required to emit the flat record).')
    parser.add_argument('--distribution-evidence-dir', type=Path, default=None, help='Where the flat record lands (defaults to var/distribution-install-readiness).')
    return parser

def main(argv: list[str] | None=None) -> int:
    """Run the Homebrew reacquisition verification from the command line."""
    args = _parser().parse_args(argv)
    evidence = run_homebrew_acquisition(cohort_dir=args.cohort_dir, evidence_dir=args.evidence_dir, tap=args.tap, brew_executable=args.brew, timeout_seconds=args.timeout_seconds, release_cohort_dir=args.release_cohort_dir, row_id=args.row_id, distribution_evidence_dir=args.distribution_evidence_dir)
    print(evidence)
    return 0
