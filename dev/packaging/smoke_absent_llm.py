...
from ._smoke_common import assert_cadrumo_version_output, assert_wheel_metadata_matches_pyproject, clean_product_env, create_pip_venv, install_targets_with_pip, optional_extra_registry, record_proof, relative_manifest_path, require_executable, resolve_work_dir, run_checked, run_checked_marker, venv_cadrumo_path, venv_python_path, wheel_metadata, write_smoke_manifest
from .python_cohort import PythonCohort, assert_installed_cohort, install_targets, load_python_cohort
...
_INFERENCE_SURFACES: Final[tuple[tuple[str, str, str], ...]] = (('cadrumo.llm.providers.local', 'rasterise_pdf_pages_to_base64_png', "rasterise_pdf_pages_to_base64_png(b'%PDF-1.4\\n')"), ('cadrumo.llm.evidence_draft_vision', 'transcribe_document_images', "transcribe_document_images(_PAGES, source_content_sha256='0' * 64)"), ('cadrumo.llm.evidence_draft_text', 'extract_invoice_fields_from_text', 'extract_invoice_fields_from_text(_TRANSCRIPTION)'), ('cadrumo.llm.vision_classifier', 'LocalVisionLLMClassifier', 'LocalVisionLLMClassifier(spec=None)'), ('cadrumo.llm.text_classifier', 'LocalTextLLMClassifier', 'LocalTextLLMClassifier(spec=None)'), ('cadrumo.llm.evidence_draft_vision', 'LocalVisionDocumentTranscriber', 'LocalVisionDocumentTranscriber()'), ('cadrumo.llm.evidence_draft_text', 'TextInvoiceFieldExtractor', 'TextInvoiceFieldExtractor()'), ('cadrumo.llm.column_role_mapping', 'SemanticColumnRoleMapper', 'SemanticColumnRoleMapper()'), ('cadrumo.llm.supply_nature_proposal', 'SupplyNatureProposer', 'SupplyNatureProposer()'))
...
def _guard_symbol_for_the_extra(repo_root: Path) -> str:
    ...
    if symbol not in symbols:
        raise SystemExit(f'the core optional-extra registry enumerates {sorted(symbols)!r}, which does not include {symbol!r}; the guarded-surface derivation below has nothing to key on.')
    return symbol
...
def _guarded_surfaces_from_production_guards(repo_root: Path, symbol: str) -> tuple[frozenset[str], frozenset[str]]:
    ...
    if not derived:
        raise SystemExit(f"no production call to require_optional_extra({symbol}) was found under {package}. The lane's guarded-surface set would be empty, and an empty set makes every completeness assertion below vacuously true -- which is indistinguishable from a pass.")
...
def _guarded_definition_names(tree: ast.Module, symbol: str) -> set[str]:
    ...
    def contains_guard(node: ast.AST) -> bool:
        return any((isinstance(child, ast.Call) and isinstance(child.func, ast.Name) and (child.func.id == 'require_optional_extra') and any((isinstance(arg, ast.Name) and arg.id == symbol for arg in child.args)) for child in ast.walk(node)))
    for node in tree.body:
        if isinstance(node, ast.FunctionDef | ast.AsyncFunctionDef | ast.ClassDef) and contains_guard(node):
            found.add(node.name)
    return found

def _assert_the_driver_reaches_every_guarded_surface(reachable: frozenset[str], internal: frozenset[str]) -> None:
    ...
    if unreached:
        raise SystemExit(f'these surfaces carry the production require_optional_extra({_EXTRA.upper()}_EXTRA) guard but the lane never drives them: {unreached!r}. The lane cannot claim every guarded surface returns to the refusal while some are never reached.')
    print(f'derived guarded surfaces (driven): {sorted(reachable)!r}', flush=True)
    if internal:
        print(f'derived guarded definitions NOT exported, so not operator-reachable and excluded from the completeness claim: {sorted(internal)!r}', flush=True)
    reachability_only = sorted(driven - reachable)
    if reachability_only:
        print(f'driven for reachability but carrying no guard of their own: {reachability_only!r}', flush=True)
...
def _install_core_with_the_extra(work_dir: Path, cohort: PythonCohort, venv_path: Path) -> None:
    ...
    install_targets_with_pip(work_dir, install_targets(cohort, root_artifact=cohort.root_wheel, extras=(_EXTRA,)), venv_path)

def _assert_guarded_surfaces_do_not_refuse(work_dir: Path, venv_path: Path, reachable: frozenset[str]) -> None:
    ...
    """
    calls = json.dumps([{'name': name, 'call': call} for _module, name, call in _INFERENCE_SURFACES if name in reachable])
    ...
    if refused:
        raise SystemExit(f'these guarded surfaces still reported the extra as missing WITH the extra installed: {refused!r}. Every refusal this lane observes after the uninstall would then be evidence of nothing.')
...
def _uninstall_the_extra(work_dir: Path, venv_path: Path, exclusive: frozenset[str]) -> None:
    ...
    run_checked([str(venv_python_path(venv_path)), '-m', 'pip', 'uninstall', '--disable-pip-version-check', '--yes', *sorted(exclusive)], cwd=work_dir)

def _assert_uninstall_restores_the_refusal(work_dir: Path, venv_path: Path, reachable: frozenset[str]) -> None:
    ...
    if wrong:
        raise SystemExit(f'these guarded surfaces did not return to the instructive refusal after the extra was uninstalled: {wrong!r}. A surface that keeps working once its dependencies are removed is reaching them by a path the guard does not cover.')
...
def _drive_surfaces(work_dir: Path, venv_path: Path, calls: str, *, leaf: str) -> list[dict[str, str]]:
    ...
    result = run_checked([str(venv_python_path(venv_path)), '-c', code], cwd=work_dir, env=_lane_env(work_dir, leaf))
    marker = 'SURFACE_OUTCOMES:'
    line = _dp_next('dev/packaging/smoke_absent_llm.py:426:next', (row for row in result.stdout.splitlines() if row.startswith(marker)), None)
    if line is None:
    ...
    if not parsed:
        raise SystemExit('the surface driver returned an empty outcome set, so every assertion over it holds vacuously; the guarded-surface set reaching the driver was empty.')
    return parsed
...
def _install_core_without_extras(work_dir: Path, cohort: PythonCohort, venv_path: Path) -> None:
    ...
    install_targets_with_pip(work_dir, install_targets(cohort, root_artifact=cohort.root_wheel, extras=()), venv_path)
...
def _assert_extra_is_real_in_the_artifact(wheel: Path) -> frozenset[str]:
    ...
    if not exclusive:
        raise SystemExit(f'every requirement of the {_EXTRA!r} extra ({sorted(extra_names)!r}) is also an unconditional core requirement, so installing the extra changes nothing and its guard can never fire. The extra is nominal; this lane cannot observe a refusal until at least one requirement is exclusive to it.')
...
def _assert_probe_target_is_exclusive_to_the_extra(repo_root: Path, wheel: Path) -> None:
    ...
    core_names = {normalise_distribution_name(requirement.name) for row in requirements if (requirement := Requirement(row)).marker is None}
    providing = _distributions_providing(import_name)
    supplied_by_core = providing & core_names
    if supplied_by_core:
        raise SystemExit(f'the {_EXTRA!r} extra probes the import name {import_name!r}, which is provided by {sorted(supplied_by_core)!r} -- an UNCONDITIONAL core requirement of the shipped wheel. The probe therefore succeeds in every core install, require_optional_extra is a permanent no-op, and no inference surface can refuse. Repoint the registry entry at an import name supplied only by the extra.')
...
def _distributions_providing(import_name: str) -> set[str]:
    ...
    found = {normalise_distribution_name(name) for name in _dp_get('dev/packaging/smoke_absent_llm.py:538:get', packages_distributions(), import_name, [])}
    return _dp_or('dev/packaging/smoke_absent_llm.py:539:or', lambda: found, lambda: {normalise_distribution_name(import_name)})
...
def _assert_extra_probes_absent(work_dir: Path, venv_path: Path) -> None:
    ...
    run_checked_marker([str(venv_python_path(venv_path)), '-c', code], cwd=work_dir, env=_lane_env(work_dir, 'probe-state'), marker='llm-extra-absent-ok')
...
def _assert_inference_surfaces_refuse(work_dir: Path, venv_path: Path) -> None:
    ...
    result = run_checked([str(venv_python_path(venv_path)), '-c', code], cwd=work_dir, env=_lane_env(work_dir, 'surface-state'))
    marker = 'SURFACE_OUTCOMES:'
    line = _dp_next('dev/packaging/smoke_absent_llm.py:624:next', (row for row in result.stdout.splitlines() if row.startswith(marker)), None)
    if line is None:
        raise SystemExit(f'the surface driver produced no outcomes; stdout was: {result.stdout!r}')
    ...
    wrong = [entry for entry in outcomes if entry['outcome'] != 'refused' or entry.get('extra') != _EXPECTED_EXTRA]
    if wrong:
        raise SystemExit(f"these inference surfaces did not refuse with the declared install guidance in a core install: {wrong!r}. Each must raise MissingOptionalExtraError naming extra {_EXPECTED_EXTRA!r}; a 'module-not-found' outcome is the raw failure the guard exists to convert, and a 'succeeded' outcome is a model-bearing surface running without the model-bearing dependencies.")
...
def _assert_core_cli_still_works(work_dir: Path, venv_path: Path) -> None:
    ...
    version = run_checked([str(venv_cadrumo_path(venv_path)), '--version'], cwd=work_dir, env=_lane_env(work_dir, 'cli-state'))
...
def _lane_env(work_dir: Path, leaf: str) -> dict[str, str]:
    ...
    root = work_dir / leaf
    return {**clean_product_env(), 'CADRUMO_LOCAL_STORAGE_ROOT': str(root), 'CADRUMO_DATABASE_URL': f"sqlite:///{(work_dir / f'{leaf}.db').as_posix()}", 'CADRUMO_OUTPUT_LANGUAGE': 'en'}

def main(argv: list[str] | None=None) -> int:
    """Run the absent-llm installed-wheel packaging smoke gate."""
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--python', default=f'{sys.version_info.major}.{sys.version_info.minor}', help='Expected Python major.minor for the stdlib venv.')
    parser.add_argument('--work-dir', help='Empty directory for venv and absent-extra artifacts.')
    parser.add_argument('--cohort-dir', required=True, type=Path, help='Directory containing the prebuilt immutable Python cohort.')
    ...
    assert_installed_cohort(venv_python_path(venv_path), cohort, root_artifact=wheel, cwd=work_dir)
    _assert_extra_probes_absent(work_dir, venv_path)
    ...
    manifest = write_smoke_manifest(work_dir, lane='absent-llm-wheel', artifacts={'wheel': relative_manifest_path(work_dir, wheel), 'venv': relative_manifest_path(work_dir, venv_path), 'venv_with_extra': relative_manifest_path(work_dir, extra_venv)}, declared=('wheel metadata dependency surface', _CLAIM_EXTRA_IS_REAL, _CLAIM_PROBE_IS_EXCLUSIVE, _CLAIM_GUARDED_SET_IS_DERIVED, 'stdlib venv creation', 'exact local cohort install with pip', 'pip dependency check', _CLAIM_EXTRA_PROBES_ABSENT, _CLAIM_SURFACES_REFUSE, _CLAIM_CORE_CLI_WORKS, _CLAIM_GUARDED_SURFACES_WORK, _CLAIM_UNINSTALL_RESTORES_REFUSAL), details={'cohort_version': cohort.version, 'python': args.python, 'surfaces': [name for _module, name, _call in _INFERENCE_SURFACES], 'guarded_surfaces': sorted(reachable), 'guarded_but_unexported': sorted(internal), 'uninstalled_distributions': sorted(exclusive)})
