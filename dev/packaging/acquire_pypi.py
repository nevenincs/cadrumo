...
from ._acquire_common import PYTHON_COHORT_WHEEL_NAMES, AcquisitionError, require_command_succeeded, run_installed_cli_oracle, venv_executable, verify_python_cohort_download
...
def _run(argv: list[str], *, cwd: Path, log: Path) -> CommandResult:
    ...
    log.write_text(f'argv={json.dumps(argv)}\ncwd={cwd}\nexit_code={completed.returncode}\nstdout:\n{completed.stdout}\nstderr:\n{completed.stderr}\n', encoding=_UTF_8, newline='\n')
    return completed

def _download_cohort_wheels(venv_python: Path, *, cohort: PythonCohort, index_url: str, download_dir: Path, run_root: Path, logs: Path) -> None:
    ...
    for distribution in PYTHON_COHORT_WHEEL_NAMES:
        ...
        completed = _run([str(venv_python), '-m', 'pip', 'download', '--no-deps', '--only-binary=:all:', '--index-url', index_url, '--dest', str(download_dir), f'{distribution}=={version}'], cwd=run_root, log=logs / f'pip-download-{distribution}.log')
        require_command_succeeded(returncode=completed.returncode, stderr=completed.stderr, mechanism='pip download', endpoint=index_url, version=version, next_step=f'publish {distribution}=={version} to {index_url} and rerun')

def run_pypi_acquisition(*, cohort_dir: Path, evidence_dir: Path, index_url: str, python: str, uv_executable: Path | None, timeout_seconds: float, release_cohort_dir: Path | None=None, row_id: str | None=None, distribution_evidence_dir: Path | None=None) -> Path:
    ...
    create = _run([str(uv), 'venv', str(venv), '--python', python, '--seed'], cwd=run_root, log=logs / 'uv-venv.log')
    ...
    _download_cohort_wheels(venv_python, cohort=cohort, index_url=index_url, download_dir=download_dir, run_root=run_root, logs=logs)
    verified = verify_python_cohort_download(download_dir, cohort, mechanism='pip download', endpoint=index_url)
    install = _run([str(uv), 'pip', 'install', '--python', str(venv_python), '--index-url', index_url, f'cadrumo=={cohort.version}', f'cadrumo-data-manuals=={cohort.version}', f'cadrumo-data-official=={cohort.version}'], cwd=run_root, log=logs / 'uv-pip-install.log')
    require_command_succeeded(returncode=install.returncode, stderr=install.stderr, mechanism='uv pip install', endpoint=index_url, version=cohort.version, next_step=f'publish cadrumo=={cohort.version} to {index_url} and rerun')
    aeat = venv_executable(venv, 'aeat')
    tax_evidence = run_installed_cli_oracle(cli=aeat, storage_root=run_root / 'oracle-state', work_dir=run_root / 'oracle-work', cohort=cohort, timeout_seconds=timeout_seconds)
    evidence = {'schema': 'cadrumo.packaging.acquire-pypi.v1', 'status': 'passed', 'completed_at': datetime.now(UTC).isoformat().replace('+00:00', 'Z'), 'index_url': index_url, 'cohort': {'source_commit': cohort.source_commit, 'version': cohort.version, 'sha256': {name: cohort.sha256[name] for name in PYTHON_COHORT_WHEEL_NAMES}}, 'verified_wheels': {name: str(path) for name, path in verified.items()}, 'installed_tax_oracle': tax_evidence.to_jsonable()}
    ...
    if release_cohort_dir is not None and row_id is not None:
        ...
        emit_installed_oracle_evidence(directory=_dp_or('dev/packaging/acquire_pypi.py:236:or', lambda: distribution_evidence_dir, lambda: _DEFAULT_DISTRIBUTION_EVIDENCE_DIR), row_id=row_id, cohort=release_cohort, tax_evidence=tax_evidence, acquisition=AcquisitionIdentity(mechanism='pip', source=index_url), destination=DestinationIdentity(kind='pypi-index-install', locator=str(venv), version=release_cohort.manifest.version))
...
def main(argv: list[str] | None=None) -> int:
    ...
    evidence = run_pypi_acquisition(cohort_dir=args.cohort_dir, evidence_dir=args.evidence_dir, index_url=args.index_url, python=args.python, uv_executable=args.uv, timeout_seconds=args.timeout_seconds, release_cohort_dir=args.release_cohort_dir, row_id=args.row_id, distribution_evidence_dir=args.distribution_evidence_dir)
    print(evidence)
    return 0
