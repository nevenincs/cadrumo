...
def _distributions_providing(import_name: str) -> set[str]:
    ...
    found = {normalise_distribution_name(name) for name in _dp_get('dev/packaging/tests/test_absent_llm_boundary.py:58:get', packages_distributions(), import_name, [])}
    return _dp_or('dev/packaging/tests/test_absent_llm_boundary.py:59:or', lambda: found, lambda: {normalise_distribution_name(import_name)})
...
def test_every_optional_extra_probes_a_package_core_does_not_supply() -> None:
    ...
    dormant = {extra: sorted(supplied) for extra, import_name in registry_extras.items() if (supplied := (_distributions_providing(import_name) & core_names))}
    assert dormant == {}, f'these extras probe an import name supplied by an unconditional core requirement: {dormant}. The probe therefore succeeds in every core install, require_optional_extra is a permanent no-op, and every surface behind that guard runs unguarded. Repoint the registry entry at an import name supplied only by the extra -- do not relax this gate, and do not delete the core requirement, which is load-bearing where it is declared.'
...
def test_the_llm_extra_declares_a_requirement_core_does_not() -> None:
    ...
    core_names = _core_requirement_names()
    exclusive = {normalise_distribution_name(requirement.split('>')[0].split('<')[0].split('=')[0].strip()) for requirement in extra_requirements} - core_names
    assert exclusive, f'every requirement of the llm extra ({extra_requirements!r}) is also an unconditional core requirement, so `pip install cadrumo[llm]` installs nothing `pip install cadrumo` did not, and the extra cannot be detected by any probe'
...
def test_the_surface_inventory_names_real_entry_points() -> None:
    ...
    assert missing == [], f'the absent-llm lane names surfaces that do not resolve: {missing}. The lane would fail on the import line having proved nothing about refusals.'
