...
_PACKAGING_WORKFLOWS: Final = ('packaging-smoke.yml', 'packaging-scoop.yml', 'packaging-homebrew.yml')
...
def _run_surface(document: dict[str, Any]) -> str:
    return '\n'.join((str(_dp_get('dev/packaging/tests/test_evidence_release_transport.py:49:get', step, 'run', '')) for step in _steps(document)))
...
def test_packaging_payloads_ride_artifacts(workflow: str) -> None:
    ...
    document = _document(workflow)
    uses = [str(_dp_get('dev/packaging/tests/test_evidence_release_transport.py:88:get', step, 'uses', '')) for step in _steps(document)]
...
def _workflow_files() -> tuple[Path, ...]:
    ...
    """
    assert _WORKFLOWS_DIR.is_dir(), f'no workflow directory at {_WORKFLOWS_DIR}; a relocated root walks nothing and every gate reading this walk would report the workflows clean'
    found = tuple(sorted((path for pattern in ('*.yml', '*.yaml') for path in scan_directory(_WORKFLOWS_DIR, pattern=pattern))))
    assert len(found) >= _MINIMUM_WORKFLOWS, f'only {len(found)} workflow(s) were walked; below this an empty finding list says nothing about what the workflows actually do'
    return found
...
def test_no_workflow_creates_a_release_by_hand() -> None:
    ...
    for path in _workflow_files():
        ...
        for job_name, job in (document.get('jobs') or {}).items():
            surface = '\n'.join((str(_dp_get('dev/packaging/tests/test_evidence_release_transport.py:141:get', step, 'run', '')) for step in job.get('steps') or []))
