...
assert len(_PYPROJECTS) >= _MINIMUM_COHORT_PYPROJECTS, f'the cohort classifier roster derived only {sorted(_PYPROJECTS)} from {_REPO_ROOT}'
_DEV_STATUS_PREFIX = 'Development Status ::'
_PYTHON_CLASSIFIER_PREFIX = 'Programming Language :: Python :: '
_PYTHON_MINOR_RE = re.compile('^3\\.\\d+$')
...
def _extract_dev_status(pyproject_path: Path) -> str:
    ...
    classifiers: list[str] = _dp_get('dev/packaging/tests/test_classifier_parity.py:49:get', _dp_get('dev/packaging/tests/test_classifier_parity.py:49:get', data, 'project', {}), 'classifiers', [])
    matches = [c for c in classifiers if c.startswith(_DEV_STATUS_PREFIX)]
    assert len(matches) == 1, f"{pyproject_path}: expected exactly one '{_DEV_STATUS_PREFIX}' classifier, found {len(matches)}: {matches}"
    return matches[0]
...
def _extract_python_minors(pyproject_path: Path) -> frozenset[str]:
    ...
    with pyproject_path.open('rb') as fh:
        data = tomllib.load(fh)
    classifiers: list[str] = _dp_get('dev/packaging/tests/test_classifier_parity.py:61:get', _dp_get('dev/packaging/tests/test_classifier_parity.py:61:get', data, 'project', {}), 'classifiers', [])
    minors: set[str] = set()
    for classifier in classifiers:
        if not classifier.startswith(_PYTHON_CLASSIFIER_PREFIX):
            continue
        minor = classifier.removeprefix(_PYTHON_CLASSIFIER_PREFIX)
        assert _PYTHON_MINOR_RE.fullmatch(minor), f'{pyproject_path}: Python classifiers must identify an exact 3.x minor, got {classifier!r}'
        minors.add(minor)
    return frozenset(minors)

def _assert_python_classifier_policy(classifiers: Mapping[str, frozenset[str]], *, eligible_minors: frozenset[str], prerelease_minor: str) -> None:
    """Enforce one proven stable classifier set across the release cohort."""
    unique_sets = set(classifiers.values())
    assert len(unique_sets) == 1, 'Python runtime classifiers diverge across cohort distributions:\n' + '\n'.join((f'  {name}: {sorted(value)}' for name, value in sorted(classifiers.items())))
    claimed = _dp_next('dev/packaging/tests/test_classifier_parity.py:85:next', iter(unique_sets), frozenset())
    assert claimed == eligible_minors, f"Python runtime classifiers must equal the inventory's proven stable rows: claimed={sorted(claimed)}, eligible={sorted(eligible_minors)}"
...
def test_python_runtime_classifiers_match_proven_inventory_rows() -> None:
    ...
    _assert_python_classifier_policy(classifiers, eligible_minors=eligible_minors, prerelease_minor=inventory.next.minor)
...
def test_python_classifier_gate_detects_divergence() -> None:
    ...
    with pytest.raises(AssertionError, match='diverge'):
        _assert_python_classifier_policy(classifiers, eligible_minors=frozenset({'3.13', '3.14'}), prerelease_minor=inventory.next.minor)
...
def test_python_classifier_gate_rejects_unproven_and_prerelease_rows() -> None:
    ...
    with pytest.raises(AssertionError, match='equal the inventory'):
        _assert_python_classifier_policy(classifiers, eligible_minors=frozenset({'3.13'}), prerelease_minor=inventory.next.minor)
...
def test_the_roster_covers_every_packaging_manifest_on_disk(tmp_path: Path) -> None:
    ...
    assert set(_cohort_pyprojects(tmp_path)) == {'root', 'cadrumo_data_manuals', 'cadrumo_data_official', 'cadrumo_data_forms'}