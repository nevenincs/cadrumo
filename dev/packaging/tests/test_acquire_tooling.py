...
import pytest
from .. import acquire_github_release, acquire_homebrew, acquire_pypi
from .._acquire_common import AcquisitionError, refuse_unavailable, require_command_succeeded, verify_artifact_digest, verify_python_cohort_download, verify_release_download
from ..cohort_manifest import REQUIRED_ARTIFACT_KINDS, BuildIdentity, LoadedReleaseCohort, SourceIdentity, create_manifest, load_release_cohort, write_manifest
...
def _make_python_cohort(tmp_path: Path) -> tuple[PythonCohort, Path]:
    ...
    download.mkdir()
    wheel_payloads = {'cadrumo': (f'cadrumo-{_VERSION}-py3-none-any.whl', b'cadrumo wheel bytes'), 'cadrumo-data-manuals': (f'cadrumo_data_manuals-{_VERSION}-py3-none-any.whl', b'manuals wheel bytes'), 'cadrumo-data-official': (f'cadrumo_data_official-{_VERSION}-py3-none-any.whl', b'official wheel bytes')}
    sha256: dict[str, str] = {}
    ...
    cohort = PythonCohort(directory=tmp_path, manifest=tmp_path / 'python-cohort.json', source_commit=_COMMIT, version=_VERSION, root_wheel=download / wheel_payloads['cadrumo'][0], root_sdist=tmp_path / f'cadrumo-{_VERSION}.tar.gz', source_archive=tmp_path / 'cadrumo-source.zip', runtime_wheelhouse=tmp_path / 'cadrumo-runtime-wheelhouse.zip', runtime_wheelhouse_manifest={}, manuals_wheel=download / wheel_payloads['cadrumo-data-manuals'][0], manuals_sdist=tmp_path / f'cadrumo_data_manuals-{_VERSION}.tar.gz', official_wheel=download / wheel_payloads['cadrumo-data-official'][0], official_sdist=tmp_path / f'cadrumo_data_official-{_VERSION}.tar.gz', sha256=sha256)
...
def _make_release_cohort(tmp_path: Path) -> tuple[LoadedReleaseCohort, Path]:
    ...
    manifest = create_manifest(root=root, version='0.99.0', source=SourceIdentity(commit=_COMMIT, tag='v0.99.0'), created_at=datetime.now(UTC), builder=BuildIdentity(implementation='dev.packaging.release_cohort', format_version=1, python='3.13.0', uv='0.5.0', platform='linux', architecture='x86_64', build_constraints_sha256='b' * 64), artifacts=artifacts)
...
def test_refuse_unavailable_names_endpoint_and_version() -> None:
    ...
    with pytest.raises(SystemExit) as caught:
        refuse_unavailable(mechanism='pip download', endpoint='https://pypi.org/simple', version='1.2.3', reason='the version is not on the index', next_step='publish it')
...
def test_require_command_succeeded_passes_on_zero() -> None:
    ...
    require_command_succeeded(returncode=0, stderr='', mechanism='gh release download', endpoint='nevenincs/cadrumo@v1.2.3', version='1.2.3', next_step='n/a')
...
def test_require_command_succeeded_refuses_on_nonzero() -> None:
    ...
    with pytest.raises(AcquisitionError) as caught:
        require_command_succeeded(returncode=1, stderr='release not found: v1.2.3', mechanism='gh release download', endpoint='nevenincs/cadrumo@v1.2.3', version='1.2.3', next_step='publish the v1.2.3 release')
    message = str(caught.value)
...
def test_verify_artifact_digest_accepts_matching_bytes(tmp_path: Path) -> None:
    ...
    assert verify_artifact_digest(mechanism='release download', endpoint='https://example/cadrumo.bin', name='cadrumo', path=artifact, expected_sha256=expected) == expected
...
def test_verify_artifact_digest_refuses_tampered_bytes(tmp_path: Path) -> None:
    ...
    with pytest.raises(AcquisitionError, match='digest mismatch'):
        verify_artifact_digest(mechanism='release download', endpoint='https://example/cadrumo.bin', name='cadrumo', path=artifact, expected_sha256='c' * 64)
...
def test_verify_python_cohort_download_accepts_matching_wheels(tmp_path: Path) -> None:
    ...
    resolved = verify_python_cohort_download(download, cohort, mechanism='pip download', endpoint='https://pypi.org/simple')
...
def test_verify_python_cohort_download_refuses_tampered_wheel(tmp_path: Path) -> None:
    ...
    with pytest.raises(AcquisitionError, match='digest mismatch'):
        verify_python_cohort_download(download, cohort, mechanism='pip download', endpoint='https://pypi.org/simple')
...
def test_verify_release_download_accepts_complete_cohort(tmp_path: Path) -> None:
    ...
    resolved = verify_release_download(cohort, download, mechanism='gh release download', endpoint='nevenincs/cadrumo@v0.99.0')
...
def test_verify_release_download_refuses_missing_asset(tmp_path: Path) -> None:
    ...
    missing.unlink()
    with pytest.raises(AcquisitionError) as caught:
        verify_release_download(cohort, download, mechanism='gh release download', endpoint='nevenincs/cadrumo@v0.99.0')
...
def _homebrew_formula(cohort: PythonCohort) -> dict[str, Any]:
    ...
    return {'urls': {'stable': {'url': 'https://example/cadrumo.tar.gz', 'checksum': cohort.sha256['cadrumo-sdist']}}, 'resources': [{'name': 'cadrumo-data-manuals', 'checksum': cohort.sha256['cadrumo-data-manuals-sdist']}, {'name': 'cadrumo-data-official', 'checksum': cohort.sha256['cadrumo-data-official-sdist']}]}
...
@pytest.mark.parametrize('parser_factory', [acquire_pypi._parser, acquire_github_release._parser, acquire_homebrew._parser])
def test_parsers_require_cohort_and_evidence_dirs(parser_factory, capsys: pytest.CaptureFixture[str]) -> None:
    """Every acquisition script requires --cohort-dir and --evidence-dir."""
    with pytest.raises(SystemExit):
        parser_factory().parse_args([])
    refusal = capsys.readouterr().err
    required_line = _dp_next('dev/packaging/tests/test_acquire_tooling.py:398:next', (line for line in refusal.splitlines() if 'the following arguments are required' in line), '')
