...
from ..evidence import AcquisitionIdentity, CommandTranscript, DestinationIdentity, DistributionEvidence, EvidenceStatus, ExecutionIsolation, InstalledExecutableIdentity, ResultIdentity, create_distribution_evidence, current_runtime_identity, evidence_identifier
from ..evidence_scrub import SCRUBBED_MACHINE_ID, SCRUBBED_USER, SCRUBBED_WORKSPACE, EvidenceScrubError, default_runner_tokens, default_workspace_roots, find_residual_leaks, scrub_distribution_evidence
...
def _leaking_evidence(cohort: LoadedReleaseCohort, *, observations: dict[str, JsonValue] | None=None) -> DistributionEvidence:
    """Build a real validated row saturated with runner metadata."""
    transcript = CommandTranscript.from_output(argv=(f'{_HOME}\\pipx\\venvs\\cadrumo\\Scripts\\aeat.exe', '--version'), cwd=f'{_HOME}\\work\\cadrumo', started_at=datetime(2026, 7, 20, 10, 0, tzinfo=UTC), completed_at=datetime(2026, 7, 20, 10, 1, tzinfo=UTC), exit_status=0, stdout='cadrumo 0.2.1', stderr='', relevant_output=('cadrumo 0.2.1', f'ran on {_HOSTNAME}'))
    result = ResultIdentity(status=EvidenceStatus.PASSED, assertions=('installed CLI computed DP200014:00562=23000.00',), observations={'cli_oracle': {'resolved_executable': f'{_HOME}\\pipx\\venvs\\cadrumo\\Scripts\\aeat.exe', 'storage_root': f'/home/{_USERNAME}/.local/state/cadrumo'}, **_dp_or('dev/packaging/tests/test_evidence_scrub.py:99:or', lambda: observations, lambda: {})})
    return create_distribution_evidence(row_id='python-windows-x86-64', cohort=cohort, runtime=current_runtime_identity(), client=None, isolation=ExecutionIsolation(checkout_imports_removed=True, ambient_product_executables_removed=True, installed_executables=(InstalledExecutableIdentity(name='aeat', path=f'{_HOME}\\pipx\\venvs\\cadrumo\\Scripts\\aeat.exe', sha256='a' * 64),)), acquisition=AcquisitionIdentity(mechanism='pip', source='https://pypi.org/simple'), commands=(transcript,), result=result, observed_at=datetime(2026, 7, 20, 10, 2, tzinfo=UTC), destination=DestinationIdentity(kind='pypi-index-install', locator=f'{_HOME}\\venv', version='0.2.1'))
...
def test_scrub_keeps_non_leaking_fields_and_structure_intact(tmp_path: Path) -> None:
    ...
    assert scrubbed.cohort == evidence.cohort
...
def test_novel_leaking_field_fails_closed(tmp_path: Path) -> None:
    ...
    """
    evidence = _leaking_evidence(_release_cohort(tmp_path / 'cohort'), observations={'brand_new_diag_field': {'deep': [f'logged at /Users/{_USERNAME}/Library/Logs']}})
...
def test_unc_path_is_refused_not_silently_shipped(tmp_path: Path) -> None:
    ...
    evidence = _leaking_evidence(_release_cohort(tmp_path / 'cohort'), observations={'share': '\\\\fileserver01\\evidence\\row.json'})
...
def test_escaped_windows_home_inside_embedded_json_is_scrubbed(tmp_path: Path) -> None:
    """A JSON-escaped ``C:\\\\Users\\\\name`` inside an observation string is caught."""
    embedded = json.dumps({'cwd': f'{_HOME}\\work'})
    evidence = _leaking_evidence(_release_cohort(tmp_path / 'cohort'), observations={'embedded_transcript': embedded})
    scrubbed = scrub_distribution_evidence(evidence, tokens=_TOKENS)
...
def test_windows_workspace_root_is_redacted_tail_preserved(tmp_path: Path) -> None:
    ...
    survive verbatim.
    """
    evidence = _leaking_evidence(_release_cohort(tmp_path / 'cohort'), observations={'workspace_diag': {'resolved_executable': f'{_WIN_WORKSPACE}\\var\\pip-venv\\Scripts\\aeat.exe', 'storage_root': f'{_WIN_WORKSPACE}\\var\\tax-oracle-state'}})
...
def test_posix_workspace_root_redaction_precedes_home_scrub(tmp_path: Path) -> None:
    ...
    evidence = _leaking_evidence(_release_cohort(tmp_path / 'cohort'), observations={'workspace_diag': {'cwd': f'{_POSIX_WORKSPACE}/var/outside-checkout'}})
...
def test_workspace_root_survives_fails_closed(tmp_path: Path) -> None:
    ...
    evidence = _leaking_evidence(_release_cohort(tmp_path / 'cohort'), observations={'workspace_diag': {'cwd': f'{_WIN_WORKSPACE}\\var\\state'}})
    document = evidence.model_dump(mode='json', exclude={'evidence_id'})
...
def test_default_workspace_roots_reads_ci_env_longest_first() -> None:
    ...
    env = {'GITHUB_WORKSPACE': '/home/runner/work/cadrumo/cadrumo', 'RUNNER_WORKSPACE': '/home/runner/work/cadrumo'}
