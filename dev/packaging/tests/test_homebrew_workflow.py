...
def test_homebrew_workflow_declares_every_generated_target_row() -> None:
    ...
    assert {(row['id'], _runner(row['runner']), row['expected_os'], row['expected_arch']) for row in rows} == {('macos-arm64', ('self-hosted', 'macOS', 'ARM64'), 'Darwin', 'arm64'), ('linux-x86_64', ('self-hosted', 'Linux', 'X64'), 'Linux', 'x86_64'), ('linux-arm64', ('self-hosted', 'Linux', 'ARM64'), 'Linux', 'aarch64')}
...
def test_homebrew_workflow_consumes_one_successful_commit_bound_cohort() -> None:
    ...
    commands = '\n'.join((str(_dp_get('dev/packaging/tests/test_homebrew_workflow.py:71:get', step, 'run', '')) for step in steps))
...
def test_homebrew_workflow_mints_every_row_from_the_immutable_cohort() -> None:
    ...
    job = document['jobs']['cadrumo-homebrew-acquisition']
    rows = job['strategy']['matrix']['include']
    assert {row['row_id'] for row in rows} == {'homebrew-macos-arm64', 'homebrew-linux-x86-64', 'homebrew-linux-arm64'}
    steps = job['steps']
    emit = next((step for step in steps if step.get('name') == 'Emit the sanctioned Homebrew distribution-evidence record'))
    ...
    artifact_names = [str((step.get('with') or {}).get('name')) for step in steps if 'upload-artifact' in str(_dp_get('dev/packaging/tests/test_homebrew_workflow.py:150:get', step, 'uses', ''))]
    ...
    assert steps.index(diagnostics) < steps.index(next((step for step in steps if step.get('name') == 'Clean up the retained Homebrew install')))
...
def test_the_structural_check_detects_a_branch_test_that_gates_a_dispatch() -> None:
    ...
    """
    pre_fix = chr(10).join(('test "$(jq -r .conclusion <<<"$run_json")" = "success"', 'test "$(jq -r .head_branch <<<"$run_json")" = "main"', 'if [ "$event" = "workflow_dispatch" ]; then', '  ancestry=$(gh api .../compare/main...$sha --jq .status)', 'else', '  test "$event" = "push"', 'fi'))
