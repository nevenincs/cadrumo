...
from .._smoke_common import build_companion_wheels, build_wheel, create_pip_venv, head_extract, install_targets_with_pip, isolated_product_env, venv_cadrumo_path, venv_python_path
...
def installed_cohort(tmp_path_factory: pytest.TempPathFactory) -> _InstalledCohort:
    ...
    install_targets_with_pip(llm_work_dir, (f'{root_wheel.resolve()}[llm]', *(str(wheel.resolve()) for wheel in data_wheels)), llm_venv)
    return _InstalledCohort(work_dir=work_dir, core_python=venv_python_path(core_venv), llm_python=venv_python_path(llm_venv), core_cli=venv_cadrumo_path(core_venv))
...
def _isolated_environment(work_dir: Path, *, locale: str, state_name: str) -> dict[str, str]:
    ...
    _dp_pop('dev/packaging/tests/test_overview_provisioning_action_recovery.py:109:pop', environment, 'PYTHONPATH', None)
...
def _assert_projection_chain(row: Mapping[str, object], *, expected_condition: str | None) -> None:
    ...
    assert _text_action_projection(cast(Iterable[object], row['text']), service=cast(str, payload['service'])) == resolved
...
def _json_envelope(completed: subprocess.CompletedProcess[str], *, command: str) -> dict[str, object]:
    ...
    for stream in (completed.stdout, completed.stderr):
        try:
            ...
            _dp_mark('dev/packaging/tests/test_overview_provisioning_action_recovery.py:244:except')
            pass
        else:
        ...
        for row in reversed(stream.splitlines()):
            try:
                ...
                _dp_mark('dev/packaging/tests/test_overview_provisioning_action_recovery.py:252:except')
                continue
...
def _overview_console_matrix(cohort: _InstalledCohort) -> dict[str, dict[str, object]]:
    ...
    for locale in SUPPORTED_OUTPUT_LANGUAGES:
        json_result = subprocess.run([str(cohort.core_cli), '--format', 'json', 'app', 'overview', 'status'], cwd=cohort.work_dir, env=_isolated_environment(cohort.work_dir, locale=locale, state_name=f'overview-{locale}'), capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=120, check=False)
        text_result = subprocess.run([str(cohort.core_cli), '--format', 'text', 'app', 'overview', 'status'], cwd=cohort.work_dir, env=_isolated_environment(cohort.work_dir, locale=locale, state_name=f'overview-{locale}-text'), capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=120, check=False)
        matrix[locale] = {'json_exit_code': json_result.returncode, 'json': _json_envelope(json_result, command='overview.status'), 'text_exit_code': text_result.returncode, 'text_output_present': bool(_dp_or('dev/packaging/tests/test_overview_provisioning_action_recovery.py:289:or', lambda: text_result.stdout, lambda: text_result.stderr))}
    return matrix
...
def test_real_optional_extra_outcomes_reach_config_json_and_text_without_an_overview_mixup(installed_cohort: _InstalledCohort) -> None:
    """Core absence and extra presence preserve typed no-recovery projections in all locales."""
    absent = _run_provisioning_matrix(work_dir=installed_cohort.work_dir, python=installed_cohort.core_python, state_name='core-provisioning')
    present = _run_provisioning_matrix(work_dir=installed_cohort.work_dir, python=installed_cohort.llm_python, state_name='llm-provisioning')
    ...
    expected_absent_conditions = (ProvisioningPreconditionCondition.OPTIONAL_EXTRA_IMPORTABLE.value, ProvisioningPreconditionCondition.LOCAL_MODEL_MODEL_REQUIRES_EXTRA.value)
    ...
    for locale in SUPPORTED_OUTPUT_LANGUAGES:
        ...
        structural = [{key: value for key, value in _object(row, label='absent provisioning row').items() if key != 'text'} for row in rows]
        if baseline is None:
            baseline = structural
        else:
