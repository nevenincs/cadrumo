...
_SHELL_COMMAND_BOUNDARY = '(?:^|[\\r\\n]|&&|\\|\\||[;|])'
_ENVIRONMENT_PREFIX = '(?:(?:env\\s+)?(?:[A-Za-z_]\\w*=(?:[^\\s;&|]+|\\"[^\\"]*\\"|\'[^\']*\')\\s+)+)'
_UV_RUN_PREFIX = '(?:uv\\s+run(?:\\s+--[a-z][\\w-]*(?:=(?:[^\\s;&|]+|\\"[^\\"]*\\"|\'[^\']*\'))?)*\\s+)?'
_PROHIBITED_CADRUMO_HUMAN_COMMAND = re.compile(f'(?im){_SHELL_COMMAND_BOUNDARY}[ \\t]*(?:{_ENVIRONMENT_PREFIX})?{_UV_RUN_PREFIX}cadrumo(?=\\s|$|[;&|])')
_PROHIBITED_AEAT_PRODUCT_FORMS = (('python-import', re.compile('(?i)\\b(?:from\\s+aeat(?:\\.|\\s+import\\b)|import\\s+(?:[a-z_]\\w*(?:\\.[a-z_]\\w*)*\\s*,\\s*)*aeat(?:\\.|(?=\\s|$|[;"\'])))')), ('python-module', re.compile('(?i)\\bpython(?:\\d+(?:\\.\\d+)*)?\\s+-m\\s+aeat(?:\\.[a-z_]\\w*)*(?=\\s|$)')), ('distribution-install', re.compile('(?i)\\b(?:(?:uv\\s+)?pip\\s+install|uv\\s+add)\\b[^&|;\\r\\n]*?(?<![\\w-])aeat(?=\\[|\\s|$|[<>=!~@;"\'])')), ('former-distribution', re.compile('(?i)(?<![\\w-])aeat(?:-cli|-data(?:-[\\w-]+)?|_data(?:_[\\w-]+)?)(?![\\w-])')), ('former-source-path', re.compile('(?i)(?<![\\w])(?:src|packaging)[/\\\\]aeat(?:[/\\\\_.-]|$)')))
...
_PORTABLE_LEGS: dict[str, dict[str, object]] = {'cadrumo-packaging-smoke-windows': {'name': 'Cadrumo / Windows / Python 3.13 / wheel artifacts', 'runs_on': ['self-hosted', 'Windows', 'X64'], 'cohort_asset': 'cadrumo-python-cohort-windows.tar.gz', 'evidence_asset': 'packaging-smoke-evidence-windows.tar.gz'}, 'cadrumo-packaging-smoke-macos': {'name': 'Cadrumo / macOS / Python 3.13 / wheel artifacts', 'runs_on': ['self-hosted', 'macOS', 'ARM64'], 'cohort_asset': 'cadrumo-python-cohort-macos.tar.gz', 'evidence_asset': 'packaging-smoke-evidence-macos.tar.gz'}}
...
def _run_command_lines(job: dict[str, object]) -> set[str]:
    ...
    return {line.strip() for step in steps if isinstance(step, dict) for line in str(_dp_get('dev/packaging/tests/test_packaging_smoke_workflow.py:95:get', step, 'run', '')).splitlines() if line.strip()}
...
def _assert_single_release_cohort_builder(document: dict[str, object]) -> None:
    ...
    builders = [(name, job) for name, job in jobs.items() if isinstance(job, dict) and 'uv run --no-sync python -m dev.packaging.release_cohort build --output var/release-cohort' in _run_command_lines(job)]
...
def test_immutable_cohort_is_built_once_and_every_python_row_binds_it() -> None:
    ...
    checkout = next((step for step in build['steps'] if str(_dp_get('dev/packaging/tests/test_packaging_smoke_workflow.py:127:get', step, 'uses', '')).startswith('actions/checkout@')))
    ...
    build_surface = '\n'.join((str(_dp_get('dev/packaging/tests/test_packaging_smoke_workflow.py:131:get', step, 'run', '')) for step in build['steps'] if 'run' in step))
    assert 'cadrumo-release-cohort.tar.gz' in build_surface
    build_uses = '\n'.join((str(_dp_get('dev/packaging/tests/test_packaging_smoke_workflow.py:133:get', step, 'uses', '')) for step in build['steps']))
    assert 'actions/upload-artifact@' in build_uses
    assert '--sort=name --mtime=@0 --owner=0 --group=0 --numeric-owner' in build_surface
    legs = {'oracle-emit-linux': ('python-linux-x86-64', ['self-hosted', 'Linux', 'X64']), 'oracle-emit-windows': ('python-windows-x86-64', ['self-hosted', 'Windows', 'X64']), 'oracle-emit-macos': ('python-macos-arm64', ['self-hosted', 'macOS', 'ARM64'])}
    for job_name, (row_id, runs_on) in legs.items():
        leg = jobs[job_name]
        assert leg['needs'] == 'build-release-cohort'
        assert leg['runs-on'] == runs_on
        surface = '\n'.join((str(_dp_get('dev/packaging/tests/test_packaging_smoke_workflow.py:150:get', step, 'run', '')) for step in leg['steps'] if 'run' in step))
        ...
        leg_uses = '\n'.join((str(_dp_get('dev/packaging/tests/test_packaging_smoke_workflow.py:156:get', step, 'uses', '')) for step in leg['steps']))
...
def test_workflow_runs_canonical_cadrumo_packaging_gates() -> None:
    ...
    assert set(document['jobs']) == {'cadrumo-packaging-smoke', *_PORTABLE_LEGS, 'build-release-cohort', 'oracle-emit-linux', 'oracle-emit-windows', 'oracle-emit-macos', 'runner-queue-watchdog'}
    ...
    for key, spec in _PORTABLE_LEGS.items():
        ...
        assert {'just packaging-smoke', 'uv run --no-sync python -m dev.packaging.evidence'} <= leg_commands
...
def test_workflow_evidence_and_product_identity_follow_the_binding_tuple() -> None:
    ...
    campaign = next((step for step in job['steps'] if 'just packaging-smoke-ci' in str(_dp_get('dev/packaging/tests/test_packaging_smoke_workflow.py:226:get', step, 'run', ''))))
    checkpoint = next((step for step in job['steps'] if step.get('run') == 'uv run --no-sync python -m dev.packaging.evidence'))
    ...
    for key, spec in _PORTABLE_LEGS.items():
        ...
        leg_checkpoint = next((s for s in leg['steps'] if s.get('run') == 'uv run --no-sync python -m dev.packaging.evidence'))
    ...
    asset_names = re.findall('(?:cadrumo-python-cohort|packaging-smoke-evidence)-(?:linux|windows|macos)\\.tar\\.gz', _WORKFLOW.read_text(encoding='utf-8'))
    ...
    for one_job in document['jobs'].values():
        ...
        for step in one_job['steps']:
            label_lines.append(str(_dp_get('dev/packaging/tests/test_packaging_smoke_workflow.py:278:get', step, 'name', '')))
            if 'run' in step:
...
@pytest.mark.parametrize('surface', ('aeat --version', 'uv run --no-sync aeat app registry verify', "echo 'AEAT is the Spanish tax authority'", 'pip install cadrumo && aeat --version'))
...
@pytest.mark.parametrize('surface', ('echo preflight\ncadrumo --version', 'uv run --frozen cadrumo app registry verify', 'env MODE=ci cadrumo --version', 'MODE=ci cadrumo --version', 'echo preflight && cadrumo --version'))
...
@pytest.mark.parametrize(('surface', 'expected_family'), (('from aeat import core', 'python-import'), ('python -c "import os, aeat"', 'python-import'), ('python -m aeat config check', 'python-module'), ('pip install aeat', 'distribution-install'), ('pip install aeat-data-official', 'former-distribution'), ('uv add aeat-data-manuals', 'former-distribution'), ('ruff check src/aeat/', 'former-source-path'), ('uv build packaging/aeat_data_official', 'former-distribution')))
...
def test_the_version_identity_guard_runs_before_the_cohort_is_built() -> None:
    ...
    names = [str(_dp_get('dev/packaging/tests/test_packaging_smoke_workflow.py:361:get', step, 'name', '')) for step in build['steps']]
    surface = '\n'.join((str(_dp_get('dev/packaging/tests/test_packaging_smoke_workflow.py:362:get', step, 'run', '')) for step in build['steps'] if 'run' in step))
...
def test_the_seal_guard_asks_no_destination_anything() -> None:
    ...
    """
    build = _cohort_build_job()
    guard = next((step for step in build['steps'] if 'Refuse to seal' in str(_dp_get('dev/packaging/tests/test_packaging_smoke_workflow.py:394:get', step, 'name', ''))))
    ...
    assert 'GH_TOKEN' not in str(_dp_get('dev/packaging/tests/test_packaging_smoke_workflow.py:398:get', guard, 'env', {})), 'the seal step needs no forge credential'