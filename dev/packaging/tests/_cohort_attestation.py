...
def add_test_runtime_wheelhouse(directory: Path, artifacts: dict[str, str], digests: dict[str, str]) -> Path:
    ...
    floors = {'linux-aarch64': 'glibc-2.28', 'linux-x86-64': 'glibc-2.28', 'macos-arm64': 'macos-14.0', 'windows-x86-64': 'windows-10'}
    platform_rows = {target: rows for target in ('linux-aarch64', 'linux-x86-64', 'macos-arm64', 'windows-x86-64')}
    wheel_record = {filename: {'distribution': 'cadrumo-test-dependency', 'sha256': hashlib.sha256(payload).hexdigest(), 'size': len(payload), 'version': '1.0.0'}}
    runtime_entries = {runtime: {'platforms': platform_rows, 'python': runtime, 'status': 'ready', 'wheels': wheel_record} for runtime in ('3.13', '3.14')}
    manifest = {'lock_sha256': hashlib.sha256(lock).hexdigest(), 'platform_floors': floors, 'runtimes': runtime_entries, 'schema': 'cadrumo.runtime-wheelhouse.v3'}
...
def make_test_command_spec_attestation(directory: Path, artifacts: dict[str, str], *, source_commit: str) -> dict[str, object]:
    ...
    try:
        member_digest = _projection_digest(_artifact_command_projection(root_wheel, root_sdist, directory / artifacts['source-archive']))
    except (OSError, tarfile.TarError, zipfile.BadZipFile):
        _dp_mark('dev/packaging/tests/_cohort_attestation.py:91:except')
        member_digest = '0' * 64
    value: dict[str, object] = {'schema': 'cadrumo.command-spec-cohort.v1', 'node_count': 1, 'source_commit': source_commit, 'forbidden_artifacts_absent': True, 'root_wheel_sha256': sha256_path(root_wheel), 'root_sdist_sha256': sha256_path(root_sdist), 'source_archive_sha256': sha256_path(directory / artifacts['source-archive']), 'artifact_members_sha256': member_digest, 'origins_sha256': '1' * 64, 'identities_sha256': 'b' * 64, 'locales_sha256': 'c' * 64, 'policies_sha256': 'd' * 64, 'schemas_sha256': 'e' * 64, 'import_budgets_sha256': 'f' * 64}
    value['envelope_sha256'] = _projection_digest(value)
    return value

def make_minimal_test_python_cohort(directory: Path, *, version: str, source_commit: str='a' * 40) -> dict[str, str]:
    ...
    for distribution in ('cadrumo', 'cadrumo-data-manuals', 'cadrumo-data-official'):
        ...
        requires = (f'cadrumo-data-manuals=={version}', f'cadrumo-data-official=={version}') if distribution == 'cadrumo' else ()
        metadata = '\n'.join((f'Name: {distribution}', f'Version: {version}', *(f'Requires-Dist: {item}' for item in requires), '')).encode()
    ...
    manifest = {'artifacts': artifacts, 'sha256': digests, 'source_commit': source_commit, 'version': version, 'command_spec_attestation': make_test_command_spec_attestation(directory, artifacts, source_commit=source_commit)}
