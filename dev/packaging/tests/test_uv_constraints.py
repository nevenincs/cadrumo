...
from ..uv_constraints import export_runtime_constraints, local_product_packages, render_constraints_file
...
def test_the_product_rows_are_excluded(exported: tuple[str, ...]) -> None:
    ...
    assert not named & local, f'a bundle-local product wheel was pinned as an index requirement: {sorted(named & local)}'
...
def test_the_excluded_names_are_the_lockfile_s_own_non_registry_packages() -> None:
    ...
    packages = [entry for entry in _dp_get('dev/packaging/tests/test_uv_constraints.py:101:get', document, 'package', ()) if isinstance(entry, dict)]
...
def test_a_lockfile_with_no_local_package_refuses(tmp_path: pathlib.Path) -> None:
    ...
    """
    (tmp_path / 'uv.lock').write_text('[[package]]' + chr(10) + 'name = "anyio"' + chr(10) + 'source = { registry = "https://pypi.org/simple" }' + chr(10), encoding='utf-8')
...
def test_a_missing_lockfile_refuses_rather_than_exporting_nothing(tmp_path: pathlib.Path) -> None:
...
def test_every_rendered_row_is_terminated() -> None:
    ...
    assert [line for line in rendered.splitlines() if not line.startswith('#')] == ['alpha==1.0', 'bravo==2.0']
