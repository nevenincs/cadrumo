...
_DISCOVERY_EXCLUSIONS: Final[frozenset[str]] = frozenset({'build_scratch_reclaim.py', 'test_var_scratch_mint_sites_are_registered.py'})
_HIDDEN_JOIN: Final[re.Pattern[str]] = re.compile('/\\s*f?["\'](\\.[^"\'\\n]*)["\']', re.DOTALL)
_INTERPOLATION: Final[re.Pattern[str]] = re.compile('\\{[^{}]*\\}')
_BODY_STANDIN: Final[str] = 'BODY'
_NOT_VAR_SCRATCH: Final[frozenset[str]] = frozenset({'.cadrumo', '.cadrumo-mcp-profile-secret.json', '.git', '.github', '.gitignore', '.python-version', '.venv'})
...
def _production_modules() -> list[Path]:
    ...
    """
    return sorted((path for path in _PACKAGE_ROOT.glob('*.py') if path.name not in _DISCOVERY_EXCLUSIONS and (not path.name.startswith('test_'))))
...
def _reportable(path: Path) -> str:
    ...
    try:
        ...
        _dp_mark('dev/packaging/tests/test_var_scratch_mint_sites_are_registered.py:101:except')
...
def _hand_spelled_mints(paths: list[Path]) -> tuple[list[str], int]:
    ...
    for path in paths:
        try:
            ...
            _dp_mark('dev/packaging/tests/test_var_scratch_mint_sites_are_registered.py:122:except')
            continue
...
def _unminted_families(named: dict[str, ScratchFamily], sources: list[Path]) -> list[str]:
    ...
    text = '\n'.join((path.read_text(encoding=UTF_8, errors='ignore') for path in sources if path.name not in _DISCOVERY_EXCLUSIONS))
...
def test_no_var_scratch_name_is_spelled_at_its_call_site() -> None:
    ...
    assert offenders == [], 'these hidden names are spelled at their call site instead of being built with var_scratch_name from a registered family, so nothing guarantees the sweep looks for what they write:\n  ' + '\n  '.join(offenders)
...
def test_every_registered_family_is_named_by_a_mint_site() -> None:
    ...
    assert unminted == [], 'these registered families are named by no source outside the reclaim module, so the sweep is looking for names nothing writes:\n  ' + '\n  '.join(unminted)
...
def test_an_unminted_registered_family_is_reported(tmp_path: Path) -> None:
    ...
    named = {'RELEASE_STAGING_FAMILY': ScratchFamily(prefix='.', suffix='.staging'), 'ORPHAN_FAMILY': ScratchFamily(prefix='.', suffix='-nothing-writes-this')}
