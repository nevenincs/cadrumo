...
def scoped_repo(tmp_path: Path) -> Path:
    """A real source tree carrying files in the proof scope."""
    repo = tmp_path / "repo"
...
def test_fingerprint_tracks_scope_content(scoped_repo: Path) -> None:
    """The fingerprint is deterministic and changes with any scoped edit."""
...
def test_strengthened_prober_invalidates_carried_proofs(scoped_repo: Path) -> None:
    """An edit under dev/packaging changes the fingerprint.
...
def test_record_then_lookup_roundtrip_with_provenance(scoped_repo: Path, tmp_path: Path) -> None:
    ...
    path = record(cache, "quick-core-install", source_fp, env_fp)
    assert path.is_file()
    ...
    assert carried.origin.source_digest == source_fp
...
def test_store_is_bounded_with_oldest_first_eviction(scoped_repo: Path, tmp_path: Path) -> None:
    ...
    for index in range(5):
        path = record(cache, f"kind-{index}", source_fp, env_fp, max_records=3)
