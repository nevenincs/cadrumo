...
from ..build_scratch_reclaim import COHORT_BUILD_TREE_FAMILY, COHORT_SOURCE_ARCHIVE_FAMILY, COMMAND_SPEC_BYTECODE_FAMILY, RELEASE_COHORT_INTEGRATION_FAMILY, RELEASE_STAGING_FAMILY, VAR_SCRATCH_FAMILIES, ScratchFamily, _owning_pid, matching_family, remove_scratch, remove_tree, sweep_var_scratch, var_scratch_name
pytestmark = [pytest.mark.unit, pytest.mark.hex_entrypoint]
_DAY: Final[float] = 24 * 60 * 60
_VAR_MEMBER_PATTERNS: Final[tuple[re.Pattern[str], ...]] = (re.compile('"var"\\s*/\\s*"([A-Za-z0-9._-]+)"'), re.compile('var/([A-Za-z0-9._-]+)'))
_DISCOVERY_EXCLUSIONS: Final[frozenset[str]] = frozenset({'build_scratch_reclaim.py', 'test_build_scratch_reclaim.py'})
...
def _var_naming_surfaces() -> list[Path]:
    ...
    nothing notices.
    """
    return [*(path for path in REPO_ROOT.joinpath('dev').rglob('*.py') if path.name not in _DISCOVERY_EXCLUSIONS), *sorted((path for path in REPO_ROOT.joinpath('.github', 'workflows').iterdir() if path.suffix in ('.yml', '.yaml'))), REPO_ROOT / 'justfile']
...
def _live_var_members() -> frozenset[str]:
    ...
    for source in _var_naming_surfaces():
        try:
            ...
            _dp_mark('dev/packaging/tests/test_build_scratch_reclaim.py:175:except')
            continue
...
def test_discovered_live_var_members_cover_the_readiness_gate_inputs() -> None:
    ...
    assert {'release-cohort', 'packaging-smoke', 'packaging-smoke-evidence', 'distribution-install-readiness', 'packaging-smoke-cohort'} <= discovered
...
@pytest.mark.parametrize('name', ['release-cohort', 'release-cohort-integration', 'release-cohort-integration-abc', 'release-python-9f', '.staging', 'packaging-smoke-cohort', 'binary-closure-real-cp313'])
...
@pytest.mark.parametrize('name', ['release-cohort-integration-0979c0c733fe-source', 'release-cohort-integration-4812-9f2a-source', '.release-cohort-9f2a.staging', '.packaging-smoke-cohort-1.staging', '.4812-release-cohort-9f2a.staging', '.4812-release-python-9f2a-source', '.4812-release-python-9f2a-source.zip', '.4812-probe-command-spec-bytecode'])
...
def test_sweep_does_not_follow_a_link_named_like_scratch(tmp_path: Path) -> None:
    ...
    try:
        if sys.platform == 'win32':
            ...
            subprocess.run([command_shell, '/c', 'mklink', '/J', str(link), str(target)], check=True, capture_output=True)
        else:
...
def test_an_abandoned_cohort_build_tree_and_its_archive_are_reclaimed(tmp_path: Path) -> None:
    ...
    """
    var = _var(tmp_path)
    tree = _aged(_scratch_tree(var, _foreign_owner_name(COHORT_BUILD_TREE_FAMILY, 'release-python-9f2a')), 20 * 60)
    archive = _aged(_scratch_file(var, _foreign_owner_name(COHORT_SOURCE_ARCHIVE_FAMILY, 'release-python-9f2a')), 20 * 60)
    removed, spared = sweep_var_scratch(var, now=time.time())
...
def test_every_minted_name_lands_in_the_family_it_was_minted_from() -> None:
    ...
    assert {family: matching_family(name) for family, name in minted.items()} == {family: family for family in VAR_SCRATCH_FAMILIES}
...
@pytest.mark.parametrize('owner', ['²', '9' * 5000, '0', '-4', str(2 ** 31)])
