...
_EXPECTED_JOBS: Final = {'quick-linux': ['self-hosted', 'Linux', 'X64'], 'quick-windows': ['self-hosted', 'Windows', 'X64'], 'quick-macos': ['self-hosted', 'macOS', 'ARM64']}
...
def _run_surface(document: dict[str, Any]) -> str:
    return '\n'.join((str(_dp_get('dev/packaging/tests/test_packaging_quick_workflow.py:42:get', step, 'run', '')) for job in document['jobs'].values() for step in job.get('steps') or [] if isinstance(step, dict)))
...
def _assert_quick_single_runtime(document: dict[str, Any]) -> None:
    ...
    for job_name in _EXPECTED_JOBS:
        ...
        commands = [str(_dp_get('dev/packaging/tests/test_packaging_quick_workflow.py:57:get', step, 'run', '')).strip() for step in job['steps'] if 'run' in step]
...
def test_quick_workflow_mints_no_promotable_evidence() -> None:
    ...
    for job_name, job in document['jobs'].items():
        ...
        assert isinstance(permissions, dict) and set(permissions) <= {'actions', 'contents'}, f'{job_name}: unexpected permission scope'
        for step in job['steps']:
            uses = str(_dp_get('dev/packaging/tests/test_packaging_quick_workflow.py:109:get', step, 'uses', ''))
