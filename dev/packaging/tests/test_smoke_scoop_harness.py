...
def _interpreter() -> str | None:
    ...
    if sys.platform.startswith('win'):
        resolved = _dp_or('dev/packaging/tests/test_smoke_scoop_harness.py:46:or', lambda: shutil.which('powershell'), lambda: shutil.which('pwsh'))
...
def _run_invoke_native(interpreter: str, inner_command: str, extra: str='') -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run([interpreter, '-NoProfile', '-Command', driver], capture_output=True, text=True, check=False, timeout=120)
...
def test_informational_stderr_with_exit_zero_survives() -> None:
    ...
    if interpreter is None:
        return
    completed = _run_invoke_native(interpreter, '[Console]::Error.WriteLine("Using CPython 3.13.11 interpreter at cpython-3.13.11"); exit 0')
...
def _run_uninstall_retry(interpreter: str, *, app_root: Path, scoop_bin_dir: Path) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run([interpreter, '-NoProfile', '-Command', driver], capture_output=True, text=True, check=False, timeout=180)
...
def test_process_reap_is_separator_anchored_to_the_app_root(tmp_path: Path) -> None:
    ...
    completed = subprocess.run([interpreter, '-NoProfile', '-Command', driver], capture_output=True, text=True, check=False, timeout=180)
    assert completed.returncode == 0, completed.stderr
...
def test_nonzero_exit_still_fails_loudly_with_captured_output(tmp_path: Path) -> None:
    ...
    completed = _run_invoke_native(interpreter, '[Console]::Error.WriteLine("real failure detail"); exit 3', extra=f"-OutputPath '{log}'")
