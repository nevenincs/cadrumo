...
if not __package__:
    _dp_mark('dev/packaging/release_cohort.py:28:ifnot')
    __package__ = 'dev.packaging'
from cadrumo.core.directory_scan import scan_directory
from ._hashing import sha256_path
from .build_scratch_reclaim import RELEASE_STAGING_FAMILY, matching_family, sweep_var_scratch, var_scratch_name
from .cohort_manifest import ArtifactKind, BuildIdentity, LoadedReleaseCohort, SourceIdentity, create_manifest, load_release_cohort, write_manifest
from .python_cohort import PythonCohort, build_python_cohort, source_snapshot_drift
...
def _run(argv: list[str], *, cwd: Path, env: dict[str, str] | None=None) -> subprocess.CompletedProcess[str]:
    completed = subprocess.run(argv, cwd=cwd, env=env, check=False, capture_output=True, text=True, encoding=_UTF_8, errors='strict')
...
def _generate_channel_artifacts(*, clean_root: Path, cohort: PythonCohort, output: Path, env: dict[str, str]) -> tuple[Path, Path]:
    scoop = output / 'scoop' / 'cadrumo.json'
    _run([sys.executable, str(clean_root / 'packaging' / 'scoop' / 'generate.py'), '--cohort-dir', str(cohort.directory), '--version', cohort.version, '--output', str(scoop)], cwd=clean_root, env=env)
    homebrew_dir = output / 'homebrew'
    _run([sys.executable, str(clean_root / 'packaging' / 'homebrew' / 'generate.py'), '--cohort-dir', str(cohort.directory), '--lock', str(clean_root / 'uv.lock'), '--version', cohort.version, '--output-dir', str(homebrew_dir)], cwd=clean_root, env=env)
    return (scoop.resolve(strict=True), (homebrew_dir / 'Formula' / 'cadrumo.rb').resolve(strict=True))
...
def _source_tag(clean_root: Path, *, commit: str, version: str, requested: str | None) -> str | None:
    ...
    if requested is not None:
        ...
        if requested != f'v{version}':
            raise SystemExit(f'source tag {requested!r} does not match cohort version {version!r}')
        return requested
...
def _build_identity(clean_root: Path) -> BuildIdentity:
    ...
    if len(observed_uv) < 2 or observed_uv[0] != 'uv':
        raise SystemExit(f"could not read a uv version from {uv_version!r}; the cohort records the build's uv, so an unreadable one cannot be stamped")
    python_version = platform.python_version()
    if platform.python_implementation() != 'CPython' or python_version != _REQUIRED_PYTHON_VERSION:
        raise SystemExit(f'release cohort requires CPython {_REQUIRED_PYTHON_VERSION}, got {platform.python_implementation()} {python_version}')
    constraints = (clean_root / _BUILD_CONSTRAINTS).resolve(strict=True)
    return BuildIdentity(implementation='dev.packaging.release_cohort', format_version=1, python=python_version, uv=uv_version, platform=platform.system(), architecture=platform.machine(), build_constraints_sha256=sha256_path(constraints))

def build_from_clean_source(*, clean_root: Path, output_dir: Path, expected_commit: str, requested_tag: str | None) -> LoadedReleaseCohort:
    ...
    scoop, homebrew = _generate_channel_artifacts(clean_root=root, cohort=cohort, output=output, env=env)
    tag = _source_tag(root, commit=commit, version=cohort.version, requested=requested_tag)
    manifest = create_manifest(root=output, version=cohort.version, source=SourceIdentity(commit=commit, tag=tag), created_at=datetime.now(UTC), builder=builder, artifacts=(('cadrumo-wheel', ArtifactKind.PYTHON_WHEEL, cohort.root_wheel), ('cadrumo-sdist', ArtifactKind.PYTHON_SDIST, cohort.root_sdist), ('cadrumo-source-archive', ArtifactKind.PYTHON_SOURCE_ARCHIVE, cohort.source_archive), ('cadrumo-runtime-wheelhouse', ArtifactKind.PYTHON_WHEELHOUSE, cohort.runtime_wheelhouse), ('cadrumo-data-manuals-wheel', ArtifactKind.PYTHON_WHEEL, cohort.manuals_wheel), ('cadrumo-data-manuals-sdist', ArtifactKind.PYTHON_SDIST, cohort.manuals_sdist), ('cadrumo-data-official-wheel', ArtifactKind.PYTHON_WHEEL, cohort.official_wheel), ('cadrumo-data-official-sdist', ArtifactKind.PYTHON_SDIST, cohort.official_sdist), ('python-cohort-manifest', ArtifactKind.PYTHON_MANIFEST, cohort.manifest), ('scoop-manifest', ArtifactKind.SCOOP_MANIFEST, scoop), ('homebrew-formula', ArtifactKind.HOMEBREW_FORMULA, homebrew)))
    ...
    if observed != declared:
        raise SystemExit(f'release cohort contains undeclared or missing files: declared={sorted(declared)!r}, observed={sorted(observed)!r}')
    return load_release_cohort(output)

def build_release_cohort(*, repo_root: Path, output_dir: Path, expected_commit: str | None=None, source_tag: str | None=None) -> LoadedReleaseCohort:
    ...
    commit = _dp_or('dev/packaging/release_cohort.py:355:or', lambda: expected_commit, lambda: head)
    if len(commit) != 40 or any((character not in '0123456789abcdef' for character in commit)):
        raise SystemExit(f'expected commit must be one lowercase full SHA: {commit!r}')
    if commit != head:
        raise SystemExit(f'expected commit does not equal the currently checked-out HEAD: expected {commit}, got {head}')
    var = (root / 'var').resolve()
    ...
    with tempfile.TemporaryDirectory(prefix='cadrumo-release-') as temporary:
        ...
        if git is None:
            raise SystemExit('git is required to construct the release cohort')
        _run([git, '-c', 'core.longpaths=true', 'clone', '--no-hardlinks', '--quiet', str(root), str(clean_root)], cwd=root)
        cloned_commit = _git(clean_root, 'rev-parse', 'HEAD')
        if cloned_commit != commit:
            raise SystemExit(f'local clean clone resolved {cloned_commit}, expected source commit {commit}')
        ...
        argv = [sys.executable, '-m', 'dev.packaging.release_cohort', 'build-clean', '--output', str(staging), '--expected-commit', commit]
...
def main() -> int:
    ...
    if args.command == 'build':
        cohort = build_release_cohort(repo_root=_REPO_ROOT, output_dir=args.output, expected_commit=args.expected_commit, source_tag=args.source_tag)
    elif args.command == 'build-clean':
        cohort = build_from_clean_source(clean_root=_REPO_ROOT, output_dir=args.output, expected_commit=args.expected_commit, requested_tag=args.source_tag)
    else:
        cohort = load_release_cohort(args.cohort_dir)
    print(json.dumps({'cohort_id': cohort.manifest.cohort_id, 'directory': str(cohort.directory), 'source_commit': cohort.manifest.source.commit, 'source_tag': cohort.manifest.source.tag, 'version': cohort.manifest.version}, sort_keys=True))
    return 0
