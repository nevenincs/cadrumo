"""Scrub runner metadata out of distribution-evidence rows at mint time.
...
from .evidence import DistributionEvidence, EvidenceIdentityPayload, evidence_identifier
if TYPE_CHECKING:
...
_HOME_USER_RE: Final[re.Pattern[str]] = re.compile('(?P<prefix>(?:[A-Za-z]:)?[\\\\/]{1,2}(?:Users|home)[\\\\/]{1,2})(?P<user>[^\\\\/\\"\'\\s]+)', re.IGNORECASE)
_UNC_HOST_RE: Final[re.Pattern[str]] = re.compile('(?<![\\w:\\\\])[\\\\]{2,4}[A-Za-z0-9][A-Za-z0-9._-]*[\\\\/]')
...
def default_workspace_roots(env: Mapping[str, str] | None=None) -> tuple[str, ...]:
    ...
    roots = {value for var in _WORKSPACE_ROOT_ENV_VARS if (value := _dp_get('dev/packaging/evidence_scrub.py:107:get', source, var, '').strip()) and len(value) >= 3}
...
def _token_pattern(token: str) -> re.Pattern[str]:
    ...
    return re.compile(f'(?<![A-Za-z0-9]){re.escape(token)}(?![A-Za-z0-9])', re.IGNORECASE)

def _workspace_pattern(root: str) -> re.Pattern[str]:
    """Match an absolute workspace root, tolerant of separator style and doubling.
    ...
    """
    segments = [segment for segment in re.split('[\\\\/]+', root) if segment]
    if not segments:
        return re.compile('(?!)')
    body = '[\\\\/]{1,2}'.join((re.escape(segment) for segment in segments))
    return re.compile(f'[\\\\/]{{0,2}}{body}', re.IGNORECASE)

def _scrub_text(value: str, token_patterns: Iterable[re.Pattern[str]], workspace_patterns: Iterable[re.Pattern[str]]=()) -> str:
    scrubbed = value
    for pattern in workspace_patterns:
        scrubbed = pattern.sub(SCRUBBED_WORKSPACE, scrubbed)
    scrubbed = _HOME_USER_RE.sub(f'\\g<prefix>{SCRUBBED_USER}', scrubbed)
...
def _rewrite_strings(node: object, token_patterns: tuple[re.Pattern[str], ...], workspace_patterns: tuple[re.Pattern[str], ...]=()) -> object:
    if isinstance(node, str):
...
def find_residual_leaks(document: dict[str, object], tokens: Iterable[str], workspace_roots: Iterable[str]=()) -> list[str]:
...
def scrub_distribution_evidence(evidence: DistributionEvidence, *, tokens: tuple[str, ...] | None=None, workspace_roots: tuple[str, ...] | None=None) -> DistributionEvidence:
    ...
    scrubbed: dict[str, object] = {key: value if key in _UNSCRUBBABLE_TOP_LEVEL else _rewrite_strings(value, token_patterns, workspace_patterns) for key, value in document.items()}
    residual = find_residual_leaks(scrubbed, machine_tokens, roots)
    if residual:
        raise EvidenceScrubError('runner metadata survived scrubbing; refusing to mint the row:\n' + '\n'.join((f'  - {leak}' for leak in residual)))
...
__all__ = ['SCRUBBED_MACHINE_ID', 'SCRUBBED_USER', 'SCRUBBED_WORKSPACE', 'EvidenceScrubError', 'default_runner_tokens', 'default_workspace_roots', 'find_residual_leaks', 'scrub_distribution_evidence']