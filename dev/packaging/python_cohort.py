...
from .build_scratch_reclaim import COHORT_BUILD_TREE_FAMILY, COHORT_SOURCE_ARCHIVE_FAMILY, COMMAND_SPEC_BYTECODE_FAMILY, var_scratch_name
...
def cohort_stamped_wheel_data_paths() -> frozenset[str]:
    ...
    return frozenset((locate(Path(root.as_posix())).as_posix() for locate in (registry_identity_stamp_location, shipped_verdict_location)))
...
def _run(argv: list[str], *, cwd: Path) -> subprocess.CompletedProcess[str]:
    completed = subprocess.run(argv, cwd=cwd, check=False, capture_output=True, text=True, encoding=_UTF_8, errors='strict')
...
def _artifact_command_projection(root_wheel: Path, root_sdist: Path, source_archive: Path) -> tuple[tuple[str, str], ...]:
    ...
    sdist_members = tuple((('sdist', PurePosixPath(*PurePosixPath(name).parts[1:]).as_posix()) for name in raw_sdist_members if PurePosixPath(name).parts[0] == archive_root))
...
def _cached_artifact_command_projection(root_wheel: Path, root_sdist: Path, source_archive: Path, *, digests: tuple[str, str, str]) -> tuple[tuple[str, str], ...]:
...
def _forbidden_command_artifacts(projection: tuple[tuple[str, str], ...]) -> tuple[tuple[str, str], ...]:
    """Return every projected member whose filename names a command authority artifact."""
    return tuple(((kind, member) for kind, member in projection if PurePosixPath(member).name in _FORBIDDEN_COMMAND_ARTIFACT_NAMES))

def _validate_command_spec_attestation(value: object, *, expected_source_commit: str | None=None, expected_root_wheel_sha256: str | None=None, expected_root_sdist_sha256: str | None=None, expected_source_archive_sha256: str | None=None) -> dict[str, object]:
    if not isinstance(value, dict):
        raise SystemExit('Python cohort CommandSpec attestation must be a JSON object')
    expected = {'schema', 'node_count', 'source_commit', 'forbidden_artifacts_absent', *_ATTESTATION_DIGEST_FIELDS}
    ...
    comparisons = {'source_commit': expected_source_commit, 'root_wheel_sha256': expected_root_wheel_sha256, 'root_sdist_sha256': expected_root_sdist_sha256, 'source_archive_sha256': expected_source_archive_sha256}
...
def _probe_installed_command_specs(*, site_root: Path, work_root: Path) -> dict[str, Any]:
    ...
    try:
        for mode in ('projection', 'aeat config profile list', 'aeat app registry inspect', 'aeat app modelo work calculate'):
            environment['AEAT_COMMAND_SPEC_PROBE_MODE'] = mode
            completed = subprocess.run([sys.executable, '-S', '-c', _COMMAND_SPEC_PROBE], cwd=work_root, env=environment, check=False, capture_output=True, text=True, encoding=_UTF_8, errors='strict')
            if completed.returncode != 0:
    ...
    projection['import_budgets'] = {'graph_projection_first_party_modules': projection['import_budgets']['graph_projection_first_party_modules'], 'handler_modules_loaded': projection['import_budgets']['handler_modules_loaded'], 'selected_path_deltas': [item for selected_projection in projections[1:] for item in selected_projection['import_budgets']['selected_path_deltas']]}
    return projection

def _install_relative_probe_reads(projection: dict[str, Any], *, site_root: Path) -> dict[str, Any]:
    ...
    return {**projection, 'origins': [[name, member_of(f'origin {name}', origin)] for name, origin in projection['origins']], 'packaged_resources': [member_of('packaged resource', resource) for resource in projection['packaged_resources']]}

def _assert_probe_reads_are_wheel_members(projection: dict[str, Any], artifact_projection: tuple[tuple[str, str], ...]) -> None:
    ...
    if unshipped:
        raise SystemExit(f'CommandSpec attestation names files the root wheel does not ship: {unshipped[:20]!r}')

def _command_spec_attestation(projection: dict[str, Any], artifact_projection: tuple[tuple[str, str], ...], *, source_commit: str, root_wheel_sha256: str, root_sdist_sha256: str, source_archive_sha256: str) -> dict[str, object]:
    ...
    to remove.
    """
    attestation: dict[str, object] = {'schema': _COMMAND_SPEC_ATTESTATION_SCHEMA, 'node_count': len(projection['identities']), 'source_commit': source_commit, 'root_wheel_sha256': root_wheel_sha256, 'root_sdist_sha256': root_sdist_sha256, 'source_archive_sha256': source_archive_sha256, 'artifact_members_sha256': _projection_digest(artifact_projection), 'forbidden_artifacts_absent': not _forbidden_command_artifacts(artifact_projection), **{f'{field}_sha256': _projection_digest(projection[field]) for field in ('identities', 'locales', 'policies', 'schemas', 'import_budgets', 'origins')}}
    attestation['envelope_sha256'] = _projection_digest(attestation)
    return _validate_command_spec_attestation(attestation, expected_source_commit=source_commit, expected_root_wheel_sha256=root_wheel_sha256, expected_root_sdist_sha256=root_sdist_sha256, expected_source_archive_sha256=source_archive_sha256)

def _single(directory: Path, pattern: str, *, label: str) -> Path:
    matches = scan_directory(directory, pattern=pattern)
    if len(matches) != 1:
        raise SystemExit(f'expected exactly one {label} matching {pattern!r} in {directory}; got {[path.name for path in matches]!r}')
...
def _wheel_identity(wheel: Path) -> tuple[str, str, tuple[str, ...]]:
    with zipfile.ZipFile(wheel) as archive:
        ...
        if len(metadata_names) != 1:
            raise SystemExit(f'expected one METADATA member in {wheel}; got {metadata_names!r}')
    ...
    return (normalise_distribution_name(name), version, tuple(_dp_or('dev/packaging/python_cohort.py:782:or', lambda: metadata.get_all('Requires-Dist'), lambda: ())))
...
def _sdist_identity(sdist: Path) -> tuple[str, str, tuple[str, ...]]:
    try:
        with tarfile.open(sdist, mode='r:gz') as archive:
            ...
            if len(metadata_names) != 1:
                raise SystemExit(f'expected one PKG-INFO member in {sdist}; got {[member.name for member in metadata_names]!r}')
    ...
    return (normalise_distribution_name(name), version, tuple(_dp_or('dev/packaging/python_cohort.py:807:or', lambda: metadata.get_all('Requires-Dist'), lambda: ())))

def _validate_companion_pins(requirements: tuple[str, ...], *, version: str, artifact_kind: str) -> None:
    parsed = tuple((Requirement(row) for row in requirements))
    for companion in _DISTRIBUTIONS[1:]:
        matches = tuple((requirement for requirement in parsed if normalise_distribution_name(requirement.name) == companion))
        if len(matches) != 1:
            raise SystemExit(f'root {artifact_kind} must declare exactly one dependency on {companion}')
        requirement = matches[0]
        if requirement.extras or requirement.marker is not None or str(requirement.specifier) != f'=={version}':
            raise SystemExit(f'root {artifact_kind} must require {companion}=={version} unconditionally and without extras; found {requirement}')

def _validate_wheel_contract(root_wheel: Path, manuals_wheel: Path, official_wheel: Path) -> str:
    ...
    observed = {root_name: version, manuals_name: manuals_version, official_name: official_version}
    if observed != {name: version for name in _DISTRIBUTIONS}:
        raise SystemExit(f'Python cohort distribution identities or versions drifted: expected {{name: {version!r} for name in {_DISTRIBUTIONS!r}}}, got {observed!r}')
    _validate_companion_pins(requirements, version=version, artifact_kind='wheel')
...
def _validate_sdist_contract(root_sdist: Path, manuals_sdist: Path, official_sdist: Path, *, expected_version: str) -> None:
    ...
    observed = {root_name: root_version, manuals_name: manuals_version, official_name: official_version}
    expected = {name: expected_version for name in _DISTRIBUTIONS}
    if observed != expected:
        raise SystemExit(f'Python cohort sdist identities or versions drifted: expected {expected!r}, got {observed!r}')
    _validate_companion_pins(requirements, version=expected_version, artifact_kind='sdist')
    for sdist in (root_sdist, manuals_sdist, official_sdist):
        if sdist.stat().st_size >= PYPI_FILE_CAP_BYTES:
            raise SystemExit(f"{sdist.name} exceeds PyPI's 100 MB per-file cap: {sdist.stat().st_size} bytes")
...
def source_snapshot_drift(repo_root: Path) -> tuple[str, ...]:
    ...
    completed = _run(['git', 'status', '--porcelain=v1', '--untracked-files=all'], cwd=repo_root)
...
def _stamp_bundled_registry_records_into_build_tree(build_root: Path) -> frozenset[str]:
    ...
    return frozenset((path.relative_to(resolved_source_root).as_posix() for path in (stamped.identity_path, stamped.verdict_path)))
...
def _assert_closed_cohort_inventory(cohort_dir: Path, declared_filenames: Collection[str]) -> None:
    ...
    observed = {path.relative_to(cohort_dir).as_posix() for path in scan_directory(cohort_dir, recursive=True) if path.is_file() and path.name not in _BUILD_TOOL_EMITTED_FILES}
    if observed != declared:
        raise SystemExit(f'Python cohort file inventory drifted: declared={sorted(declared)!r}, observed={sorted(observed)!r}')
...
def _assert_source_archive_binds_wheelhouse(source_archive: Path, wheelhouse_manifest: dict[str, Any]) -> None:
...
def attest_command_specs(*, site_root: Path, root_wheel: Path, root_sdist: Path, source_archive: Path, source_commit: str, work_root: Path, digests: tuple[str, str, str] | None=None) -> dict[str, object]:
    ...
    """
    projection = _install_relative_probe_reads(_probe_installed_command_specs(site_root=site_root, work_root=work_root), site_root=site_root)
    resolved = _dp_or('dev/packaging/python_cohort.py:1030:or', lambda: digests, lambda: (sha256_path(root_wheel), sha256_path(root_sdist), sha256_path(source_archive)))
    artifact_projection = _cached_artifact_command_projection(root_wheel, root_sdist, source_archive, digests=resolved)
    _assert_probe_reads_are_wheel_members(projection, artifact_projection)
    return _command_spec_attestation(projection, artifact_projection, source_commit=source_commit, root_wheel_sha256=resolved[0], root_sdist_sha256=resolved[1], source_archive_sha256=resolved[2])
...
def build_python_cohort(repo_root: Path, output_dir: Path) -> PythonCohort:
    ...
    if drift:
        raise SystemExit(f'immutable cohort construction requires a clean source snapshot; commit or remove drift before building: {drift[:20]!r}')
    ...
    try:
        _run(['git', 'archive', '--format=zip', '-o', str(archive), source_commit], cwd=root)
        ...
        _run([uv, 'build', '--wheel', '--sdist', '--project', str(build_root / 'packaging' / 'cadrumo_data_manuals'), '--out-dir', str(output)], cwd=build_root)
        wheelhouse = build_runtime_wheelhouse(build_root, output / 'cadrumo-runtime-wheelhouse.zip')
        shutil.move(archive, retained_source_archive)
        _run([uv, 'build', '--wheel', '--sdist', '--project', str(build_root / 'packaging' / 'cadrumo_data_official'), '--out-dir', str(output)], cwd=build_root)
        ...
        runtime_wheelhouse = _single(output, 'cadrumo-runtime-wheelhouse*.zip', label='runtime dependency wheelhouse')
        manuals_wheel = _single(output, 'cadrumo_data_manuals-*.whl', label='manuals wheel')
        official_wheel = _single(output, 'cadrumo_data_official-*.whl', label='official wheel')
        manuals_sdist = _single(output, 'cadrumo_data_manuals-*.tar.gz', label='manuals sdist')
        official_sdist = _single(output, 'cadrumo_data_official-*.tar.gz', label='official sdist')
        version = _validate_wheel_contract(root_wheel, manuals_wheel, official_wheel)
        _validate_sdist_contract(root_sdist, manuals_sdist, official_sdist, expected_version=version)
        artifacts = {'cadrumo': root_wheel.name, 'cadrumo-sdist': root_sdist.name, 'source-archive': retained_source_archive.name, 'runtime-wheelhouse': runtime_wheelhouse.name, 'cadrumo-data-manuals': manuals_wheel.name, 'cadrumo-data-manuals-sdist': manuals_sdist.name, 'cadrumo-data-official': official_wheel.name, 'cadrumo-data-official-sdist': official_sdist.name}
        sha256 = {name: sha256_path(output / filename) for name, filename in artifacts.items()}
        command_spec_attestation = attest_command_specs(site_root=(build_root / _BUILD_TREE_SOURCE_DIR).resolve(strict=True), root_wheel=root_wheel, root_sdist=root_sdist, source_archive=retained_source_archive, source_commit=source_commit, work_root=output.parent, digests=(sha256['cadrumo'], sha256['cadrumo-sdist'], sha256['source-archive']))
    finally:
        if archive.exists():
            archive.unlink()
        if build_root.exists():
    ...
    manifest.write_text(json.dumps({'artifacts': artifacts, 'sha256': sha256, 'source_commit': source_commit, 'version': version, 'command_spec_attestation': command_spec_attestation}, indent=2, sort_keys=True) + '\n', encoding=_UTF_8, newline='\n')
    ...
    return PythonCohort(directory=output, manifest=manifest, source_commit=source_commit, version=version, root_wheel=root_wheel, root_sdist=root_sdist, source_archive=retained_source_archive, runtime_wheelhouse=runtime_wheelhouse, runtime_wheelhouse_manifest=wheelhouse.manifest, manuals_wheel=manuals_wheel, manuals_sdist=manuals_sdist, official_wheel=official_wheel, official_sdist=official_sdist, sha256=sha256, command_spec_attestation=command_spec_attestation)
...
def load_python_cohort(directory: Path) -> PythonCohort:
    ...
    if not isinstance(artifacts, dict) or not isinstance(sha256, dict) or (not isinstance(source_commit, str)) or (re.fullmatch('[0-9a-f]{40}', source_commit) is None) or (not isinstance(version, str)) or (not version):
        raise SystemExit(f'Python cohort manifest has an invalid schema: {document!r}')
    expected_keys = {'cadrumo', 'cadrumo-sdist', 'source-archive', 'runtime-wheelhouse', 'cadrumo-data-manuals', 'cadrumo-data-manuals-sdist', 'cadrumo-data-official', 'cadrumo-data-official-sdist'}
    if set(artifacts) != expected_keys or set(sha256) != expected_keys:
        raise SystemExit(f'Python cohort manifest keys drifted: artifacts={set(artifacts)!r}, sha256={set(sha256)!r}')
    ...
    for name in sorted(expected_keys):
        ...
        if actual != digest:
            raise SystemExit(f'cohort artifact digest mismatch for {name!r}: expected {digest}, got {actual}')
    ...
    command_spec_attestation = _validate_command_spec_attestation(command_spec_attestation_value, expected_source_commit=source_commit, expected_root_wheel_sha256=root_wheel_digest, expected_root_sdist_sha256=root_sdist_digest, expected_source_archive_sha256=source_archive_digest)
    projection = _cached_artifact_command_projection(resolved['cadrumo'], resolved['cadrumo-sdist'], resolved['source-archive'], digests=(root_wheel_digest, root_sdist_digest, source_archive_digest))
    ...
    runtime_wheelhouse = load_runtime_wheelhouse(resolved['runtime-wheelhouse'], expected_lock_sha256=_sealed_lock_digest(resolved['source-archive']))
    observed_version = _validate_wheel_contract(resolved['cadrumo'], resolved['cadrumo-data-manuals'], resolved['cadrumo-data-official'])
    if observed_version != version:
        raise SystemExit(f'cohort manifest version {version!r} != wheel version {observed_version!r}')
    _validate_sdist_contract(resolved['cadrumo-sdist'], resolved['cadrumo-data-manuals-sdist'], resolved['cadrumo-data-official-sdist'], expected_version=version)
    return PythonCohort(directory=cohort_dir, manifest=manifest, source_commit=source_commit, version=version, root_wheel=resolved['cadrumo'], root_sdist=resolved['cadrumo-sdist'], source_archive=resolved['source-archive'], runtime_wheelhouse=resolved['runtime-wheelhouse'], runtime_wheelhouse_manifest=runtime_wheelhouse.manifest, manuals_wheel=resolved['cadrumo-data-manuals'], manuals_sdist=resolved['cadrumo-data-manuals-sdist'], official_wheel=resolved['cadrumo-data-official'], official_sdist=resolved['cadrumo-data-official-sdist'], sha256={str(name): str(digest) for name, digest in sha256.items()}, command_spec_attestation=command_spec_attestation)

def digest_install_target(name: str, artifact: Path, *, extras: tuple[str, ...]=(), digest: str | None=None) -> str:
...
def root_install_target(root_artifact: Path, *, extras: tuple[str, ...]=(), digest: str | None=None) -> str:
...
def install_targets(cohort: PythonCohort, *, root_artifact: Path, extras: tuple[str, ...]=()) -> tuple[str, ...]:
    """Return explicit local targets that prevent companion index resolution."""
    return (root_install_target(root_artifact, extras=extras, digest=_cohort_digest_for(cohort, root_artifact)), digest_install_target('cadrumo-data-manuals', cohort.manuals_wheel, digest=cohort.sha256.get('cadrumo-data-manuals')), digest_install_target('cadrumo-data-official', cohort.official_wheel, digest=cohort.sha256.get('cadrumo-data-official')))

def _verify_direct_urls(direct_urls: object, cohort: PythonCohort, root_artifact: Path) -> None:
    ...
    expected_artifacts = {'cadrumo': root_artifact.resolve(), 'cadrumo-data-manuals': cohort.manuals_wheel, 'cadrumo-data-official': cohort.official_wheel}
    for name, artifact in expected_artifacts.items():
        ...
        base_url, _, fragment = str(_dp_or('dev/packaging/python_cohort.py:1460:or', lambda: recorded_url, lambda: '')).partition('#')
        if not isinstance(direct_url, dict) or base_url != artifact.as_uri():
            raise SystemExit(f'{name} installed from an unrelated origin: {direct_url!r}')
        expected_sha = cohort.sha256['cadrumo-sdist'] if name == 'cadrumo' and artifact == cohort.root_sdist else cohort.sha256[name]
        ...
        recorded_sha = (_sha_candidate if isinstance(_sha_candidate, str) else None) or fragment.removeprefix('sha256=') or None
        if recorded_sha != expected_sha:
            raise SystemExit(f'{name} installed digest drifted: expected {expected_sha}, recorded {recorded_sha!r}')
        origin_sha = sha256_path(artifact.resolve(strict=True))
        if origin_sha != expected_sha:
            raise SystemExit(f'{name} origin bytes drifted after install: expected {expected_sha}, hashed {origin_sha}')

def assert_installed_cohort(python: Path, cohort: PythonCohort, *, root_artifact: Path, cwd: Path) -> dict[str, Any]:
    ...
    requirements = set(_dp_or('dev/packaging/python_cohort.py:1508:or', lambda: document.get('root_requirements'), lambda: ()))
    expected_requirements = {f'cadrumo-data-manuals=={cohort.version}', f'cadrumo-data-official=={cohort.version}'}
    if not expected_requirements <= requirements:
        raise SystemExit(f'installed root metadata lost companion pins: {requirements!r}')
...
def main() -> int:
    ...
    print(json.dumps({'directory': str(cohort.directory), 'sha256': cohort.sha256, 'source_commit': cohort.source_commit, 'version': cohort.version}, sort_keys=True))
    return 0
