...
__all__ = ['TRACKED_DATA_ROOTS', 'DependencySurfaces', 'ProofContractError', 'assert_attachment_and_llm_surfaces', 'assert_cadrumo_version_output', 'assert_cli_smoke', 'assert_installed_data', 'assert_optional_extra_registry_matches_pyproject', 'assert_wheel_contains_tracked_data', 'assert_wheel_metadata_matches_pyproject', 'build_companion_wheels', 'build_sdist', 'build_source_data_paths', 'build_wheel', 'clean_product_env', 'commit_defined_build_root', 'create_pip_venv', 'expected_wheel_data_paths', 'extract_source_commit', 'find_repo_root', 'head_extract', 'install_targets_with_pip', 'install_wheel', 'installed_product_env', 'isolated_product_env', 'optional_extra_registry', 'pyproject_surfaces', 'record_proof', 'recorded_proofs', 'relative_manifest_path', 'require_executable', 'requirement_name', 'reset_proof_ledger', 'resolve_work_dir', 'run_checked', 'tracked_source_data_paths', 'validate_frozen_exports', 'venv_bin_dir', 'venv_cadrumo_path', 'venv_python_path', 'wheel_metadata', 'write_smoke_manifest']
_UTF_8: Final[str] = UTF_8
_REPRESENTATIVE_DATA_LEAVES = ('registry/aeat/modelos/036/manifest.toml', 'registry/cadrumo/user_profile/schema.toml', 'corpus/aeat_official/disenos_registro/modelo_100/manifest.json')
...
_COMPANION_HOOKS = ('packaging/cadrumo_data_manuals/hatch_build.py', 'packaging/cadrumo_data_official/hatch_build.py')
_DATA_COMPANION_PROJECTS = (('cadrumo-data-manuals', 'packaging/cadrumo_data_manuals', 'cadrumo_data_manuals-*.whl'), ('cadrumo-data-official', 'packaging/cadrumo_data_official', 'cadrumo_data_official-*.whl'))
_RENTA_PDF_ALLOW_LIST = {f'src/cadrumo/_data/corpus/manuals/renta/{year}/part1/source.pdf' for year in ('2020', '2021', '2022', '2023', '2024', '2025')} | {'src/cadrumo/_data/corpus/manuals/renta/2025/part2-deducciones-autonomicas/source.pdf'}
_CORE_ABSENT_NAMES = {'anthropic', 'google-api-python-client', 'playwright', 'pytest', 'ruff', 'semgrep', 'sphinx', 'torch'}
_CORE_PRESENT_TRANSITIVE_NAMES = {'numpy', 'pywin32', 'pillow', 'lxml'}
_EXTRAS_PRESENT_NAMES = {'anthropic', 'google-api-python-client', 'playwright'}
_DEV_PRESENT_NAMES = {'deptry', 'pytest', 'ruff', 'sphinx', 'torch'}
...
def head_extract(repo_root: Path, work_dir: Path) -> Path:
    ...
    run_checked(['git', 'archive', '--format=zip', '-o', str(archive), 'HEAD'], cwd=repo_root, env=_git_env(repo_root))
...
def commit_defined_build_root(repo_root: Path, work_dir: Path) -> Path:
    ...
    if not dirty:
        return repo_root
    print(f'working tree carries {len(dirty.splitlines())} uncommitted path(s); building from a pristine HEAD extract so the artifacts correspond to a commit', flush=True)
...
def extract_source_commit(repo_root: Path, work_dir: Path, source_commit: str) -> Path:
    ...
    run_checked(['git', 'archive', '--format=zip', '-o', str(archive), source_commit], cwd=repo_root, env=_git_env(repo_root))
    with zipfile.ZipFile(archive) as bundle:
        bundle.extractall(extract_root)
    archive.unlink()
    return extract_root
...
def _wsl_path_from_windows_gitdir(gitdir: str) -> Path | None:
    ...
    match = re.fullmatch('([A-Za-z]):/(.+)', normalized)
    if match is None:
        return None
...
def run_checked(argv: list[str], *, cwd: Path, env: dict[str, str] | None=None, expected: set[int] | None=None) -> CommandResult:
    ...
    if completed.returncode not in expected_codes:
        ...
        raise SystemExit(_dp_or('dev/packaging/_smoke_common.py:347:or', lambda: completed.returncode, lambda: 1))
    return completed

def run_checked_marker(argv: list[str], *, cwd: Path, marker: str, env: dict[str, str] | None=None) -> CommandResult:
...
def _requirement_applies_to_current_platform(requirement: str) -> bool:
    ...
    if not marker:
        return True
    match = re.fullmatch('sys_platform\\s*==\\s*[\'\\"]([^\'\\"]+)[\'\\"]', marker)
    if match is None:
        return True
...
def _dependency_group_entries(groups: dict[str, Any], group_name: str, *, stack: tuple[str, ...]=()) -> tuple[str | dict[str, Any], ...]:
    ...
    raw_entries = _dp_get('dev/packaging/_smoke_common.py:421:get', groups, group_name, [])
...
def _dependency_group_applies_to_current_platform(entry: str | dict[str, Any]) -> bool:
    ...
    if not isinstance(marker, str):
        return True
    match = re.fullmatch('sys_platform\\s*==\\s*[\'\\"]([^\'\\"]+)[\'\\"]', marker.strip())
    if match is None:
        return True
    return sys.platform == match.group(1)
...
def pyproject_surfaces(repo_root: Path) -> DependencySurfaces:
    ...
    project_requirements = _dp_get('dev/packaging/_smoke_common.py:456:get', project, 'dependencies', [])
    project_names = {requirement_name(req) for req in _dp_get('dev/packaging/_smoke_common.py:457:get', project, 'dependencies', [])}
    project_active_names = {requirement_name(req) for req in project_requirements if _requirement_applies_to_current_platform(req)}
    optional_dependencies = _dp_get('dev/packaging/_smoke_common.py:461:get', project, 'optional-dependencies', {})
    ...
    optional_active_names = {requirement_name(req) for req in optional_requirements if _requirement_applies_to_current_platform(req)}
    dependency_groups = _dp_get('dev/packaging/_smoke_common.py:468:get', pyproject, 'dependency-groups', {})
    ...
    dev_active_names = {_dependency_group_name(entry) for entry in dev_entries if _dependency_group_applies_to_current_platform(entry)}
    return DependencySurfaces(project_name=project_name, project_names=project_names, project_active_names=project_active_names, optional_names=optional_names, optional_active_names=optional_active_names, extras=extras, dev_names=dev_names, dev_active_names=dev_active_names)
...
def optional_extra_registry(repo_root: Path) -> tuple[dict[str, str], set[str]]:
    ...
    for node in module.body:
        ...
        for target, value in assignments:
            ...
            if isinstance(extra, ast.Constant) and isinstance(extra.value, str) and isinstance(import_name, ast.Constant) and isinstance(import_name.value, str):
...
def assert_optional_extra_registry_matches_pyproject(repo_root: Path) -> None:
    ...
    with (repo_root / 'pyproject.toml').open('rb') as handle:
        pyproject = tomllib.load(handle)
    optional_dependencies = _dp_get('dev/packaging/_smoke_common.py:534:get', pyproject['project'], 'optional-dependencies', {})
    ...
    if missing_pyproject:
        raise SystemExit(f'core optional-extra registry names extras missing from pyproject.toml: {missing_pyproject!r}')
    ...
    aggregate = set(_dp_get('dev/packaging/_smoke_common.py:545:get', optional_dependencies, 'all', []))
    missing_aggregate = sorted((f'{project_name}[{extra}]' for extra in registry_extras if f'{project_name}[{extra}]' not in aggregate))
...
def wheel_metadata(wheel: Path) -> tuple[list[str], set[str]]:
    ...
    return (metadata.get_all('Requires-Dist') or [], {normalise_distribution_name(extra) for extra in metadata.get_all('Provides-Extra') or []})
...
def _validated_source_data_inventory(source_root: Path, inventory: set[str], *, origin: str) -> set[str]:
    ...
    if absent:
        raise SystemExit(f'{len(absent)} shipped-data files named by {origin} are absent from {source_root}: {_format_path_sample(absent)}. Reconcile these paths before packaging: restore the tracked files, or remove them from git tracking if they were intentionally retired.')
    return inventory
...
def build_source_data_paths(source_root: Path) -> set[str]:
    ...
    extracted = {path.relative_to(source_root).as_posix() for path in scan_directory(data_root, recursive=True) if path.is_file()}
...
def _companion_corpus_ownership(repo_root: Path) -> dict[str, frozenset[str]]:
    ...
    for relative_hook in _COMPANION_HOOKS:
        ...
        if missing:
            raise SystemExit(f'companion hook {relative_hook} is missing literal ownership declarations: {sorted(missing)!r}')
...
def _assert_split_files_have_companion_owners(repo_root: Path, paths: set[str]) -> None:
    ...
    for path in sorted(paths):
        ...
        if Path(path).suffix.lower() not in _dp_get('dev/packaging/_smoke_common.py:691:get', ownership, subdir, frozenset()):
            unowned.append(path)
    if unowned:
        raise SystemExit(f'{len(unowned)} root-excluded corpus binaries have no companion hook owner: {_format_path_sample(unowned)}')
...
def assert_wheel_contains_tracked_data(repo_root: Path, wheel: Path, expected: set[str] | None=None) -> None:
    ...
    with zipfile.ZipFile(wheel) as archive:
        actual_paths = {info.filename for info in archive.infolist() if not info.is_dir() and info.filename.startswith(f'{_WHEEL_DATA_PREFIX}/')}
    missing = sorted(expected_paths - actual_paths)
    unexpected = sorted(actual_paths - expected_paths)
    if missing or unexpected:
        raise SystemExit(f'wheel data payload differs from the tracked runtime set: missing={_format_path_sample(missing)}, unexpected={_format_path_sample(unexpected)}')
...
def _assert_export_surface(name: str, names: set[str], *, present: AbstractSet[str]=frozenset(), absent: AbstractSet[str]=frozenset()) -> None:
...
def validate_frozen_exports(repo_root: Path, uv: str) -> None:
    ...
    core = run_checked([uv, 'export', '--frozen', '--no-dev', '--no-emit-project', '--no-hashes'], cwd=repo_root)
    extras = run_checked([uv, 'export', '--frozen', '--all-extras', '--no-dev', '--no-emit-project', '--no-hashes'], cwd=repo_root)
    dev = run_checked([uv, 'export', '--frozen', '--all-extras', '--all-groups', '--no-emit-project', '--no-hashes'], cwd=repo_root)
    ...
    _assert_export_surface('core', core_names, present=surfaces.project_active_names, absent=(surfaces.external_optional_active_names | surfaces.dev_only_active_names | _CORE_ABSENT_NAMES) - _CORE_PRESENT_TRANSITIVE_NAMES)
    _assert_export_surface('extras', extras_names, present=surfaces.project_active_names | surfaces.external_optional_active_names | _EXTRAS_PRESENT_NAMES, absent=surfaces.dev_only_active_names - _CORE_PRESENT_TRANSITIVE_NAMES)
    _assert_export_surface('dev', dev_names, present=surfaces.project_active_names | surfaces.external_optional_active_names | surfaces.dev_active_names | _DEV_PRESENT_NAMES)
...
def build_companion_wheels(work_dir: Path, uv: str, *, build_root: Path) -> tuple[Path, Path]:
    ...
    for project_name, project_dir, wheel_glob in _DATA_COMPANION_PROJECTS:
        run_checked([uv, 'build', '--project', str(build_root / project_dir), '--out-dir', str(out_dir)], cwd=build_root)
        ...
        if wheel.stat().st_size >= PYPI_FILE_CAP_BYTES:
            raise SystemExit(f"{wheel.name} exceeds PyPI's 100 MB per-file cap: {wheel.stat().st_size} bytes")
        wheels.append(wheel)
    if len(wheels) != 2:
...
def install_wheel(repo_root: Path, work_dir: Path, wheel: Path, uv: str, python: str, *, extras: tuple[str, ...]=(), companion_wheels: tuple[Path, ...]=()) -> Path:
    ...
    companion_targets = tuple((digest_install_target(normalise_distribution_name(companion.name.split('-')[0]), companion) for companion in companion_wheels))
    run_checked([uv, 'pip', 'install', '--python', str(venv_python_path(venv_path)), target, *companion_targets], cwd=repo_root)
...
def create_pip_venv(work_dir: Path, python_executable: str) -> Path:
    ...
    try:
        ...
        run_checked(['uv', 'pip', 'install', '--python', str(venv_python_path(venv_path)), 'pip'], cwd=work_dir)
    ...
    if python_executable[0].isdigit() and requested_major_minor not in version.stdout:
        raise SystemExit(f'pip venv interpreter {version.stdout.strip()!r} does not match requested {python_executable!r}')
    record_proof('stdlib venv creation')
    return venv_path

def install_targets_with_pip(work_dir: Path, targets: tuple[str, ...], venv_path: Path) -> None:
    """Install explicit local targets in one pip transaction."""
    python = venv_python_path(venv_path)
    run_checked([str(python), '-m', 'pip', 'install', '--disable-pip-version-check', '--no-cache-dir', *targets], cwd=work_dir)
...
def clean_product_env() -> dict[str, str]:
    ...
    for name in ('PYTHONPATH', 'PYTHONHOME', 'PYTHONUSERBASE', 'VIRTUAL_ENV', 'CONDA_PREFIX', 'CONDA_DEFAULT_ENV', 'UV_PROJECT_ENVIRONMENT'):
        _dp_pop('dev/packaging/_smoke_common.py:1099:pop', environment, name, None)
    return environment
...
def isolated_product_env(storage_root: Path) -> dict[str, str]:
    ...
    return {**clean_product_env(), 'CADRUMO_LOCAL_STORAGE_ROOT': str(storage_root), 'CADRUMO_DATABASE_URL': f"sqlite:///{(storage_root / 'cadrumo.db').as_posix()}"}
...
def installed_product_env(storage_root: Path, venv_path: Path) -> dict[str, str]:
    ...
    environment.update({'PATH': str(venv_bin_dir(venv_path.resolve())), 'PYTHONNOUSERSITE': '1', 'PYTHONDONTWRITEBYTECODE': '1'})
    return environment
...
def assert_attachment_and_llm_surfaces(work_dir: Path, venv_path: Path) -> None:
    ...
    run_checked_marker([str(venv_python_path(venv_path)), '-c', code], cwd=work_dir, env=env, marker='attachment-and-llm-surfaces-ok')
...
def assert_cli_smoke(work_dir: Path, venv_path: Path) -> None:
    ...
    version = run_checked([cadrumo, '--version'], cwd=work_dir, env=installed_product_env(work_dir / 'version-state', venv_path))
    ...
    default_check = run_checked([cadrumo, '--format', 'json', 'config', 'check'], cwd=work_dir, env=default_env, expected={1, 2})
    default_payload = _json_payload(default_check.stdout)
    if default_payload.get('status') != 'success' or _dp_get('dev/packaging/_smoke_common.py:1266:get', default_payload, 'result', {}).get('ok') is not False:
        raise SystemExit(f'default config check did not report typed missing-dependency diagnostics: {default_payload!r}')
    storage_root = work_dir / 'profile-root'
    storage_root.mkdir(parents=True, exist_ok=True)
    env = {**installed_product_env(storage_root, venv_path), 'CADRUMO_OUTPUT_LANGUAGE': 'en', 'CADRUMO_SECRET_PASSPHRASE': secrets.token_urlsafe(24), 'CADRUMO_SECRET_STORE_BACKEND': 'unsecured'}
    create = run_checked([cadrumo, '--format', 'json', 'config', 'profile', 'create', 'packaging-smoke', '--entity-type', 'natural_person', '--tax-id', '00000000T', '--name', 'Packaging', '--surnames', 'Smoke', '--irpf-income-categories', 'actividad_economica', '--tax-residence-ccaa', 'madrid', '--quiet', '--accept-defaults', '--no-llm-vision', '--no-google-export'], cwd=work_dir, env=env)
    ...
    result = _dp_get('dev/packaging/_smoke_common.py:1323:get', ready_payload, 'result', {})
...
def relative_manifest_path(work_dir: Path, path: Path) -> str:
    ...
    try:
        ...
        _dp_mark('dev/packaging/_smoke_common.py:1349:except')
        return str(resolved_path)

def write_smoke_manifest(work_dir: Path, *, lane: str, artifacts: dict[str, str], declared: tuple[str, ...], details: dict[str, Any] | None=None) -> Path:
    ...
    if unperformed:
        raise ProofContractError(f'{lane}: declared proofs never executed: {unperformed!r}; recorded this run: {list(recorded)!r}')
    manifest = PackagingSmokeManifest(ok=True, lane=lane, completed_at=datetime.now(UTC), work_dir=str(work_dir.resolve()), artifacts=artifacts, checks=recorded, details=_dp_or('dev/packaging/_smoke_common.py:1389:or', lambda: details, lambda: None))
