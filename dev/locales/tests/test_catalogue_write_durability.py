...
_MANAGER = Path(__file__).resolve().parents[1] / "manager.py"
...
def _truncating_writes(tree: ast.Module) -> list[str]:
    ...
    for node in ast.walk(tree):
        if not isinstance(node, ast.Call):
            continue
        target = node.func
        is_truncating_method = isinstance(target, ast.Attribute) and target.attr in _TRUNCATING_WRITE_ATTRIBUTES
        is_truncating_open = (
...
def test_manager_has_no_truncating_durable_write() -> None:
    """The manager must persist every catalogue edit atomically."""
    tree = ast.parse(_MANAGER.read_text(encoding="utf-8"))

    assert _truncating_writes(tree) == [], (
        "dev/locales/manager.py performs a truncating durable write. A locale catalogue "
