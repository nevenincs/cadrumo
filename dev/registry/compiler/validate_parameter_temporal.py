"""Public temporal validation for parameter values and bracket tables.

These helpers accumulate diagnostics for a revision's parameter temporal
surfaces. The module also exposes the bracket-window query used by compiler
coverage proofs, so callers can use the same calculation as the validator.
"""
