"""Public relation selector helpers for resolving revision and period coverage.
...
def select_relation_source_revisions(
    ...
    accumulator-convention conversion: every caller (`validate_relation_sources`
...
def _coverage_segment_owner(
    ...
    *,
    ...
    if not owners and is_beyond_latest_modelled_source_year(segment_start, candidates):
...
def source_year_upper_bound(candidates: tuple[ModeloRevision, ...]) -> int | None:
    ...
    :func:`is_beyond_latest_modelled_source_year`.
    """
    bound: int | None = None
...
def is_beyond_latest_modelled_source_year(start: int, candidates: tuple[ModeloRevision, ...]) -> bool:
    ...
    """
    bound = source_year_upper_bound(candidates)
