...
__all__ = [
    ...
    "M303_2023_VARIABLE_ENVELOPE_ROLES",
    "M303SemanticCensus2023",
...
M303_2023_REVIEW_HOME_TOTALS: Final[dict[str, int]] = {
    "casilla": 105,
    "computed": 5,
    "draft": 2,
    "filler": 9,
...
M303_2023_VARIABLE_ENVELOPE_ROLES: Final[tuple[str, ...]] = (
    "opening_tag",
    "modelo",
    "discriminant",
    "filing_year",
...
def census_m303_2023_semantic_map(
    ...
    if duplicate_source_anchors:
        raise ValueError(f"M303 2023 parsed source repeats anchors: {duplicate_source_anchors!r}")
    ...
    if duplicate_semantic_anchors:
        raise ValueError(f"M303 2023 semantic map assigns an anchor more than once: {duplicate_semantic_anchors!r}")
    ...
    if len(source_anchor_sequence) != M303_2023_FIXED_ANCHOR_COUNT:
    ...
    if review_home_totals != M303_2023_REVIEW_HOME_TOTALS:
        raise ValueError(f"M303 2023 reviewed semantic-home totals drifted: {review_home_totals!r}")

    simplified_source_anchors = {("DP30302", ordinal) for ordinal in (*range(6, 78), *range(90, 152))}
    ...
    if envelope_roles != M303_2023_VARIABLE_ENVELOPE_ROLES:
        raise ValueError("M303 2023 DP30300 prefix semantic roles drifted")
