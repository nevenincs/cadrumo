...
def test_f0165_historic_exact_export_proof_restores_current_01683(monkeypatch) -> None:
    gap = SimpleNamespace(
        disposition=M200CasillaDisposition.REVISION_MISSING_DECLARATION,
    ...
    assert not refused
    assert accepted[0].id == "01683"
    assert accepted[0].export_refs == (gap.export_field_id,)
