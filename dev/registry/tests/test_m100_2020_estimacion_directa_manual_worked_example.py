"""Oracle test for M100 2020 casilla 0226 grounded against the AEAT manual's own
worked example (estimación directa simplificada, actividad profesional).

Ground truth (bundled AEAT Manual práctico de Renta 2020, Parte 1, Capítulo 7
...
profesional en estimación directa, modalidad simplificada)"):
    ...
    raw_evidence_locator: corpus/manuals/renta/2020/part1/source.pdf.extracted.md#L15992-L16145

This is the SAME médico radiólogo caso práctico the 2024 manual reuses verbatim
(compare `test_m100_2024_estimacion_directa_manual_worked_example.py`); the
raw ingreso/gasto figures are byte-identical across both editions, only the
table's per-row presentation differs slightly (2020 lists "Adquisición libros
y revistas" and "Asistencia VI Congreso" as two separate rows instead of one
merged "Otros conceptos fiscalmente deducibles" row; the totals are identical
either way).

Raw taxpayer facts quoted verbatim from the manual (fiscal column unless
noted; line numbers refer to the extracted markdown):

    Ingresos íntegros (L16004-16005):
      - Honorarios por prestación de servicios: 124.000
      - Conferencias y publicaciones: 10.800
    Nota (2), L16096-16097: "Las cantidades percibidas en concepto de
    ...
    Nota (1), L16094-16095: autoconsumo (10 radiografías a precio de mercado
      de 60 €) = 600.
    Nota (3), L16098-16100: variación de existencias (16.100 finales -
      13.100 iniciales) = +3.000, ingreso íntegro.
    L16070 (table row "Total ingresos"): 137.800 (registrado) / 138.400
      (fiscal).

    Gastos (L16006-16030, fiscal column at L16071-16087):
      - Sueldos y salarios: 18.900 registrado / 17.700 fiscal (nota 4,
        L16101-16105: 1.200 € pagados a la esposa no son deducibles).
      - Seguridad Social: 5.900.
      - RETA del titular de la actividad: 3.300.
      - Compras material radiológico y sanitario: 19.000 (nota 5,
        L16111-16122: la compra íntegra entra como gasto; la variación de
      ...
      - Amortización del local: 2.900; amortización del equipamiento
        radiológico: 5.000 → dotaciones 7.900 (nota 6, L16123-16124).
      - IVA soportado en gastos corrientes: 1.600 (nota 7, L16125-16126:
        deducible por tratarse de actividad exenta de IVA).
      - Tributos no estatales: 1.700.
      - Adquisición libros y revistas médicas: 1.300 (own table row).
      - Asistencia VI Congreso Radiológico: 1.000 ("Otros conceptos
        fiscalmente deducibles" per the table's own parenthetical, L16085).
      - Suministro eléctrico 4.000 + agua 300 + gas 1.000 + telefonía e
        Internet 2.500 = 7.800 (single "Suministro" table row, L16081).
      - Reparación conservación: 3.800 (own table row).
      - Limpieza del local: 4.500 (own table row); recibo de comunidad:
        1.700 (own table row) → both parenthesised "otros servicios
        exteriores" per the table's own parenthetical, L16083-16084.
    L16087 (table row "Total gastos"): 79.500 (registrado) / 78.300 (fiscal).

    Nota (9), L16132-16136: "Conjunto de provisiones deducibles y gastos de
      difícil justificación" — the note's own arithmetic
      (5%×(135.400-72.000)=3.170) diverges from the table's own stated
      Total ingresos/Total gastos fiscal figures (138.400/78.300, which give
      5%×60.100=3.005); both figures exceed the 2.000 € cap, so the deduction
      is 2.000 € either way ("En este caso podrá deducir 2.000 euros al
      superar el 5 por 100 ... dicha cuantía"). This test grounds the
      registry's own 0222 computation from the table's own Total
      ingresos/Total gastos figures (138.400/78.300 → 5%×60.100=3.005,
      capped at 2.000), not from nota (9)'s internally-inconsistent
      135.400/72.000 arithmetic.

    L16090 (table row "Rendimiento neto"): 58.300 (registrado) / 58.100
      (fiscal).
    L16091 (table row "Rendimiento neto reducido"): 58.100.
    L16092 (table row "Rendimiento neto reducido total"): 58.100.
    Notas (10)/(11), L16137-16145: state the taxpayer has no rendimientos
      con período de generación superior a 2 años (art. 32.2.1º/32.3
      reductions do not apply), so "el rendimiento neto reducido de la
      actividad coincide con el rendimiento neto" and "el rendimiento neto
      reducido total coincide con el rendimiento neto reducido" — i.e. no
      further reduction is applied beyond the table's own 58.100 figure.
...
"58.100" — the identical anomaly documented for the 2024 manual's copy of
this same caso práctico (see that test's module docstring). That figure
appears nowhere else in the manual, is arithmetically unreachable from any
combination of the raw figures quoted above, is inconsistent with all three
explicit table rows (all 58.100), and is independently re-derivable
bottom-up from every raw line item via the registry's OWN
0180/0218/0221/0222/0223/0224 formula chain (see below) — the 61.400 figure
is not. This test grounds the target on 58.100.

Registry mapping (M100 2020 revision, casillas 0163-0236,
"actividad_est_directa" section — identical casilla numbering to the 2024
revision for this section):

    0171 "Ingresos de explotación" = Honorarios + Conferencias
      = 124.000 + 10.800 = 134.800. The manual's own nota (2) confirms
      conferencias/publicaciones income counts as actividad-económica
      income, so both line items are lumped into the single "ingresos de
      explotación" registry box.
    ...
    0186 "Seguridad Social del titular de la actividad" = 3.300 (RETA).
    ...
    0218 "Suma ([0181]..[0194] + [0198]..[0200] + [0202] + [0203] + [0205]
    ...
    0222 "Conjunto de provisiones deducibles y gastos de difícil
      ...
      = 2.000 — matches the table's own stated cap (nota 9's own
      arithmetic diverges but agrees on the 2.000 € cap outcome; see above).
    ...
    0225 "Reducciones de rendimientos generados en más de 2 años" = 0
      (notas 10/11: no hay rendimientos con período de generación > 2
      años). Unlike the 2024 revision, the 2020 revision has no separate
      "Reducción Copa América" casilla in this chain.
    0226 "Rendimiento neto reducido ([0224]-[0225])" = 58.100 - 0 = 58.100.
...
(138.400, 78.300, 2.000, 58.100) before reaching 0226. A companion test
flips the estimación-directa modalidad binding (0224's own ``if_then_else``
branch selector) from simplificada to normal and asserts 0226 changes — a
constant-returning formula chain would fail that check.
"""
...
# (es-normal, tax-residence-ccaa) and two relations (rel-130, rel-131
# pagos fraccionados) directly by id; every other formula target is either
# self-contained or feeds from an un-referenced manual-input casilla that
# defaults to zero when absent from ``inputs``. Supplying these four values
...
def test_0226_manual_worked_example_medico_radiologo_simplificada() -> None:
    ...
    Oracle: AEAT Manual práctico de Renta 2020, Parte 1, Cap. 7, caso práctico
    ...
    all 58.100 (L16090-16092). Every raw ingreso/gasto figure feeding the
...
def test_0226_anti_tautology_modalidad_switch_changes_value() -> None:
    ...
    diverge, mirroring the 2024 revision's companion test for this same
    scenario. A formula that ignored the modalidad binding, or always
    returned a constant, would fail this check.
    """
