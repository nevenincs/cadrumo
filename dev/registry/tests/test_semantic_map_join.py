...
def test_join_refuses_cell_only_anchor_variant_without_fuzzy_matching(_m200_snapshot) -> None:
...
def test_join_refuses_ambiguous_parser_anchor_before_constructing_any_design(_m200_snapshot) -> None:
    """Duplicate parser identities cannot select one matching semantic entry."""
    base_intermediate = _intermediate(_m200_snapshot)
    ...
    with pytest.raises(RegistryValidationError, match="duplicate exact anchors"):
        join_record_design_semantics(semantic_map, ambiguous_intermediate, _m200_snapshot)


def test_join_never_uses_anomaly_exception_as_missing_mapping_authority(_m200_snapshot) -> None:
    ...
    exception = SemanticMapAnomalyException(
        source_ref="aeat-dr-200-2025",
        source_sha256=_m200_snapshot.sources["aeat-dr-200-2025"].sha256,
