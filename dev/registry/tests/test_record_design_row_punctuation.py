"""Development-only: two ways a row's own punctuation cost it its position.
...
accepted that -- ``naturaleza_or_none`` strips " ." before matching -- so the
...
from ..record_design_pdf_repairs import collapse_stuttered_row_prefix
...
def test_a_type_written_with_abbreviation_punctuation_still_parses() -> None:
    row = parse_pdf_row("52 464 13 An. Complementaria (7) - Numero de Justificante anterior", 1)
...
def test_a_stuttered_ordinal_and_position_prefix_is_dropped() -> None:
    assert collapse_stuttered_row_prefix(("99 1592 99 1592 17 Num Deducciones",)) == ("99 1592 17 Num Deducciones",)


def test_an_ordinary_row_is_untouched() -> None:
    assert collapse_stuttered_row_prefix(("99 1592 17 Num Deducciones",)) == ("99 1592 17 Num Deducciones",)
...
def test_a_near_miss_is_not_treated_as_a_stutter() -> None:
    ...
    assert collapse_stuttered_row_prefix((line,)) == (line,)
