...
_CADRUMO_ROOT = Path(__file__).resolve().parents[3] / "src/cadrumo"
...
def _binding_records(value: object) -> Iterator[dict[str, object]]:
    if isinstance(value, dict):
        ...
        if isinstance(provider, dict) and provider.get("kind") == "ledger_iva_aggregation":
            yield {"id": record.get("id"), "selector": provider}
