...
_PROFILES: dict[str, tuple[tuple[str, ...], str, tuple[str, ...]]] = {
    "idi_limit_information": (
        ("deducciones_i_d_i_excluidas_de_limite", "informacion_adicional_para_el_calculo_de_limites_d"),
        "is_deduccion_idi_excluida_limite_info_adicional",
        _COMMON_LEGAL,
    ),
    "deduction_total_pending": (
        ("deducc_para_incentivar_determ_actividades", "total"),
        "is_deduccion_actividades_total_pendiente",
        _COMMON_LEGAL,
    ),
    "deduction_total_future": (
        ("deducc_para_incentivar_determ_actividades", "total"),
        "is_deduccion_idi_suma_pendiente",
        _COMMON_LEGAL,
    ),
    "deduction_total_applied": (("liquidacion_iv", "otras_deducciones"), "is_liquidacion_iv_importe", _COMMON_LEGAL),
    "aid_excess_quota": (
        ("conversion_activos_impuesto_diferido_credito_exigi", "exceso_cuota_liquida_positiva"),
        "is_conversion_aid_exceso_cuota_importe",
        ("ley-27-2014:art-130",),
    ),
    "public_interest_generated": (
        ("deducc_para_incentivar_determ_actividades", "2026_otras_deducciones_relativas_a_programas_de_ap"),
        "is_deduccion_acontecimiento_interes_publico_otras",
        _COMMON_LEGAL,
    ),
    "public_interest_applied": (
        ("deducc_para_incentivar_determ_actividades", "2026_otras_deducciones_relativas_a_programas_de_ap"),
        "is_deduccion_eventos_especiales_aplicado_periodo",
        _COMMON_LEGAL,
    ),
    "public_interest_future": (
        ("deducc_para_incentivar_determ_actividades", "2026_otras_deducciones_relativas_a_programas_de_ap"),
        "is_deduccion_eventos_especiales_pendiente",
        _COMMON_LEGAL,
    ),
    "nivelacion_dotacion": (
        ("reserva_de_nivelacion", "dotacion_de_la_reserva"),
        "is_reserva_nivelacion_dotacion",
        ("ley-27-2014:art-105",),
    ),
    "nivelacion_pendiente": (
        ("reserva_de_nivelacion", "dotacion_de_la_reserva"),
        "is_reserva_nivelacion_dotacion_pendiente",
        ("ley-27-2014:art-105",),
    ),
    "nivelacion_dispuesta": (
        ("reserva_de_nivelacion", "dotacion_de_la_reserva"),
        "is_reserva_nivelacion_dotacion_dispuesta",
        ("ley-27-2014:art-105",),
    ),
    "canarias_financier_cinema": (
        ("informacion_adicional_para_el_calculo_de_limites_d", "2025_financiador_deduccion_por_producciones_cinema"),
        "is_informacion_adicional_limites_deducciones_canarias_generada",
        _COMMON_LEGAL,
    ),
    "canarias_financier_live": (
        ("informacion_adicional_para_el_calculo_de_limites_d", "2025_financiador_deduccion_por_espectaculos_en_viv"),
        "is_informacion_adicional_limites_deducciones_canarias_generada",
        _COMMON_LEGAL,
    ),
    "canarias_rd": (
        ("informacion_adicional_para_el_calculo_de_limites_d", "2025_deduccion_por_investigacion_y_desarrollo_en_c"),
        "is_informacion_adicional_limites_deducciones_canarias_generada",
        _COMMON_LEGAL,
    ),
    "canarias_it": (
        ("informacion_adicional_para_el_calculo_de_limites_d", "2025_deduccion_por_innovacion_tecnologica_en_canar"),
        "is_informacion_adicional_limites_deducciones_canarias_generada",
        _COMMON_LEGAL,
    ),
    "canarias_producer_cinema": (
        ("informacion_adicional_para_el_calculo_de_limites_d", "2025_productor_deduccion_por_producciones_cinemato"),
        "is_informacion_adicional_limites_deducciones_canarias_generada",
        _COMMON_LEGAL,
    ),
    "canarias_producer_live": (
        ("informacion_adicional_para_el_calculo_de_limites_d", "2025_productor_deduccion_por_espectaculos_en_vivo"),
        "is_informacion_adicional_limites_deducciones_canarias_generada",
        _COMMON_LEGAL,
    ),
    "riib_anticipated": (
        ("reg_especial_reserva_inversiones_illes_balears", "inversiones_anticipadas_2025"),
        "is_reserva_inversiones_illes_balears_importe",
        ("ley-31-2022:da-70",),
    ),
    "donation_general": (
        ("deduccion_donativos_entidades_sin_fines_lucro", "donaciones_de_caracter_general"),
        "is_deduccion_donativos_general",
        ("ley-49-2002:art-20",),
    ),
    "donation_priority": (
        ("deduccion_donativos_entidades_sin_fines_lucro", "donaciones_para_actividades_prioritarias_de_mecena"),
        "is_deduccion_donativos_prioritarias",
        ("ley-49-2002:art-20", "ley-49-2002:art-22"),
    ),
}
_MANUAL_TOKENS = {
...
class Adjudication:
    """One target-year-reviewed S13 declaration receipt row."""
...
class CompiledM200UniqueAuthority:
    """The immutable S13 receipt used to render canonical bytes."""
...
def render_canonical_declaration(authority: CompiledM200UniqueAuthority, casilla_id: str) -> str:
    """Render the sole permitted canonical declaration for an S13 member."""
    ...
    return (
        "\n".join(
            (
                ...
                "data_type = 'money'",
                f"semantic_role = {_literal(row.semantic_role)}",
                "required = false",
                "input_kind = 'manual'",
...
def promoted_candidate_ids(
    authority: CompiledM200UniqueAuthority, *, casillas_root: Path | None = None
) -> frozenset[str]:
    """Return only receipt- and byte-verified S13 candidates."""
...
def _require_header(raw: dict[str, object]) -> None:
    ...
    if (
        ...
        or not isinstance(raw.get("reviewed_by"), str)
        or not isinstance(raw.get("reviewed_at"), str)
    ):
...
def _parse_row(raw: object) -> Adjudication:
    ...
    try:
        identifier, export, profile, column = (
            str(raw["casilla_id"]),
            str(raw["export_field_id"]),
            str(raw["profile"]),
            str(raw["column"]),
        )
...
def _target_fields_and_map() -> tuple[dict[str, object], dict[str, object], str]:
    ...
    if (
        source is None
        or manual is None
        or source.sha256 != TARGET_SOURCE_SHA256
    ...
    design = load_record_design_intermediate(
        bundled_path(), catalogues.sources, source_ref=TARGET_SOURCE_REF, filing_year=2024, design_epoch="2024"
    )
    ...
    return (
        fields,
        {str(entry.export_field_id): entry for entry in semantic_map.entries},
...
def _require_legal_coverage(rows: tuple[Adjudication, ...]) -> None:
    legal = {
        key: value
        for path in bundled_path("registry", "aeat", "legal").glob("*.toml")
        for key, value in load_catalogue_file(path).legal.items()
    }
    for row in rows:
