"""The parser-read record design, and the one loader that assembles it.

The official record-design binary remains the coordinate authority. This module
selects that binary through the registry catalogue, runs the shipped parser, and
projects its output into a frozen, source-anchored representation: which binary
an epoch was read from, in what format, what each field's exact anchor within it
is, and how the fixed records, variable envelopes and auxiliary headers compose.
It does not read extracted derivatives or parse the source independently.

Both halves -- the representation and the loader that builds it -- are one
contract, because the callers that hold a parsed design are the callers that ask
for one. The export-tree generator, the render-profile eligibility question, the
declaration validators, the filing-export proof and the analysis screens all sit
outside this package and address the same aggregate; the semantic join and the
provenance manifest key off the same anchors. A caller wanting a parsed design
asks the loader here; a caller merely holding one of its fields or its source
needs only the types here. Splitting the two across a public type module and a
private loader module would have left every one of those callers reaching into a
private module for the loader, which is the boundary breach the split was meant
to avoid.

Assembly detail stays private to this module: the builders below project the
shipped parser's sheets, envelopes and closings into these records, and nothing
outside calls them.
"""
