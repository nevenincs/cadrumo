...
_REOPENING_CONDITIONS: Final[Mapping[ModeloWorkspaceActionDisposition, str]] = {
    ModeloWorkspaceActionDisposition.C1_BOUNDED_REVIEW: "never reopens: terminal C1 disposition",
    ModeloWorkspaceActionDisposition.C1_OR_C2_READ_PENDING: (
        "reopens on migration to a numbered C1/C2 destination in the same commit"
    ),
    ModeloWorkspaceActionDisposition.C4_MUTATION_PENDING: (
        "reopens once the C3/C4 conformance suites are green and this is enrolled as a C4 action"
    ),
    ModeloWorkspaceActionDisposition.FLOW_OWNED: (
        "reopens only if C4 assigns this wizard a distinct disposition"
    ),
    ModeloWorkspaceActionDisposition.DEFERRED: (
        "reopens only if a future accepted decision moves this into scope"
    ),
}
"""The condition a row on each arm is scheduled by, closed per arm.

Every populated arm carries exactly one condition, so this is a vocabulary
rather than a convention that happens to hold. The three arms absent from it
are the two delivered ones and the reserved non-visual one, none of which has
a population yet; leaving those unconstrained is deliberate, because inventing
a condition for an arm nothing occupies would be guessing rather than
recording what the table says.
"""
...
class ModeloWorkspaceActionClassificationV1(BaseModel):
    ...
    def _check_the_reopening_condition_belongs_to_the_disposition(self) -> ModeloWorkspaceActionClassificationV1:
        """Refuse a row whose reopening condition belongs to a different arm.

        A row carries three narrative fields beside its disposition and only
        the reason was validated, which left the condition a row is SCHEDULED
        BY free to contradict the arm it is scheduled AS. That is not
        cosmetic: a mutation carrying a read's condition is queued behind the
        read migration and never enrolled for the conformance its own arm
        requires, and a row sitting under the wrong condition is
        indistinguishable from a row nobody has considered.

        The failure is not hypothetical. One row reclassified from read to
        mutation had its disposition and reason updated and its reopening
        condition left behind, and nothing detected the contradiction until
        the arms were counted against their conditions.
        """
        expected = _REOPENING_CONDITIONS.get(self.disposition)
        if expected is not None and self.reopening_condition != expected:
            raise ValueError(
                f"reopening condition for {self.action_identity!r} belongs to a different "
                f"disposition: {self.disposition.value!r} requires {expected!r}, got "
                f"{self.reopening_condition!r}",
            )
        return self
