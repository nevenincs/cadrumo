...
def _run_generators_in_verified_copy(
    *,
    root: Path,
    ...
    if not expected and not generated_paths:
        # Nothing is generated, so there are no generated bytes to admit and no
        # owner to run. Snapshotting and copying the whole repository here would
        # verify a copy against an empty admission set, and the minutes it takes
        # are minutes in which unrelated churn elsewhere in the tree refuses a
        # replay that never depended on it.
        return (), {}
    paths = _git_snapshot_paths(root)
    files = _snapshot(root, paths)
    temporary_candidate = Path(tempfile.gettempdir())
...
def replay_object_name_component(
    ...
    *,
    ...
    try:
        ...
        # Execute declared owners against an exact post-transform copy. Only their
        # reviewed generated bytes are admitted to the live transaction.
        generator_outcomes, generated_payloads = _run_generators_in_verified_copy(
            root=root,
        ...
        for relative in sorted(generated_paths):
            payload = generated_payloads.get(relative)
            if payload != proposed_payloads[relative]:
                raise ObjectNameReplayError(f"live generator output differs from the receipt: {relative}")
            target = _safe_path(root, relative, allow_missing_leaf=True)
            missing = []
            parent = target.parent
            while parent != root and not parent.exists():
        ...
        primary_failure_active = primary_error is not None
