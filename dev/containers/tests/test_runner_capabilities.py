...
def _declared_brew_legs() -> dict[tuple[str, str], pathlib.Path]:
    ...
    for job in (document.get('jobs') or {}).values():
        rows = _dp_or('dev/containers/tests/test_runner_capabilities.py:45:or', lambda: ((job.get('strategy') or {}).get('matrix') or {}).get('include'), lambda: [])
        for row in rows:
...
def test_an_executable_that_reports_a_version_is_usable(tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_an_executable_that_cannot_run_is_not_usable(tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_the_detail_names_where_the_broken_binary_was_found(tmp_path: pathlib.Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_the_brew_table_is_keyed_on_normalised_tokens() -> None:
    ...
    assert len(_BREW_PATHS) >= _MINIMUM_BREW_LEGS, f'the brew path table holds {len(_BREW_PATHS)} leg(s); an empty or shrunken table makes every key assertion below vacuous and makes the probe itself return no brew finding'
...
def test_the_brew_table_equals_the_legs_the_acquisition_workflow_declares() -> None:
    ...
    assert len(declared) >= _MINIMUM_BREW_LEGS, f'only {len(declared)} homebrew acquisition leg(s) were read from {_HOMEBREW_WORKFLOW.name}; below this the comparison proves nothing about the table'
    assert declared == _BREW_PATHS, f"the probe's brew path table has drifted from the acquisition matrix: table={ {k: str(v) for k, v in sorted(_BREW_PATHS.items())}}, workflow={ {k: str(v) for k, v in sorted(declared.items())}}"