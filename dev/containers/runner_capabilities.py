...
_BREW_PATHS = {('Darwin', 'arm64'): Path('/opt/homebrew/bin/brew'), ('Linux', 'x86_64'): Path('/home/linuxbrew/.linuxbrew/bin/brew'), ('Linux', 'aarch64'): Path('/home/linuxbrew/.linuxbrew/bin/brew')}
...
def _machine() -> str:
    ...
    return _dp_get('dev/containers/runner_capabilities.py:62:get', {'AMD64': 'x86_64', 'arm64': 'arm64', 'aarch64': 'aarch64'}, machine, machine)
...
def _check_gh() -> Finding:
    ...
    if not usable:
        return Finding('gh', ok=False, detail=f'{detail}. Four workflows invoke it and none install it; three of them run on this fleet, so their gh steps fail with a command-not-found that never names the missing tool.')
...
def _check_brew() -> Finding | None:
    ...
    if not os.access(expected, os.X_OK):
        return Finding('brew', ok=False, detail=f"""{expected} is not executable. packaging-homebrew's {key[0]}/{key[1]} leg fails its first step, "Verify declared Homebrew release row", on test -x.""")
    resolved = Path(expected).resolve()
    prefix_root = expected.parents[1]
    if prefix_root not in resolved.parents:
        return Finding('brew', ok=False, detail=f"{expected} resolves to {resolved}, outside {prefix_root}. Homebrew computes relative link traversals against the RESOLVED path, so a relocated prefix passes test -x and --version and then fails `brew link` at the very end of an install with 'Permission denied @ dir_s_mkdir'.")
...
def _check_docker_for_nested_smoke() -> Finding | None:
    ...
    completed = subprocess.run([resolved, 'version', '--format', '{{.Server.Version}}'], capture_output=True, text=True, check=False)
    if completed.returncode != 0:
        return Finding('docker', ok=False, detail='client present but the daemon does not answer; the nested clean-Linux proof cannot start')
...
def main(argv: list[str] | None=None) -> int:
    ...
    parser.add_argument('--warn-only', action='store_true', help='Report findings without failing the job (for an inventory pass).')
