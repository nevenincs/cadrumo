"""Machine-readable CI reports for this repository's pytest runs.

Loaded as a plugin from the repository-root ``conftest.py`` rather than written
into it, because the conftest's own ``pytest_configure`` is ``trylast`` - it
announces the run log after everything else has configured - and the junitxml
option has to be set BEFORE the junitxml plugin reads it. A plugin module gets
its own hook implementation with default ordering, which is early enough.
"""
...
def pytest_configure(config: pytest.Config) -> None:
    """Point this run at its JUnit report, when one was asked for."""
