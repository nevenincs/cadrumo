...
canonical :func:`~dev.docs.terminology.legal_projection.legal_target_record_id`
helper), and
...
def coverage_report_path() -> Path:
    """Return a fresh per-run destination for the coverage report under ``.logs``.

    A measurement artefact, not shipped data: it is regenerated by a coverage
    run and read back by nothing, so it is run output rather than a source.
    Run output belongs in the repository's scratch log root, which the next
    ``just clean-apply`` reclaims -- never beside this harness under ``dev/``,
    where it survived every clean and had to be hidden by a ``.gitignore``
    entry to stay out of commits.

    The path is allocated, not fixed: :func:`allocate_run_directory` stamps a
    date, pid and uuid, so two coverage runs cannot overwrite each other's
