...
* Stable channel facts (channel id, platform, install commands, package /
  marketplace / bucket / tap names) live in the descriptor this module reads.
...
Unproven channels
-----------------
The descriptor carries no availability state, and this generator withholds
nothing: every declared channel renders its install commands verbatim. A
channel that cannot be proven is therefore removed from the descriptor rather
than left declared and gated, which is also why every declared row is a
required row in :mod:`dev.release.readiness`.

What stops the page advertising a channel ahead of its passing distribution
evidence is the claims gate in :mod:`dev.docs.tests.test_distribution_claims`,
which scans the hand-authored prose for acquisition claims and demands a
passing evidence record for every row the claimed channel owns. Land the
evidence in the same change that adds the channel to the descriptor.
...
def render_zone(descriptor: DownloadDescriptor) -> str:
    ...
    and the registry it comes from), the per-channel install sections, and the
    Tier-2 mount element that ``initDownloadCards()`` fills at runtime.
    """
    lines: list[str] = []
