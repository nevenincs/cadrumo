# Repository TUI evaluation harness
...
```sh
uv run --no-sync python -m dev.tui.harness surfaces
uv run --no-sync python -m dev.tui.harness open manager
uv run --no-sync python -m dev.tui.harness press down enter
uv run --no-sync python -m dev.tui.harness type "12345678Z"
uv run --no-sync python -m dev.tui.harness undo
```
