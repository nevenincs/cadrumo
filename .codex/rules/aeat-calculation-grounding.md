...
Emit every casilla in engine_result.values, not only computed entries. Input and bound casillas must produce CasillaObservation rows pulled from the registry casilla definition (legal_refs, source_refs). Computed casillas pull the same fields from the matching engine entry. Never drop a casilla on the way to the persisted revision.
