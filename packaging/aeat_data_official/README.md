...
and normative PDFs — for the [`aeat`](https://github.com/nevenincs/cadrumo)
Spanish-tax toolkit.
...
`cadrumo-data-official` is version-locked to the `aeat` distribution: it and its
sibling `cadrumo-data-manuals` ship at the same version as `aeat`, enforced by a
