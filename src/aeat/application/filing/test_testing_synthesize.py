"""Unit tests for :func:`aeat.application.filing.testing.synthesize_filing_draft`.

The helper is a pure construction surface with no AEAT round-trip, so
every test is a strict pydantic validation plus a content-hash sanity
check.

The structural write guard remains intact: this helper constructs
read-shaped :class:`aeat.domain.filing.FilingDraft` records with no
