"""Structural write-guard for :mod:`aeat.application.filing.reconciliation`.

Mirrors ``aeat.adapters.outbound.aeat.sede.test_no_write_surface``.
Walks every ``.py`` source under the subpackage and fails if a
forbidden call-context verb appears.

The reconciliation layer is a pure compare and MUST remain structurally
free of any AEAT-mutation verb, English or Spanish. The forbidden-verb
list is loaded from ``_no_write_surface_fixture.txt`` alongside this
test module.
"""
...
def _load_forbidden_verbs() -> tuple[str, ...]:
    """Read the forbidden-verb fixture, skipping blanks and ``#`` comments."""
...
class TestNoWriteModeLiteral:
    """Assert no boundary record declares a non-read structural mode."""

    def test_no_write_mode_literal(self) -> None:
        """Fail if any source file types a record's ``mode`` as a write literal."""
        mode_char = "w"
