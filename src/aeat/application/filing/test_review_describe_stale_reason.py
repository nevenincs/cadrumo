...
`describe_stale_reason` maps each :class:`FilingApprovalStaleReason`
...
from . import FilingApprovalStaleReason, describe_stale_reason
...
def test_describe_stale_reason_renders_approval_basis_version_changed() -> None:
    assert (
        describe_stale_reason(FilingApprovalStaleReason.APPROVAL_BASIS_VERSION_CHANGED)
        == "approval basis version changed"
    )


def test_describe_stale_reason_renders_draft_payload_changed() -> None:
    assert describe_stale_reason(FilingApprovalStaleReason.DRAFT_PAYLOAD_CHANGED) == "draft payload changed"
...
def test_describe_stale_reason_renders_draft_review_changed() -> None:
    ...
    assert describe_stale_reason(FilingApprovalStaleReason.DRAFT_REVIEW_CHANGED) == "draft validation surface changed"


def test_describe_stale_reason_renders_transaction_catalogue_changed() -> None:
    assert (
        describe_stale_reason(FilingApprovalStaleReason.TRANSACTION_CATALOGUE_CHANGED)
        == "transaction catalogue changed"
    )


def test_describe_stale_reason_renders_category_profiles_changed() -> None:
    assert describe_stale_reason(FilingApprovalStaleReason.CATEGORY_PROFILES_CHANGED) == "category profiles changed"
...
def test_describe_stale_reason_renders_schema_formula_changed() -> None:
    ...
    assert (
        describe_stale_reason(FilingApprovalStaleReason.SCHEMA_FORMULA_CHANGED)
        == "schema or formula provenance changed"
    )


@pytest.mark.parametrize("reason", list(FilingApprovalStaleReason))
def test_describe_stale_reason_returns_lowercase_phrase(reason: FilingApprovalStaleReason) -> None:
...
@pytest.mark.parametrize("reason", list(FilingApprovalStaleReason))
def test_describe_stale_reason_returns_non_empty_phrase(reason: FilingApprovalStaleReason) -> None:
    phrase = describe_stale_reason(reason)
...
@pytest.mark.parametrize("reason", list(FilingApprovalStaleReason))
def test_describe_stale_reason_phrase_contains_no_underscores(reason: FilingApprovalStaleReason) -> None:
...
def test_describe_stale_reason_covers_every_enum_member() -> None:
    ...
    explicit = {
        FilingApprovalStaleReason.APPROVAL_BASIS_VERSION_CHANGED: "approval basis version changed",
        FilingApprovalStaleReason.DRAFT_PAYLOAD_CHANGED: "draft payload changed",
        FilingApprovalStaleReason.DRAFT_REVIEW_CHANGED: "draft validation surface changed",
        FilingApprovalStaleReason.TRANSACTION_CATALOGUE_CHANGED: "transaction catalogue changed",
        FilingApprovalStaleReason.CATEGORY_PROFILES_CHANGED: "category profiles changed",
        FilingApprovalStaleReason.SCHEMA_FORMULA_CHANGED: "schema or formula provenance changed",
    }

    # Every current enum member is covered by the explicit map.
    assert set(explicit.keys()) == set(FilingApprovalStaleReason)
