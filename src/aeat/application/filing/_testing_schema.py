"""Pydantic v2 schema for synthetic filing-history fixtures.
...
and re-loaded by
:func:`aeat.application.filing._testing_loader.load_filing_history`.
"""
...
class FilingRecordScenario(StrEnum):
    ...
    Attributes:
        CLEAN: A baseline successful filing with no special handling.
        COMPLEMENTARIA: A correction filing referencing a prior CSV.
        WITH_ERRORS: A filing whose validation surface raised errors.
        AMENDED: A filing that has been superseded by a later one.
        CANCELLED: A filing whose status moved to cancelled.
        ROUNDING: A borderline case exercising decimal rounding edges.
    """
...
class FixtureCasilla(BaseModel):
    ...
    Attributes:
        casilla_id: Stable casilla ID (e.g. ``"03"``). Carried as a
            loose string so fixtures can reference codes that are not
            yet enumerated in the typed casilla database.
        ...
        value: The scalar value carried by this casilla. ``None``
            means the casilla is present on the form but left blank.
    """
