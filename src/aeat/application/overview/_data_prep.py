...
def build_data_prep_walkthrough(
    *,
    ...
    steps.append(_work_unit_step(modelo=modelo, filing_year=period.year, period=period, work_units=work_units))
...
def _import_step(period: Period, period_transactions: tuple[Transaction, ...]) -> DataPrepStep:
    ...
    if count == 0:
        return DataPrepStep(
            ...
            summary=f"0 transactions recorded for {period.registry_token} {period.year}.",
            next_command="aeat app ledger import --file <statement.csv>",
        )
    return DataPrepStep(
        step_id=DataPrepStepId.IMPORT_TRANSACTIONS,
        state=DataPrepStepState.DONE,
        summary=f"{count} transaction(s) recorded for {period.registry_token} {period.year}.",
        next_command=f"aeat app ledger list --period {period.registry_token} --year {period.year}",
    )
...
def _invoices_step(period: Period, invoice_catalogue: InvoiceCatalogue) -> DataPrepStep:
    ...
    if count == 0:
        return DataPrepStep(
            ...
            state=DataPrepStepState.PENDING,
            summary=f"0 business invoice(s) registered for {period.registry_token} {period.year}.",
            next_command="aeat app ledger invoice add --help",
        )
    return DataPrepStep(
        step_id=DataPrepStepId.REGISTER_INVOICES,
        state=DataPrepStepState.DONE,
        summary=f"{count} business invoice(s) registered for {period.registry_token} {period.year}.",
...
def _readiness_step(preflight_report: LedgerPreflightReport) -> DataPrepStep:
    ...
    if issue_count == 0:
        return DataPrepStep(
            ...
            f"--period {preflight_report.period.registry_token} --year {preflight_report.period.year}",
        )
    return DataPrepStep(
        step_id=DataPrepStepId.RESOLVE_READINESS,
