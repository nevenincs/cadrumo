...
def test_bulk_capture_reports_registry_unsupported_modelos_as_local_boundaries(tmp_path: Path) -> None:
    ...
    assert report.modelos == ("036", "721")
    assert report.captured_count == 0
    assert report.failed_count == 2
    failures = {failure.modelo: failure for failure in report.failures}
    assert set(failures) == {"036", "721"}
    assert failures["036"].year == 2024
    assert failures["036"].error_type == "LiveApplicationInputError"
    assert "declares no filed-declarations live read surface" in failures["036"].message
