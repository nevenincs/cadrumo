...
def _find_observation(
    ...
    *,
    ...
    """Scan iter_modelo and return the envelope for (filing_year, period) or None.

    The production retrieval path for informativa observations: iter_modelo
    performs a full-scan over the namespace, decrypting and filtering in Python.
    This is the correct API since the ``object_key`` column is ``EncryptedString``
    (AES-256-GCM with a random nonce), which means a SQL WHERE-by-key query can
    never match the stored ciphertext.
    """
