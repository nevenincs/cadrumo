...
def _seed_censo(bucket_id: str) -> None:
    from ...live._censo import CensoSnapshot, CensoSnapshotRepository
...
def _seed_notifications(bucket_id: str) -> None:
    ...
    from ...live._notifications import PersistedNotificationsSnapshot, _notifications_repository
...
def _seed_expedientes(bucket_id: str) -> None:
    from ....core.config import load_settings
    from ...live._expedientes import PersistedExpedientesSnapshot, _expedientes_repository
...
def _seed_verify_observation(bucket_id: str) -> None:
    from ...live._verify import VerifyObservation, VerifyObservationRepository, VerifySurface
...
def _seed_business_operation_invoice(bucket_id: str) -> None:
    from ...ledger._business_operation_invoice import (
...
def _filed_artefact(body: bytes) -> object:
    from ....adapters.outbound.aeat.sede._schema import FiledDeclaracionArtefact
...
_CASES: tuple[StoreCase, ...] = (
    ...
    StoreCase("aeat.application.live.borrador_100_snapshot", _seed_borrador_100, _verify_borrador_100),
    StoreCase("aeat.application.modelo.m036_declaration", _seed_m036, _verify_m036),
    StoreCase("aeat.application.live.verify_observations", _seed_verify_observation, _verify_verify_observation),
    StoreCase("aeat.application.user_profile.snapshot", _seed_user_profile_snapshot, _verify_user_profile_snapshot),
    ...
    StoreCase("aeat.domain.submission.records", _seed_submission, _verify_submission),
    StoreCase("aeat.application.evidence.bundles", _seed_evidence_bundle, _verify_evidence_bundle),
    StoreCase("aeat.calculations.iva_compensation.history", _seed_iva_compensation, _verify_iva_compensation),
    StoreCase("aeat.application.filing.history", _seed_filing_history, _verify_filing_history),
    StoreCase("aeat.domain.filing.drafts", _seed_draft, _verify_draft),
    StoreCase("aeat.domain.usage_ratios", _seed_usage_ratios, _verify_usage_ratios),
    StoreCase("aeat.domain.invoices", _seed_invoice_catalogue, _verify_invoice_catalogue),
    StoreCase("aeat.domain.modelos.verification_reports", _seed_verification_reports, _verify_verification_reports),
    StoreCase(
    ...
    StoreCase(
        "aeat.application.live.iva_remote_state_acquisitions",
    ...
    StoreCase("aeat.application.live.notifications_snapshot", _seed_notifications, _verify_notifications),
    StoreCase("aeat.application.live.expedientes_snapshot", _seed_expedientes, _verify_expedientes),
)
