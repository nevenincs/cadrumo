...
def _next_wizard_action(
    *,
    ...
    if not has_profile:
        return "aeat config init --profile NAME --tax-id NIF --activity TEXT --iva-regime REGIME"
    if missing_required:
        return f"aeat config profile set {missing_required[0]} VALUE"
    if missing_enrolment:
        return f"aeat config profile set {missing_enrolment[0]} general"
    if not auth_provider:
        return "aeat config auth configure --provider certificate --certificate-path PATH"
    if not login_ready:
