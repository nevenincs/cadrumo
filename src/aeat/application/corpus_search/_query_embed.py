...
(:mod:`._embed_build`); the model is needed at runtime ONLY to embed the
operator's live query into the same vector space so a cosine search can run.
This module owns that one live use of ``model2vec``, behind the capability-
gated ``aeat[search]`` extra: absent the extra, :func:`search_extra_available`
