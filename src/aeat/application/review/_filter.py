...
- ``aeat app review list --filter status=pending --filter kind=received``
- ``aeat app modelo status --filter status=pending --period {period} --modelo {modelo}``
...
class InvoiceReviewFilterKey(StrEnum):
    """Closed catalogue of ``aeat app review --filter`` keys.
...
class DeclarationReviewFilterKey(StrEnum):
    """Closed catalogue of ``aeat app modelo status --filter`` keys.
...
class InvoiceReviewFilterSpec(BaseModel):
    """Typed ``aeat app review --filter`` spec.
...
class DeclarationReviewFilterSpec(BaseModel):
    """Typed ``aeat app modelo status --filter`` spec.
