...
Every mutating noun-group in the redesigned CLI
...
Entries shipped in this initial registry:
  ...
  - ``aeat app ledger evidence``     (locked CRUD reference)
  - ``aeat app ledger payable-invoice``
  - ``aeat app ledger collectible-invoice``
...
USAGE_RATIOS = MutatingNounGroupContract(
    ...
    # Key-value-as-record exception per the contract; the operator edits
...
INVENTORY = MutatingNounGroupContract(
    ...
    # per the inventory specification.
...
APODERADO = MutatingNounGroupContract(
    ...
    # Lifecycle-only exception per specification:
