...
from ...modelo import calculate_modelo_revision_from_bucket_aggregation_with_diagnostics
from ...modelo._action_errors import ModeloAggregationBindingError
from ...modelo._verification_actions import _manual_fact_basis_entries
...
def test_m210_gross_income_source_mode_keeps_manual_and_ledger_authority_exclusive(tmp_path: Path) -> None:
    ...
    assert all(
        entry.casilla_id != "rendimientos_integros"
        for entry in _manual_fact_basis_entries(
            ledger.input_values_by_casilla_id,
            ledger.observations,
            m210_gross_income_source_mode=ledger.m210_gross_income_source_mode,
        )
    )
