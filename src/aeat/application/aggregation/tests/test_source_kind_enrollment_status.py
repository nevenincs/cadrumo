...
Register items D4/D5 require every deferred source kind to be governed rather
than merely enumerated: the deferrals ADR
(`2026-07-02-arch-remediation-source-kind-deferrals-adr`) re-ratifies each kind
with a typed promotion target (owning ADR stem + trigger) instead of a
free-prose comment. This gate reads that declaration
(:data:`DEFERRED_SOURCE_KIND_TARGETS`) and asserts:

* every deferred kind carries a promotion target whose owning ADR resolves to a
  real decision record and whose trigger is stated (S08); and
...
def test_deferred_membership_is_derived_from_the_targets_mapping() -> None:
    ...
    new deferred kind without an owning ADR + trigger impossible: a kind is
    deferred only by being a key in ``DEFERRED_SOURCE_KIND_TARGETS``.
    """
    assert frozenset(DEFERRED_SOURCE_KIND_TARGETS) == DEFERRED_SOURCE_KINDS


def test_every_deferred_kind_carries_an_owning_adr_and_trigger() -> None:
    """Each deferred kind names the decision that owns it and its promotion trigger (S08)."""
    offences: list[str] = []
...
def test_every_owning_adr_resolves_to_a_real_decision_record() -> None:
    """Each deferred kind's owning ADR stem resolves to a shipped ``.vault/adr`` file.

    A typo'd or dangling owning-ADR stem is caught here rather than being
    ...
    assert not missing, "Deferred kinds citing an unresolvable owning ADR:\n" + "\n".join(f"  * {m}" for m in missing)


def test_no_deferred_kind_has_a_fired_trigger() -> None:
    """A deferred kind whose named dependency has been promoted must not stay deferred (S09).
    ...
    for kind, target in DEFERRED_SOURCE_KIND_TARGETS.items():
        ...
        if dependency not in DEFERRED_SOURCE_KINDS:
            fired.append(
                ...
                f"re-ratify its deferral with a new trigger (owning ADR {target.owning_adr})."
            )
...
def test_dependency_triggers_reference_a_real_deferred_or_enrolled_kind() -> None:
    ...
    pre-fire state this campaign ships in), so the S09 check is live.
    """
    for kind, target in DEFERRED_SOURCE_KIND_TARGETS.items():
        dependency = target.promotion_depends_on
        if dependency is None:
