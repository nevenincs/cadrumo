...
own invariant: children must sum to the parent with the parent's sign.
"""
...
def derive_child_amounts(gross: Decimal, proportions: Sequence[Decimal]) -> tuple[Decimal, ...]:
    ...
    the rounding remainder so the amounts sum to ``gross`` exactly. Every amount
    carries the sign of ``gross`` (the children of an outgoing transaction are all
    outgoing).

    Args:
        gross: The parent transaction gross (signed).
    ...
    Raises:
        ValueError: When fewer than two proportions are supplied.
    """
