...
def _seed_sofia_profile(objects: SecureObjectRepository) -> None:
    record = UserProfileRecord(
        ...
        facts=(
            ...
            UserProfileFact(path="renta_taxpayer.sex", value="mujer"),
...
def test_sofia_m100_2025_work_create_and_calculate_exposes_0186_and_0199(
    ...
    assert str(binding_by_id[_M100_SS_BINDING].source) == "ledger_renta_expense_aggregation"
    assert str(binding_by_id[_M100_OTHER_EXPENSES_BINDING].source) == "ledger_renta_expense_aggregation"
    ...
    assert not any(
        diagnostic.source_kind == "ledger_renta_expense_aggregation" for diagnostic in result.source_diagnostics
