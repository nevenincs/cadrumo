...
class AmendmentOverrideCasillaError(ModeloError):
    """Raised when ``amend_modelo_revision`` is asked to apply an
    override against a casilla id the registry does not declare for
    the baseline's modelo / filing year / period. The amend path
    refuses silently fabricating casillas in the corrected revision
    because the corrected revision is the legal basis of the
    complementaria filing."""
...
def _reject_unknown_override_casillas(
    *,
    ...
    """Refuse override casilla ids the registry does not declare.

    A complementaria filing is built from the corrected casilla map.
    If an override carries a casilla id the registry has no record of,
    the resulting revision would carry a fabricated casilla that never
    flows through formula evaluation, ledger projection, or filing
    export — i.e. operator-typed garbage masquerading as a legal
    correction. The amend verb refuses such overrides up front.
    """
