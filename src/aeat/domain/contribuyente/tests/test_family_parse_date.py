...
def test_descendant_info_birth_date_from_iso_string() -> None:
    info = DescendantInfo(birth_date="2020-06-01")  # type: ignore[arg-type]
...
def test_descendant_info_adoption_date_from_iso_string() -> None:
    info = DescendantInfo(birth_date=date(2020, 1, 1), adoption_date="2021-05-10")  # type: ignore[arg-type]
    assert info.adoption_date == date(2021, 5, 10)


def test_descendant_info_birth_date_bad_format_raises() -> None:
    with pytest.raises(ValidationError):
        DescendantInfo(birth_date="01-06-2020")  # type: ignore[arg-type]
...
def test_renta_descendant_birth_date_from_iso_string() -> None:
    p = RentaDescendantProfile(birth_date="1990-11-22")  # type: ignore[arg-type]
...
def test_renta_descendant_death_date_from_iso_string() -> None:
    p = RentaDescendantProfile(birth_date=date(1990, 1, 1), death_date="2023-07-04")  # type: ignore[arg-type]
    assert p.death_date == date(2023, 7, 4)


def test_renta_descendant_birth_date_bad_format_raises() -> None:
    with pytest.raises(ValidationError):
        RentaDescendantProfile(birth_date="22/11/1990")  # type: ignore[arg-type]
...
def test_renta_ascendant_birth_date_from_iso_string() -> None:
    p = RentaAscendantProfile(birth_date="1955-03-08")  # type: ignore[arg-type]
...
def test_renta_ascendant_death_date_from_iso_string() -> None:
    p = RentaAscendantProfile(birth_date=date(1955, 1, 1), death_date="2022-12-31")  # type: ignore[arg-type]
    assert p.death_date == date(2022, 12, 31)


def test_renta_ascendant_birth_date_bad_format_raises() -> None:
    with pytest.raises(ValidationError):
        RentaAscendantProfile(birth_date="not-a-date")  # type: ignore[arg-type]
