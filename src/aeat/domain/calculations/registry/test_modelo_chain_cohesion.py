...
The AEAT filing cycle has several well-defined chains where a periodic
modelo (filed monthly or quarterly) feeds an annual-summary modelo:
...
- 303 (IVA autoliquidacion trimestral)    → 390 (IVA resumen anual)
...
that expectation so a future change cannot silently drop the
relation. When a relation is missing, the test fails with a message
that names both the feeder and the summary modelo so the gap is
...
# filing cycle requires. Add a pair here when a new feeder→summary chain
# is established by an AEAT order or rebuild ADR; remove a pair when an
# AEAT change deprecates it. The list is the contract.
