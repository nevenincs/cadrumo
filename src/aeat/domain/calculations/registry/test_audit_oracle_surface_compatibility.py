...
def _bind_first_cross_reference(
    ...
    *,
    ...
    cross_references[0] = cross_references[0].model_copy(
        update={"oracle_id": oracle_id, "surface": surface}
    )
...
def test_compatible_pair_passes_audit() -> None:
    modelo = _bind_first_cross_reference(
        _modelo_130(), oracle_id="vat-checker", surface="public_read_surface"
    )
...
def test_static_official_documentation_surface_rejects_every_oracle() -> None:
    modelo = _bind_first_cross_reference(
        _modelo_130(), oracle_id="any-oracle", surface="static_official_documentation"
    )
    catalogue = LiveParityCatalogue()
...
def test_authenticated_read_surface_rejects_open_simulator_oracle() -> None:
    modelo = _bind_first_cross_reference(
        _modelo_130(), oracle_id="sim-oracle", surface="authenticated_read_surface"
    )
    catalogue = LiveParityCatalogue()
    catalogue.register(
        _StubOracle("sim-oracle", surface_kind="open_simulator"), environment="production"
    )
...
def test_lookup_failure_does_not_double_report_surface_incompatibility() -> None:
    ...
    modelo = _bind_first_cross_reference(
        _modelo_130(), oracle_id="absent", surface="static_official_documentation"
    )
    catalogue = LiveParityCatalogue()
...
def test_every_allow_listed_pair_passes_audit(surface: str, surface_kind: OracleSurfaceKind) -> None:
    modelo = _bind_first_cross_reference(
        _modelo_130(), oracle_id="probe", surface=surface
    )
    catalogue = LiveParityCatalogue()
