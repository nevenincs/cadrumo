"""Regression tests for Modelo 100 2024 settlement-chain tail (S361).
...
Before S361, all six casillas had ``input_kind = "manual"`` and no formula
in the 2024 revision, so they stayed at 0.  After S361 they are
...
def test_s361_0587_cuota_liquida_total_is_computed(m100_2024_snapshot) -> None:
    """After S361, casilla 0587 must equal 0585 + 0586 (not stay at zero).

    S361 regression guard: before the fix, 0587 had no formula in the 2024
    revision and defaulted to 0.  After S361 the formula
    ...
    result = calculate_registry_snapshot(
        ...
        # chain produces 0500 = 0505 = 55500 (same technique as S353 tests).
    ...
    assert result.values["0587"] != Decimal("0"), (
        ...
        "the S361 regression has re-appeared: check "
...
def test_s361_0609_total_pagos_a_cuenta_computed_from_0598(m100_2024_snapshot) -> None:
    """After S361, casilla 0609 must aggregate retenciones including 0598.
...
def test_s361_0610_cuota_diferencial_computed(m100_2024_snapshot) -> None:
    """After S361, casilla 0610 must equal 0595 - 0609.
...
def test_s361_0670_resultado_declaracion_computed(m100_2024_snapshot) -> None:
    """After S361, casilla 0670 must equal 0610 for a taxpayer with no adjustments.
...
def test_s361_settlement_chain_not_zero_for_non_zero_base(m100_2024_snapshot) -> None:
    ...
    This is the weakest regression guard: before S361, every one of
    0587/0595/0609/0610/0670 was 0 regardless of inputs, because no formula
    existed in the 2024 revision.  After S361, the chain must produce non-zero
...
def test_s361_anti_tautology_retenciones_change_affects_chain(m100_2024_snapshot) -> None:
