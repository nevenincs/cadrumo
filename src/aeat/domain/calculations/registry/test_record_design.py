...
def test_modelo_347_legacy_positional_chart_pdf_extracts_reviewable_record_data() -> None:
    ...
    assert all(
        field.content == "Extracted from legacy Modelo 347 visual chart geometry."
        for sheet in sheets
        for field in sheet.fields
    )


def test_generated_non_table_pdf_does_not_activate_legacy_visual_chart_fallback(tmp_path: Path) -> None:
    pdf_path = tmp_path / "non-record-design.pdf"
    _write_pdf_lines(
        pdf_path,
        (
            "MODELO 347 REGISTRO DE TIPO 1 REGISTRO DE DECLARANTE",
            "This page names one legacy heading but has no position ruler or record field geometry.",
        ),
    )
