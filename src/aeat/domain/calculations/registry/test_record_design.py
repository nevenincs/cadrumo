...
from ._record_design import extract_record_design_pdf, extract_record_design_pdf_bytes, extract_record_design_workbook
...
def test_modelo_840_record_design_pdf_reuses_record_design_sheet_model() -> None:
    sheets = {
        sheet.name: sheet
        for sheet in extract_record_design_pdf(
            _RECORD_DESIGN_ROOT / "modelo_840" / "files" / "01-840-orden-hac-2572-2003-99-kb-pdf.pdf"
        )
    }
...
def test_modelo_190_record_design_pdf_extracts_narrative_type_one_and_two_records() -> None:
    sheets = {
        ...
        for sheet in extract_record_design_pdf(
            ...
            / "modelo_190"
            / "files"
            / "01-190-orden-eha-3127-2009-de-10-de-noviembre-actualizada-por-orden-hac-1431-2025-de-3-de-dicie.pdf"
        )
    }
...
def test_modelo_193_record_design_pdf_preserves_split_field_titles_across_lines() -> None:
    sheets = {
        ...
        for sheet in extract_record_design_pdf(
            ...
            / "modelo_193"
            / "files"
            / "01-193-orden-eha-3377-2011-actualizado-por-orden-hac-1430-2025-de-3-de-diciembre-357-kb-pdf.pdf"
        )
    }
    declarante = sheets["Tipo 1 - Registro De Declarante"]
...
def test_modelo_347_record_design_pdf_keeps_distinct_type_two_layouts() -> None:
    sheets = extract_record_design_pdf(
        _RECORD_DESIGN_ROOT
        / "modelo_347"
        / "files"
...
def test_modelo_347_legacy_positional_chart_pdf_extracts_reviewable_record_data() -> None:
    sheets = extract_record_design_pdf(
        _RECORD_DESIGN_ROOT / "modelo_347" / "files" / "02-347-ejercicio-2008-y-2009-30-kb-pdf.pdf"
    )
    ...
    assert len(declarante.fields) == 18
    assert declarante.fields[15].offset == 391
    assert declarante.fields[15].description == "N.I.F. DEL REPRESENTANTE LEGAL."
    assert declarado.fields[4].description == "N.I.F. DEL DECLARADO."
    assert declarado.fields[-1].offset == 130
    assert declarado.fields[-1].length == 371
    assert inmueble.fields[11].description == "REFERENCIA CATASTRAL."
    assert all(
        field.content == "Extracted from legacy Modelo 347 positional ruler chart."
        for sheet in sheets
        for field in sheet.fields
    )


def test_target_pdf_record_design_corpus_is_parseable() -> None:
    pdfs = [
        path
        for modelo in ("modelo_100", "modelo_190", "modelo_193", "modelo_347", "modelo_840")
        for path in sorted((_RECORD_DESIGN_ROOT / modelo / "files").glob("*.pdf"))
    ]

    parsed = {path.name: extract_record_design_pdf(path) for path in pdfs}

    assert len(parsed) == 22
    assert all(parsed.values())
    assert sum(len(sheet.fields) for sheets in parsed.values() for sheet in sheets) >= 11_700
