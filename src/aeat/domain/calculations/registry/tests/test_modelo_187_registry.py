"""Tests for the committed Modelo 187 registry foundation."""
...
def _load_modelo_187():
    return _committed_modelo("187")


def test_modelo_187_validator_accepts_committed_definition() -> None:
    modelo, catalogues = _load_modelo_187()
    assert modelo.id == "187"
    assert modelo.revisions, "187 must declare at least one revision"
    RegistryValidator(catalogues, source_root=bundled_path()).validate_modelo(modelo)


def test_modelo_187_formula_owned_by_construct() -> None:
    modelo, _ = _load_modelo_187()
    revision = modelo.revisions["2019-y-siguientes"]
    owned = set().union(*(set(c.formulas) for c in revision.constructs))
    assert "modelo-187-total" in owned


def test_modelo_187_total_copies_source_casilla() -> None:
