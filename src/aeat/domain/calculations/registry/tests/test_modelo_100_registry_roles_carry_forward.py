...
@pytest.mark.parametrize("filing_year", [2022, 2024, 2025])
def test_modelo_100_carry_forward_remaining_regional_rows_use_regional_roles(filing_year: int) -> None:
    revision = _modelo_100_snapshot(filing_year).revision
    expected_rows = _CARRY_FORWARD_REMAINING_REGIONAL_ROWS[filing_year]
    casillas_by_id = {casilla.id: casilla for casilla in revision.casillas if casilla.id in expected_rows}
    legacy_rows = [
        casilla.id
        for casilla in casillas_by_id.values()
