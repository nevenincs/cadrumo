...
def test_official_casilla_44_grounds_in_prorrata_regularizacion_chapter(year: int) -> None:
    ...
    casilla = _m303_casilla(year, "44")
    assert casilla is not None, f"M303 {year} must declare casilla 44"
    refs = set(casilla.legal_refs)
    missing = _PRORRATA_REGULARIZACION_CHAPTER - refs
    assert not missing, (
        f"M303 {year}: casilla 44 (regularización prorrata por porcentaje "
...
@pytest.mark.parametrize("year", [2020, 2024])
def test_official_casilla_44_does_not_stand_on_generic_framework_alone(year: int) -> None:
    ...
    assert _PRORRATA_REGULARIZACION_CHAPTER & refs, (
        f"M303 {year}: casilla 44 cites only the generic deduction framework "
