...
def test_runtime_rejects_casilla_number_alias_for_semantic_input() -> None:
    snapshot = resources().modelos.authority.snapshot("303", filing_year=2025, period="1T")
    casilla = next(casilla for casilla in snapshot.revision.casillas if casilla.id == _M303_SEMANTIC_INPUT_CASILLA)
    assert casilla.number != casilla.id
...
def test_metadata_alias_targets_include_export_refs_without_accepting_them_as_ids() -> None:
    snapshot = resources().modelos.authority.snapshot("303", filing_year=2025, period="1T")
    casilla = next(
    ...
    assert casilla_metadata_alias_targets(snapshot.revision, export_ref) == (casilla.id,)
