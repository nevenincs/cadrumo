...
now skips that disk pickle under pytest -- ``_registry_disk_cache_enabled()`` is
...
from .._loader import _registry_disk_cache_enabled
...
def test_registry_disk_cache_disabled_under_pytest() -> None:
    ...
    assert _registry_disk_cache_enabled() is False


def test_registry_disk_cache_enabled_without_pytest_markers(
    ...
    assert _registry_disk_cache_enabled() is True
