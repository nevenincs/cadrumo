...
def test_validator_accepts_committed_definition(mid, rev, approval, plazo, doc) -> None:
...
@pytest.mark.parametrize("mid,rev,approval,plazo,doc", _MODELOS)
def test_approval_and_plazo_resolve_as_legal_authority(mid, rev, approval, plazo, doc) -> None:
...
@pytest.mark.parametrize("mid,rev,approval,plazo,doc", _MODELOS)
def test_on_demand_solicitud_carries_no_fabricated_windows(mid, rev, approval, plazo, doc) -> None:
