"""Engine-side keying-invariant gate (per dual-keying ADR D1/D2).

The dual-keying ADR's contract is **engine-side**, not registry-data-side.
...
``^.*$`` — and even if it could, the ADR doesn't actually constrain the
``number`` field's shape. The ADR constrains the **engine's canonical-key
...
the ADR ratifies). Display-side references to ``casilla.number`` in error
...
modules fails this gate immediately. That's the actual #127 ADR contract
made executable.

See `.vault/adr/2026-06-01-m303-form-vs-semantic-casilla-dual-keying-adr.md`
(D1/D2/D4) for the convention rationale. A follow-up ADR amendment
...
# calculate / verify / parse pipeline the ADR's D1/D2 contract names.
...
# state on ``.number`` would invert the ADR's contract. Any production
...
def test_engine_formula_runtime_keys_canonical_lookup_on_casilla_id() -> None:
    ...
    lookup dict ``casillas_by_number`` that would invert the ADR contract.
    """
    ...
    assert _FORBIDDEN_LOOKUP_TABLE not in src, (
        ...
        f"must consult casilla.id, never casilla.number (per #127 ADR D1/D2)"
    )
    assert _CANONICAL_LOOKUP_TABLE in src, (
        ...
        f"updating this gate (per #127 ADR D1/D2)"
    )
...
def test_verifier_keys_canonical_lookup_on_casilla_id() -> None:
    ...
    the dual-keying ADR's Finding C). It must never gain a parallel
    ...
    assert _FORBIDDEN_LOOKUP_TABLE not in src, (
        ...
        f"(per #127 ADR D1/D2 and Finding C)"
    )
