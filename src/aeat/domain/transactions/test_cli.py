...
def _load_catalogue() -> TransactionCatalogue:
    """Read the persisted catalogue via the repository."""
    return TransactionCatalogueRepository().load()


def _save_catalogue(catalogue: TransactionCatalogue) -> None:
...
def _write_ndjson_from_provider(tmp_path: Path, fixture_name: str, *, ofx: bool = False) -> tuple[Path, list[Decimal]]:
    ...
    fixture = FIXTURES_DIR / "financial" / fixture_name
...
def test_financial_txs_build_accepts_direct_csv_source(tmp_path: Path) -> None:
    ...
    fixture = FIXTURES_DIR / "financial" / "synthetic-transactions.csv"
...
def _register_mixed_home_office_provider() -> str:
    ...
    from aeat.domain.transactions import LLMClassificationResponse, register_classifier
...
def test_classify_llm_applies_default_ratio_from_category_profile(tmp_path: Path) -> None:
    ...
    """
    from aeat.domain.transactions import unregister_classifier
...
def test_classify_llm_category_hint_forces_category(tmp_path: Path) -> None:
    ...
    from aeat.domain.categories import SpendingCategory
    from aeat.domain.transactions import LLMClassificationResponse, register_classifier, unregister_classifier
