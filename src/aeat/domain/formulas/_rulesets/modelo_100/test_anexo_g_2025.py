"""Unit tests for Modelo 100 Anexo G — cuotas + tarifas (ejercicio 2025).

Exercises the cuota chain (tarifa estatal general + tarifa estatal del
ahorro + Ceuta/Melilla 60 % deduction) of
:data:`aeat.domain.formulas._rulesets.MODELO_100_2025` against worked
...
class TestProgressiveTarifaEstatalGeneral:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (Decimal("12450.00"), Decimal("1182.75")),
    ...
    def test_tarifa_blg_progressive_anchors(
        ...
        # For lower rendimientos, the cap-protected reducción reduces BLG;
        # we drive the test only at BLG values >= 19747.50 to avoid this.
        if blg < Decimal("19747.50") and blg > Decimal("0"):
            pytest.skip("Skipping BLG below 19747.50 — art. 20 reducción active")
        inputs["0001"] = blg
        derived = _derived(MODELO_100_2025, inputs)
...
class TestProgressiveTarifaAhorro2025:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (Decimal("500000.00"), Decimal("65940.00")),  # post Ley 7/2024 top 15%
        ],
    )
    def test_tarifa_ahorro_progressive_anchors(
        ...
        """Verify 0560 against the 5-bracket post-Ley-7/2024 ahorro scale."""
        inputs = _zero_inputs(MODELO_100_2025)
