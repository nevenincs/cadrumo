...
from .._formula import FormulaDefinition
...
# in `_spans_overlap` when that ruleset registers.
...
_CITATIONS = (
    ...
    make_citation(
        ...
        # every other citation across the ruleset fleet.
...
def _make_casillas(legal_basis: tuple[LegalCitation, ...]) -> tuple[CasillaDefinition, ...]:
    ...
    return (
        ...
        *_computed("00562", _label("Cuota integra", "Gross tax amount", "Bruttó adóösszeg"), legal_basis),
        *_input(
            "00582",
            _label(
        ...
        *_computed("00611", _label("Cuota diferencial", "Tax differential", "Adókülönbözet"), legal_basis),
        *_computed(
            "00621",
...
def _make_formulas(year: int) -> tuple[FormulaDefinition, ...]:
    ...
    return (
        ...
        formula(
            casilla_id="00621",
            formula_id=f"modelo_200.{year}.liquido_a_ingresar",
