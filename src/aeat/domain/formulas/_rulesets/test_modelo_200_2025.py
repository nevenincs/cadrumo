"""Unit tests for the Modelo 200 2025 ruleset."""

from __future__ import annotations

from datetime import date
from decimal import Decimal

import pytest

from .._engine import Engine
from . import MODELO_200_2024, MODELO_200_2025

pytestmark = [pytest.mark.unit, pytest.mark.domain_model]


def _provided(
    *,
    base: Decimal,
    rate: Decimal,
    cuota_liquida: Decimal,
    retenciones: Decimal = Decimal("0.00"),
    p1: Decimal = Decimal("0.00"),
    p2: Decimal = Decimal("0.00"),
    p3: Decimal = Decimal("0.00"),
    abono: Decimal = Decimal("0.00"),
    incremento: Decimal = Decimal("0.00"),
) -> dict[str, Decimal]:
    cuota_integra = (base * rate / Decimal("100")).quantize(Decimal("0.01"))
    cuota_diferencial = cuota_liquida - retenciones - p1 - p2 - p3
    liquido = cuota_diferencial + incremento - abono
    return {
        "00547": Decimal("0.00"),
        "00550": base,
        "00552": base,
        "00558": rate,
        "00560": cuota_integra,
        "00562": cuota_integra,
        "00582": Decimal("0.00"),
        "00592": cuota_liquida,
        "00599": retenciones,
        "00601": p1,
        "00603": p2,
        "00605": p3,
        "00615": abono,
        "00619": incremento,
        "00611": cuota_diferencial,
        "00621": liquido,
    }


class TestModelo200Ruleset2025:
    def test_ruleset_id_and_effective_range(self) -> None:
        assert MODELO_200_2025.ruleset_id == "modelo_200.2025"
        assert MODELO_200_2025.effective_from == date(2025, 1, 1)
        assert MODELO_200_2025.effective_to == date(2025, 12, 31)

    def test_2025_no_drift_from_2024_for_page_14_arithmetic(self) -> None:
        provided = _provided(
            base=Decimal("750000.00"),
            rate=Decimal("25.00"),
            cuota_liquida=Decimal("187500.00"),
            retenciones=Decimal("7500.00"),
            p1=Decimal("40000.00"),
            p2=Decimal("40000.00"),
            p3=Decimal("40000.00"),
        )
        report_2024 = Engine().audit_against(ruleset=MODELO_200_2024, provided=provided, tolerance=Decimal("0.01"))
        report_2025 = Engine().audit_against(ruleset=MODELO_200_2025, provided=provided, tolerance=Decimal("0.01"))
        assert {e.casilla_id: e.value for e in report_2024.ledger.entries} == {
            e.casilla_id: e.value for e in report_2025.ledger.entries
        }

    def test_formula_ids_use_2025_namespace(self) -> None:
        formula_00562 = MODELO_200_2025.formula_for("00562")
        formula_00611 = MODELO_200_2025.formula_for("00611")
        formula_00621 = MODELO_200_2025.formula_for("00621")
        assert formula_00562 is not None
        assert formula_00611 is not None
        assert formula_00621 is not None
        assert formula_00562.formula_id == "modelo_200.2025.cuota_integra"
        assert formula_00611.formula_id == "modelo_200.2025.cuota_diferencial"
        assert formula_00621.formula_id == "modelo_200.2025.liquido_a_ingresar"


@pytest.mark.parametrize(
    ("base", "rate", "label"),
    [
        (Decimal("1000000.00"), Decimal("25.00"), "general"),
        (Decimal("200000.00"), Decimal("24.00"), "reduced-size-transitional"),
        (Decimal("100000.00"), Decimal("15.00"), "startup"),
        (Decimal("50000.00"), Decimal("21.00"), "microempresa-2025-first-band"),
    ],
)
def test_tax_rate_split_examples_2025(base: Decimal, rate: Decimal, label: str) -> None:
    """LIS art. 29 rate examples are verified through casilla 00558."""
    del label
    provided = _provided(
        base=base,
        rate=rate,
        cuota_liquida=(base * rate / Decimal("100")).quantize(Decimal("0.01")),
    )
    report = Engine().audit_against(ruleset=MODELO_200_2025, provided=provided, tolerance=Decimal("0.01"))
    assert report.is_clean(), [(d.casilla_id, d.computed_value, d.user_value) for d in report.discrepancies]


@pytest.mark.parametrize(
    ("provided", "expected"),
    [
        (
            _provided(
                base=Decimal("100000.00"),
                rate=Decimal("25.00"),
                cuota_liquida=Decimal("25000.00"),
            ),
            {"00562": Decimal("25000.00"), "00611": Decimal("25000.00"), "00621": Decimal("25000.00")},
        ),
        (
            _provided(
                base=Decimal("50000.00"),
                rate=Decimal("21.00"),
                cuota_liquida=Decimal("10500.00"),
                retenciones=Decimal("500.00"),
                p1=Decimal("1000.00"),
                p2=Decimal("1000.00"),
                p3=Decimal("1000.00"),
            ),
            {"00562": Decimal("10500.00"), "00611": Decimal("7000.00"), "00621": Decimal("7000.00")},
        ),
        (
            _provided(
                base=Decimal("125000.00"),
                rate=Decimal("15.00"),
                cuota_liquida=Decimal("18750.00"),
                abono=Decimal("250.00"),
                incremento=Decimal("100.00"),
            ),
            {"00562": Decimal("18750.00"), "00611": Decimal("18750.00"), "00621": Decimal("18600.00")},
        ),
    ],
)
def test_computed_casillas_are_cent_exact_2025(
    provided: dict[str, Decimal],
    expected: dict[str, Decimal],
) -> None:
    """Three anchored cases exercise every computed M200 casilla."""
    report = Engine().audit_against(ruleset=MODELO_200_2025, provided=provided, tolerance=Decimal("0.01"))
    assert report.is_clean(), [(d.casilla_id, d.computed_value, d.user_value) for d in report.discrepancies]
    ledger = {entry.casilla_id: entry.value for entry in report.ledger.entries}
    for casilla_id, value in expected.items():
        assert ledger[casilla_id] == value
