...
class TestCatalogueLoad:
    def test_default_catalogue_loads_from_registry(self, catalogue) -> None:
    ...
    def test_catalogue_codes_are_uppercase(self, catalogue) -> None:
        for scope in catalogue.scopes:
            assert scope.code == scope.code.upper()

    def test_catalogue_iva_scope_binds_modelo_303_390(self, catalogue) -> None:
...
class TestAllTokenExpansion:
    def test_all_token_expands_to_every_code(self, catalogue) -> None:
    ...
    def test_parse_with_all_expands_in_place(self, catalogue) -> None:
        result = parse_scope_tokens((ALL_TOKEN,), catalogue)
        assert set(result) == catalogue.code_set()

    def test_parse_with_all_plus_extras_dedups(self, catalogue) -> None:
