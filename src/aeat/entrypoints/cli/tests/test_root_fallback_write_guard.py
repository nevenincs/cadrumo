...
_GUARDED_PREDICATE_PATHS: tuple[str, ...] = (
    ...
    "app modelo reconcile work --from-justificante justificante.pdf",
    "app modelo reconcile-from-justificante justificante.pdf work",
...
def test_profile_switch_remains_recovery_path_on_root_fallback_database(tmp_path: Path) -> None:
    """`profile switch` reaches profile resolution instead of the root-fallback guard."""

    result = _run_aeat(tmp_path, ("config", "profile", "switch", "does-not-exist"))
...
def test_root_fallback_guard_predicate_covers_profile_bound_mutations(verb_path: str) -> None:
    """The central guard covers known mutation surfaces discovered during S09 review."""
