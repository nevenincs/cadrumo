"""Regression tests for Modelo 600 Path-B refusal stub.

``aeat app modelo work create --modelo 600`` must return a legally-grounded
refusal payload rather than a generic crash or a "Modelo desconocido 600"
error.

Modelo 600 is the self-assessment for the Impuesto sobre Transmisiones
Patrimoniales y Actos Jurídicos Documentados (ITP-AJD) on property
transfers under Ley 28/1990 (ITPyAJD).  The tax is cedido (devolved) to
the Autonomous Communities; the autoliquidación must be filed at the Hacienda
of the CCAA where the transferred asset is located.  The national AEAT CLI
cannot route this filing to the correct autonomic authority.
...
def test_work_create_600_refuses_with_legal_authority_message(
    _isolated_cli_backend: Path,
) -> None:
    """work create --modelo 600 is refused with a legally-grounded message
    ...
    ``Modelo desconocido 600``.  The refusal message must cite the governing
    ...
    ("Create stub modelo 600 refused"), so the test fails if the locale key
    ...
    # Must redirect to the autonomic Hacienda (CCAA), not AEAT Sede.
...
def test_work_create_600_refusal_fires_before_profile_check(
    _isolated_cli_backend: Path,
) -> None:
    """The stub guard fires before the active-profile requirement.

    An operator without an active profile still gets the M600 refusal
