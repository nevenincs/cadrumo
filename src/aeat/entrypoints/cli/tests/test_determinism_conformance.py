...
def _committed_db_fingerprint(profile: TestRuntimeProfile) -> str:
    ...
    cannot be reliably removed in-process on Windows (its mmap handle outlives
    ``engine.dispose()``); this is why the whole-tree ``compute_db_sha256`` — sound
    at a between-process replay boundary where no connection is open — is not used
    for this in-session no-op proof.
    """
