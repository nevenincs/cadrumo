...
The generator is importable production code and is therefore subject to the
same Google-style docstring gates as any other module.  It is also the
``SCHEMA_REGISTRY`` consumer anticipated by :mod:`aeat.core.json_contract`.
...
def collect_live_leaf_paths_in_subprocess() -> list[str]:
    ...
    code = textwrap.dedent(
        ...
        from aeat.entrypoints.cli._doc_reference import (
            _force_lazy_imports,
            _collect_leaf_paths,
