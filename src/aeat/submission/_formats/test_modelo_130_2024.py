...
class TestModelo1302024Shape:
    def test_total_record_length(self) -> None:
        # Field content is 879 bytes (positions 1-879). The on-wire
        # stream adds a 2-byte CRLF terminator handled by the serialiser.
        assert RECORD_LENGTH == 879
    ...
    def test_header_block(self) -> None:
        """MODELO literal + EJERCICIO + PERIODO + NIF + APELLIDOS_NOMBRE
        + TIPO_DECLARACION span positions 1-79."""
        ...
        assert by_id["EJERCICIO"].offset == 4
        assert by_id["PERIODO"].offset == 8
        assert by_id["NIF_DECLARANTE"].offset == 10
        assert by_id["NIF_DECLARANTE"].length == 9
        assert by_id["TIPO_DECLARACION"].offset == 79
