...
# ── Happy path + explicit dry-run contract ──────────────────────────────
...
class TestHappyPath:
    ...
    def test_explicit_dry_run_propagates(self) -> None:
        """Explicit dry-run must call submit_draft with dry_run=True."""
        fx = _fixtures()
...
class TestAbortReasons:
    ...
    def test_live_submit_propagates_explicit_false(self) -> None:
        """Live mode is forwarded to the submission engine as explicit dry_run=False."""
        fx = _fixtures()
