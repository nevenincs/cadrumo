...
- Single-record schemas (Modelo 130): ``ENCODING``, ``RECORD_LENGTH``,
  ``RECORD_SPECS``, ``REQUIRED_HEADER_FIELDS``.
- Envelope schemas (Modelo 303, future 390): ``ENCODING``, ``ENVELOPE``,
  ``REQUIRED_HEADER_FIELDS``.

A generator change that smuggles in an extra top-level symbol
(debug helper, cache, side-effect callable) would make the schema
module's import behaviour unpredictable and would leak the
underscore-prefix/internal convention. This test pins the surface
so that kind of drift fails at collection.

Implementation: we read each module's ``__all__`` and its non-
underscore-prefixed globals; both must match the canonical set
exactly. The ``_SEGMENT_*`` constants the generator emits are
underscore-prefixed by design so they don't pollute the public
surface — they are internal building blocks for ``ENVELOPE``.
"""
...
def _authored_public_names(module: ModuleType) -> frozenset[str]:
    ...
    ``_record_spec`` the module uses as building blocks, (c) the
