...
Both the local-filesystem and Google Drive backends persist a
``content_hash`` (optionally ``sha256-``-prefixed) alongside each object and
re-check it on read. This module centralises the prefix-stripping convention
and the compare-and-raise kernel so the two backends share one integrity
contract; each backend keeps its own message, context, and verification policy.
"""
...
def verify_content_hash(
    ...
    *,
    ...
    full 64-character hex string (the Drive backend's policy); without it any
    non-empty stripped digest is compared against ``actual_hash`` (the local
    backend's policy). ``actual_hash`` is supplied by the caller because the
    local backend reuses it to stamp the written sidecar.
    """
    if not stored_hash:
        return
