...
:class:`~aeat.application.storage.calc_sheets.SheetExportPlan` facets into A1
...
def _coerce_cell_value(value: Decimal | str | bool | None) -> object:
    """Convert a :class:`~aeat.application.storage.calc_sheets.SheetValueCell` value.
...
def _build_value_data(value_cells: Iterable[SheetValueCell]) -> list[dict[str, Any]]:
    """Build ``values.batchUpdate`` entries for :class:`~aeat.application.storage.calc_sheets.SheetValueCell`."""
...
def _build_formula_data(formula_cells: Iterable[SheetFormulaCell]) -> list[dict[str, Any]]:
    """Build entries for :class:`~aeat.application.storage.calc_sheets.SheetFormulaCell` records.
...
def _build_row_set_header_data(row_sets: Iterable[SheetRowSet]) -> list[dict[str, Any]]:
    """Emit Detalle-tab header cells for :class:`~aeat.application.storage.calc_sheets.SheetRowSet`."""
    data: list[dict[str, Any]] = []
