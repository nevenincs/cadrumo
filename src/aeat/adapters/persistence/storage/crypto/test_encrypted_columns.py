...
class TestEncryptedString:
    ...
    def test_round_trip_ascii(self, session: Session) -> None:
        row = _CryptoRow(secret_text="hello world")  # noqa: S106 — fixture literal, not a credential
        session.add(row)
        session.commit()
        session.expire_all()
        loaded = session.execute(select(_CryptoRow)).scalar_one()
        assert loaded.secret_text == "hello world"  # noqa: S105 — fixture literal, not a credential
...
class TestCrossTypeReplayPrevention:
    ...
    def test_string_ciphertext_does_not_decrypt_as_bytes(
        ...
        session.add(_CryptoRow(secret_text="payload"))  # noqa: S106 — fixture literal, not a credential
        session.commit()
        with engine.connect() as conn:
