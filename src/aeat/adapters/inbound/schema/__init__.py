"""BOE-PDF extraction adapter for AEAT modelo schemas.
...
- :class:`BoeOrdenExtractor` — line-based PDF extractor bound to a
  :class:`~aeat.domain.schema.FetchedSchemaSource`.

Consumers MUST import from this module only; this is an inbound adapter
and its implementation details are internal.

Non-goals:

- The intermediate-representation (IR) types (:class:`~aeat.domain.schema.Modelo`,
  :class:`~aeat.domain.schema.Casilla`, etc.) live in
  :mod:`aeat.domain.schema` — this adapter imports from there.
- The cache helpers (:func:`~aeat.domain.schema.save_modelo_to_cache`,
  :func:`~aeat.domain.schema.load_modelo_from_cache`) remain in
  :mod:`aeat.domain.schema`.
"""
...
from ....domain.schema._fetch import FetchedSchemaSource
