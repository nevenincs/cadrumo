...
treating each cuota íntegra component as a literal the operator fills
in. The *real* computation of the cuota íntegra goes through Spain's
progressive IRPF tax scale — the **tarifa progresiva** — applied
separately to the base liquidable general and the base liquidable del
ahorro, split further into state and autonomous halves.

This module implements the state halves (common to every operator
regardless of CCAA) as a piecewise-linear tax calculation, and exposes
:func:`validate_tarifa_estatal`, the post-validator that the verdict
