...
Delegates all discovery and drift logic to :class:`aeat.apidocs.ApiStubManager`
so this file is the single enforcement surface for module-to-stub correspondence.
Run ``python -m aeat.apidocs scaffold`` to regenerate stubs; run
``python -m aeat.apidocs scaffold --check`` for a zero-drift gate.
...
from aeat.apidocs import ApiStubManager
...
def test_every_source_module_has_a_stub() -> None:
    ...
    ``python -m aeat.apidocs scaffold`` to regenerate the stub tree.
    """
