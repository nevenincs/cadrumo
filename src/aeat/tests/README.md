...
| `domain_core`           | root modules, non-domain `cli`, `setup`, bundled `src/aeat/tests/*.py`             | `just test-domain infra`                         |
...
`src/aeat/tests/test_marker_integrity.py` walks every `test_*.py` and
`_test_*.py` module under `src/aeat/` via `ast` and fails
CI if any module lacks a compliant `pytestmark` assignment. The
collection hook (shared helper in `aeat.tests._marker_hook`, invoked from
both the repo-root `conftest.py` and `src/aeat/tests/conftest.py`) additionally
...
Constants live in `src/aeat/tests/conftest.py` as `LIVE_ACCESS_MARKERS`,
...
2. `src/aeat/tests/conftest.py` layers on a runtime check: when live items are
...
Test modules (`test_*.py`, `_test_*.py`, `src/aeat/tests/`) are omitted from
coverage.
