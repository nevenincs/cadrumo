...
3. Close the quarter's books BEFORE any calculation: hand off to
   `llevar-libro` to complete the quarter's imports, then `clasificar` for
...
4. Drive each surfaced obligation through its owning per-modelo skill
   (`preparar-modelo-303`, `preparar-modelo-130`, `preparar-modelo-111`,
...
5. Surface deadline pressure honestly. If the window is already tight, say
   ...
   this is no longer a quarter close — route to `regularizar-atrasos`,
   which owns the past-due and recargo path.
6. After each filing's local export, the taxpayer files in the AEAT portal
   and `reconciliar` pulls the official evidence. Track the quarter as done
...
- Any already-missed deadline was routed to `regularizar-atrasos`, not
