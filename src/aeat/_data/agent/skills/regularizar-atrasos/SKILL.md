...
4. Before any calculation, bring the ledger for each overdue period to
   clean: hand off to `llevar-libro` and then `clasificar`, scoped to that
...
5. Drive each overdue obligation through its owning per-modelo skill
   (`preparar-modelo-303`, `preparar-modelo-130`, and siblings) over the
...
6. Distinguish missing from wrong. This itinerary covers obligations that
   ...
   that needs correcting, route to `rectificar-declaracion` — the
...
7. After each local export the taxpayer files in the AEAT portal
   themselves, then hand off to `reconciliar`. The recargo AEAT actually
...
- No statement claims the matter is closed with AEAT before `reconciliar`
...
export, human files, `reconciliar`). This itinerary's job ends when the
