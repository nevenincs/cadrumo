...
- The year's four quarters are filed and reconciled — or their gaps are
  ...
  `regularizar-atrasos` BEFORE preparing the annual summary that folds it
  in.
...
3. Complete the year's ledger first: hand off to `llevar-libro` and
   `clasificar` for any tail of the year still unclassified. Annual totals
...
4. Prepare the January resúmenes before the annual declaraciones, each via
   ...
   `rectificar-declaracion`) or the ledger changed after a quarter was
...
6. After each filing's local export, the taxpayer files in the AEAT portal
   and `reconciliar` pulls the official evidence, exactly as in any other
   season.
...
- Unfiled quarters were routed to `regularizar-atrasos` before any annual
  summary folding them in was prepared.
- Corrections to already-filed quarters were routed to
  `rectificar-declaracion`, never improvised inside an annual preparation.
