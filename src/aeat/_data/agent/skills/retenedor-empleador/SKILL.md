...
simultaneously match `autonomo-estimacion-directa`, `autonomo-modulos`, or
`pyme-sociedad` on their entity-type/regime axis, and both itineraries' obligations
...
4. Sequence the itinerary:
   - The ledger must record every withheld payment with its retención category
     and rate before any calculation: hand off to `llevar-libro`, then
     `clasificar`.
   - The quarterly retenciones self-assessment(s) the calendar surfaces (e.g.
     ...
     demonstrated by `preparar-modelo-303`, substituting the retención modelo's
     own casillas.
