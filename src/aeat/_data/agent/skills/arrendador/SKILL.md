...
`autonomo-estimacion-directa` / `autonomo-modulos`; route both when both facts
...
- The taxpayer is onboarded (`alta-contribuyente` has run;
...
4. Sequence the itinerary:
   - Ledger records every rent receipt and deductible expense (IBI, comunidad,
     ...
     calculation: hand off to `llevar-libro`, then `clasificar`.
   - The annual Renta declaration (Modelo 100, when the calendar surfaces it):
     ...
     `preparar-modelo-130`, substituting Modelo 100's own casillas.
   - If the profile also declares actividad económica income, hand off to
     `autonomo-estimacion-directa` or `autonomo-modulos` for the quarterly
...
5. After the filing's local export, hand off to `reconciliar` once the taxpayer
...
this profile also matches), the reconciler (`reconciliar`) pulls the official
