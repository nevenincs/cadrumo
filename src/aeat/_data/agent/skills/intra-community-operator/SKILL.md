...
`autonomo-estimacion-directa`, `autonomo-modulos`, or `pyme-sociedad` on their
...
- The taxpayer's primary regime itinerary (`autonomo-estimacion-directa`,
  `autonomo-modulos`, or `pyme-sociedad`) is already sequencing the domestic IVA
...
4. Sequence the itinerary:
   - Ensure the ledger's intra-community transactions carry their
     ...
     to `llevar-libro`, then `clasificar`.
   - The recapitulative declaration (Modelo 349, when the calendar surfaces it):
     ...
     `preparar-modelo-303`, substituting Modelo 349's own casillas.
