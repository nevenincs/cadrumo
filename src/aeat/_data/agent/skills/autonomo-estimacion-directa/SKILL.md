---
name: autonomo-estimacion-directa
description: >-
...
(`autonomo-modulos` owns that predicate). This skill is thin: it sequences the
...
4. Sequence the itinerary by cadence, in the order the calendar returns them:
   - Ledger stays current before any calculation: hand off to `llevar-libro`,
     then `clasificar`.
...
`reconciliar` after the human files). This itinerary's job ends once every
