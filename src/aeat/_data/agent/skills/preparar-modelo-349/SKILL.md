...
- An active profile exists (`aeat app overview status` reports one) and
  ...
  `intra-community-operator` for the itinerary that routes here.
...
8. Cross-check with the periodic IVA declaration: the same intra-community
   ...
   yet prepared for this period, hand off to `preparar-modelo-303` (or the
   `intra-community-operator` itinerary that sequences both).
