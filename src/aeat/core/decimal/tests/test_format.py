...
def test_format_decimal_plain() -> None:
    for value, expected in (
        ...
        assert format_decimal(value) == expected, repr(value)
...
def test_format_decimal_normalize() -> None:
    for value, expected in (
        (Decimal("12.34"), "12.34"),
        (Decimal("-12.34"), "-12.34"),
        (Decimal("0"), "0"),
        ...
        assert format_decimal(value, normalize=True) == expected, repr(value)
...
def test_format_decimal_none_value_policy() -> None:
    for value, none_value, expected in (
        (None, "0", "0"),
        (None, "", ""),
        (None, "N/A", "N/A"),
        ...
        assert format_decimal(value, none_value=none_value) == expected, repr(value)
...
def test_format_decimal_normalize_with_none_policy() -> None:
    ...
    for value, expected in (
        ...
        assert format_decimal(value, normalize=True, none_value="0") == expected, repr(value)
...
def test_format_decimal_censo_caller_pattern() -> None:
    ...
    """
    for value, expected_after_caller_patch in (
        ...
        assert result == expected_after_caller_patch, repr(value)
