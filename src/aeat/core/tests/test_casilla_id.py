...
@pytest.mark.parametrize(
    ("value", "error_match"),
    (
        pytest.param(1, r"example 1 is not a canonical casilla\.id", id="integer"),
        pytest.param(b"01", r"example b'01' is not a canonical casilla\.id", id="bytes"),
    ),
)
def test_validated_casilla_id_rejects_non_string_values_without_coercion(
    value: object,
    error_match: str,
) -> None:
