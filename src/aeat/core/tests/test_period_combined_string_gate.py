...
ALLOWLIST: tuple[AllowlistRule, ...] = (
    ...
    AllowlistRule(
        path=_path(
            r"^src/aeat/adapters/inbound/declaracion/tests/test_parser_boundary_m131_current_year\.py$"
        ),
        reason="parser boundary test preserves the committed synthetic justificante filename",
    ),
    AllowlistRule(
        path=_path(
            r"^src/aeat/application/(?:calculations/tests/test_prorrata_regularizacion|"
            r"modelo/tests/test_(?:prorrata_regularizacion_source_timing|review_package(?:_collab_audit|_counter_sign|_feedback|_recipient_encryption|_review_only_workspace|_signing)? )|"
            r"overview/tests/test_data_prep)|entrypoints/cli/tests/test_modelo_review_package_verb\.py$".replace("? )", "?)")
        ),
