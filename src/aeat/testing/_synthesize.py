...
a synthetic ``FilingDraft`` whose status is :class:`APPROVED`.

This helper never reaches AEAT — it operates entirely on local
fixture bytes plus pure construction. The five-layer write guard
remains intact.

Public API:

    from aeat.testing import synthesize_filing_draft
    ...
    assert draft.status == FilingDraftStatus.APPROVED
"""
