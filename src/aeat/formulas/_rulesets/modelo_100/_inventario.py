...
LIFO (last-in, first-out) is **not admissible** for tax purposes per
LIS art. 17.6 — the closed enum below makes this constraint structural
...
class ValuationMethod(StrEnum):
    ...
    17.6. Any caller attempting to construct an ``InventoryRecord`` for
    LIFO will hit a Pydantic validation failure at the field level.
    """
...
class InventoryRecord(BaseModel):
    ...
    The variación de existencias (final - initial) feeds the
    rendimiento neto computation in Anexo D normal. The selected
    `method` is informational at this layer — the formula DSL operates
    on the resulting Decimal values supplied by the caller, not on the
    method itself; the closed enum exists to document and enforce the
    LIS art. 17 admissibility constraint at construction time.
    """
