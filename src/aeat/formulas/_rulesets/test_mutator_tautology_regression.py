...
def test_old_killed_equals_populated_pattern_is_unreachable() -> None:
    ...
    :class:`ast.Assign` *and* :class:`ast.AnnAssign` node looking for
    a single-target assignment of the literal name ``populated`` to
    the literal name ``killed``. The annotated form
    (``killed: int = populated``) was added to the walker after a
    gemini-code-assist review pointed out that the bare-Assign-only
    walker would let typed code re-introduce the tautology silently.
    The aggregator now derives ``killed`` empirically from the
    per-class harness coverage generators; reintroducing the literal
    assignment in either form would re-enable the failure mode that
    issue #457 fixed. AST-based detection ignores docstrings and
