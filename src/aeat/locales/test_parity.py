...
import yaml
from aeat.locales.manager import LocaleManager, StrictUniqueKeyLoader
...
def test_codebase_to_locale_parity(locales_state):
    ...
    codebase_keys, locale_keys_map, files = locales_state
...
def test_inter_locale_parity(locales_state):
    ...
    for name, keys in locale_keys_map.items():
        ...
        if missing or extra:
            ...
            if missing: msg += f" Missing {len(missing)} keys."
            if extra: msg += f" Has {len(extra)} extra keys."
            errors.append(msg)
