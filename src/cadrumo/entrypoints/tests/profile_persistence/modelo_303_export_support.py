...
def external_filing_observations(*, casilla_values, snapshot):
    ...
    This is test-fixture composition: the registry remains the authority for
    legal/source references, while the adapter tests construct the persisted
    external baseline without reaching into a private application helper.
    """
    definitions = casillas_by_id(snapshot.revision)
    observations = []
    for casilla_id, value in casilla_values.items():
        definition = definitions.get(casilla_id)
        if definition is None:
            raise ValueError(f"fixture casilla {casilla_id!r} is absent from the registry snapshot")
        observations.append(
            CasillaObservation(
...
def _m303_snapshot_ref(period: str) -> RegistrySnapshotRef:
    return published_authority_operation().snapshot("303", filing_year=2026, period=period).snapshot_ref
