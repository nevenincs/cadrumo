...
def test_lazy_create_help_declares_exactly_one_canonical_machine_secret_option_pair() -> None:
    ...
    # Recovery is enrolled after creation by its own verb; create carries no handoff pair.
    assert "--recovery-handoff-fd" not in help_result.output
    assert "--recovery-verification-fd" not in help_result.output
    ...
    assert not any(
        parameter.name in {"recovery_handoff_fd", "recovery_verification_fd"} for parameter in schema.parameters
    )
    assert schema.recovery_handoff_contract is None
