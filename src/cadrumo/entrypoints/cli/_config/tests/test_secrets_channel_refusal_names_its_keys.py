...
def _refusal_for(payload: str, monkeypatch: pytest.MonkeyPatch) -> CliRefusedBoundaryError:
    """Feed ``payload`` to the real stdin reader and return its refusal."""
    stream = io.TextIOWrapper(io.BytesIO(payload.encode("utf-8")))
    monkeypatch.setattr(sys, "stdin", stream)
    with pytest.raises(CliRefusedBoundaryError) as caught:
        read_secrets_stdin(_ProbeSecrets)
    return caught.value


def test_a_malformed_payload_names_every_accepted_key(monkeypatch: pytest.MonkeyPatch) -> None:
    """DISCRIMINATING: the refusal an unpromptable caller has to act on."""
    refusal = _refusal_for("this is not json", monkeypatch)

    context = refusal.context or {}
    assert context.get("expected_fields") == "password, confirmation"


def test_a_wrong_shaped_object_names_every_accepted_key(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    refusal = _refusal_for('{"passphrase": "x"}', monkeypatch)

    context = refusal.context or {}
    assert context.get("expected_fields") == "password, confirmation"


def test_the_refusal_never_carries_a_supplied_value(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    refusal = _refusal_for(f'{{"password": "{sent_value}", "surplus": "x"}}', monkeypatch)
