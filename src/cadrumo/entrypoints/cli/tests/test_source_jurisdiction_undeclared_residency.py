...
contract: ``_resolve_source_jurisdiction`` defaults ``--source-jurisdiction`` to
...
def test_an_undeclared_residency_resolves_to_none_not_es() -> None:
    ...
    resolved = _resolve_source_jurisdiction(None, fiscal_residency=None, irpf_special_regime=None)
...
def test_a_declared_resident_still_defaults_to_es() -> None:
    ...
    """
    resolved = _resolve_source_jurisdiction(
        None,
...
def test_an_operator_supplied_value_wins_over_every_profile_signal() -> None:
    ...
    assert _resolve_source_jurisdiction("PT", fiscal_residency=None, irpf_special_regime=None) == "PT"
    assert (
