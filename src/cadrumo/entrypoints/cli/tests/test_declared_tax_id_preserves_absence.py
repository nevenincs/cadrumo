...
contract: ``_declared_tax_id`` returns the empty string for a profile that
...
def test_declared_tax_id_returns_the_declared_value() -> None:
    ...
    assert _declared_tax_id(record) == "12345678Z"
...
def test_projection_fabricates_where_the_declared_read_preserves_absence() -> None:
    ...
    declared = _declared_tax_id(empty)
...
def test_declared_tax_id_strips_surrounding_whitespace() -> None:
    ...
    assert _declared_tax_id(_record(UserProfileFact(path="identity.tax_id", value="   "))) == ""
    assert _declared_tax_id(_record(UserProfileFact(path="identity.tax_id", value=" 12345678Z "))) == "12345678Z"
