...
def test_an_operator_answer_reaches_the_ladder() -> None:
    ...
    """
    result = _confirm(_SUPPLIER_CIF, "--scope", IvaTerritorialScope.from_registry("es_canarias").value)

    assert result.exit_code == 0, result.output
    assert _ladder_scope() == IvaTerritorialScope.from_registry("es_canarias").value


def test_the_subject_is_positional() -> None:
    """A single-subject verb takes its subject as an argument, never as an option."""
    rejected = _confirm("--id", _SUPPLIER_CIF, "--scope", IvaTerritorialScope.from_registry("es_mainland").value)
...
def test_a_retry_is_a_no_op_that_says_so() -> None:
    ...
    """
    first = _confirm_json(_SUPPLIER_CIF, "--scope", IvaTerritorialScope.from_registry("eu_member").value)
    assert first.exit_code == 0, first.output
    assert _payload(first)["result"]["recorded"] is True

    again = _confirm_json(_SUPPLIER_CIF, "--scope", IvaTerritorialScope.from_registry("eu_member").value)
...
def test_withdrawing_reopens_the_question_so_a_correction_can_land() -> None:
    ...
    assert _confirm(_SUPPLIER_CIF, "--scope", IvaTerritorialScope.from_registry("es_mainland").value).exit_code == 0
    assert _ladder_scope() == IvaTerritorialScope.from_registry("es_mainland").value
...
def test_an_unverifiable_identifier_refuses_rather_than_storing_a_key_for_nothing() -> None:
    ...
    result = _confirm("not-a-tax-id", "--scope", IvaTerritorialScope.from_registry("es_mainland").value)
...
def test_an_identification_only_conflict_refuses_instead_of_raising() -> None:
    ...
    assert EUMemberState.from_registry("de").value in conflicted.output
    assert EUMemberState.from_registry("fr").value in conflicted.output
...
def test_an_identification_only_retry_reports_the_stored_answer() -> None:
    ...
    again = _confirm_json(_SUPPLIER_CIF, "--identification-state", EUMemberState.from_registry("de").value)
    ...
    assert notice["context"]["identification_state"] == EUMemberState.from_registry("de").value
...
def test_the_accepted_scopes_are_offered_on_a_parse_failure() -> None:
    ...
    assert result.exit_code != 0
    assert IvaTerritorialScope.from_registry("es_canarias").value in result.output
