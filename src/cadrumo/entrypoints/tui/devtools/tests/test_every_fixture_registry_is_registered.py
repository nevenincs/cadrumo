"""Gate: a declared fixture registry must be reachable from ``SURFACES``.
...
def declared_registries() -> dict[str, tuple[Any, ...]]:
    """Return every ``*_FIXTURES`` spec tuple the devtools package declares.
    ...
    """
    package = importlib.import_module("cadrumo.entrypoints.tui.devtools")
...
def test_the_package_still_declares_fixture_registries() -> None:
    ...
    assert len(registries) >= 2, (
        f"found only {len(registries)} fixture registry/registries in the devtools "
