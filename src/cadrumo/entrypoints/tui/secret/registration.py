...
_SURFACE_REFUSAL_KEYS: dict[str, str] = {
    "application.user_profile.errors.profile_already_exists": (
        "flows.registration.refusal.profile_already_exists"
    ),
}
...
class RecoveryOfferScreen(Screen[None]):
    ...
    BINDINGS: ClassVar = [Binding("escape", "skip_recovery", "", show=False)]
...
class RecoveryCodeScreen(Screen[None]):
    ...
    BINDINGS: ClassVar = [Binding("escape", "cancel_code", "", show=False)]
