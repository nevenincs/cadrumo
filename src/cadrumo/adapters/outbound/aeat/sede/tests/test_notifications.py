...
class TestNavigateAndParseLandingGuard:
    ...
    async def test_returns_rows_on_real_capture_with_warmup(self) -> None:
        ...
        async with opened_http_boundary() as boundary:
            ...
            assert [url for url in boundary.requested_urls if url == _SUMMARY_URL] == [_SUMMARY_URL]
            assert [url for url in boundary.requested_urls if url == _QUERY_URL] == [_QUERY_URL]
        assert snap.rows
