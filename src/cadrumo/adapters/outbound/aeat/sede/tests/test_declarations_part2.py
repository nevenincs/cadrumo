...
class TestSubmittedFileObservation:
    ...
    def test_modelo_130_redacted_submitted_file_matches_registry_calculation(self) -> None:
        with _indexed_authority_for_test().operation() as _authority_operation_for_test:
            ...
            binding = next(
                item
                for item in snapshot.revision.bindings
                if item.id == "irpf.previous_year_economic_activity_net_income"
            )
    ...
    def test_submitted_file_coverage_scores_fully_extracted_modelo_130_filing(self) -> None:
        with _indexed_authority_for_test().operation() as _authority_operation_for_test:
            ...
            coverage = _submitted_file_coverage_for_casillas(
                snapshot=snapshot, body=body, casillas=casillas, operation=_authority_operation_for_test
            )
...
class TestFiledObservationBindings:
    ...
    def test_numeric_casillas_enrol_while_non_numeric_ones_are_skipped(self) -> None:
        ...
        with _indexed_authority_for_test().operation() as _authority_operation_for_test:
            ...
            assert {
                skip.casilla_id
                for skip in non_numeric_observed_casillas(observation, operation=_authority_operation_for_test)
            } == {_M100_CASILLA_0180}
