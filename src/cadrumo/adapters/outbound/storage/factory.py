...
:class:`adapters.outbound.storage.protocol.StorageProvider` configured for the
...
def _parse_kind(raw: str) -> ProviderKind:
    ...
    if not cleaned:
        raise OutboundStorageValidationError(
            ...
            context={"value": raw},
            translated_message="adapters.outbound.storage._factory.errors.kind_empty",
    ...
    try:
        ...
        raise OutboundStorageValidationError(
            ...
            translated_message="adapters.outbound.storage._factory.errors.kind_unknown",
            precondition_verdict=_configuration_validation_verdict(
                "storage.factory.provider_kind.valid",
                field="cadrumo_storage_provider_kind",
            ),
...
def _build_oauth_desktop_credentials(*, profile: str) -> Credentials:
    ...
    if token is None:
        raise OutboundStorageValidationError(
            ...
            context={"profile": profile},
            translated_message="adapters.outbound.storage._factory.errors.google_token_missing",
            precondition_verdict=_configuration_validation_verdict(
    ...
    try:
        ...
        raise OutboundStorageError(
            ...
            translated_message="adapters.outbound.storage._factory.errors.google_auth_import_failed",
        ) from exc
    return Credentials(
...
def get_storage_provider(
    *,
    ...
    raise OutboundStorageValidationError(
        ...
        translated_message="adapters.outbound.storage._factory.errors.kind_unhandled",
    )
