...
class TestThePromptCopiesTheLegendAndNeverChoosesOne:
    ...
    def test_every_declared_phrase_reaches_the_compiled_prompt(self, *, operation: PinnedAuthorityOperation) -> None:
        text = build_invoice_extraction_prompt(period=default_extraction_period(), operation=operation).text
...
class TestTheLegendIsTranscribedLikeEveryOtherCopiedField:
    ...
    def test_the_field_is_declared_asked_for_and_anchored(self, *, operation: PinnedAuthorityOperation) -> None:
        text = build_invoice_extraction_prompt(period=default_extraction_period(), operation=operation).text
