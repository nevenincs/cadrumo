...
def test_open_round_trip_exposes_the_dek() -> None:
...
def test_close_zeroises_the_dek_buffer() -> None:
...
def test_reads_against_sealed_session_raise_bucket_locked() -> None:
    ...
    with pytest.raises(BucketLockedError) as dek_exc:
        _ = session.dek
    assert dek_exc.value.bucket_id == _BUCKET_ID
...
def test_two_sessions_do_not_alias_buffers() -> None:
    session_a = _open_session(bucket_id=_BUCKET_ID, dek=b"A" * 32)
    session_b = _open_session(bucket_id=_OTHER_BUCKET_ID, dek=b"B" * 32)

    buffer_a = session_a._dek_buffer
    buffer_b = session_b._dek_buffer
    assert id(buffer_a) != id(buffer_b)

    # Mutating one buffer does not bleed into the other.
    buffer_a[0] = 0xFF
    assert session_b.dek == b"B" * 32
...
def test_open_rejects_wrong_size_key_material() -> None:
