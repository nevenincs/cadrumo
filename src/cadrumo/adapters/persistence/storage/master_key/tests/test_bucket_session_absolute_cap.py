...
A `BucketSession` holds an immutable absolute-lifetime deadline supplied at
`open_resumed()`; `touch()` clamps the sliding idle deadline to it, and both
...
def _open(*, idle_minutes: int, absolute_minutes: int) -> BucketSession:
    return BucketSession.open_resumed(
        ...
        absolute_deadline=_NOW + timedelta(minutes=absolute_minutes),
    )
...
def test_an_idle_deadline_past_the_cap_is_refused() -> None:
    # The resumed record carries both deadlines, so an idle deadline wider than
    # the absolute cap is a malformed record rather than something to clamp.
    with pytest.raises(StorageValidationError, match="idle_deadline must not exceed absolute_deadline"):
        _open(idle_minutes=90, absolute_minutes=60)
