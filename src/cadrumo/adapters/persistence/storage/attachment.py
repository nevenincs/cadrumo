...
:class:`~domain.attachments.protocols.AttachmentStoreProtocol`. The
...
:class:`~adapters.persistence.storage.sql.secure_objects.SecureObjectRepository` persistence
substrate. Blob rows are framed byte payloads governed by
:data:`adapters.persistence.storage.secure_object_namespaces.ATTACHMENT_BLOB_NAMESPACE`; manifest
rows wrap :class:`Attachment` payloads in
:class:`~adapters.persistence.storage.envelope.contract.Envelope` records governed by
:data:`adapters.persistence.storage.secure_object_namespaces.ATTACHMENT_MANIFEST_NAMESPACE`.
...
class AttachmentStore(BaseModel):
    ...
    Implements :class:`~domain.attachments.protocols.AttachmentStoreProtocol`
    by storing raw document bytes under their SHA-256 digest in
    :data:`adapters.persistence.storage.secure_object_namespaces.ATTACHMENT_BLOB_NAMESPACE` and
