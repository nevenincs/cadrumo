...
bucket: a real encrypted IVA-history capability write, a real profile with
``identity.tax_id``, and a real sealed ``CalculationRevision`` in the work-unit /
revision catalogues.
"""
...
def _seed_ports() -> ModeloIvaWalletSeedPorts:
    """Bind the IVA-wallet ports locally to this adapter test's secure store."""
...
def _history_repository():
    """Expose the composed history capability without importing its adapter."""
...
def test_correction_overwrites_balance_and_emits_audit_event() -> None:
    ...
    persisted = _history_repository().load_period(Period.from_year_and_code(_SEED_YEAR, _SEED_PERIOD))
...
def test_correction_refused_when_same_period_sealed_303_consumed_the_seed() -> None:
    ...
    persisted = _history_repository().load_period(_SEED_FILING_PERIOD)
    assert persisted is not None
    assert persisted.available_end_amount == Decimal("500.00")
