...
def test_bulk_classify_columns_never_carry_evidence_fields() -> None:
    ...
    from cadrumo.application.ledger.models import BULK_CLASSIFY_ALLOWED_COLUMNS

    evidence_fields = frozenset({"purchase_invoice_evidence_id", "attachment_ids"})
    assert not (BULK_CLASSIFY_ALLOWED_COLUMNS & evidence_fields)
