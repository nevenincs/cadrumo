...
from cadrumo.application.filing.history_ports import FilingHistoryPersistenceError, FilingHistoryPorts
...
def _active_bucket_runtime(tmp_path: Path) -> Iterator[TestRuntimeProfile]:
    """Bind this adapter test to a fresh encrypted profile bucket."""
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id=_BUCKET_ID) as profile:
        yield profile


def _new_repository(runtime: TestRuntimeProfile) -> ModeloHistoryRepository:
    """Compose the application history facade with its real adapter port."""
    return ModeloHistoryRepository(
        ports=FilingHistoryPorts(
            repository=FilingHistoryRepositoryAdapter(objects=runtime.repository),
...
@pytest.fixture
def repo(_active_bucket_runtime: TestRuntimeProfile) -> ModeloHistoryRepository:
...
class TestSaveLoad:
    def test_round_trip(
        ...
        repo.save(history)
        loaded = _new_repository(_active_bucket_runtime).load("130")
