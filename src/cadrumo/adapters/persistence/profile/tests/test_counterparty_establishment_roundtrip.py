...
def test_establishment_fact_roundtrips_through_encrypted_storage(
    ...
    assert loaded.territorial_scope == IvaTerritorialScope.from_registry("es_canarias")
    ...
    assert loaded.territorial_scope != IvaTerritorialScope.from_registry("eu_member")
...
def test_persisted_fact_answering_neither_question_is_refused_at_load(
    ...
    assert stored["territorial_scope"] == IvaTerritorialScope.from_registry("es_canarias").value, (
    ...
    _rewrite(envelope)
    narrowed = CounterpartyEstablishmentRepository(objects=secure_objects).load(original.counterparty_key)
