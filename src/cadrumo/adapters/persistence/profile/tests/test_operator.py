...
def test_test_operator_auth_reports_the_active_profile() -> None:
    ...
    result = run_operator_auth_test(
        "certificate", operator_probe_ports=_OPERATOR_PROBE_PORTS, operator_scope_ports=_OPERATOR_SCOPE_PORTS
    )
...
def test_configure_operator_auth_refuses_when_no_active_profile_bucket(tmp_path: Path) -> None:
    ...
    from cadrumo.application.auth.operator_results import AuthConfigureNoActiveBucketError
...
def test_inspect_operator_auth_configured_is_false_without_certificate_path() -> None:
    ...
    result = inspect_operator_auth(
        operator_probe_ports=_OPERATOR_PROBE_PORTS, operator_scope_ports=_OPERATOR_SCOPE_PORTS
    )
...
def test_configure_operator_auth_certificate_with_file_is_complete(tmp_path: Path) -> None:
    ...
    result = configure_operator_auth(
        "certificate", certificate_path=cert_path, operator_scope_ports=_OPERATOR_SCOPE_PORTS
    )
...
def test_auth_test_probes_explicitly_requested_provider() -> None:
    ...
    probe = run_operator_auth_test(
        "clave_movil", operator_probe_ports=_OPERATOR_PROBE_PORTS, operator_scope_ports=_OPERATOR_SCOPE_PORTS
    )
...
def test_clave_live_auth_guard_rejects_mismatched_active_profile_identity() -> None:
    ...
    with override_settings(cadrumo_clave_movil_dni_nie=SecretStr("00000001R")):
        result = configure_operator_auth("clave_movil", operator_scope_ports=_OPERATOR_SCOPE_PORTS)

    assert result.identity_alignment == "mismatch"
    assert result.complete is False
    assert result.incomplete_reason == result.identity_alignment_detail
    assert result.precondition_verdict is not None
    assert result.precondition_verdict.failed_condition_id == "auth.clave_movil.identity_aligned"
