...
SYNTHETIC_PROFILE_ID = "12345678Z"
SYNTHETIC_EXPEDIENTE_ID = "200030300000000Z"
SESSION_BUCKET_ID = "45454545-4545-4454-8454-454545454545"


M303_DISPONIBLE_CASILLA: CasillaId = validated_casilla_id("iva.compensacion-disponible-fin-periodo")
_M303_COMPENSACION_PENDIENTE_ANTERIORES_CASILLA: CasillaId = validated_casilla_id(
    "iva.compensacion-pendiente-periodos-anteriores"
)
M303_POSTERIOR_CASILLA: CasillaId = validated_casilla_id("iva.compensacion-pendiente-periodos-posteriores")
M303_RESULTADO_CASILLA: CasillaId = validated_casilla_id("iva.resultado")
M303_GENERADA_CASILLA: CasillaId = validated_casilla_id("iva.compensacion-generada-periodo")
...
@cache
def registry_snapshot(modelo: str, filing_year: int, period: str):
...
@cache
def modelo_130_justificante_pdf_bytes() -> bytes:
    return (FIXTURES_DIR / "justificantes" / "modelo_130_2026Q1.pdf").read_bytes()


@cache
def modelo_303_justificante_pdf_bytes() -> bytes:
...
#: keeps them honest.
MODELO_130_FIXTURE_CSV = "ABCD1234EFGH5678"
MODELO_303_FIXTURE_CSV = "ZZZZ9999YYYY8888"
...
@contextmanager
def secure_backend(tmp_path: Path):
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id=SESSION_BUCKET_ID) as profile:
        yield profile.paths.database_file


@contextmanager
def profile_backend(tmp_path: Path, *, tax_id: str):
    with (
...
def parsed_303_submitted_file_observation(
    *,
    year: int,
    period: str,
    expediente_id: str,
    ...
    casilla_69: Decimal,
    casilla_71: Decimal,
    headers: tuple[ObservedHeaderFact, ...] = (M303_DECLARATION_TYPE_C,),
) -> FiledDeclaracionObservation:
    ...
    observed = tuple(
        ...
        for casilla_id, official_number, value in (
            ...
            (M303_POSTERIOR_CASILLA, "87", casilla_87),
            (M303_RESULTADO_CASILLA, "69", casilla_69),
    ...
    return FiledDeclaracionObservation(
        ...
        authenticated_identity=SYNTHETIC_PROFILE_ID,
        artefacts=(artefact,),
        casillas=observed,
        headers=headers,
        extraction_coverage={"submitted_file": 1.0},
        registry_snapshot_ref=registry_snapshot("303", year, period).snapshot_ref,
    )


def stored_130_justificante_observation(
    store: FiledDeclaracionObservationStore,
    *,
    authenticated_identity: str = "00000000T",
    expediente_id: str = "13020260410ABCD1234EFGH5678",
    captured_csv: str = MODELO_130_FIXTURE_CSV,
) -> FiledDeclaracionObservation:
    return stored_justificante_observation(
        store,
        modelo="130",
        pdf_bytes=modelo_130_justificante_pdf_bytes(),
...
def stored_303_justificante_observation(
    store: FiledDeclaracionObservationStore,
    *,
    authenticated_identity: str = "00000000T",
    ...
    captured_csv: str = MODELO_303_FIXTURE_CSV,
) -> FiledDeclaracionObservation:
    return stored_justificante_observation(
        store,
        modelo="303",
        pdf_bytes=modelo_303_justificante_pdf_bytes(),
        authenticated_identity=authenticated_identity,
        expediente_id=expediente_id,
        captured_csv=captured_csv,
    )
...
def stored_justificante_observation(
    store: FiledDeclaracionObservationStore,
    *,
    modelo: str,
    pdf_bytes: bytes,
    ...
    return FiledDeclaracionObservation(
        ...
        authenticated_identity=authenticated_identity,
        artefacts=(artefact,),
        registry_snapshot_ref=registry_snapshot(modelo, 2026, period.registry_token).snapshot_ref,
    )


def seed_current_130_filing(
    *,
    bucket_id: str,
...
def prior_303_observation(
    *,
    ...
    year: int = 2026,
    period: str = "1T",
    expediente_id: str = SYNTHETIC_EXPEDIENTE_ID,
    presented_at: datetime = CAPTURED_AT,
    status: str = "ALTA",
    ...
    return FiledDeclaracionObservation(
        ...
        casillas=(
            ...
            *(
            ...
            ObservedCasillaValue(
                casilla_id=M303_POSTERIOR_CASILLA,
            ...
            *(
                (
                    ObservedCasillaValue(
                        casilla_id=M303_GENERADA_CASILLA,
                        value=str(generated),
                        value_kind=CasillaValueKind.NUMERIC,
                        source_artefact_kind="submitted_file",
