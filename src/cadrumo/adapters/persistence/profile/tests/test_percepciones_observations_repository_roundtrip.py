...
def _repository() -> PercepcionObservationRepositoryAdapter:
    """Bind the application port to the active test profile's secure store."""
    return PercepcionObservationRepositoryAdapter(
...
def test_anti_tautology_strict_payload_rejects_dropped_field() -> None:
    ...
    PercepcionObservationRepositoryAdapter.payload_type.model_validate(full)
    partial = {k: v for k, v in full.items() if k != "observation"}
    with pytest.raises(ValidationError):
        PercepcionObservationRepositoryAdapter.payload_type.model_validate(partial)
...
def test_envelope_refuses_a_capture_instant_without_utc(captured_at: datetime) -> None:
    ...
    """
    with pytest.raises(ValidationError):
        PercepcionObservationRepositoryAdapter.payload_type(
            modelo="190",
            filing_year=2024,
            period=Period.from_year_and_code(2024, "0A"),
...
def test_envelope_accepts_a_utc_capture_instant() -> None:
    ...
    payload = PercepcionObservationRepositoryAdapter.payload_type(
        modelo="190",
        filing_year=2024,
        period=Period.from_year_and_code(2024, "0A"),
        observation=_observation(nif="11111111H", clave="A"),
