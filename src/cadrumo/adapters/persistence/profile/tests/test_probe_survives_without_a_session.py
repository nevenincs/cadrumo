"""Real profile-storage proof that auth readiness probes do not read without a session.
...
from cadrumo.application.auth.operator_probes import live_auth_identity_state
from cadrumo.application.preflight import run_preflight_checks
...
class _CertificateHealth:
    def evaluate(self, request: CertificateHealthProbeRequest) -> CertificateHealthObservation:
        del request
        return CertificateHealthObservation(severity=CertificateHealthBand.OK, days_until_expiry=365)


class _ClaveIdentity:
    def classify(self, raw: str) -> ClaveIdentityObservation:
        del raw
        return ClaveIdentityObservation(kind="DNI")


def _probe_ports(*, active_profile_session_bound: bool) -> OperatorProbePorts:
    return OperatorProbePorts(
        active_profile_session=_SessionPresence(active_profile_session_bound),
        certificate_health=_CertificateHealth(),
        clave_identity=_ClaveIdentity(),
    )


_LOCKED_OPERATOR_PROBE_PORTS = _probe_ports(active_profile_session_bound=False)
_BOUND_OPERATOR_PROBE_PORTS = _probe_ports(active_profile_session_bound=True)
