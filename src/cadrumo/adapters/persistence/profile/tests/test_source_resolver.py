...
def _resolver(reader: InvoiceSourceCatalogueReader) -> InvoiceCatalogueSourceResolver:
    return InvoiceCatalogueSourceResolver(
        ports=InvoiceSourceResolverPorts(catalogue_reader=reader),
    )


class _InMemoryCatalogueReader:
    """Adapter-test-owned reader fake for public resolver projections."""

    def __init__(self, invoices: Sequence[Invoice]) -> None:
        self._catalogue = build_invoice_catalogue(tuple(invoices))
...
def _public_resolution(
    ...
    *,
    ...
    """Resolve local fixture data through the public application seam."""
    return _resolver(_InMemoryCatalogueReader(invoices)).resolve(context)


def _public_m349_claves(invoices: Sequence[Invoice]) -> tuple[str, ...]:
    """Return M349 claves emitted by the resolver's public detail-row seam."""
...
def test_invoice_catalogue_source_resolver_emits_scalar_values_and_provenance(
    ...
    repository.save(build_invoice_catalogue((declarable, other_bucket, domestic)))
...
def test_invoice_catalogue_source_resolver_folds_received_acquisition_for_m349(
    ...
    repository.save(build_invoice_catalogue((acquisition,)))
    _modelos, _catalogues = bundled_registry_tree()
    _modelo_349 = next(candidate for candidate in _modelos if candidate.id == "349")
    snapshot = SimpleNamespace(revision=select_revision(_modelo_349, filing_year=2026, period="1T"))
...
def test_invoice_catalogue_source_resolver_projects_domestic_m347_summary_from_invoice_totals(
    ...
    repository = InvoiceCatalogueRepository(objects=secure_profile.repository)
    repository.save(build_invoice_catalogue((collectible, payable, floor_control)))
...
def test_invoice_catalogue_source_resolver_accepts_xi_goods_for_m349(
    ...
    repository.save(build_invoice_catalogue((declarable,)))
    _modelos, _catalogues = bundled_registry_tree()
    _modelo_349 = next(candidate for candidate in _modelos if candidate.id == "349")
    snapshot = SimpleNamespace(revision=select_revision(_modelo_349, filing_year=2026, period="1T"))
...
def test_invoice_catalogue_source_resolver_reports_a_degraded_catalogue_read() -> None:
    ...
    resolution = _resolver(DegradedReader()).resolve(
        CalculationSourceContext(
            bucket_id=_BUCKET_ID,
            modelo="349",
            filing_year=2026,
...
def test_converted_foreign_invoice_projects_its_euro_value_not_its_face_value(
    ...
    repository.save(build_invoice_catalogue((converted,)))
    _modelos, _catalogues = bundled_registry_tree()
    _modelo_349 = next(candidate for candidate in _modelos if candidate.id == "349")
    snapshot = SimpleNamespace(revision=select_revision(_modelo_349, filing_year=2026, period="1T"))
...
def test_unconverted_foreign_invoice_is_withheld_from_projection(
    ...
    repository.save(build_invoice_catalogue((unconverted,)))
    _modelos, _catalogues = bundled_registry_tree()
    _modelo_349 = next(candidate for candidate in _modelos if candidate.id == "349")
    snapshot = SimpleNamespace(revision=select_revision(_modelo_349, filing_year=2026, period="1T"))
...
def test_a_service_category_alone_resolves_its_m349_clave() -> None:
    ...
    assert set(_public_m349_claves((issued, received))) == {"S", "I"}
...
def test_capability_parity_m349_declares_every_intracommunity_capability(
    ...
    """
    repository = InvoiceCatalogueRepository(objects=secure_profile.repository)
    repository.save(build_invoice_catalogue(_capability_bucket_invoices(_BUCKET_ID)))
    _modelos, _catalogues = bundled_registry_tree()
    _modelo_349 = next(candidate for candidate in _modelos if candidate.id == "349")
    snapshot = SimpleNamespace(revision=select_revision(_modelo_349, filing_year=2026, period="1T"))
...
def test_m347_clave_f_declares_a_mediated_sale_ordinary_sale_of_the_same_amount_does_not(
    ...
    resolution = _public_resolution((mediated_sale, ordinary_sale), context=context)

    assert {item.source_ref for item in resolution.provenance} == {
        f"collectible_invoice:{mediated_sale.invoice_id}",
        f"collectible_invoice:{ordinary_sale.invoice_id}",
    }
...
def test_m347_clave_g_declares_only_air_transport_purchases_not_other_mediated_purchases(
    ...
    assert {item.source_ref for item in resolution.provenance} == {
        f"payable_invoice:{air_transport_purchase.invoice_id}",
        f"payable_invoice:{non_air_mediated_purchase.invoice_id}",
    }
...
def test_m347_filer_declaration_roles_fails_closed_to_empty_for_a_profile_absent_bucket(
    ...
    purchase = _invoice(
        bucket_id=None,
        kind=InvoiceKind.RECEIVED,
        invoice_number="M347-ABSENT-ROLE-2026-001",
        issued_at=date(2026, 5, 1),
...
def test_m347_filer_declaration_roles_reaches_a_role_set_by_the_real_operator_path(
    ...
    collection = _invoice(
        bucket_id=None,
        kind=InvoiceKind.ISSUED,
        invoice_number="M347-ROLE-COLLECTION-2026-001",
        issued_at=date(2026, 5, 1),
        counterparty_tax_id="C3333333G",
    ...
    ordinary = _invoice(
        bucket_id=None,
        kind=InvoiceKind.ISSUED,
        invoice_number="M347-ROLE-ORDINARY-2026-001",
        issued_at=date(2026, 5, 1),
        counterparty_tax_id="C3333333G",
        counterparty_name="Cliente Pagador SL",
        counterparty_country="ES",
...
def test_m347_clave_c_declares_the_beneficiary_not_the_payer_through_the_real_resolver(
    ...
    resolution = _public_resolution((collection_invoice, ordinary_invoice), context=context)

    assert {item.source_ref for item in resolution.provenance} == {
        f"collectible_invoice:{collection_invoice.invoice_id}",
        f"collectible_invoice:{ordinary_invoice.invoice_id}",
    }
    # The public summary sees two parties only when the persisted collector role
    # causes the collected amount to be attributed to its beneficiary.
    assert resolution.binding_values["modelo-347-declarante-numero-personas-entidades"] == Decimal("2")
    assert resolution.binding_values["modelo-347-declarante-importe-total-anual-operaciones"] == Decimal("7000.00")
...
def test_m347_clave_c_requires_the_filer_role_a_beneficiary_fact_alone_is_not_enough(
    ...
    ordinary_invoice = _invoice(
        bucket_id=None,
        kind=InvoiceKind.ISSUED,
        invoice_number="M347-COBRO-2026-003",
        issued_at=date(2026, 5, 1),
        counterparty_tax_id="C3333333G",
        counterparty_name="Cliente Pagador SL",
        counterparty_country="ES",
    ...
    assert {item.source_ref for item in resolution.provenance} == {
        f"collectible_invoice:{collection_shaped_invoice.invoice_id}",
        f"collectible_invoice:{ordinary_invoice.invoice_id}",
    }
    # Without the filer's persisted role, both records remain the same payer.
    assert resolution.binding_values["modelo-347-declarante-numero-personas-entidades"] == Decimal("1")
    assert resolution.binding_values["modelo-347-declarante-importe-total-anual-operaciones"] == Decimal("7000.00")
...
def test_m347_clave_e_declares_a_subvencion_from_a_public_administration_ordinary_payment_does_not(
    ...
    resolution = _public_resolution((subvencion_invoice, ordinary_invoice), context=context)
...
def test_m347_clave_e_requires_the_public_administration_role_a_subvencion_fact_alone_is_not_enough(
    ...
    resolution = _public_resolution((subvencion_shaped_invoice,), context=context)

    assert {item.source_ref for item in resolution.provenance} == {
        f"collectible_invoice:{subvencion_shaped_invoice.invoice_id}",
    }
    assert resolution.binding_values["modelo-347-declarante-numero-personas-entidades"] == Decimal("1")
...
def test_m347_clave_d_declares_an_acquisition_outside_activity_the_same_activity_purchase_does_not(
    ...
    resolution = _public_resolution((outside_activity_purchase, within_activity_purchase), context=context)
...
def test_m347_clave_d_requires_the_filer_role_the_fact_alone_is_not_enough(
    ...
    resolution = _public_resolution((outside_activity_shaped_purchase,), context=context)
...
def test_the_invoice_stores_contribute_nothing_to_m303_or_m390(modelo_id: str, period: str) -> None:
    ...
    offenders = {
        revision_id: [
            ...
            if binding.source in InvoiceCatalogueSourceResolver.owned_sources
        ]
...
def test_an_explicit_operation_type_resolves_a_clave_without_any_iva_category() -> None:
    ...
    assert _public_m349_claves((invoice,)) == ("E",)
...
def test_the_informativas_are_unaffected_by_that_divergence(
    ...
    """
    repository = InvoiceCatalogueRepository(objects=secure_profile.repository)
    repository.save(build_invoice_catalogue((_same_facts_invoice(with_category=False),)))
    context = CalculationSourceContext(
        bucket_id=_BUCKET_ID,
        modelo="347",
        filing_year=2026,
    ...
    resolution = _resolver(repository).resolve(context)
...
def _m349_resolution(repository: InvoiceCatalogueRepository):
    ...
    _modelo_349 = next(candidate for candidate in _modelos if candidate.id == "349")
    snapshot = SimpleNamespace(revision=select_revision(_modelo_349, filing_year=2026, period="1T"))
    return _resolver(repository).resolve(
        CalculationSourceContext(
            bucket_id=_BUCKET_ID,
            modelo="349",
            filing_year=2026,
...
def test_an_inferred_entrega_clave_is_disclosed_when_the_bucket_also_holds_an_importation(
    ...
    """
    repository = InvoiceCatalogueRepository(objects=secure_profile.repository)
    repository.save(build_invoice_catalogue((_ic_supply_without_operation_type(), _third_country_import())))
...
def test_the_same_supply_is_silent_when_the_taxpayer_never_imports(
    ...
    """
    repository = InvoiceCatalogueRepository(objects=secure_profile.repository)
    repository.save(build_invoice_catalogue((_ic_supply_without_operation_type(),)))
...
def test_a_stated_operation_type_is_never_disclosed_as_inferred(
    ...
    repository.save(build_invoice_catalogue((stated, _third_country_import())))
...
# but the set of declarable FACTS it contributes. The public resolver envelope
# carries those facts through its aggregate, detail-row and provenance channels.
#
...
def test_declarable_fact_contract_covers_every_observation_fact_the_stores_contribute() -> None:
    ...
    The contract is pinned against the observation model's own declarable
    surface. Identity and rectification axes are excluded by name, which forces
    a newly-added declarable field to fail here rather than slip past the
    coverage proof unnoticed.
    """
...
def test_m349_declarable_facts_are_reachable_on_the_canonical_path(
    ...
    row = rows[0]
    canonical_facts = {
        "party_tax_id": row.nif_comunitario,
        "country_code": row.codigo_pais,
        "transaction_date": canonical.issued_at,
        "base_amount": row.importe,
        "invoice_total_amount": canonical.grand_total_eur,
        "intracommunity_clave": row.clave_operacion,
        "operation_clave": None,
        "party_legal_name": row.razon_social,
    }
...
def test_m347_declarable_facts_are_reachable_on_the_canonical_path(
    ...
    # The public source-resolution envelope exposes the invoice facts through
    # its aggregate/provenance channels; operation-clave detail rows are not a
    # part of the M347 resolver contract.
    public_facts = {
        "party_tax_id": canonical.counterparty_tax_id,
        "country_code": canonical.counterparty_country,
        "transaction_date": canonical.issued_at,
        "base_amount": canonical.base_total_eur,
        "invoice_total_amount": canonical.grand_total_eur,
        "intracommunity_clave": None,
        "party_legal_name": canonical.counterparty_name,
    }
    assert {name: declared_facts[name] for name in public_facts} == public_facts
...
def test_an_unattributed_invoice_in_the_bucket_store_is_still_declared(
    ...
    repository.save(build_invoice_catalogue((unattributed,)))
    _modelos, _catalogues = bundled_registry_tree()
    _modelo_349 = next(candidate for candidate in _modelos if candidate.id == "349")
    snapshot = SimpleNamespace(revision=select_revision(_modelo_349, filing_year=2026, period="1T"))
...
def test_an_invoice_naming_another_bucket_is_still_excluded(
    ...
    repository.save(build_invoice_catalogue((foreign,)))
    _modelos, _catalogues = bundled_registry_tree()
    _modelo_349 = next(candidate for candidate in _modelos if candidate.id == "349")
    snapshot = SimpleNamespace(revision=select_revision(_modelo_349, filing_year=2026, period="1T"))
