...
def _structured_draft() -> InvoiceDraft:
    """Provide the public draft contract consumed by the downstream waist hops.

    Structured-reader extraction is covered at its application seam. This
    adapter integration module starts at the already typed draft and verifies
    that confirmation, catalogue persistence, and source resolution retain its
...
def _evidence() -> EvidenceInput:
    """Build the typed in-memory carrier used by the routing assertion."""
...
class TestHop1Ingest:
    ...
    def test_the_bytes_come_back_out_of_the_encrypted_store_unchanged(
        ...
        evidence = resolve_attachment_evidence_input(
            attachment_id,
...
class TestHop7WhereTheChainActuallyTerminates:
    ...
    def _revision() -> ModeloRevision:
        ...
        return published_authority_operation().snapshot("303", filing_year=2024, period=str(period.code)).revision
    ...
    def test_the_invoice_resolver_runs_and_honestly_returns_nothing_for_303(
        ...
        resolution = InvoiceCatalogueSourceResolver(
            ports=InvoiceSourceResolverPorts(
                catalogue_reader=InvoiceCatalogueRepository(bucket_id=_BUCKET_ID),
            ),
        ).resolve(context)
