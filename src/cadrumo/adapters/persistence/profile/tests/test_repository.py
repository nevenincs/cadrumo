...
@pytest.fixture
def repo(_active_bucket_runtime: TestRuntimeProfile) -> ModeloDraftRepository:
    return ModeloDraftRepository(
        bucket_id=_BUCKET_ID,
        objects=_active_bucket_runtime.repository,
    )
...
class TestSaveLoad:
    def test_round_trip_preserves_payload(self, repo: ModeloDraftRepository) -> None:
        ...
        repo_b = ModeloDraftRepository(
            bucket_id=_BUCKET_ID,
            objects=repo.secure_object_repository,
        )
