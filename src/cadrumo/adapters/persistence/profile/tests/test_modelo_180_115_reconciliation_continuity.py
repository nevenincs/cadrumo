...
This module is the cross-year behavior coverage for Modelo 180.
...
# equals the sum of the four 115 quarterly values. Casilla 03 is _produced
...
def _calculate_115(
    *,
    ...
    snapshot = published_authority_operation().snapshot(_MODELO_115, filing_year=filing_year, period=period)
    binding_values = {
...
def _calculate_180(
    *,
    ...
    snapshot = published_authority_operation().snapshot(_MODELO_180, filing_year=filing_year, period="0A")
...
def test_modelo_115_quarterly_engine_produces_retenciones_casilla(tmp_path: Path) -> None:
    ...
    The value is _produced by the real engine from the manual base input, never
...
def test_modelo_180_relation_prefill_aggregates_115_quarters(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path), bundled_indexed_authority().operation() as operation:
        ...
        snapshot_180 = published_authority_operation().snapshot(_MODELO_180, filing_year=_YEAR_N, period="0A")
...
def test_modelo_180_year_isolation_ignores_prior_year_observations(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path), bundled_indexed_authority().operation() as operation:
        ...
        snapshot_180_n1 = published_authority_operation().snapshot(_MODELO_180, filing_year=_YEAR_N_PLUS_1, period="0A")
...
def test_modelo_180_115_reconciliation_enrolls_two_renta_years(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path), bundled_indexed_authority().operation() as operation:
        ...
        # the _produced casilla count for the feeder is evidenced by summing per quarter.
        ...
        snapshot_180_n = published_authority_operation().snapshot(_MODELO_180, filing_year=_YEAR_N, period="0A")
