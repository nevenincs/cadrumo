...
from cadrumo.adapters.persistence.storage.crypto.encrypted_columns import secure_object_key_digest
...
def test_retencion_observation_survives_encrypted_storage_roundtrip(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path):
        ...
        repo = RetencionObservationRepositoryAdapter()
...
def test_public_observation_contract_rejects_dropped_field() -> None:
    """The public observation contract rejects a dropped load-bearing field.

    The application DTO is the inward seam. Testing its required fields keeps
    this test independent of the adapter's private encrypted-envelope model.
    """
    full = _observation(
        nif="44444444A",
    ...
    RetencionObservation.model_validate(full)
    partial = {key: value for key, value in full.items() if key != "perceptor_nif"}
    with pytest.raises(ValidationError):
        RetencionObservation.model_validate(partial)
...
def test_persist_helper_writes_set_readable_by_load(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path):
        ...
        persist_retencion_observations(
            ports=RetencionObservationPorts(repository=RetencionObservationRepositoryAdapter()),
            modelo="180",
            filing_year=2024,
            period=period,
            observations=observations,
        )
        loaded = RetencionObservationRepositoryAdapter().load_observations("180", period)
...
def test_repository_refuses_a_capture_instant_without_utc(captured_at: datetime, tmp_path: Path) -> None:
    """The public repository operation refuses a naive or non-UTC capture instant.
...
def test_repository_accepts_a_utc_capture_instant(tmp_path: Path) -> None:
    """The public repository operation accepts a UTC capture instant."""
    observation = _observation(
        nif="11111111H",
        scheme=RetencionScheme("actividades_economicas"),
        retencion=Decimal("100"),
    ...
    with isolated_runtime_profile(tmp_path=tmp_path):
        ...
        assert RetencionObservationRepositoryAdapter().load_observations("180", period) == (observation,)


def _capture_payload(period: Period):
    """Build one fully-populated adapter record through its public builder."""
    return RetencionObservationRepositoryAdapter().build_observation_payload(
        modelo="180",
        filing_year=2024,
        period=period,
        observation=_observation(
...
def test_an_evidence_authority_value_cannot_enter_this_store(tmp_path: Path) -> None:
