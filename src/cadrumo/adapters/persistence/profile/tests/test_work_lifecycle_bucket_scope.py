...
@pytest.fixture
def a_bound_repository(tmp_path: Path) -> Iterator[WorkLifecyclePorts]:
...
def test_get_refuses_a_unit_owned_by_another_bucket(a_bound_repository) -> None:
    ...
    ports = a_bound_repository
    foreign = _unit(_BUCKET_B)

    with pytest.raises(WorkUnitNotFoundError):
        get_work_unit(foreign.work_unit_id, ports=ports)
...
def test_rename_refuses_a_unit_owned_by_another_bucket(a_bound_repository) -> None:
    ...
    events = ports.bucket_event_repository
    foreign = _unit(_BUCKET_B)

    with pytest.raises(WorkUnitNotFoundError):
        rename_work_unit(foreign.work_unit_id, "renamed by A", actor="operator-A", ports=ports, clock=_T0)
...
def test_discard_refuses_a_unit_owned_by_another_bucket(a_bound_repository) -> None:
    ...
    with pytest.raises(WorkUnitNotFoundError):
        discard_work_unit(foreign.work_unit_id, actor="operator-A", ports=ports, clock=_T0)
...
def test_same_bucket_lifecycle_still_works(a_bound_repository) -> None:
    ...
    assert get_work_unit(own.work_unit_id, ports=ports).bucket_id == _BUCKET_A

    renamed = rename_work_unit(own.work_unit_id, "renamed", actor="operator-A", ports=ports, clock=_T0)
    assert renamed.name == "renamed"

    discarded = discard_work_unit(own.work_unit_id, actor="operator-A", ports=ports, clock=_T0)
...
def test_the_foreign_unit_is_genuinely_present_in_the_catalogue(a_bound_repository) -> None:
    ...
    assert {unit.bucket_id for unit in list_work_units(ports=ports)} == {_BUCKET_A, _BUCKET_B}
