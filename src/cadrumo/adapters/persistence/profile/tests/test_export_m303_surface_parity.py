...
def test_a_modelo_303_revision_passes_the_election_precondition_on_both_surfaces(
    isolated_backend: None,
    tmp_path: Path,
) -> None:
    """Both surfaces clear the election precondition and stop at the same later gate."""
    with bundled_indexed_authority().operation() as operation:
        _taxpayer_nif, _bucket_id, verified, *_repositories = _build_verified_modelo_303_revision(operation=operation)
