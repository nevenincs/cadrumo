...
Exercises :mod:`~application.modelo.review_package_recipient_registry`
...
See Also:
    :class:`~adapters.persistence.profile.review_package_recipient_registry.RecipientFingerprintRegistryAdapter`:
        Encrypted public-key trust register adapter under test.
...
from cadrumo.adapters.persistence.profile.review_package_recipient_registry import (
...
def _ports(profile: TestRuntimeProfile) -> RecipientFingerprintRegistryPorts:
    """Compose the real adapter behind the application registry capability."""
...
def _get(ports: RecipientFingerprintRegistryPorts, recipient_id: str) -> RecipientFingerprintRecord:
    return get_recipient_fingerprint(recipient_id, ports=ports)


def _remove(ports: RecipientFingerprintRegistryPorts, recipient_id: str) -> RecipientFingerprintRegister:
    return remove_recipient_fingerprint(recipient_id, ports=ports)
...
def test_load_returns_empty_register_when_absent(tmp_path: Path) -> None:
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id="135dbe14-59b2-4418-9731-552c860fcf78") as profile:
        ...
        assert ports.registry_repository.load() == RecipientFingerprintRegister()
...
def test_add_then_load_roundtrips_with_strict_equality(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id="77a6c02c-9b4e-458b-877d-0bb8ad233ee1") as profile:
        ...
        reloaded = ports.registry_repository.load()
...
def test_encrypted_registry_load_refuses_duplicate_persisted_recipient_id(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id="19c86bf5-83c1-4901-a9ed-ebbab0460c1a") as profile:
        ports = _ports(profile)
        _add(ports, recipient_id="acct", public_key_hex=_fresh_public_key_hex(), added_at=_NOW)
        expected = _add(ports, recipient_id="gestor", public_key_hex=_fresh_public_key_hex(), added_at=_NOW)

        assert ports.registry_repository.load() == expected
        ...
        with pytest.raises(RecipientFingerprintRegistryError, match="recipient fingerprint registry load failed"):
            ports.registry_repository.load()
...
def test_register_is_never_stored_as_plaintext(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id="f5a353fa-361e-4bf2-abc3-83675e2ef80e") as profile:
        ports = _ports(profile)
        _add(ports, recipient_id="my-accountant", public_key_hex=public_key_hex, added_at=_NOW)
...
def test_add_refuses_duplicate_recipient_id(tmp_path: Path) -> None:
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id="cdfb8758-b634-4a3a-a773-5d8a82b2ed31") as profile:
        ports = _ports(profile)
        _add(ports, recipient_id="my-accountant", public_key_hex=_fresh_public_key_hex(), added_at=_NOW)
...
def test_add_then_remove_then_list_reflects_removal(tmp_path: Path) -> None:
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id="dcffda71-679b-4381-8aa9-25fe689e8f55") as profile:
        ports = _ports(profile)
        _add(ports, recipient_id="a", public_key_hex=_fresh_public_key_hex(), added_at=_NOW)
        _add(ports, recipient_id="b", public_key_hex=_fresh_public_key_hex(), added_at=_NOW)

        _remove(ports, "a")
        remaining = list_recipient_fingerprints(ports=ports)
...
def test_two_buckets_maintain_independent_registers(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id="3e2c956e-7ade-4471-ae5e-1a4573f5d4a7") as profile_two:
        register_two = _ports(profile_two).registry_repository.load()
...
def test_known_vector_fingerprint_survives_the_encrypted_registry_roundtrip(tmp_path: Path) -> None:
    ...
    ``RecipientFingerprintRegistryAdapter`` persistence boundary (a genuine
