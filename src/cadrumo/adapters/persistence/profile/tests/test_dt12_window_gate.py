"""DT 12ª window fact-gate through the profile persistence boundary (LIVE).
...
def _create_m100_work_unit(*, operation: PinnedAuthorityOperation) -> tuple[str, str]:
    ...
    snapshot = published_authority_operation().snapshot("100", filing_year=_FILING_YEAR, period=period.registry_token)
