...
from cadrumo.application.ledger.evidence import MediaKind, PurchaseInvoiceEvidence, PurchaseInvoiceEvidenceDocument
...
def test_resolve_purchase_invoice_evidence_reads_secure_storage_into_memory(
    ...
    resolved = resolve_purchase_invoice_evidence_input(record, store=store, ports=evidence_input_ports)
...
def test_resolve_attachment_evidence_input_round_trips_bytes(
    ...
    resolved = resolve_attachment_evidence_input(record.attachment_id, store=store, ports=evidence_input_ports)
...
def test_unreadable_bytes_are_refused_however_respectable_the_declared_type(
    ...
    with pytest.raises(PurchaseInvoiceEvidenceInputError) as excinfo:
        resolve_attachment_evidence_input(attachment_id, store=store, ports=evidence_input_ports)
