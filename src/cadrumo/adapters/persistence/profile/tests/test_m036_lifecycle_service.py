...
from cadrumo.adapters.persistence.storage.secure_object_namespaces import LIVE_M036_DECLARATION_NAMESPACE
...
def _ports(bucket_id: str) -> M036LifecyclePorts:
    """Bind the real profile adapters for the active isolated runtime."""
    return build_m036_lifecycle_ports(bucket_id=bucket_id)


def _record(command: M036DeclarationCommand, *, bucket_id: str) -> M036DeclarationResult:
    """Call the application service through its required composed ports."""
    return record_m036_declaration(command, bucket_id=bucket_id, ports=_ports(bucket_id))


def _list(*, bucket_id: str) -> tuple[M036DeclarationResult, ...]:
    """Read declarations through the same bound application capability."""
    return list_m036_declarations(bucket_id=bucket_id, ports=_ports(bucket_id))
...
def test_record_alta_persists_result_with_bucket_scope(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id=_PROFILE_ID) as runtime:
        result = _record(_alta_command(), bucket_id=runtime.bucket_id)
...
def test_record_modificacion_emits_modificacion_event(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id=_PROFILE_ID) as runtime:
        _record(_alta_command(), bucket_id=runtime.bucket_id)
        result = _record(command, bucket_id=runtime.bucket_id)
        catalogue = BucketEventHistoryRepository().load()
...
def test_record_is_idempotent_for_identical_input(tmp_path: Path) -> None:
    ...
    """
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id=_PROFILE_ID) as runtime:
        first = _record(_alta_command(), bucket_id=runtime.bucket_id)
        second = _record(_alta_command(), bucket_id=runtime.bucket_id)
        catalogue = BucketEventHistoryRepository().load()
...
def test_record_with_distinct_justificante_yields_distinct_record(
    ...
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id=_PROFILE_ID) as runtime:
        draft = _record(_alta_command(), bucket_id=runtime.bucket_id)
        confirmed = _record(
...
def test_record_accepts_the_owning_profile_and_reads_back(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id=_PROFILE_ID) as runtime:
        result = _record(_alta_command(), bucket_id=runtime.bucket_id)
        recorded = _list(bucket_id=runtime.bucket_id)
...
def test_split_record_write_shape_commits_between_stores(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path, bucket_id=_PROFILE_ID) as runtime:
        ...
        declarations = _ports(runtime.bucket_id).declaration_repository
        recorder = WriteUnitRecorder(runtime.repository.engine)
