...
class _Availability:
    """Inward offered-option record for pure discovery derivation tests."""
...
class _AvailabilityReport:
    """Inward offered-option report implementing the application protocol."""

    items: tuple[_Availability, ...]
    discovered_at: datetime

    @property
    def offered_pairs(self) -> tuple[tuple[str, int], ...]:
        """Return all offered modelo/year pairs in report order."""
...
def _confidently_excluded(profile: TaxpayerProfile) -> set[str]:
    ...
    from cadrumo.application.overview.coverage import build_obligation_coverage
...
def test_an_out_of_scope_modelo_is_absent() -> None:
    from cadrumo.domain.calculations.registry.modelo_obligation_scope import OUT_OF_SCOPE_OBLIGATIONS
...
def _availability(*items: tuple[str, tuple[int, ...]]) -> _AvailabilityReport:
    return _AvailabilityReport(
        items=tuple(_Availability(modelo=modelo, ejercicios=years) for modelo, years in items),
...
def test_discovery_classifies_the_live_offered_options_at_its_owning_boundary() -> None:
    profile = _autonomo()
    ...
    report = asyncio.run(
        discover_filed_history(
            filed_data_port=_DiscoveryPort(),
            profile=profile,
            today=_TODAY,
        )
    )
