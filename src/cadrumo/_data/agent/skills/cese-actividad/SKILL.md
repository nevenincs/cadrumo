...
5. Close the books for the final period: hand off to `llevar-libro` and
   `clasificar` for the last quarter's records, then drive each surviving
   obligation through its owning per-modelo skill in deadline order,
   exactly as a `cierre-trimestre` would — the last quarter is a quarter
   close with a shorter future.
6. The following year, when the annual windows open, `resumen-anual` owns
...
`resumen-anual` owns the final annual season. This itinerary closes when
