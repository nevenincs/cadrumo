...
- The taxpayer is onboarded (`aeat app overview status` reports an active
  profile) — run `alta-contribuyente` first for a brand-new user.
...
5. Route the steady state to the WHO itinerary the recorded facts now
   select (`autonomo-estimacion-directa`, `autonomo-modulos`,
   `pyme-sociedad`, with `retenedor-empleador` or
   `intra-community-operator` overlays as the facts declare), and start
   the bookkeeping habit immediately: hand off to `llevar-libro` so the
