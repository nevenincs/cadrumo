...
This skill diffs against `preparar-modelo-130` (the shared pago-fraccionado
spine: work create -> calculate -> verify -> revision review -> export ->
reconcile is identical) and `preparar-modelo-200` (M202's own parent form,
...
`pyme-sociedad` for the itinerary that sequences Modelo 202 alongside Modelo
...
8. Once all obligatory instalments (1P/2P/3P) for the `filing_year` are filed,
   hand off to `preparar-modelo-200` for the annual return - it folds this
