...
def _validate_secret_state(snapshot: OperationPersistedSnapshot) -> None:
    ...
    _validate_executor_entry(snapshot, entered_at)
    if requirement is None:
        return
    _validate_secret_requirement(snapshot, requirement, entered_at)


def _validate_executor_entry(snapshot: OperationPersistedSnapshot, entered_at: datetime | None) -> None:
...
def _validate_secret_requirement(
    snapshot: OperationPersistedSnapshot,
...
def _validate_deadline_and_cancellation_state(snapshot: OperationPersistedSnapshot) -> None:
    ...
    _validate_deferred_cancellation(snapshot, acknowledged_at)
    _validate_safety_timestamps(execution_deadline, cleanup_deadline, requested_at, acknowledged_at)
    _validate_execution_deadline(snapshot.started_at, execution_deadline)
    if requested_at is None:
        _validate_unrequested_cancellation(cleanup_deadline, acknowledged_at)
        return
    _validate_requested_cancellation(snapshot, requested_at, cleanup_deadline, acknowledged_at)


def _validate_deferred_cancellation(snapshot: OperationPersistedSnapshot, acknowledged_at: datetime | None) -> None:
...
def _validate_safety_timestamps(
    execution_deadline: datetime | None,
    cleanup_deadline: datetime | None,
...
def _validate_execution_deadline(started_at: datetime, execution_deadline: datetime | None) -> None:
...
def _validate_unrequested_cancellation(cleanup_deadline: datetime | None, acknowledged_at: datetime | None) -> None:
...
def _validate_requested_cancellation(
    snapshot: OperationPersistedSnapshot,
    ...
    _validate_cancellation_acknowledgement(snapshot, requested_at, acknowledged_at)
...
def _validate_cancellation_acknowledgement(
    snapshot: OperationPersistedSnapshot,
    requested_at: datetime,
    acknowledged_at: datetime | None,
) -> None:
