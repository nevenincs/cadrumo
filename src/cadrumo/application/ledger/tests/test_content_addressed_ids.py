...
from ._business_operation_invoice_support import _make_payable_svc
...
class TestInvoiceIdContentAddressed:
    def test_invoice_id_equals_content_digest_and_roundtrips(
        ...
        with frozen_clock(_INSTANT):
            ...
            reloaded = _make_payable_svc(isolated_settings, secure_objects).view(
                bucket_id=_BUCKET_ID,
                invoice_id=record.invoice_id,
            )
        assert reloaded == record

    def test_invoice_id_is_content_derived_not_constant(
        self,
        isolated_settings: Settings,
        secure_objects: SecureObjectRepository,
    ) -> None:
    ...
    def test_invoice_id_genuine_duplicate_preserved_under_frozen_clock(
        self,
        isolated_settings: Settings,
        secure_objects: SecureObjectRepository,
    ) -> None:
