...
_CIF = "B12345674"
_DEFAULT_SCOPE = IvaTerritorialScope.from_registry("es_mainland")


class _InMemoryCounterpartyEstablishmentRepository(CounterpartyEstablishmentRepositoryProtocol):
    """Inward fake for the counterparty fact policy tests."""
...
def repository() -> CounterpartyEstablishmentRepositoryProtocol:
    """Return a fresh inward capability for each policy test."""
...
def _resolve(repository: CounterpartyEstablishmentRepositoryProtocol):
...
def test_one_answer_serves_every_later_read(repository: CounterpartyEstablishmentRepositoryProtocol) -> None:
...
def test_an_unanswered_identification_resolves_to_nothing(
    ...
    """
    _confirm(repository, scope=IvaTerritorialScope.from_registry("eu_member"))
...
def test_answering_the_identification_later_is_an_addition_not_a_conflict(
    ...
    """
    first = _confirm(repository, scope=IvaTerritorialScope.from_registry("es_mainland"))
...
def test_a_different_identification_refuses_and_names_both_values(
    ...
    _confirm(repository, identification_state=EUMemberState.from_registry("de"))
