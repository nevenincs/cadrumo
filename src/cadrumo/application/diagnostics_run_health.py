...
``cadrumo app diagnostics run-health`` operator surface (GitHub issue #407).
...
per-provider, backing the sibling ``cadrumo app diagnostics runs`` listing verb
...
error-kind breakdown, backing the ``cadrumo app diagnostics latency`` and
``cadrumo app diagnostics errors`` verbs (also GitHub issue #407). Neither
...
backing the ``cadrumo app diagnostics llm-usage`` verb (also GitHub issue #407).
