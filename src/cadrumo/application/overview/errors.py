...
read failures. They inherit from :class:`~core.errors.hierarchy.CadrumoError` so the
...
class OverviewCalendarError(OverviewError):
    """Raised when :func:`application.overview.calendar.build_overview_calendar` fails."""


class OverviewAgendaError(OverviewError):
    """Raised when :func:`application.overview.agenda.build_overview_agenda` fails."""


class OverviewBacklogError(OverviewError):
    """Raised when :func:`application.overview.backlog.build_overview_backlog` fails."""


class OverviewExplainError(OverviewError):
    """Raised when :func:`application.overview.explain.build_overview_explain` fails."""
