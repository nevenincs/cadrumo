...
_TIPO_GRAVAMEN = validated_casilla_id("tipo_gravamen", surface="es_pt_canones_test")
_CUOTA_INTEGRA = validated_casilla_id("cuota_integra", surface="es_pt_canones_test")
...
def _resolve_rate(*, tipo_renta: str, country_code: str, base: str) -> tuple[Decimal, Decimal]:
    ...
    text_inputs = {validated_casilla_id("tipo_renta", surface="es_pt_canones_test"): tipo_renta}
    casilla_inputs = {
        validated_casilla_id("rendimientos_integros", surface="es_pt_canones_test"): Decimal(base),
        validated_casilla_id("gastos_deducibles", surface="es_pt_canones_test"): Decimal("0"),
        validated_casilla_id("retencion_practicada", surface="es_pt_canones_test"): Decimal("0"),
    }
...
def test_pt_canones_resolves_treaty_ceiling_of_5_percent(tmp_path: Path) -> None:
    ...
    with isolated_runtime_profile(tmp_path=tmp_path):
        tipo, cuota = _resolve_rate(tipo_renta="canones", country_code="PT", base="1000.00")
