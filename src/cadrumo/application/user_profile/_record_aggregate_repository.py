...
The :class:`ProfileRecordAggregateRepository` owns cross-store aggregate ordering. The shared
...
tombstones. :meth:`ProfileRecordAggregateRepository.load` runs
...
See Also:
    :class:`~cadrumo.application.user_profile.ProfileRecordLifecycle`
...
from ._record_storage import ProfileAggregateStoragePort, default_profile_aggregate_storage
from ._repository import ProfileRecordRepository, refresh_output_language_hint
...
def _default_kdf_params(storage: ProfileAggregateStoragePort) -> Any:
    ...
    """
    return storage.default_kdf_params()


class ProfileRecordSummary(BaseModel):
    """A typed one-row summary of a registered profile.

    Returned by :meth:`ProfileRecordAggregateRepository.list`; carries only the
...
class ProfileRecordAggregateRepository:
    ...
    def __init__(
        ...
        *,
        root: Path | None = None,
        secure_objects: Any | None = None,
        schema: ProfileSchemaDefinition | None = None,
        storage: ProfileAggregateStoragePort | None = None,
    ) -> None:
        ...
        Args:
            ...
            storage: Optional application storage port. ``None`` binds the
                real persistence adapter through the default composition
                provider.
        """
        self._root = effective_storage_root(root)
        self._secure_objects = secure_objects
        self._schema = schema
        self._storage = storage or default_profile_aggregate_storage()
    ...
    def create(
        ...
        *,
        ...
        with active_profile_pointer_transaction(self._root) as pointer_transaction:
            ...
            paths = self._storage.bucket_paths(self._root, resolved_id)
            ...
            if self._storage.manifest_path(paths).is_file():
            ...
            kdf_params = _default_kdf_params(self._storage)
            created_at = now()
            bucket_dek_path = self._storage.keystore_path(self._root, resolved_id) / self._storage.bucket_dek_filename
            if not bucket_dek_path.is_file():
                ...
                raise self._storage.storage_validation_error(
            ...
            key_schedule = self._storage.bucket_dek_v1_schedule()
            ...
            manifest_schema_version = self._storage.manifest_schema_version
            ...
            try:
                ...
                self._storage.write_manifest(
                    self._storage.bucket_paths(self._root, resolved_id),
                    self._storage.build_manifest(
    ...
    def load(self, profile_id: str) -> ProfileAggregate:
        ...
        Raises:
            ProfileNotFoundError: If the bucket directory or manifest
                is absent.
        """
        paths = self._storage.bucket_paths(self._root, profile_id)
        if not self._storage.manifest_path(paths).is_file():
        ...
        try:
            manifest = self._storage.read_manifest(paths)
        except self._storage.storage_validation_error_type as exc:
    ...
    def save(self, aggregate: ProfileAggregate) -> None:
        ...
        """
        paths = self._storage.bucket_paths(self._root, aggregate.profile_id)
        current_manifest = self._storage.read_manifest(paths)
        self._storage.write_manifest(
            paths,
            self._storage.build_manifest(
    ...
    def rename(
        ...
        *,
        ...
        paths = self._storage.bucket_paths(self._root, profile_id)
        manifest = self._storage.read_manifest(paths)
        self._storage.write_manifest(paths, manifest.model_copy(update={"label": trimmed}))
    ...
    def list(self) -> Sequence[ProfileRecordSummary]:
        ...
        by every :class:`ProfileRecordAggregateRepository` write. Tombstoned profiles
        are included so callers that need the full inventory (repair,
        audit) see them; live-surface callers filter on ``status``.

        Each element is a :class:`ProfileRecordSummary`; live label lookups
        use :func:`~cadrumo.application.workflow.read_profile_bucket`.
        """
        buckets_root = self._root / self._storage.buckets_dirname
        if not buckets_root.is_dir():
            return ()
        summaries: list[ProfileRecordSummary] = []
        for entry in buckets_root.iterdir():
            if not entry.is_dir():
                continue
            try:
                paths = self._storage.bucket_paths(self._root, entry.name)
            except ValueError:
                _log.debug(
            ...
            try:
                manifest = self._storage.read_manifest(paths)
            except (self._storage.storage_validation_error_type, ValidationError, OSError) as exc:
    ...
    def _refuse_create_onto_registered_bucket(self, resolved_id: str, paths: Any) -> NoReturn:
    ...
    def _lifecycle_repository(self, profile_id: str) -> ProfileRecordRepository:
        ...
        """
        return ProfileRecordRepository(bucket_id=profile_id, objects=self._secure_objects)

    def _lifecycle_service(self, profile_id: str) -> ProfileRecordLifecycle:
    ...
    def _ensure_bucket_directory(self, profile_id: str) -> None:
        ...
        if not paths.bucket_dir.exists():
            self._storage.provision_bucket_directory(self._root, profile_id)
            return
    ...
    def _remove_bucket_directory(self, profile_id: str) -> None:
        ...
        """
        target = self._storage.bucket_paths(self._root, profile_id).bucket_dir
        if not target.exists():
            return
        self._storage.trash_rename_and_remove(target)


__all__ = ["ProfileRecordAggregateRepository", "ProfileRecordSummary"]
