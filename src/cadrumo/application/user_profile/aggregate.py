...
type ProfileRestoreAuthority = Literal["password"]
...
published by a restore, but not what authorised it. ``password`` means the
profile's own credential unwrapped its own envelope. A recovery code never
restores a capsule: it proves the enrolled wrapper of a COMMITTED capsule so a
lost passphrase can be replaced in place, which is a different door.
"""
