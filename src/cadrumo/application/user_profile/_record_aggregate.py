...
class ProfileAggregate(BaseModel):
    ...
    created_at: datetime
    # The concrete manifest KDF record is a persisted-format type owned by the
    # storage adapter.  The aggregate carries it without inspecting it; the
    # application storage port validates and writes the record at the boundary.
    kdf_params: Any
