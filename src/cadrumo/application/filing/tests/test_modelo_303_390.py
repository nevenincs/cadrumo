...
def test_modelo_390_export_produces_fichero_boe_from_real_registry(tmp_path: Path) -> None:
    ...
    ``test_testing_registry.test_unsupported_modelo_fails_at_registry_boundary``;
    this test instead pins the positive case now that 390 carries an
    ``export_layouts`` definition.
    """
    ...
    draft = build_draft(
        ...
        inputs={
            ...
            "iva.anual.repercutido.recargo.general": Decimal("1248.00"),
            "iva.anual.repercutido.recargo.reducido": Decimal("624.00"),
            "iva.anual.compensacion-ultimo-periodo-97": Decimal("0"),
            "iva.anual.compensacion-generada-ejercicio-no-97": Decimal("0"),
        },
