...
def _envelope(arrival: M303RegimenSimplificadoValueArrival) -> M303ExportApplicabilityEnvelope:
    return M303ExportApplicabilityEnvelope(
        ...
        regimen_simplificado_applicable=arrival.applicable,
        regimen_simplificado=arrival if arrival.applicable else None,
    )
...
def test_complete_rows_reach_exact_source_targets_before_unsupported_export(tmp_path: Path) -> None:
    ...
    with pytest.raises(FilingExportError, match="no complete export_layouts definition"):
        export_draft(
            ...
            schema_provider=provider,
            m303_applicability=_envelope(arrival),
        )
...
def test_missing_applicable_fact_refuses_before_unsupported_layout_or_target(tmp_path: Path) -> None:
    ...
    with pytest.raises(FilingExportError, match="applicable facts do not match the annual Orden"):
        export_draft(
            ...
            producer_snapshot=snapshot,
            schema_provider=provider,
            m303_applicability=_envelope(_arrival(applicable=True, omit_last_fact=True)),
        )
    assert not output.exists()
...
def test_invalid_producer_snapshot_refuses_before_unsupported_layout(tmp_path: Path) -> None:
    provider = _provider()
    draft, snapshot = _draft_and_snapshot(provider)
    invalid = snapshot.model_copy(update={"model_profile": object()})
    output = tmp_path / "m303-invalid-producer.txt"

    with pytest.raises(FilingExportError, match="filing producer snapshot is incomplete"):
        export_draft(
            draft,
            output_path=output,
            producer_snapshot=invalid,
            schema_provider=provider,
            m303_applicability=_envelope(_arrival(applicable=False)),
        )
    assert not output.exists()
