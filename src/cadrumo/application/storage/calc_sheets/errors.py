...
Every subclass inherits from :class:`cadrumo.core.errors.hierarchy.CadrumoError`; the
project error registry's ``__init_subclass__`` hook binds each subclass
to its declared :class:`cadrumo.core.errors.error_codes.ErrorCode` row at import time.
"""
...
class CalcSheetsRecordError(CoreValidationError):
    ...
    Inherits from :class:`~cadrumo.core.errors.hierarchy.CoreValidationError` so
