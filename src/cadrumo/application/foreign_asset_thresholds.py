...
def foreign_asset_declaration_thresholds_for_parameters(
    *,
    ...
    catalogue = resolve_foreign_asset_obligation_catalogue(effective_date=filing_date)
    groups = tuple(sorted(catalogue.groups_established_by_legal_refs(legal_refs), key=lambda group: group.value))
    if not groups:
