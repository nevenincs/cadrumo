...
class TestAbortReasons:
    ...
    def test_draft_validation_next_action_remains_locale_neutral(
        self,
        language: str,
    ) -> None:
        """The real engine keeps persisted next-action diagnostics locale-neutral."""
        fx = _fixtures()
        fx.draft = _ConcreteDraft(
            findings=(ModeloFinding(severity=BaseSeverity.ERROR, message="blocking rule violated"),),
        )
        fx.draft_builder.draft = fx.draft
    ...
    def test_draft_build_refusal_next_action_remains_locale_neutral(
        self,
        language: str,
    ) -> None:
        """A real builder refusal persists canonical diagnostics under every locale."""
        fx = _fixtures()
