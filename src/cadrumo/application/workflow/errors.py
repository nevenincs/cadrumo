...
All workflow errors inherit from :class:`core.errors.hierarchy.CadrumoError` per
...
:class:`application.workflow.run_models.WorkflowResult`. Callers who want
...
class WorkflowComponentError(WorkflowError):
    ...
    context on the surrounding :class:`application.workflow.run_models.WorkflowStep`, and
    lets the workflow abort with
    :attr:`application.workflow.abort.WorkflowAbortReason.UNHANDLED_EXCEPTION`.
    """
...
class WorkflowAbortedError(WorkflowError):
    ...
    :class:`application.workflow.run_models.WorkflowResult` whose ``aborted_reason`` is set.
...
class WorkflowInputMismatchError(CoreValidationError):
    ...
    :class:`application.modelo.workflow_gate._RevisionInputsProvider`
...
class WorkflowAbortSignalError(WorkflowError):  # internal control-flow signal, not a public error type
    ...
    — it never propagates outside :class:`application.workflow.engine.WorkflowEngine`.
