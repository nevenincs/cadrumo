...
def modelo_202_incomplete_modality_finding(
    *,
    work_unit: WorkUnit,
    profile: TaxpayerProfile,
...
def _raise_if_modelo_202_modality_incomplete(
    *,
    ...
    if profile is None:
        return
    finding = modelo_202_incomplete_modality_finding(work_unit=work_unit, profile=profile)
    if finding is None:
        return
...
def cross_period_verification_declarations(
    ...
    **kwargs: object,
    ...
    # TODO(fact-relocation): resolve cross-period verification declarations from selected registry revisions
    return ()
