...
def _m202_advisory_predicate(revision_id: str) -> VerificationPredicateDefinition:
    ...
    revision = compiled_bundled_authority().validate_modelo("202").revisions[revision_id]
...
def test_m202_advisory_silent_when_no_resultado_contable() -> None:
    ...
    for revision_id in _M202_REVISION_IDS:
        ...
        assert evaluate_verification_predicates((predicate,), negative, workflow_profile()) == [], revision_id
...
def _m202_2025_predicate(predicate_id: str) -> VerificationPredicateDefinition:
    ...
    revision = compiled_bundled_authority().validate_modelo("202").revisions["2025-y-siguientes"]
...
def test_m202_2025_b2_tramo_advisory_holds_trivially_when_base_not_used() -> None:
    ...
    for predicate_id, antecedent, consequent in _M202_B2_TRAMO_CASES:
        ...
        assert evaluate_verification_predicates((predicate,), explicit_zero, workflow_profile()) == [], predicate_id
        assert evaluate_verification_predicates((predicate,), absent, workflow_profile()) == [], predicate_id
