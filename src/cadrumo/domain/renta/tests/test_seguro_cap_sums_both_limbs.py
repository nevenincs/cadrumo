...
def _cap(*, operation: PinnedAuthorityOperation, **counts: int) -> Decimal | None:

    return _resolve_statutory_cap(
        rule=_rule(operation=operation),
...
def test_the_cap_sums_each_limb_over_its_own_population(
    counts: dict[str, int], expected: Decimal, note: str, *, operation: PinnedAuthorityOperation
) -> None:

    assert _cap(**counts, operation=operation) == expected, note


def test_the_mixed_household_is_a_thousand_euros_above_the_old_flat_shape(
    *, operation: PinnedAuthorityOperation
    ...
    lawful = _cap(general=2, discapacidad=1, operation=operation)
...
def test_uncounted_populations_fall_back_to_the_ordinary_limb_not_to_zero(
    *, operation: PinnedAuthorityOperation
) -> None:
...
def test_a_count_naming_a_variant_the_rule_does_not_declare_is_refused(*, operation: PinnedAuthorityOperation) -> None:
