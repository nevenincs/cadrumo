...
def authority_operation() -> Iterator[PinnedAuthorityOperation]:
    """Resolve activity selectors inside one generation-pinned authority operation."""
...
def test_selector_resolution_refuses_a_non_entity_set_fact(authority_operation: PinnedAuthorityOperation) -> None:
    """A selector facade cannot reinterpret a fact from another payload family.

    The selector catalogue resolves through the real authority, so the refusal
    under test is the entity-set narrowing rather than the catalogue lookup.
    """
