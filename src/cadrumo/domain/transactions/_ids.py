...
The ``TransactionId`` constraint shape lives in :mod:`cadrumo.core.identity`
because the alias is consumed by code in a sibling domain
(:mod:`cadrumo.domain.invoices`) in addition to the owning transaction
...
re-routing through ``cadrumo.core.identity`` at every consumer site.
"""
