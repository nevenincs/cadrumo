...
class Attachment(BaseModel):
    ...
    The manifest also records the originating channel (:class:`domain.attachments.enums.AttachmentSource`),
    the document kind (:class:`domain.attachments.enums.AttachmentKind`), and
    ...
    Attributes:
        attachment_id: 64-character lowercase hex SHA-256 digest. Equals
            :attr:`sha256`.
        kind: Document kind. See :class:`domain.attachments.enums.AttachmentKind`.
        source: Channel the bytes were captured from. See
            :class:`domain.attachments.enums.AttachmentSource`.
