...
rounds with :func:`cadrumo.core.money.rounding.round_to_cents`, and raises
...
def dt12_regime_window_eligibility(
    *,
    ...
    - Contingencia in 2011–2014: eligible "hasta la finalización del octavo
      ejercicio siguiente", i.e. ``contingencia_year <= rescate_year <= contingencia_year + 8``.
    - Contingencia in 2010 or earlier: eligible "hasta el 31 de diciembre de
      2018", i.e. ``contingencia_year <= rescate_year <= 2018``.

    A rescate perceived before the contingencia year is never eligible: the
    window opens with the contingency.

    Raises:
        PensionReduccionError: When either year is not a plausible calendar year.
    """
