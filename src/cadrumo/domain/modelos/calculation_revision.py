...
selection â€” every consumer (year-aggregation, amendment delta,
...
* ``BORRADOR`` â€” newly calculated; mutable in the sense that
  re-running ``calculate`` creates a new revision rather than
  editing this one. Multiple borradores can coexist.
* ``VERIFICADO_COMPLETO`` â€” ``verify`` ran cleanly: all required
...
* ``PRESENTADO`` â€” paired with a :class:`ModeloRecord`. The revision
  ...
  any time.
* ``PRESENTADO_SUPERSEDIDO`` â€” a later verified revision was filed
  against the same tuple. The revision and its filing record remain
  in the audit trail.
* ``DESCARTADO`` â€” operator abandoned the revision before filing.
...
class CalculationSourceRef(BaseModel):
    ...
    exactly the resolverâ†’source-objectâ†’fingerprint trace that lets an audit reader
    ...
    Attributes:
        ...
        dependency_treatment: The registry's declared dependency treatment for
            ...
            elsewhere on the revision â€” it is the sole persisted trace of whether
...
class CalculationRevision(BaseModel):
    ...
    # resolverâ†’source-objectâ†’fingerprint trace projected
    ...
    # values (M184 atribuciÃ³n members, M232 operaciones vinculadas,
