...
def test_the_key_carries_the_fingerprints_the_body_reads() -> None:
    ...
    assert fingerprint_key(fingerprints).fingerprints == fingerprints
...
def test_the_key_digest_uses_the_canonical_framed_content_hash() -> None:
    ...
    assert fingerprint_key(fingerprints).digest == content_hash_hex(_fingerprint_key_payload(fingerprints))
    assert len(fingerprint_key(fingerprints).digest) == 64
...
def test_the_hash_is_the_digest_hash_and_not_the_corpus_hash() -> None:
    ...
    small = fingerprint_key(_fingerprints(1))
    large = fingerprint_key(_fingerprints(5000))
...
def test_equal_corpora_produce_one_cache_entry() -> None:
    ...
    first = fingerprint_key(_fingerprints(64))
    second = fingerprint_key(_fingerprints(64))
...
def test_any_corpus_change_changes_the_key(
    ...
    original = fingerprint_key(rows)
    changed = fingerprint_key(mutate(rows))
