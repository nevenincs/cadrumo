"""Calc-verify: the IRPF art.66 ahorro-base estatal escala resolves correctly.

The escala estatal del ahorro is registered as a ``bracket_table``
ParameterDefinition ``renta-{year}-escala-estatal-base-ahorro`` under
each ejercicio, with ``bracket_axis = "filing_period"`` and
``legal_refs = ["ley-35-2006:art-66"]``. The runtime computes
...
are transcribed from external AEAT authority:
  ...
  - The "Incremento en cuota íntegra estatal (euros)" column published
    in the AEAT Manual práctico de Renta, section "Gravamen de la base
    liquidable del ahorro — Gravamen estatal — Normativa: Art. 66.1
    ...
    text of art.66 Ley 35/2006 (BOE-A-2006-20764).

  - The worked example on AEAT Manual Renta 2025 Parte 1, page 954
    ("Don A.B.C., residente en Aragón"): base liquidable del ahorro
    2.800 EUR, mínimo personal y familiar absorbed in full by the
    general base. Manual: "Gravamen estatal 2.800 x 9,50% = 266".
...
15% effective 2025. Each year's brackets are grounded against that
year's AEAT manual.
"""
...
def test_ahorro_escala_2020_top_bracket_is_open_11_5_percent() -> None:
    """Pre-Ley 11/2020 the 2020 estatal ahorro escala had only three
...
def test_ahorro_escala_2025_top_marginal_rate_is_15_percent() -> None:
    """Ley 7/2024 raised the top estatal ahorro marginal rate to 15%
...
def test_ahorro_escala_2023_2024_top_marginal_rate_is_14_percent() -> None:
    """Ley 31/2022 set the >300.000 estatal ahorro tier at 14% for
...
def test_ahorro_escala_2021_2022_top_marginal_rate_is_13_percent() -> None:
    """Ley 11/2020 set the open >200.000 estatal ahorro tier at 13% for
