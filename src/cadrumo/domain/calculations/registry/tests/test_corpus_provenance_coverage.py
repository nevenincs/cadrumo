...
#: Name fragments marking a file as a generated derivative of the payload it
#: sits beside rather than an independently sourced document.
_DERIVATIVE_MARKERS = (".extracted.json", ".extracted.md")
...
_NON_PAYLOAD = frozenset({"PROVENANCE.md", "manifest.json", "README.md"})
...
def _manifest_stored_paths(subdir: Path) -> frozenset[str]:
    ...
    return frozenset(
        ...
        if (located := artefact.get(key)) is not None
    )
...
def test_the_payload_enumeration_reaches_every_corpus_category() -> None:
    ...
    every_category = {
        entry.name for entry in scan_directory(root, select=DirectoryEntryKind.DIRECTORIES) if any(iter_directory(entry))
    }
...
def test_both_committed_manifest_artefact_location_keys_are_honoured() -> None:
    ...
    manifests = scan_directory(
        _aeat_root(), pattern="manifest.json", recursive=True, select=DirectoryEntryKind.FILES
    )
