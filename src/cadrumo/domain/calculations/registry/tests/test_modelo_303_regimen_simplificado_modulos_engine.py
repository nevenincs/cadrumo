...
def test_all_source_registry_runtime_callers_state_explicit_intent() -> None:
    """Every direct source caller states scope; M303 tests cannot blanket-pass ``None``."""
    source_root = Path(__file__).parents[4]
    omissions: list[str] = []
    unintentional_m303_none: list[str] = []
    direct_call_count = 0
    allowed_none = {
        (
            "domain/calculations/registry/tests/test_modelo_303_regimen_simplificado_modulos_engine.py",
            "test_missing_scope_blocks_any_m303_calculation",
        ),
    }
    ...
    for source in source_root.rglob("*.py"):
        ...
        for node in ast.walk(tree):
            ...
            m303_focused = "m303" in relative_path.lower() or "modelo_303" in enclosing.name.lower()
            explicit_none = isinstance(scope_keywords[0].value, ast.Constant) and scope_keywords[0].value.value is None
...
def test_registry_runtime_signature_has_no_scope_default_or_omission_escape() -> None:
