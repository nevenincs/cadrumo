...
def _minimal_source_ref() -> SourceReference:
    return SourceReference(
        ...
        corpus_path="registry/aeat/sources/aeat-dr-130-2019-v12.pdf", sha256="a" * 64, bytes=1024,
...
def _minimal_catalogues() -> RegistryCatalogues:
    ...
    workbook = source.model_copy(update={"id": _WORKBOOK_SOURCE_ID, "evidence_tier": "layout_authority", "kind": "record_design"})
...
def _minimal_revision(
    *, bindings: tuple[DataBindingDefinition, ...] = (), export_layouts: tuple[ExportLayoutDefinition, ...] = ()
    ...
    return ModeloRevision(
        ...
        reviewed_at=date(2026, 7, 1), authority_grade=RegistryAuthorityGrade.FILING,
        ...
        casillas=(CasillaDefinition(
            id=validated_casilla_id("01", surface="artifact_runtime_fixture"), number="01",
            localization_keys=("test.schema.casilla.label",), section=("test",), input_kind=InputKind.MANUAL,
            legal_refs=(_LEGAL_ID,), source_refs=(_SOURCE_ID,),
        ),),
        workbook_parity_refs=(WorkbookParityReference(
            id="wp.test", workbook_source=_WORKBOOK_SOURCE_ID, fixture_id="test", formula_coverage="static_layout",
            runner_required=False, tolerance=0, legal_refs=(_LEGAL_ID,), source_refs=(_WORKBOOK_SOURCE_ID,),
        ),),
        ...
        bindings=bindings, export_layouts=export_layouts,
    )
