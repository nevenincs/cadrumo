...
from ..record_design_pdf_rows import split_glued_ordinal_position
...
def test_a_glued_row_is_split_when_both_constraints_agree() -> None:
    row = split_glued_ordinal_position(
...
def test_the_split_is_refused_when_the_ordinal_does_not_continue() -> None:
    ...
    assert split_glued_ordinal_position("23 3 Num Modelo", 10, previous=_previous("7", 1, 2)) is None
...
def test_the_split_is_refused_when_the_position_does_not_resume() -> None:
    ...
    assert split_glued_ordinal_position("23 3 Num Modelo", 10, previous=_previous("1", 1, 5)) is None


def test_a_row_that_already_parses_is_never_resplit() -> None:
    assert split_glued_ordinal_position("2 3 3 Num Modelo", 10, previous=_previous("1", 1, 2)) is None


def test_the_first_row_of_a_record_is_never_split() -> None:
    """With no previous row there is nothing to close the constraint."""
    assert split_glued_ordinal_position("23 3 Num Modelo", 10, previous=None) is None
