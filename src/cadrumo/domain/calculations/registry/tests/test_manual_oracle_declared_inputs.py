...
_UNMIGRATED_PAYLOADS: Mapping[str, str] = {
    ...
    "modelo-303-2025-prorrata-general-regularizacion.json": (
        "MIGRATABLE, waiting only on extraction verification. Not ledger-driven, unlike its five IVA siblings, and the 2025 IVA manual now carries extracted text, so its locators are constructible. Blocked only until that extraction has been read back against the pdf for the pages this payload cites -- a locator into an unverified extraction claims a precision it may not have. Its two consumers, test_prorrata_regularizacion_oracle.py and test_prorrata_regularizacion_source_resolver.py, must move together"
    ),
