"""Public-surface stability guard for :mod:`cadrumo.domain.justificante`.

The rehoming of :class:`JustificanteError` under
:class:`cadrumo.adapters.inbound.pdf.PdfModeloImportError` must not accidentally prune
...
The function now lives in :mod:`cadrumo.adapters.outbound.aeat.verify` because
...
def test_justificante_public_surface_has_every_frozen_symbol() -> None:
    ...
    assert not missing, f"cadrumo.domain.justificante.__all__ is missing frozen symbols: {sorted(missing)}"
