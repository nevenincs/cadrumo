"""Resolve which Member State a party is VAT-IDENTIFIED in — never where it is established.
...
party operates under a German VAT identification, and — the reader's inference —
...
**Spain cannot arise from a printed VAT number here**, and that is a property of
...
__all__ = ["vat_identification_state_for_printed_tax_identifier"]


def vat_identification_state_for_printed_tax_identifier(
    printed_identifier: str | None,
) -> EUMemberState | None:
    """Return the Member State a printed VAT number identifies the party in.
    ...
    Returns:
        ...
        in, or ``None`` when no VAT number was recognised.
