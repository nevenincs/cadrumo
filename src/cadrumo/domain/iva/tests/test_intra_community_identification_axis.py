...
def test_an_es_prefix_over_a_body_that_fails_the_checksum_states_nothing(printed: str) -> None:
    ...
    """
    with _indexed_authority_for_test().operation() as _authority_operation_for_test:
        assert identification_state_for_printed_tax_identifier(printed, operation=_authority_operation_for_test) is None
...
def test_the_checksum_is_what_admits_the_spanish_number() -> None:
    ...
    with _indexed_authority_for_test().operation() as _authority_operation_for_test:
        ...
        assert (
            identification_state_for_printed_tax_identifier("ESFRANCISCO", operation=_authority_operation_for_test)
            is None
        )
        assert identification_state_for_printed_tax_identifier(
            _SPANISH_IVA, operation=_authority_operation_for_test
