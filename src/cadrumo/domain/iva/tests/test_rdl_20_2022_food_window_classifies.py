...
def test_the_five_percent_food_rate_classifies_across_its_whole_grounded_span(on_date: date) -> None:
    ...
    assert rate_kinds_for_declared_rate(EUMemberState._from_registry("es"), _FIVE, on_date) == (
        IvaRateKind("reduced"),
    ), (
...
def test_the_successor_rate_still_holds_its_own_window() -> None:
    ...
    assert rate_kinds_for_declared_rate(EUMemberState._from_registry("es"), seven_five, date(2024, 8, 1)) == ()
    assert rate_kinds_for_declared_rate(EUMemberState._from_registry("es"), seven_five, date(2024, 11, 1)) == (
        IvaRateKind("reduced"),
    )
