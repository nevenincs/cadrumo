...
def test_the_full_configured_range_is_not_a_replacement() -> None:
    ...
    """
    healthy = [f"gw{ordinal}" for ordinal in range(DEFAULT_WORKER_COUNT)]
    assert replacement_occurred(healthy, worker_count=DEFAULT_WORKER_COUNT) is False
...
def test_the_first_ordinal_past_the_range_is_a_replacement() -> None:
    ...
    and fails only here.
    """
    observed = [f"gw{ordinal}" for ordinal in range(DEFAULT_WORKER_COUNT)]
    observed.append(f"gw{DEFAULT_WORKER_COUNT}")
    assert replacement_occurred(observed, worker_count=DEFAULT_WORKER_COUNT) is True


def test_no_observed_workers_reports_no_replacement() -> None:
    """A run that observed no workers has no replacement to report."""
    assert replacement_occurred([], worker_count=DEFAULT_WORKER_COUNT) is False
...
def test_an_unreadable_worker_id_is_refused_rather_than_skipped(malformed: str) -> None:
    ...
    with pytest.raises(ValueError, match="Unrecognised xdist worker id"):
        replacement_occurred([malformed], worker_count=DEFAULT_WORKER_COUNT)
...
def test_each_wrong_detector_is_caught_by_the_assertions_above() -> None:
    ...
    """
    boundary_id = f"gw{DEFAULT_WORKER_COUNT}"
    ...
    assert off_by_one([boundary_id], worker_count=DEFAULT_WORKER_COUNT) is False
    assert replacement_occurred([boundary_id], worker_count=DEFAULT_WORKER_COUNT) is True

    # Always-clean: the classic decoration, green on every input.
    def always_clean(worker_ids: list[str], *, worker_count: int) -> bool:
        return False

    assert always_clean([boundary_id], worker_count=DEFAULT_WORKER_COUNT) is False
    ...
    assert skips_unreadable(["worker1"], worker_count=DEFAULT_WORKER_COUNT) is False
    with pytest.raises(ValueError, match="Unrecognised xdist worker id"):
        replacement_occurred(["worker1"], worker_count=DEFAULT_WORKER_COUNT)
