...
Testing is **pytest-only**. `unittest`, `unittest.mock`, and the third-party `mock` package are banned globally (ruff `TID251`). Unit tests may use real fakes or pytest fixtures; **live tests must never contain mocks, patches, fakes, or stubs**, and `tests/conftest.py` enforces an extended banned-import set in any file that declares a `@pytest.mark.live` item (including `pytest_mock`, `pytest_httpx`, `time_machine`, `freezegun`, `vcr`). Every test must carry exactly one of `@pytest.mark.unit` or `@pytest.mark.live` — missing or duplicate markers fail collection. Unit tests live inside each module's directory (Rust-style colocated tests). Live tests are skipped by default; opt in with `AEAT_LIVE_TESTS_ENABLED=1`. Google Workspace live tests additionally require `AEAT_LIVE_TESTS_GOOGLE=1` and project-owned fixtures provisioned via `just google-fixtures-provision` — see `scripts/README.md`. Coverage is measured against `src/aeat` with a 60% floor via `just test-cov`. The authoritative testing reference is [`tests/README.md`](tests/README.md); decisions are recorded in `.vault/adr/2026-04-17-pytest-only-testing-adr.md`.
