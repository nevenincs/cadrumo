...
## 2026-09-22 Ledger return consumed and IVA integration resumed

- IVA directly read and acknowledged `handoffs/2026-09-22-ledger-shared-file-return.md` on the shared `tui/modelo` worktree. Direct coordinator forwarding is no longer treated as a prerequisite. Returned integration commit `01b78c102116`, Ledger delivery commit `d2858767ea`, returned contracts and wheel identities are those in that handback. The file retains no Ledger writer reservation over its completed slice; income/TUI reservations are separate. No active IVA process was inherited. Ledger's 15 shared boundary, 32 IVA manual/category, 19 runner-caller checks and installed receipts belong to their original source/package identities, not relabelled to current HEAD. Import-boundary gate remains red/unavailable under quality ownership.
- Exact returned-source IVA checks on branch `tui/modelo`: `uv run --no-sync pytest -q -n0 -m unit src/cadrumo/application/ledger/tests/test_manual_iva_category_membership.py src/cadrumo/entrypoints/tui/ledger/tests/test_ledger_classification_iva.py` passed 8 (6 deselected), log `20260922T043217.031427Z-pytest-15508-b4550ca1/run.log`; the same paths with `-m integration` passed 6 (8 deselected), log `20260922T043223.830668Z-pytest-40908-a4ebe1bf/run.log`. The exact installed TUI capture/classification/reopen node in `dev/acceptance/iva/tests/test_installed_tui_capture_reopen.py` passed 1, exit 0, 75.90 s, log `20260922T043430.175430Z-pytest-20384-32b420ca/run.log`; it proves the installed ledger slice, not TUI Modelo 303 calculation. These are new checks, not a repeated full CLI lane. No category/classification regression was observed.
- A peer-authored new disposition was appended to `handoffs/2026-09-22-iva-modelo-evidence-contract.md` on the shared worktree after source `81cf71d1db`: typed four-input existing-operation extension is accepted with a named IVA-only lease over the calculate operation, composition, Modelo TUI lifecycle/overview and focused tests. It directs `SECURE_REFERENCE` request storage/sensitive policy, public request schema versioning and explicit old-pending refusal. The older trailing communication bullets still say lease not received and must be reconciled by that file's owner; IVA does not edit the peer-dirty handoff. Terra IVA calculate core owns operation/composition, Terra IVA TUI evidence owns lifecycle/overview, with no overlap; lead owns integration/acceptance. Other peers' concurrent `dev/acceptance/iva` edits are preserved and not staged by IVA. No installed Modelo 303 TUI result is claimed yet.
- Luna's bounded current public invoice trace found CLI `invoice add --line` supports multiple ordered structured lines with public canonical line readback; TUI `ledger/invoice_entry.py` and `ledger/models.py` expose only one scalar base/rate input, and `ledger/record_views.py` omits line-level detail. The invoice observation screen iterates persisted lines, but M303 binding remains transaction-ledger-owned with invoice/transaction consistency screening. Thus CLI multi-line support is not TUI multi-line capture or M303 parity. Next disjoint owner must be assigned only `ledger/invoice_entry.py`, `ledger/models.py` and focused TUI invoice tests after checking no Ledger writer reservation; no alternate invoice representation or duplicated aggregation is authorized.
- Terra IVA calculate core was assigned only the existing operation/composition and focused tests. Agreed request API: `ModeloWorkCalculateOrdinaryM303EvidenceRequestV1` with two explicit booleans plus two `Hex64Str` secure attachment coordinates, nested as `ModeloWorkCalculateRequest.ordinary_m303_filing_evidence`; executor derives period/bucket/authority and uses existing M303 authoring. Terra IVA TUI evidence was assigned only lifecycle/overview/new evidence UI and focused tests. It implemented lifecycle typed pass-through and two narrow tests; its proposed modal is held because nine distinct locale keys are absent and locale catalogs are outside its lease. Lead requested a narrow locale lease asynchronously; no answer/locale changes yet. No core/TUI check or installed calculation result claimed in this line.

## 2026-09-23 ordinary-M303 evidence contract landed; lease returned

- The commits, checks and returned surfaces are recorded once in `handoffs/2026-09-23-iva-modelo-evidence-lease-return.md`; this entry records only checkpoint state. HEAD at this entry: `976971d1be`.
- Installed runs 1–9 of `dev/acceptance/iva/installed_m303_evidence_journey.py` were rehearsal-only (wheels from a copied temp tree at `aacf15b688` plus uncommitted edits, wheel r1 `7db07690…`, r5 `e9873a90…`); none is acceptance evidence. They found and drove fixes for: wall-clock admission, the dotted-casilla BadIdentifier (`3e824a2785`), the missing failure receipt, refresh-generation races, the INTEGRITY-classified attestation refusals (`c0063d730a`) and the Results gap below. Run 9 proved the TUI-led calculate child only.
- `63a201418e` also carries CALENDAR-01's concurrently staged S06 paths (facts 0066/0067, `legal/dias-inhabiles-age.toml`, BOE captures, two tests, calendar ledger/plan). A plain `git commit` swept them in; they remain CALENDAR's. History was not rewritten; reported to CALENDAR and ADMIN. IVA now commits only with `git commit -- <paths>`.
- Finding (assigned to IVA by ADMIN as a scoped feature, sequenced after monthly 303): the Modelo workspace Results, Inputs and Verification destinations can never show values in production. `application/modelo/workspace.py` assembles only STATIC_INSPECTION with the constant UNMEASURED `STATIC_INSPECTION_WORK_REVIEW_FACET`; `_workspace_model_validation.require_static_projection_scope` forbids an AVAILABLE review there, and no graded-snapshot admission exists in `src/`, although the accepted tui-registry-api-gate decision specifies one. The installed journey therefore proves the fresh-TUI readback as "revision listed current and `verificado_completo`" and records the numeric Results readback as unexercised, never as a pass. CALENDAR owns workspace static-inspection performance (`0901ef1134`); coordinate one writer on `workspace.py`.
- Pre-existing failures, not introduced by IVA: `entrypoints/tests/test_operation_composition.py` (4), `adapters/persistence/operations/tests/test_workspace_refresh_target_resolution.py` (3), `core/errors/tests/test_exception_base_hygiene.py` (2, actividad_asset and withholding modules), `core/errors/tests/test_registry_enforcement.py` (1, unregistered `AuthorityStoreFormatError` from the v2 cutover). Locale audit: `missing=25` per locale, all `sheets.detalle.headers.*` from `application/storage/calc_sheets/engine.py` (reported to ADMIN for an owner). Import gate loadability: peer-dirty `dev/acceptance/income_tax/cli_journey.py`, assigned by ADMIN to the PROFILE/quality brief.
- Open IVA targets: final installed proof on a wheel built from `976971d1be` (running); monthly Modelo 303 (grounding in progress); graded-snapshot workspace admission; T2 TUI multi-line invoice capture; T4 matrix V3/V4/V6/V8/V9/V10/V11; temp cleanup to be surfaced to the user.
- Temp cleanup, 2026-09-23 22:00 (coordinator-authorized): the clean worktrees `iva01-final` and `iva01-proof/source` were removed with plain `git worktree remove`. `iva01-baseline` (detached at `aacf15b688`) was removed with `--force` only after a byte-level check. 12 of its 14 modified files matched a committed blob (5 at HEAD; 7 at `c0063d730a` or `58523f3fa7`). The two that matched no committed blob were discarded as superseded rehearsal copies: `entrypoints/tui/modelo/view/overview.py` (the pre-fix, non-injective `edit_control_id`, replaced in `3e824a2785`) and `dev/acceptance/iva/installed_m303_evidence_journey.py` (a pre-`ReopenReadback` draft with the removed `pattern`/`brief` receipt fields, superseded by `056513f0c1`). 30 non-git synthetic leftovers (proof and rehearsal venvs, wheels, run stores, logs, gate output) were deleted by exact path, each verified to be inside `%TEMP%\claude\` and to contain no reparse point. Kept: `iva01-slot` and its current diagnostic log.
- 2026-09-23 22:45 state: HEAD includes `a5eb4dc920` (S01–S05 of `2026-09-23-iva-workflow-plan`) and its vault close. Remaining: S07 docs sequence goldens (contracts and seed edited, uncommitted; refresh held for host memory), S06 installed proof (queued for the next installed-run slot), then the graded-snapshot workspace feature, T2 TUI multi-line invoice capture and the T4 matrix items. The seven pre-existing failures reported earlier are fixed by the coordinator in `1aa9828a79`.
